...
class _selectorItem extends Modul_Controller
{
    ...
    public function selectItem()
    {
        ...
        $selectorFiltersAdditional = isset($this->configUi[$jenisTr]['selectorFiltersAdditional']) ? $this->configUi[$jenisTr]['selectorFiltersAdditional'] : array();
        ...
        if(sizeof($selectorFiltersAdditional)>0){
            foreach ($selectorFiltersAdditional as $fff){
                $o->addFilter($fff);
            }
        }
                ...
                if (sizeof($selectorFields) > 0) {
                    ...
                    foreach ($selectorFields as $f) {
                        ...
                        if ($f != 'kode' && is_numeric($row->$f)) {
                            ...
                            $newFields = in_array($f, arrAvailFields()) ? formatNota($f, $row->$f) : (isset($row->$f) ? $row->$f : "");
