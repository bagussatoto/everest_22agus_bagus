...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        //-------------------------
        $pairedItemBreakDownValidator = isset($this->configUi[$this->jenisTr]["shoppingCartPairedItemBreakDownValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartPairedItemBreakDownValidator"] : array();
        $pairedItemBreakDownPartValidator = isset($this->configUi[$this->jenisTr]["shoppingCartPairedItemBreakDownPartValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartPairedItemBreakDownPartValidator"] : array();
        //-------------------------
        $pairedItemPotongValidator = isset($this->configUi[$this->jenisTr]["shoppingCartPairedItemPotongValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartPairedItemPotongValidator"] : array();
        $pairedItemPotongPartValidator = isset($this->configUi[$this->jenisTr]["shoppingCartPairedItemPotongPartValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartPairedItemPotongPartValidator"] : array();
        //-------------------------
            ...
            if (sizeof($source) > 0) {
                foreach ($source as $pID => $pSpec) {
                    ...
                    if (!array_key_exists($pID, $target)) {
                        $errMsgs[] = "hasil $label " . $pSpec['name'] . " belum dipilih. silahkan dipilih dahulu.";
                    }
        ...
        if (sizeof($pairedItemBreakDownValidator) > 0) {
            if (isset($pairedItemBreakDownValidator[1]["enabled"]) && ($pairedItemBreakDownValidator[1]["enabled"] == true)) {
                if (isset($_SESSION[$cCode]["items"]) && sizeof($_SESSION[$cCode]['items']) > 0) {
                    foreach ($_SESSION[$cCode]["items"] as $id => $spec) {
                        ...
                        if (isset($_SESSION[$cCode]["items4"][$id])) {
                            foreach ($_SESSION[$cCode]["items4"][$id] as $sid => $iiSpec) {
                                ...
                                if($iiSpec["sub_hpp_avg"] == 0){
                                    $nama = $iiSpec["nama"];
                                    $msg = "hpp $nama hasil konversi produk $produk_nama_items sebesar 0. silahkan diperiksa lagi. code: " . __LINE__;
                                    mati_disini($msg);
                                }
                            }
                        }
...
//                            $msg .= "[$sub_hpp_avg_items4] [$sub_hpp_avg_items]";
        ...
        if (sizeof($pairedItemBreakDownPartValidator) > 0) {
            if (isset($pairedItemBreakDownPartValidator[1]["enabled"]) && ($pairedItemBreakDownPartValidator[1]["enabled"] == true)) {
                if (isset($_SESSION[$cCode]["items"]) && sizeof($_SESSION[$cCode]['items']) > 0) {
                    foreach ($_SESSION[$cCode]["items"] as $id => $spec) {
                        $produk_nama_items = $spec["nama"];
                        if (sizeof($_SESSION[$cCode]["items4"][$id]) == 0) {
                            $msg = "Unit produk $produk_nama_items terdeteksi part tidak lengkap. Silahkan diperiksa lagi.";
                            mati_disini($msg);
                        }
                    }
                }
        ...
        // konversi potong
        if (sizeof($pairedItemPotongValidator) > 0) {
            if (isset($pairedItemPotongValidator[1]["enabled"]) && ($pairedItemPotongValidator[1]["enabled"] == true)) {
                if (isset($_SESSION[$cCode]["items"]) && sizeof($_SESSION[$cCode]['items']) > 0) {
                    foreach ($_SESSION[$cCode]["items"] as $id => $spec) {
                        $produk_nama_items = $spec["nama"];
                        $sub_hpp_avg_items = $spec["sub_hpp_avg"];
...
//                            $msg .= "[$sub_hpp_avg_items4] [$sub_hpp_avg_items]";
        ...
        if (sizeof($pairedItemPotongPartValidator) > 0) {
            if (isset($pairedItemPotongPartValidator[1]["enabled"]) && ($pairedItemPotongPartValidator[1]["enabled"] == true)) {
                if (isset($_SESSION[$cCode]["items"]) && sizeof($_SESSION[$cCode]['items']) > 0) {
                    //cek sisa potongan
                    $arrSisaPotong = array();
                    foreach ($_SESSION[$cCode]["items"] as $id => $spec) {
                        $sisa_potong = $spec["sisa_dipakai"];
                        $prod_main_id = $spec["id"];
                        if($sisa_potong*1<>0){
                            $arrSisaPotong[$prod_main_id] = $spec;
                        }
                    }
                    $strSisa = "";
                    if(count($arrSisaPotong)>0){

                        $arrListProduk = array();
                        $tmpProduk = $this->db->get("produk")->result();
                        foreach($tmpProduk as $k => $rows){
                            $arrListProduk[] = $rows;
                        }

                        //mode autoselect
                        $autoSelectItems=array();
                        $belumPilihTarget=array();
                        $cloningProduk=array();
                        foreach($arrSisaPotong as $cid => $spec){
                            $sisa_potong = $spec["sisa_dipakai"];
                        $size_nama = $spec["size_nama"];
                            $sisa_potong = $spec['sisa_dipakai'];

                            $produk_part_jenis_id = $spec['produk_part_jenis_id'];
                            $produk_part_kategori_id = $spec['produk_part_kategori_id'];
                            $produk_part_ukuran_id = $spec['produk_part_ukuran_id'];
                            $merek_id = $spec['merek_id'];
                            $satuan_nilai = $spec['satuan_nilai'];
                            $pattern_A = count(explode(" ", $spec['nama']));

                            foreach($arrListProduk as $kk => $prow){
                                $pattern_B = count(explode(" ", $prow->nama));
                                if( $prow->produk_part_jenis_id == $produk_part_jenis_id &&
                                    $prow->produk_part_kategori_id == $produk_part_kategori_id &&
                                    $prow->produk_part_ukuran_id == $produk_part_ukuran_id &&
                                    $prow->merek_id == $merek_id &&
                                    $prow->satuan_nilai*1 == $sisa_potong*1 &&
                                    $pattern_B == $pattern_A
                                ){
                                    if($prow->satuan_nilai*1 != $satuan_nilai*1 ){
                                        $autoSelectItems[$cid][$prow->id] = (array)$prow;
                        }
                                    else{
                                        if($prow->id*1 == $cid*1){
                                            $belumPilihTarget[$cid][$prow->id] = (array)$prow;
                        }
                                    }
                                }
                            }
                        ...
                        $strSisa .= "<div class='box box-warning'>";
                        $strSisa .= "<div class='box-header with-border'>";
                        $strSisa .= "masih terdapat sisa potongan konversi.<br>Silahkan tentukan menjadi produk lain supaya konversi dapat dilanjutkan";
                        $strSisa .= "</div>";
                        $strSisa .= "<div class='box-body'>";
                        ...
                        $strSisa .= "<table class='table dataTable compact display'>";
                        $strSisa .= "<thead class='bg-gray'>";
                        $strSisa .= "<tr>";
                        foreach($arrTableHeader as $ky => $lab){
                            $strSisa .= "<th>$lab</th>";
                        }
                        $strSisa .= "<th class='text-center'>KET</th>";
                        $strSisa .= "</tr>";
                        $strSisa .= "</thead>";
                        $strSisa .= "<tbody>";
                        foreach($arrSisaPotong as $cid => $spec){
                            ...
                            foreach($arrTableHeader as $ky => $lab){
                                $strSisa .= "<td>".$spec[$ky]."</td>";
                            }

                            if(isset($belumPilihTarget[$cid])){
                                $strSisa .= "<td class='text-red text-bold text-center'>belum pilih produk target</td>";
                            }
                        else{
                                if(isset($autoSelectItems[$cid])){ //jika produk tujuan sesuai dengan satuan nilai
                                    $produk_sisa_nama = $autoSelectItems[$cid]['nama'];

                                    $strNamaProdukSisa = "";
                                    foreach($autoSelectItems[$cid] as $ppidsisa => $sisaRows){
                                        $strNamaProdukSisa .= "<div class='text-bold text-green'>" . $sisaRows['nama'] . "</div>";
                        }
                                    $strSisa .= "<td class='text-center text-bold'><i class='fa fa-check-square text-green text-bold'></i> PRODUK SISA READY<br>$strNamaProdukSisa</td>";
                                }
                                else{
                                    $strSisa .= "<td class='text-warning text-bold text-center'>Create Produk <b>($sisa_potong $size_nama)</b><br><span data-satuan='$sisa_potong' data-nama='$nama' data-id='$cid' class='btn btn-xs btn-danger cloning-btn'>COPY & EDIT DARI PRODUK ASAL</span></td>";
                                    $cloningProduk[$cid] = $spec;
                                }
                            }
                            $strSisa .= "</tr>";
                        }
                        ...
                        if(count($belumPilihTarget)>0){
                            $strSisa .= "<div class='callout callout-danger'>";
                            $strSisa .= "<h4 class='text-white'><i class='fa fa-warning'></i> TRANSAKSI BELUM BISA DILANJUTKAN!<BR>KEMUNGKINAN ADA SATU ATAU DUA PRODUK YANG BELUM MEMILIH PRODUK TUJUAN.</h4>";
                            $strSisa .= "</div>";
                    }
                        else{

                            if(count($cloningProduk)>0){
                                $strSisa .= "<div class='callout callout-danger'>";
                                $strSisa .= "<h4 class='text-white'><i class='fa fa-warning'></i> TRANSAKSI BELUM BISA DILANJUTKAN!</h4>";
                                $strSisa .= "<p>";
                                $strSisa .= "- ada 1 atau lebih produk yang belum memiliki produk target konversi.<br>";
                                $strSisa .= "- buat produk baru untuk menampung sisa pemotongan.<br>";
                                $strSisa .= "- untuk menambah produk baru, klik tombol merah pada setiap masing-masing produk.";
                                $strSisa .= "</p>";
                                $strSisa .= "</div>";
                }
                            else{
                                $strSisa .= "<div class='callout callout-info'>";
                                $strSisa .= "<h4 class='text-red'><i class='fa fa-warning'></i> Perlu diperhatikan!</h4>";
                                $strSisa .= "<p>";
                                $strSisa .= "- stok sisa akan masuk menjadi produk baru sesuai jumlah sisa.<br>";
                                $strSisa .= "- jika produk stok sisa belum ada, Anda akan di arahkan untuk membuat data produk.<br>";
                                $strSisa .= "- harap teliti saat penambahan produk Baru.";
                                $strSisa .= "</p>";
                                $strSisa .= "</div>";

                                $jsBolehLanjut .= "setTimeout( function(){ top.$('.btn-lanjutkan').removeClass('hidden') }, 500);";
            }
                        ...
                        if(count($autoSelectItems)>0){
                            foreach($autoSelectItems as $main_id => $row1){
                                foreach($row1 as $anak_id => $row){
//                                    $when .= "top.$('#result').load('".base_url()."konversi/_shoppingCart/recordPairedItemKonversi/1339/note?val=$anak_id&iid=$main_id'),";
                                    $when .= "top.$.get('".base_url()."konversi/_shoppingCart/recordPairedItemKonversi/1339/note?val=$anak_id&iid=$main_id'),";
                                }
                            }
                        }
                        ...
                        echo "<script>
                                ...
                                var arrSisaPotong    = atob('".base64_encode(json_encode($arrSisaPotong))."');
                                var autoSelectItems  = atob('".base64_encode(json_encode($autoSelectItems))."');
                                var belumPilihTarget = atob('".base64_encode(json_encode($belumPilihTarget))."');

                                top.BootstrapDialog.closeAll();
                                top.BootstrapDialog.show({
                                    title:'Kelola Sisa Potongan',
                                    message: top.$('<div></div>').html(\"$strSisa\"),
                                    draggable:true,
                                    size: top.BootstrapDialog.SIZE_WIDE,
                                    closable:true,
                                    animation: false,
                                    buttons: [{
                                        label: 'Buat & Lanjutkan',
                                        icon: 'fa fa-send',
                                        autospin: true,
                                        action: function(dialogRef){
                                            dialogRef.enableButtons(false);
                                            dialogRef.setClosable(false);
                                            var autoSelectItems_f = JSON.parse(autoSelectItems);
//                                            top.jQuery.each(autoSelectItems_f, function(pid_utama, b){
//                                                top.jQuery.each(b, function(pid_anak, row){
//                                                    top.$('#result').load('".base_url()."konversi/_shoppingCart/recordPairedItemKonversi/1339/note?val='+pid_anak+'&iid='+pid_utama)
//                                                });
//                                            })
//                                            top.document.getElementById('result').src='".base_url()."konversi/Transaksi/validate/1339';

                                            top.$.when(
                                                $when
                                            ).done(function(response1, response2, response3) {
                                                console.log('link sudah selesai di buka');
                                                top.document.getElementById('result').src='".base_url()."konversi/Transaksi/validate/1339'
                                            });
                                        },
                                        cssClass: 'btn-success btn-lanjutkan hidden'
                                    },
                                    {
                                        label: 'Close',
                                    ...
                                    onshown: function(dialogRef){
                                        $jsBolehLanjut
                                        \n\n
                                        top.$('.cloning-btn').on('click', function(){
                                            console.log('tombol di klik');
                                            var nama = top.$(this).attr('data-nama')
                                            var pid = top.$(this).attr('data-id')
                                            var satuan_nilai = top.$(this).attr('data-satuan')
                                            top.BootstrapDialog.show({
                                                title:\"Cloning \"+nama,
                                                cssClass: \"edit-dialog\",
                                                message: top.$(\"<div id='l1'></div>\").load('".base_url()."statik/Data/cloning/Produk/'+pid+'?1=1&pfid=l1&satuan_nilai='+satuan_nilai),
                                                draggable:false,
                                                closable:true,
...
//arrPrint($arrayOnprogress);
    ...
    public function viewUndoneItemsIndex_()
    {
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            foreach ($tmpHist as $row) {
                ...
                if ($pakai_ini == 1) {
                    ...
                    $detail = $this->viewDetailTransaksi($tmpReg_result[$row->transaksi_id], $arrItemShow, $row->jenis_master);
                    $tblItems = $detail;
                }
...
//                                matiHEre(__LINE__." gak bisa  edit evpre".$evPre." evPost".$evPost." evCom".$evCom." evMaster".$evMaster);
                            }
                        }
                        else{
                            matiHEre(__LINE__."gak boleh edit");
                        }
        ...
        if (sizeof($tmpReg) > 0) {
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $eParams) {
                    foreach ($eParams as $kreg => $vreg) {
                        if ($kreg != "transaksi_id") {
                            $$kreg = blobDecode($vreg);
                        }
                    }
...
//                    $rekening_alias = isset($rekeningAlias[$row->rekening]) ? $rekeningAlias[$row->rekening] : $row->rekening;
        ...
        //-------------------------------------
        $pairedItemBreakdown = isset($this->configUi[$jenisMasterTrans]['shoppingCartPairedItemBreakDown']) ? $this->configUi[$jenisMasterTrans]['shoppingCartPairedItemBreakDown'] : array();
        $pairedItemBreakdownEnabled = isset($this->configUi[$jenisMasterTrans]['shoppingCartPairedItemBreakDown']['enabled']) && $this->configUi[$jenisMasterTrans]['shoppingCartPairedItemBreakDown']['enabled'] == true ? true : false;
        $pairedItemPotong = isset($this->configUi[$jenisMasterTrans]['shoppingCartPairedItemPotong']) ? $this->configUi[$jenisMasterTrans]['shoppingCartPairedItemPotong'] : array();
        $pairedItemPotongEnabled = isset($this->configUi[$jenisMasterTrans]['shoppingCartPairedItemPotong']['enabled']) && $this->configUi[$jenisMasterTrans]['shoppingCartPairedItemPotong']['enabled'] == true ? true : false;
        //-------------------------------------
        ...
        $data = array(
            ...
            "itemLabels2" => isset($item2LabelsTop) ? $item2LabelsTop : array(),
            "itemLabels3" => isset($item3LabelsTop) ? $item3LabelsTop : array(),
...
// matiHEre();
    ...
    //-------
    public function viewDetailTransaksi($arrDataTransaksi, $arrItemShow, $mode = "1336")
    {
        $tblItems = "";
//arrPrintKuning($arrDataTransaksi["items4"]);
...
//                $tblItems .= "</tr>";
                ...
                $tblItems .= "</tr>";
                $noh = 0;
                foreach ($arrDataTransaksi["items"] as $pid => $iSpecc) {
//                    foreach ($arrDataTransaksi["items2_sum"][$pid] as $iSpec) {
                        $iSpec = $arrDataTransaksi["items2_sum"][$pid];
                        $noh++;
                        $tblItems .= "<tr>";
                        $tblItems .= "<td>" . $noh . "</td>";
                        foreach ($arrItemShow as $k => $iLabel) {
...
//                    }
            ...
            case "1334":
                ...
                $noh = 0;
                foreach ($arrDataTransaksi["items"] as $pid => $iSpec) {
                    $iiSpec = isset($arrDataTransaksi["items2_sum"][$pid]) ? $arrDataTransaksi["items2_sum"][$pid] : array();
//                    foreach ($arrDataTransaksi["items2_sum"][$pid] as $iSpec) {
                        $noh++;
                        $tblItems .= "<tr>";
                        $tblItems .= "<td>" . $noh . "</td>";
                        foreach ($arrItemShow as $k => $iLabel) {
                            $tblItems .= "<td>" . $iiSpec[$k] . "</td>";
                        }
                        $tblItems .= "</tr>";
//                    }
                }
