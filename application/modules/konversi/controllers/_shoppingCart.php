...
class _shoppingCart extends Modul_Controller
{
    ...
    public function viewCart()
    {
        ...
        $shoppingCartAmountValue = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][$stepNumber] : NULL;
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "kategori_id" => isset($iSpec['kategori_id']) ? $iSpec['kategori_id'] : "",
                        "kategori_nama" => isset($iSpec['kategori_nama']) ? $iSpec['kategori_nama'] : "",
                        "sub_kategori_id" => isset($iSpec['sub_kategori_id']) ? $iSpec['sub_kategori_id'] : "",
                        "sub_kategori_nama" => isset($iSpec['sub_kategori_nama']) ? $iSpec['sub_kategori_nama'] : "",
                        "merek_id" => isset($iSpec['merek_id']) ? $iSpec['merek_id'] : "",
                        "merek_nama" => isset($iSpec['merek_nama']) ? $iSpec['merek_nama'] : "",
                        "produk_part_kategori_id" => isset($iSpec['produk_part_kategori_id']) ? $iSpec['produk_part_kategori_id'] : "",
                        "produk_part_kategori_nama" => isset($iSpec['produk_part_kategori_nama']) ? $iSpec['produk_part_kategori_nama'] : "",
                        "produk_part_jenis_id" => isset($iSpec['produk_part_jenis_id']) ? $iSpec['produk_part_jenis_id'] : "",
                        "produk_part_jenis_nama" => isset($iSpec['produk_part_jenis_nama']) ? $iSpec['produk_part_jenis_nama'] : "",
                        "produk_part_ukuran_id" => isset($iSpec['produk_part_ukuran_id']) ? $iSpec['produk_part_ukuran_id'] : "",
                        "produk_part_ukuran_nama" => isset($iSpec['produk_part_ukuran_nama']) ? $iSpec['produk_part_ukuran_nama'] : "",
                        "satuan_nilai" => isset($iSpec['satuan_nilai']) ? $iSpec['satuan_nilai'] : "",
                        "sisa_dipakai" => isset($iSpec['sisa_dipakai']) ? $iSpec['sisa_dipakai'] : "",
                    );
                    ...
                    $tmp['hpp_satuan'] = isset($iSpec['hpp_avg']) ? $iSpec['hpp_avg'] * 1 / ($iSpec['jml'] * $iSpec['satuan_nilai']) : 0;

                    //direwrite karena ada transaksi lama yg non modul, seingga selector nyasar
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "konversi/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "konversi/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//            cekHijau($pernyataanNote);
        ...
        $data = array(
            ...
            "pairedItemPotong" => $pairedItemPotong,
            "pairedItemPotongEnabled" => isset($pairedItemPotongEnabled) ? $pairedItemPotongEnabled : false,
            "items4" => isset($_SESSION[$cCode]["items4"]) ? $_SESSION[$cCode]["items4"] : array(),

            "pairedItem" => isset($pairedItem) ? $pairedItem : array(),
            "pairedItemTarget" => isset($_SESSION[$cCode][$pairedItemTarget]) ? $_SESSION[$cCode][$pairedItemTarget] : array(),
            "pairedItemTargetPotong" => isset($_SESSION[$cCode]["items4"]) ? $_SESSION[$cCode]["items4"] : array(),
...
//                                $selItems[$row->$srcKey] = $row->$label . " " . $kode;
        ...
        //==selector for pairedItemPotong
        $pakai_ini = 1;
        if ($pakai_ini) {
            //DROPDOWN PRODUK KONVERSI
            if (count($pairedItemPotong) > 0) {
                if (isset($pairedItemPotong['enabled']) && ($pairedItemPotong['enabled'] == true)) {
                    $mdlName = $pairedItemPotong['mdlName'];
                    $srcLabel = isset($pairedItemPotong['srcLabel']) ? $pairedItemPotong['srcLabel'] : array();
                    $this->load->model("Mdls/$mdlName");
                    $pro = new $mdlName();
                    if (isset($pairedItemPotong['mdlFilter']) && (count($pairedItemPotong['mdlFilter']) > 0)) {
                        foreach ($pairedItemPotong['mdlFilter'] as $filter) {
                            $pro->addFilter($filter);
                        }
                    }
                    if (!empty($items)) {
                        ...
                        foreach ($items as $k => $rowItems) {
                            $pID = $rowItems['id'];
                            $pattern_A = count(explode(" ", $rowItems['nama']));
                            if (count($tmp) > 0) {
                                foreach ($tmp as $row) {
                                    $sID = $row->id;
                                    $sNama = $row->nama;
                                    $kategori_id = $row->kategori_id * 1 > 0 ? $row->kategori_id : 0;
                                    $sub_kategori_id = $row->sub_kategori_id * 1 > 0 ? $row->sub_kategori_id : 0;
                                    $kapasitas_id = $row->kapasitas_id * 1 > 0 ? $row->kapasitas_id : 0;
                                    $outdoor_id = $row->outdoor_id * 1 > 0 ? $row->outdoor_id : 0;
                                    $produk_part_ukuran_id = $row->produk_part_ukuran_id * 1 > 0 ? $row->produk_part_ukuran_id : 0;
                                    $produk_part_ukuran_nama = $row->produk_part_ukuran_nama ? $row->produk_part_ukuran_nama : "";
                                    $produk_part_kategori_id = $row->produk_part_kategori_id ? $row->produk_part_kategori_id : 0;
                                    $satuan_nilai = $row->satuan_nilai ? $row->satuan_nilai : 0;
                                    $merek_id = $row->merek_id ? $row->merek_id : 0;
                                    $pattern_B = count(explode(" ", $sNama));
                                    //detect produk sparepart, non unit, atau unit
                                    if ($kategori_id == 1 && $outdoor_id > 0) {
                                        //unit AC
//                                        cekMerah("ini unit AC");
                                    }
                                    else {
                                        if ($kategori_id == 3 && $sub_kategori_id == 5) {
                                            //sparepart
//                                        if( $produk_part_ukuran_id == $rowItems['produk_part_ukuran_id'] && $satuan_nilai < $rowItems['satuan_nilai'] &&
//                                            $merek_id == $rowItems['merek_id'] && $pattern_B == $pattern_A
//                                        )
                                            if ($produk_part_ukuran_id == $rowItems['produk_part_ukuran_id'] && $satuan_nilai < $rowItems['satuan_nilai'] &&
                                                $merek_id == $rowItems['merek_id'] && $produk_part_kategori_id == $rowItems['produk_part_kategori_id']) {
                                                if (count($srcLabel) > 0) {
                                                    foreach ($srcLabel as $srcKey => $label) {
                                                        $kode = isset($row->kode) ? $row->kode : "--";
                                                        $folder = isset($row->folders_nama) ? $row->folders_nama : "--";
                                                        $keterangan = isset($row->keterangan) ? $row->keterangan : "--";
                                                        $barcode = isset($row->barcode) ? $row->barcode : "--";
                                                        $kategori = isset($row->kategori_nama) ? $row->kategori_nama : "--";
                                                        $sub_kategori = isset($row->sub_kategori_nama) ? $row->sub_kategori_nama : "--";
                                                        $satuan_nilai = isset($row->satuan_nilai) ? $row->satuan_nilai : "--";

                                                        $konversiItems[$pID][$row->$srcKey] = $row->$label;
                                                        $konversiItemsKode[$pID][$row->$srcKey] = $kode;
                                                        $konversiItemsFolder[$pID][$row->$srcKey] = $folder;
                                                        $konversiItemsKeterangan[$pID][$row->$srcKey] = $keterangan;
                                                        $konversiItemsBarcode[$pID][$row->$srcKey] = $barcode;
                                                        $konversiItemsKategori[$pID][$row->$srcKey] = $kategori;
                                                        $konversiItemsSubKategori[$pID][$row->$srcKey] = $sub_kategori;
                                                        $konversiItemsSatuanNilai[$pID][$row->$srcKey] = $satuan_nilai;
                                                    }
                                                }
                                            }
                                        }
                                        else {
                                            if ($kategori_id == 3 && $sub_kategori_id == 4 && $kapasitas_id * 1 > 0) {
                                                //non unit AC
//                                        cekKuning("ini ac non unit");
                                            }
                                            else {
                                                if ($kategori_id == 3 && $sub_kategori_id == 4 && $kapasitas_id == 0) {
                                                    //non unit AC
//                                        cekKuning("produk selain AC ==> " . $sID . " || " . $sNama);
                                                }
                                                else {
                                                    //selain dari 3 ini
//                                        cekHitam("mbuh ini unit,no unit atau sparepart....???");
//                                        cekHitam($sID . " || " . $sNama);
                                                }
                                            }
                                        }
                        ...
                        $data['konversiItems'] = $konversiItems;
                        $data['konversiItemsKode'] = $konversiItemsKode;
                        $data['konversiItemsFolder'] = $konversiItemsFolder;
                        $data['konversiItemsKeterangan'] = $konversiItemsKeterangan;
                        $data['konversiItemsBarcode'] = $konversiItemsBarcode;
                        $data['konversiItemsKategori'] = $konversiItemsKategori;
                        $data['konversiItemsSubKategori'] = $konversiItemsSubKategori;
                        $data['konversiItemsSatuanNilai'] = $konversiItemsSatuanNilai;
                        $data['pairedKonversiItems'] = isset($_SESSION[$cCode][$pairedItemTarget]) ? $_SESSION[$cCode][$pairedItemTarget] : array();
                    }
                }
            }
        }
                                ...
                                foreach ($tmp as $row) {
                                    if (count($srcLabel) > 0) {
                                        foreach ($srcLabel as $srcKey => $label) {
                                            ...
                                            $konversiItems[$row->$srcKey] = $row->$label;
                                            $konversiItemsKode[$row->$srcKey] = $kode;
                                            $konversiItemsFolder[$row->$srcKey] = $folder;
                                            $konversiItemsKeterangan[$row->$srcKey] = $keterangan;
                                            $konversiItemsBarcode[$row->$srcKey] = $barcode;
                                            $konversiItemsKategori[$row->$srcKey] = $kategori;
                                            $konversiItemsSubKategori[$row->$srcKey] = $sub_kategori;
                                        }
                                    }
                                }
                            }
 ...
 * ---------------------------*/ // arrPrintPink($_SESSION[$cCode]);
    ...
    public function recordPairedItem()
    {
        ...
        $arrDataBarcode = array(
            ...
            "heater" => array(
                "heater_id" => "heater_barcode",
            ),
            "part" => array(
                "produk_part_id_1" => "produk_part_barcode_1",
                "produk_part_id_2" => "produk_part_barcode_2",
                "produk_part_id_3" => "produk_part_barcode_3",
            ),
        );
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                $rows = $row;
                $produk_target = $row->id;
                $produk_jenis = $row->kategori_nama;
                $satuan = strlen($row->size_nama) > 0 ? $row->size_nama : "n/a";
...
//                cekBiru(__LINE__);
                ...
                if (!array_key_exists($val, $_SESSION[$cCode]["items2_sum"])) {
                    $tmp = array(
                        ...
                        "kategori_nama" => isset($row->kategori_nama) ? $row->kategori_nama : "",
                        "sub_kategori_id" => isset($row->sub_kategori_id) ? $row->sub_kategori_id : "",
                        "sub_kategori_nama" => isset($row->sub_kategori_nama) ? $row->sub_kategori_nama : "",
                        "merek_id" => isset($row->merek_id) ? $row->merek_id : "",
                        "merek_nama" => isset($row->merek_nama) ? $row->merek_nama : "",
                        "produk_part_kategori_id" => isset($row->produk_part_kategori_id) ? $row->produk_part_kategori_id : "",
                        "produk_part_kategori_nama" => isset($row->produk_part_kategori_nama) ? $row->produk_part_kategori_nama : "",
                        "produk_part_jenis_id" => isset($row->produk_part_jenis_id) ? $row->produk_part_jenis_id : "",
                        "produk_part_jenis_nama" => isset($row->produk_part_jenis_nama) ? $row->produk_part_jenis_nama : "",
                        "produk_part_ukuran_id" => isset($row->produk_part_ukuran_id) ? $row->produk_part_ukuran_id : "",
                        "produk_part_ukuran_nama" => isset($row->produk_part_ukuran_nama) ? $row->produk_part_ukuran_nama : "",
                        "satuan_nilai" => isset($row->satuan_nilai) ? $row->satuan_nilai : "",
                        "jml_serial" => isset($jml_serial) ? $jml_serial : 0,
                        "size_id" => isset($row->size_id) ? $row->size_id : "",
                        "size_nama" => isset($row->size_nama) ? $row->size_nama : "",
                    );

                    if (sizeof($priceConfig) > 0) {
                        if ($priceConfig["enabled"] == true) {
...
//                            arrPrintPink($tmpH);
                            if (sizeof($tmpH) > 0) {
                                ...
                                $errMsgs = array();
                                foreach ($priceConfig['key_label'] as $key => $val) {
                                    $errMsgs[] = "$key " . htmlspecialchars($row->nama) . " belum ditentukan. Silahkan diseting dahulu.";
                                }
                                if (sizeof($errMsgs) > 0) {
            ...
            if ($pakai_ini == 1) {
                ...
                $tmp = $_SESSION[$cCode]['items2_sum'][$iID];
                $arrCat = array();
                $arrCode = array();
                $arrCodeSku = array();
                    ...
                    foreach ($arrDataTambahan as $cat => $catSpec) {
                        foreach ($catSpec as $dkey => $dval) {
                            if (isset($rows->$dval) && ($rows->$dval != NULL)) {
                                $_SESSION[$cCode]['items4'][$produk_target][$rows->$dval] = array();
                                //--------------
                                if (!isset($arrCat[$cat])) {
                                    $arrCat[$cat] = 0;
                                }
...
//                    $_SESSION[$cCode]['items2'][$id][$rows->kode] = array();
                    $arrCat["kode"] = 1;
                    $arrCode[$rows->kode] = 1;
                }
                ...
                if (sizeof($arrCat) > 0) {
                    foreach ($arrCat as $kcat => $vcat) {
                        $new_vcat = $vcat * $_SESSION[$cCode]['items2_sum'][$iID]["jml"];
                        if ($keterangan == "") {
            ...
            if (sizeof($arrCode) > 0) {
                $arrCodeSku = array_keys($arrCode);
            }
            else {
                $msg = "SKU indoor/outdoor Produk $p_nama tidak terdaftar. Silahkan diperiksa data produk anda atau hubungi admin. code: " . __LINE__;
                mati_disini($msg);
            }
...
//            }
            ...
            if (isset($_GET['newQty']) && !isset($_SESSION[$cCode]['items2_sum'][$iID])) {
                matiHEre("silahkan pilih produk hasil konversi terlebih dahulu");
            }
        }
...
//                $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                ...
                cekBiru(__LINE__);
                $tmpJml = $_SESSION[$cCode]['items'][$iID]['jml'];
                $tmpJmlDefault = ($_SESSION[$cCode]['items'][$iID]['jml'] * $_SESSION[$cCode]['items'][$iID]['satuan_nilai']);
...
//mati_disini("pID: $val");
                        if (sizeof($tmpH) > 0) {
                            ...
                            $errMsgs = array();
                            foreach ($priceConfig['key_label'] as $key => $val) {
                                $errMsgs[] = "$key " . htmlspecialchars($row->nama) . " is required (" . __LINE__ . ")";
                            }
                            if (sizeof($errMsgs) > 0) {
                                $_SESSION['errMsg'] = implode("<br>", $errMsgs);
...
//                            mati_disini("---");
                    ...
                    $_SESSION[$cCode]["items"][$iID]["id_target"] = $rowss->id;
                    $_SESSION[$cCode]["items"][$iID]["name_target"] = $rowss->nama;
                }
                else {
                    if (sizeof($itemNumLabels) > 0) {
                        echo("iterating subNums..");
...
//        arrPrint($_GET);
    ...
    //----------------
    public function recordPairedItemKonversi()
    {
        ...
        $mainClonerConfig = isset($this->configUi[$this->jenisTr]['mainCloner']['rsltItems']) ? $this->configUi[$this->jenisTr]['mainCloner']['rsltItems'] : array();
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        $shoppingCartPairedItemPotong = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong'] : array();
...
//arrPrint($_SESSION[$cCode]["items4"]);
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                $hpp_avg = $_SESSION[$cCode]["items"][$iID]['hpp_avg'];
                $satuan_nilai = $_SESSION[$cCode]["items"][$iID]['satuan_nilai'];
                $hpp_satuan = $hpp_avg / $satuan_nilai;

                if (!array_key_exists($val, $_SESSION[$cCode]["items4"][$iID])) {
                    $tmpBkData = array(
                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                        "editTarget" => MODUL_PATH . $shoppingCartPairedItemPotong["itemRecorder"] . "/" . $this->jenisTr . "?id=$iID&sid=" . $bkSpec->id . "&newQty=",
                        "removeTarget" => MODUL_PATH . $shoppingCartPairedItemPotong["itemRecorderRemove"] . "/" . $this->jenisTr . "?id=$iID&sid=" . $bkSpec->id . "&newQty=",
                        "id" => $bkSpec->id,
                        "jml" => 1,
                        "qty" => 1,
                        "harga" => $hpp_satuan * $bkSpec->satuan_nilai,
                        "hpp_avg" => $hpp_satuan * $bkSpec->satuan_nilai,
                        "hpp_avg_satuan" => $hpp_satuan,
                        "subtotal" => ($hpp_satuan * $bkSpec->satuan_nilai) * 1,
                        "kode" => isset($bkSpec->kode) ? $bkSpec->kode : "",
                        "barcode" => isset($bkSpec->barcode) ? $bkSpec->barcode : "",
                        "nama" => isset($bkSpec->nama) ? $bkSpec->nama : "",
                        "satuan_id" => isset($bkSpec->satuan_id) ? $bkSpec->satuan_id : "",
                        "satuan_nama" => isset($bkSpec->satuan_nama) ? $bkSpec->satuan_nama : "",
                        "size_id" => isset($bkSpec->size_id) ? $bkSpec->size_id : "",
                        "size_nama" => isset($bkSpec->size_nama) ? $bkSpec->size_nama : "",

                        "kategori_id" => isset($bkSpec->kategori_id) ? $bkSpec->kategori_id : "",
                        "kategori_nama" => isset($bkSpec->kategori_nama) ? $bkSpec->kategori_nama : "",
                        "sub_kategori_id" => isset($bkSpec->sub_kategori_id) ? $bkSpec->sub_kategori_id : "",
                        "sub_kategori_nama" => isset($bkSpec->sub_kategori_nama) ? $bkSpec->sub_kategori_nama : "",
                        "merek_id" => isset($bkSpec->merek_id) ? $bkSpec->merek_id : "",
                        "merek_nama" => isset($bkSpec->merek_nama) ? $bkSpec->merek_nama : "",
                        "produk_part_kategori_id" => isset($bkSpec->produk_part_kategori_id) ? $bkSpec->produk_part_kategori_id : "",
                        "produk_part_kategori_nama" => isset($bkSpec->produk_part_kategori_nama) ? $bkSpec->produk_part_kategori_nama : "",
                        "produk_part_jenis_id" => isset($bkSpec->produk_part_jenis_id) ? $bkSpec->produk_part_jenis_id : "",
                        "produk_part_jenis_nama" => isset($bkSpec->produk_part_jenis_nama) ? $bkSpec->produk_part_jenis_nama : "",
                        "produk_part_ukuran_id" => isset($bkSpec->produk_part_ukuran_id) ? $bkSpec->produk_part_ukuran_id : "",
                        "produk_part_ukuran_nama" => isset($bkSpec->produk_part_ukuran_nama) ? $bkSpec->produk_part_ukuran_nama : "",
                        "satuan_nilai" => isset($bkSpec->satuan_nilai) ? $bkSpec->satuan_nilai : "",
                        "sub_hpp_avg" => ($hpp_satuan * $bkSpec->satuan_nilai) * 1,
                        "sub_satuan_nilai" => $bkSpec->satuan_nilai * 1,
                        "total_dipakai" => $bkSpec->satuan_nilai * 1,
                    );

                    $_SESSION[$cCode]['items4'][$iID][$bkSpec->id] = $tmpBkData;
                }
        ...
        //inject ke gerbang items
        $totaldipakai = 0;
        foreach ($_SESSION[$cCode]['items4'][$iID] as $trgID => $preITems) {
            $totaldipakai += $preITems["satuan_nilai"] * $preITems["jml"];
//            arrPrint($preITems);
//            cekMErah($preITems["sub_satuan_nilai"]);
        }
//arrPrint($_SESSION[$cCode]['items4'][$iID]);
//        cekHitam(count($_SESSION[$cCode]['items4'][$iID]));
//        matiHere($totaldipakai);
//matiHere();
//        if ($valValue != null && $valCol != null) {
//            $_SESSION[$cCode]["items2_sum"][$iID][$valCol] = $valValue;
//        }
...
//arrPrintWebs($_SESSION[$cCode]["items4"][$iID]);
        if (isset($_SESSION[$cCode]["items"][$iID])) {
            $_SESSION[$cCode]['items'][$iID]["total_dipakai"] = $totaldipakai;
            $_SESSION[$cCode]['items'][$iID]["subtotal"] = $totaldipakai;
//            $_SESSION[$cCode]['items'][$iID]["sisa_dipakai"]= $_SESSION[$cCode]['items'][$iID]["satuan_nilai"]-$totaldipakai;
            $produk_nama_items = $_SESSION[$cCode]["items"][$iID]["nama"];
            $sub_hpp_avg_items = $_SESSION[$cCode]["items"][$iID]["sub_hpp_avg"];

            $sub_hpp_avg_items4 = 0;

            foreach ($_SESSION[$cCode]["items4"][$iID] as $sid => $iiSpec) {
...
//                arrPrint($_SESSION[$cCode]["items4"][$iID]);
//                matiHere("sub_hpp_avg_items4: $sub_hpp_avg_items4 || =>> " . $_SESSION[$cCode]["items"][$iID]["hpp_avg"]);

                $_SESSION[$cCode]["items"][$iID]["hpp_avg_target"] = $sub_hpp_avg_items4;
                if ($sub_hpp_avg_items4 < $_SESSION[$cCode]["items"][$iID]["hpp_avg"]) {
                    if (!isset($_SESSION[$cCode]["itemSisa"][$iID])) {

                        $rowItems = (object)$_SESSION[$cCode]["items"][$iID];

                        // cek ke produk sejenis
                        $this->load->model("Mdls/MdlProduk");
                        $bp = new MdlProduk();
                        $tmpBP = $bp->lookupAll()->result();
                        $tmpSisa = array();
                        if (count($tmpBP) > 0) {
                            foreach ($tmpBP as $row) {
                                $sID = $row->id;
                                $sNama = $row->nama;
                                $kategori_id = $row->kategori_id * 1 > 0 ? $row->kategori_id : 0;
                                $sub_kategori_id = $row->sub_kategori_id * 1 > 0 ? $row->sub_kategori_id : 0;
                                ...
                                if ($kategori_id == 1 && $outdoor_id > 0) {
                                    ...
                                    if ($kategori_id == 3 && $sub_kategori_id == 5) {
                                        ...
                                        if ($produk_part_ukuran_id == $rowItems['produk_part_ukuran_id'] && $satuan_nilai < $rowItems['satuan_nilai']) {
                                            ...
                                            $tmpSisa[$row->satuan_nilai] = array(
                                                "id" => $row->id,
                                                "jml" => 1,
                                                "qty" => 1,
                                                "harga" => $hpp_satuan * $row->satuan_nilai,
                                                "hpp_avg" => $hpp_satuan * $row->satuan_nilai,
                                                "hpp_avg_satuan" => $hpp_satuan,
                                                "subtotal" => ($hpp_satuan * $row->satuan_nilai) * 1,
                                                "kode" => isset($row->kode) ? $row->kode : "",
                                                "barcode" => isset($row->barcode) ? $row->barcode : "",
                                                "nama" => isset($row->nama) ? $row->nama : "",
                                                "satuan_id" => isset($row->satuan_id) ? $row->satuan_id : "",
                                                "satuan_nama" => isset($row->satuan_nama) ? $row->satuan_nama : "",
                                                ...
                                                "produk_part_ukuran_id" => isset($row->produk_part_ukuran_id) ? $row->produk_part_ukuran_id : "",
                                                "produk_part_ukuran_nama" => isset($row->produk_part_ukuran_nama) ? $row->produk_part_ukuran_nama : "",
                                                "satuan_nilai" => isset($row->satuan_nilai)
                                            );
                                        }
                                    }
                                    else {
...
//                                        cekHitam($sID . " || " . $sNama);
                        ...
                        $_SESSION[$cCode]["itemSisa"][$val] = $tmpSisa;
                    }
                }
            }
        }
...
//        heInitGates_he_cart($this->jenisTr, $initMasterValues);
        ...
        echo "top.$('#shopping_cart').load('" . base_url() . $this->modul . "/" . "_shoppingCart/viewCart/" . $this->jenisTr . "?epreketek=yes&populate=1&selID=$iID');";
        echo "}";
        echo "</script>";
