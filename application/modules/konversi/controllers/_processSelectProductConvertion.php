<?php

require_once "Modul_Controller.php";

class _processSelectProductConvertion extends Modul_Controller
{
    public function __construct()
    {
        parent::__construct();
        // $this->jenisTr = $this->uri->segment(4);
        $cCode = $this->cCode;

    }

    public function select__OLD()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $id = $_GET['id']; // produk_id source/sumber konversi
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;

        $cCode = $this->cCode;

        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel'];

        $this->load->model("Mdls/" . $selectorSrcModel);
        $b = new $selectorSrcModel();


        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][1] : array();
        $priceConfig = isset($this->configUi[$this->jenisTr]['selectedPrice']) ? $this->configUi[$this->jenisTr]['selectedPrice'] : array();
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;


        $tmpB = $b->lookupByID($id)->result();
        if (sizeof($tmpB) > 0) {

            //region ekstrak items source/sumber konversi
            foreach ($tmpB as $row) {
                $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
//                cekHere(":: " . $row->kode);
                $tmpJml = 1;
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    cekMerah("masuk locker config");

                    $mdlName = $lockerConfig['mdlName'];
                    $this->load->model("Mdls/" . $mdlName);
                    $c = new $mdlName();
                    $c->addFilter("produk_id='$id'");
//                    $c->addFilter("id='$id'");//==id locker
                    $c->addFilter("state='active'");
                    $c->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                    $c->addFilter("gudang_id=" . $this->session->login['gudang_id']);
                    $tmpC = $c->lookupAll($id)->result();
                    cekHere($this->db->last_query());

//                    $persediaan = sizeof($tmpC) > 0 ? $tmpC[0]->persediaan : "0";
                    if (sizeof($tmpC) > 0) {
                        arrPrint($tmpC);
                        foreach ($tmpC as $row) {
                            $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                            $nama = $row->nama;

                            $jml_now = $row->jumlah;
                            if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                                $jml_sudah_diambil = 0;
                                $jml_diperlukan = 1;
                                $jml_nambah = 1;
                            }
                            else {
                                if (isset($_GET['newQty'])) {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $_GET['newQty'];
                                    $jml_nambah = $jml_diperlukan - $jml_sudah_diambil;
                                }
                                else {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $jml_sudah_diambil + $jml;
                                    $jml_nambah = $jml;
                                }
                            }
                            //  region validasi stok
                            if ($jml_nambah > $jml_now) {
                                echo "<script>top.alert('stok $nama tidak cukup. (perlu $jml_diperlukan, nambah $jml_nambah stok $jml_now)')";
                                echo "</script>";
                                die();
                            }
                            //  endregion validasi stok


                            $this->db->trans_start();

                            //  region update locker active
                            $where = array(
                                "id" => $row->id,
                            );
                            $data_active = array(
                                "jumlah" => $jml_now - $jml_nambah,
                                "state" => "active",
                            );
                            $c->updateData($where, $data_active);
                            cekHere($this->db->last_query());
                            //  endregion update locker active


                            //  region locker hold
                            $array_hold_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "hold", $this->session->login['id'], "0", $this->session->login['gudang_id']);
//                            arrPrint($array_hold_sebelumnya);
//                            mati_disini();
                            if (sizeof($array_hold_sebelumnya) > 0) {
                                $where = array(
                                    "id" => $array_hold_sebelumnya['id'],
                                );
                                $data_hold = array(
                                    "jumlah" => $array_hold_sebelumnya['jumlah'] + $jml_nambah,
                                );
                                $c->updateData($where, $data_hold);
                                cekHere($this->db->last_query());
                            }
                            else {
                                $data_hold = array(
                                    "jenis" => "produk",
                                    "cabang_id" => $this->session->login['cabang_id'],
                                    "produk_id" => $id,
                                    "nama" => $nama,
                                    "satuan" => $row->satuan,
                                    "state" => "hold",
                                    "jumlah" => $jml_nambah,
                                    "oleh_id" => $this->session->login['id'],
                                    "oleh_nama" => $this->session->login['nama'],
                                    "gudang_id" => $this->session->login['gudang_id'],
                                );
                                $c->addData($data_hold);
                                cekHere($this->db->last_query());
                            }
                            //  endregion locker hold

                            $this->db->trans_complete() or die("Gagal bro");

                            $tmpJml = $jml_diperlukan;

                        }
                    }
                    else {
                        mati_disini("tidak ditemukan item " . $row->nama . " di locker stock.");
                    }

                }
                $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    $tmp = array(
                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                        "id" => $id,
                        "jml" => $tmpJml,
                        "harga" => 0,
                        "subtotal" => 0,
                        "kode" => isset($row->kode) ? $row->kode : "",
                    );
                    if (sizeof($priceConfig) > 0) {
                        $mdlName = $priceConfig['model'];
                        $this->load->model("Mdls/" . $mdlName);
                        $h = new $mdlName();
                        $h->addFilter("produk_id='$id'");
                        $h->addFilter("status='1'");
                        $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
//                        $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                        $h->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                        $tmpH = $h->lookupAll($id)->result();
                        cekMerah($this->db->last_query());
                        if (sizeof($tmpH) > 0) {
                            $rawPrices = array();
                            foreach ($tmpH as $hSpec) {
                                foreach ($priceConfig['key_label'] as $key => $val) {
                                    if ($key == $hSpec->jenis_value) {
                                        $rawPrices[$key] = isset($hSpec->nilai) ? $hSpec->nilai : 0;
                                    }
                                }
                            }
                            $prices = normalizePrices("produk", $rawPrices);
                            if (sizeof($prices) > 0) {
                                foreach ($prices as $k => $v) {
                                    $tmp[$k] = $v;
                                }
                            }
                            $tmp['harga'] = isset($tmp[$priceConfig['mainSrc']]) ? $tmp[$priceConfig['mainSrc']] : 0;
                        }

                    }
                    foreach ($fieldSrcs as $key => $src) {

                        $tmp[$key] = makeValue($src, $tmp, $tmp, $row->$src);
                    }
                    //===perhitungan subtotal
                    //region perhitungan subtotal items
                    if ($subAmountConfig != null) {
                        $subtotal = makeValue($subAmountConfig, $tmp, $tmp, $row->$src);
                    }
                    else {
                        $subtotal = 0;
                        cekHijau("subtotal NOL");
                    }
                    $tmp["subtotal"] = $subtotal;

                    $_SESSION[$cCode]['items'][$id] = $tmp;
                    //endregion
                }
                else {
                    if (isset($_GET['newQty'])) {
                        $_SESSION[$cCode]['items'][$id]['jml'] = $_GET['newQty'];
                    }
                    else {
                        $_SESSION[$cCode]['items'][$id]['jml'] += $jml;
                    }

                    if (isset($_SESSION[$cCode]['items2_sum'][$id])) {

                        $_SESSION[$cCode]['items2_sum'][$id]['jml'] = $_SESSION[$cCode]['items'][$id]['jml'];
                    }

                    if (sizeof($itemNumLabels) > 0) {
                        echo("iterating subNums..");
                        arrPrint($itemNumLabels);
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]['items'][$id][$key] = $newValue;
                                echo "replacing value for $key with " . $newValue . "<br>";
                            }

                        }

                        foreach ($itemNumLabels as $key => $label) {
                            $_SESSION[$cCode]['items'][$id]["sub_" . $key] = ($_SESSION[$cCode]['items'][$id][$key] * $_SESSION[$cCode]['items'][$id]["jml"]);
                        }
                        $_SESSION[$cCode]['items'][$id]['sub_nett'] = ($_SESSION[$cCode]['items'][$id]['nett'] * $_SESSION[$cCode]['items'][$id]['jml']);
                        $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                    }
                }
            }
            //endregion


            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                $_SESSION[$cCode]['main']['harga'] = 0;
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
                }
            }
        }
//        arrPrint($_SESSION[$cCode]['items']);
        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
            "sellerName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        // matiHere(__METHOD__ . __LINE__);
        /* --------------------------------------------------
         * ngereload shoping cart dlm modul
         * --------------------------------------------------*/
        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";
    }

    public function multiSelect()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $items = $_GET['items'];

        $arrItems = isset($_GET['items']) ? unserialize(base64_decode($items)) : array();
        $arrTrID = isset($_GET['trs']) ? unserialize(base64_decode($_GET['trs'])) : array();


        $cCode = "_TR_" . $this->jenisTr;

        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel'];

        $this->load->model("Mdls/" . $selectorSrcModel);
        $b = new $selectorSrcModel();


        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][1] : array();
        $priceConfig = isset($this->configUi[$this->jenisTr]['selectedPrice']) ? $this->configUi[$this->jenisTr]['selectedPrice'] : array();
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;

        if (sizeof($arrItems) > 0) {
            foreach ($arrItems as $id => $jmlParam) {

                $tmpB = $b->lookupByID($id)->result();
                cekHere($this->db->last_query());
                arrPrint($tmpB);

                $jml = $jmlParam;
                if (sizeof($tmpB) > 0) {
                    foreach ($tmpB as $row) {
                        $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                        $tmpJml = $jmlParam;
                        if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                            cekMerah("masuk locker config");

                            $mdlName = $lockerConfig['mdlName'];
                            $this->load->model("Mdls/" . $mdlName);
                            $c = new $mdlName();
                            $c->addFilter("produk_id='$id'");
                            $c->addFilter("state='active'");
                            $c->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                            $c->addFilter("gudang_id=" . $this->session->login['gudang_id']);
                            $tmpC = $c->lookupAll($id)->result();
                            cekHere($this->db->last_query());


                            if (sizeof($tmpC) > 0) {
                                arrPrint($tmpC);
                                foreach ($tmpC as $row) {
                                    $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                                    $nama = $row->nama;

                                    $jml_now = $row->jumlah;
                                    if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                                        $jml_sudah_diambil = 0;
                                        $jml_diperlukan = 1;
                                        $jml_nambah = 1;
                                    }
                                    else {
                                        if (isset($_GET['newQty'])) {
                                            $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                            $jml_diperlukan = $_GET['newQty'];
                                            $jml_nambah = $jml_diperlukan - $jml_sudah_diambil;
                                        }
                                        else {
                                            $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                            $jml_diperlukan = $jml_sudah_diambil + $jml;
                                            $jml_nambah = $jml;
                                        }
                                    }
                                    //  region validasi stok
                                    if ($jml_nambah > $jml_now) {
                                        echo "<script>top.alert('stok $nama tidak cukup. (perlu $jml_diperlukan, nambah $jml_nambah stok $jml_now)')";
                                        echo "</script>";
                                        die();
                                    }
                                    //  endregion validasi stok


                                    $this->db->trans_start();

                                    //  region update locker active
                                    $where = array(
                                        "id" => $row->id,
                                    );
                                    $data_active = array(
                                        "jumlah" => $jml_now - $jml_nambah,
                                        "state" => "active",
                                    );
                                    $c->updateData($where, $data_active);
                                    cekHere($this->db->last_query());
                                    //  endregion update locker active


                                    //  region locker hold
                                    $array_hold_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "hold", $this->session->login['id'], "0", $this->session->login['gudang_id']);
                                    if (sizeof($array_hold_sebelumnya) > 0) {
                                        $where = array(
                                            "id" => $array_hold_sebelumnya['id'],
                                        );
                                        $data_hold = array(
                                            "jumlah" => $array_hold_sebelumnya['jumlah'] + $jml_nambah,
                                        );
                                        $c->updateData($where, $data_hold);
                                        cekHere($this->db->last_query());
                                    }
                                    else {
                                        $data_hold = array(
                                            "jenis" => "produk",
                                            "cabang_id" => $this->session->login['cabang_id'],
                                            "produk_id" => $id,
                                            "nama" => $nama,
                                            "satuan" => $row->satuan,
                                            "state" => "hold",
                                            "jumlah" => $jml_nambah,
                                            "oleh_id" => $this->session->login['id'],
                                            "oleh_nama" => $this->session->login['nama'],
                                            "gudang_id" => $this->session->login['gudang_id'],
                                        );
                                        $c->addData($data_hold);
                                        cekHere($this->db->last_query());
                                    }
                                    //  endregion locker hold


                                    $this->db->trans_complete() or die("Gagal bro");

                                    $tmpJml = $jml_diperlukan;

                                }
                            }
                            else {
                                mati_disini("tidak ditemukan item " . $row->nama . " di locker stock.");
                            }

                        }

                        $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");
                        if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                            $tmp = array(
                                "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                "id" => $id,
                                "jml" => $tmpJml,
                                "harga" => 0,
                                "subtotal" => 0,
                            );

                            if (sizeof($priceConfig) > 0) {
                                $mdlName = $priceConfig['model'];
                                $this->load->model("Mdls/" . $mdlName);
                                $h = new $mdlName();
                                $h->addFilter("produk_id='$id'");
                                $h->addFilter("status='1'");
                                $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
//                                $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                                $h->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                                $tmpH = $h->lookupAll($id)->result();
                                cekMerah($this->db->last_query());
                                if (sizeof($tmpH) > 0) {
                                    $rawPrices = array();
                                    foreach ($tmpH as $hSpec) {
                                        foreach ($priceConfig['key_label'] as $key => $val) {
                                            if ($key == $hSpec->jenis_value) {
                                                $rawPrices[$key] = isset($hSpec->nilai) ? $hSpec->nilai : 0;
                                            }
                                        }
                                    }
                                    $prices = normalizePrices("produk", $rawPrices);
                                    if (sizeof($prices) > 0) {
                                        foreach ($prices as $k => $v) {
                                            $tmp[$k] = $v;
                                        }
                                        $tmp['harga'] = isset($tmp[$priceConfig['mainSrc']]) ? $tmp[$priceConfig['mainSrc']] : 0;
                                    }
                                }
                            }

                            foreach ($fieldSrcs as $key => $src) {
                                $tmpEx = $cal->multiExplode($src);
                                arrPrint($tmpEx);
                                if (sizeof($tmpEx) > 1) {//===berarti mengandung karakter simbol perhitungan
                                    cekBiru("$key perhitungan");
                                    $newSrc = $src;
                                    foreach ($tmpEx as $key2 => $val2) {
                                        echo "$key2 - $val2 <br>";
                                        if (!is_numeric($val2)) {
                                            if (isset($tmp[$val2]) && $tmp[$val2] > 0) {
                                                $newSrc = str_replace($val2, $tmp[$val2], $newSrc);
                                            }
                                            else {
                                                $newSrc = str_replace($val2, 0, $newSrc);
                                            }
                                        }
//                                else {
//                                    if (isset($_SESSION[$cCode]['main'][$val2]) && $_SESSION[$cCode]['main'][$val2] > 0) {
//                                        $newSrc = str_replace($val2, $_SESSION[$cCode]['main'][$val2], $newSrc);
//                                    } else {
//                                        if (isset($_SESSION[$cCode]['main'][$val2]) && $_SESSION[$cCode]['main'][$val2] > 0) {
//                                            $newSrc = str_replace($val2, $_SESSION[$cCode]['main'][$val2], $newSrc);
//                                        } else {
//                                            $newSrc = str_replace($val2, 0, $newSrc);
//                                        }
//                                    }
//                                }
                                    }
                                    cekBiru("$$src -> $newSrc -> " . $cal->calculate($newSrc));
                                    $tmp[$key] = $cal->calculate($newSrc);
                                }
                                else {
                                    cekBiru("$key BUKAN perhitungan");
                                    $tmp[$key] = $row->$src;
                                }


                            }

                            //===perhitungan subtotal
                            $cal = new FieldCalculator();


                            if ($subAmountConfig != null) {
                                $tmpEx = $cal->multiExplode($subAmountConfig);
                                if (sizeof($tmpEx) > 1) {
                                    $newSrc = $subAmountConfig;
                                    foreach ($tmpEx as $key2 => $val2) {
                                        if (isset($tmp[$val2])) {
                                            $newSrc = str_replace($val2, $tmp[$val2], $newSrc);
                                            cekKuning("$val2 direplace dengan " . $tmp[$val2]);
                                        }
                                        else {
                                            $newSrc = str_replace($val2, "0", $newSrc);
                                            cekKuning("$val2 direplace dengan NOL");
                                        }

                                    }
                                    $subtotal = $cal->calculate($newSrc);
                                    cekHijau("subtotal dari perhitungan $subAmountConfig $newSrc");

                                }
                                else {
                                    $subtotal = 0;
                                    cekHijau("subtotal dari perhitungan yang gak ada");
                                }
                            }
                            else {
                                $subtotal = 0;
                                cekHijau("subtotal NOL");
                            }
                            $tmp["subtotal"] = $subtotal;
                            $_SESSION[$cCode]['items'][$id] = $tmp;
//                    die();
                        }
                        else {
                            if (isset($_GET['newQty'])) {
                                $_SESSION[$cCode]['items'][$id]['jml'] = $_GET['newQty'];
                                $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                            }
                            else {
                                $_SESSION[$cCode]['items'][$id]['jml'] += $jml;
                                $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                            }

                            if (sizeof($itemNumLabels) > 0) {
                                echo("iterating subNums..");
                                foreach ($itemNumLabels as $key => $label) {
                                    if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                        $newValue = $_GET[$key];
                                        $tmp[$key] = $newValue;
                                        $_SESSION[$cCode]['items'][$id][$key] = $newValue;
                                        echo "replacing value for $key with " . $newValue . "<br>";
                                    }

                                }

                                foreach ($itemNumLabels as $key => $label) {
                                    $_SESSION[$cCode]['items'][$id]["sub_" . $key] = ($_SESSION[$cCode]['items'][$id][$key] * $_SESSION[$cCode]['items'][$id]["jml"]);
                                }
                                $_SESSION[$cCode]['items'][$id]['sub_nett'] = ($_SESSION[$cCode]['items'][$id]['nett'] * $_SESSION[$cCode]['items'][$id]['jml']);

                                $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                            }


                        }
                    }

                    if (sizeof($_SESSION[$cCode]['items']) > 0) {
                        $_SESSION[$cCode]['main']['harga'] = 0;
                        foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                            $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
                        }
                    }

                }
                else {
                    cekMerah("tidak ada itemnya!");
                    die();
                }

            }
        }

        if (sizeof($arrTrID) > 0) {
            $_SESSION[$cCode]['main']['references'] = $arrTrID;
        }

        //-----------------------------------------------------
        $dtime_now = dtimeNow();
        $dtime_now_ex = explode(" ", $dtime_now);
        $date_now = str_replace("-", "", $dtime_now_ex[0]);
        $time_now = str_replace(":", "", $dtime_now_ex[1]);
        $bookingNumber = "$date_now" . "$time_now";
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"]) || ($_SESSION[$cCode]["main"]["bookingNumber"] == null)) {
            $_SESSION[$cCode]["main"]["bookingNumber"] = $bookingNumber;
        }
        //-----------------------------------------------------

        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";

    }

    public function remove()
    {
        $id = $_GET['id'];
        $cCode = "_TR_" . $this->jenisTr;
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();


        if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
            cekBiru("melibatkan session");
            if (isset($_SESSION[$cCode]['items'][$id])) {
                cekBiru("ada barang, cek lokernya");
                $this->db->trans_start();

                $mdlName = $lockerConfig['mdlName'];
                $this->load->model("Mdls/" . $mdlName);

                $c = new $mdlName();
                $array_hold_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "hold", $this->session->login['id'], "0", $this->session->login['gudang_id']);
                $where = array(
                    "id" => $array_hold_sebelumnya['id'],
                );
                $data_hold = array(
                    "jumlah" => 0,
                );
                $c->updateData($where, $data_hold);


                $c = new $mdlName();
                $array_active_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "active", "0", "0", $this->session->login['gudang_id']);
                $where = array(
                    "id" => $array_active_sebelumnya['id'],
                );
                $data_active = array(
                    "jumlah" => $array_active_sebelumnya['jumlah'] + $array_hold_sebelumnya['jumlah'],
                );
                $c->updateData($where, $data_active);


                $this->db->trans_complete() or die("Gagal bro");
            }
            else {
                cekBiru("TIDAK ada barang, ga jadi cek loker");
            }
        }
        else {
            cekBiru("TIDAK melibatkan session");
        }


        if (isset($_SESSION[$cCode]['items'][$id])) {
            $_SESSION[$cCode]['items'][$id] = null;
            unset($_SESSION[$cCode]['items'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail_values'][$id])) {
            $_SESSION[$cCode]['tableIn_detail_values'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail_values'][$id]);
        }


        if (isset($_SESSION[$cCode]['items2'][$id])) {
            cekBiru("hapus items2 $id");
            $_SESSION[$cCode]['items2'][$id] = null;
            unset($_SESSION[$cCode]['items2'][$id]);
        }
        if (isset($_SESSION[$cCode]['items4'][$id])) {
//            cekBiru("hapus items4 $id");
            $_SESSION[$cCode]['items4'][$id] = null;
            unset($_SESSION[$cCode]['items4'][$id]);
        }
        if (isset($_SESSION[$cCode]['itemSisa'][$id])) {
            $_SESSION[$cCode]['itemSisa'][$id] = null;
            unset($_SESSION[$cCode]['itemSisa'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail2'][$id])) {
            cekBiru("hapus tableIn_detail2 $id");
            $_SESSION[$cCode]['tableIn_detail2'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail2'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail_values2'][$id])) {
            cekBiru("hapus tableIn_detail_values2 $id");
            $_SESSION[$cCode]['tableIn_detail_values2'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail_values2'][$id]);
        }

        if (isset($_SESSION[$cCode]['items2_sum'][$id])) {
            cekBiru("hapus items2_sum $id");
            $_SESSION[$cCode]['items2_sum'][$id] = null;
            unset($_SESSION[$cCode]['items2_sum'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail2_sum'][$id])) {
            cekBiru("hapus tableIn_detail2_sum $id");
            $_SESSION[$cCode]['tableIn_detail2_sum'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail2_sum'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail_values2_sum'][$id])) {
            cekBiru("hapus tableIn_detail_values2_sum $id");
            $_SESSION[$cCode]['tableIn_detail_values2_sum'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail_values2_sum'][$id]);
        }

        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";

    }

    public function updateValues()
    {
        echo "---------------------------your input params needed------------------------------";
        arrprint($_POST);
        $cCode = "_TR_" . $this->jenisTr;
        $rawParam = $_POST['param'];
        arrPrint($rawParam);
        die("updating.............................. (will be available sooner or later)");
        $rawParam = $_GET['param'];
        $param = unserialize(base64_decode($rawParam));
        if (is_array($param) && sizeof($param) > 0) {

        }
    }

    public function resultSelect()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $id = $_GET['id']; // produk_id source/sumber konversi
        $id_2 = isset($_GET['id2']) ? $_GET['id2'] : 0; // produk_id result/hasil konversi
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;

        $cCode = "_TR_" . $this->jenisTr;

        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel'];

        $this->load->model("Mdls/" . $selectorSrcModel);
        $b = new $selectorSrcModel();

        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][1] : array();
        $priceConfig = isset($this->configUi[$this->jenisTr]['selectedPrice']) ? $this->configUi[$this->jenisTr]['selectedPrice'] : array();
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;


        $tmpB2 = array();
        if ($id_2 > 0) {
            $tmpB2 = $b->lookupByID($id_2)->result();
        }
        //region ekstrak items result/hasil konversi
        if (sizeof($tmpB2) > 0) {
            foreach ($tmpB2 as $row) {
                $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                $tmpJml = 1;

                $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");

                if (!isset($_SESSION[$cCode]['items2_sum'])) {
                    $_SESSION[$cCode]['items2_sum'] = array();
                }
                if (!array_key_exists($id_2, $_SESSION[$cCode]['items2_sum'])) {
                    $tmp = array(
                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                        "id" => $id_2,
                        "jml" => $tmpJml,
                        "harga" => 0,
                        "subtotal" => 0,
                    );
                    if (sizeof($priceConfig) > 0) {
                        $mdlName = $priceConfig['model'];
                        $this->load->model("Mdls/" . $mdlName);
                        $h = new $mdlName();
                        $h->addFilter("produk_id='$id_2'");
                        $h->addFilter("status='1'");
                        $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
//                        $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                        $h->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                        $tmpH = $h->lookupAll($id_2)->result();
                        cekMerah($this->db->last_query());
                        if (sizeof($tmpH) > 0) {
                            foreach ($tmpH as $hSpec) {
                                foreach ($priceConfig['key_label'] as $key => $val) {
                                    if ($key == $hSpec->jenis_value) {
                                        $tmp[$val] = isset($hSpec->nilai) ? ($hSpec->nilai + 0) : 0;
                                    }
                                }
                            }
                        }
//                        $tmp['harga'] = isset($tmpH[0]->nilai) ? ($tmpH[0]->nilai + 0) : 0;
                    }
                    foreach ($fieldSrcs as $key => $src) {
                        $tmpEx = $cal->multiExplode($src);
                        if (sizeof($tmpEx) > 1) {//===berarti mengandung karakter simbol perhitungan
                            cekBiru("$key perhitungan");
                            $newSrc = $src;
                            foreach ($tmpEx as $key2 => $val2) {

                                if (!is_numeric($val2)) {
                                    if (isset($tmp[$val2]) && $tmp[$val2] > 0) {
                                        $newSrc = str_replace($val2, $tmp[$val2], $newSrc);
                                    }
                                    else {
                                        $newSrc = str_replace($val2, 0, $newSrc);
                                    }
                                }
//                                else {
//                                    if (isset($_SESSION[$cCode]['main'][$val2]) && $_SESSION[$cCode]['main'][$val2] > 0) {
//                                        $newSrc = str_replace($val2, $_SESSION[$cCode]['main'][$val2], $newSrc);
//                                    } else {
//                                        if (isset($_SESSION[$cCode]['main'][$val2]) && $_SESSION[$cCode]['main'][$val2] > 0) {
//                                            $newSrc = str_replace($val2, $_SESSION[$cCode]['main'][$val2], $newSrc);
//                                        } else {
//                                            $newSrc = str_replace($val2, 0, $newSrc);
//                                        }
//                                    }
//                                }
                            }
                            cekBiru("$$src -> $newSrc -> " . $cal->calculate($newSrc));
                            $tmp[$key] = $cal->calculate($newSrc);
                        }
                        else {
                            cekBiru("$key BUKAN perhitungan");
                            $tmp[$key] = $row->$src;
                        }
                    }

                    //===perhitungan subtotal
                    //region perhitungan subtotal items
                    $cal = new FieldCalculator();


                    if ($subAmountConfig != null) {
                        $tmpEx = $cal->multiExplode($subAmountConfig);
                        if (sizeof($tmpEx) > 1) {
                            $newSrc = $subAmountConfig;
                            foreach ($tmpEx as $key2 => $val2) {
                                if (isset($tmp[$val2])) {
                                    $newSrc = str_replace($val2, $tmp[$val2], $newSrc);
                                    cekKuning("$val2 direplace dengan " . $tmp[$val2]);
                                }
                                else {
                                    $newSrc = str_replace($val2, "0", $newSrc);
                                    cekKuning("$val2 direplace dengan NOL");
                                }

                            }
                            $subtotal = $cal->calculate($newSrc);
                            cekHijau("subtotal dari perhitungan $subAmountConfig $newSrc");

                        }
                        else {
                            $subtotal = 0;
                            cekHijau("subtotal dari perhitungan yang gak ada");
                        }
                    }
                    else {
                        $subtotal = 0;
                        cekHijau("subtotal NOL");
                    }
                    $tmp["subtotal"] = $subtotal;
                    $_SESSION[$cCode]['items2_sum'][$id_2] = $tmp;
                    //endregion
                }
                else {

                    $_SESSION[$cCode]['items2_sum'][$id_2]['jml'] = $_SESSION[$cCode]['items'][$id]['jml'];
                    if (sizeof($itemNumLabels) > 0) {
                        echo("iterating subNums..");
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]['items2_sum'][$id_2][$key] = $newValue;
                                echo "replacing value for $key with " . $newValue . "<br>";
                            }

                        }

                        foreach ($itemNumLabels as $key => $label) {
                            $_SESSION[$cCode]['items2_sum'][$id_2]["sub_" . $key] = ($_SESSION[$cCode]['items2_sum'][$id_2][$key] * $_SESSION[$cCode]['items2_sum'][$id_2]["jml"]);
                        }
                        $_SESSION[$cCode]['items2_sum'][$id_2]['sub_nett'] = ($_SESSION[$cCode]['items2_sum'][$id_2]['nett'] * $_SESSION[$cCode]['items2_sum'][$id_2]['jml']);
                        $_SESSION[$cCode]['items2_sum'][$id_2]['subtotal'] = ($_SESSION[$cCode]['items2_sum'][$id_2]['jml'] * $_SESSION[$cCode]['items2_sum'][$id_2]['harga']);
                    }
                }
            }

        }
        //endregion


        //-----------------------------------------------------
        $dtime_now = dtimeNow();
        $dtime_now_ex = explode(" ", $dtime_now);
        $date_now = str_replace("-", "", $dtime_now_ex[0]);
        $time_now = str_replace(":", "", $dtime_now_ex[1]);
        $bookingNumber = "$date_now" . "$time_now";
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"]) || ($_SESSION[$cCode]["main"]["bookingNumber"] == null)) {
            $_SESSION[$cCode]["main"]["bookingNumber"] = $bookingNumber;
        }
        //-----------------------------------------------------

        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";


    }


    public function select()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $get_id = $id = $produk_id = $_GET['id'];
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;
        $stepNum = $this->uri->segment(5) > 0 ? $this->uri->segment(5) : 1;

        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;

        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel'];
        //-----------------------------------------------
        if (!isset($_SESSION[$cCode]['items2'][$id])) {
            $_SESSION[$cCode]['items2'][$id] = array();
        }
        if (!isset($_SESSION[$cCode]['items4'][$id])) {
            $_SESSION[$cCode]['items4'][$id] = array();
        }
//        $_SESSION[$cCode]['items4'] = array();
//        $arrDataTambahan = array(
//            "outdoor" => array(
//                "outdoor_id" => "outdoor_nama",
//            ),
//            "indoor" => array(
//                "indoor_id_1" => "indoor_nama_1",
//                "indoor_id_2" => "indoor_nama_2",
//                "indoor_id_3" => "indoor_nama_3",
//                "indoor_id_4" => "indoor_nama_4",
//            ),
//            "heater" => array(
//                "heater_id" => "heater_nama",
//            ),
//            "part" => array(
//                "produk_part_id_1" => "produk_part_nama_1",
//                "produk_part_id_2" => "produk_part_nama_2",
//                "produk_part_id_3" => "produk_part_nama_3",
//            ),
//        );
        $arrDataBarcode = array(
            "outdoor" => array(
                "outdoor_id" => "outdoor_sku",
            ),
            "indoor" => array(
                "indoor_id_1" => "indoor_sku_1",
                "indoor_id_2" => "indoor_sku_2",
                "indoor_id_3" => "indoor_sku_3",
                "indoor_id_4" => "indoor_sku_4",
            ),
            "heater" => array(
                "heater_id" => "heater_barcode",
            ),
            "part" => array(
                "produk_part_id_1" => "produk_part_barcode_1",
                "produk_part_id_2" => "produk_part_barcode_2",
                "produk_part_id_3" => "produk_part_barcode_3",
            ),
        );
        //-----------------------------------------------
        $arrDataTambahan = isset($this->configUi[$this->jenisTr]['produkUnitPart']) ? $this->configUi[$this->jenisTr]['produkUnitPart'] : array();


        // detektor tanda kurawal {}
        if (substr($selectorModel, 0, 1) == "{") {
            $selectorModel = trim($selectorModel, "{");
            $selectorModel = trim($selectorModel, "}");
            $selectorModel = str_replace($selectorModel, $_SESSION[$cCode]['main'][$selectorModel], $selectorModel);
        }
        else {
            cekkuning("TIDAK mengandung kurawal @" . __LINE__ . __CLASS__);
        }
        if (substr($selectorSrcModel, 0, 1) == "{") {
            $selectorSrcModel = trim($selectorSrcModel, "{");
            $selectorSrcModel = trim($selectorSrcModel, "}");
            $selectorSrcModel = str_replace($selectorSrcModel, $_SESSION[$cCode]['main'][$selectorSrcModel], $selectorSrcModel);
        }
        else {
            cekkuning("TIDAK mengandung kurawal @" . __LINE__ . " " . __METHOD__);
        }


        $this->load->model("Mdls/" . $selectorSrcModel);
        $b = new $selectorSrcModel();

        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][1] : array();
        $priceConfig = isset($this->configUi[$this->jenisTr]['selectedPrice']) ? $this->configUi[$this->jenisTr]['selectedPrice'] : array();
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $pairedItemBreakdown = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown'] : array();

        $tmpB = $b->lookupByID($id)->result();
        // showLast_query("lime");
//         matiHere(__LINE__ . " " .__METHOD__);

        if (sizeof($tmpB) > 0) {

            foreach ($tmpB as $row) {
                $rows = $row;
                $produk_jenis = $rows->kategori_nama;
                $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";

                $kategori_id = $row->kategori_id * 1 > 0 ? $row->kategori_id : 0;
                $sub_kategori_id = $row->sub_kategori_id * 1 > 0 ? $row->sub_kategori_id : 0;
                $kapasitas_id = $row->kapasitas_id * 1 > 0 ? $row->kapasitas_id : 0;
                $outdoor_id = $row->outdoor_id * 1 > 0 ? $row->outdoor_id : 0;
                $jml_serial = $row->jml_serial;
                //detect produk sparepart, non unit, atau unit
                if ($kategori_id == 1 && $outdoor_id > 0) {
                    //unit AC

                }
                else {
                    if ($kategori_id == 3 && $sub_kategori_id == 5) {
                        //sparepart

                    }
                    else {
                        if ($kategori_id == 3 && $sub_kategori_id == 4 && $kapasitas_id * 1 > 0) {
                            //non unit AC

                        }
                        else {
                            //selain dari 3 ini
                        }
                    }
                }
                cekBiru(__LINE__);
                $tmpJml = 1;
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    cekMerah("masuk locker config");

                    $mdlName = $lockerConfig['mdlName'];
                    $this->load->model("Mdls/" . $mdlName);
                    $c = new $mdlName();
                    $c->addFilter("produk_id='$id'");
                    //                    $c->addFilter("id='$id'");//==id locker
                    $c->addFilter("state='active'");
                    $c->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                    $c->addFilter("gudang_id=" . $this->session->login['gudang_id']);


                    $tmpC = $c->lookupAll($id)->result();
                    cekHere($this->db->last_query());

//                    $persediaan = sizeof($tmpC) > 0 ? $tmpC[0]->persediaan : "0";
                    if (sizeof($tmpC) > 0) {
                        arrPrint($tmpC);
                        foreach ($tmpC as $row) {
                            $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                            $nama = $row->nama;

                            $jml_now = $row->jumlah;
                            if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                                $jml_sudah_diambil = 0;
                                $jml_diperlukan = 1;
                                $jml_nambah = 1;
                            }
                            else {
                                if (isset($_GET['newQty'])) {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $_GET['newQty'];
                                    $jml_nambah = $jml_diperlukan - $jml_sudah_diambil;
                                }
                                else {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $jml_sudah_diambil + $jml;
                                    $jml_nambah = $jml;
                                }
                            }

                            //  region validasi stok
                            if ($jml_nambah > $jml_now) {
                                echo "<script>top.alert('stok $nama tidak cukup. (perlu $jml_diperlukan, nambah $jml_nambah stok $jml_now)')";
                                echo "</script>";
                                die();
                            }
                            //  endregion validasi stok


                            $this->db->trans_start();

                            //  region update locker active
                            $where = array(
                                "id" => $row->id,
                            );
                            $data_active = array(
                                "jumlah" => $jml_now - $jml_nambah,
                                "state" => "active",
                            );
                            $c->updateData($where, $data_active);
                            cekHere($this->db->last_query());
                            //  endregion update locker active


                            //  region locker hold
                            $array_hold_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "hold", $this->session->login['id'], "0", $this->session->login['gudang_id']);
                            //                            arrPrint($array_hold_sebelumnya);
                            //                            mati_disini();
                            if (sizeof($array_hold_sebelumnya) > 0) {
                                $where = array(
                                    "id" => $array_hold_sebelumnya['id'],
                                );
                                $data_hold = array(
                                    "jumlah" => $array_hold_sebelumnya['jumlah'] + $jml_nambah,
                                );
                                $c->updateData($where, $data_hold);
                                cekHere($this->db->last_query());
                            }
                            else {
                                $data_hold = array(
                                    "jenis" => "produk",
                                    "cabang_id" => $this->session->login['cabang_id'],
                                    "produk_id" => $id,
                                    "nama" => $nama,
                                    "satuan" => $row->satuan,
                                    "state" => "hold",
                                    "jumlah" => $jml_nambah,
                                    "oleh_id" => $this->session->login['id'],
                                    "oleh_nama" => $this->session->login['nama'],
                                    "gudang_id" => $this->session->login['gudang_id'],
                                );
                                $c->addData($data_hold);
                                cekHere($this->db->last_query());
                            }
                            //  endregion locker hold

                            $this->db->trans_complete() or die("Gagal bro");

                            $tmpJml = $jml_diperlukan;

                        }
                    }
                    else {
                        mati_disini("tidak ditemukan item " . $row->nama . " di locker stock.");
                    }

                }
                $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    $tmp = array(
                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                        "id" => $id,
                        "jml" => $tmpJml,
                        "harga" => 0,
                        "subtotal" => 0,
                        "kode" => isset($row->kode) ? $row->kode : "",
                        "kategori_id" => isset($row->kategori_id) ? $row->kategori_id : "",
                        "kategori_nama" => isset($row->kategori_nama) ? $row->kategori_nama : "",
                        "sub_kategori_id" => isset($row->sub_kategori_id) ? $row->sub_kategori_id : "",
                        "sub_kategori_nama" => isset($row->sub_kategori_nama) ? $row->sub_kategori_nama : "",
                        "merek_id" => isset($row->merek_id) ? $row->merek_id : "",
                        "merek_nama" => isset($row->merek_nama) ? $row->merek_nama : "",
                        "produk_part_kategori_id" => isset($row->produk_part_kategori_id) ? $row->produk_part_kategori_id : "",
                        "produk_part_kategori_nama" => isset($row->produk_part_kategori_nama) ? $row->produk_part_kategori_nama : "",
                        "produk_part_jenis_id" => isset($row->produk_part_jenis_id) ? $row->produk_part_jenis_id : "",
                        "produk_part_jenis_nama" => isset($row->produk_part_jenis_nama) ? $row->produk_part_jenis_nama : "",
                        "produk_part_ukuran_id" => isset($row->produk_part_ukuran_id) ? $row->produk_part_ukuran_id : "",
                        "produk_part_ukuran_nama" => isset($row->produk_part_ukuran_nama) ? $row->produk_part_ukuran_nama : "",
                        "satuan_nilai" => isset($row->satuan_nilai) ? $row->satuan_nilai : "",
                        "jml_serial" => isset($jml_serial) ? $jml_serial : 0,
                    );
                    if (sizeof($priceConfig) > 0) {
                        $mdlName = $priceConfig['model'];
                        $this->load->model("Mdls/" . $mdlName);
                        $h = new $mdlName();
                        $h->addFilter("produk_id='$id'");
                        $h->addFilter("status='1'");
                        $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
//                        $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                        $h->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                        $tmpH = $h->lookupAll($id)->result();
                        cekMerah($this->db->last_query());
                        if (sizeof($tmpH) > 0) {
                            $rawPrices = array();
                            foreach ($tmpH as $hSpec) {
                                foreach ($priceConfig['key_label'] as $key => $val) {
                                    if ($key == $hSpec->jenis_value) {
                                        $rawPrices[$key] = isset($hSpec->nilai) ? $hSpec->nilai : 0;
                                    }
                                }
                            }
                            $prices = normalizePrices("produk", $rawPrices);
                            if (sizeof($prices) > 0) {
                                foreach ($prices as $k => $v) {
                                    $tmp[$k] = $v;
                                }
                            }
                            $tmp['harga'] = isset($tmp[$priceConfig['mainSrc']]) ? $tmp[$priceConfig['mainSrc']] : 0;
                        }

                    }
                    //------------------------------------------------------
                    foreach ($fieldSrcs as $key => $src) {
                        if (is_array($src) && sizeof($src) > 0) {
                            foreach ($src as $srcSpec) {
                                if (isset($tmp[$srcSpec]) || isset($rows->$srcSpec)) {
                                    $tmp[$key] = makeValue($srcSpec, $tmp, $tmp, isset($rows->$srcSpec) ? $rows->$srcSpec : "-");
                                }
                            }
                        }
                        else {
                            $tmp[$key] = makeValue($src, $tmp, $tmp, isset($rows->$src) ? $rows->$src : 0);
                        }
                    }
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $tmp[$key] = $newValue;
                            }
                        }
                    }


                    //region perhitungan subtotal items
                    if ($subAmountConfig != null) {
                        $subtotal = makeValue($subAmountConfig, $tmp, $tmp, $row->$src);
                    }
                    else {
                        $subtotal = 0;
                        cekHijau("subtotal NOL");
                    }
                    $tmp["subtotal"] = $subtotal;
//arrPrintPink($tmp);
                    $_SESSION[$cCode]['items'][$id] = $tmp;
                    //endregion
                }
                else {
                    /* --------------------------------------------------
                     * jika sudah ada dalam item distop saja
                     * --------------------------------------------------*/
                    if ($this->jenisTr == "1339") {// khusus untuk konversi potong
                        if (isset($_SESSION[$cCode]['items'][$id])) {
                            $prnama = $_SESSION[$cCode]['items'][$id]['nama'];
                            $md = url_segment(1);
                            $laert = "Item <b>$prnama</b> sudah masuk shoping cart $md, harap memilih produk lainnya <i class='meta'>@" . __LINE__ . "</i>";

                            matiHere("$laert");
                        }
                    }
                    else {
                        if (isset($_GET['newQty'])) {
                            $_SESSION[$cCode]['items'][$id]['jml'] = $_GET['newQty'];
                        }
                        else {
                            $_SESSION[$cCode]['items'][$id]['jml'] += $jml;
                        }
//                        matiHere("[$get_id = $id = $produk_id] jml: " . $_SESSION[$cCode]['items'][$produk_id]["jml"] . " === " . $_GET['newQty']);
                    }

                    if (isset($_SESSION[$cCode]['items2_sum'][$id])) {
                        $_SESSION[$cCode]['items2_sum'][$id]['jml'] = $_SESSION[$cCode]['items'][$id]['jml'];
                    }

                    if (sizeof($itemNumLabels) > 0) {
                        echo("iterating subNums..");
//                        arrPrint($itemNumLabels);
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
//                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]['items'][$id][$key] = $newValue;
                                echo "replacing value for $key with " . $newValue . "<br>";
                            }

                        }

                        foreach ($itemNumLabels as $key => $label) {
                            $_SESSION[$cCode]['items'][$id]["sub_" . $key] = ($_SESSION[$cCode]['items'][$id][$key] * $_SESSION[$cCode]['items'][$id]["jml"]);
                        }
                        $_SESSION[$cCode]['items'][$id]['sub_nett'] = ($_SESSION[$cCode]['items'][$id]['nett'] * $_SESSION[$cCode]['items'][$id]['jml']);
                        $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                    }
                }


            }

            /* ----------------------------------------------------------
             * diskon-diskonan-grosir
             * ----------------------------------------------------------*/
            //----------------------------------------
            $pakai_ini = 1;
            if ($pakai_ini == 1) {
                $tmp = $_SESSION[$cCode]['items'][$id];
                $arrCat = array();
                $arrCode = array();
                $arrCodeSku = array();
                if ($produk_jenis == "unit") {
                    foreach ($arrDataTambahan as $cat => $catSpec) {
                        foreach ($catSpec as $dkey => $dval) {
                            if (isset($rows->$dval) && ($rows->$dval != NULL)) {
                                $_SESSION[$cCode]['items2'][$produk_id][$rows->$dval] = array();
                                //--------------
                                if (!isset($arrCat[$cat])) {
                                    $arrCat[$cat] = 0;
                                }
                                $arrCat[$cat] += 1;
                                //--------------
                                cekHere(":: $dval ::");
                                if (!isset($arrCode[$rows->$dval])) {
                                    $arrCode[$rows->$dval] = 0;
                                }
                                $arrCode[$rows->$dval] += 1;
                                //--------------
//                                $arrCodeSku[] = $rows->$dval;
//                                $key_neww = isset($arrDataBarcode[$cat][$dkey]) ? $arrDataBarcode[$cat][$dkey] : null;
//                                cekBiru(":: $key_neww ::");
//                                if ($key_neww != null) {
//                                    $arrCodeSku[] = $rows->$key_neww;
//                                    if (!isset($arrCode[$rows->$key_neww])) {
//                                        $arrCode[$rows->$key_neww] = 0;
//                                    }
//                                    $arrCode[$rows->$key_neww] += 1;
//                                }
                                //--------------
                            }
                        }
                    }
                }
                else {
                    $_SESSION[$cCode]['items2'][$produk_id][$rows->kode] = array();
                    $arrCat["kode"] = 1;
                    $arrCode[$rows->kode] = 1;
                }

                $keterangan = "";
                $static_keterangan = "";
                if (sizeof($arrCat) > 0) {
                    foreach ($arrCat as $kcat => $vcat) {
                        $new_vcat = $vcat * $_SESSION[$cCode]['items'][$id]["jml"];
                        if ($keterangan == "") {
                            $keterangan = " $new_vcat $kcat";
                        }
                        else {
                            $keterangan .= "<br> $new_vcat $kcat";
                        }


                        if ($static_keterangan == "") {
                            $static_keterangan = " $vcat $kcat";
                        }
                        else {
                            $static_keterangan .= "<br> $vcat $kcat";
                        }
                        $new_keyy = "qty_" . $kcat;
                        $tmp[$new_keyy] = $vcat;
                    }
                }
                if (sizeof($arrCode) > 0) {
//                    matiHere(__LINE__);
                    foreach ($arrCode as $kcat => $vcat) {
                        $new_vcat = $vcat * $_SESSION[$cCode]['items'][$id]["jml"];
                        $tmp[$kcat] = $new_vcat;
                    }
                }
                $tmp['keterangan'] = $keterangan;
                $tmp['static_keterangan'] = $static_keterangan;
                //----------------------------------------
                $_SESSION[$cCode]['items'][$produk_id] = $tmp;
            }
            if (sizeof($arrCode) > 0) {
                $arrCodeSku = array_keys($arrCode);
            }
            else {
                $msg = "SKU indoor/outdoor Produk $p_nama tidak terdaftar. Silahkan diperiksa data produk anda atau hubungi admin. code: " . __LINE__;
                mati_disini($msg);
            }

            //----------------------------------------
            if (isset($_SESSION[$cCode]['items2_sum'][$id])) {
                $pakai_ini = 1;
                if ($pakai_ini == 1) {
                    $tmp2 = $_SESSION[$cCode]['items2_sum'][$id];
                    $p_nama2 = $tmp2["nama"];
                    $arrCat2 = array();
                    $arrCode2 = array();
                    $arrCodeSku2 = array();
//                    if ($tmp2["kategori_nama"] == "unit") {
                    if ($tmp2["jml_serial"] >= 2) {
                        foreach ($arrDataTambahan as $cat => $catSpec) {
                            foreach ($catSpec as $dkey => $dval) {
                                if (isset($tmp2[$dval]) && ($tmp2[$dval] != NULL)) {
//                                    $_SESSION[$cCode]['items2'][$produk_id][$rows->$dval] = array();
                                    //--------------
                                    if (!isset($arrCat2[$cat])) {
                                        $arrCat2[$cat] = 0;
                                    }
                                    $arrCat2[$cat] += 1;
                                    //--------------
                                    if (!isset($arrCode2[$tmp2[$dval]])) {
                                        $arrCode2[$tmp2[$dval]] = 0;
                                    }
                                    $arrCode2[$tmp2[$dval]] += 1;
                                }
                            }
                        }
                    }
                    else {
//                        $_SESSION[$cCode]['items2'][$produk_id][$rows->kode] = array();
                        $arrCat2["kode"] = 1;
                        $arrCode2[$tmp2["kode"]] = 1;
                    }

                    $keterangan = "";
                    $static_keterangan = "";
                    if (sizeof($arrCat2) > 0) {
                        foreach ($arrCat2 as $kcat => $vcat) {
                            $new_vcat = $vcat * $_SESSION[$cCode]['items2_sum'][$id]["jml"];
                            if ($keterangan == "") {
                                $keterangan = " $new_vcat $kcat";
                            }
                            else {
                                $keterangan .= "<br> $new_vcat $kcat";
                            }
                            if ($static_keterangan == "") {
                                $static_keterangan = " $vcat $kcat";
                            }
                            else {
                                $static_keterangan .= "<br> $vcat $kcat";
                            }
                            $new_keyy = "qty_" . $kcat;
                            $tmp2[$new_keyy] = $vcat;
                        }
                    }
                    if (sizeof($arrCode2) > 0) {
                        foreach ($arrCode2 as $kcat => $vcat) {
                            $new_vcat = $vcat * $_SESSION[$cCode]['items2_sum'][$id]["jml"];
                            $tmp2[$kcat] = $new_vcat;
                        }
                    }
                    $tmp2['keterangan'] = $keterangan;
                    $tmp2['static_keterangan'] = $static_keterangan;
                    //----------------------------------------
                    $_SESSION[$cCode]['items2_sum'][$produk_id] = $tmp2;
                }
                if (sizeof($arrCode2) > 0) {
                    $arrCodeSku2 = array_keys($arrCode2);
                }
                else {
                    $msg = "SKU indoor/outdoor Produk $p_nama2 tidak terdaftar. Silahkan diperiksa data produk anda atau hubungi admin. code: " . __LINE__;
                    mati_disini($msg);
                }
            }


//            arrPrintWebs($arrCodeSku);
//            matiHere("[$get_id = $id = $produk_id] jml: " . $_SESSION[$cCode]['items'][$produk_id]["jml"]);
//
//arrPrint($_SESSION[$cCode]['items']);
//mati_disini(__LINE__);

            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                $_SESSION[$cCode]['main']['harga'] = 0;
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    if ($get_id == $id) {

                        $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
                        //--------------------------------
                        if (isset($pairedItemBreakdown["enabled"]) && ($pairedItemBreakdown["enabled"] == true)) {
                            $bk = new $selectorSrcModel();
                            $bk->addFilter("kode in ('" . implode("','", $arrCodeSku) . "')");// kolom sku
//                        $bk->addFilter("barcode in ('" . implode("','", $arrCodeSku) . "')");// kolom barcode
                            $tmpBk = $bk->lookupAll()->result();
                            //                    showLast_query("biru");
                            //                    cekHere(sizeof($tmpBk));
                            if (sizeof($tmpBk) > 0) {
                                foreach ($tmpBk as $bkSpec) {
                                    $qty_sub_target = isset($iSpec[$bkSpec->kode]) ? $iSpec[$bkSpec->kode] : 0;// ganti ke kode (sku) lagi tgl 18 desember 2023
                                    $tmpBkData = array(
                                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                        "editTarget" => MODUL_PATH . $pairedItemBreakdown["itemRecorder"] . "/" . $this->jenisTr . "?id=$id&sid=" . $bkSpec->id . "&newQty=",
                                        "id" => $bkSpec->id,
//                                    "jml" => isset($iSpec[$bkSpec->barcode]) ? $iSpec[$bkSpec->barcode] : 0,
//                                    "qty" => isset($iSpec[$bkSpec->barcode]) ? $iSpec[$bkSpec->barcode] : 0,
                                        "jml" => $qty_sub_target,// ganti ke kode (sku) lagi tgl 18 desember 2023
                                        "qty" => $qty_sub_target,// ganti ke kode (sku) lagi tgl 18 desember 2023
                                        "harga" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["harga"]) ? $_SESSION[$cCode]['items4'][$id][$bkSpec->id]["harga"] : 0,
                                        "hpp_avg" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["hpp_avg"]) ? $_SESSION[$cCode]['items4'][$id][$bkSpec->id]["hpp_avg"] : 0,
//                                    "subtotal" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["subtotal"]) ? $_SESSION[$cCode]['items4'][$id][$bkSpec->id]["subtotal"] : 0,
//                                    "sub_hpp_avg" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["sub_hpp_avg"]) ? $_SESSION[$cCode]['items4'][$id][$bkSpec->id]["sub_hpp_avg"] : 0,
                                        "subtotal" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["harga"]) ? ($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["harga"] * $qty_sub_target) : 0,
                                        "sub_hpp_avg" => isset($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["hpp_avg"]) ? ($_SESSION[$cCode]['items4'][$id][$bkSpec->id]["hpp_avg"] * $qty_sub_target) : 0,
                                        "kode" => isset($bkSpec->kode) ? $bkSpec->kode : "",
                                        "barcode" => isset($bkSpec->barcode) ? $bkSpec->barcode : "",
                                        "nama" => isset($bkSpec->nama) ? $bkSpec->nama : "",
                                        "satuan_id" => isset($bkSpec->satuan_id) ? $bkSpec->satuan_id : "",
                                        "satuan_nama" => isset($bkSpec->satuan_nama) ? $bkSpec->satuan_nama : "",
                                    );
                                    $_SESSION[$cCode]['items4'][$id][$bkSpec->id] = $tmpBkData;
                                }
                            }
                        }
                        //--------------------------------

                    }

                }
            }
        }

        //-----------------------------------------------------
        $dtime_now = dtimeNow();
        $dtime_now_ex = explode(" ", $dtime_now);
        $date_now = str_replace("-", "", $dtime_now_ex[0]);
        $time_now = str_replace(":", "", $dtime_now_ex[1]);
        $bookingNumber = "$date_now" . "$time_now";
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"]) || ($_SESSION[$cCode]["main"]["bookingNumber"] == null)) {
            $_SESSION[$cCode]["main"]["bookingNumber"] = $bookingNumber;
        }
        //-----------------------------------------------------

        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
            "sellerName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        // matiHere(__METHOD__ . __LINE__);
        /* --------------------------------------------------
         * ngereload shoping cart dlm modul
         * --------------------------------------------------*/
        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";
    }

    public function selectPairedItem()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $id = $produk_id = $_GET['id'];
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;
        $stepNum = $this->uri->segment(5) > 0 ? $this->uri->segment(5) : 1;

        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;

        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel'];
        //-----------------------------------------------
        if (!isset($_SESSION[$cCode]['items2'][$id])) {
            $_SESSION[$cCode]['items2'][$id] = array();
        }
        if (!isset($_SESSION[$cCode]['items4'][$id])) {
            $_SESSION[$cCode]['items4'][$id] = array();
        }
        $arrDataTambahan = array(
            "outdoor" => array(
                "outdoor_id" => "outdoor_nama",
            ),
            "indoor" => array(
                "indoor_id_1" => "indoor_nama_1",
                "indoor_id_2" => "indoor_nama_2",
                "indoor_id_3" => "indoor_nama_3",
                "indoor_id_4" => "indoor_nama_4",
            ),
            "heater" => array(
                "heater_id" => "heater_nama",
            ),
            "part" => array(
                "produk_part_id_1" => "produk_part_nama_1",
                "produk_part_id_2" => "produk_part_nama_2",
                "produk_part_id_3" => "produk_part_nama_3",
            ),
        );
        $arrDataBarcode = array(
            "outdoor" => array(
                "outdoor_id" => "outdoor_barcode",
            ),
            "indoor" => array(
                "indoor_id_1" => "indoor_barcode_1",
                "indoor_id_2" => "indoor_barcode_2",
                "indoor_id_3" => "indoor_barcode_3",
                "indoor_id_4" => "indoor_barcode_4",
            ),
            "heater" => array(
                "heater_id" => "heater_barcode",
            ),
            "part" => array(
                "produk_part_id_1" => "produk_part_barcode_1",
                "produk_part_id_2" => "produk_part_barcode_2",
                "produk_part_id_3" => "produk_part_barcode_3",
            ),
        );
        //-----------------------------------------------
        $arrDataTambahan = isset($this->configUi[$this->jenisTr]['produkUnitPart']) ? $this->configUi[$this->jenisTr]['produkUnitPart'] : array();

        // detektor tanda kurawal {}
        if (substr($selectorModel, 0, 1) == "{") {
            $selectorModel = trim($selectorModel, "{");
            $selectorModel = trim($selectorModel, "}");
            $selectorModel = str_replace($selectorModel, $_SESSION[$cCode]['main'][$selectorModel], $selectorModel);
        }
        else {
            cekkuning("TIDAK mengandung kurawal @" . __LINE__ . __CLASS__);
        }
        if (substr($selectorSrcModel, 0, 1) == "{") {
            $selectorSrcModel = trim($selectorSrcModel, "{");
            $selectorSrcModel = trim($selectorSrcModel, "}");
            $selectorSrcModel = str_replace($selectorSrcModel, $_SESSION[$cCode]['main'][$selectorSrcModel], $selectorSrcModel);
        }
        else {
            cekkuning("TIDAK mengandung kurawal @" . __LINE__ . " " . __METHOD__);
        }


        $this->load->model("Mdls/" . $selectorSrcModel);
        $b = new $selectorSrcModel();

        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][1] : array();
        $priceConfig = isset($this->configUi[$this->jenisTr]['selectedPrice']) ? $this->configUi[$this->jenisTr]['selectedPrice'] : array();
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $pairedItemBreakdown = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown'] : array();

        $tmpB = $b->lookupByID($id)->result();
//        showLast_query("lime");
        // matiHere(__LINE__ . " " .__METHOD__);

        if (sizeof($tmpB) > 0) {

            foreach ($tmpB as $row) {
                $rows = $row;
                $produk_jenis = $rows->kategori_nama;
                $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";

                $tmpJml = 1;
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    cekMerah("masuk locker config");

                    $mdlName = $lockerConfig['mdlName'];
                    $this->load->model("Mdls/" . $mdlName);
                    $c = new $mdlName();
                    $c->addFilter("produk_id='$id'");
                    //                    $c->addFilter("id='$id'");//==id locker
                    $c->addFilter("state='active'");
                    $c->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                    $c->addFilter("gudang_id=" . $this->session->login['gudang_id']);


                    $tmpC = $c->lookupAll($id)->result();
                    cekHere($this->db->last_query());

//                    $persediaan = sizeof($tmpC) > 0 ? $tmpC[0]->persediaan : "0";
                    if (sizeof($tmpC) > 0) {
                        arrPrint($tmpC);
                        foreach ($tmpC as $row) {
                            $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                            $nama = $row->nama;

                            $jml_now = $row->jumlah;
                            if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                                $jml_sudah_diambil = 0;
                                $jml_diperlukan = 1;
                                $jml_nambah = 1;
                            }
                            else {
                                if (isset($_GET['newQty'])) {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $_GET['newQty'];
                                    $jml_nambah = $jml_diperlukan - $jml_sudah_diambil;
                                }
                                else {
                                    $jml_sudah_diambil = $_SESSION[$cCode]['items'][$id]['jml'];
                                    $jml_diperlukan = $jml_sudah_diambil + $jml;
                                    $jml_nambah = $jml;
                                }
                            }
                            //  region validasi stok
                            if ($jml_nambah > $jml_now) {
                                echo "<script>top.alert('stok $nama tidak cukup. (perlu $jml_diperlukan, nambah $jml_nambah stok $jml_now)')";
                                echo "</script>";
                                die();
                            }
                            //  endregion validasi stok


                            $this->db->trans_start();

                            //  region update locker active
                            $where = array(
                                "id" => $row->id,
                            );
                            $data_active = array(
                                "jumlah" => $jml_now - $jml_nambah,
                                "state" => "active",
                            );
                            $c->updateData($where, $data_active);
                            cekHere($this->db->last_query());
                            //  endregion update locker active


                            //  region locker hold
                            $array_hold_sebelumnya = $c->cekLoker($this->session->login['cabang_id'], $id, "hold", $this->session->login['id'], "0", $this->session->login['gudang_id']);
                            //                            arrPrint($array_hold_sebelumnya);
                            //                            mati_disini();
                            if (sizeof($array_hold_sebelumnya) > 0) {
                                $where = array(
                                    "id" => $array_hold_sebelumnya['id'],
                                );
                                $data_hold = array(
                                    "jumlah" => $array_hold_sebelumnya['jumlah'] + $jml_nambah,
                                );
                                $c->updateData($where, $data_hold);
                                cekHere($this->db->last_query());
                            }
                            else {
                                $data_hold = array(
                                    "jenis" => "produk",
                                    "cabang_id" => $this->session->login['cabang_id'],
                                    "produk_id" => $id,
                                    "nama" => $nama,
                                    "satuan" => $row->satuan,
                                    "state" => "hold",
                                    "jumlah" => $jml_nambah,
                                    "oleh_id" => $this->session->login['id'],
                                    "oleh_nama" => $this->session->login['nama'],
                                    "gudang_id" => $this->session->login['gudang_id'],
                                );
                                $c->addData($data_hold);
                                cekHere($this->db->last_query());
                            }
                            //  endregion locker hold

                            $this->db->trans_complete() or die("Gagal bro");

                            $tmpJml = $jml_diperlukan;

                        }
                    }
                    else {
                        mati_disini("tidak ditemukan item " . $row->nama . " di locker stock.");
                    }

                }
                $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    $tmp = array(
                        "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                        "id" => $id,
                        "jml" => $tmpJml,
                        "harga" => 0,
                        "subtotal" => 0,
                        "kode" => isset($row->kode) ? $row->kode : "",
                    );
                    if (sizeof($priceConfig) > 0) {
                        $mdlName = $priceConfig['model'];
                        $this->load->model("Mdls/" . $mdlName);
                        $h = new $mdlName();
                        $h->addFilter("produk_id='$id'");
                        $h->addFilter("status='1'");
                        $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
//                        $h->addFilter("jenis_value='" . $priceConfig['label'] . "'");
                        $h->addFilter("cabang_id=" . $this->session->login['cabang_id']);
                        $tmpH = $h->lookupAll($id)->result();
                        cekMerah($this->db->last_query());
                        if (sizeof($tmpH) > 0) {
                            $rawPrices = array();
                            foreach ($tmpH as $hSpec) {
                                foreach ($priceConfig['key_label'] as $key => $val) {
                                    if ($key == $hSpec->jenis_value) {
                                        $rawPrices[$key] = isset($hSpec->nilai) ? $hSpec->nilai : 0;
                                    }
                                }
                            }
                            $prices = normalizePrices("produk", $rawPrices);
                            if (sizeof($prices) > 0) {
                                foreach ($prices as $k => $v) {
                                    $tmp[$k] = $v;
                                }
                            }
                            $tmp['harga'] = isset($tmp[$priceConfig['mainSrc']]) ? $tmp[$priceConfig['mainSrc']] : 0;
                        }

                    }
                    //------------------------------------------------------
                    foreach ($fieldSrcs as $key => $src) {
                        if (is_array($src) && sizeof($src) > 0) {
                            foreach ($src as $srcSpec) {
                                if (isset($tmp[$srcSpec]) || isset($rows->$srcSpec)) {
                                    $tmp[$key] = makeValue($srcSpec, $tmp, $tmp, isset($rows->$srcSpec) ? $rows->$srcSpec : "-");
                                }
                            }
                        }
                        else {
                            $tmp[$key] = makeValue($src, $tmp, $tmp, isset($rows->$src) ? $rows->$src : 0);
                        }
                    }
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $tmp[$key] = $newValue;
                            }
                        }
                    }


                    //region perhitungan subtotal items
                    if ($subAmountConfig != null) {
                        $subtotal = makeValue($subAmountConfig, $tmp, $tmp, $row->$src);
                    }
                    else {
                        $subtotal = 0;
                        cekHijau("subtotal NOL");
                    }
                    $tmp["subtotal"] = $subtotal;
//arrPrintPink($tmp);
                    $_SESSION[$cCode]['items'][$id] = $tmp;
                    //endregion
                }
                else {
                    if (isset($_GET['newQty'])) {
                        $_SESSION[$cCode]['items'][$id]['jml'] = $_GET['newQty'];
                    }
                    else {
                        $_SESSION[$cCode]['items'][$id]['jml'] += $jml;
                    }

                    if (isset($_SESSION[$cCode]['items2_sum'][$id])) {
                        $_SESSION[$cCode]['items2_sum'][$id]['jml'] = $_SESSION[$cCode]['items'][$id]['jml'];
                    }

                    if (sizeof($itemNumLabels) > 0) {
                        echo("iterating subNums..");
                        arrPrint($itemNumLabels);
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]['items'][$id][$key] = $newValue;
                                echo "replacing value for $key with " . $newValue . "<br>";
                            }

                        }

                        foreach ($itemNumLabels as $key => $label) {
                            $_SESSION[$cCode]['items'][$id]["sub_" . $key] = ($_SESSION[$cCode]['items'][$id][$key] * $_SESSION[$cCode]['items'][$id]["jml"]);
                        }
                        $_SESSION[$cCode]['items'][$id]['sub_nett'] = ($_SESSION[$cCode]['items'][$id]['nett'] * $_SESSION[$cCode]['items'][$id]['jml']);
                        $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * $_SESSION[$cCode]['items'][$id]['harga']);
                    }
                }


            }

            /* ----------------------------------------------------------
             * diskon-diskonan-grosir
             * ----------------------------------------------------------*/
            //----------------------------------------
            $pakai_ini = 0;
            if ($pakai_ini == 1) {

                $tmp = $_SESSION[$cCode]['items'][$id];
                $arrCat = array();
                $arrCode = array();
                $arrCodeSku = array();
                if ($produk_jenis == "unit") {
                    foreach ($arrDataTambahan as $cat => $catSpec) {
                        foreach ($catSpec as $dkey => $dval) {
                            if (isset($rows->$dval) && ($rows->$dval != NULL)) {
                                $_SESSION[$cCode]['items2'][$produk_id][$rows->$dval] = array();
                                //--------------
                                if (!isset($arrCat[$cat])) {
                                    $arrCat[$cat] = 0;
                                }
                                $arrCat[$cat] += 1;
                                //--------------
                                if (!isset($arrCode[$rows->$dval])) {
                                    $arrCode[$rows->$dval] = 0;
                                }
                                $arrCode[$rows->$dval] += 1;
                                //--------------
//                                $arrCodeSku[] = $rows->$dval;
                                $key_neww = isset($arrDataBarcode[$cat][$dkey]) ? $arrDataBarcode[$cat][$dkey] : null;
                                if ($key_neww != null) {
                                    $arrCodeSku[] = $rows->$key_neww;
                                    if (!isset($arrCode[$rows->$key_neww])) {
                                        $arrCode[$rows->$key_neww] = 0;
                                    }
                                    $arrCode[$rows->$key_neww] += 1;
                                }
                                //--------------
                            }
                        }
                    }
                }
                else {
                    $_SESSION[$cCode]['items2'][$produk_id][$rows->barcode] = array();
                    $arrCat["barcode"] = 1;
                    $arrCode[$rows->barcode] = 1;
                }
                $keterangan = "";
                $static_keterangan = "";
                if (sizeof($arrCat) > 0) {
                    foreach ($arrCat as $kcat => $vcat) {
                        $new_vcat = $vcat * $_SESSION[$cCode]['items'][$id]["jml"];
                        if ($keterangan == "") {
                            $keterangan = " $new_vcat $kcat";
                        }
                        else {
                            $keterangan .= "<br> $new_vcat $kcat";
                        }


                        if ($static_keterangan == "") {
                            $static_keterangan = " $vcat $kcat";
                        }
                        else {
                            $static_keterangan .= "<br> $vcat $kcat";
                        }
                        $new_keyy = "qty_" . $kcat;
                        $tmp[$new_keyy] = $vcat;
                    }
                }
                if (sizeof($arrCode) > 0) {
                    foreach ($arrCode as $kcat => $vcat) {
                        $new_vcat = $vcat * $_SESSION[$cCode]['items'][$id]["jml"];
                        $tmp[$kcat] = $new_vcat;
                    }
                }
                $tmp['keterangan'] = $keterangan;
                $tmp['static_keterangan'] = $static_keterangan;
                //----------------------------------------
                $_SESSION[$cCode]['items'][$produk_id] = $tmp;
            }


            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                $_SESSION[$cCode]['main']['harga'] = 0;
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);

                    //--------------------------------
                    if (isset($pairedItemBreakdown["enabled"]) && ($pairedItemBreakdown["enabled"] == true)) {
                        $bk = new $selectorSrcModel();
                        //                    $bk->addFilter("parent_id='$id'");
                        $bk->addFilter("barcode in ('" . implode("','", $arrCodeSku) . "')");
                        $tmpBk = $bk->lookupAll()->result();
                        //                    showLast_query("biru");
                        //                    cekHere(sizeof($tmpBk));
                        if (sizeof($tmpBk) > 0) {
                            foreach ($tmpBk as $bkSpec) {
                                $tmpBkData = array(
                                    "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                    "editTarget" => MODUL_PATH . $pairedItemBreakdown["itemRecorder"] . "/" . $this->jenisTr . "?id=$id&sid=" . $bkSpec->id . "&newQty=",
                                    "id" => $bkSpec->id,
                                    "jml" => isset($iSpec[$bkSpec->barcode]) ? $iSpec[$bkSpec->barcode] : 0,
                                    "qty" => isset($iSpec[$bkSpec->barcode]) ? $iSpec[$bkSpec->barcode] : 0,
                                    "harga" => 0,
                                    "hpp_avg" => 0,
                                    "subtotal" => 0,
                                    "kode" => isset($bkSpec->kode) ? $bkSpec->kode : "",
                                    "barcode" => isset($bkSpec->barcode) ? $bkSpec->barcode : "",
                                    "nama" => isset($bkSpec->nama) ? $bkSpec->nama : "",
                                    "satuan_id" => isset($bkSpec->satuan_id) ? $bkSpec->satuan_id : "",
                                    "satuan_nama" => isset($bkSpec->satuan_nama) ? $bkSpec->satuan_nama : "",
                                );
                                $_SESSION[$cCode]['items4'][$id][$bkSpec->id] = $tmpBkData;
                            }
                        }
                    }
                    //--------------------------------


                }
            }
        }

        //-----------------------------------------------------
        $dtime_now = dtimeNow();
        $dtime_now_ex = explode(" ", $dtime_now);
        $date_now = str_replace("-", "", $dtime_now_ex[0]);
        $time_now = str_replace(":", "", $dtime_now_ex[1]);
        $bookingNumber = "$date_now" . "$time_now";
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"]) || ($_SESSION[$cCode]["main"]["bookingNumber"] == null)) {
            $_SESSION[$cCode]["main"]["bookingNumber"] = $bookingNumber;
        }
        //-----------------------------------------------------

        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
            "sellerName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        // matiHere(__METHOD__ . __LINE__);
        /* --------------------------------------------------
         * ngereload shoping cart dlm modul
         * --------------------------------------------------*/
        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";
    }

    public function removePairedItem()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $id = $produk_id = $_GET['id'];
        $sub_id = $sub_produk_id = $_GET['sid'];
//        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;
//        $stepNum = $this->uri->segment(5) > 0 ? $this->uri->segment(5) : 1;

        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;

        if (isset($_SESSION[$cCode]["items4"][$id][$sub_id])) {
            $hold_dipakai = $_SESSION[$cCode]["items4"][$id][$sub_id]["total_dipakai"];
            $total_dipakai = $_SESSION[$cCode]["items"][$id]["total_dipakai"];

//            cekHitam($hold_dipakai);
//            cekHitam($sisa_dipakai);
//            matiHere($sisa_dipakai+$hold_dipakai);
//            $_SESSION[$cCode]["items"][$id]["total_dipakai"] = 0;
            $_SESSION[$cCode]["items"][$id]["total_dipakai"] = $total_dipakai - $hold_dipakai;
//            matiHere("hitung ulang total_dipakai");
            $_SESSION[$cCode]["items4"][$id][$sub_id] = null;
            unset($_SESSION[$cCode]["items4"][$id][$sub_id]);
        }


        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
            "sellerName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);


        /* --------------------------------------------------
         * ngereload shoping cart dlm modul
         * --------------------------------------------------*/
        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";
    }

    public function selectPaired()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();

        $id = $produk_id = $_GET['id'];
        $subid = $sub_produk_id = $_GET['sid'];
        $stepNum = $this->uri->segment(5) > 0 ? $this->uri->segment(5) : 1;

        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;
        $pairedItemBreakdown = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown'] : array();

        if (isset($_SESSION[$cCode]["items"][$id])) {

            $produk_nama_items = $_SESSION[$cCode]["items"][$id]["nama"];
            $sub_hpp_avg_items = $_SESSION[$cCode]["items"][$id]["sub_hpp_avg"];
            $sub_hpp_avg_items4 = 0;

            if (isset($_SESSION[$cCode]["items4"][$id][$subid])) {
                //validasi sebelum nulis session
                if ($_GET['jml'] > 0) {
                    $prevdipakai = $_SESSION[$cCode]["items"][$id]["total_dipakai"];
                    $prevsubitem = $_SESSION[$cCode]["items4"][$id][$subid]["total_dipakai"];
                    $new_total_dipake = $_GET['jml'] * $_SESSION[$cCode]["items4"][$id][$subid]["satuan_nilai"];
                    $new_prev = $prevdipakai - $prevsubitem;

                    //kebanyakan hasil potongan
//                    cekHitam($new_total_dipake);
                    $sisa_dipakai = $_SESSION[$cCode]["items"][$id]["satuan_nilai"] - $new_prev;
//                    cekMerah($sisa_dipakai);
//                    cekBiru($new_total_dipake);
                    if ($sisa_dipakai < $new_total_dipake) {
                        $satuan_nilai = $_SESSION[$cCode]["items4"][$id][$subid]["satuan_nilai"] * 1;
                        $size_nama = $_SESSION[$cCode]["items4"][$id][$subid]["size_nama"];
                        matiHere("Qty konversi berlebihan, tersedia $sisa_dipakai $size_nama, permintaan " . $_GET['jml'] . "(x)@$satuan_nilai $size_nama" . " silahkan kurangi qty produk konversi");
                    }
                    else {
                        //masih bis konversi
                    }

                }

//                matiHere(__LINE__." ".__FUNCTION__);
//                if(isset($_GET['newQty']) && $_GET['newQty']*1>0){
//                    $_SESSION[$cCode]["items4"][$id][$subid]["jml"] = $_GET['newQty']*1;
//                    $_SESSION[$cCode]["items4"][$id][$subid]["qty"] = $_GET['newQty']*1;
//                }

                if (isset($_GET['jml']) && $_GET['jml'] * 1 > 0) {
                    $_SESSION[$cCode]["items4"][$id][$subid]["jml"] = $_GET['jml'] * 1;
                    $_SESSION[$cCode]["items4"][$id][$subid]["qty"] = $_GET['jml'] * 1;
                    $_SESSION[$cCode]["items4"][$id][$subid]["total_dipakai"] = ($_GET['jml'] * $_SESSION[$cCode]["items4"][$id][$subid]["satuan_nilai"]) * 1;
                }
//                $_SESSION[$cCode]["items4"][$id][$subid]["harga"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
//                $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                $_SESSION[$cCode]["items4"][$id][$subid]["sub_hpp_avg"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
            }
            $total_dipakai = 0;
            foreach ($_SESSION[$cCode]["items4"][$id] as $sid => $iiSpec) {
                $sub_hpp_avg_items4 += isset($iiSpec["sub_hpp_avg"]) ? $iiSpec["sub_hpp_avg"] : 0;
                $total_dipakai += $iiSpec["total_dipakai"];
            }

//            matiHere();
            $_SESSION[$cCode]["items"][$id]["total_dipakai"] = $total_dipakai;
            cekHere("[sub_hpp_avg_items4: $sub_hpp_avg_items4] [sub_hpp_avg_items: $sub_hpp_avg_items]");
            $selisih = $sub_hpp_avg_items4 - $sub_hpp_avg_items;

            if ($_SESSION[$cCode]["items"][$id]["total_dipakai"] > $_SESSION[$cCode]["items"][$id]["satuan_nilai"]) {
                //kebanyakan hasil potongan
                $sisa_dipakai = $_SESSION[$cCode]["items"][$id]["total_dipakai"];
                matiHere("Qty konversi berlebihan, tersedia $sisa_dipakai silahkan kurangi qty produk konversi, supaya sisa potongan 0");
            }


//            if($_SESSION[$cCode]["items"][$id]["total_dipakai"] < $_SESSION[$cCode]["items"][$id]["satuan_nilai"]){
//                //kurang  hasil potongan
//                matiHere("silahkan tambahkan qty produk konversi/ tambahkan produk lain supaya sisa potongan 0");
//            }

//            if($_SESSION[$cCode]["items"][$id]["total_dipakai"]==$_SESSION[$cCode]["items"][$id]["satuan_nilai"]){
//                //lolos
//            }
//

            if ($selisih > 1) {
                $msg = "akumulasi hpp hasil konversi produk $produk_nama_items tidak sama dengan hpp sumber konversi. silahkan diperiksa lagi. code: " . __LINE__;
                mati_disini($msg);
            }
            else {
                $_SESSION[$cCode]["items"][$id]["hpp_avg_target"] = $sub_hpp_avg_items4;
                if ($sub_hpp_avg_items4 < $_SESSION[$cCode]["items"][$id]["hpp_avg"]) {
                    if (!isset($_SESSION[$cCode]["itemSisa"][$id])) {
                        $rowItems = (object)$_SESSION[$cCode]["items"][$id];
                        // cek ke produk sejenis
                        $this->load->model("Mdls/MdlProduk");
                        $bp = new MdlProduk();
                        $tmpBP = $bp->lookupAll()->result();
//                        showLast_query("biru");
                        $tmpSisa = array();
                        if (count($tmpBP) > 0) {
                            foreach ($tmpBP as $row) {
                                $sID = $row->id;
                                $sNama = $row->nama;
                                $kategori_id = $row->kategori_id * 1 > 0 ? $row->kategori_id : 0;
                                $sub_kategori_id = $row->sub_kategori_id * 1 > 0 ? $row->sub_kategori_id : 0;
                                $kapasitas_id = $row->kapasitas_id * 1 > 0 ? $row->kapasitas_id : 0;
                                $outdoor_id = $row->outdoor_id * 1 > 0 ? $row->outdoor_id : 0;
                                $produk_part_ukuran_id = $row->produk_part_ukuran_id * 1 > 0 ? $row->produk_part_ukuran_id : 0;
                                $produk_part_ukuran_nama = $row->produk_part_ukuran_nama ? $row->produk_part_ukuran_nama : "";
                                $satuan_nilai = $row->satuan_nilai ? $row->satuan_nilai : 0;

                                // detect produk sparepart, non unit, atau unit
                                if ($kategori_id == 1 && $outdoor_id > 0) {
                                    // unit AC
//                                    cekMerah("ini unit AC");
                                }
                                else {
                                    if ($kategori_id == 3 && $sub_kategori_id == 5) {
//                                        cekKuning("******");
                                        //sparepart
                                        if ($produk_part_ukuran_id == $rowItems->produk_part_ukuran_id && $satuan_nilai < $rowItems->satuan_nilai) {
                                            $kode = isset($row->kode) ? $row->kode : "--";
                                            $folder = isset($row->folders_nama) ? $row->folders_nama : "--";
                                            $keterangan = isset($row->keterangan) ? $row->keterangan : "--";
                                            $barcode = isset($row->barcode) ? $row->barcode : "--";
                                            $kategori = isset($row->kategori_nama) ? $row->kategori_nama : "--";
                                            $sub_kategori = isset($row->sub_kategori_nama) ? $row->sub_kategori_nama : "--";

                                            $tmpSisa[$row->satuan_nilai] = array(
                                                "id" => $row->id,
                                                "jml" => 1,
                                                "qty" => 1,
                                                "harga" => $hpp_satuan * $row->satuan_nilai,
                                                "hpp_avg" => $hpp_satuan * $row->satuan_nilai,
                                                "hpp_avg_satuan" => $hpp_satuan,
                                                "subtotal" => ($hpp_satuan * $row->satuan_nilai) * 1,
                                                "kode" => isset($row->kode) ? $row->kode : "",
                                                "barcode" => isset($row->barcode) ? $row->barcode : "",
                                                "nama" => isset($row->nama) ? $row->nama : "",
                                                "satuan_id" => isset($row->satuan_id) ? $row->satuan_id : "",
                                                "satuan_nama" => isset($row->satuan_nama) ? $row->satuan_nama : "",

                                                "kategori_id" => isset($row->kategori_id) ? $row->kategori_id : "",
                                                "kategori_nama" => isset($row->kategori_nama) ? $row->kategori_nama : "",
                                                "sub_kategori_id" => isset($row->sub_kategori_id) ? $row->sub_kategori_id : "",
                                                "sub_kategori_nama" => isset($row->sub_kategori_nama) ? $row->sub_kategori_nama : "",
                                                "merek_id" => isset($row->merek_id) ? $row->merek_id : "",
                                                "merek_nama" => isset($row->merek_nama) ? $row->merek_nama : "",
                                                "produk_part_kategori_id" => isset($row->produk_part_kategori_id) ? $row->produk_part_kategori_id : "",
                                                "produk_part_kategori_nama" => isset($row->produk_part_kategori_nama) ? $row->produk_part_kategori_nama : "",
                                                "produk_part_jenis_id" => isset($row->produk_part_jenis_id) ? $row->produk_part_jenis_id : "",
                                                "produk_part_jenis_nama" => isset($row->produk_part_jenis_nama) ? $row->produk_part_jenis_nama : "",
                                                "produk_part_ukuran_id" => isset($row->produk_part_ukuran_id) ? $row->produk_part_ukuran_id : "",
                                                "produk_part_ukuran_nama" => isset($row->produk_part_ukuran_nama) ? $row->produk_part_ukuran_nama : "",
                                                "satuan_nilai" => isset($row->satuan_nilai)
                                            );
                                        }
                                    }
                                    else {
                                        if ($kategori_id == 3 && $sub_kategori_id == 4 && $kapasitas_id * 1 > 0) {
                                            //non unit AC
                                            if ($subid == $sID) {
                                                cekKuning("ini ac non unit [$subid == $sID]");
                                                $_SESSION[$cCode]["items4"][$id][$subid]["harga"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                                                $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                                                $_SESSION[$cCode]["items4"][$id][$subid]["sub_hpp_avg"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                                                $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                                                $sisa_hpp_input = $_SESSION[$cCode]["items"][$id]["sub_hpp_avg"] - $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"];
                                                if ($sisa_hpp_input > 0) {
                                                    foreach ($_SESSION[$cCode]["items4"][$id] as $ssID => $ssSpec) {
//                                                        if (($ssID != $subid) && ($ssSpec["hpp_avg"] == 0)) {
                                                        if (($ssID != $subid)) {
                                                            $_SESSION[$cCode]["items4"][$id][$ssID]["harga"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                                            $_SESSION[$cCode]["items4"][$id][$ssID]["hpp_avg"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                                            $_SESSION[$cCode]["items4"][$id][$ssID]["sub_hpp_avg"] = $sisa_hpp_input;
                                                            $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"] = $sisa_hpp_input;
                                                            $sisa_hpp_input = $sisa_hpp_input - $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"];
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                        else {
                                            if ($kategori_id == 3 && $sub_kategori_id == 4 && $kapasitas_id == 0) {
                                                //non unit AC
//                                                cekKuning("produk selain AC ==> " . $sID . " || " . $sNama);
                                            }
                                            else {
                                                //selain dari 3 ini
//                                                cekHitam("mbuh ini unit,no unit atau sparepart....???");
//                                        cekHitam($sID . " || " . $sNama);
                                            }
                                        }
                                    }
                                }
                            }
                        }
                        $_SESSION[$cCode]["itemSisa"][$id] = $tmpSisa;
                    }
                    else {
                        if ($pairedItemBreakdown["hitungUlangHppTarget"] == true) {

                            cekHere("sudah ada sesi [itemSisa][$id]");
                            cekKuning("ini ac non unit [$subid == $sID]");
                            $_SESSION[$cCode]["items4"][$id][$subid]["harga"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                            $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                            $_SESSION[$cCode]["items4"][$id][$subid]["sub_hpp_avg"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                            $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                            $sisa_hpp_input = $_SESSION[$cCode]["items"][$id]["sub_hpp_avg"] - $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"];
                            if ($sisa_hpp_input > 0) {
                                foreach ($_SESSION[$cCode]["items4"][$id] as $ssID => $ssSpec) {
                                    if (($ssID != $subid)) {
                                        $_SESSION[$cCode]["items4"][$id][$ssID]["harga"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                        $_SESSION[$cCode]["items4"][$id][$ssID]["hpp_avg"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                        $_SESSION[$cCode]["items4"][$id][$ssID]["sub_hpp_avg"] = $sisa_hpp_input;
                                        $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"] = $sisa_hpp_input;
                                        $sisa_hpp_input -= $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"];
                                    }
                                }
                            }
                            cekHitam("[$sisa_hpp_input]");
                        }

                    }
                }
                else {
                    if ($pairedItemBreakdown["hitungUlangHppTarget"] == true) {

                        cekKuning("ini ac non unit [$subid == $sID]");
                        $_SESSION[$cCode]["items4"][$id][$subid]["harga"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                        $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"] = isset($_GET['hpp_avg']) ? $_GET['hpp_avg'] : 0;
                        $_SESSION[$cCode]["items4"][$id][$subid]["sub_hpp_avg"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                        $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"] = $_SESSION[$cCode]["items4"][$id][$subid]["jml"] * $_SESSION[$cCode]["items4"][$id][$subid]["hpp_avg"];
                        $sisa_hpp_input = $_SESSION[$cCode]["items"][$id]["sub_hpp_avg"] - $_SESSION[$cCode]["items4"][$id][$subid]["subtotal"];
                        if ($sisa_hpp_input > 0) {
                            foreach ($_SESSION[$cCode]["items4"][$id] as $ssID => $ssSpec) {
                                if (($ssID != $subid)) {
                                    $_SESSION[$cCode]["items4"][$id][$ssID]["harga"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                    $_SESSION[$cCode]["items4"][$id][$ssID]["hpp_avg"] = $sisa_hpp_input / $_SESSION[$cCode]["items4"][$id][$ssID]["jml"];
                                    $_SESSION[$cCode]["items4"][$id][$ssID]["sub_hpp_avg"] = $sisa_hpp_input;
                                    $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"] = $sisa_hpp_input;
                                    $sisa_hpp_input -= $_SESSION[$cCode]["items4"][$id][$ssID]["subtotal"];
                                }
                            }
                        }
                        cekHitam("[$sisa_hpp_input]");
                    }

                }
            }
        }

        //-----------------------------------------------------
        $dtime_now = dtimeNow();
        $dtime_now_ex = explode(" ", $dtime_now);
        $date_now = str_replace("-", "", $dtime_now_ex[0]);
        $time_now = str_replace(":", "", $dtime_now_ex[1]);
        $bookingNumber = "$date_now" . "$time_now";
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"]) || ($_SESSION[$cCode]["main"]["bookingNumber"] == null)) {
            $_SESSION[$cCode]["main"]["bookingNumber"] = $bookingNumber;
        }
        //-----------------------------------------------------
        if ($_SERVER['REMOTE_ADDR'] == "202.65.117.72") {
//            arrPrintPink($_SESSION[$cCode]["itemSisa"]);
//            unset($_SESSION[$cCode]["itemSisa"]);
//            mati_disini(__LINE__);
        }


        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
            "sellerName" => my_name(),
            "placeID" => my_cabang_id(),
            "placeName" => my_cabang_nama(),
            "divID" => my_div_id(),
            "divName" => my_div_nama(),
            "cabangID" => my_cabang_id(),
            "cabangName" => my_cabang_nama(),
            "gudangID" => my_gudang_id(),
            "gudangName" => my_gudang_nama(),
            "jenis_usaha" => my_jenis_usaha(),
            "tokoID" => my_toko_id(),
            "tokoNama" => my_toko_nama(),
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUiJenis['steps'][1]['target'],
            "jenisTrName" => $this->configUiJenis['steps'][1]['label'],
            "stepNumber" => 1,
            "stepCode" => $this->configUiJenis['steps'][1]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);

        /* --------------------------------------------------
         * ngereload shoping cart dlm modul
         * --------------------------------------------------*/
        echo "<script>";
        echo "  if(top.document.getElementById('shopping_cart')){";
        echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "  }";
        echo "</script>";
    }
}


