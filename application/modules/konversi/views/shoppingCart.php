...
if (isset($items)) {
    ...
    if(top.document.getElementById('btnProcess')){
        top.$('#btnProcess').prop('disabled', false);
    }
    </script>";
...
//                arrPrintHijau($main);
        ...
        /*=============== BADGE PPN / NON PPN =================*/
        echo "<table class='table dataTable compact table-condensed no-padding table-bordered no-margin stripe'>";
        /*===============header shoping cart======================*/
        if (isset($itemLabels)) {
...
//            cekHitam($no);
            if ($showItems) {
                ...
                $int_colid = $iID . "_" . $no;//biar gak duplikasi id tablenya kalau hanya variable no pasti duplikasi
                $bgColor = "transparent";
                ...
                $bgColor_tr = "#c5e9ff";
                echo "<tr style='background-color:$bgColor_tr;font-weight: 700;' id='tr_" . $iSpec['id'] . "' bgcolor=$bgColor>";
                ...
                if (isset($itemLabels['jml'])) {
                    $qtyParam = "+removeCommas(document.getElementById('jml_$int_colid').value)";
                }
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    if (in_array($key, $editableFields)) {
                        ...
                        $readOnly[$key] = "readonly_xxz";
                        $listMode[$key] = "text";
                        $backColor[$key] = "$bgColor_tr";
                    }
                    $colID[$key] = $key . "_" . $int_colid;
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    echo "<td align='left'>";
                    $colID = $key . "_" . $int_colid;
                    $keyID = $key;
                    $noID = $int_colid;
                    $tabIndexNum = $colCtr . $int_colid;
                    $fieldVal = "";
                    if (substr($key, 0, 1) == "*") {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            ...
                            echo "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control form-control-sm text-right' 
                            style='color:$color;' value='" . niceDecimal($fieldVal) . "' 
                        ...
                        case "text":
                            $add_data_val = "";
                            $back_color = "";
                            if (isset($backColor[$key])) {
                                $back_color = $backColor[$key];
                            }
                            if (is_array($label)) {
                                $add_data_val = isset($iSpec[$label["addKey"]]) ? "<br>" . $iSpec[$label["addKey"]] : "";
                            }
                            if (is_numeric($fieldVal)) {
                                echo "<span keyid=$keyID noid=$noID id=$colID class='form-control form-control-sm text-right' style='color:$color;background:#f0f0f0;background-color:$back_color;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                            }
                            else {
                                if (strlen($fieldVal) > 10) {
                                    echo "<span keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;background-color:$back_color;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
                                }
                                else {
                                    echo "<span keyid=$keyID noid=$noID id=$colID class='form-control form-control-sm ' style='color:$color;border:0px;background-color:$back_color;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
                                }
                            }
                            break;
                    }
                ...
                if (!$avoidRemove) {
                    echo "<td width='1%'>";
                    echo "<a class='btn btn-xs btn-danger' title='remove this item' data-toggle='tooltip' data-placement='left' onclick=\"document.getElementById('result').src='" . $iSpec['removeTarget'] . "';\"><span class='glyphicon glyphicon-remove'></span></a>";
                    echo "</td>";
                }
                //endregion
                ...
                if ($pairedItemEnabled == true) {
                    if (!isset($pairedItemTarget[$iID])) {
                        $bgcolorwarning = "background-color:red;";
                        $pairedItemNoTarget[$iID] = $iID;
                    }
                    else {
                        $bgcolorwarning = "";
                    }
                    ...
                    echo "<td line='" . __LINE__ . "' colspan='" . $c_colspan . "' style='$bgcolorwarning'>";
                    //==pairedItems, if any
                    if (isset($selItems) && sizeof($selItems) > 0) {
                        echo "<select line='" . __LINE__ . "'
...
//arrPrintKuning($iSpec);
                                ...
                                if (isset($itemLabels2['jml'])) {
                                    $qtyParam = "+removeCommas(document.getElementById('jml_$an').value)";
                                }
                                foreach ($itemLabels2 as $key => $label) {
                                    $listMode[$key] = "input";
                                    $keyupEvent[$key] = "";
...
//                                            }
                            ...
                            echo "<tr class='bg-grey-2 text-uppercase'>";
                            echo "<th colspan='$colspan' class='text-muted text-center'>";
                            echo "<div class='alert alert-danger'>" . $pairedItemBreakdown["warning"] . "</div>";
                            echo "</th>";
                            echo "</tr>";
                            $arrProdukKonversiNoPart[$iID] = 1;
                        }
...
//                arrPrintWebs($konversiItems);
                ...
                if ($pairedItemPotongEnabled == true) {
                    if (!isset($pairedItemTargetPotong[$iID])) {
                        $bgcolorwarning = "background-color:red;";
                        $pairedItemNoTargetPotong[$iID] = $iID;
                    }
                    else {
                        if (sizeof($pairedItemTargetPotong[$iID]) > 0) {
                            $bgcolorwarning = "";
                        }
                        else {
                            $bgcolorwarning = "background-color:red;";
...
//arrPrintKuning($iSpec);
                                ...
                                $qtyParam = "";
                                if (isset($itemLabels2['jml'])) {
                                    $qtyParam = "+removeCommas(document.getElementById('jml_$nor').value)";
                                }
                                foreach ($itemLabels2 as $key => $label) {
                                    $listMode[$key] = "input";
                                    $keyupEvent[$key] = "";
...
//                                            }
        ...
        if (isset($items2) && sizeof($items2) > 0) {
            ...
            if (isset($pairedItem["warningShow"]) && ($pairedItem["warningShow"] == true)) {
                echo "<tr class='bg-info'>";
                echo "<td colspan='$jmlKolomHeader'>";

                echo "<div class='panel no-margin alert alert-danger' style='font-size:15px;'>"; // anakan table
                echo "<span class='fa fa-exclamation-triangle'></span>&nbsp;&nbsp;";

                echo $pairedItem["warning"];
...
//            echo "</div>";
        ...
        if (isset($arrProdukKonversiNoPart) && (sizeof($arrProdukKonversiNoPart) > 0)) {
            echo "<script>
                if(top.document.getElementById('btnProcess')){
        ...
        if (isset($pairedItemNoTarget) && (sizeof($pairedItemNoTarget) > 0)) {
            cekHere("konversi 1 to 1 ...");
            echo "<script>
            if(top.document.getElementById('btnProcess')){
                top.$('#btnProcess').prop('disabled', true);
            }
        ...
        if (isset($pairedItemNoTargetPotong) && (sizeof($pairedItemNoTargetPotong) > 0)) {
            cekHere("konversi potong...");
            echo "<script>
            if(top.document.getElementById('btnProcess')){
                top.$('#btnProcess').prop('disabled', true);
            }
