...
//                cekHitam(":: $nilai ::");
                if (isset($vvSpec["link_mutasi"])) {
                    $link_mutasi = base_url() . $vvSpec["link_mutasi"] . $mainData[$vvSpec["key_id"]] . "?o=" . $mainData[$vvSpec["cabang_id"]];
                    cekHere($link_mutasi);
                    $$kkey = "<a href='$link_mutasi' target='_blank'>
                        <span id='$kkey' class='btn btn-info text-uppercase'> $label: $nilai </span>
                        </a>";
                }
                else {
...
//        cekHere($pettycash_plafon);
        ...
        $p->addTags(
            array(
                ...
                //--------------
                "pettycash_plafon" => isset($pettycash_plafon) ? $pettycash_plafon : "",
                "pettycash_saldo" => isset($pettycash_saldo) ? $pettycash_saldo : "",
                "pettycash_terpakai" => isset($pettycash_terpakai) ? $pettycash_terpakai : "",
//                "pettycash_plafon" => "",
//                "pettycash_saldo" => "",
//                "pettycash_terpakai" => "",
                //--------------
                    ...
                    if (array_key_exists($key, $inputOptions)) {
                        ...
                        if (isset($inputOptions[$key]["dataField"]) && (sizeof($inputOptions[$key]["dataField"]) > 0)) {
                            ...
                            foreach ($inputOptions[$key]["dataField"] as $sels => $seDetails) {
                                ...
                                $str .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                                        <input type='radio' name='switch_ppn_item_$iID' value='$selsValue' $checked disabled
                                        oonclick=\"$('#result').load('" . MODUL_PATH . "$processSelct$jenisTr?id=$iID&ppn='+this.value+'&ppnTargetItems=$selsItemsTarget&ppnTargetMain=$selsMainTarget&overWriteMain=$mainOverwrite');\"
                                        >
                                        <span>$selsLabel</span>
                                        </label>";
                            }
...
//                                    $faktur .= "</tr>";
                ...
                if (($noteEnabled == true) || ((isset($imageEnabled)) && ($imageEnabled == true))) {
                    ...
                    if (isset($iSpec['note']) && strlen($iSpec['note']) > 1) {
                        $iVal = isset($iSpec['note']) ? $iSpec['note'] : "<span class='meta'>tidak ada catatan</span>";
                        echo $iVal;
                    }
                    else {
                        echo "<span class='meta'>tidak ada catatan</span>";
                    }
...
//                    echo $iSpec[$key];
                    ...
                    if (count($shopingCartFakturItems) > 0) {
                        if (isset($showFormulirFakturRefillMode[$iID]) && ($showFormulirFakturRefillMode[$iID] == true)) {
                            ...
                            $faktur .= "<td>&nbsp;</td>";
                            $faktur .= "<td colspan='" . (6) . "'>";
                            $faktur .= "<table class='table dataTable compact table-bordered table-condensed'>";
                            $faktur .= "<thead>";
                            $faktur .= "<tr class='bg-primary'>";
                            foreach ($shopingCartFakturParam["fields"] as $ff => $ff_abels) {
                                $faktur .= "<th>$ff_abels</th>";
                                ...
                                if (sizeof($formulirFaktur) > 0) {
                                    ...
                                    foreach ($formulirFaktur as $ctt => $fSpec) {
                                        if ($iID == $ctt) {
                                            ...
                                            foreach ($shopingCartFakturParam["fields"] as $fff => $f_labels) {
                                                $value = "<span class='no-border no-shadow'>" . formatField($fff, $fSpec[$fff]) . "</span>";
                                                $faktur .= "<td >$value <span class='text-danger text-bold text-blink'> $labels </span>";
                                                $faktur .= "</td>";
                                            }
                                            $faktur .= "</tr>";
                                        }
...
//                            cekKuning(":: $key editable ::");
                            if (is_numeric($val)) {
                                ...
                                if (!$allowIncrement) {
                                    ...
                                    $addEvent = " onkeyup=\"this.value=addCommas(this.value);\"  oonblur=\"top.$('#result').load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val='+removeCommas(this.value))\" onmouseout=\"top . $('#result') . load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val=' + removeCommas(this . value))\" ";
                                }
                                ...
                                if ($pakai_ini == 1) {
                                    ...
                                    foreach ($shopingCartFakturParam["fields"] as $fff => $f_labels) {
                                        if (isset($shopingCartFakturParam["editableFields"][$fff])) {
                                            ...
                                            $value = "<input type='$inputType' ids='$fff' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name)\">";
                                        }
                                        else {
                                            $value = formatField($fff, $shopingCartFakturItems[$fff]);
                                        }
                                        $faktur .= "<td ids='td_$fff'>$value <span class='text-danger text-bold text-blink'>$labels </span></td>";
                                    }
                                    $faktur .= "</tr>";
                                    //-----------------------------
                                }
...
//                            echo $mainValues[$key . "_tax"];
                ...
                $no = 0;
                if (isset($showAddMain) && ($showAddMain == true)) {
...
//        $cPrint    .= "<SMALL>------------------------------------------";
    ...
    case "selectPaymentSrc":
        ...
        if (sizeof($items) > 0) {
            ...
            $strContent .= "<table id='tbl_pettycash' class='table cTable-modal table-condensed table-responsive no-padding table-bordered'>";
            ...
            foreach ($items as $key => $val) {
                ...
                if (sizeof($itemLabels) > 0) {
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        $strContent .= "<td  class='" . $val['class_bg'] . "' data-order='$data_o'>";
                        if (isset($val[$key])) {
                            $strContent .= strlen($val[$key]) > 0 ? formatField_he_format($key, $val[$key]) : "-";
                            if (is_numeric($val[$key])) {
                                if (!isset($total[$key])) {
                                    $total[$key] = 0;
                                }
...
<input type=button class='btn btn-success btn-block' id='btnSave' value='$btnLabel' disabled 
            ...
            $strContent .= "<script>
                    ...
                    $(document).ready( function(){
                        ...
                        var table = $('#tbl_pettycash').DataTable({
                            dom: 'lBfrtip',
                            stateSave: false,
                            order: [[ 5, 'asc' ]],
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            ...
                            footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var arrayFooter = $('#tbl_pettycash>tfoot>tr>th');
                                        var dpageTotal = [];
                                        jQuery.each(arrayFooter, function(i,d){
...
//        if (isset($scriptBottom)) {
        $strContent .= "<script>  $('.modal-dialog').removeClass('modal-lg').addClass('modal-xl');</script>";
//        }
