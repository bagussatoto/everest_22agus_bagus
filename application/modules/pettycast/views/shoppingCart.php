                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            ...
                            if (isset($keyupAction) && $keyupAction == true) {
                                ...
                                echo "onkeyup=\"$('#shopping_cart').trigger('change');if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }\"";
                            }
                            echo ">";
                        ...
                        case "text":
                            ...
                            break;
                        case "combo":
//                            arrPrint($inputOptions[$key]);
                            if (isset($inputOptions[$key]["dataField"]) && (sizeof($inputOptions[$key]["dataField"]) > 0)) {
                                $processSelct = $inputOptions[$key]["processSelect"];
                                $selsMainTarget = $seDetails['ppn']['srcMain'];
                                $selsItemsTarget = $seDetails['ppn']['srcItem'];
                                $mainOverwrite = $seDetails['ppn']['overWriteMain'];
                                $jenisTr = isset($arrHeaderElementJenis) ? $arrHeaderElementJenis : "";
                                echo "<select name='switch_ppn' 
                                onchange=\"$('#result').load('" . MODUL_PATH . "$processSelct$jenisTr?id=$iID&ppn='+this.value+'&ppnTargetItems=$selsItemsTarget&ppnTargetMain=$selsMainTarget&overWriteMain=$mainOverwrite');\">";
                                echo "<option>--- pilih disini ---</option>";
                                foreach ($inputOptions[$key]["dataField"] as $sels => $seDetails) {
                                    $selsLabel = $seDetails['label'];
                                    $selsValue = $seDetails['value'];
                                    $selsMainTarget = $seDetails['srcMain'];
                                    $selsItemsTarget = $seDetails['srcItem'];
                                    $mainOverwrite = $seDetails['overWriteMain'];
                                    $currentPPN = isset($iSpec[$selsMainTarget]) ? $iSpec[$selsMainTarget] : 0;
                                    $ppnPersenItems = isset($iSpec['ppnVendor']) ? $iSpec['ppnVendor'] : 0;
//                                    cekHere("[$selsValue == $currentPPN]");
//                                    $autoTerapkan = ($ppnPersenItems != $currentPPN) && ($selsValue == $currentPPN) ? true : false;
                                    $checked = ($selsValue == $currentPPN) ? "selected" : "";
                                    $jenisTr = isset($arrHeaderElementJenis) ? $arrHeaderElementJenis : "";
                                    echo "<option value='$selsValue' $checked>$selsLabel</option>";
                                }
                                echo "</select>";
                            }
                            break;
...
//                            arrPrint($inputOptions[$key]);
                            if (isset($inputOptions[$key]["dataField"]) && (sizeof($inputOptions[$key]["dataField"]) > 0)) {
                                ...
                                foreach ($inputOptions[$key]["dataField"] as $sels => $seDetails) {
                                    ...
                                    $selsItemsTarget = $seDetails['srcItem'];
                                    $mainOverwrite = $seDetails['overWriteMain'];
                                    $currentPPN = isset($iSpec[$selsMainTarget]) ? $iSpec[$selsMainTarget] : NULL;
                                    $ppnPersenItems = isset($iSpec['ppnVendor']) ? $iSpec['ppnVendor'] : 0;
//                                    $autoTerapkan = ($ppnPersenItems != $currentPPN) && ($selsValue == $currentPPN) ? true : false;
                                    $checked = ($selsValue == $currentPPN) ? "checked" : "";
//                                    $checked_2 = ($selsValue == $iSpec[$selsMainTarget]) ? "checked" : "";
//                                    cekHere("[$selsValue == $currentPPN] [$selsMainTarget] [$checked]  || ==> " . $iSpec[$selsMainTarget]);
                                    $jenisTr = isset($arrHeaderElementJenis) ? $arrHeaderElementJenis : "";
                                    echo "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                                        <input type='radio' name='switch_ppn_item_$iID' value='$selsValue' $checked 
                                        onclick=\"$('#result').load('" . MODUL_PATH . "$processSelct$jenisTr?id=$iID&ppn='+this.value+'&ppnTargetItems=$selsItemsTarget&ppnTargetMain=$selsMainTarget&overWriteMain=$mainOverwrite');\"
                                        >
                                        <span>$selsLabel</span>
                                        </label>";
                                }
...
//                        $faktur .= "<div class='panel panel-default' style=' margin-top: 10px;'>";
                            $faktur .= "<table class='table dataTable compact table-bordered'>";
//                        $faktur .= "<thead>";
                            $faktur .= "<thead>";
...
//arrPrint($formulirFaktur);
                                    $faktur .= "<tbody>";
                                    foreach ($formulirFaktur as $ctt => $fSpec) {
                                        if ($iID == $ctt) {
                                            $faktur .= "<tr iID='$iID' ctt='$ctt' line='" . __LINE__ . "'>";
                                            foreach ($shopingCartFakturParam["fields"] as $fff => $f_labels) {
                                                $linkFaktur = MODUL_PATH . $shopingCartFakturTarget . "/";
                                                $labels = "";
                                                $btn_formulir = "";
                                                ...
                                                if (isset($shopingCartFakturParam["editableFields"][$fff])) {
                                                    ...
                                                    if ($shopingCartFakturParam["editableFields"][$fff] == "checkbox") {
                                                        ...
                                                        $value = ($ctt == 0) ? "<input type='$inputType' ids='$fff' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name,$ctt)\">" : "";
                                                        ...
                                                        $vals = "value";
                                                        $checked = "";
                                                        $value = "<input type='$inputType' ids='$fff' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name,$ctt)\">";
                                                    }
                                                }
                                                else {
                                                    $value = "<span class='form-control no-border no-shadow'>" . formatField($fff, $fSpec[$fff]) . "</span>";
                                                }
                                                $faktur .= "<td ids='td_$fff" . "_" . $iID . "'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                                                $faktur .= $btn_formulir;
...
//                                    </script>";
                    ...
                    if (count($shopingCartFakturItems) > 0) {
                        if (isset($showFormulirFaktur[$iID]) && ($showFormulirFaktur[$iID] == true)) {
                                    ...
                                    foreach ($formulirFaktur as $ctt => $fSpec) {
                                        if ($iID == $ctt) {
                                            ...
                                            foreach ($shopingCartFakturParam["fields"] as $fff => $f_labels) {
                                                ...
                                                if (isset($shopingCartFakturParam["editableFields"][$fff])) {
                                                    $inputType = $shopingCartFakturParam["editableFields"][$fff];
                                                    $maxType = ($inputType == "date") ? "max=".date("Y-m-d") : "";
                                                    $defValues = isset($fSpec[$fff]) ? $fSpec[$fff] : "";
                                                    if ($shopingCartFakturParam["editableFields"][$fff] == "checkbox") {
                                                        $classinputType = "";
                                                        $labels = ($ctt == 0) ? "tic disini jika faktur belum tersedia" : "";
                                                        $vals = "checked";
                                                        $checked = (isset($fSpec[$fff]) && $fSpec[$fff] == "true") ? $vals : "";
                                                        $value = ($ctt == 0) ? "<input type='$inputType' ids='$fff' $maxType class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name,$ctt)\">" : "";
                                                        $btn_formulir = (($ctt == 0) && ($countItems > 1)) ? "<button class='btn btn-warning' onclick=\"$('#result').load('$cloneFormulirFaktur');\"><span class='glyphicon glyphicon-plus'></span> Tambah Formulir Faktur</button>" : "";
                                                        $btn_formulir_delete = ($ctt > 0) ? "<button class='btn btn-danger' onclick=\"$('#result').load('$cloneFormulirFakturDelete/$ctt');\"><span class='glyphicon glyphicon-trash'></span></button>" : "";
                                                    }
                                                    else {
                                                        ...
                                                        $vals = "value";
                                                        $checked = "";
                                                        $value = "<input type='$inputType' $maxType ids='$fff' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name,$ctt)\">";
                                                    }
                                                }
                                                else {
                                                    $value = "<span class='form-control no-border no-shadow'>" . formatField_he_format($fff, $fSpec[$fff]) . "</span>";
                                                }
                                                $faktur .= "<td ids='td_$fff" . "_" . $iID . "'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                                                $faktur .= $btn_formulir;
                                                $faktur .= $btn_formulir_delete;
...
//                                    </script>";
                ...
                if ($noteEnabled == true) {
                    ...
                    if (isset($noteType)) {
                        switch ($noteType) {
                            case "textarea":
                                echo "<textarea id='note_$iID' class='form-control' placeholder='write notes here'
                            ...
                            case "text":
                            default:
                                echo "<input onfocus='select()' id='note_$iID' type=text class='form-control' value='$noteVal' placeholder='write notes here'
                                onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $noteRecorder . "?val='+encodeURIComponent(this.value)+'&iid=$iID';}\"
                                *onmouseout=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $noteRecorder . "?val='+encodeURIComponent(this.value)+'&iid=$iID';}\"
                                >";
                                break;
                        ...
                        case "radio":
                            ...
                            if (isset($inputOptions[$key]["dataField"]) && (sizeof($inputOptions[$key]["dataField"]) > 0)) {
                                ...
                                foreach ($inputOptions[$key]["dataField"] as $sels => $seDetails) {
                                    ...
                                    $checked = ($selsValue == $currentPPN) ? "checked" : "";
                                    $jenisTr = isset($arrHeaderElementJenis) ? $arrHeaderElementJenis : "";
//                                    cekHere("[$selsValue == $currentPPN] [$selsMainTarget] [$selsItemsTarget] [$selsMainTarget] [$mainOverwrite] [$iID] [$processSelct] [$jenisTr] ");
                                    $isi .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                                        <input type='radio' name='switch_ppn_item_$iID' value='$selsValue' $checked 
                                        onclick=\"$('#result').load('" . MODUL_PATH . "$processSelct$jenisTr?id=$iID&ppn='+this.value+'&ppnTargetItems=$selsItemsTarget&ppnTargetMain=$selsMainTarget&overWriteMain=$mainOverwrite');\"
                                        >
                                        <span>$selsLabel</span>
                ...
                $faktur = "";
                if (count($shopingCartFakturItems) > 0) {
                    if (isset($showFormulirFakturRefillMode[$iID]) && ($showFormulirFakturRefillMode[$iID] == true)) {
                        $faktur .= "<tr>";
                        $faktur .= "<td>&nbsp;</td>";
                        $faktur .= "<td colspan='" . (6) . "'>";
                        $faktur .= "<table class='table dataTable compact table-bordered'>";
                                        ...
                                        foreach ($shopingCartFakturParam["fields"] as $fff => $f_labels) {
                                            ...
                                            if (isset($shopingCartFakturParam["editableFields"][$fff])) {
                                                ...
                                                if ($shopingCartFakturParam["editableFields"][$fff] == "checkbox") {
                                                    ...
                                                    $vals = "checked";
                                                    $checked = isset($fSpec[$fff]) && $fSpec[$fff] == "true" ? $vals : "";
                                                    $value = ($ctt == 0) ? "<input type='$inputType' $maxType ids='$fff' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name,$ctt)\">" : "";
                                                    $btn_formulir = (($ctt == 0) && ($countItems > 1)) ? "<button class='btn btn-warning' onclick=\"$('#result').load('$cloneFormulirFaktur');\"><span class='glyphicon glyphicon-plus'></span> Tambah Formulir Faktur</button>" : "";
                                                    $btn_formulir_delete = ($ctt > 0) ? "<button class='btn btn-danger' onclick=\"$('#result').load('$cloneFormulirFakturDelete/$ctt');\"><span class='glyphicon glyphicon-trash'></span></button>" : "";
                                                }
                                                else {
                                                    switch ($inputType) {
                                                        case "combo":
                                                            $selectorTarget = "'" . $linkFaktur . "$iID/?get='+this.value";
                                                            $value = "<select class='form-control' onchange=\"document.getElementById('result').src=$selectorTarget;\">";
//                                                            $value = "<select class='form-control' onchange=\"eksekutor2(this.value,this.name,this.key,$ctt);\">";
                                                            $value .= "<option value=''>-select-</option>";
                                                            if (isset($pairData) && (sizeof($pairData) > 0)) {
                                                                foreach ($pairData as $iidd => $iiData) {
                                                                    $nama = $iiData["nama"];
                                                                    $key = $iiData["key"];
                                                                    $selected = ($iidd == $fSpec[$iiData["key"]]) ? "selected" : "";
                                                                    $arrSub = array(
                                                                        "id" => $iidd,
                                                                        "nama" => $nama,
                                                                        "key" => $key,
                                                                    ...
                                                                    $subEncode = blobEncode($arrSub);
                                                                    $value .= "<option value='$subEncode' $selected>" . $nama . "</option>";
                                                                }
                                                            }
                                                            $value .= "</select>";
                                                            break;
...
//                                                            cekHere("[$fff]");
                                                            ...
                                                            $vals = "value";
                                                            $checked = "";
                                                            $value = "<input type='$inputType' $maxType ids='$fff' class='$classinputType' name='$fff' 
                                                            onclick='this.select()' value='$defValues' $checked 
                                                            onblur=\"eksekutor(this.$vals,this.name,$ctt)\">";
                                                            break;
                                                    }
...
//                                                cekHere("[$fff] || " . $fSpec[$fff]);
                                                $value = "<span class='form-control no-border no-shadow'>" . formatField_he_format($fff, $fSpec[$fff]) . "</span>";
                                            }
                                            $faktur .= "<td ids='td_$fff'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                                            $faktur .= $btn_formulir;
                                            $faktur .= $btn_formulir_delete;
                                            $faktur .= "</td>";
                                            if (is_numeric($fSpec[$fff])) {
...
//                                    $faktur .= "</tr>";
                                ...
                                $faktur .= "<script>
                                                ...
                                                function eksekutor2(nilai, nama, key, ctt){
                                                    $('#result').load('$linkFaktur'+ctt+'?nilai='+nilai+'&nama='+nama)
                                                }
                                            </script>";
                            }
...
//                    cekHere("tidak ada faktur items");
                }
                echo $faktur;
...
//                arrPrint($addRowHiddens[$k]);
        ...
        if ((!$avoidRemove) || (!$avoidRemoveAll_items)) {
            $tombol_delete .= "<span class='pull-left'>";
            $tombol_delete .= "<a class='btn btn-sm btn-danger text-bold' href='javascript:void(0);' title='remove ALL ITEMS' data-toggle='tooltip' data-placement='right' onclick=\"confirm_alert_result('Attention !!!','Remove all items on shopping cart?','$resetLink','YES CLEAR');\"><i class='fa fa-trash'> </i> Clear Shoping Cart</a>";
            $tombol_delete .= "</span>";
        }
        //endregion
        ...
        //tombol refresh
//        $tombol_reload = "";
//        $tombol_reload .= "<span class='btn_calculate pull-rights'>";
//        $tombol_reload .= "<a class='btn btn-sm btn-info text-bold' href='javascript:void(0);' title='recalculate all' onclick=\"$('.glyphicon.glyphicon-refresh').parent().click()\"><i class='fa fa-calculator'></i> CALCULATE</a>";
//        $tombol_reload .= "</span>";
        //tombol refresh
...
//            echo "</div>";
        ...
        echo "<script>
                ...
                if (typeof top.initShoppingCart === 'function') {
                    top.initShoppingCart();
                }
                </script>";
        ...
        if (isset($items) && (sizeof($items) > 0)) {
            ...
            $link = "$noteRecorder?val=";
            foreach ($items as $iiSpec) {
                $iid = $iiSpec['id'];
                foreach ($arrGet as $get) {
                    $get_idx = $get . "_" . $iid;
                    echo "<script>
                        //var note = $('#$get_idx').val();
//                       console.log($get_idx);
//                       console.log(note);
//                         $('#result').load('$link'+encodeURIComponent(note)+'&iid=$iid&reload=0');
                    </script>";
...
if (isset($mainData)) {
    if (isset($pettycashHeader)) {
        foreach ($pettycashHeader as $kkey => $vvSpec) {
            $label = isset($vvSpec["label"]) ? $vvSpec["label"] : "";
            $nilai = isset($mainData[$vvSpec["key"]]) ? $mainData[$vvSpec["key"]] : 0;
            $nilai = number_format($nilai, 0, ",", ".");
            $text = "$label: $nilai";
//            $$kkey = "<span id='$kkey' class='btn btn-info text-uppercase'> $label: $nilai </span>";
            echo "<script>$('#$kkey').text('$text');</script>";
        }
    }
}
