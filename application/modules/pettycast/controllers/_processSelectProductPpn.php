...
//        }
    ...
    public function select()
    {
        ...
        if (isset($_SESSION[$cCode]['items'])) {
            $newPpn = $this->ppnFactor * $ppn;// ikut sesi login prosentase ppn nya, sebelumnya ditembak 10%
        ...
        //-----------------------------------------------------
        if ($_SESSION[$cCode]['main'][$ppnTargetMain] == 0) {
            ...
            if (isset($shopingCartFaktur["gateTarget"])) {
                ...
                if (isset($_SESSION[$cCode][$gateTarget]) && (sizeof($_SESSION[$cCode][$gateTarget]) > 0)) {
//                    $_SESSION[$cCode][$gateTarget] = NULL;// kenapa items dibuang ya?, saat diklik NON PPN
...
//mati_disini("[$cCode] [$id] [$ppn] [$ppnTargetItems] [$ppnTargetMain] [$overWriteVendor]");
...
//                $_SESSION[$cCode]['main'][$overWriteVendor] = $newPpn;
//            }
            cekHere("[$ppnTargetItems] [$overWriteVendor] [$newPpn]");
            foreach ($_SESSION[$cCode]['items'][$id] as $aaaaaaaaaaaaaaa) {
                $_SESSION[$cCode]['items'][$id][$ppnTargetItems] = $newPpn;
                $_SESSION[$cCode]['items'][$id][$overWriteVendor] = $newPpn;
                // tambahan ppnPersenCheck yang berisi 0 atau 1 sesuai dengan yang dipilih... (non PPN, PPN)
                $_SESSION[$cCode]['items'][$id][$ppnTargetMain] = $ppn;
...
//        }
    ...
    // untuk refill pettycash, (opsi nonppn/ppn) berada di items2_sum
    public function selectItemRefill()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();
        $id = $_GET['id'];
        ...
        if (isset($_SESSION[$cCode]['items2_sum'])) {
            $newPpn = $this->ppnFactor * $ppn;// ikut sesi login prosentase ppn nya, sebelumnya ditembak 10%
            cekHere("[$ppnTargetItems] [$overWriteVendor] [$newPpn]");
            foreach ($_SESSION[$cCode]['items2_sum'][$id] as $aaaaaaaaaaaaaaa) {
                $_SESSION[$cCode]['items2_sum'][$id][$ppnTargetItems] = $newPpn;
                $_SESSION[$cCode]['items2_sum'][$id][$overWriteVendor] = $newPpn;
                // tambahan ppnPersenCheck yang berisi 0 atau 1 sesuai dengan yang dipilih... (non PPN, PPN)
                $_SESSION[$cCode]['items2_sum'][$id][$ppnTargetMain] = $ppn;
