...
//        echo ($time_line);
        ...
        //-----------------------------------------------------
        $arrJenisTrBiayaCek = array("671", "1671");
        if (in_array($jenisTr, $arrJenisTrBiayaCek)) {
            $link_pihak = MODUL_PATH . "_processPihak/select/$jenisTr/MdlCabang?id=" . CB_ID_PUSAT . "&no_load=1";
            $scriptBottomBiaya = "
...
//            $str_scriptBottom .= " \n top.$('#kepoinList').load('$link_kepoinList')";
            ...
            $str_scriptBottom .= " \n
                         ...
                         var kepoinList = function () {
                         top.$('#kepoinList').html(\"<div class='row' style='margin-top: 14px; padding-bottom: 14px;'> <span class='col-lg-12 text-center'><img width='5%' src='" . base_url() . "assets/images/loading_16_p.gif'></span> </div>\");
                     setTimeout(function () {
                                      if (top.$('#kepoinList').find('img').length > 0) {
                                          top.$('#kepoinList').load('$link_kepoinList', function(){kepoinList})
                                          }
                                      },1000);
                     }\n
                    kepoinList();\n\n
        ...
        $link_historiList = base_url() . $this->modul . "/History/showData/$jenisTr/?limit=10";
//        $str_scriptBottom .= " \n top.$('#historyList').load('$link_historiList')";
        $str_scriptBottom .= " \n

                                 var historyList = function () {
                         top.$('#historyList').html(\"<div class='row' style='margin-top: 14px; padding-bottom: 14px;'> <span class='col-lg-12 text-center'><img width='5%' src='" . base_url() . "assets/images/loading_16_p.gif'></span> </div>\");
                     setTimeout(function () {
                                      if (top.$('#historyList').find('img').length > 0) {
                                          top.$('#historyList').load('$link_historiList', function(){historyList})
                                          }
                                      },1000);
                     }\n
                    historyList();\n\n
        ...
        if (isset($scriptBottomBiaya)) {
            $str_scriptBottom .= $scriptBottomBiaya;
        }


        //-------------------------------------------------------------------------------
        $pettycashHeader = isset($this->configUi[$this->jenisTr]['pettycashHeader']) ? $this->configUi[$this->jenisTr]['pettycashHeader'] : array();
        //-------------------------------------------------------------------------------
        ...
        $data = array(
            ...
            "mainData" => isset($_SESSION[$cCode]["main"]) ? $_SESSION[$cCode]["main"] : array(),
            "pihak_disabled" => isset($this->configUi[$jenisTr]["pihakDisabled"]) ? $this->configUi[$jenisTr]["pihakDisabled"] : "",
            "time_line" => $time_line,
        );
        //endregion
...
//        matiHere(__LINE__);
    ...
    public function preview()
    {
        ...
        $shopingCartFaktur = isset($configUi[$this->jenisTr]['addMainSource'][1]) ? $configUi[$this->jenisTr]['addMainSource'][1] : array();
        ...
        $shoppingCartEditableFieldsOptions = isset($this->configUi[$this->jenisTr]['shoppingCartEditableFieldsOptions'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartEditableFieldsOptions'][1] : array();
            ...
            if (isset($_SESSION[$cCode]['items2_sum'])) {
                foreach ($_SESSION[$cCode]['items2_sum'] as $iSpec) {
                    $tmp = array(
                        ...
                        "stok" => isset($iSpec['stok']) ? $iSpec['stok'] : "",
                        "stok_center" => isset($iSpec['stok_center']) ? $iSpec['stok_center'] : "0",
                        "sisa" => isset($iSpec['sisa']) ? $iSpec['sisa'] : 0,
                    );
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
        ...
        $itemSrcLabels = $itemSrcLabels + $itemSrcNumLabels + array("subtotal" => "subtotal");
        ...
        if ($pakai_ini == 1) {
            ...
            if (count($shopingCartFaktur) > 0) {
                if (isset($shopingCartFaktur["refillMode"]) && ($shopingCartFaktur["refillMode"] == true)) {
                    if (isset($shopingCartFaktur["gateTarget"])) {
                        $gateTarget = $shopingCartFaktur["gateTarget"];
                        $dataFaktur = $_SESSION[$cCode][$gateTarget];
                        $showRefillMode = true;
                    }
//                    $cekPPNRefill = isset($shopingCartFaktur["refillModePpn"]) ? $shopingCartFaktur["refillModePpn"] : NULL;
//                    $cekPPNRefill_nilai = isset($_SESSION[$cCode]["main"][$cekPPNRefill]) ? $_SESSION[$cCode]["main"][$cekPPNRefill] : 0;
//                    $showRefillMode = ($cekPPNRefill_nilai > 0) ? true : false;
...
//                $_SESSION[$cCode]["main"]["skip_faktur"]="true";
                        ...
                        foreach ($_SESSION[$cCode]["items"] as $pi => $val) {
                            foreach ($shopingCartFaktur["fields"] as $key_faktur => $faktur_labels) {
                                $dataFaktur[$pi][$key_faktur] = isset($_SESSION[$cCode]["items"][$pi][$key_faktur]) ? $_SESSION[$cCode]["items"][$pi][$key_faktur] : "";
                            }
                            if (isset($shopingCartFaktur["addFields"])) {
                                $skip_faktur = false;
                                if (isset($_SESSION[$cCode]["items"]["skip_faktur"]) && ($_SESSION[$cCode]["items"]["skip_faktur"] == "true")) {
                                    $skip_faktur = true;
                                }
                                foreach ($shopingCartFaktur["addFields"] as $key_faktur_add => $faktur_src_add) {
                                    $dataFaktur[$pi][$key_faktur_add] = (($skip_faktur == false) && isset($_SESSION[$cCode]["items"][$pi][$faktur_src_add])) ? $_SESSION[$cCode]["items"][$pi][$faktur_src_add] : "";
                                }
                            }
                        }
                    }
...
//                        $showRefillMode = true;
                    ...
                    if (isset($shopingCartFaktur["pairData"])) {
                        $mdlPair = $shopingCartFaktur["pairData"]["mdlName"];
                        $this->load->model("Mdls/$mdlPair");
                        $pp = New $mdlPair();
                        $ppTmp = $pp->lookupAll()->result();
                        $pairData = array();
                        if (sizeof($ppTmp) > 0) {
                            foreach ($ppTmp as $ppSpec) {
                                $pairData[$ppSpec->id] = array(
                                    "id" => $ppSpec->id,
                                    "nama" => $ppSpec->nama,
                                    "key" => $shopingCartFaktur["pairData"]["key"],
                                );
                            }
                        }
                    }
...
//                    $cekPPNRefill_nilai = isset($_SESSION[$cCode]["main"][$cekPPNRefill]) ? $_SESSION[$cCode]["main"][$cekPPNRefill] : 0;
//                    $showRefillMode = ($cekPPNRefill_nilai > 0) ? true : false;
                    foreach ($_SESSION[$cCode][$gateTarget] as $pi => $val) {
                        $showRefillMode[$pi] = false;
                        if (isset($val[$shopingCartFaktur["gateItemCek"]]) && ($val[$shopingCartFaktur["gateItemCek"]] == 1)) {
                            $showRefillMode[$pi] = true;
                        }
                    }
                }
                else {
                    ...
                    foreach ($_SESSION[$cCode]["items"] as $pi => $val) {
                        ...
                        if (isset($val[$shopingCartFaktur["gateItemCek"]]) && ($val[$shopingCartFaktur["gateItemCek"]] == 1)) {
                            ...
                            $showFakturItem[$pi] = true;
                        }
                        else {
                            if (isset($shopingCartFaktur["addFields"])) {
                                foreach ($shopingCartFaktur["addFields"] as $key_faktur_add => $faktur_src_add) {
                                    $dataFaktur[$pi][$key_faktur_add] = 0;
                                }
                            }
                            if (isset($shopingCartFaktur["editableFields"])) {
                                foreach ($shopingCartFaktur["editableFields"] as $key_faktur_add_edit => $faktur_src_add_edit) {
                                    $dataFaktur[$pi][$key_faktur_add_edit] = 0;
                                }
                            }
                        }
                    }
                    if (isset($shopingCartFaktur["gateTarget"])) {
                        $gateTarget = $shopingCartFaktur["gateTarget"];
                        foreach ($_SESSION[$cCode]["items"] as $pi => $val) {
                            if (!isset($_SESSION[$cCode][$gateTarget][$pi])) {
                                $_SESSION[$cCode][$gateTarget][$pi] = $dataFaktur[$pi];
                            }
                        }
                        foreach ($_SESSION[$cCode]["items"] as $pi => $val) {
                            if (!empty($_SESSION[$cCode][$gateTarget][$pi])) {
                                foreach ($dataFaktur[$pi] as $aa => $bb) {
                                    $_SESSION[$cCode][$gateTarget][$pi][$aa] = $bb;
                                }
                            }
                        }
                    }
...
//            "showFormulirFaktur" => isset($show) ? $show : false,
            "showFormulirFaktur" => isset($showFakturItem) ? $showFakturItem : array(),
            "formulirFaktur" => isset($_SESSION[$cCode][$gateTarget]) ? $_SESSION[$cCode][$gateTarget] : array(),
            //--------
            "showFormulirFakturRefillMode" => isset($showRefillMode) ? $showRefillMode : false,
            "pairData" => isset($pairData) ? $pairData : array(),
...
//        $trans_cabang_nama = isset($_SESSION[$cCode]["main"]["transaksi__cabang_nama"]) ? $_SESSION[$cCode]["main"]["transaksi__cabang_nama"] : 0;
        $stepCodeMain = $_SESSION[$cCode]["main"]["stepCode"];
        $stepCodeValidateEmployee = stepCodeByEmployeeID($stepCodeMain, my_id());

//        $new_login_cabang_id = "1";
//        $new_login_cabang_nama = "Jakarta";
//        $trans_cabang_id = "1";
//        $trans_cabang_nama = "Jakarta";
        cekHere("lokasi login: [$new_login_cabang_id] [$new_login_cabang_nama]");
        cekHere("lokasi gerbang main: [$trans_login_cabang_id] [$trans_login_cabang_nama]");
        cekHere("kode transaksi gerbang main: " . $_SESSION[$cCode]["main"]["stepCode"]);
...
//            mati_disini($msg);
//        }
        if ($stepCodeValidateEmployee == false) {
            $msg = "Transaksi gagal diselesaikan karena ";
            $msg .= "anda tidak memiliki hak akses " . $_SESSION[$cCode]["main"]["jenisTrName"] . ", code: " . $stepCodeMain;
                            ...
                            if (sizeof($tmpData) > 0) {
                                ...
                                $tr->writePaymentSrc($insertID, array(
                                    ...
                                    "extern_nilai2" => (isset($externSrc['extern_nilai2']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai2']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai2']] : 0,
                                        ...
                                        "extern_date2" => makeValue($externSrc['extern_date2'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                        "dpp_ppn" => makeValue($externSrc['dpp_ppn'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
...
//                        matiHere($externSrc_id."::".$externSrc['id']);
