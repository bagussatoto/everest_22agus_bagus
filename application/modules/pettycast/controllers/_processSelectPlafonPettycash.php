...
class _processSelectPlafonPettycash extends Modul_Controller
{
...
//arrPrint($tmpB);
        ...
        $ba = new MdlBankAccount();
        $ba->setFilters(array());
        $ba->addFilter("cabang_id='$id'");
        $ba->addFilter("jenis='pettycash'");
        ...
        if (sizeof($tmpBa) > 0) {
            ...
            die(lgShowAlert("account pettycash " . $tmpB[0]->nama . " belum didefine. segra hubungi admin."));
        }
//arrPrint($tmpBa);
//mati_disini($this->db->last_query());

        //  region baca plafon dan saldo sisa pettycash 1010010040
        $this->load->model("Coms/ComRekeningPembantuKas");
        $pf = new ComRekeningPembantuKas();
        $pf->setFilters(array());
//        $pf->addFilter("cabang_id='$id'");
        $pf->addFilter("extern_id='$pcID'");
        $tmpPF = $pf->fetchBalances('1010010040');

        $lastPlafon = isset($tmpPF[0]->debet) ? $tmpPF[0]->debet : 0;
        //  endregion
