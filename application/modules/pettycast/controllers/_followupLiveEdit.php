...
//        echo "<script>$actionTarget</script>";
    ...
    public function updateItemField()
    {
        ...
        $shopingCartPairRebuild = isset($this->configUi[$this->jenisTr]['shopingCartPairRebuild'][$intoStep]) ? $this->configUi[$this->jenisTr]['shopingCartPairRebuild'][$intoStep] : array();

        //----------------------------
        $closedRequest = isset($this->configCore[$this->jenisTr]['closedRequest'][$intoStep]) ? $this->configCore[$this->jenisTr]['closedRequest'][$intoStep] : array();
        $closedRequestValues = isset($this->configCore[$this->jenisTr]['closedRequestValue'][$intoStep]) ? $this->configCore[$this->jenisTr]['closedRequestValue'][$intoStep] : array();
        //----------------------------
        ...
        if (isset($_SESSION[$cCode]['items'][$id][$key])) {
            ...
            //region value approve
            if(isset($closedRequestValues["enabled"]) && $closedRequestValues["enabled"]==true){
                $src_gate = $shopingCartPairRebuild["src"]["source"];
                $val_orig=$closedRequestValues["key"];
                $val_orig_target=$closedRequestValues["key"]."_original";
                $val_target=$closedRequestValues["key"]."_no_approve";
                if (!isset($_SESSION[$cCode]['items'][$id][$val_orig . "_original"])) {
                    $_SESSION[$cCode]['items'][$id][$val_orig_target] = $_SESSION[$cCode]['items'][$id][$src_gate];
                }
                $val_no_approve = ($_SESSION[$cCode]['items'][$id][$val_orig_target]-$val ) > 0 ? $_SESSION[$cCode]['items'][$id][$val_orig_target]-$val: 0;
                $_SESSION[$cCode]['items'][$id][$val_target] = $val_no_approve;//harga_ho approve
                $selisih_koreksi = $_SESSION[$cCode]['items'][$id][$val_orig_target] - $val;
                $_SESSION[$cCode]['items'][$id]["koreksi_nilai_plus"] = $selisih_koreksi > 0 ? $selisih_koreksi:0;//diapprove naik request 10000 approve 15000
                $_SESSION[$cCode]['items'][$id]["koreksi_nilai_minus"] = $selisih_koreksi < 0 ? $selisih_koreksi *-1:0;//diapprove turun dikurangi request 10000 approve 9000


//                matiHere(__LINE__);
            }
//            arrPrintWebs($_SESSION[$cCode]['items'][$id]);
//            cekHitam($closedRequestValues['enabled']);
//            matiHEre(__LINE__." :: ".$this->jenisTr.":: ".$intoStep."::".$closedRequestValues["enabled"]);
            //endregion
...
//        cekHere($id);
        ...
        if(count($shopingCartPairRebuild)>0){
//            matiHere(__LINE__);
            $src_gate = "";
            /*
             *dipertimbangkan kalau diremove itemnya
             * hanya di edit nilainya
             */
//            $mdlName=isset($shopingCartPairRebuild["validate"]["mdlName"]) ? $shopingCartPairRebuild["validate"]["mdlName"]:"";
            $validateRun=isset($shopingCartPairRebuild["validate"]) ? $shopingCartPairRebuild["validate"]:array();
            if(count($validateRun)>0){
                /*
                 * 2024-12-05
                 * README case untuk pettycash
                 * BARU SUPORT Untuk LockerValue, untuk rekening belum kepikir caranya
                 * jika koreksi_nilai_minus maka akan mengurangi plafon jika cukup, jika tidak cukup wajib dientikan dan minta untuk menambah plafon. nilai direset kembali keawal dan reload cart
                 * jika koreksi nilai plus, maka akan dikembalikan ke plafon.
                 */
                $val_key_validate=$closedRequestValues["key"];
                $key_target_main = $validateRun["target"];
                $modelpath = isset($validateRun["modelPath"]) ? $validateRun["modelPath"]:matiHere("model path belum diseting, silahkan hubungi admin web untuk melakukan seting");
                $mdlName = $validateRun["mdlName"];
                $mdlMethod = $validateRun["mdlMethod"];
                $rekening = isset($validateRun["rekening"]) ? $validateRun["rekening"]:matiHere("model path belum diseting, silahkan hubungi admin web untuk melakukan seting");
                $this->load->model($modelpath."/".$mdlName);
//                arrPrint($_SESSION[$cCode]['main']);
                cekHitam($mdlName);
                $m = new $mdlName();
                if(count($validateRun["mdlFilter"])>0){
                    foreach ($validateRun['mdlFilter'] as $prKey => $prVal){
                        $prVal = makeValue($prVal, $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], $static = 0);
                        $m->addFilter("$prKey='$prVal'");
                    }
                }
                $tmpo = $m->$mdlMethod($rekening);
                $curentAvailSaldo = $tmpo[0]->nilai*1;
                $newAllValue = 0;
                $original_value = 0;
                $reloadcart = false;
                if(count($tmpo)>0){
                    foreach($_SESSION[$cCode]['items'] as $i_items){
                        $newAllValue += $i_items[$val_key_validate];
                        $original_value += isset($i_items[$val_key_validate."_original"]) ? $i_items[$val_key_validate."_original"] : $i_items["max_harga"];
                    }
                    if($curentAvailSaldo >=$newAllValue){
                        $_SESSION[$cCode]["main"][$key_target_main]=$original_value-$newAllValue;
                    }
                    else{
                        /*
                         * reset ke nilai awal
                         */
                        $reloadcart=true;
                        $_SESSION[$cCode]['items'][$id][$key] = $_SESSION[$cCode]['items'][$id][$val_key_validate."_original"];//kembali ke nilai originalnya
                        $_SESSION[$cCode]['items'][$id]["koreksi_nilai_plus"] = 0;
                        $_SESSION[$cCode]['items'][$id]["koreksi_nilai_minus"]=0;
                        //--------------------------------------------
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_SESSION[$cCode]['items'][$id][$key])) {
                                $_SESSION[$cCode]['items'][$id]["sub_" . $key] = ($_SESSION[$cCode]['items'][$id][$key] * $_SESSION[$cCode]['items'][$id]["jml"]);
                        ...
                        if ($val < $_SESSION[$cCode]['items'][$id][$key]) {
                            if (isset($_SESSION[$cCode]['main']['status_4'])) {
                                $_SESSION[$cCode]['main']['status_4'] = 11;
                            }
                            if (isset($_SESSION[$cCode]['main']['trash_4'])) {
                                $_SESSION[$cCode]['main']['trash_4'] = 0;
                            }
                            ...
                            //==bila ada yang dirubah/edit menjadi lebih kecil, maka menjadi partial
                            if (in_array($key, $arrPartialKey)) {
                                $_SESSION[$cCode]['main']['partial'] = 0;
                            }
                            //----------------------------------------------------------------
                        }

                    }
//                    cekBiru("original value :: ".$original_value. " :: vs :: new value ::".$newAllValue);
//                    cekBiru($newAllValue);
//matiHere($curentAvailSaldo);
                }
                else{
                    matiHEre("Gagal mendeteksi rekening:  ".$rekening." . silahkan hubungi admin web untuk melakukan pengecekan rekening terdaftar");
                }
                resetValues($this->jenisTr);
                fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis, $this->configValuesJenis, my_ppn_factor());
...
//        echo "<script>$actionTarget</script>";
                    ...
                    echo "  }";
                    echo "</script>";
                    $rrLabel = $validateRun["errorLabel"];
                    $rrLabelFixing = $validateRun["errorFixing"];
                    $rrLabel .= "<br>Tersedia <span class='text-success text-bold'>".number_format($curentAvailSaldo,"0")."</span>";
                    $rrLabel .= "<br>Approve <span class='text-danger text-bold'>".number_format($newAllValue,"0")."</span>";
                    $rrLabel .= "<br>$rrLabelFixing";
                    matiHere($rrLabel);
                }
...
//            arrPrint($shopingCartPairRebuild);
//            matiHere(__LINE__);
