...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg()
    {
        ...
        //region efaktur
        $shopingCartFaktur = isset($this->configUi[$tmpTr[0]->jenis_master]['addMainSource'][$currentStepNum]) ? $this->configUi[$tmpTr[0]->jenis_master]['addMainSource'][$currentStepNum] : array();
        $dataFaktur = array();
        if (count($shopingCartFaktur) > 0) {
            $total_show = 0;
            foreach ($itemsRegistries as $pi => $val) {
                ...
                foreach ($shopingCartFaktur["fields"] as $key_faktur => $faktur_labels) {
                    $dataFaktur[$pi][$key_faktur] = isset($itemsRegistries[$pi][$key_faktur]) ? $itemsRegistries[$pi][$key_faktur] : "";
                }
                if (isset($shopingCartFaktur["addFields"])) {
                    foreach ($shopingCartFaktur["addFields"] as $key_faktur_add => $faktur_src_add) {
                        $total_show += (isset($itemsRegistries[$pi][$faktur_src_add])) ? $itemsRegistries[$pi][$faktur_src_add] : 0;
                    }
                }
            }
//            $show = ($total_show > 0) ? true : false;
            $show = (isset($main["ppnPersenCheck"]) && ($main["ppnPersenCheck"] == 1)) ? true : false;
            $showAddMain = isset($shopingCartFaktur["addMainSourceShow"]) ? $shopingCartFaktur["addMainSourceShow"] : false;
        }
        //endregion
...
//            "showFormulirFaktur" => isset($show) ? $show : false,
            ...
            "formulirFaktur" => isset($itemsRegistries) ? $itemsRegistries : array(),
//            "formulirFaktur" => isset($_SESSION[$cCode][$gateTarget]) ? $_SESSION[$cCode][$gateTarget] : array(),
        );
        //endregion
//arrPrintHitam($dataFaktur);
//arrPrintHitam($items);
