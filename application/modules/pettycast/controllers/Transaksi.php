...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $shoppingCartValidatorsComparison = isset($this->configUiJenis["shoppingCartValidatorsComparison"]) ? $this->configUiJenis["shoppingCartValidatorsComparison"] : array();
        $shoppingCartValidatorsComparisonLabel = isset($this->configUiJenis["shoppingCartValidatorsComparisonLabel"]) ? $this->configUiJenis["shoppingCartValidatorsComparisonLabel"] : array();
        ...
        $shoppingCartFakturValidator = isset($this->configUi[$this->jenisTr]["shoppingCartFakturValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartFakturValidator"] : array();
        ...
        if (sizeof($shoppingCartValidatorsComparison) > 0) {
            $result = array();
            $labels = array();
            foreach ($shoppingCartValidatorsComparison as $field => $label) {
                if (!isset($_SESSION[$cCode]['main'][$field])) {
            ...
            if ($result["sumber"] > $result["target"]) {
                ...
                $labelTarget = isset($labels["target"]) ? $labels["target"] : "";
                //----
                $labelSrc = isset($shoppingCartValidatorsComparisonLabel[$labelSrc]) ? $shoppingCartValidatorsComparisonLabel[$labelSrc] : $labelSrc;
                $labelTarget = isset($shoppingCartValidatorsComparisonLabel[$labelTarget]) ? $shoppingCartValidatorsComparisonLabel[$labelTarget] : $labelTarget;
                //----
                $errMsgs[] = "Nilai $labelSrc lebih besar dari nilai $labelTarget. Silahkan dikoreksi lagi.";
                $errRows[] = "test";
            }
        ...
        if(sizeof($shoppingCartFakturValidator)>0){
            if(isset($shoppingCartFakturValidator["enabled"]) && ($shoppingCartFakturValidator["enabled"] == true)){
                $cekGerbang = $shoppingCartFakturValidator["cek"];
                $cekGerbangValue = $shoppingCartFakturValidator["cekValue"];
                $cekGerbangParam = $shoppingCartFakturValidator["cekGerbangParam"];
                if(isset($_SESSION[$cCode]["items2_sum"]) && (sizeof($_SESSION[$cCode]["items2_sum"])>0)){
                    $arrMsgLabel = array();
                    foreach ($_SESSION[$cCode]["items2_sum"] as $ii => $iiSpec){
                        if(isset($iiSpec[$cekGerbang]) && ($iiSpec[$cekGerbang] == $cekGerbangValue)){
                            if(isset($iiSpec[$cekGerbangParam]) && ($iiSpec[$cekGerbangParam] > 0)){
                                // ada vendornya...
                            }
                            else{
                                $label = $shoppingCartFakturValidator["label"];
                                $label = str_replace("{biaya_nama}", $iiSpec["nama"], $label);
                                $arrMsgLabel[] = $label;
                            }
                        }
                    }
                    if(sizeof($arrMsgLabel)>0){
                        $msg = implode("<br>", $arrMsgLabel);
        ...
        if (sizeof($errMsgs) > 0) {
            ...
            var dateEfaktur = top.$('input[name=dateFaktur]');
            var efaktur = top.$('input[name=eFaktur]');
            var filePhoto = top.$('input[name=file]');

            var err = 0;
            console.error( top.countObj(dateEfaktur) )
            if( top.countObj(dateEfaktur) ){

                if( top.countObj(dateEfaktur) ){
                    top.jQuery.each(dateEfaktur , function(a,b){
                        console.log( top.$(b).val() );
                        if( top.$(b).val() == '' ){
                            top.$(b).addClass('bg-red')
                            err += 1
                        }
                        else{
                            top.$(b).removeClass('bg-red')
                        }
                    });
                }

                if( top.countObj(efaktur) ){
                    top.jQuery.each(efaktur , function(a,b){
                        console.log( top.$(b).val() );
                        if( top.$(b).val() == '' ){
                            top.$(b).addClass('bg-red')
                            err += 1
                ...
                if( top.countObj(filePhoto) ){
                    top.jQuery.each(filePhoto , function(a,b){
                        console.log( top.$(b).val() );
                        if( top.$(b).val() == '' ){
                            var tmpID = top.$(b).parent().attr('id');
                            var ids = tmpID.replace('myForm_', '');
                            if( top.$('input[name=img_'+ids+']').length ){
//                                console.log(top.$('input[name=img_'+ids+']'))
                            }
                            else{
                                top.$(b).addClass('bg-red')
//                                err += 1
            ...
            if(err*1>0){
                ...
                top.swal('cek kembali', 'ada form yang harus di isi, lihat form dengan backgound merah....', 'info');
            }
            else{
                $actionTarget
            }
...
// matiHEre("hoop".__LINE__);
    ...
    public function index()
    {
        ...
        $arrJenisTrBiayaCek = array("672", "1672", "771", "1771");
                        ...
                        foreach ($tmpResult as $tmpSpec) {
                            ...
                            if ($pakai_ini == 1) {
                                ...
                                if (in_array($row->sub_step_number, $arrStepAllow)) {
                                    ...
                                    if ($evPre == null && $evPost == null && $evCom == null) {
//                                        $transaksiID_reject = $row->transaksi_id;
                                        $transaksiID_reject = $row->id_master;
                                        $masterRevertStep = ($currentStepNum - 1);
                                        $childRevertStep = -($currentStepNum);
...
//                                $link_reject = MODUL_PATH . "FollowUp/doRevert/" . $this->jenisTr . "/$transaksiID_reject/$masterRevertStep/$childRevertStep/$currentStepNum?mode=autoPrePreview";
                                            $link_reject = MODUL_PATH . "FollowUp/followupPrePreview/" . $this->jenisTr . "/$transaksiID_reject/$nextStepNum/" . $row->sub_step_number . "?getmode=reject";
                                            $actionLabel_edit = "Reject 1 step";
                                            $tmp2Datas['action'] .= "<button class='btn btn-danger btn-block btn-xs' title='turn this entry into $stepLabel' href='javascript:void(0)' 
                                $disabled_hold $disabled_button_reject
                                onclick =\"document.getElementById('result').src='$link_reject'\">" . $actionLabel_edit . " </button>";
