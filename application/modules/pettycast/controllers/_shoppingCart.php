        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pettycast/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pettycast/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
            ...
            if (isset($_SESSION[$cCode]['items2_sum'])) {
                ...
                foreach ($_SESSION[$cCode]['items2_sum'] as $iSpec) {
                    ...
                    $tmp = array("id" => $iSpec['id'],
                        ...
                        "ppnPersenCheck" => isset($iSpec['ppnPersenCheck']) ? $iSpec['ppnPersenCheck'] : "",
                    );
                    if ($noteEnabled) {
                        $tmp['note'] = isset($iSpec['note']) ? $iSpec['note'] : "";
                    }
                        ...
                        if (sizeof($addRowsConfigs[$eName][$currentValue]) > 0) {
                            ...
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                ...
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    //                                    $defVal = $origDefValue;
                                    $addRows[$oValueName] = "<input type=text autocomplete='off' id='$oValueName' class='form-control text-right' style='font-size:17px;' $disabled 
placeholder='$oValueName' value='$defVal' $minValStr $maxValStr 
                ...
                switch ($eSpec['elementType']) {
                    ...
                    BootstrapDialog.show(
                        ...
                        switch ($eSpec['inputType']) {
                            case "combo":
                                $elStr[$eName] .= "<select class='form-control select2' data-live-search=\"true\" onchange=\"hiliteDiv(this);document.getElementById('result').src=$selectorTarget;\">";
...
//                                            cekHere("[$src] newValue_r ::: $newValue_r");
                                            if ($src == 'saldo') {
...
//                    else {
//                    }
//                    arrPrintWebs($shopingCartFaktur);
                    foreach ($_SESSION[$cCode]["items"] as $pi => $val) {
...
//        arrPrint($dataFaktur);
        ...
        if ($shopingCartReload) {
            ...
            if (isset($_GET['stop1']) && $_GET['stop1'] == 1) {
                echo "<script>";
                echo "top.HoldOn.close();\n";
                echo "</script>";
            }
            else {
                echo "<script>";
                echo "top.HoldOn.open();\n";
 ...
 * ---------------------------*/ // arrPrintPink($_SESSION[$cCode]);
    ...
    public function recordItemColumn()
    {
        ...
        if (isset($_GET['reload'])) {
            ...
            top.$('#note_$iID').css({'background-color': 'lightgreen', 'color': 'black'});
