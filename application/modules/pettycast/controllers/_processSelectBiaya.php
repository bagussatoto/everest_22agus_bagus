...
class _processSelectBiaya extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        $addMainSource = isset($this->configUi[$this->jenisTr]['addMainSource'][1]) ? $this->configUi[$this->jenisTr]['addMainSource'][1] : array();
        $ppnPersenCheck = isset($this->configUi[$this->jenisTr]['shoppingCartEditableFieldsOptions'][1]['ppnPersenCheck']) ? $this->configUi[$this->jenisTr]['shoppingCartEditableFieldsOptions'][1]['ppnPersenCheck'] : array();
...
//                    }
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        echo "iterating subNums... *** " . sizeof($itemNumLabels) . " ***<br>";
...
//                        $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * ($_SESSION[$cCode]['items'][$id]['harga'] + $_SESSION[$cCode]['items'][$id]['ppn']));
        ...
        if (isset($_SESSION[$cCode]["items"][$id]["ppnPersenCheck"]) && ($_SESSION[$cCode]["items"][$id]["ppnPersenCheck"] == NULL)) {
            // default ke non_ppn == true
            $default_pajak = true;
        }
        elseif (!isset($_SESSION[$cCode]["items"][$id]["ppnPersenCheck"])) {
            // default ke non_ppn == true
            $default_pajak = true;
        }
        else {
            // default ke non_ppn == false
            $default_pajak = false;
        }

        if ($default_pajak == true) {
            if (sizeof($addMainSource) > 0) {
                if (isset($addMainSource["defaultCheck"]) && ($addMainSource["defaultCheck"] != NULL)) {
                    $defaultCheckParam = isset($ppnPersenCheck["dataField"][$addMainSource["defaultCheck"]]) ? $ppnPersenCheck["dataField"][$addMainSource["defaultCheck"]] : array();
                    $defValue = $defaultCheckParam["value"];
                    $defSrcItem = $defaultCheckParam["srcItem"];
                    $defOverWriteMain = $defaultCheckParam["overWriteMain"];
                    $defSrcMain = $defaultCheckParam["srcMain"];
                    $jenisTr = $this->jenisTr;
                    $link = MODUL_PATH . "_processSelectProductPpn/selectItem/$jenisTr?id=$id&ppn=$defValue&ppnTargetItems=$defSrcItem&ppnTargetMain=$defSrcMain&overWriteMain=$defOverWriteMain";
                    echo "<script>";
                    echo "  if(top.document.getElementById('result')){";
                    echo "  top.$('#result').load('$link');";
                    echo "  }";
                    echo "</script>";
                }
            }
        ...
        // dibawah ini untuk mereload shoppingcart...
        if (isset($_GET['minValue'])) {
            echo "<script>";
            echo " top.initShoppingCart();";
            ...
            top.$('input[id_jml=$idGet]').css({'background-color': 'lightgreen', 'color': 'black'})
...
//        die();
        if (isset($_SESSION[$cCode]['items'][$id])) {
            ...
            if (isset($shopingCartFaktur["gateTarget"])) {
                ...
                if (isset($_SESSION[$cCode][$gateTarget][$id]) && (sizeof($_SESSION[$cCode][$gateTarget][$id]) > 0)) {
                    $_SESSION[$cCode][$gateTarget][$id] = NULL;
                    unset($_SESSION[$cCode][$gateTarget][$id]);
                }
            }
            //----------------------------------------------
        }
