...
class _shoppingCart extends Modul_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->enforceMutationGuard(array('viewCart', 'fetchModelDescription', 'fetchElement', 'fetchItemsElement'));
    ...
    public function viewCart()
    {
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                // $this->debugArrPrint($_SESSION[$cCode]['items']);
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "taxes/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "taxes/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;

                    $items[] = $tmp;

                    //                    $this->debugArrPrint($shoppingCartAmountValue);
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        if (sizeof($shopingCartParamForceEditable) > 0) {
            foreach ($shopingCartParamForceEditable as $paramsKey => $paramGate) {
                ...
                //                $this->debugArrPrint($paramForceEditable);
        ...
        //$this->debugArrPrint($elementMutasi);
        //-------------------------------------------------------------------------------
    ...
    public function fetchModelDescription()
    {
        ...
        $cCode = "_TR_" . $jenisTr;
        $mdlName = $this->getSafeMdlsModelName($this->uri->segment(4));
        if ($mdlName === null) {
            show_error("Invalid model name", 400);
            return;
        }
        $id = $this->uri->segment(5);
        $label = $this->decodeSafeBase64Param('label', "");
        $labelSrc = $this->decodeSafeBase64Param('labelSrc', "");
        $descSrc = $this->decodeSafeBase64Param('desc', "");

        if (is_array($label)) {
            $label = implode("+", $label);
        }
        if (is_array($labelSrc)) {
            $labelSrc = implode("+", $labelSrc);
        }
        if (is_array($descSrc)) {
            $descSrc = implode("+", $descSrc);
        }
        $label = (string)$label;
        $labelSrc = (string)$labelSrc;
        $descSrc = (string)$descSrc;

        $mdlName = $this->loadSafeMdlsModel($mdlName);
        $oo = new $mdlName();
        $oo->addFilter("id='$id'");
        ...
        if (isset($_SESSION[$cCode]['items'])) {
            foreach ($_SESSION[$cCode]['items'] as $id => $item) {
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    ...
                    if (!$this->finalizeSafeTransaction("Gagal saat menyimpan perubahan loker.")) {
                        return;
                    }
                }
    ...
    public function recordAddRow()
    {
        ...
        //                // $this->debugArrPrint($mains);
        //                // arrPrintPink($dTmpDiskons);
        //                // $this->debugArrPrint($cekData->num_rows());
        //                if ($cekData > 0) {
 ...
 * ---------------------------*/ // arrPrintPink($_SESSION[$cCode]);
        ...
        if ($cekData > 0) {
            ...
            if (!$delete) {
                ...
                $this->failMutationResponse("Gagal saat menghapus tmp diskon sebelumnya.");
                return;
            }
            $this->debugShowLastQuery("orange");
        }

        $inserted = $td->addData($dTmpDiskons);
        if (!$inserted) {
            $this->db->trans_rollback();
            $this->failMutationResponse("Gagal saat menyimpan tmp diskon.");
            return;
        }
        $this->debugShowLastQuery("lime");
        // matiHere("belum commit " . __LINE__);

        if (!$this->finalizeSafeTransaction("Gagal saat menyimpan tmp diskon.")) {
            return;
        }
    ...
    public function recordPairedItem()
    {
        ...
        $selectorSrcModel = $this->loadSafeMdlsModel($selectorSrcModel);
...
//                            $_SESSION[$cCode]['rsltItems'][$iID]["sub_" . $key] = ($_SESSION[$cCode]['rsltItems'][$iID][$key] * $_SESSION[$cCode]['rsltItems'][$iID]["jml"]);
    ...
    public function recordImage()
    {
        ...
        if ($files !== null) {
            list($isUploadValid, $uploadErrorMessage) = $this->validateUploadedImageFile($files, 5 * 1024 * 1024);
            if (!$isUploadValid) {
                echo swalAlert(array(
                    "type" => "warning",
                    "html" => $uploadErrorMessage,
                ));
            }
            else {
                $ext = strtolower((string)pathinfo((string)$files['name'], PATHINFO_EXTENSION));
                $rand = function_exists('random_bytes') ? bin2hex(random_bytes(16)) : uniqid("img_", true);
                $files['name'] = $rand . "." . $ext;
                $cUrl_result = upload_image($files);
                if (is_object($cUrl_result) && isset($cUrl_result->full_url)) {
