...
//    private $jenisTr;
    ...
    {
        parent::__construct();
        $this->enforceMutationGuard(array('getJenisTr', 'setJenisTr'));
    }
...
//            $this->debugArrPrint($_SESSION[$cCode]['items_child'][$id]);
...
//arrPrintCyan($_SESSION[$cCode]["items"]);
...
//        $this->debugArrPrint($elementRelConfig);
//        $this->debugArrPrint($relConfig);

        //endregion

        $elementConfigs = $elementConfigs + $relConfig;
//        $this->debugArrPrint($elementConfigs);
...
//            echo(lgShowAlert("NOT replacing $key with $val"));
    ...
    public function updateSourceField()
    {
        ...
        if (isset($_SESSION[$cCode]["items6_sum"][$detail_key])) {
            if ($key == "dpp_final") {
                $ppn_final = ($ppnFactor / 100) * $value;
                $subtotal = $value + $ppn_final;
                //----
                //----
                $_SESSION[$cCode]["items6_sum"][$detail_key][$key] = $value;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_final"] = $ppn_final;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["nilai_entry"] = $subtotal;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_sudah_faktur"] = $ppn_final;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_belum_faktur"] = $ppn_final;
...
//            mati_disini("key: $key");
            if ($key == "eFaktur") {
                // region validasi/checker format nomer faktur 2 digit pertama termasuk dalam kode nomer seri faktur pajak
                if ($gunggunganMode == 0) {
                ...
                // endregion validasi/checker format nomer faktur 2 digit pertama termasuk dalam kode nomer seri faktur pajak

                $_SESSION[$cCode]["items6_sum"][$detail_key][$key] = $value;
//                $_SESSION[$cCode]["main"][$key] = $value;
            }
            if ($key == "ppn_nilai_faktur") {
                $selisih = $value - $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_final"];
                $selisih = ($selisih < 0) ? ($selisih * -1) : $selisih;
                $selisih_f = number_format($selisih);
                if ($selisih > 100) {
                    $msg = "Nilai PPN yang anda isi salah, terdapat selisih $selisih_f. <br>Silahkan koreksi kembali. code: " . __LINE__;
                    mati_disini_original($msg);
                }
                $_SESSION[$cCode]["items6_sum"][$detail_key][$key] = $value;
            }
        }
        ...
        if ($id == "eFaktur") {
            $ppn_nilai = $_SESSION[$cCode]["main"]["ppn_out_bulat"];
            if ($ppn_nilai > 0) {
                if ($val == NULL) {
                    $msg = "Nomer eFaktur tidak valid. Silahkan isikan nomer eFaktur dengan benar. code: " . __LINE__;
                    mati_disini_original($msg);
                }
            }
...
//
        if ($gunggunganMode == 1) {
            echo "<script>";
            echo "top.$('#followupPreview').load('" . MODUL_PATH . "FollowUp/followupPreviewMasal/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7) . "?rawBuilderURL=$rawBuilderURL');";
            echo "</script>";
        }
        else {
            echo "<script>";
...
//        arrprint($_SESSION[$cCode]['items_komposisi']);
        ...
        if (isset($_SESSION[$cCode]['items_komposisi']) && (sizeof($_SESSION[$cCode]['items_komposisi']) > 0)) {
            ...
            $this->debugArrPrint($arrComponentsOriginal);
...
//        $this->debugArrPrint($_SESSION[$cCode]['items2']);
//
//        cekHitam(":: cetak gerbang ITEMS2_SUM");
//        $this->debugArrPrint($_SESSION[$cCode]['items2_sum']);
//
//        cekHitam(":: cetak gerbang ITEMS3_SUM");
//        $this->debugArrPrint($_SESSION[$cCode]['items3_sum']);
...
//        $this->debugArrPrint($this->uri->segment_array());
        ...
        $updated = $tr->updatePaymentSrc($where, $data);
        if (!$updated) {
            $this->db->trans_rollback();
            $this->failMutationResponse("Gagal saat berusaha memperbarui payment source.");
            return;
        }
        ...
        $tr->setTableName($tbl);
        $inserted = $tr->addData($dataEfaktur);
        if (!$inserted) {
            $this->db->trans_rollback();
            $this->failMutationResponse("Gagal saat berusaha menyimpan riwayat e-faktur.");
            return;
        }
