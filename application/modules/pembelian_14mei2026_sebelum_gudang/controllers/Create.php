...
class Create extends Modul_Controller
{
    ...
    public function index()
    {
        ...
        switch ($this->jenisTr) {
            case "460"://fg purchasing import, 31-08-2022
        ...
        * ---------------------------------------------------------------------*/
        ...
        if (sizeof($mems) > 0 && sizeof($dataAccess['creators']) > 0) {
            if (sizeof(array_intersect($mems, $dataAccess['creators'])) > 0) {
                ...
                if ($disabled_pihak != NULL) {
                    $addClick = "#";
                }
                $addPihakStr = "<span class='input-group-btn'>
                                        <a href='javascript:void(0)' $disabled_pihak title='add $dataLabel' data-toggle='tooltip' data-placement='right' class='btn btn-default' onclick=\"$addClick\">
...
\n\n
        ...
        $calcStockPath = isset($this->configUi[$this->jenisTr]['calcStockPath']) ? $this->configUi[$this->jenisTr]['calcStockPath'] : "#";
...
//            "pihakExternLabel" => isset($_SESSION[$cCode]['main']['pihakExternLabel']) ? $_SESSION[$cCode]['main']['pihakExternName'] . "(" . $this->configUi[$jenisTr]["pihakExternLabel"] . ")" : isset($this->configUi[$jenisTr]["pihakExternLabel"]) ? $this->configUi[$jenisTr]["pihakExternLabel"] : "",
            ...
            "pihak_nama" => isset($_SESSION[$cCode]["main"]["pihakName"]) ? $_SESSION[$cCode]["main"]["pihakName"] : "",
            "modul_ini" => MODUL_PATH,
    ...
    public function preview()
    {
        ...
        $shopingCartAddProdukFields = isset($this->configUi[$this->jenisTr]['produkUnitDisc']["viewFields"]) ? $this->configUi[$this->jenisTr]['produkUnitDisc']["viewFields"] : array();
        ...
        $data = array(
            ...
            "addProdukDiscLabel" => $shopingCartAddProdukFields,
            "addProdukDisc" => isset($_SESSION[$cCode]["items5_sum"]) ? $_SESSION[$cCode]["items5_sum"] : array(),
    ...
    public function save()
    {
        ...
        $this->jenisTr = ($this->uri->segment(4) != NULL) ? $this->uri->segment(4) : $this->jenisTr;
        $cCode = $this->cCode;
...
//            unset($_SESSION[$cCode]["main"]["bookingNumber"]);
            ...
            cekHere($this->cCode);
            arrPrintPink($this->configUiJenis);
            ...
            arrPrintWebs($cContent);
            arrPrint($appliedCounters_inText);
//            mati_disini();
...
//                    if(count($dParams)>1){
//                        if (!in_array($cRawParams, $this->configCore[$this->jenisTr]["counters"])) {
...
//                    if(count($arrRawParams)*1 > 1){
//                        $cRawParamsValues = array();
//                        foreach($arrRawParams as $key){
//                            $cRawParamsValues[$key] = $_SESSION[$cCode]["main"][$key];
//                        }
//                        $cRawParamsValuesK = implode("|", array_keys($cRawParamsValues));
//                        $cRawParamsValuesV = implode("|", $cRawParamsValues);
//                        $arrNomorNota[$cRawParamsValuesK] = digit_4($cContent[$cRawParamsValuesK][$cRawParamsValuesV]);
//                    }
//                    else{
//                        $cRawParamsValuesK = $arrRawParams[0];
//                        $cRawParamsValuesV = $_SESSION[$cCode]["main"][$arrRawParams[0]];
//                        if($arrRawParams[0]=="fulldate"){
//                            $arrNomorNota[$arrRawParams[0]] = "".date("mY", strtotime($cRawParamsValuesV));
//                        }
//                        elseif($arrRawParams[0]=="stepCode"){
//                            $arrNomorNota[$arrRawParams[0]] = $cRawParamsValuesV; //ini harus ori tidak boleh di masking/ diformat
////                            $arrNomorNota[] = digit_4($cContent[$cRawParamsValuesK][$cRawParamsValuesV]);
//                        }
//                        elseif($arrRawParams[0]=="placeID"){
//                            $arrNomorNota[$arrRawParams[0]] = digit_2($cRawParamsValuesV);
//                        }
//                        elseif($arrRawParams[0]=="olehID"){
//                            $arrNomorNota[$arrRawParams[0]] = digit_4($cRawParamsValuesV);
//                        }
//                        elseif($arrRawParams[0]=="pihakID"){
//                            $arrNomorNota[$arrRawParams[0]] = digit_4($cRawParamsValuesV);
//                        }
//                        else{
//                            $arrNomorNota[$arrRawParams[0]] = $cRawParamsValuesV;
//                        }
//                    }
//                }
//            }
...
//            $arrPatern = array("/-","..","--","/.");
//            $arrPaternTo = array("/",".",".-","/");
//            $tmpNomorNota = str_replace($arrPatern, $arrPaternTo, $tmpNomorNota);
//            $tmpNomorNotaAlias = str_replace($arrPatern, $arrPaternTo, $tmpNomorNotaAlias);
//
//            arrPrintWebs($arrNomorNota);
//            cekMerah("tmpNomorNota: $tmpNomorNota");
//            cekMerah("tmpNomorNotaAlias: $tmpNomorNotaAlias");

//            $tmpNomorNota = "999999999999999999";
//            $tmpNomorNotaAlias = "999999999999999999";

//            mati_disini(__LINE__ . " || TESTING PRE-PO [$tmpNomorNota] [$tmpNomorNotaAlias]");
            ...
            //$newNum = getNewNumberFormat($new_counter,$_SESSION[$cCode]['main']);

            arrPrintWebs($newNum);
            cekMerah("==========================");
            arrPrintWebs($new_counter);
            ...
            if (sizeof($iterator) > 0) {
                ...
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    if (isset($_SESSION[$cCode][$srcGateName])) {
                        echo "[$cCtr] sub-postProcessor: $comName, sending values [$srcGateName] <br>";
                    ...
                    foreach ($payConfigs[1] as $paymentSrcConfig) {
                        ...
                        if ($paymentMethod == "update") {
                            ...
                            $arrDataPym = array(
                                ...
                                "extern_label2" => (isset($externSrc['extern_label2']) && ($_SESSION[$cCode]['main'][$externSrc['extern_label2']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_label2']] : "",
                                "extern_nilai2" => (isset($externSrc['extern_nilai2']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai2']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai2']] : 0,
                                "extern_nilai5" => (isset($externSrc['extern_nilai5']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai5']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai5']] : 0,
...
//            cekMerah("TRANSAKSI DONE");
            ...
            if (isset($this->configUiJenis["connectToEdit"][1])) {
                if ($this->configUiJenis["connectToEdit"][1]["enabled"] == true) {
                    ...
                    $tCodeTargetJenisTransaksi = $this->configUiJenis["connectToEdit"][$stepNumCurrent]["connectTo"];
                    $modul_transaksi = $this->modul;
                    if ($connecTo == NULL) {
...
//            mati_disini("EDIT BERHASIL, saat ini sedang maintenance.");
    ...
    public function preCancelPackingPreview()
    {
        ...
        $noteEnabled = isset($this->configUi[$this->jenisTr]['shoppingCartNoteEnabled']) && $this->configUi[$this->jenisTr]['shoppingCartNoteEnabled'] == true ? true : false;
        $imageEnabled = isset($this->configUi[$this->jenisTr]['shoppingCartImageEnabled']) && $this->configUi[$this->jenisTr]['shoppingCartImageEnabled'] == true ? true : false;
        $shopingCartMsg = isset($this->configUi[$this->jenisTr]['shopingCartMsg']) ? $this->configUi[$this->jenisTr]['shopingCartMsg'] : array();//untuk menampilkan notif jika PO memiliki titipan yang akan di fullfill
        $inputLabels = array();
        $rawPrevURL = isset($_GET['rawPrev']) ? $_GET['rawPrev'] : "";
        ...
        //region notif fullfill realsi titipan supplier
        $msg_titipan = "";
        $show_titipan = false;
        if (count($shopingCartMsg) > 0) {
            $key_titipan = $shopingCartMsg["key"];

            if (isset($_SESSION[$cCode]["main"][$key_titipan])) {
                $show_titipan = $_SESSION[$cCode]["main"][$key_titipan];
            }
            if ($show_titipan) {
                $nomer_um = formatField_he_format("nomer_nolink", $_SESSION[$cCode]["main"]["titipan_reference_nama"]);

                $nomer_po = formatField_he_format("nomer_nolink", $_SESSION[$cCode]["main"]["titipan_extern2_nama"]);
                $msg_titipan = "PO No $nomer_po  berelasi dengan titipan No $nomer_um. <br>";
                $msg_titipan .= "close/fullfilment akan menghilangkan relasi dan saldo akan berpindah ke titipan non relasi.";

            }
        }
        //endregion
        arrPrint($this->uri->segment(3));
        $data = array(
            "mode" => $this->uri->segment(3),
            "template" => $this->configUi[$jenisTr]["template"],
            "title" => $this->configUi[$jenisTr]["label"],
            ...
            "currentStep" => 1,
            "pairedValue" => isset($_SESSION[$cCode]['pairs']) ? $_SESSION[$cCode]['pairs'] : array(),
            "notif_titipan" => $msg_titipan,
            "show_titipan" => $show_titipan,
    ...
    public function doPreCancelPacking()
    {
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['tableIn_master']) && sizeof($_SESSION[$cCode]['tableIn_master']) > 0) {
                ...
                //-------
                $_SESSION[$cCode]['tableIn_master']['reference_id'] = $_SESSION[$cCode]['main']['transaksiDatas'];
                $_SESSION[$cCode]['tableIn_master']['reference_nomer'] = $_SESSION[$cCode]['main']['transaksiDatas__nomer'];
                $_SESSION[$cCode]['tableIn_master']['reference_jenis'] = $_SESSION[$cCode]['main']['transaksiDatas__jenis'];
                $_SESSION[$cCode]['tableIn_master']['reference_id_top'] = $_SESSION[$cCode]['main']['transaksiDatas__id_master'];
                $_SESSION[$cCode]['tableIn_master']['reference_nomer_top'] = $_SESSION[$cCode]['main']['transaksiDatas__nomer_top'];
                $_SESSION[$cCode]['tableIn_master']['reference_jenis_top'] = $_SESSION[$cCode]['main']['transaksiDatas__jenis_master'];
                //-------
...
//                cekHijau("MAIN ENTRIES QUERY LINE: " . __LINE__ . "<BR>" . $this->db->last_query());
//                arrPrintWebs($_SESSION[$cCode]['tableIn_master']);

//                arrPrint($udTr[0]->nomer);
//                arrPrint($udTr[0]->ids_his);
//                arrPrint($nomer_top_replacer);
//                matiHere();

                /*
                 * LOGIC JIKA MEMBUAT FULLFILLMENT PENJUALAN
                 * id_master di replace dengan id_master transaksi UTAMA nya.
                 * @ $id_master_reference defaultnya adalah $insertID , namun jika transaksi adalah fullfillment (1982)
                 * maka $insertID di belokan ke id_master bawaan dari FULLFILLMENT.
                 */
                $arrJenisFullfill = array(
                    "1967", "1961", "1960"
                );
                $id_master_reference = $insertID;
//                if ($_SESSION[$cCode]['main']['jenis'] == "1982") {
                if (in_array($_SESSION[$cCode]['main']['jenis'], $arrJenisFullfill)) {
                    $id_master_reference = isset($_SESSION[$cCode]['main']['masterID']) ? $_SESSION[$cCode]['main']['masterID'] : $insertID;
                }
//mati_disini(":: $id_master_reference ::");

                $epID = $tr->writeMainEntries_entryPoint($insertID, $id_master_reference, $_SESSION[$cCode]['tableIn_master']);
//                showLast_query("hijau");

                $mongoList['main'] = array($insertID, $epID);
                $insertNum = $_SESSION[$cCode]['tableIn_master']['nomer'];
                $_SESSION[$cCode]['main']['nomer'] = $insertNum;
                $_SESSION[$cCode]['main']['nomer2'] = $_SESSION[$cCode]['tableIn_master']['nomer2'];
                if ($insertID < 1) {
                    die("Gagal saat berusaha  write transaction entry pada " . __FILE__ . " baris " . __LINE__);
                }
                //==transaksi_id dan nomor nota diinject kan ke gate utama
                $injectors = array(
                    "transaksi_id" => $insertID,
                    "nomer" => $tmpNomorNota,
//                    "nomer2" => $_SESSION[$cCode]['tableIn_master']['nomer2'],
                    "nomer2" => $tmpNomorNotaAlias,
                );
                $arrInjectorsTarget = array(
                    "items",
...
//                    "nomer2" => $_SESSION[$cCode]['main']['nomer2'],
                ...
                cekHijau("QUERY LINE: " . __LINE__ . "<BR>" . $this->db->last_query());
            ...
            if (strlen($connector) > 0) {
                ...
                if (sizeof($steps) == 1) {
                    ...
                    $tCodeTargetJenisTransaksi = $this->configUi[$connector]['steps'][1]['target'];
                    $modul_transaksi = $this->modul;
...
//        matiHEre(__LINE__);
        ...
        $sesCcode = $this->session->$cCode;
        $sesItems = $sesCcode['items'];

        $cabang_id = CB_ID_PUSAT;
        $toko_id = my_toko_id();
//        $pihakID = isset($sesCcode["main"]["pihakID"])? $sesCcode["main"]["pihakID"]:"";

        $this->load->library("Diskon");
        $dp = new Diskon();
        $strDiskon = $dp->cekDiskonPembelian($sesItems, $cabang_id, $toko_id);
// print_r($strDiskon['html']);
//        echo"*****";
        echo $strDiskon['html'];
    }
