...
class _processSelectProduct extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("undefine ppn factor, please logout and login again. code: " . __LINE__);
        ...
        $requiredItem = isset($this->configUi[$this->jenisTr]['shopingCartRequiredItem']) ? $this->configUi[$this->jenisTr]['shopingCartRequiredItem'] : array();
        $pairAddProdukDisc = isset($this->configUi[$this->jenisTr]['produkUnitDisc']) ? $this->configUi[$this->jenisTr]['produkUnitDisc'] : array();
...
//        cekLime($this->db->last_query());
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                if (count($requiredItem) > 0) {
                    $temp_data_serial = array(
                        2 => array(
                            "outdoor_nama" => "sku <b class='text-danger'>outdoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku produk<br><br>",
                            "indoor_nama_1" => "sku  <b class='text-danger'>indoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku produk<r>",
                        ),
                        3 => array(
                            "outdoor_nama" => "sku <b class='text-danger'>outdoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku produk<br><br>",
                            "indoor_nama_1" => "sku <b class='text-danger'>indoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku produk<br><br>",
                            "indoor_nama_2" => "sku <b class='text-danger'>indoor#1</b> tidak tersedia! silahkan hubungi bagian data untuk update sku produk<r>",
                        ),
                        4 => array(
                            "outdoor_nama" => "sku <b class='text-danger'>outdoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku data produk<br><br>",
                            "indoor_nama_1" => "sku <b class='text-danger'>indoor</b> tidak tersedia! silahkan hubungi bagian data untuk update sku data produk<br><br>",
                            "indoor_nama_2" => "sku <b class='text-danger'>indoor#1</b> tidak tersedia! silahkan hubungi bagian data untuk update sku data produk<br><br>",
                            "indoor_nama_3" => "sku <b class='text-danger'>indoor#2</b> tidak tersedia! silahkan hubungi bagian data untuk update sku data produk<r>",
                        ),
                    );

                    $errorLabel = "";
                    foreach ($requiredItem as $req_key => $errLabel) {
//                        cekHere("[$req_key] || " . $row->$req_key);// ngecek julah serial
                        switch ($row->$req_key) {
                            case "2":
                                foreach ($temp_data_serial[$row->$req_key] as $ee_key => $eeErrorLabel) {
                                    if (strlen($row->$ee_key) == "0") {
                                        $errorLabel .= "$eeErrorLabel";
                                    }
                                }
//                                matiHEre(__LINE__);
                                ...
                                break;
                            case "3":
//                                foreach ($temp_data_serial[$row->$ee_key] as $ee_key => $eeErrorLabel) {
                                foreach ($temp_data_serial[$row->$req_key] as $ee_key => $eeErrorLabel) {
                                    if (strlen($row->$ee_key) == "0") {
//                                        cekHere("[$ee_key] [$eeErrorLabel]");
                                        $errorLabel .= "$eeErrorLabel";
                                    }
                                }
                                break;
...
//                                foreach ($temp_data_serial[$row->$ee_key] as $ee_key => $eeErrorLabel) {
                    ...
                    if (strlen($errorLabel) > 0) {
                        $xx = lgShowWarning("$produk_label_2", $errorLabel);
                        echo $xx;
                        die();
                    }
                }
...
//                        showLast_query("kuning");
                    ...
                    //tambahan untuk tambahan diskon sub produk
                    arrPrint($pairAddProdukDisc);

                    if (count($pairAddProdukDisc) > 0) {
                        $mdlName = $pairAddProdukDisc["mdlName"];
                        $srcKey = $pairAddProdukDisc["srcKey"];
                        $srcTarget = $pairAddProdukDisc["target"];
                        $srcFieldsBonus = $pairAddProdukDisc["srcFields"];
                        $this->load->model("Mdls/" . $mdlName);
                        $pd = new $mdlName();
//                        matiHere($srcKey);
                        $tempPairaddDisc = $pd->fetchPairDiskon($id, $_SESSION[$cCode]['items'][$id]["pihakID"]);
//                        cekBiru($this->db->last_query());
//                        arrPrint($tempPairaddDisc);
//                        ceklIme($this->db->last_query());
//                        matiHere($mdlName);
//                        arrPrint($tempPairaddDisc);
                        if (isset($_SESSION[$cCode]['items'][$id][$srcKey]) && count($tempPairaddDisc) > 0) {
                            $curentQty = $_SESSION[$cCode]['items'][$id][$srcKey];
                            $minQty = $tempPairaddDisc[0]->qty_min;
                            $harga = $tempPairaddDisc[0]->produk_rel_harga;
                            $isKelipatan = $tempPairaddDisc[0]->kelipatan;
                            $xFactor = 0;
                            if ($curentQty >= $minQty) {
                                if ($isKelipatan == "1") {
                                    $valueX = pembulatanKebawah($curentQty / $minQty);
                                    $hasil = $valueX["hasil"] * $tempPairaddDisc[0]->produk_rel_qty;
                                    $factorPengurang = $hasil * $minQty;
                                    $sisa = $curentQty - $factorPengurang;
                                }
                                else {
                                    $hasil = $tempPairaddDisc[0]->produk_rel_qty;
                                    $factorPengurang = 0;
                                    $sisa = 0;
                                }
                                ...
                                $tmp2 = array();
                                foreach ($tempPairaddDisc as $tempPairaddDisc_0) {
//                                    arrprint($tempPairaddDisc_0);
                                    foreach ($srcFieldsBonus as $key_src => $src_key2) {
                                        if (isset($tempPairaddDisc_0->$key_src)) {
                                            $tmp2[$key_src] = is_numeric($tempPairaddDisc_0->$key_src) ? $tempPairaddDisc_0->$key_src * 1 : $tempPairaddDisc_0->$key_src;
                                        }
                                    }
                                }
...
//                                    arrprint($tmp2);
//                                    matiHere($srcTarget);
                                    $_SESSION[$cCode][$srcTarget][$id] = $tmp2;
                                }
//                                arrPrint($tmp2);
//                                matiHEre();
//                                arrPrint($valueX);
                                ...
                                //reset
//                                matiHere(__LINE__);
                                unset($_SESSION[$cCode][$srcTarget][$id]);
                            }
...
//        }
    ...
    public function multiSelectBiNew()
    {
        // unset($_SESSION["Bi"]);
    ...
    public function multiSelectBi()
    {
        ...
        //        mati_disini();
        // cekMErah($this->uri->segment_array());
        // $this->load->library("FieldCalculator");
        // $cal = new FieldCalculator();
                    ...
                    foreach ($tmpB as $row) {
                        ...
                        if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                            $tmp = array(
                                ...
                                "subtotal" => 0,
                                "harga_old" => 0,
                                "ppn" => isset($_SESSION[$cCode]['main']['ppnPersenCheck']) ? $_SESSION[$cCode]['main']['ppnPersenCheck'] : 11,
                                "satuan" => strlen($rows->satuan) > 0 ? $rows->satuan : "n/a",
                            );
...
//            );
            $result["status_c"] = 1;
            $result["url_c"] = MODUL_PATH . "_shoppingCart/viewCart/$jenisTr/MdlSupplier?id=$getSupplierID";
            $result["frame_c"] = "result";
            echo json_encode($result);
        }
        else {
            echo "<script>";
            echo "  if(top.document.getElementById('shopping_cart')){";
            echo "  top.$('#shopping_cart').load('" . base_url() . $this->modul . "/_shoppingCart/viewCart/" . $this->jenisTr . "?selID=');";
            echo "  }";
            echo "</script>";
        }
    ...
    public function multiSelectItems()
    {
        ...
        $jml = isset($_GET['jml']) ? $_GET['jml'] : 1;
        $stepNum = $this->uri->segment(5) > 0 ? $this->uri->segment(5) : 1;
        $items = isset($_GET['items']) ? blobDecode($_GET['items']) : array();
