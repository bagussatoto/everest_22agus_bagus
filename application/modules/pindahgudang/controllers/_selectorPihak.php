...
//        cekMerah(":: $mdlName ::");
        ...
        $pihakConfirm = isset($this->configUi[$jenisTr]["pihakConfirm"]) ? $this->configUi[$jenisTr]["pihakConfirm"] : false;
            ...
            foreach ($tmpO as $row) {
                ...
                if (strlen($tmpName) > 1) {
                    ...
                    $subItems = array(
                        ...
                        "confirm" => $pihakConfirm,
//                        "gudang_asal" => my_gudang_nama(),
//                        "gudang_tujuan" => $tmpName,
                    );
                    switch ($jenisTr) {
                        case "1687":
                            $subItems["gudang_asal"] = $tmpName;
                            $subItems["gudang_tujuan"] = my_gudang_nama();
                            break;
                        default:
                        case "1587":
                            $subItems["gudang_asal"] = my_gudang_nama();
                            $subItems["gudang_tujuan"] = $tmpName;
                            break;
                    }
                    $items[] = $subItems;
                }
...
//        cekMerah(":: $mdlName ::");
        $fields = $this->configUi[$jenisTr]['selectorFields'];
        $addPihakFields = $this->configUi[$jenisTr]['addPihakFields'];
        $pihakFilters = isset($this->configUi[$jenisTr]['pihakFilters']) ? $this->configUi[$jenisTr]['pihakFilters'] : array();
        $pihakAddStaticEntry = isset($this->configUi[$jenisTr]['pihakAddStaticEntry']) ? $this->configUi[$jenisTr]['pihakAddStaticEntry'] : array();
...
//                arrPrintKuning(arrAvailFields());
                if (strlen($tmpName) > 1) {
                    ...
                    $hsl = "";
                    foreach ($addPihakFields as $addField) {
                        if ($hsl == "") {
                            $hsl = $row->$addField;
                        }
                        else {
                    ...
                    $newTmpName_hsl = $hsl . " &nbsp; " . $newTmpName;

                    $items[] = array(
                        "id" => $row->id,
//                        "label" => $newTmpName,
                        "label" => $newTmpName_hsl,
                        "target" => $processor,
                        "label_view" => isset($row->$pihakView) ? $row->$pihakView : "",
                    );
                }
