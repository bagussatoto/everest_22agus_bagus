...
class Printing extends Modul_Controller
{
    ...
    public function viewReceipt_OLD()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            $itemNumLabels = isset($this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum] : "";
            //            $itemNumLabels2 = isset($this->configLayout[$this->jenisTr]['receipCartNumFields2'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receipCartNumFields2'][$currentStepNum] : $this->configUi[$this->jenisTr]['shoppingCartNumFields2'][$currentStepNum];
...
//        matiHere(__LINE__);
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            $receiptInWordConfig2 = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptInword2'][$currentStepNum]['in_word']) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptInword2'][$currentStepNum]['in_word'] : array();
            $receiptDetailSrcFields = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailSrcFields'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailSrcFields'][$currentStepNum] : array();
            $headerField = isset($this->configLayout[$tmpTr[0]->jenis_master]['headerField']) ? $this->configLayout[$tmpTr[0]->jenis_master]['headerField'] : "registry";
            //region tabel2 tarikan untuk kolom2 nilai (hpp, ppn, dll)
            $tmpVal_main = $tr->lookupMainValuesByTransID($trID);
            $tmpVal_detail = $tr->lookupDetailValuesByTransID($trID);
            //endregion
...
//                                    arrPrint($main);
            ...
            //            cekKuning($tmpTr[0]->jenis_master);
            //            cekKuning($mainJenis);
            // cekMerah($headerField);
            /* --------------------------------------
             * nek dak ada ini brati ambil dari registry
             * ------------------------------------------------*/
            if ($headerField == "heTransaksi_layout") {

                $receiptDetailFields = $headerField = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields'][$currentStepNum] : array();
                $receiptSumFields = $receiptSumFieldsLayout = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFields'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFields'][$currentStepNum] : array();
            }

//            arrPrintHijau($itemsRegistries);
...
//
        ...
        //-----arrpri---------------
//        arrPrintKuning($receiptSumFields);
        // arrPrintKuning($elements);
        // arrPrintKuning($arrCompanyProfile);

        $receiptEfakturFields = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturFields'][$step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturFields'][$step_number] : array();
        //--------------------
        // cekHere($row->step_number);
        // arrPrint($receiptEfakturFields);
        //arrPrintWebs($globalVars);

        //region untuk menampilkan nota yang dipilih
        $master_item_label = isset($this->configUi[$tmpTr[0]->jenis_master]['master_item_label']) ? $this->configUi[$tmpTr[0]->jenis_master]['master_item_label'] : array();
        $master_item = array();
        foreach ($master_item_label as $ky => $kyLabel) {
            if (isset($main[$ky]) && ($main[$ky] != NULL)) {
                $master_item[$ky] = $main[$ky];
            }
        }
        //endregion
...
//        arrPrint($itemLabels);
//        cekHere("[stepNumber: $stepNumber]");
        switch ($tmpTr[0]->jenis_master) {
            case "7468":
                $viewMode = "viewReceiptReg";
                if(isset($globalVars["modePymSrc"]) && ($globalVars["modePymSrc"] == "pymSrc")){
                    $itemLabels = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFieldsPymSrc'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFieldsPymSrc'][$row->step_number] : array();
//                    arrPrintWebs($this->configUi[$tmpTr[0]->jenis_master]['shoppingCartFieldsPymSrc']);
//                    arrPrintWebs($itemLabels);
                    foreach ($itemsRegistries as $iid => $iidSpec){
                        foreach ($iidSpec as $iik => $iiv){
                            if(!isset($items[$iid][$iik])){
                                $items[$iid][$iik] = $iiv;
                            }
                        }
                    }
...
//            "mode" => "viewReceipt_mod",
            ...
            "tagihan_um" => isset($main['tagihan']) ? $main['tagihan'] : array(),
            "project__harga" => isset($main['referensi_so_project__harga']) ? $main['referensi_so_project__harga'] : 0,
            "pph23" => isset($main['pph23']) ? $main['pph23'] : 0,
            "kas_nilai" => isset($main['kas_nilai']) ? $main['kas_nilai'] : 0,
            "biaya_transfer" => isset($main['biaya_transfer']) ? $main['biaya_transfer'] : 0,
            ...
            "companyProfile" => (object)$arrCompanyProfile,
            "mainElements" => $elements,
            // "mainElements"             => $mainElements['customerDetails']['contents'],
            "elementConfigs" => $elementConfigs,
            "signHeader" => $signHeader,
            "footer" => $footer,
            "availValueKeys" => $availValueKeys,
            ...
            "modePymSrc" => isset($globalVars["modePymSrc"]) ? $globalVars["modePymSrc"] : NULL,
        );
