...
//arrPrintWebs($trTr);
        ...
        if (sizeof($trTr) > 0) {
            foreach ($trTr as $rows) {
                ...
                $notes_cancel_trans = isset($configUiJenis['cancel_trans'][0]) ? $configUiJenis['cancel_trans'][0] : NULL;
//arrPrintWebs($rows);
...
//cekHere($notes);
            ...
            $trash_trans_cancel = $trTr[0]->cancel_transaksi_id;
        }
...
//            $strContent .= "<div>NOTE:<br><span class='meta'>{mau di kasi notes apa}</span></div>";
            if ($trash_delete == 1) {
                ...
                if($trash_trans_cancel!= NULL){

                }
                else{

                    if ($notes_cancel_trans != NULL) {
                        foreach ($globalDatas as $key => $val) {
                            if (!is_array($val)) {
                                $notes_cancel_trans = str_replace("{" . $key . "}", $val, $notes_cancel_trans);
                            }
                        }
                        $notes_final = $notes_cancel_trans;
                    }
                    else {
                        $notes_final = "";
                    }
