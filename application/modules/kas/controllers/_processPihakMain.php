...
//        $this->jenisTr = $this->uri->segment(4);
    ...
    public function select()
    {
        ...
        $pihakMainRegistries = isset($this->configUi[$this->jenisTr]['pihakMainRegistries']) ? $this->configUi[$this->jenisTr]['pihakMainRegistries'] : array();
...
//                            matiHere($selectMethod);
            ...
            if (sizeof($pihakMainRegistries) > 0) {
                ...
                if (sizeof($trregTmp) > 0) {
                    ...
                    if (sizeof($main) > 0) {
                        foreach ($pihakMainRegistries as $keyval => $vals) {
                            $_SESSION[$cCode]['main'][$keyval] = isset($main[$vals]) ? $main[$vals] : 0;
                        }
                    }
                }
            }
...
//                                cekHitam("HAHAHA " . $f_ey[0]);
                ...
                if (sizeof($tmpO) == 1) {
                    $tmpid = $tmpO[0]->id;
                    $processor = MODUL_PATH . $this->configUi[$jenisTr]['selectorProcessor'] . "/$jenisTr?selector&id=$tmpid&minValue=0";
                    echo "<script>
                   top.$('#result').load('$processor')
                 </script>
                ";
                }
            }
