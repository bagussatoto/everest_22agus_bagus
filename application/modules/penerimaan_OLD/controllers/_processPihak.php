...
//            echo "</script>";
    ...
    public function selectTaxes_OLD()
    {
        ...
        $ppnGlobal = 100 + $ppnFactor;
        $dppValues = (100 * $srcValue) / $ppnGlobal;

        $ppn = round($dppValues * (10 / 100), 0);
        $tempPajak = pembulatan_pajak($dppValues, $ppnFactor);
    ...
    // ini versi ppn dan dpp ppn hanya ambil dari gerbang hasil invoicing yang sudah disimpan di payment source 749
    // oh iya, untuk pph22 masih ngitung, monggo di update ulang
...
//        $tempPajak = pembulatan_pajak($dppValues, $ppnFactor);
        ...
        //--------------------------
        $dppValues = $_SESSION[$cCode]["main"]["dpp_ppn"];
        $ppn = $_SESSION[$cCode]["main"]["ppn"];
        //--------------------------
        $newVal = $dppValues + $ppn;
        $pphNilai = round(($faktorPPH22 / 100) * $dppValues, 0);
        if ($_GET["p"] == "bendahara_negara") {
            // arrPrint($tempPajak);
//            matiHEre("under maintenance :: pph22: $pphNilai, dppPPN: $dppValues, ppn: $ppn");
            $arrTemp = array(
                "dpp_nilai" => $dppValues,
                "pph22_nilai" => $pphNilai,
                "ppn_nilai_dibayar" => $ppn,
...
//        matiHere();
        $val = $_GET["state"];
//        $new_val = $val=="true" ? 1:0;
        $_SESSION[$cCode]["main"][$_GET['key']] = $val;
//        matiHere(__LINE__."::".$val);
...
//                $ppn_final = ($ppnFactor / 100) * $value;
                ...
                $_SESSION[$cCode]["items6_sum"][$detail_key]["dpp_nppn_final"] = $subtotal;
