...
//arrPrint($items);
    ...
    public function selectPihakSalesman()
    {
        ...
        $mdlName = $this->uri->segment(5);
        $fields = $this->configUi[$jenisTr]['selectorFields'];
        $pihakFilters = isset($this->configUi[$jenisTr]['selectorFilters2']) ? $this->configUi[$jenisTr]['selectorFilters2'] : array();
        $pihakAddStaticEntry = isset($this->configUi[$jenisTr]['pihakAddStaticEntry']) ? $this->configUi[$jenisTr]['pihakAddStaticEntry'] : array();
...
//        cekHitam($this->db->last_query());
//        arrPrint($tmpIds);
        $processor = MODUL_PATH . $this->configUi[$jenisTr]['selectorProcessor2'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['selectorModel2'];
        if (sizeof($tmpO) > 0) {
            if (sizeof($configPair) > 0) {
                foreach ($tmpO as $row) {
                    if (isset($pairedStock[$row->id])) {
...
//                        "label" => $newTmpName,
    ...
    public function selectPihak_radio()
    {
