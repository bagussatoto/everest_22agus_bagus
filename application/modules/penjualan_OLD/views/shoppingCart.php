        ...
        foreach ($items as $iSpec) {
            ...
            if ($showItems) {
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            echo "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control text-right text-red text-bold bg-lime' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
...
//                            $pemicuGerbang = "onblur=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';}\" $keyupData ";
                        ...
                        case "text":
                            ...
                            if (is_numeric($fieldVal) && $keyID != 'barcode' && $keyID != 'produk_kode') {
                                echo "<span isNumeric keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:#$color;background:#f0f0f0;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                                echo "<script>
