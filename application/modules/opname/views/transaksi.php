...
//            arrPrintWebs($downloadConfig);
            $dlabel = $downloadConfig['label'];
            $djudul = strtoupper($dlabel);
            $dAction = $downloadConfig['action'];
            $dJenisTr = $downloadConfig['jenisTr'];
            $dAttr = $downloadConfig['attr'];
        ...
        if (isset($downloadConfigSupplies) && sizeof($downloadConfigSupplies) > 0) {
            //            arrPrintWebs($downloadConfig);
            $dlabel = $downloadConfigSupplies['label'];
            $dAction = $downloadConfigSupplies['action'];
            $dJenisTr = $downloadConfigSupplies['jenisTr'];
            $dAttr = $downloadConfigSupplies['attr'];
...
//                            cekkuning("$elName dataField");
    ...
    case "followupPreview":
        ...
        echo "<ul class='list-group'>";
//arrPrint($pairedItemSerial);
//arrPrintWebs($dataSerial);
//matiHere($pairedItemSerialLink);
...
//                            cekMerah(":: $key NOT editable ::");
                        ...
                        echo "<td $tdOpt >$strVal</td>";
//                        echo "";
                        if(is_numeric($val) && (in_array($key, $summary_bawah))){
                    ...
                    echo "</tr>";
                    /*
                     * untuk nampilin pair serial yang keluar
                     */
                    $ini_rowserial ="";
                    if(isset($pairedItemSerial[$id])){

                        $tableData = "";
                        if(!empty($dataSerial[$id])){
                            $tableData .= "<table id='_table_$id' class='table dataTable compact table-bordered table-striped table-hovered dataTableCompact'>";

                            $tableData .= "<thead>";
                            $nu=0;
                            foreach($dataSerial[$id] as $_sku => $_sereal){
                                $_label = explode('_', $keyName[$_sku])[0];
                                $enc_sku = preg_replace('/[^A-Za-z0-9\-]/', '', $_sku);
                                $nu++;
                                $jc = $nu%2 == 0 ? "a" : "f";
                                $tableData .= "<th>";
//                                $tableData .= "$_sku";
                                $tableData .= "<div style='background:#".$jc."2de00;' class='text-center text-bold text-uppercase'>$_label</div>";
                                $tableData .= "<div _ids='$_sku' ids='$enc_sku' class='wadah_atas_$enc_sku text-red text-small head_uniq'></div>";
                                $tableData .= "</th>";
                            }
...
//                            $tableData .= "<tr>";
                            foreach($dataSerial[$id] as $_sku => $_sereal){
                                $enc_sku = preg_replace('/[^A-Za-z0-9\-]/', '', $_sku);
                                $tableData .= "<td>";
                                foreach($_sereal as $kd => $bd){
//                                    arrprintwebs($bd);
                                    $tableData .= "<div class='col-md-6'>";
                                    $tableData .= "<div class='checkbox no-margin'>";

                                    if(isset($selectedSerial[$id][$_sku][$bd["extern_nama"]])){
                                        $checked = "checked";
                                    }
                                    else{
                                        $checked = $bd['checked'] == 1 ? "checked" : "";
                                    }
                                    $tableData .= "<label for='".$bd['extern_nama']."'>";
                                    $tableData .= "<input id='".$bd['extern_nama']."' class='c' _id='$id' _sku='$_sku' sku='$enc_sku' type='checkbox' $checked>" ;
                                    $tableData .=  $bd['extern_nama'];
                                    $tableData .=  "</label>";
                                    $tableData .= "</div>";
                                    $tableData .= "</div>";
                                }
...
//                            $tableData .= "</tr>";
                            $tableData .= "</tbody>";
                            $tableData .= "</table>";
                            $tableData .= "<div id='wadah_checker_$id' class='hidden wadah_checker'>";
                            $tableData .= "<div id='child_main'>".$iSpec['qty_selisih']."</div>";
                            foreach($dataSerial[$id] as $_sku => $_sereal){
                                $enc_sku = preg_replace('/[^A-Za-z0-9\-]/', '', $_sku);
                                $tableData .= "<span id='child_$enc_sku'>".count($_sereal)."</span>";
                            }
                            $tableData .= "</div>";
                        }

                        $colsan_serial = count($itemLabels)+1;
                        $ini_rowserial .="<tr>";
                        $ini_rowserial .="<td colspan='".$colsan_serial."'>";
                        $ini_rowserial .="$tableData";
                        $ini_rowserial .="</td>";
                        $ini_rowserial .="</tr>";
                    }
...
//                arrPrintPink($total_bawah);

                echo "<tr line=" . __LINE__ . " class='bg-info'>";
                echo "<td align='right'></td>";
                foreach ($itemLabels as $key => $label) {
                    $val_bawah = isset($total_bawah[$key]) ? $total_bawah[$key] : "";
                    $val_bawah_f = formatField($key, $val_bawah);
                    echo "<td align='right' style='$addStyle;font-weight:bold;font-size:15px;'>$val_bawah_f</td>";
                }
                echo "</tr>";
                echo "<tr line=" . __LINE__ . " class='bg-info'>";
                echo "<td align='right' colspan='8'></td>";
                foreach ($itemLabels as $key => $label) {
                    if(in_array($key, $summary_bawah)){
                        echo "<td align='left' style='font-weight:bold;ffont-size:15px;'>$label</td>";
                    }
                }
                echo "</tr>";
...
//                }
        ...
        /*
 *  SCRIPT UNTUK HANDLE UI CHECKBOX
 * */
        echo "
                ...
                <script>
                    $('.c').bind('change', function(){
                        ...
                        var sku = $(this).attr('_sku');
                        var encsku = $(this).attr('sku');
                        var serial = $(this).attr('id');

                        // klo mau pake mode GET
                         $('#result').load('$pairedItemSerialLink?state='+$(this).is(':checked')+'&sku='+encodeURI(sku)+'&id='+id+'&serial='+encodeURI(serial));

                        // klo mau pake mode POST
                        //$('#result').load('$pairedItemSerialLink', {state:$(this).is(':checked'),sku:sku,id:id,serial:serial});
                        updateWadah(encsku);
                    });

                    function updateWadah(sku){
                        var total = $(\"input[sku='\"+sku+\"']\").length;
                        var checked = $(\"input[sku='\"+sku+\"']:checked\").length;
                        var aktif = (total-checked);
                        $('.wadah_atas_'+sku).html('TOTAL: <a>'+total + '</a>  ||  AKAN DI HAPUS: <a>'+checked+'</a>'+'  ||  SISA YG AKTIF: <a>'+ aktif +'</a>')
                    }
                    ...
                    //INI BELUM SELESAI
                    //RENCANANYA MAU BUAT VALIDASI KIRI KANAN / INDOOR OUTDOOR SUDAH BALANCE / SEIMBANG NILAI NYA 50:50 ATAU BELUM
                    var arr = $('.wadah_checker');
                    jQuery.each(arr, function(a, b){
                        var minus = $( 'div', $(b)).html();
                        var arrSku = $( 'span', $(b));
                        var checkValue=0;
                        jQuery.each(arrSku, function(aa, bb){
                            var jml_now = $(bb).html();
                            var jml_hrusnya = jml_now - (minus*-1);
                            console.log('jml_now: '+jml_now);
                            console.log('jml_hrusnya: '+jml_hrusnya);
                        })
                    });
                    //RENCANANYA MAU BUAT VALIDASI KIRI KANAN / INDOOR OUTDOOR SUDAH BALANCE / SEIMBANG NILAI NYA 50:50 ATAU BELUM
