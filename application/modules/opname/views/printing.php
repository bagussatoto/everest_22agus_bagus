        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    $no++;
                    $stok_qty_t = isset($iSpec["stok"]) ? $iSpec["stok"] : 0;
                    $debet_qty_t = isset($iSpec["qty_debet"]) ? $iSpec["qty_debet"] : 0;
                    $kredit_qty_t = isset($iSpec["qty_kredit"]) ? $iSpec["qty_kredit"] : 0;
                    $opname_qty_t = isset($iSpec["qty_opname"]) ? $iSpec["qty_opname"] : 0;


                    // $stok_qty +=$stok_qty_t;
                    // $debet_qty +=$debet_qty_t;
                    // $kredit_qty +=$kredit_qty_t;
                    // $opname_qty +=$opname_qty_t;
                    //ini sementara
                    // if(!isset($mainValues["stok"])){
                    //     $mainValues["stok"] = 0;
                    // }
                    // if(!isset($mainValues["qty_debet"])){
                    //     $mainValues["qty_debet"] = 0;
                    // }
                    // if(!isset($mainValues["qty_kredit"])){
                    //     $mainValues["qty_kredit"] = 0;
                    // }
                    // if(!isset($mainValues["qty_opname"])){
                    //     $mainValues["qty_opname"] = 0;
                    // }
                    //
                    //
                    // $mainValues["stok"] +=$stok_qty_t;
                    // $mainValues["qty_debet"] +=$debet_qty_t;
                    // $mainValues["qty_kredit"] +=$kredit_qty_t;
                    // $mainValues["qty_opname"] +=$opname_qty_t;
                ...
                if (isset($sumRows) && sizeof($sumRows) > 0) {
                     ...
                     * jumlah bawah
                     * ------------------------------------------------------------------------*/
                    // cekBiru(sizeof($itemLabels));
                    // arrPrint($itemLabels);
                    $contentStr .= "<tr class='bg-danger' line=" . __LINE__ . ">";
                    $contentStr .= "<td>-</td>";
                    foreach ($itemLabels as $key => $label) {
                        $main_key = $sumRows[$key];
                        $sumNilai = isset($mainValues[$main_key]) ? $mainValues[$main_key] : "-";
                        $sumNilai_f = formatField_he_format($key, $sumNilai);

                        $contentStr .= "<td>$sumNilai_f</td>";
                    }
                    $contentStr .= "</tr>";
                }
