...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            // $resultItems = blobDecode($tmpReg[0]->rsltItems);
            // arrPrint($resultItems);
            ...
            //endregion
            // arrPrintPink($receiptRsltItems);

            $arrItemResult = array();
            foreach ($receiptRsltItems as $receiptRsltItem) {
                $sub_hpp = $receiptRsltItem['sub_hpp'];

                // $arrItemResult[$receiptRsltItem['id']] = $receiptRsltItem;
                if(!isset($arrItemResult[$receiptRsltItem['id']]['sub_hpp'])){
                    $arrItemResult[$receiptRsltItem['id']]['sub_hpp'] = 0;
                }
                $arrItemResult[$receiptRsltItem['id']]['sub_hpp'] += $sub_hpp;
            }
            // matiHEre("stopppdulu brooo");
            // cekBiru($arrItemResult);
            // cekBiru(count($tmpTr));
            ...
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582")) {
                ...
                foreach ($tmpTr as $row) {
                    ...
                    //     $arrBaru = array_replace($arrItemResult[$id], array_filter($rawItems[$row->produk_id]));
                    //     $arrLama = array_replace(array_filter($childTableInValueParams[$row->produk_id]), array_filter($rawItems[$row->produk_id]));
                    //     $rawItems[$row->produk_id] = isset($arrItemResult[$id]) ? $arrBaru : $arrLama;
                    if(isset($arrItemResult[$id])){
                        $rawItems[$row->produk_id]['nilai_kredit'] = $arrItemResult[$id]['sub_hpp'];
                    }
                    // }
                    // else {
                        $rawItems[$row->produk_id] = array_replace(array_filter($childTableInValueParams[$row->produk_id]), array_filter($rawItems[$row->produk_id]));
        ...
        // arrPrintHijau($tmpTr[0]->jenis_master . " ['receipSumFields'] ".$row->step_number);
        //--------------------
        ...
        // arrPrint($receiptSumFieldsLayout);
        ...
        //reseter jumlah produk di main
        // arrprint($main);
        // unset($main["stok"]);
        // unset($main["qty_debet"]);
        // unset($main["qty_kredit"]);
        // unset($main["qty_opname"]);
        // cekBiru($itemLabels);
        $data = array(
            //            "mode"       => $this->uri->segment(2),
            "mode"                     => $mode,
    ...
    public function viewReceiptOpname()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            //region take from registries
            $tmpReg0 = $tr->lookupDataRegistriesByMasterID($trID)->result();
            $tmpReg = array();
            foreach ($tmpReg0 as $spec) {
                foreach ($spec as $key_reg => $val_reg) {
                    if ($key_reg != "transaksi_id") {
                        $tmpReg[$key_reg] = $val_reg;
                    }
                }
