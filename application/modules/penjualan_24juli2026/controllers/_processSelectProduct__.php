    ...
    public function select()
    {
        ...
        * -----------------------------------------------------------*/
        ...
        foreach ($potongan_nilai as $dcu_kategori => $dcu) {
            ...
            if ($nilai_dcu > 0) {
                // if(isset($_SESSION[$cCode]["main"]["nett1_bulat"])){
                //     $new_nett1_bulat = $_SESSION[$cCode]["main"]["nett1_bulat"] - $nilai_dcu;
                // }
                // cekOrange(__LINE__ . " nilai_dcu:: $nilai_dcu == $new_nett1_bulat");
                // $_SESSION[$cCode]["main"]["nett1_bulat"] = $new_nett1_bulat;
            }
        }
        // -----------------------------------------------------------------------------end
        ...
        if (sizeof($tmpB) > 0) {
            ...
            /* -----------------------------------------------------------
             * logic yg harus ditambahkan 2023-12-29
             * jumlah unit tidak boleh masuk akumusai unit, jika sudah mendapatkan diskon kategori
             *
             * jika akumulasi unit masuk diskon unit, maka masing2 produk menadapatkan potongan sesuai dengan diskon berjenng level 1
             * arghhhh...............
             * ---------------------------------------*/
            ...
            //     // ---------------------------------------------------------
            //
            //     $potongan_nilai = $this->selectorDiskonKategori($_SESSION[$cCode]);
            //
            //     arrPrintKuning($potongan_nilai);
            //     // arrPrintKuning($produk_jenis);
        ...
        cekHitam(sizeof($potongan_nilai) . " masuk diskon kategori @" . __LINE__);
        if (sizeof($potongan_nilai) > 0) {

            $rows = array();
            foreach ($_SESSION[$cCode]['items'] as $prodId => $sessItem) {
                $pro_jml = $sessItem['jml'];
                $pro_harga_reseller = $sessItem['jual_reseller'];
...
arrPrint($item['nilai']);
                    if ($item['nilai'] > 0) {
                        ...
                        // $calc_hasils = $this->selectorDiskon($id, $pro_harga_reseller, $pro_jml, $rows, $sesmain);
                        $calc_hasils = $ld->selectorDiskon($prodId, $pro_harga_reseller, $pro_jml, $rows, $sesmain);
                        $calc_hasil = $calc_hasils["grosir"];
                ...
                cekHitam($prodId ." @". __LINE__);
                arrPrintWebs($calc_hasil);

                $tmp2['harga'] = $calc_hasil['harga_af'];
                $itemsPorduks = $_SESSION[$cCode]['items'][$prodId];
                arrPrintHijau($itemsPorduks);

                foreach ($tmp2 as $ky_tmp2 => $val_tmp2) {
                    if(array_key_exists($ky_tmp2, $itemsPorduks)){
                        cekHere("$ky_tmp2 $prodId");
                        // $_SESSION[$cCode]['items'][$prodId][$ky_tmp2] = $val_tmp2;
                    }
                }
            }
        }
    ...
    private function selectorDiskon($produk_id, $produk_harga, $produk_jml, $produk_speks = array(), $ses_mains = array())
    {
        ...
        $src_diskon_00 = $src_diskon = $dp->CallProdukDiskon($produk_id);
    ...
    {
        matiHere("pindah ke library diskon");
