...
//if($transaksi_idd == 627525){
//    cekHere("[$disabled_hold] [$disabled_button_reject]");
//}
                ...
                if ($allowFollowup) {
                    ...
                    if ($this->session->login["employee_type"] == "employee_kirim") {
                        ...
                        if ($allowScaner == false || $isMob == 1) {
                            ...
                            $tmp['action'] .= "<button id='btn_approve_$transaksi_idd' class='btn $btn_block $class_button' title='turn this entry into $stepLabel' 
                                $disabled_hold
                                href='javascript:void(0)' 
                                onClick =\"top.open_holdon();$followupLink\">" . $actionLabel . "</button>";
                        }
                            ...
                            $tmp['action'] .= "<button id='btn_approve_$transaksi_idd' class='btn btn-primary btn-block' $disabled_button title='turn this entry into $stepLabel' href='javascript:void(0)' onClick =\"$followupLink\">" . $actionLabel . "</button>";
                        }
                        $tmp['action'] .= isset($strPengirim) ? $strPengirim : "";
                        $tmp['action'] .= "</div class='input-group'>";
                        $tmp['action'] .= $add_note;
                ...
                if ($pakai_ini == 1) {
                    ...
                    if (in_array($row->sub_step_number, $arrStepAllow)) {
                        ...
                        $evPost = evaluatePostProcessors_he_menu($this->jenisTr, $currentStepNum, $this->configCoreJenis, $this->configUiJenis);
                        $evCom = evaluateComponents_he_menu($this->jenisTr, $currentStepNum, $this->configCoreJenis, $this->configUiJenis);
//                        if ($row->customers_id == 3169) {
//                            cekHitam($row->transaksi_id . " || evPreProcc: $evPre, evPostProcc: $evPost, evCom: $evCom [$disabled_hold || $disabled_button_reject]");
//                        }
