...
//        );
        ...
        $selectorDetailModel = $this->configUi[$this->jenisTr]['selectorDetailModel'];
        //-----------------------------------------------
                ...
                foreach ($row as $key_reg => $val_reg) {
                    ...
                    switch ($key_reg) {
                        ...
                        case "main":
                            ...
                            if (!isset($main["ppnFactor"])) {
                                matiHEre("old transaksi ppn 10 persen ." . $id);
                            }
                            else {
                                $_SESSION[$cCode]['main']["ppnFactor"] = $main["ppnFactor"];
                            }
                            break;
                    }
                }
...
//arrPrint($tmpB);
            ...
            foreach ($tmpB as $row) {
                ...
                if ($tmpJml > 0) {
                    //--------------
                    $this->load->model("Mdls/" . $selectorDetailModel);
                    $bd = new $selectorDetailModel();
                    $bd->setFilters(array());
                    $tmpBD = $bd->lookupByID($id)->result();
                    $produk_jenis = $tmpBD[0]->kategori_nama;
                    $rows = $tmpBD[0];
                    //--------------
...
//arrPrintPink($tmp);
                    ...
                    if ($produk_jenis == "unit") {
                        ...
                        $_SESSION[$cCode]['items2'][$id][$rows->barcode] = array();
                        $arrCat["barcode"] = 1;
        ...
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("undefine ppn factor @" . __LINE__ . " " . __METHOD__);
...
//                }
    ...
    public function edit()
    {
        ...
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $arrDataTambahan = isset($this->configUi[$this->jenisTr]['produkUnitPart']) ? $this->configUi[$this->jenisTr]['produkUnitPart'] : array();
        $selectorDetailModel = isset($this->configUi[$this->jenisTr]['selectorDetailModel']) ? $this->configUi[$this->jenisTr]['selectorDetailModel'] : NULL;
...
//                        }
            ...
            if ($selectorDetailModel != NULL) {
                $this->load->model("Mdls/$selectorDetailModel");
                $b = new $selectorDetailModel();
                $b->setFilters(array());
                $tmpB = $b->lookupByID($id)->result();
                if (sizeof($tmpB) > 0) {
                    foreach ($tmpB as $row) {
    ...
    public function cancel()
    {
        ...
        if (isset($_SESSION[$cCode]['items'][$id])) {
            if (isset($_GET['newQty'])) {
                //qty tidak boleh 0, kalau item tidak di-closed
                //maka klik tanda silang/remove saja
                if ($_GET['newQty'] < 1) {
                    $item_nama = $_SESSION[$cCode]['items'][$id]['nama'];
                    $msg = "jumlah cancel/close $item_nama tidak boleh 0. Bila item ini tidak ada cancel, maka klik tombol silang/remove di masing-masing item.";
...
//                    $msg = "Insufficient m:$max_jml - s:$sent_jml - c:$cancel_jml - p:$packed_jml of:<br><red class='text-red'>$kode $nama</red><hr>$max_qty $satuan stock available";
                    $item_nama = $_SESSION[$cCode]['items'][$id]['nama'];
                    $msg = "Jumlah cancel/close $item_nama yang diisi melebihi jumlah order. Silahkan dikoreksi lagi.";
                    $alerts = array(
                        "type" => "warning",
