...
//        }
        ...
        * ---------------------------------------------------------------------*/
        ...
        //region menambah reseller (kalau berwenang)
        $pihakModelProject = isset($this->configUi[$this->jenisTr]['pihakModelProjek']) ? $this->configUi[$this->jenisTr]['pihakModelProjek'] : "";
        $dataAccess = isset($this->config->item('heDataBehaviour')[$pihakModelProject]) ? $this->config->item('heDataBehaviour')[$pihakModelProject] : array(
            "viewers" => array(),
            "creators" => array(),
            "creatorAdmins" => array(),
            "updaters" => array(),
        ...
        $dataLabel = isset($this->config->item('heDataBehaviour')[$pihakModelProject]) ? $this->config->item('heDataBehaviour')[$pihakModelProject]['label'] : $pihakModelProject;
        $mems = isset($this->session->login['membership']) ? $this->session->login['membership'] : array();
        $addPihakStrProject = "";
        $addLinkProject = base_url() . "Data/add/" . str_replace("Mdl", "", $pihakModelProject);
            ...
            if (sizeof(array_intersect($mems, $dataAccess['creators'])) > 0) {
                $addClickProject = "
                    BootstrapDialog.show(
                                   {
                                        title:'New " . $dataLabel . "',
                                        message: $('<div></div>').load('" . $addLinkProject . "'),
                                        draggable:true,
                                        closable:true,
                                        size:top.BootstrapDialog.SIZE_WIDE,
                                        }
                                        );";
                $addPihakStrProject = "<span class='input-group-btn'>
                                        <a href='javascript:void(0)' title='add $dataLabel' data-toggle='tooltip' data-placement='right' class='btn btn-default' onclick=\"$addClickProject\">
        ...
        // arrPrintKuning($sesItems);

        $this->load->library("Diskon");
        $dp = new Diskon();
        $dp->setTokoId(my_toko_id());
        $src_diskon = $dp->CallProdukDiskon();
        ...
        $data = array(
            ...
            "resellerNama" => isset($_SESSION[$cCode]['main']['pihakResellerName']) ? $_SESSION[$cCode]['main']['pihakResellerName'] : "",
            "pihakNama" => isset($_SESSION[$cCode]['main']['pihakName']) ? $_SESSION[$cCode]['main']['pihakName'] . "(" . $this->configUi[$jenisTr]["pihakLabel"] . ")" : "",
            ...
            "addPihakStrProject" => $addPihakStrProject,
            ...
            "sesItems" => $sesItems,
            "diskon_produks" => $diskon_produks,
                        // ----------------------
            "pihakProjekCallerDelete" => MODUL_PATH . "_processPihak/removeReseller/$jenisTr",
            ...
            "pihakProjekLabel" => isset($_SESSION[$cCode]['main']['pihakResellerName']) ? $_SESSION[$cCode]['main']['pihakResellerName'] : isset($this->configUi[$jenisTr]["pihakProjekLabel"]) ? $this->configUi[$jenisTr]["pihakProjekLabel"] : "",
    ...
    public function preview()
    {
        ...
        $itemSubNumLabels = isset($configUi[$this->jenisTr]['shoppingCartSubNumFields'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartSubNumFields'][$stepNumber] : array();
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items'])) {
                foreach ($_SESSION[$cCode]['items'] as $iSpec) {
                    ...
                    if(isset($itemSubNumLabels) && count($itemSubNumLabels) > 0){
                        foreach ($itemSubNumLabels as $skey => $itemSubNumLabel) {
        ...
        // $itemLabels = $itemLabels + $itemNumLabels + array("subtotal_dropshiper" => "sub-amount");
        $itemLabels = $itemLabels + $itemNumLabels + $itemSubNumLabels;
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
            ...
            if (strlen($connector) > 0) {
                ...
                if (sizeof($steps) == 1) {
                    ...
                    $clonerTransaction = isset($this->configUi[$this->jenisTr]['clonerTransaction'][$oldStep]) ? $this->configUi[$this->jenisTr]['clonerTransaction'][$oldStep] : array();
            ...
            $connectToStepByOptionData = isset($this->configUi[$this->jenisTr]['connectToStepByOption'][1]) ? $this->configUi[$this->jenisTr]['connectToStepByOption'][1] : NULL;
            ...
            if ($connectToStepByOptionData != NULL) {
                ...
                if (isset($connectToStepByOptionData[$keyMethode])) {
                    ...
                    $connectToStepMainBuilder = isset($this->configUi[$this->jenisTr]['connectToStepMainBuilder'][1]) ? $this->configUi[$this->jenisTr]['connectToStepMainBuilder'][1] : array();
                    ...
                    $oldStep = $_SESSION[$oldCode]['main']["step_number"];
                    $clonerTransaction = isset($this->configUi[$this->jenisTr]['clonerTransaction'][1]) ? $this->configUi[$this->jenisTr]['clonerTransaction'][1] : array();
                    if (sizeof($clonerTransaction) && isset($clonerTransaction['main']['cloner'])) {
                        cekHere("i am here");
                        $rebuildGate = array();
                        if (isset($clonerTransaction['resetGate']) && (sizeof($clonerTransaction['resetGate']) > 0)) {
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
    ...
    public function recordColumn()
    {
        ...
        // unset($_SESSION[$cCode]["main"]["ppnFactor"]);
    ...
    //tool generator salespakageto reguler
    public function generatePakageToRegulerSales()
    {
        //region connecting antar cabang
        $masterID = "120503";//jangan lupa diganti broooo
        // $this->jenisTr="1582";
        cekHitam($masterID);
        $this->load->model("MdlTransaksi");
        $tr = new MdlTransaksi();
        $dataRegistry = $tr->lookupDataRegistriesByMasterID($masterID)->result();
        $cCode = $this->cCode;

        foreach ($dataRegistry as $data) {
            foreach ($data as $kolom => $value) {
                if ($kolom == "transaksi_id") {

                }
                else {
                    $_SESSION[$cCode][$kolom] = blobDecode($value);
                }
            }
            arrPrint($data);
        }
        arrPrint($_SESSION[$cCode]["tableIn_master"]);
        $tmpNomorNota = isset($_SESSION[$cCode]["tableIn_master"]["inv"]) ? $_SESSION[$cCode]["tableIn_master"]["inv"] : "";
    ...
    public function infoDiskon()
    {
        ...
        * -----------------------------------------------------*/
        ...
        $this->load->library("Diskon");
        $dp = new Diskon();
        $strDiskon = $dp->cekDiskonProduk($sesItems, $cabang_id, $toko_id);
