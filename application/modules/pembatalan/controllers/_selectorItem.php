...
class _selectorItem extends Modul_Controller
{
    ...
    public function selectItem()
    {
         ...
         * ----------------------------------------------------------------------------------------------------------*/
        if (strlen($key) >= 4) {
            $this->db->limit(10); //dimatikan karena tidak bisa select all produk
...
//        cekungu(__LINE__ . " ==== sampai membaca isi nota: $val");
...
//            cekungu(__LINE__ . " ==== sampai membaca isi nota: $val");
            ...
            $tmpP = array();
            if (isset($dataSrc) && (sizeof($dataSrc) > 0)) {
            ...
            foreach ($tmpO as $row) {
                ...
                $tmp['target'] = $row->trash_4 == 0 ? $processor : "#";
                ...
                if (sizeof($selectorFields) > 0) {
                    ...
                    if ($row->suppliers_nama != NULL) {
                        $tmp['label'] .= "<span style='font-size:1em;margin:0px 2px 0px 2px;color:#000000;' class='no-padding no-border text-left'>" . $row->suppliers_nama . "</span>";
                    }
                    if ($row->customers_nama != NULL) {
                        $tmp['label'] .= "<span style='font-size:1em;margin:0px 2px 0px 2px;color:#000000;' class='no-padding no-border text-left'>" . $row->customers_nama . "</span>";
                    }

                    $tmp['label'] = rtrim($tmp['label'], "| ");
                    $tmp['label'] = ($tmp['label'])."**";

                    unset($tmp['target_allow']);
                    unset($tmp['style']);
                    ...
                    if ($row->trash_4 == 0) {
                        ...
                        $li_style = "background:#aaa";
                        $reject_name = $row->cancel_name;
                        $reject_date = formatTanggal("Y-m-d", $row->cancel_dtime);

                        $tmp['label'] = "<span style='font-size:1.1em;margin:0px 2px 0px 2px;color:#f10505;' class='no-padding no-border text-left text-bold'> Sudah di reject oleh " . $reject_name . "|" . $reject_date . "</span>";
                        $tmp['style'] = $li_style;
                        $tmp['target_allow'] = "disabled";
                    }
                    //------
                    if ($_SESSION[$cCode]['main']['pihakExternRevertReference'] == true) {

                        $this->load->model("Mdls/MdlTransaksiDataEfaktur");
                        $tf = New MdlTransaksiDataEfaktur();
                        $tf->addFilter("produk_id='$pID'");
                        $tf->addFilter("jumlah='1'");
                        $trTmp = $tf->lookupAll()->result();
                        showLast_query("hitam");
                        cekHitam(count($trTmp));
//                        if (sizeof($trTmp) > 0) {
//                            $nomer = formatField("nomer_nolink", $trTmp[0]->nomer);
//                            $oleh_name = $trTmp[0]->oleh_nama;
//                            $dtime = $trTmp[0]->dtime;
//                            $efaktur = $trTmp[0]->efaktur;
//                            $date_faktur = $trTmp[0]->date_faktur;
//                            $tmp['label'] = "<span style='font-size:1.1em;margin:0px 2px 0px 2px;color:#f10505;' class='no-padding no-border text-left text-bold'> Nomer Efaktur sudah diinput $nomer oleh $oleh_name ($dtime) </span>";
//                            $tmp['style'] = $li_style;
//                            $tmp['target_allow'] = "disabled";
//                        }
