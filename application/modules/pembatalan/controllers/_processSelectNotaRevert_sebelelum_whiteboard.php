...
//        showLast_query("hitam");
        ...
        cekungu("sampai membaca isi nota: $val");
...
//        showLast_query("biru");
        ...
        $endtime = microtime(true); // Bottom of page
        $val = $endtime - $starttime;
        cekungu("sampai membaca registry nota: $val");
...
//            mati_disini("$jenisMasterRef :: $transID_ref");
            ...
            $pembatalanValidateConfig = isset($this->config->item('heTransaksi_pembatalanValidate')[$jenis_master]) ? $this->config->item('heTransaksi_pembatalanValidate')[$jenis_master] : array();
//            $pembatalanValidateConfig = array();
            if (sizeof($pembatalanValidateConfig) > 0) {
                foreach ($pembatalanValidateConfig as $pembatalanSpec) {
                    $mdlNameValidate = $pembatalanSpec['mdlName'];
                    $mdlFilterValidate = isset($pembatalanSpec['mdlFilter']) ? $pembatalanSpec['mdlFilter'] : array();
...
//                        arrPrintWebs($dTr);
            ...
            $endtime = microtime(true); // Bottom of page
            $val = $endtime - $starttime;
            cekungu("sampai membaca pembatalan validate: $val");

            // ================== tambahan membaca activity
            // ==================
            cekHitam("$jenisTr_reference");
//            $pembatalanChecker = array();
...
//            matiHere(__LINE__);
            ...
            switch ($jenis_master) {
                ...
                case "5823spd":
                    ...
                    $t->addFilter("jenis='110'");
                    $tTmp = $t->lookUpAll()->result();
                    showLast_query("kuning");
                    if (sizeof($tTmp) > 0) {
                        $next_step = $tTmp[0]->next_step_num;
                        if ($next_step > 2) {
//                            arrPrintKuning($tTmp[0]);
                            $nomer_alias = $tTmp[0]->nomer2;
                            $msg = "Transaksi Penjualan Nomer <strong>$tr_no_dibatalkan</strong> tidak bisa dibatalkan karena Nomer Faktur PPN Keluaran sudah dibuat. ";
                            $msg .= "Bila akan dibatalkan dimulai dari Finance (batalkan Otorisasi Faktur PPN Keluaran Nomer <strong>$nomer_alias</strong> di DC/Pusat). ";
//                            die(lgShowAlertMerah($msg));
                            mati_disini($msg);
                        }
                    }
...
//        arrPrintCyan($preProcessor['detail']);
//        mati_disini(__LINE__);
...
//        cekUngu("[fetchSwapPreProc]");
//        arrPrintCyan($swapPreProc);
//        arrPrintKuning($swapPreFifo);
//        mati_disini(__LINE__);
...
//        if ($_SERVER['REMOTE_ADDR'] == "202.65.117.72") {
//            arrPrintHitam($revertedJurnal_new);
