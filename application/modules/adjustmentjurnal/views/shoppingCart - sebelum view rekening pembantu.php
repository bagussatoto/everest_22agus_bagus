...
if (isset($items)) {
    ...
    if (sizeof($itemsjurnal) > 0) {
        ...
        /*=============== BADGE PPN / NON PPN =================*/
        echo "<table class='table table-condensed no-padding table-bordered no-margin table-hover'>";
        /*===============header shoping cart======================*/
        ...
        if(sizeof($itemsjurnal)>0){
            ...
            foreach($itemsjurnal as $rekID =>$label){
                $i++;
                echo "<tr class='bg-grey-2 text-uppercase text-left'>";
                echo "<td>$i</td>";
                echo "<td>$rekID</td>";
                echo "<td>$label</td>";
                foreach ($headerMaster as $master_h =>$masterLabel){
                                ...
                                if(isset($jurnalEditableFields[$master_h])){
                                    foreach($jurnalEditableFields[$master_h] as $editable){
                                        if($kolomSrc == $editable){
                                            ...
                                            echo "<td><input type='text' id='".$colID[$key]."' value='".niceDecimal($val)."'  class='text-right' autocomplete='off' onclick='this.select()' $pemicuGerbang></td>";
                                        }
                                    }
                                    ...
                                    echo "<td>".formatField_he_format("harga",$itemsJurnal2[$rekID][$master_h][$kolomSrc])."</td>";
                                }
                ...
                if(isset($header2[$master_key])){
                    foreach($header2[$master_key] as $k =>$y){
                        if(isset($sumfields["sumFooter"][$master_key][$k])){
                            echo "<td>".formatField("subtotal",$sumfields["sumFooter"][$master_key][$k])."</td>";
                        }
                    }
                }
            }
