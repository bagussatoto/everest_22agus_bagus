...
//                $strOnprogFooter = "";
        ...
        //endregion
        $str .= $strOnprog;
        if(isset($create_display) && ($create_display>1)){

            $str .= "<script>";
            $str .= "document.getElementById('9999?gr=dW5kZWZpbmU').style='display:none';";
            $str .= "</script>";
        }
...
//                    $str.=formatField($k,$pSpec[$k]);
    ...
    case "preview":
        ...
        $valueHeaderLabel = array(
            "debet_prev" => "red",
            "kredit_prev" => "red",

            "debet_curent_ori" => "blue",
            "kredit_curent_ori" => "blue",
...
//            arrPrint($items);
            ...
            foreach ($itemLabels as $key => $label) {
                $headColor = isset($valueHeaderLabel[$key]) && $valueHeaderLabel[$key] != "" ? "border-bottom: 2px solid " . $valueHeaderLabel[$key] . ";" : "";
                echo "<th class='text-muted text-center' style='font-weight:bold; $headColor'>";
                echo $label;
                echo "</th>";
            }
            echo "</tr>";
            ...
            foreach ($items as $iSpec) {
                $no++;
                $fieldVal = "";
                $sumDebet += $iSpec["debet_adj"];
                $sumKredit += $iSpec["kredit_adj"];

                foreach ($valueHeaderLabel as $ky => $lb) {
                    if (!isset($footerSum[$ky . "_sum"])) {
                        $footerSum[$ky . "_sum"] = 0;
                    }
                    $footerSum[$ky . "_sum"] += isset($iSpec[$ky]) ? $iSpec[$ky] : 0;
                }
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    $rek_type = $key_split[0];
                    $key_suffix = $key_split[1];

                    echo "<td rek_type='$rek_type'>";
                    if (substr($key, 0, 1) == "*") {
                        ...
                        if (isset($iSpec['childs']) && $key == "nama") {
                            $rowChild = count($iSpec['childs']);

//                            if($rek_type=="debet"){
//
//                            }
//                            else if($rek_type=="kredit"){
//
//                            }
//                            else{
//                                if(array_key_exists($key, $valueHeaderLabel)){
//                                    $fieldVal = isset($iSpec[$key."_"]) ? "<span rek_type='$rek_type' style='cursor: pointer;' class='text-bold text-primary' title='click untuk melihat rincian pembantunya...' onclick=\"openChild('pem_".$iSpec['head_code']."')\">".formatField_he_format($key,$iSpec[$key."_"])." ($rowChild pembantu)</span>" : "";
//                                }
//                                else{
                            $fieldVal = isset($iSpec[$key]) ? "<span rek_type='$rek_type' style='cursor: pointer;' class='text-bold text-primary' title='click untuk melihat rincian pembantunya...' onclick=\"openChild('pem_" . $iSpec['head_code'] . "')\">" . formatField_he_format($key, $iSpec[$key]) . " ($rowChild pembantu)</span>" : "";
//                                }
//                            }
...
//                            if(array_key_exists($key, $valueHeaderLabel)){
//                                $fieldVal = isset($iSpec[$key."_"]) ? formatField_he_format($key,$iSpec[$key."_"],0) : "";
//                            }
//                            else{
                            $fieldVal = isset($iSpec[$key]) ? formatField_he_format($key, $iSpec[$key], 0) : "";
//                            }
                        }
                    }
                    echo $fieldVal;
                ...
                if (isset($iSpec['childs'])) {
                    $childs = $iSpec['childs'];
                    $sumTotalFooter = array();
                    foreach ($childs as $externIDPem => $pemData) {
                        echo "<tr style='font-size: 10px;background-color:#f9daff' class='pem_" . $iSpec['head_code'] . " no-padding collapse out'>";
                        echo "<td class='no-padding'>&nbsp;</td>";
                        echo "<td class='no-padding'>&nbsp;</td>";
                        echo "<td style='padding-left: 15px!important;' class='no-padding text-capitalize'>";
                        echo "- " . $pemData['extern_nama'];
                        echo "</td>";
                        foreach ($valueHeaderLabel as $ky => $pemLabel) {
                            echo "<td style='padding-right: 5px!important;' class='no-padding text-right'>" . (isset($pemData[$ky]) ? number_format($pemData[$ky]) : 0) . "</td>";
                            if (!isset($sumTotalFooter['total_' . $ky])) {
                                $sumTotalFooter['total_' . $ky] = 0;
                            }
                            $sumTotalFooter['total_' . $ky] += (isset($pemData[$ky]) ? $pemData[$ky] * 1 : 0);
                        }
                        echo "</tr>";
                    }
                    echo "<tr style='font-size: 12px;background-color:#F9FFC1' class='pem_" . $iSpec['head_code'] . " no-padding text-green text-bold collapse out'>";
                    echo "<th class='no-padding'>&nbsp;</th>";
                    echo "<th class='no-padding'>&nbsp;</th>";
                    echo "<th class='no-padding'>&nbsp;</th>";
                    foreach ($valueHeaderLabel as $ky => $pemLabel) {
                        echo "<th style='padding-right: 5px!important;' class='no-padding text-right'>" . (isset($sumTotalFooter['total_' . $ky]) ? number_format($sumTotalFooter['total_' . $ky]) : 0) . "</th>";
                    }
                    echo "</tr>";
                }
            ...
            foreach ($valueHeaderLabel as $ky => $lb) {
                echo "<th class='text-right'>" . formatField_he_format("debet_adj", $footerSum[$ky . "_sum"]) . "</th>";
            }
...
//                            cekkuning("$elName dataField");
            ...
            echo "<script>
                        function openChild(classed){
                            var class_pem = $('.'+classed);
                            jQuery.each(class_pem, function(a,b){
                                if($(b).hasClass('out')) {
...
//        if (isset($items) && sizeof($items) > 0) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    $rowSpanSub = isset($subDetail[$id]) ? sizeof($subDetail[$id]) : "1";
...
//                            cekKuning(":: $key editable ::");
                            ...
                            if (isset($subDetail[$id]) && $key == "nama") {
                                $rowChild = count($subDetail[$id]);
                                $strVal = "<span style='cursor: pointer;' class='text-bold text-primary' title='click untuk melihat rincian pembantunya...' onclick=\"followUpOpenChild('pem_" . $detailValues[$id]['rekening'] . "')\">" . formatField_he_format($key, $val) . " ($rowChild pembantu)</span>";
                            }
                            else {
...
//                    arrPrint($subDetail);
//                    if(isset($subDetail) && sizeof($subDetail)>0){
//                        if(isset($subDetail[$id])){
//                            foreach($subDetail[$id] as $dataSubdetail){
////                                echo "<tr style='background-color:#f6c7ff' class='pem_".$dataSubdetail['rekening_id']." no-padding collapse out'>";
////                                echo "<td></td>";
////                                echo "<td></td>";
////                                foreach($subDetailLabel as $key =>$label){
////                                    echo "<td><i>".( isset($dataSubdetail[$key]) ? formatField($key,$dataSubdetail[$key]) : formatField($key,0) )."</i></td>";
////                                }
////                                echo "</tr>";
//
//                                $childs = $subDetail;
...
//                }
            ...
            echo "<script>
                        function followUpOpenChild(classed){
                            var class_pem = $('.'+classed);
                            jQuery.each(class_pem, function(a,b){
                                if($(b).hasClass('out')) {
                                    $(b).addClass('in').removeClass('out');
...
//                                            + \"<div class='text-right'>\"+addCommas(total5)+\"</div>\"
    ...
    case "viewResume":
        ...
        if (isset($detil_item_rslt) && sizeof($detil_item_rslt) > 0) {
            ...
            echo "<h4 detil_item_rslt class='text-blue'><span class='fa fa-book'>$detail_title</span></h4>";
            ...
            if (isset($detail_items) && sizeof($detail_items) > 0) {
                if (isset($itemLabels) && (sizeof($itemLabels) > 1)) {
                    ...
                    echo "<h4 detail_items class='text-blue'><span class='fa fa-book'>$detail_title </span></h4>";
                    echo "<div class='table-responsive'>";
                    echo "<table class='table table-bordered table-condensed' style='background:#ffffff;'>";
                    echo "<tr bgcolor='#f5f5f5'>";
                    echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
                    ...
                    foreach ($detail_items as $iSpec) {
                        ...
                        foreach ($itemLabels as $key => $label) {
                            ...
                            if (substr($key, 0, 1) == "*") {
                                ...
                                if ($key == "kredit_adj") {
                                    $total_kredit += isset($iSpec[$key]) ? $iSpec[$key] * 1 : 0;
                                }
                                else if ($key == "debet_adj") {
                                    $total_debet += isset($iSpec[$key]) ? $iSpec[$key] * 1 : 0;
                                }
                                else {
...
//                    arrPrint($addRows);
                    ...
                    echo "<th>" . formatField_he_format('debet_adj', $total_debet) . "</th>";
                    echo "<th>" . formatField_he_format('kredit_adj', $total_kredit) . "</th>";
                    ...
                    if (isset($detail2_items) && sizeof($detail2_items) > 0) {
                        echo "<div detail2_items class='table-responsive'>";
                        echo "<table class='table table-bordered table-condensed' style='background:#ffffff;'>";
                        echo "<tr bgcolor='#f5f5f5'>";
                        echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
...
//                        echo "<div detail3_items class='table-responsive'>";
//                        echo "<table class='table table-bordered table-condensed' style='background:#ffffff;'>";
//                        echo "<tr bgcolor='#f5f5f5'>";
//                        echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
...
//                    }
                    ...
                    if (isset($itemsNotApprove) && sizeof($itemsNotApprove) > 0) {
                        echo "<h4 itemsNotApprove class='text-blue'><span class='fa fa-book'>$detail_not_approve_title</span></h4>";
                        echo "<div class='table-responsive'>";
                        echo "<table class='table table-bordered table-condensed' style='background:#ffffff;'>";
                        echo "<tr bgcolor='#f5f5f5'>";
                        echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
