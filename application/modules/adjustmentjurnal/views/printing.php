...
//                $contenStr5 .= "</div>";
        ...
        if($customLeftElements){
            $elementLabels['leftElements'] = "";
        }

        if($customRightElements){
            $elementLabels['rightElements'] = "";
        }

        $rightElementTable ="";
        if(!empty($mainLabels)){
            $rightElementTable .="<table class='table dataTable compact table-borderless'>";
            foreach($mainLabels as $key => $label){
                if(isset($mainValues[$key])){
                    $rightElementTable .="<tr class='p-2'>";
                    $rightElementTable .="<td class='no-padding text-capitalize'>$label</td>";
                    $rightElementTable .="<td class='no-padding'>: </td>";
                    $rightElementTable .="<td class='no-padding text-capitalize'>".formatField_he_format($key, $mainValues[$key])."</td>";
                    $rightElementTable .="</tr>";
                }
            }
            $rightElementTable .="</table>";
            $elementLabels['leftElements'] = $rightElementTable;
        }
