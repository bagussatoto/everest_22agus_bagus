...
//        $_SESSION[$cCode]['tableIn_master_values']['partial'] = 1;
    ...
    public function updateItemField()
    {
        ...
        // cekMerah();
        // cekBiru($_GET);
        if (isset($_GET['jml_print'])) {
            $_SESSION['barcode_print']['items'][$id]['jml_print'] = $_GET['jml_print'];
            die();
        }
...
//        }
        ...
        $this->load->helper("he_value_builder");
//        resetValues($this->jenisTr);
        resetValues_he_value_builder($this->jenisTr, $this->configCoreJenis);
        fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis, $this->configValuesJenis, my_ppn_factor());
...
//        echo "</html>";
    ...
    public function updateMainFieldByStep()
    {
        ...
        $this->load->helper("he_value_builder");
        resetValues($this->jenisTr);
        fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis);
...
//        echo "</script>";
        ...
        $this->load->helper("he_value_builder");
        fillValues_he_value_builder($jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis);
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    public function updatePrice()
    {
        ...
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][$intoStep]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][$intoStep] : null;
        $items_child = isset($this->configUi[$this->jenisTr]['shopingCartDetailFields'][$intoStep]['fields']) ? $this->configUi[$this->jenisTr]['shopingCartDetailFields'][$intoStep]['fields'] : array();
        $fields = isset($this->configUi[$this->jenisTr]['itemPrice'][$intoStep]['fields']) ? $this->configUi[$this->jenisTr]['itemPrice'][$intoStep]['fields'] : array();
        $ppnHarga = isset($this->configUi[$this->jenisTr]['itemPrice'][$intoStep]['ppn']) ? $this->configUi[$this->jenisTr]['itemPrice'][$intoStep]['ppn'] : 10;
        ...
        $cid = $_GET['cid'];
        $val = $_GET['val'];
        $minVal = $_GET['h'] + ($_GET['h'] * 0.025) * 1;
        if ($val < $minVal) {
            $label = htmlspecialchars(blobDecode($_GET['name']));

            $_SESSION[$cCode]['itemPrice'][$cid][$pid][$key] = $val;
            matiHEre("harga jual $label di " . $cab[$cid] . " lebih rendah dari HPP! harga jual harus lebih besar  2,5% dari " . formatfield("nilai", $_GET['h']) . " ( sebesar " . formatField("nilai", $minVal) . " ) harga yang dimasukan saat ini sebesar (" . formatField("nilai", $val) . ") silahkan diperbaiki");
        }
//        matiHere($minVal);
//        if (sizeof($_SESSION[$cCode]['itemPrice'][$cid][$pid][$key])){
//            unset($_SESSION[$cCode]['itemPrice'][$cid][$pid][$key]);
//
//        }
//        $_SESSION[$cCode]['itemPrice'][$cid][$pid][$key]=$val;
//        arrPrint($fields);
        foreach ($fields as $i => $col) {
            if ($col == $key) {
                ...
                if ($col == "hpp") {
                    ...
                    $value = $val + $val * ($ppnHarga / 100);
                }
            }

            $_SESSION[$cCode]['itemPrice'][$cid][$pid][$col] = $value;
        }
...
//        matiHere();
        ...
        resetValues($this->jenisTr);
//        fillValues($this->jenisTr, $fromStep, $intoStep);
//        fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCore[$this->jenisTr], $this->configUi[$this->jenisTr]);
        ...
        if (!isset($_GET['spc'])) {
            ...
            echo "<script>";
            echo "top.$('.kalkulasi_modal').removeClass('hidden');";
            echo "</script>";
        }
//        echo "<html>";
//        echo "<head>";
...
//        echo "</html>";
    ...
    public function updatePriceMb()
    {
        ...
        if ($val < $minVal) {
            ...
            echo "
            <script>
                var source = top.$('input[name=" . $_GET['pid'] . "_" . $_GET['cid'] . "]');
                top.$(source).css('background-color', '#e428288a');
                top.$(source).val(0);
                top.$(source).on('keyup', function(){
                    top.$(source).css('background-color', '#fff');
                })
            </script>
        ";
...
//        fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCore[$this->jenisTr], $this->configUi[$this->jenisTr]);
        ...
        $actionTarget = "top.BootstrapDialog.closeAll();top.BootstrapDialog.show(
                                   {
                                       title:'Followup preview',
                                       message: " . 'top.$' . "('<div></div>').load('" . MODUL_PATH . "FollowUp/followupPreviewMb/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(6) . "?rawBuilderURL=$rawBuilderURL'),
                                        size:top.BootstrapDialog.SIZE_WIDE,
                                        draggable:false,
                                        animate:false,
                                        closable:false,
...
//        arrPrint($_SESSION[$cCode]['itemPrice']);
//        arrPrintWebs($_GET);
//        arrPrintWebs($_POST);

        $valOnSession = $_SESSION[$cCode]['itemPrice'][$_GET['cid']][$_GET['pid']][$_GET['key']];

        echo "
            <script>
                var source = top.$('input[name=" . $_GET['pid'] . "_" . $_GET['cid'] . "]');
                var value = top.$(source).val();

                if(value==top.addCommas($valOnSession)){
                    top.$(source).addClass('text-bold');
                    top.$(source).css('background-color', '#8bc34a87');
                    setTimeout( function(){
                        top.$(source).css('background-color', '#fff');
                    }, 1500);
                }
...
//        echo "</html>";
    ...
    public function removePrintBarcode()
    {
        $cCode = cCodeBuilderMisc(url_segment(4));
        if (isset($_GET['main'])) {
            unset($_SESSION[$cCode]['main']['print_barcode']);
            // die();
        }
        ...
        // echo "<script>$actionTarget</script>";
        // echo "<script>top.$('<div></div>').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7) . "?rawBuilderURL=$rawBuilderURL')</script>";
    }
}