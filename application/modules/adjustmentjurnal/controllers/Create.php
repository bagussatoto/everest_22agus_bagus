...
class Create extends Modul_Controller
{
    ...
    public function preview()
    {
        ...
        $arrItems4 = isset($_SESSION[$cCode]['items4']) ? $_SESSION[$cCode]['items4'] : array(); //rekening babon
        $arrItems6 = isset($_SESSION[$cCode]['items6_sum']) ? $_SESSION[$cCode]['items6_sum'] : array(); //anaknya cuy

        $arrRekPem=array();
        if (isset($_SESSION[$cCode]['items7_sum'])) { //dipakai untuk view rekening pembantu items
            $arrItemsTmp_items7 = $_SESSION[$cCode]['items7_sum'];
            foreach($arrItemsTmp_items7 as $tmpKeyR => $rd){
                $splRek = explode("_", $tmpKeyR);
                $varMainRek = $splRek[0];
                $varExtrnID = $splRek[1];

                $arrRekPem[$varMainRek][$varExtrnID] = $rd;


                $debet_prev         = isset($arrItems6[$varMainRek]['prev'][$varExtrnID]['debet_prev']) ? $arrItems6[$varMainRek]['prev'][$varExtrnID]['debet_prev'] : 0;
                $kredit_prev        = isset($arrItems6[$varMainRek]['prev'][$varExtrnID]['kredit_prev']) ? $arrItems6[$varMainRek]['prev'][$varExtrnID]['kredit_prev'] : 0;
                $debet_curent_ori   = isset($arrItems6[$varMainRek]['curent'][$varExtrnID]['debet_curent_ori']) ? $arrItems6[$varMainRek]['curent'][$varExtrnID]['debet_curent_ori'] : 0;
                $kredit_curent_ori  = isset($arrItems6[$varMainRek]['curent'][$varExtrnID]['kredit_curent_ori']) ? $arrItems6[$varMainRek]['curent'][$varExtrnID]['kredit_curent_ori'] : 0;
                $debet_adj          = isset($arrItems6[$varMainRek]['adj'][$varExtrnID]['debet_adj']) ? $arrItems6[$varMainRek]['adj'][$varExtrnID]['debet_adj'] : 0;
                $kredit_adj         = isset($arrItems6[$varMainRek]['adj'][$varExtrnID]['kredit_adj']) ? $arrItems6[$varMainRek]['adj'][$varExtrnID]['kredit_adj'] : 0;
                ...
                $arrRekPem[$varMainRek][$varExtrnID]['debet_prev']          = $debet_prev;
                $arrRekPem[$varMainRek][$varExtrnID]['kredit_prev']         = $kredit_prev;
                $arrRekPem[$varMainRek][$varExtrnID]['debet_curent_ori']    = $debet_curent_ori;
                $arrRekPem[$varMainRek][$varExtrnID]['kredit_curent_ori']   = $kredit_curent_ori;
                $arrRekPem[$varMainRek][$varExtrnID]['debet_adj']           = $debet_adj;
                $arrRekPem[$varMainRek][$varExtrnID]['kredit_adj']          = $kredit_adj;

                $arrRekPem[$varMainRek][$varExtrnID]['debet_after']         = $debet_after;
                $arrRekPem[$varMainRek][$varExtrnID]['kredit_after']        = $kredit_after;
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                foreach ($_SESSION[$cCode]['items'] as $iSpec) {
                    $tmp = array(
                        ...
                        "coa_code"      => isset($iSpec['coa_code']) ? $iSpec['coa_code'] : "0",
                        "head_code"     => isset($iSpec['head_code']) ? $iSpec['head_code'] : "0",
                    );

                    if(isset($arrRekPem[$tmp['head_code']])){
                        $tmp['childs'] = $arrRekPem[$tmp['head_code']];
                    }
        ...
        //endregion lookup items from shopping cart sessions
...
//					    cekKuning("$eName TIDAK terdaftar pada relInputs");
    ...
    public function save()
    {
        ...
        $gmr = img_working();
        echo "\n" . lgWait("doFollowup_auto: Harap menunggu ...., <br>Transaksi sedang diproses <br>halaman akan refresh setelah proses selesai<br>Sangat tergantung seberapa banyak data yang diproses <img src='$gmr'><div class='clearfix'>&nbsp;</div> <div id='h_wadah_human_time_start'></div> <div id='header_wadah_progress'></div> <div id='wadah_progress'></div> <div id='wadah_human_time_start'></div> <div id='wadah_microtime_start'></div>");
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            //region injector jurnal untuk build auto rekening detail
            $preIterator = isset($this->configCore[$this->jenisTr]['preInjectedjurnal'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preInjectedjurnal'][$jenisTrTarget]['detail'] : array();
            $srcJurnalItems = array();
            if (sizeof($preIterator) > 0) {
                // arrprint($preIterator);
                $cCtr = 0;
                foreach ($preIterator as $tComSpec) {
                    $cCtr++;
                    // arrPrint($tComSpec);

                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $rsltGateName = $tComSpec['resultGate'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
                    $resultParams = isset($tComSpec['resultParams']) ? $tComSpec['resultParams'] : array();
                    $switchResultParams = isset($tComSpec['switchResultParams']) ? $tComSpec['switchResultParams'] : false;
                    ...
                    $preParams = array();
                    foreach ($_SESSION[$cCode][$srcGateName] as $preParams) {
                        // arrPrint($preParams);
                        if (isset($tComSpec['static'])) {
                            $preParams["static"] = $tComSpec['static'];
                            foreach ($tComSpec['static'] as $key => $value) {
                                $realValue = makeValue($value, $preParams, $preParams, 0);
                                $subParams['static'][$key] = $realValue;
                                // $preParams['static'][$key] = $value;
                            }
                        ...
                        if ($tobeExecuted) {
                            $m->pair(0, $tmpOutParams[$cCtr], $preParams, $rsltGateName, $tComSpec['static']) or die("Tidak berhasil memasang  values pada pre-processor: $comName/" . $this->jenisTr . "/" . __FUNCTION__ . "/" . __LINE__);
                            $srcJurnalItems = $m->exec();
                ...
                cekMerah("gak ada auto builder");
            }
            //endregion

            //region injector jurnal untuk build auto rekening master
            $preIterator = isset($this->configCore[$this->jenisTr]['preInjectedjurnal'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preInjectedjurnal'][$jenisTrTarget]['master'] : array();
            $srcjurnal = array();
            if (sizeof($preIterator) > 0) {
                // arrprint($preIterator);
                $cCtr = 0;
                foreach ($preIterator as $tComSpec) {
                    ...
                    $preParams = array();
                    if (isset($tComSpec['recapStatic'])) {
                        $preParams["static"] = $tComSpec['static'];
                        foreach ($tComSpec['recapStatic'] as $key => $value) {
                            $realValue = makeValue($value, $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0);
                            $subParams['static'][$key] = $realValue;
                    ...
                    if ($tobeExecuted) {
                        $m->pair(0, $tmpOutParams[$cCtr], $_SESSION[$cCode][$srcGateName], $rsltGateName, $tComSpec['static']) or die("Tidak berhasil memasang  values pada pre-processor: $comName/" . $this->jenisTr . "/" . __FUNCTION__ . "/" . __LINE__);
                        $srcjurnal = $m->exec();
            ...
            //region postproc auto builder
            // $preIterator = isset($this->configCore[$this->jenisTr]['InsertNeracaAdjTmp'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['InsertNeracaAdjTmp'][$jenisTrTarget]['master'] : array();
            // $srcjurnal = array();
            // if (sizeof($preIterator) > 0) {
            //     // arrprint($preIterator);
            //     $cCtr = 0;
            ...
            arrPrint($_SESSION[$cCode]['rsltItems']);
            ...
            //region sub-component custom dari builder preproc
            if (sizeof($srcJurnalItems) > 0) {
                // arrPrint($srcJurnalItems);
                $sourceDetailKey = $srcJurnalItems["components"][$this->jenisTr]["detail"]["src"];
                if (isset($_SESSION[$cCode][$sourceDetailKey]) && sizeof($_SESSION[$cCode][$sourceDetailKey]) > 0) {
                    foreach ($_SESSION[$cCode][$sourceDetailKey] as $paramsToexec) {
                        $cCtr = 0;
                        foreach ($paramsToexec as $tempExec) {
                            $cCtr++;
                            $tmpOutParams[$cCtr] = array();
                            // arrPrint($tempExec);
                            // matiHEre();
                            if (!isset($tempExec['static']["transaksi_id"])) {
                                $tempExec['static']["transaksi_id"] = $insertID;
                            }
                            if (!isset($tempExec['static']["transaksi_no"])) {
                                $tempExec['static']["transaksi_no"] = $insertNum;
                            }
                            $comName = "Com" . ucfirst($tempExec["comName"]);
                            $this->load->model("Coms/" . $comName);
                            $m = new $comName();
                            if (isset($tempExec['loop']) && sizeof($tempExec['loop']) > 0) {
                                $tmpOutParams[$cCtr][] = array("loop"=>$tempExec['loop'],"static"=>$tempExec['static']);
                            }
                            else {
                                $tmpOutParams[$cCtr][] = array("static"=>$tempExec['static']);
                            }
                            if (sizeof($tmpOutParams[$cCtr]) > 0) {
                                $tobeExecuted = true;
                            }
                        ...
                        // arrPrint($paramsToexec);
                    }
                }
                // arrPrint($_SESSION[$cCode][$sourceDetailKey]);
                // matiHEre(__LINE__ . " exec auto pembantu");
            }
...
//             matiHEre();
            ...
            if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                ...
                if (sizeof($srcjurnal) > 0) {
                    $iterator = isset($srcjurnal['components'][$jenisTrTarget]['master']) ? $srcjurnal['components'][$jenisTrTarget]['master'] : array();
                }
                else {
                    $iterator = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master'] : array();
                        ...
                        // arrPrintWebs($iterator);
                        // arrPrintKuning($_SESSION[$cCode]["rsltItems"]);
            // matiHEre(__LINE__);
            if (sizeof($iterator) > 0) {
                $tmpOutParams = array();
...
// matiHEre($srcGateName);
                    ...
                    // arrprint($tComSpec['static']);
                    if (isset($tComSpec['static'])) {
                        foreach ($tComSpec['static'] as $key => $value) {
                            // $realValue = makeValue($value, $_SESSION[$cCode][$srcGateName], $_SESSION[$cCode][$srcGateName], 0);
                            $realValue = makeValue($value, $_SESSION[$cCode][$srcGateName], $_SESSION[$cCode][$srcGateName], 0);
                            $tmpOutParams['static'][$key] = $realValue;
                            // $tmpOutParams['static'][$value] = $realValue;
                    ...
                    if ($tobeExecuted) {
                        ...
                        // cekHitam("setelah builder 2 jalan");
            ...
            /*
             * ini bagian nulis ke neraca_tmp_adj
             */
            if(isset($this->configCore[$this->jenisTr]['AutoPostProc'][$jenisTrTarget]["master"])){
                $autoPostProcConfigs = isset($this->configCore[$this->jenisTr]['AutoPostProc'][$jenisTrTarget]["master"]) ? $this->configCore[$this->jenisTr]['AutoPostProc'][$jenisTrTarget]["master"] : array();

                if(sizeof($autoPostProcConfigs)>0){
                    $tmpOutParams = array();
                    foreach($autoPostProcConfigs as $autoPostProcConfigSpec){
                        $mdlName = "Com".$autoPostProcConfigSpec["comName"];
                        $srcGateName = $autoPostProcConfigSpec["srcGateName"];
                        // $tmpOutParams["src"]= $_SESSION[$cCode][$srcGateName];
                        $tmpOutParams = array();
                        foreach($_SESSION[$cCode][$srcGateName] as $rekID =>$tempVAlUE){
                            $rekCategory = detectRekCategory($rekID);
                            $rekLabel = $_SESSION[$cCode]["items2"][$rekID];
                            foreach($tempVAlUE as $src_key =>$src_val){
                                if($src_key == "debet"){
                                    $newKey = "kredit";
                                }
                                else{
                            ...
                            if(isset($autoPostProcConfigSpec["static"])){
                                foreach($autoPostProcConfigSpec["static"] as $key =>$value){
                                    $realValue = makeValue($value, $_SESSION[$cCode]["main"], $_SESSION[$cCode]["main"], 0);
                                    $tmpOutParams[$rekID][$key] = $realValue;
                                }
                            }
                            if (!isset($temp2[$rekID]["transaksi_id"])) {
                                $tmpOutParams[$rekID]["transaksi_id"] = $insertID;
                            }
                            if (!isset($tmpOutParams[$rekID]["transaksi_no"])) {
                                $tmpOutParams[$rekID]["transaksi_no"] = $insertNum;
                            }

                            // $tmpOutParams['static']["date_target"] = date("Y-m-d");
                            $tmpOutParams[$rekID]["fulldate"] = date("Y-m-d");
                            $tmpOutParams[$rekID]["dtime"] = date("Y-m-d H:i:s");
                            $tmpOutParams[$rekID]["keterangan"] = $this->configUi[$this->jenisTr]['steps'][1]['label'] . " nomor " . $tmpNomorNota . " oleh " . $this->session->login['nama'];

                        }
// arrPrint($tmpOutParams);
                        // matiHEre();
                        if(sizeof($tmpOutParams)>0){
                            $this->load->model("Coms/".$mdlName);
                            $m = new $mdlName();
                            $m->pair($tmpOutParams) or matiHere("gagal menyimpan transaksi, pastikan koneksi anda tidak mengalami gangguan");
                            $m->exec();
                            // ceklIme($this->db->last_query());
                            // matiHEre();
                        }
                        ...
                        //     }
                        // }
                        // if (!isset($temp2["transaksi_id"])) {
                        //     $temp2["transaksi_id"] = $insertID;
                        // }
                        // if (!isset($temp2["transaksi_no"])) {
                        //     $temp2["transaksi_no"] = $insertNum;
                        // }
                        //
                        // // $tmpOutParams['static']["date_target"] = date("Y-m-d");
                        // $temp2["fulldate"] = date("Y-m-d");
                        // $temp2["dtime"] = date("Y-m-d H:i:s");
                        // $temp2["keterangan"] = $this->configUi[$this->jenisTr]['steps'][1]['label'] . " nomor " . $tmpNomorNota . " oleh " . $this->session->login['nama'];
                    
                    }
                    // arrPrint($tmpOutParams);
                }
                // arrPrint($autoPostProcConfigs);
            }
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
            ...
            if ($pakai_ini == 1) {
                ...
                if ($mongRegID > 0) {
                    ...
                    $tr->setFilters(array());
                    $tr->setSortBy(array());
                    $tr->addFilter("transaksi_id=$mongRegID");
                    $tmpTrm = $tr->lookUpAll()->result();
                    $mong->setTableName("transaksi_data_registry");
                    if (sizeof($tmpTrm) > 0) {
                        foreach ($tmpTrm as $tmpMain) {
                            $transksi_main = json_decode(json_encode($tmpMain), true);
                            arrPrint($transksi_main);
    ...
    public function recordColumn()
    {
        ...
        $vg->buildValue($this->jenisTr, '', $initMasterValues, $this->modul);
