...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        // $modelFilterPairedItems = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItem']['mdlFilter']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItem']['mdlFilter'] : array();
        // arrPrint($modelFilterPairedItems);
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "date_target"=>isset($iSpec['date_target']) ? $iSpec['date_target'] : "",
                    );
...
//                            $main[$key] = $valItem;
        ...
        //item3 sum
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items3'])) {
                $no = 0;
                foreach ($_SESSION[$cCode]['items3'] as $PID =>$iSpec_0) {

                    foreach($iSpec_0 as $childID =>$iSpec){
                        $no++;
                        $tmp = array(
                            "id" => $iSpec['id'],
                            "nama" => isset($iSpec['nama']) ? $iSpec['nama'] : "",
...
//         matiHEre();
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount']) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$stepNumber] == true) {
            ...
            if(isset($itemLabels[$shoppingCartAmountValue])){
        ...
        if (sizeof($pairedItem) > 0) {
            if (isset($pairedItem['enabled']) && ($pairedItem['enabled'] == true)) {
                ...
                foreach($items as $itemsMaster){
                            ...
                            foreach ($tmp as $row) {
                                if (sizeof($srcLabel) > 0) {
                                    foreach ($srcLabel as $label) {
                                        ...
                                        // $selItemsKeterangan[$itemsMaster["id"]][$row->$srcKey] = $keterangan;
                                        // $selItemsBarcode[$itemsMaster["id"]][$row->$srcKey] = $barcode;
                                        $selItems[$itemsMaster["id"]][$row->$srcKey] = $row->$label;
                                    }
                                    // $selItems[$itemsMaster["id"]][$row->$srcKey] = $row->$label;
                                    $selItemsKode[$itemsMaster["id"]]["extern_id"] = $row->extern_id;
                                    $selItemsFolder[$itemsMaster["id"]]["extern_nama"] = $row->head_name;
                                    $selItemsFolder[$itemsMaster["id"]]["p_head_code"] = $row->p_head_code;
                                }
                            }
                        }
                    ...
                    // $pro = makeFilter($pairedItem['mdlFilter'], $_SESSION[$cCode]['items'], $pro);
                    // matiHere(__LINE__);
                }
                // arrPrint($selItems);
                // matiHEre(__LINE__);
...
//                "dtime" => "date",
    ...
    public function viewCart()
    {
        ...
        // matiHere($this->jenisTr);
        //----------------------------------------------
        $editableJurnalAdjustment = isset($this->configUi[$this->jenisTr]['shopingCartJurnalEditable']) ? $this->configUi[$this->jenisTr]['shopingCartJurnalEditable'] : array();
        $isSettlement = isset($this->configUi[$this->jenisTr]['isSettlement']) ? $this->configUi[$this->jenisTr]['isSettlement'] : false;
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount']) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$stepNumber] == true) {
            ...
            unset($itemLabels2['subtotal']);
            unset($itemLabels3['subtotal']);
            if(!empty($shoppingCartAmountValue) && isset($itemLabels[$shoppingCartAmountValue])){
                unset($itemLabels[$shoppingCartAmountValue]);
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        //master header 1 , kalau key diganti builder session dari proses pihak wajib disamakan karena key saling berhubungan
        $headerMaster = array(
            "rekening_code"=>"kode rekening",
            "rekening_label"=>"rekening",
            "prev"=>"GL $dateTarget",//tahun dibuat relative nantinya
            "adj"=>"adjustment",
//            "curent"=>"revisi",
            "curent"=>"GL " . date("Y"),
        );
        ...
        $data = array(
            ...
            "shopingCartAddTaxAction" => MODUL_PATH . "_processPihak/selectTaxes/" . $this->jenisTr,
            "checkTaxes" => isset($_SESSION[$cCode]['main']['selectedType_konsumen']) ? $_SESSION[$cCode]['main']['selectedType_konsumen'] : "",
            "itemsjurnal"=>isset($_SESSION[$cCode]["items2"]) ? $_SESSION[$cCode]["items2"] : array(),//rekening
            "itemsJurnal2"=> isset($_SESSION[$cCode]['items4']) ? $_SESSION[$cCode]['items4'] : array(),//rekening level 2
            "headerMaster" =>$headerMaster,
            "header2" =>$header2,
            "jurnalEditableFields"=>$editableJurnalAdjustment,
            "pembantuItems"=>isset($_SESSION[$cCode]["items5_sum"]) ? $_SESSION[$cCode]["items5_sum"]: array(),
            "pembantuItemData"=>isset($_SESSION[$cCode]["items6_sum"]) ? $_SESSION[$cCode]["items6_sum"]: array(),
            "rekeningBlacklist"=>isset($_SESSION[$cCode]["rsltItems3"]) ? $_SESSION[$cCode]["rsltItems3"]: array(),
        );
...
//                            $_SESSION[$cCode]['rsltItems'][$iID]["sub_" . $key] = ($_SESSION[$cCode]['rsltItems'][$iID][$key] * $_SESSION[$cCode]['rsltItems'][$iID]["jml"]);
    ...
    public function recordPairedItemOther()
    {
        ...
        $selectorModel = isset($this->configUi[$this->jenisTr]['selectorModelTarget']) ? $this->configUi[$this->jenisTr]['selectorModelTarget'] : array();
        $selectorSrcModel = isset($this->configUi[$this->jenisTr]['selectorSrcModelTarget']) ? $this->configUi[$this->jenisTr]['selectorSrcModelTarget'] : array();
...
// matiHEre($val);
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldPairedSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldPairedSrc'] : array("nama" => "nama");
...
//                            mati_disini("---");
                    ...
                    $_SESSION[$cCode]["items2"][$iID][$row->head_code] = $tmp;
                    // $_SESSION[$cCode]["items3_sum"][$row->head_code] = $tmp;
                    //endregion
        ...
        $initMasterValues = array(
            ...
            "jenisTr" => $this->jenisTr,
            "jenisTrMaster" => $this->jenisTr,
            "jenisTrTop" => $this->configUi['steps'][1]['target'],
            "jenisTrName" =>$this->configUi[$stepNum]['steps'][1]['label'],
            "stepNumber" => $stepNum,
            "stepCode" => $this->configUi[$stepNum]['steps'][$stepNum]['target'],
            "dtime" => dtimeNow(),
            "fulldate" => dtimeNow("Y-m-d"),
            // "jenis_pajak"=>$this->session->login['jenis_usaha'],
        );
...
//        arrPrint($_GET);
    ...
    //tambahan sub item biaya model table akan masuk ke item3
    public function pairedSubItems(){
        $id = "";//coa biaya
    }
