...
switch($mode){
    case "viewReceipt":
        ...
        $resolvedTemplate = (isset($templateFullPath) && $templateFullPath != "") ? $templateFullPath : MODUL_TEMPLATE_PATH . $template;
        $p = New Layout("$title", "", $resolvedTemplate);
...
//            $contentStr3 = "";
            $contentStr3 = "<div class='panel-header'><h4>SPESIFIKASI &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class='header' id='stamp'></span></h4> </div>";
...
//            echo $str;
            ...
            $arrTags["noteDetails"] = isset($arrTags["noteDetails"]) ? $arrTags["noteDetails"] : "";
            /* @role software_engineer_agent - Preservasi tag customerSignitures & customerSignatures agar tidak ter-overwrite string kosong */
            if (!isset($arrTags["customerSignitures"]) || $arrTags["customerSignitures"] === "") {
                $arrTags["customerSignitures"] = isset($signature['customerSignitures']['contents']) ? $signature['customerSignitures']['contents'] : (isset($mainValues['customers_nama']) ? $mainValues['customers_nama'] : "");
            }
            if (!isset($arrTags["customerSignatures"]) || $arrTags["customerSignatures"] === "") {
                $arrTags["customerSignatures"] = $arrTags["customerSignitures"];
            }
            $arrTags["sign_1"] = isset($arrTags["sign_1"]) ? $arrTags["sign_1"] : "";
            $arrTags["sign_2"] = isset($arrTags["sign_2"]) ? $arrTags["sign_2"] : "";
            $arrTags["companyProfile"] = isset($arrTags["companyProfile"]) ? $arrTags["companyProfile"] : "";
...
//            arrPrintHijau($arrTags);
    ...
    case "print_tasklist":
        ...
        $list_produk .= "<caption class='text-left' id='dengan_hormat'>Dengan Hormat,<br>Bersama ini kami terbitkan Surat Perintah Kerja (SPK) dengan rincian sebagai berikut.</caption>";
        $list_produk .= "<caption class='text-left hidden' id='harap_diterima'>&nbsp<br>Harap diterima dengan baik barang-barang dibawah ini :</caption>";
        $list_produk .= "<thead>";
        $list_produk .= "<tr>";
...
//        </div>";
        ...
        $strData .= "<div style='margin-top: 15px;' id='spk_instruksi' class='col-print-12'>
        ...
        $strData .= "
<div class='col-print-6 hidden' id='spb_keterangan'  style='margin-top:15px;'>
    <div style='
    ...
    '>
        ...
        //PENUTUP
        $strData .= "<div style='margin-top: 15px;' id='spk_penutup' class='col-print-12'>
                        <div class='container-fluid'>
        ...
        <!-- <div class='text-center'>PIHAK II</div> -->
            <div class='text-center' id='sign_pelaksana'>PELAKSANA PROJECT</div>
            <div class='text-center hidden' id='sign_spb'>PENERIMA</div>
        ...
        <!-- <div class='text-center'>PIHAK II</div> -->
            ...
            <div class='text-center hidden' id='sign_4_spb'>PETUGAS</div>
            <div style='height: 75px;'>&nbsp;</div>
        ...
        <!-- <div class='text-center'>PIHAK III</div> -->
            ...
            <div class='text-center' id='sign_3'>MENGETAHUI</div>
            <div class='text-center hidden' id='sign_3_spb'>HORMAT KAMI</div>
            <div style='height: 75px;'>&nbsp;</div>
        ...
        $option .= " <label class='text-white'><input type='checkbox' name='checkboxTitle' id='radio_sjp' value='sjp' checkedd> Surat Jalan Pengiriman</label>";
        $option .= " <label class='text-white'><input type='checkbox' name='checkboxTitle' id='radio_spb' value='spb' checkedd> Surat Penerimaan Barang</label>";
        $option .= " <button type='button' id='print' onclick=\"window.print();\" title='print/cetak'> <i class='glyphicon glyphicon-print'></i></button>";
        $option .= "</div>";
        $option .= "<script>
                            // checkbox sjp/spb dibuat single-select (seperti radio)
                            $(document).on('change', 'input[name=\"checkboxTitle\"]', function () {
                                if (this.checked) {
                                    // kalau pilih salah satu, matikan yang lain
                                    $('input[name=\"checkboxTitle\"]').not(this).prop('checked', false);
                                }
                                // kalau keduanya off -> default
                                sendTitle();
                            });
            
                            function getTitleValue() {
                                var checked = $('input[name=\"checkboxTitle\"]:checked');
                                return checked.length ? checked.val() : '';
                            }
            
            function sendTitle() {
                                var val = getTitleValue();
                        
                                /* =====================
                                   RESET TITLE
                                ===================== */
                                $('#title, #title_alt, #title_spb').addClass('hidden');
            
                                /* =====================
                                   RESET SIGNATURE
                                ===================== */
                                $('#sign_pelaksana, #sign_spb, #sign_admin').addClass('hidden');
                                $('#sign_4, #sign_4_alt, #sign_4_spb').addClass('hidden');
                                $('#sign_3, #sign_3_spb').addClass('hidden');
                        
                                /* =====================
                                   KONDISI
                                ===================== */
                                if (val === 'sjp') { // surat jalan pengiriman
                        
                                    // TITLE
                    $('#title_alt').removeClass('hidden');                    
                        
                                    // SIGN
                                    $('#sign_pelaksana').removeClass('hidden');
                                    $('#sign_admin').removeClass('hidden');
                        
                    $('#sign_4_alt').removeClass('hidden');                    
                                    $('#sign_3').removeClass('hidden');
                                    
                                    //add Hidden spb
                                    $('#title_spb').addClass('hidden');
                                    $('#harap_diterima').addClass('hidden');
                                    $('#spb_keterangan').addClass('hidden');
                        
                                    $('#dengan_hormat').removeClass('hidden');
                                    $('#spk_penutup').removeClass('hidden');
                                    $('#spk_instruksi').removeClass('hidden');
                                    
                                } else if (val === 'spb') { //surat penerimaan barang
                        
                                    // TITLE
                                    $('#title_spb').removeClass('hidden');
                                    $('#harap_diterima').removeClass('hidden');
                                    $('#spb_keterangan').removeClass('hidden');
                                     
                                    $('#dengan_hormat').addClass('hidden');
                                    $('#spk_penutup').addClass('hidden');
                                    $('#spk_instruksi').addClass('hidden');
                                    

                                    // SIGN (sesuai request SPB)
                                    $('#sign_pelaksana').addClass('hidden');
                                    $('#sign_admin').addClass('hidden');
                  
                                    $('#sign_spb').removeClass('hidden');
                        
                    $('#sign_4').addClass('hidden');
                    $('#sign_4_alt').addClass('hidden');
                                    $('#sign_4_spb').removeClass('hidden');
                        
                                    $('#sign_3').addClass('hidden');
                                    $('#sign_3_spb').removeClass('hidden');
...
//                "nilai_pekerjaan" => $list_produk,
        ...
        $p->render();
        break;
    case "print_tasklist_tambahan":
        $p = New Layout("$title", "----------", MODUL_TEMPLATE_PATH . "template/print_tasklist_tambahan.html");
        ...
        $strData .= "<thead>";
        $strData .= "<tr>";
        $strData .= "<td style='border-left: 0px!important;border-right: 0px!important;'>No SPK TAMBAHAN</td><td style='border-left: 0px!important;'>: $nomer_int_spk</td>";
        $strData .= "</tr>";
        $strData .= "<tr>";
        $tanggal_pelaksana_f = formatTanggal($tanggal_pelaksana, 'd-F-Y');
        // $strData .= "<td style='border-left: 0px!important;border-right: 0px!important;'>Tanggal Pelaksana</td><td style='border-left: 0px!important;'>: $masa_pelaksanaan_detail_pekerjaan_f</td>";
...
//        $list_produk .= "<th class='text-center'>Keterangan</th>";
        ...
        foreach($produk as $k => $dProduk){
            ...
            if(isset($arrDistribution[$dProduk->produk_dasar_id])){
                $jmlDist = $arrDistribution[$dProduk->produk_dasar_id]['jumlah'];
                $checkGak = $jmlDist == $dProduk->jml ? "checked" : "";
                $checkIcon = $jmlDist == $dProduk->jml ? "fa-check-square text-green" : "fa-square-o";
                $list_produk .= "<td class='text-center'> <i class=\"fa $checkIcon\"></i> distribusi: $jmlDist</td>";
            }
            else{
                $list_produk .= "<td class='text-center'>    <input class='no-margin' type='checkbox' disabled></td>";
            }
        ...
        if(!empty($supplies)){
            ...
            foreach($supplies as $k => $dProduk){
                ...
                if(isset($arrDistribution[$dProduk['biaya_dasar_id']])){
                    $jmlDist = $arrDistribution[$dProduk['biaya_dasar_id']]['jumlah'];
                    $checkGak = $jmlDist == $dProduk['total'] ? "checked" : "";
                    $checkIcon = $jmlDist == $dProduk['total'] ? "fa-check-square text-green" : "fa-square-o";
                    $list_produk .= "<td class='text-center'> <i class=\"fa $checkIcon\"></i> distribusi: $jmlDist</td>";
                }
                else{
                    $list_produk .= "<td class='text-center'>    <input class='no-margin' type='checkbox' disabled></td>";
...
//arrPrintWebs($arrDistribution);
        if(!empty($suppliesTambahan)){
            $numS=$num;
            foreach($suppliesTambahan as $k => $dProduk){
                $numS++;
                $list_produk .= "<tr>";
                $list_produk .= "<td class='text-center'>$numS</td>";
                $list_produk .= "<td class='text-capitalize' style='padding-left: 6px !important;'>(supplies tambahan) &nbsp; ".$dProduk['biaya_dasar_nama']."</td>";
                $list_produk .= "<td class='text-left'>      ".$dProduk['barcode']."              </td>";
                $list_produk .= "<td class='text-center'>    ".$dProduk['total']."                  </td>";
                $list_produk .= "<td class='text-center'>    ".$dProduk['satuan']."               </td>";
