...
//        $_SESSION[$cCode]['tableIn_master_values']['partial'] = 1;
    ...
    public function updateItemField()
    {
        ...
        $val = isset($_GET['val']) ? (is_numeric($_GET['val']) ? $_GET['val'] : htmlspecialchars($_GET['val'])) : "";
    ...
    public function updateSourceField()
    {
        ...
        if ($refresh == 1) {
            ...
            $actionTarget = "top.BootstrapDialog.closeAll();top.BootstrapDialog.show(
                                   {
                                       title:'Followup preview',
                message: " . 'top.$' . "('<div></div>').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7) . "/$topIDSO?rawBuilderURL=$rawBuilderURL'),
                                        size:top.BootstrapDialog.SIZE_WIDE,
                                        draggable:false,
                                        animate:false,
                                        closable:false,
            ...
            echo "  top.$('#followupPreview').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7) . "/$topIDSO?rawBuilderURL=$rawBuilderURL');";
            echo "  }";
            echo "</script>";
        }
...
//        fillValues($this->jenisTr, $fromStep, $intoStep);
    ...
    public function updateChecklistField()
    {
        ...
        $id = isset($_GET['id']) ? $_GET['id'] :0;
        $val = $_GET['val'];
        $checklistnote = blobDecode($_GET['checklistnote']);

        if (isset($_SESSION[$cCode]['main'])) {
            if (isset($_GET['checklistnote'])) {
                if($_GET['state'] && $_GET['state'] == "true" ){
                $_SESSION[$cCode]['main']['checklistnote'] = $checklistnote;
                $_SESSION[$cCode]['main']['checklistnote_cek'] = 1;
            }
                else{
                    $_SESSION[$cCode]['main']['checklistnote'] = "";
                    $_SESSION[$cCode]['main']['checklistnote_cek'] = 0;
        }
            }
        }
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    //ini bagian untuk itesm sub 2
    public function updateSubItemField()
    {
        ...
        $intoStep = $this->uri->segment(6);
        $cCode = "_TR_" . $this->jenisTr;
        $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields2'][$intoStep]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields2'][$intoStep] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue2'][$intoStep]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue2'][$intoStep] : null;
        $items_child = isset($this->configUi[$this->jenisTr]['shopingCartDetailFields2'][$intoStep]['fields']) ? $this->configUi[$this->jenisTr]['shopingCartDetailFields2'][$intoStep]['fields'] : array();
        //----------------------------
        $closedRequest = isset($this->configCore[$this->jenisTr]['closedRequest'][$intoStep]) ? $this->configCore[$this->jenisTr]['closedRequest'][$intoStep] : array();

// arrPrint($this->uri->segment_array());
// arrPrintWebs($_GET);
// arrPrintWebs($itemNumLabels);
//         matiHEre();
        //----------------------------
        $arrPartialKey = array("jml", "qty");
        $id = $_GET['id'];
        $key = $_GET['key'];
        $val = is_numeric($_GET['val']) ? $_GET['val'] : htmlspecialchars($_GET['val']);
        if (isset($_SESSION[$cCode]['items2_sum'][$id][$key])) {
            //--------------------------------------------
            if (isset($closedRequest['enabled']) && ($closedRequest['enabled'] == true)) {
                if (in_array($key, $arrPartialKey)) {
                    foreach ($arrPartialKey as $val_orig) {
                        if (!isset($_SESSION[$cCode]["items2_sum"][$id][$val_orig . "_original"])) {
                            $_SESSION[$cCode]["items2_sum"][$id][$val_orig . "_original"] = $_SESSION[$cCode]["items2_sum"][$id][$val_orig];
                        }
                    }
                    $jml_no_approve = ($_SESSION[$cCode]["items2_sum"][$id]["jml_original"] - $val) > 0 ? $_SESSION[$cCode]["items2_sum"][$id]["jml_original"] - $val : 0;
                    $_SESSION[$cCode]["items2_sum"][$id]["jml_no_approve"] = $jml_no_approve;
                    $_SESSION[$cCode]["items2_sum"][$id]["qty_no_approve"] = $jml_no_approve;

                    //-------------------------------------------------
                    // membuat session items no approve
                    if (!isset($_SESSION[$cCode]['items_noapprove'][$id])) {
                        $_SESSION[$cCode]['items_noapprove'][$id] = $_SESSION[$cCode]["items2_sum"][$id];
                    }
                    foreach ($arrPartialKey as $val_orig) {
                        $_SESSION[$cCode]['items_noapprove'][$id][$val_orig] = $jml_no_approve;
                    }
            ...
            $_SESSION[$cCode]["items2_sum"][$id][$key] = $val;
            ...
            if (isset($_SESSION[$cCode]['items2_sum'][$id]['nett'])) {
                $_SESSION[$cCode]['items2_sum'][$id]['sub_nett'] = ($_SESSION[$cCode]['items2_sum'][$id]['nett'] * $_SESSION[$cCode]["items2_sum"][$id]['jml']);
            }

            if ($subAmountConfig != null) {
                $items = $_SESSION[$cCode]['items2_sum'];
                $subtotal = makeValue($subAmountConfig, $items[$id], $items[$id], 0);
            }
            else {
                $subtotal = 0;
            }
            $_SESSION[$cCode]['items2_sum'][$id]['subtotal'] = ($subtotal);

            if ($val < $_SESSION[$cCode]['items2_sum'][$id][$key]) {
                if (isset($_SESSION[$cCode]['main']['status_4'])) {
                    $_SESSION[$cCode]['main']['status_4'] = 5;
                }
                if (isset($_SESSION[$cCode]['main']['trash_4'])) {
        ...
        if (sizeof($_SESSION[$cCode]['items2_sum']) > 0) {
            foreach ($_SESSION[$cCode]["items2_sum"] as $id_ => $iSpec) {
                $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
            }
        }
    ...
    public function removeSubItem()
    {
        ...
        if (isset($_SESSION[$cCode]['items2_sum'][$id])) {
            if (isset($closedRequest['enabled']) && ($closedRequest['enabled'] == true)) {
                // menjadi items no approve
                $_SESSION[$cCode]['items2_noapprove'][$id] = $_SESSION[$cCode]['items2_sum'][$id];
            }
