...
//showLast_query("biru");
        ...
        //---------------------------------------------------
        // Backup note & nama to prevent data loss when changing customer
        $existingNote = "";
        $existingNama = "";
        if (isset($_SESSION[$cCode]['items'][0])) {
            $existingNote = isset($_SESSION[$cCode]['items'][0]['note']) ? $_SESSION[$cCode]['items'][0]['note'] : "";
            $existingNama = isset($_SESSION[$cCode]['items'][0]['nama']) ? $_SESSION[$cCode]['items'][0]['nama'] : "";
        }
        ...
        $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama", "note" => "note");
        $selectorProcessor = isset($this->configUi[$this->jenisTr]['selectorProcessor']) ? $this->configUi[$this->jenisTr]['selectorProcessor'] : NULL;

        $tmp = array(
            "handler" => $this->modul . "/$selectorProcessor",
            "id" => "",
            "nama" => "",
            "name" => "",
            "jml_wo" => "",
        ...
        if ($subAmountConfig != null) {
            $tmp['subtotal'] = makeValue($subAmountConfig, $tmp, $_SESSION[$cCode]['items'][0], 0);
        }
        else {
            $tmp['subtotal'] = 0;
        }

        // Restore note and nama from backup
        if ($existingNote != "") {
            $tmp['note'] = $existingNote;
        }
        if ($existingNama != "") {
            $tmp['nama'] = $existingNama;
        }

        $_SESSION[$cCode]['items'][0] = $tmp;
