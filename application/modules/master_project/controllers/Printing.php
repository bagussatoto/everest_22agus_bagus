...
class Printing extends Modul_Controller
{
    ...
    public function viewProformaReceipt()
    {
        ...
        $transaksi_id = (int)$this->uri->segment(4);
    ...
    public function viewReceiptReg__()
    {
        $userGroupCenter = $this->config->item("userGroup");
        $userGroupBranch = array_merge($this->config->item("userGroup_cabang"), $this->config->item("userGroup_gudang"));
...
//cekMErah($no);
        ...
        $metode_print =isset($_GET["pr"]) ? $_GET["pr"]:"simple";
        //region read items from existing model
        $tr = new MdlTransaksi();
        $tr->addFilter("nomer='$no'");
        $tmp1 = $tr->lookUpAll()->result();
        ...
        $srcKomposisiProject = showRincian($masterID,"nota",$metode_print);

//        arrPrint($srcKomposisiProject);
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            $receiptMultitemSubField_detils = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptMultitemSubField_detils'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptMultitemSubField_detils'][$currentStepNum] : array();
            //region tabel2 tarikan untuk kolom2 nilai (hpp, ppn, dll)
            $tmpVal_main = $tr->lookupMainValuesByTransID($trID);
            $tmpVal_detail = $tr->lookupDetailValuesByTransID($trID);
            //endregion
            ...
            //region lihat sub biaya -> transaksi_data_items3_sum
            $tr = new MdlTransaksi();
            $tr->setFilters(array());
                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "rsltItems":
                                    ...
                                    break;
                                case "rsltItems3_sub":
                                    $rsltItems3_sub = unserialize(base64_decode($row->$param));
                                    break;
                                case "items3":
                                    $items3 = unserialize(base64_decode($row->$param));
                                    break;
                            }
                        }
                    }
            ...
            //endregion
            // matiHEre();
            //region builder rsltitem3_sub jika belum ada
            if (sizeof($rsltItems3_sub) == 0) {
                $tr = new MdlTransaksi();
                $tr->setFilters(array());
                $tr->setJoinedFilter(array());
                $tr->addFilter("id='$trID'");
                $tmpRslt3 = $tr->lookupJoinedSubItems2();
                if (sizeof($tmpRslt3) > 0) {
                    foreach ($tmpRslt3 as $tmpRslt3_0) {
                        $rsltItems3_sub[$tmpRslt3_0->produk_id] = array(
                            "placeID" => $tmpRslt3_0->cabang_id,
                            "gudangID" => "0",
                            "produk_id" => $tmpRslt3_0->produk_id,
                            "produk_nama" => $tmpRslt3_0->produk_nama,
                            "produk_ord_harga" => $tmpRslt3_0->produk_ord_hrg,
                            "produk_ord_jml" => $tmpRslt3_0->produk_ord_jml,
                            "qty" => $tmpRslt3_0->produk_ord_jml,
                            "transaksi_id" => $tmpRslt3_0->transaksi_id,
                        );
                    }

                }
            }
            // arrPrint($rsltItems3_sub);
            // //endregion
            ...
            $itemLabels = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields'][$currentStepNum] : array();
            $itemLabels2 = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields2'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFields2'][$currentStepNum] : array();
            //            $itemSrcLabel = "";
                        ...
                        case "dataModel":
                            foreach ($eSpec as $key => $val) {
                                ...
                                if (is_array($vTmp)) {
                                    ...
                                    foreach ($vTmp as $vTmpKey => $vTmpVal) {
                                        $vTmp[$vTmpKey] = formatField_he_format($vTmpKey, $vTmpVal);
                                    }
                                }
                                $eTmp[$key] = $vTmp;
                                if (is_array($vTmp)) {
        ...
        if (isset($globalVars["ids_his"])) {
            ...
            $addTrIdCounter = isset($tmpTrx['countersTrID']['stepCode|placeID']) ? "-" . digit_5($tmpTrx['countersTrID']['stepCode|placeID']) : "";
            //            cekHere($addTrIdCounter);
            $globalVars['nomers_prev'] = isset($tmpTrx[0]->nomer) ? formatField_he_format("nomer", $tmpTrx[0]->nomer . $addTrIdCounter) : "-";
        }
        ...
        if (sizeof($elementFixedConfig) > 0) {
            foreach ($elementFixedConfig as $key => $label) {
                if (is_array($label)) {
                    ...
                    $countersUrut = array_values($counters['stepCode|placeID'])[0];
                    $countersUrut = "-" . digit_5($countersUrut);
                    $fixedElements['fixedElements']['contents'][$label['label']] = isset($dataHist[$label['step']][$label['target']]) ? formatField_he_format($label['target'], $dataHist[$label['step']][$label['target']]) . $countersUrut : "";
                    $fixedElements['fixedElements']['label'] = $globalVars['jenis_label'];
                }
                else {
                    //                    $fixedElements['fixedElements']['contents'][$label] = isset($globalVars[$key]) ? formatField($key, $globalVars[$key]) : "";
                    if (isset($globalVars[$key])) {
                        if (is_array($globalVars[$key])) {
                            if (sizeof($globalVars[$key])) {
                                ...
                                foreach ($globalVars[$key] as $ii => $ii_val) {
                                    if ($ii_result == "") {
                                        $ii_result = formatField_he_format($key, $ii_val);
                                    }
                                    else {
                                        $ii_result .= "<br>" . formatField_he_format($key, $ii_val);
                                    }
                                }
                                $fixedElements['fixedElements']['contents'][$label] = $ii_result;
                            }
                            ...
                            $fixedElements['fixedElements']['contents'][$label] = formatField_he_format($key, $globalVars[$key]);
                        }
                    }
                    else {
                        $fixedElements['fixedElements']['contents'][$label] = "";
        ...
        if (sizeof($elementFixedNumberSO) > 0) {
            foreach ($elementFixedNumberSO as $key => $label) {
                $fixedElements['so_number']['contents'][$label] = isset($globalVars[$key]) ? formatField_he_format($key, $globalVars[$key]) : "";
            }
        }
        ...
        if ($receiptAdvanceItems == true) {
            ...
            if (isset($items3)) {
                foreach ($items3 as $id => $iSpecs) {
                    foreach ($iSpecs as $subId => $subIspec) {

                        $key_result = isset($subIspec[$receiptAdvanceItemsKey]) ? $subIspec[$receiptAdvanceItemsKey] : 0;
                        ...
                        $advanceItemsNumLabel = $advanceItemsLabel + $advanceNumItemsLabel;
                        $advanceSubItemsNumLabel = $advanceSubItemsLabel + $advanceSubNumItemsLabel;
                        $receiptAdvanceItemsLabel[$id][$subId] = sizeof($advanceItemsNumLabel) > 0 ? $advanceItemsNumLabel + array("subtotal" => "total price") : array();
                        $receiptAdvanceSubItemsLabel[$id][$subId] = sizeof($advanceSubItemsNumLabel) > 0 ? $advanceSubItemsNumLabel + array("subtotal" => "subtotal") : array();
                        $receiptAdvanceNumType[$id][$subId] = $advanceSubNumItemsLabel + array("subtotal" => "subtotal");
                        $receiptAdvanceEditableField[$id][$subId] = $advanceSubEditableField;
                    }
                }
            }
        }
        ...
        //region bagian labeling closing mulai dai header sampai sub item dan biaya
        $receiptMultiItemSubFields = isset($this->configLayout[$this->jenisTr]["receiptMultiItemSubFields"]) ? $this->configLayout[$this->jenisTr]["receiptMultiItemSubFields"] : array();
        $receiptMultitemSubField_detils = isset($this->configLayout[$this->jenisTr]["receiptMultitemSubField_detils"][$stepNumber]) ? $this->configLayout[$this->jenisTr]["receiptMultitemSubField_detils"][$stepNumber] : array();
        $receiptMultitemSubField_detil_biaya = isset($this->configLayout[$this->jenisTr]["receiptMultitemSubField_detil_biaya"][$stepNumber]) ? $this->configLayout[$this->jenisTr]["receiptMultitemSubField_detil_biaya"][$stepNumber] : array();
        ...
        $data = array(
            ...
            "noteEnabled" => $noteEnabled,
            "items" => $items,
            "items2" => sizeof($items2_sumRegistries) ? $items2_sumRegistries : $items2,
            "itemsRegistries" => $itemsRegistries,
            ...
            "mainData2" => $addMainData2,
            //--------------
            "receiptMultiItemSubFields" => $receiptMultiItemSubFields,
            "receiptMultitemSubField_detils" => $receiptMultitemSubField_detils,
            "receiptMultitemSubField_detil_biaya" => $receiptMultitemSubField_detil_biaya,
            "itemClosingFg" => $items2Registries,
            "items3" => $items3,
            "itemLabels3" => array(),
            "rsltItems3_sub" => $rsltItems3_sub,
            "rsltItems3Labels" => isset($this->configLayout[$this->jenisTr]['purchasingProjek'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['purchasingProjek'][$currentStepNum] : array(),
            ...
            "subAdvanceItems" => isset($items3) ? $items3 : array(),
            "komposisi_project"=>$srcKomposisiProject,
            //------------------------------
        );
        //endregion
    ...
    public function viewReceiptReg()
    {
        ...
        $cCode = $this->cCode;
        $metode_print =isset($_GET["pr"]) ? $_GET["pr"]:"simple";
        $metode_print =isset($_GET["pr"]) ? $_GET["pr"]:"detail";
//cekMErah($no);
        $valas = array();
...
//        arrPrint($_GET);
        ...
        $srcKomposisiProject = showRincian($masterID,"nota",$metode_print);
//        arrPrint($srcKomposisiProject);
//matiHere("under debuging please wait");
//        arrPrint($srcKomposisiProject);
        //        arrPrint($replaceStepPic);
        //endregion
...
//        arrPrint($srcKomposisiProject);
//        matiHEre();
cekMerah("items2_sumRegistries: " . json_encode($items2_sumRegistries));
cekMerah("items2: " . json_encode($items2));
//cekMerah("mode: $mode");
cekMerah("mode: $mode");
cekMerah("LINE: " . __LINE__);

        // region resolusi path template cross-module
        $heTransaksi_uiConfig = $this->config->item("heTransaksi_ui");
        $modul_transaksi = isset($heTransaksi_uiConfig[$jenisTr]["modul"]) ? $heTransaksi_uiConfig[$jenisTr]["modul"] : $this->modul;
        $templateFile = $this->configLayout[$jenisTr]["receiptTemplate"][$currentStepNum];
        if ($modul_transaksi != $this->modul) {
            $templateFullPath = "application/modules/" . $modul_transaksi . "/" . $templateFile;
        }
        else {
            $templateFullPath = "";
        }
        // endregion resolusi path template cross-module
    ...
    public function viewReceiptRegBom()
    {
        $userGroupCenter = $this->config->item("userGroup");
        $userGroupBranch = array_merge($this->config->item("userGroup_cabang"), $this->config->item("userGroup_gudang"));
...
//        matiHEre();
    ...
    public function viewReceiptCashIn()
    {
        ...
        if (sizeof($elementFixedConfig) > 0) {
            ...
            foreach ($elementFixedConfig as $key => $label) {
                //                cekHitam($key);
                $fixedElements['fixedElements']['contents'][$label] = isset($globalVars[$key]) ? formatField_he_format($key, $globalVars[$key]) : "";
                $fixedElements['fixedElements']['label'] = $globalVars['jenis_label'];
    ...
    public function viewRencanaKerja()
    {
        $this->jenisTr = $jenis_master = $this->uri->segment(4);
        $project_id = $this->uri->segment(5);
        $transaksi_id = $this->uri->segment(6);
        ...
        $tmpTr = $tr->lookupJoined();
        if (sizeof($tmpTr) > 0) {
            $tmpReg = $tr->lookupDataRegistriesByMasterID($transaksi_id)->result();
            $mongoFields = $tr->getFields()["dataRegistry"];
            $masterGates = array();
            $childGates = array();
            $masterTableInParams = array();
...
//                    arrPrint($row);
            ...
            $mainProp = $tmpTr[0];
            $globalVars = array_merge($globalVars, (array)$mainProp, $main);
            $relElementConfig = isset($this->configUi[$jenis_master]['relativeElements']) ? $this->configUi[$jenis_master]['relativeElements'] : array();
            $relElementData = array();
            if (sizeof($relElementConfig) > 0) {
                foreach ($relElementConfig as $tempRel) {
                    foreach ($tempRel as $relKey => $relTemp) {
...
//arrPrintKuning($mainElements);
        ...
        $arrProjectKomposisi = array();
        $arrProjectKomposisiWorkOrder = array();
        $arrProjectWorkOrder = array();

        $pk = New MdlProjectKomposisi();
        $pk->setFilters(array());
        $pk->addFilter("status='1'");
        $pk->addFilter("trash='0'");
        $pk->addFilter("produk_id='$project_id'");
        $pkTmp = $pk->lookupAll()->result();
//        showLast_query("biru");
        if (sizeof($pkTmp) > 0) {
            foreach ($pkTmp as $ii => $pkSpec) {
                $produk_nama = $pkSpec->produk_nama;
                $jenis = $pkSpec->jenis;
                $subtotal = $pkSpec->jml * $pkSpec->harga;
                foreach ($itemLabels as $key => $label) {
                    $arrProjectKomposisi[$jenis][$ii][$key] = $pkSpec->$key;
                    if ($key == "subtotal") {
                        $arrProjectKomposisi[$jenis][$ii][$key] = $subtotal;
                    }
                }
            }
        }
//arrPrintHijau($arrProjectKomposisi);

        $pkw = New MdlProjectKomposisiWorkorder();
        $pkw->setFilters(array());
        $pkw->addFilter("status='1'");
        $pkw->addFilter("trash='0'");
        $pkw->addFilter("produk_id='$project_id'");
        $pkwTmp = $pkw->lookupAll()->result();
//        showLast_query("biru");
        if (sizeof($pkwTmp) > 0) {
            foreach ($pkwTmp as $ii => $pkwSpec) {
                $fase_id = $pkwSpec->fase_id;
                $jenis = $pkwSpec->jenis;
                $subtotal = $pkwSpec->jml * $pkwSpec->harga;
                foreach ($itemLabels as $key => $label) {
                    $arrProjectKomposisiWorkOrder[$fase_id][$jenis][$ii][$key] = $pkwSpec->$key;
                    if ($key == "subtotal") {
                        $arrProjectKomposisiWorkOrder[$fase_id][$jenis][$ii][$key] = $subtotal;
                    }
                }
            }
        }
//arrPrintKuning($arrProjectKomposisiWorkOrder);
        ...
        $pw->setFilters(array());
        $pw->addFilter("status='1'");
        $pw->addFilter("trash='0'");
        $pw->addFilter("produk_id='$project_id'");
        $pwTmp = $pw->lookupAll()->result();
//        showLast_query("biru");
        if (sizeof($pwTmp) > 0) {
            foreach ($pwTmp as $ii => $pwSpec) {
                $fase_id = $pwSpec->id;
                $nama = $pwSpec->nama;
                $arrProjectWorkOrder[$fase_id] = $nama;
            }
        }


        $data = array(
            "mode" => "viewRencanaKerja",
            "template" => $this->configLayout[$jenisTr]["receiptTemplate"][$currentStepNum],
            "title" => "RENCANA ANGGARAN <span class='text-uppercase'>$produk_nama</span>",
            "jenisTr" => $jenisTr,
            "pihakLabel" => $this->configUi[$jenisTr]["pihakLabel"],
            "mainLabels" => $this->configLayout[$jenisTr]["receiptMainFields"],
//            "main" => $mainProp,
...
//            "sumRows" => sizeof($receiptSumFieldsLayout) > 0 ? $receiptSumFieldsLayout : $receiptSumFields,
//            "sumRows2" => $receiptSumFields2,
            "arrProjectKomposisi" => $arrProjectKomposisi,
            "arrProjectKomposisiWorkOrder" => $arrProjectKomposisiWorkOrder,
            "arrProjectWorkOrder" => $arrProjectWorkOrder,
            "itemLabels" => $itemLabels,
            "customerDetails" => "",
            "companyProfile" => $arrCompanyProfile,
            "mainElements" => $elements,
