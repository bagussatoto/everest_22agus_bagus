...
//        $fc->addFilter("bln='$bulan'");
        ...
        $accountCatException = $this->config->item("accountCatOppositeExceptions") != null ? $this->config->item("accountCatOppositeExceptions") : array();
        // $accountRekeningSort = (sizeof($fcResult) > 0 && isset($fcResult['accountRekeningSort']) && ($fcResult['accountRekeningSort'] != NULL)) ? $fcResult['accountRekeningSort'] : $this->config->item("accountRekeningSort");
        $accountRekeningSort = $masterRekening = rekening_neraca_coa_sort_he_accounting();
...
//                $mts->addFilter("date(dtime)<='$date2'");
    ...
    public function viewNeracaYearlyAdj()
    {
        ...
        $masterRekening = rekening_neraca_coa_sort_he_accounting();
        ...
        if (sizeof($tempAdjustment) > 0) {
            ...
            // arrPrint($tempAdjustment);
            // matiHEre($tempAdjustment);
            ...
            $accountRekeningSort = $masterRekening;
            $arrCat = array("aktiva", "hutang", "modal", "lain-lain-kr");
            $arrCatView = array("aktiva", "hutang", "modal");
            ...
            if (sizeof($tmp) > 0) {
                foreach ($tmp as $cabID => $nerSpec) {
                    $listCabangAll[$cabID] = $cabID;
                    foreach ($nerSpec as $rowSpec) {

                        $masterHeader["prev"] = array(
                            "label" => "Neraca <br>( " . $rowSpec[0]->thn . " )",
                            "rekening" => array(
                                "debet", "kredit",
                            ),
                        );
            ...
            $listCabangAdj = array();
            $lastDateAdjustment = array();
            if (sizeof($tempAdjustment) > 0) {
                $lastDateAdjustment = $an->fetchLastDateAdjustment();
                foreach ($tempAdjustment as $CID => $adjustmentData) {
                    $listCabangAdj[$CID] = $CID;
                    foreach ($adjustmentData as $rowSpec) {
                        // cekMerah(sizeof($rowSpec));
                        $masterHeader["adj"] = array(
                            "label" => "jurnal koreksi per <br>( " . formatTanggal($lastDateAdjustment["dtime"], "d-m-Y") . ")",
                            "rekening" => array(
                                "debet", "kredit",
                            ),
                        );
            ...
            //region builder curent neraca karena hanya cabang yang diadjust yang punya data
            foreach ($listCabangAll as $cidDef => $ccid) {
                if (!isset($listCabangAdj[$cidDef])) {
                    $injectedCurent[$cidDef]["rev_1"] = $tempNeraca[$cidDef]["0"];
                }
            }
            $lastArray = sizeof($tempAdjustment) > 0 ? $tempCurentNeraca + $injectedCurent : array();
            //endregion
            $headerNeracaAhkir = array();
            $hitungCID = 0;
            if (sizeof($lastArray) > 0 && $tempAdjustment > 0) {
                foreach ($lastArray as $CID => $tempCurentNeraca_0) {
                    $hitungCID++;

                    foreach ($tempCurentNeraca_0 as $rowSpec) {
                        // cekHitam(sizeof($rowSpec));
                        $masterHeader["curent"] = array(
                            "label" => "Neraca koreksi(" . $rowSpec[0]->thn . ")  <br>per (" . formatTanggal($lastDateAdjustment["dtime"], "d-m-Y") . ")",
                            "rekening" => array(
                                "debet", "kredit",
                            ),
                        );
...
//         matiHEre();
        ...
        foreach ($arrCatView as $cat) {
            // foreach ($accountRekeningSort as $thn_ex => $accountRekeningSort_spec) {
//                cekBiru(":: $thn_ex ::");
//                arrPrintWebs($accountRekeningSort_spec);
            foreach ($accountRekeningSort[$cat] as $rekName) {
...
//                    cekHere(":: $rekName ::");
        ...
        // arrPrint($accountRekeningSort_spec);
        // arrPrint($rekeningsNameNew);
...
//        arrPrint($rekeningsNameNew);
//        $rekeningsNameNew2 = array();
//        foreach($rekeningsNameNew as $category => $specc){
//            ksort($specc);
//            $rekeningsNameNew2[$category] = $specc;
//        }
//        arrPrint($rekeningsNameNew2);
...
//            "cabang" => $arrCabangs,
            "cabang" => array(),
            "rekeningsName" => $rekeningsNameNew,
//            "rekeningsName" => $rekeningsNameNew2,
            "rekeningsNameAlias" => $accountAlias,
            "accountConsolidation" => $accountConsolidation,
            "pakai_konsolidasi" => sizeof($rekeningsKonsolidasi) > 0 ? 0 : 1,
            "rekeningKeterangan" => $rekeningKeterangan,
...
// matiHere(__LINE__ . $views_mode);
        ...
        $data = array(
            ...
            "categories" => $arrCatView,
            "rekenings" => $rekenings,
            "arrRekeningDetils" => $arrRekeningDetils,
            "rekeningsKonsolidasiNilai" => $rekeningsKonsolidasiNilai,
            "headers" => $headerss,
            "defaultDate" => $defaultDate,
            "oldDate" => $oldDate,
...
//                "link" => "",
    ...
    public function viewPL_consolidatedTahunanAdj()
    {
        $this->load->model("Mdls/MdlRugilaba");
        $this->load->model("Mdls/MdlRugilabaAdj");
        $this->load->model("Mdls/MdlFinanceConfig");
        $this->load->model("Mdls/MdlNeracaAdjTmp");
        $periode = "tahunan";
        $rekException = array("9010");
        ...
        if ($pakai_ini == 1) {
            ...
            $_GET['tm'] = 1;
            $adjN->addFilter("periode='$periode'");
            $adjN->addFilter("thn='$defaultDate'");
            $adjNilaiTemp["adj"] = $adjN->fetchBalance3();
            // arrprint($adjNilaiTemp);
            // matiHEre(sizeof($adjNilaiTemp["adj"]));
            if (sizeof($adjNilaiTemp["adj"]) > 0) {
                if ($_GET['tm'] == 1) {
                    $defaultDate = isset($_GET['date']) ? $_GET['date'] : previousYear();
                    $accountChilds = $this->config->item("accountChilds") != null ? $this->config->item("accountChilds") : array();
                    $accountAlias = $this->config->item("accountAlias") != null ? $this->config->item("accountAlias") : array();
                    ...
                    $ner = new MdlRugilaba();
                    $ner->addFilter("periode='$periode'");
                    $tmp["prev"] = $ner->fetchBalances2($defaultDate);//$defaultDate

                    $allCab = array();
                    if (sizeof($tmp) > 0) {
                        foreach ($tmp["prev"] as $ciid => $ciidData) {
                            $allCab[$ciid] = $ciid;
                        }
                    }
                    // }
                    ...
                    //region R/L adj
                    $ra = new MdlRugilabaAdj();
                    $ra->addFilter("periode='$periode'");
                    $ra->addFilter("thn='$defaultDate'");
                    // $tempRl["curentAdj"] = $ra->fetchBalance3();
                    $tmpRl = $ra->fetchBalance3();
                    // arrPrint($tmpRl);
                    //region merger kalau tidak ada adjustment pakai nilai prev
                    $cidAdj = array();
                    foreach ($tmpRl as $cid => $cidData) {
                        $cidAdj[$cid] = $cid;
                        // cekHitam($cid);
                    }
                    $toInjectCurentRl = array();
                    foreach ($allCab as $cabID => $cab_0) {
                        if (!isset($tmpRl[$cabID])) {
                            $toInjectCurentRl[$cabID]["rev_1"] = $tmp["prev"][$cabID]["0"];
                        }
                    }
                    $tempRl["curentAdj"] = $tmpRl + $toInjectCurentRl;
                    // arrPrint($toInjectCurentRl);
                    // arrPrint($tmpRl);
                    //                 matiHere(__LINE__);
                    //endregion
                    // $tempRl = $ra->fetchBalances3($defaultDate);
                    ...
                    $categoriesSubBottom = array();
                    foreach ($categoriesAll as $ctr => $cat) {
                        if (array_key_exists($cat, $rekenings["prev"])) {
                            $categories[] = $cat;
                            $categoriesSubBottom[] = isset($categoryRLBottom[$ctr]) ? $categoryRLBottom[$ctr] : "";
                        }
                    }
                    ...
                    // arrprint($adjNilaiTemp);
                    $lastDateAdjustment = array();
                    if (sizeof($adjNilaiTemp) > 0) {
                        $lastDateAdjustment = $adjN->fetchLastDateAdjustment();
                        foreach ($adjNilaiTemp as $thn_ex => $thn_ex_spec) {
                            foreach ($thn_ex_spec as $cabID => $nerSpec) {
                                foreach ($nerSpec as $rowSpec) {
                                    $masterHeader[$thn_ex] = array(
                                        "label" => "Koreksi per<br>(" . formatTanggal($lastDateAdjustment["dtime"], "d-m-Y") . ")",
                    ...
                    if (sizeof($tempRl) > 0) {
                        foreach ($tempRl as $thn_ex => $thn_ex_spec) {
                            foreach ($thn_ex_spec as $cabID => $nerSpec) {
                                foreach ($nerSpec as $rowSpec) {
                                    $masterHeader[$thn_ex] = array(
                                        "label" => "R/L Koreksi(" . $rowSpec[0]->thn . ") <br> per (" . formatTanggal($lastDateAdjustment["dtime"], "d-m-Y") . ")",
        ...
        $oldDate = "2019-09";
        if (isset($_GET['mode']) && ($_GET['mode'] == 'lapkeuangan')) {
            $views_mode = "keuangan_rugilaba_konsolidasi_adj";
            $views = "finance";
            $headerss = array(
                "values" => "balance(IDR)",
            );
        ...
        $headersTahun = array();//sengaja diset dulu
        $cabang = array();//sengaja diset dulu
        $headersKanan = array();//sengaja diset dulu
        $data = array(
            "mode" => "$views_mode",
            // "title" => "Laporan Rugilaba Konsolidasi tahunan ",
            "title" => "Laporan Rugilaba Konsolidasi Koreksi",
            // "subTitle" => "Laporan Rugilaba Konsolidasi tahunan " . lgTranslateTime3($defaultDate),
            "subTitle" => "Laporan Rugilaba Konsolidasi koreksi " . lgTranslateTime3($defaultDate),
            "categories" => $categories,
            "rekenings" => $rekeningSelected,
            "rekeningsKonsolidasiNilai" => $rekeningsKonsolidasiNilaiSelected,
            "headers" => $headerss,
...
//            "cashflow" => base_url() . "laporankeuangan/Keuangan/viewCashflowYtd?mode=lapkeuangan&c=konsolidasi" . $getDate,
    ...
    public function laporanKeuanganKoreksi()
    {
        ...
        $data = array(
            ...
            //            "pakai_konsolidasi" => sizeof($rekeningsKonsolidasi) > 0 ? 0 : 1,
            //            "rekeningKeterangan" => $rekeningKeterangan,
            "neraca" => base_url() . "laporankeuangan/" . "Keuangan/viewNeracaYearlyAdj?mode=lapkeuangan" . $getDate,
            "rugilaba" => base_url() . "laporankeuangan/" . "Keuangan/viewPL_consolidatedTahunanAdj?mode=lapkeuangan" . $getDate,
            //            "cashflow" => base_url() . "laporankeuangan/" . "Keuangan/viewCashflowYtd?mode=lapkeuangan&c=cabang" . $getDate,
            "periode" => "$periode",
        );
        $this->load->view("finance", $data);
...
//            "rekeningKeterangan" => $rekeningKeterangan,
            ...
            //untuk adjustment
            "neraca_adj" => base_url() . "laporankeuangan/Keuangan/viewNeracaYearlyAdj?mode=lapkeuangan" . $getDate,
            "rugilaba_adj" => base_url() . "laporankeuangan/Keuangan/viewPL_consolidatedTahunanAdj?mode=lapkeuangan" . $getDate,
