...
switch ($mode) {
    ...
    case "langsung_simple":
        ...
        foreach ($arrHeaders as $kolom => $arrHeader) {
            $ix++;
            $hLabel = isset($arrHeader['label']) ? $arrHeader['label'] : $kolom;
            $summary = isset($arrHeader['summary']) && $arrHeader['summary'] == true ? $ix : false;
            $sumKoloms_0[] = $summary;
            $strHead .= "<th title='$kolom'>$hLabel</th>";
        }
        $strHead .= "</tr>";

        $sumKoloms = array_filter($sumKoloms_0);
        $jsonKolom = implode(",", $sumKoloms);
        //arrPrintPink(json_encode($sumKoloms));
        //arrPrintPink(implode(",", $sumKoloms));
        ...
        foreach ($master_data as $master_datum) {
            ...
            foreach ($arrHeaders as $kolom => $attrs) {
                ...
                if (isset($attrs['data_order'])) {
                    $nilai_order = isset($master_datum[$attrs['data_order']]) ?
                        is_numeric($master_datum[$attrs['data_order']]) ? $master_datum[$attrs['data_order']] * 1 : $master_datum[$attrs['data_order']] :
                        "";
                    $data_order = "data-order='$nilai_order'";
                }
                else {
                ...
                if (isset($attrs['links'])) {
                    ...
                    $reqKey = isset($attrs['links']['key']) ? $attrs['links']['key'] : "";
                    $reqValue = isset($master_datum[$reqKey]) ? $master_datum[$reqKey] : "none";
                    $reqKey2 = isset($attrs['links']['key_2']) ? $attrs['links']['key_2'] : "";
                    $reqValue2 = isset($master_datum[$reqKey2]) ? $master_datum[$reqKey2] : "none";
                    $reqKeyAlt = isset($attrs['links']['key_alt']) ? "&".$attrs['links']['key_alt']."=$reqValue" : "";

                    $strKeyAlt2 = "";
                    if(isset($attrs['links']['key_alt_2'])){
                        $reqKeyAlt2 = $attrs['links']['key_alt_2'];
                        $reqValueAlt2 = $master_datum[$reqKeyAlt2];
                        $reqValueEncode = blobEncode($reqValueAlt2 . " " . $master_datum[$heading_key]);
                        $strKeyAlt2 =  "&blob_ext=$reqValueEncode";
                    }

                    $linking = isset($attrs['links']['target']) ? $attrs['links']['target'] . "?$strGet" . "&$reqKey=$reqValue&ky=$reqKey".$reqKeyAlt . $strKeyAlt2 : "";
                    // cekHijau("$reqKey");

                    $linking = str_replace("{" . $reqKey . "}", $reqValue, $linking);
                    $linking = str_replace("{" . $reqKey2 . "}", $reqValue2, $linking);

                    $linkDetile = base_url() . $linking . "";
                    // cekHere($linking);
                    // cekKuning($linkDetile);
                    $linkModal = modalDialogBtn("$heading_mdl", $linkDetile);
                    $nilai_link = isset($attrs['links']['target']) ? "<a href='javascript:void(0);' onclick=\"$linkModal\" title='$link_title'>$nilai_f</a>" : $nilai_f;
                }
                elseif (isset($attrs['collapsible'])) {
                ...
                $(document).ready( delay_v2( function(){
                    ...
                    var datareview$tbl_id = $('table#$tbl_id').DataTable({
                                    ...
                                    footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var arrayFooter = $('table#$tbl_id>tfoot>tr>th');
                                        var dpageTotal = [];
                                        var jml_kolom = (arrayFooter.length) - 1;
                                        jQuery.each(arrayFooter, function(i,d){
                                            var id_n_index = parseFloat(i);
                                        ...
                                        //
                                            ...
                                            var summKolom = [$jsonKolom];
                                            // console.log('summKolom: ',summKolom);
                                            if( !isNaN(dpageTotal[id_n_index]) && id_n_index > 1){
                                                if(summKolom.length > 0){
                                                    if(summKolom.includes(id_n_index)){
                                                        console.log(id_n_index);
        ...
        // if(ipadd() != MGK_LIVE){
            ...
            $strTbl .= "<script>
                let top_navigasi = '$top_navigasi';
                if(top_navigasi == false){
                    // $('#top_navigasi').addClass('hidden');
                    $('#tombol_bulan').addClass('hidden');
                }
                </script>";
        // }
