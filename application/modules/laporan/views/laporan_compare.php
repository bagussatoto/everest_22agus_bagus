...
switch ($mode) {
    default:
        ...
        foreach ($master_data as $master_datum) {
            ...
            /*--relatif modul---*/
            $jenistr = $master_jenis = isset($master_datum['jenis_master']) ? $master_datum['jenis_master'] : "582";
            // $modulnya = isset($modules[$master_jenis]) ? $modules[$master_jenis] : $master_jenis;
            $modulnya = isset($modules[$master_jenis]) ? $modules[$master_jenis] : "penjualan";
    ...
    case "langsung_indek":
        ...
        for ($i = 1; $i <= $bln_yg_tampil; $i++) {
            $hlabel_f = $bln_nama[$i];
            $strHead .= "<th>$hlabel_f</th>";
            // $strHead .= "<th colspan='4'>$hlabel_f</th>";
        }

        $strHead .= "<th>sub total</th>";
        $strHead .= "<th>avg</th>";
        $strHead .= "</tr>";
        // ----------------------------------------------------------------------------------------
        $strBody = "";
        ...
        $sum_bawah = 0;
        $sum_avg = 0;
        // arrPrint($customer_datas);
        foreach ($customer_datas as $seller_id => $customer_data) {
            $no++;
            $strBody .= "<tr >";
            $strBody .= "<td>$no</td>";
            $strBody .= "<td title='$seller_id'>$customer_data</td>";

            $bulanan_datas = $bln_datas[$seller_id];

            for ($i = 1; $i <= $bln_yg_tampil; $i++) {

                $ky_bln = $ncGlobal = sprintf("%02d", $i);
                // $dd = isset($ccc[$ky_bln]) ? $ccc[$ky_bln]->saldo_debet * 1 : 0;
                // $val_reject = isset($bulanan_datas[$ky_bln]) ? $bulanan_datas[$ky_bln]["sum_reject"] * 1 : 0;
                // $val_closed = isset($bulanan_datas[$ky_bln]) ? $bulanan_datas[$ky_bln]["sum_closed"] * 1 : 0;
                // $val_batal = $val_reject + $val_reject;

                $val_batal = isset($bulanan_datas[$ky_bln]) ? $bulanan_datas[$ky_bln]["sum_batal"] * 1 : 0;
                $val_return = isset($bulanan_datas[$ky_bln]) ? $bulanan_datas[$ky_bln]["sum_return"] * 1 : 0;


                $val_kirim = isset($bulanan_datas[$ky_bln]) ? $bulanan_datas[$ky_bln]["sum_debet"] * 1 : 0;

                $val_kirim_netto = $val_kirim - $val_batal - $val_return;

                // $val_kirim_f = formatField_he_format("harga", $val_kirim);
                // $strBody .= "<td>$val_kirim_f</td>";
                // $val_batal_f = formatField_he_format("harga", $val_batal);
                // $strBody .= "<td>$val_batal_f</td>";
                // $val_return_f = formatField_he_format("harga", $val_return);
                // $strBody .= "<td>$val_return_f</td>";

                $link_detile = "$link_detil&bln=$ky_bln&exid=$seller_id";
                $val_kirim_netto_f = formatField_he_format("harga", $val_kirim_netto);
                $val_kirim_netto_l = "<a href='$link_detile' target='_blank'>$val_kirim_netto_f</a>";
                $strBody .= "<td class='' data-order='$val_kirim_netto'>$val_kirim_netto_f</td>";

                if (!isset($sum_bln[$ky_bln]["sum_kolom"])) {
                    $sum_bln[$ky_bln]["sum_kolom"] = 0;
                }
                $sum_bln[$ky_bln]["sum_kolom"] += $val_kirim_netto;

                if (!isset($sum_bln[$ky_bln]["sum_batal"])) {
                    $sum_bln[$ky_bln]["sum_batal"] = 0;
                }
                $sum_bln[$ky_bln]["sum_batal"] += $val_batal;

                if (!isset($sum_bln[$ky_bln]["sum_return"])) {
                    $sum_bln[$ky_bln]["sum_return"] = 0;
                }
                $sum_bln[$ky_bln]["sum_return"] += $val_return;

                if (!isset($sum_bln[$ky_bln]["sum_bruto"])) {
                    $sum_bln[$ky_bln]["sum_bruto"] = 0;
                }
                $sum_bln[$ky_bln]["sum_bruto"] += $val_kirim;

                if (!isset($sum_seller[$seller_id]["sum_row"])) {
                    $sum_seller[$seller_id]["sum_row"] = 0;
                }
                $sum_seller[$seller_id]["sum_row"] += $val_kirim_netto;
            }

            $sum_kanan = $sum_seller[$seller_id]["sum_row"];
            $sum_kanan_f = formatField_he_format("harga", $sum_kanan);
            $avg_kanan = $sum_kanan / 12;
            $avg_kanan_f = formatField_he_format("harga", $avg_kanan);
            $strBody .= "<td title='$seller_id' class='bg-warning'>$sum_kanan_f</td>";
            $strBody .= "<td title='$seller_id' class='bg-success'>$avg_kanan_f</td>";
        ...
        foreach ($arrHeaders as $kolom => $attrs) {
            ...
            for ($i = 1; $i <= $bln_yg_tampil; $i++) {
                ...
                // $sum_bruto = isset($sum_bln[$ky_bln]) ? $sum_bln[$ky_bln]["sum_bruto"] * 1 : 0;
                // $strFoot .= "<th $attr title='$format_key'>$sum_bruto</th>";
                // $sum_batal = isset($sum_bln[$ky_bln]) ? $sum_bln[$ky_bln]["sum_batal"] * 1 : 0;
                // $strFoot .= "<th $attr title='$format_key'>$sum_batal</th>";
                // $strFoot .= "<th $attr title='$format_key'>pp</th>";


                $sum_netto = isset($sum_bln[$ky_bln]) ? $sum_bln[$ky_bln]["sum_kolom"] * 1 : 0;
                $sum_netto_f = formatField_he_format("harga", $sum_netto);
                $strFoot .= "<th $attr title='$format_key'>$sum_netto_f</th>";
            }

            $strFoot .= "<th>$sum_bawah</th>";
            $strFoot .= "<th>$sum_avg</th>";
                ...
                $(document).ready( delay_v2( function(){
                    ...
                    var datareview$tbl_id = $('table#$tbl_id').DataTable({
                                    ...
                                    footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var dpageTotal = [];
                                        jQuery.each(arrayFooter, function(i,d){
                                            var id_n_index = parseFloat(i);if(id_n_index > 1){
                                            // console.log(id_n_index);
                                            dpageTotal[id_n_index] = 0;
                                            jQuery.each( $(api.column(id_n_index, { page: 'current'}).data() ), function(ii, obj){
                                                // console.log(ii);
                                                ...
                                                // if(pos < 0){
                                                    ...
                                                    // dpageTotal[id_n_index] += intVal( accounting.unformat($('span',obj).html()) );
                                                // }
                                                // else{
                                            ...
                                            // if( !isNaN(dpageTotal[id_n_index]) && dpageTotal[id_n_index] > 0 ){
                                            if( !isNaN(dpageTotal[id_n_index])){
                                                $( api.column(id_n_index).footer() ).html(
                                                    \"<div class='text-right text-primary text-bold'> \"+addCommas(dpageTotal[id_n_index])+\" </div>\"
                                                );
                                            }
                                                ...
                                                $( api.column(id_n_index).footer() ).html(
                                                    \"<div class='text-right text-primary text-bold'>-o-</div>\"
                                                );
                                            }
    ...
    case "langsung_indek2":
        ...
        foreach ($customer_datas as $seller_id => $customer_data) {
            ...
            for ($i = 1; $i <= $bln_yg_tampil; $i++) {
                ...
                // $val_kirim_netto = $val_kirim - $val_closed - $val_reject;
               ...
               $val_kirim_netto = $val_kirim - $val_batal - $val_return;
                ...
                $id_td = $seller_id . "_" . $ky_bln;
                $link_detile = $link_detil."&bln=$ky_bln&exid=$seller_id";
                $val_kirim_netto_f = formatField_he_format("harga", $val_kirim_netto);
                // $val_kirim_netto_l = "<a href='$link_detile' target='_blank' class='text-link'>$val_kirim_netto_f</a>";
                // $val_kirim_netto_l = "<a href='$link_detile'  onclick=\"return hs.htmlExpand(this, { headingText: 'Lorem ipsum' })\" class='text-link'>$val_kirim_netto_f</a>";
                // $val_kirim_netto_l = "<a href='$link_detile'  onclick=\"return hs.htmlExpand(this, { objectType: 'ajax'}\" class='text-link'>$val_kirim_netto_f</a>";
                ...
                // $val_kirim_netto_l = "<a href='$link_detile'  onmouseover=\"return hs.htmlExpand(this, { outlineType: 'rounded-white',wrapperClassName: 'draggable-header', objectType: 'ajax',} )\" class='text-link'>$val_kirim_netto_f</a>";

                $strBody .= "<td id='$id_td' data-order='$val_kirim_netto'>$val_kirim_netto_l</td>";
                                        ...
                                        jQuery.each(arrayFooter, function(i,d){
                                            ...
                                            if(id_n_index > 1){
                                                ...
                                                jQuery.each( $(api.column(id_n_index, { page: 'current'}).data() ), function(ii, obj){
                                                    ...
                                                    var sum_bawah = intVal( accounting.unformat($('span',obj).html()) );
                                                    // console.log(sum_bawah);
                                                    // if(pos < 0){
                                                    ...
                                                    if(sum_bawah > 0){
                                                        dpageTotal[id_n_index] += sum_bawah;
                                                    }
                                                    else {
                                                        // dpageTotal[id_n_index] += 2;
                                                        dpageTotal[id_n_index] += intVal( accounting.unformat($(obj).html()) );
                                                    }
                                                });
                                                // if( !isNaN(dpageTotal[id_n_index]) && dpageTotal[id_n_index] > 0 ){
        ...
        $sub_title = "<br><small>$subTitle</small>";
        $color_bar = isset($color_bar) ? $color_bar : "box-danger";
        $p->setLayoutBoxCss("$color_bar");
        $btn_colaps = "<button class='btn btn-sm btn-box-tool' data-widget='collapse'><i class='fa fa-minus'></i></button>";
        $p->setLayoutBoxHeading("$reloader $title $sub_title", $btn_colaps);
        $p->setLayoutBoxBody(true);
    ...
    case "indek":
        ...
        if (isset($sum_satu)) {
            $str = "";
            $str .= "<div id='sum_satu'></div>";
//            if ((ipadd() == "202.65.117.72") || (my_id() == 159)) {
//                $str .= "<script>$('#sum_satu').load('$sum_satu_2');</script>";
//            }
//            else {
//                cekHere("hahahah");
                $str .= "<script>$('#sum_satu').load('$sum_satu');</script>";
...
// $add = $p->selectTahun();
    ...
    case "langsung_indek2_compared":
        $bln_yg_tampil = 12;
        $colspan = sizeof($arrSubHeaders);
        $strHead = "";
        $strHead .= "<tr class='text-uppercase'>";
        ...
        for ($i = 1; $i <= $bln_yg_tampil; $i++) {
            $hlabel_f = $bln_nama[$i];
            $strHead .= "<th colspan='$colspan'>$hlabel_f</th>";
        }

        $strHead .= "<th colspan='$colspan'>sub total</th>";
        $strHead .= "<th colspan='$colspan'>avg</th>";
        $strHead .= "</tr>";

        $strHead .= "<tr class='text-uppercase'>";
        for ($i = 1; $i <= $bln_yg_tampil; $i++) {
            foreach ($arrSubHeaders as $sub){
                $strHead .= "<th>$sub</th>";
            }
        }
        foreach ($arrSubHeaders as $sub){
        ...
        foreach ($customer_datas as $seller_id => $customer_data) {
            ...
            for ($i = 1; $i <= $bln_yg_tampil; $i++) {
                foreach ($arrSubHeaders as $ky_thn){
                    ...
                    $val_batal = isset($bulanan_datas[$ky_bln][$ky_thn]) ? $bulanan_datas[$ky_bln][$ky_thn]["sum_batal"] * 1 : 0;
                    $val_return = isset($bulanan_datas[$ky_bln][$ky_thn]) ? $bulanan_datas[$ky_bln][$ky_thn]["sum_return"] * 1 : 0;
                    $val_closed = isset($bulanan_datas[$ky_bln][$ky_thn]) ? $bulanan_datas[$ky_bln][$ky_thn]["sum_closed"] * 1 : 0;
                    $val_reject = isset($bulanan_datas[$ky_bln][$ky_thn]) ? $bulanan_datas[$ky_bln][$ky_thn]["sum_reject"] * 1 : 0;


                    $val_kirim = isset($bulanan_datas[$ky_bln][$ky_thn]) ? $bulanan_datas[$ky_bln][$ky_thn]["sum_debet"] * 1 : 0;
                    ...
                    if (!isset($sum_bln[$ky_bln][$ky_thn]["sum_kolom"])) {
                        $sum_bln[$ky_bln][$ky_thn]["sum_kolom"] = 0;
                    }
                    $sum_bln[$ky_bln][$ky_thn]["sum_kolom"] += $val_kirim_netto;

                    if (!isset($sum_bln[$ky_bln][$ky_thn]["sum_batal"])) {
                        $sum_bln[$ky_bln][$ky_thn]["sum_batal"] = 0;
                    }
                    $sum_bln[$ky_bln][$ky_thn]["sum_batal"] += $val_batal;

                    if (!isset($sum_bln[$ky_bln][$ky_thn]["sum_return"])) {
                        $sum_bln[$ky_bln][$ky_thn]["sum_return"] = 0;
                    }
                    $sum_bln[$ky_bln][$ky_thn]["sum_return"] += $val_return;

                    if (!isset($sum_bln[$ky_bln][$ky_thn]["sum_bruto"])) {
                        $sum_bln[$ky_bln][$ky_thn]["sum_bruto"] = 0;
                    }
                    $sum_bln[$ky_bln][$ky_thn]["sum_bruto"] += $val_kirim;

                    if (!isset($sum_seller[$seller_id][$ky_thn]["sum_row"])) {
                        $sum_seller[$seller_id][$ky_thn]["sum_row"] = 0;
                    }
                    $sum_seller[$seller_id][$ky_thn]["sum_row"] += $val_kirim_netto;

                }
            }

            foreach ($arrSubHeaders as $ky_thn){
                $sum_kanan = $sum_seller[$seller_id][$ky_thn]["sum_row"];
                $sum_kanan_f = formatField_he_format("harga", $sum_kanan);
//                $avg_kanan = $sum_kanan / 12;
//                $avg_kanan_f = formatField_he_format("harga", $avg_kanan);
                $strBody .= "<td title='$seller_id' class='bg-warning'>$sum_kanan_f</td>";
//                $strBody .= "<td title='$seller_id' class='bg-success'>$avg_kanan_f</td>";
                if(!isset($sum_bawah[$ky_thn])){
                    $sum_bawah[$ky_thn] = 0;
                }
                $sum_bawah[$ky_thn] += $sum_kanan;
            }
            foreach ($arrSubHeaders as $ky_thn){
                $sum_kanan = $sum_seller[$seller_id][$ky_thn]["sum_row"];
//                $sum_kanan_f = formatField_he_format("harga", $sum_kanan);
...
//                $strBody .= "<td title='$seller_id' class='bg-warning'>$sum_kanan_f</td>";
                $strBody .= "<td title='$seller_id' class='bg-success'>$avg_kanan_f</td>";
                if(isset($sum_avg[$ky_thn])){
                    $sum_avg[$ky_thn] = 0;
                }
                $sum_avg[$ky_thn] += $avg_kanan;
            }
        ...
        foreach ($arrHeaders as $kolom => $attrs) {
            ...
            for ($i = 1; $i <= $bln_yg_tampil; $i++) {
                foreach ($arrSubHeaders as $ky_thn){
                    ...
                    $sum_netto = isset($sum_bln[$ky_bln][$ky_thn]) ? $sum_bln[$ky_bln][$ky_thn]["sum_kolom"] * 1 : 0;
                    $sum_netto_f = formatField_he_format("harga", $sum_netto);
                    $strFoot .= "<th $attr title='$format_key'>$sum_netto_f</th>";
            ...
            foreach ($arrSubHeaders as $ky_thn){
                $sum_bawahh = isset($sum_bawah[$ky_thn])?$sum_bawah[$ky_thn] : 0;
                $sum_bawahh_f = formatField_he_format("harga", $sum_bawahh);
                $strFoot .= "<th>$sum_bawahh_f</th>";
            }
            foreach ($arrSubHeaders as $ky_thn){
                $sum_avgg = isset($sum_avg[$ky_thn])?$sum_avg[$ky_thn] : 0;
                $sum_avgg_f = formatField_he_format("harga", $sum_avgg);
                $strFoot .= "<th>$sum_avgg_f</th>";
            }
        ...
        $strTbl .= "<div class='table-responsive tblid_$tbl_id'>";
        $strTbl .= "<table class='table table-condensade table-striped table-hover-color-red' id='$tbl_id' style='border:1px solid black;' rules='all'>";
        // $strTbl .= "<caption class='hidden'>testing</caption>";
        $strTbl .= "<thead>";
        $strTbl .= $strHead;
        $strTbl .= "</thead>";
