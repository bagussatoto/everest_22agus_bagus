...
switch ($mode) {
    ...
    case "langsung_simple_bar":
        ...
        foreach ($arrHeaders as $kolom => $arrHeader) {
            $hLabel = isset($arrHeader['label']) ? $arrHeader['label'] : $kolom;
            $strHead .= "<th rowspan='2' title='$kolom'>$hLabel</th>";
        }
        foreach ($preloader as $item) {
            $bid = $item->id;
            $bnama = $item->nama;
            $jml_anak = count($bankDataInduk[$bid]);

            $strHead .= "<td colspan='$jml_anak'>$bnama</td>";
        }

        $colspan_checker = count($checkerBank);
        if(count($checkerBank)){
            $strHead .= "<td colspan='$colspan_checker'>checking</td>";
        }

        $strHead .= "</tr>";

        $strHead .= "<tr class='text-uppercase'>";
        // arrPrintPink($preloader);
        foreach ($preloader as $item) {
            $bid = $item->id;
            $bnama = $item->nama;
            if(!isset($bankDataInduk[$bid])){
                $strHead .= "<th>--</th>";
            }
            else{

                foreach ($bankDataInduk[$bid] as $item) {
        ...
        foreach ($checkerBank as $ceker_id => $ceker_nama) {
            $strHead .= "<th title='$ceker_id'>$ceker_nama</th>";
        }
        ...
        $pengenal_masterid = "";
//        cekHijau(count($master_data));
//arrPrintWebs($bankDataInduk);
        foreach ($master_data as $master_datum) {
            $no++;
//            arrPrint($master_datum);
//            matiHere(__LINE__);
            $oleh_id = $master_datum["oleh_id"];
            $transaksi_id = $master_datum["transaksi_id"];
            $extern_id = $master_datum["extern_id"];
            $diterima = $master_datum["debet"];
            $trash_4 = $master_datum["trash_4"];

$warna_baris = $trash_4 == 1 ? "style=' background-color: #cfcbcb; color: #93ffff;'" : "";
            $strBody .= "<tr $warna_baris>";
            $strBody .= "<td>$no</td>";
            ...
            foreach ($preloader as $item) {
                ...
                if(!isset($bankDataInduk[$bid])){
                    ...
                    foreach ($bankDataInduk[$bid] as $rek_id => $item) {
                        $e_id = $master_datum["transaksi_id"];
                        $target_table = array(
                            "jenis" => $master_datum["extern2_id"],
                            "jenis_nama" => $master_datum["extern2_nama"],
                            "kasir_id" => $master_datum["oleh_id"],
                            "customer_id" => $master_datum["customers_id"],
                            "bank_id" => $bid,
                            "rek_id" => $rek_id,
                            "transaksi_id" => $master_datum["transaksi_id"],
                            "transaksi_no" => $master_datum["transaksi_no"],
                            "nilai_transaksi" => $master_datum["debet"],
                            "dtime_transaksi" => date("Y-m-d H:i:s", strtotime($master_datum["transaksi_tanggal"] . " " . $master_datum["transaksi_jam"])),
                        //    "dtime_check" => $master_datum["jenis"], // <<=== ini apa mending pas mau nulis aja??
                            "bank_nama" => $master_datum["produk_kode"],
                            "rek_nama" => $master_datum["extern_nama"],
                            "customer_nama" => $master_datum["pihak_nama"],
                            "kasir_nama" => $master_datum["oleh_nama"]
                        );


                        $my_id = my_id();

                        $nilai_rekening = $rek_id == $extern_id ? $diterima : 0;
                        $nilai_rekening_f = $nilai_rekening*1> 0 ? number_format($nilai_rekening) : $nilai_rekening;
                        $nullClass = $nilai_rekening*1> 0 ? "ada_isi" : "kosong";
                        $sudahCheck = isset($bankChecker[$e_id]) ? "text-blue text-bold" : "";

                       // $filerLogin = 1;
                       if($filerLogin){
                           if($trash_4 == 1){
                               $btnBadge = (($nilai_rekening*1) > 0) ? "<span class='badge bg-grey pull-right'><i class='fa fa-times'></i></span>" : "";
                           }
                           else{
                               $btnBadge = isset($bankChecker[$e_id]) && $nilai_rekening*1> 0 ? "<span id='btn_$e_id' class='badge btncheck bg-green pull-right'><i class='fa fa-check text-white'></i></span>" : ($nilai_rekening*1> 0 ? "<span id='btn_$e_id' class='badge btncheck bg-red text-link pull-right'>cek</span>" : "") ;
                           }
                           $valuePost = $nilai_rekening*1> 0 ? urlencode(base64_encode(gzencode(json_encode($target_table)))) : "";
                        ...
                        $strBody .= "<td data-post='$valuePost' class='$nullClass $sudahCheck'>$nilai_rekening_f$btnBadge</td>";
                    }
                }
            }
            //----------------------kolom relatif
            foreach ($checkerBank as $ceker_id => $ceker_nama) {
                $countCheck = isset($checkerBankCount[$transaksi_id][$ceker_id]) ? $checkerBankCount[$transaksi_id][$ceker_id] : 0;
                $countCheckAddsAudit = isset($audited[$transaksi_id]) ? $audited[$transaksi_id] : 0;
                $base = "";
                $audit = "";
                if($countCheck > 0){

                    $outJson=array();
                    foreach($checkBankBulk[$transaksi_id] as $cid => $rowCheck){
                        foreach($rowCheck as $rowsCheck){
                            $outJson[] = $rowsCheck;
                        }
                    }

                    $cAudit =count($countCheckAddsAudit);
                    $bulkCount = count($outJson);
                    $dtime_check = $lastcek[$transaksi_id][$ceker_id]->dtime_check;
                    $dtime_check_f = formatTanggal($dtime_check, 'd-M-Y H:i');
                    // $strCheck = "<i class='fa fa-check'></i><span class='badge bg-green'>$countCheck</span>";
                    $countCheck = $bulkCount > $countCheck ? $bulkCount : $countCheck;
                    $strCheck = "<span bulkcount='$bulkCount' class='badge bg-green'>$countCheck </span> $dtime_check_f";
                    $linkStyle = "";
                    $linkFunction = "";
//                    if($countCheck > 1){
                        $jsonDatas = base64_encode(json_encode($outJson));
                        $base = "base='$jsonDatas'";
                        $audit = "audit='$cAudit'";
                        $linkStyle = "cursor:pointer;";
//                        $linkFunction = "onclick=\"bukaDetile('$transaksi_id','$ceker_id')\"";
//                         $linkFunction = "onclicks=\"bukaDetile(this)\"";
//                         $linkFunction = "";
//                    }
                }
                else{
                    // $strCheck = "<i class='fa fa-time'></i><span class='badge'>$countCheck</span>";
                    $strCheck = "-";
                    $linkStyle = "";
                    $linkFunction = "";
                }
                // $strBody .= "<th>$countCheck</th>";
                $strBody .= "<td $base $audit style='white-space: nowrap; $linkStyle' $linkFunction>$strCheck</td>";
            }
            $strBody .= "</tr>";
        ...
        // cekBiru("$jml_autoTr");
        // arrPrintHijau($autoTr);
        $tbl_id = isset($data_id) ? $data_id : "data_ok2";
        /* ---------------------------------------------------------------------------------------
         * generator scriptuntuk masuk ke js
         * ---------------------------------------------------------------------------------------*/
        $auto_tr = "";
        ...
        // arrPrintHijau($getQuery);
        // cekHere();
        $link_data = MODUL_PATH . "Kas/cekRekeningBar?$getQuery";
        $strTblPre = "";
        if (isset($preloader)) {
            $getbid = isset($_GET['xt']) ? $_GET['xt'] : "";
            $getcek = isset($_GET['cek']) ? $_GET['cek'] : $_GET['cek'] = 0;
            $getkey = isset($_GET['ky']) ? $_GET['ky'] : $_GET['ky'] = "";
            $strTblPre .= "<div style='margin-bottom: 10px;overfloww: hidden;'>";
            // arrPrint($checkerBank);
            foreach ($checkerBank as $cekerid => $cekernama) {
                    $btn_warna = $cekerid == $getcek ? "btn-danger" : "btn-primary";
                    $strTblPre .= "<button type='button' class='btn $btn_warna btn-flatt' onclick=\"$('#sum_null').load('$link_data&xt=0&xta=0&ky=oleh_id&cek=$cekerid');open_holdon();\">$cekernama</button> ";
            }

            if(count($customers)){
                $strTblPre .= "<select class='btn btn-info btn-flatt' onchange=\"$('#sum_null').load('$link_data&xt=0&xta=0&ky=customers_id&cek=' + this.value);open_holdon();\">";
                $strTblPre .= "<option value='' >pilih </option> ";
                foreach ($customers as $cekerid => $cekernama) {
                    $checked = $cekerid == $getcek && $getkey == "customers_id" ? "selected" : "";
                    $btn_warna = $cekerid == $getcek ? "btn-danger" : "btn-primary";
                    // $strTblPre .= "<button type='button' class='btn $btn_warna btn-flatt' onclick=\"$('#sum_null').load('$link_data&xt=0&xta=0&cek=$cekerid');open_holdon();\">$cekernama</button> ";
                    $strTblPre .= "<option value='$cekerid'  $checked>$cekernama</option> ";
                }
                $strTblPre .= "</select> ";
            }
            // foreach ($preloader as $item) {
            ...
            // cekHere("$getbid");
            // $btn_warna_all = $getbid == "morlin monrom0" ? "btn-danger" : "btn-primary";
            $btn_warna_all = ($getcek == "0") ? "btn-danger" : "btn-primary";
            $strTblPre .= "<button type='button' class='btn $btn_warna_all btn-flatt' onclick=\"$('#sum_null').load('$link_data&xt=0&xta=0&cek=0');open_holdon();\">Semua</button> ";
            $strTblPre .= "</div>";
        }
        ...
        // -------------------------------------------------------------------------------------------------
        $strTbl .= "<div class='table-responsive tblid_$tbl_id'>";
        $strTbl .= "<table border='0' class='table dataTable compact table-condensade table-striped table-hover-color-red' id='$tbl_id'>";
        // $strTbl .= "<caption class='hidden'>testing</caption>";
        $strTbl .= "<thead>";
        $strTbl .= $strHead;
        $strTbl .= "</thead>";
                ...
                $(document).ready( delay_v2( function(){
                    ...
                    var datareview$tbl_id = $('table#$tbl_id').DataTable({
                                    ...
                                    footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        top.$('.btncheck').off().on('click', function(e){
                                            var datas = $(this).parent().attr('data-post');
                                            top.swal({
                                                title: '============',
                                                html: 'anda telah menandai transaksi ini sebagai sudah dicek',
                                                type: 'success',
                                                onOpen: function(){
                                                    top.swal.enableLoading();
                                                    $.ajax({
                                                        url: '".MODUL_PATH."Kas/recordKasChecker',
                                                        type: 'POST',
                                                        data: {data:datas},
                                                        dataType: 'json',
                                                        success: function(response) {
                                                            //update button jadi hijau
                                                            if(top.$('#btn_'+response.transaksi_id)){
                                                                top.$('#btn_'+response.transaksi_id)
                                                                .removeClass('bg-red')
                                                                .addClass('bg-green')
                                                                .html(\"<i class='fa fa-check text-white'></i>\");

                                                                //close swal setelah output
                                                                setTimeout(function(){
                                                                    top.swal.close();
                                                                }, 1250)
                                                            }
                                        ...
                                        var targetQtips = $(\"td .badge\");
                                        jQuery.each(targetQtips, function(a, eeeeeeeee){
                                            let baseData = $(eeeeeeeee).parent().attr('base');
                                            // let jsonData = JSON.parse(atob(baseData));
                                            let jsonData = {};
                                                try {
                                                    if (baseData) {
                                                        jsonData = JSON.parse(atob(baseData));
                                                    }
                                                } catch (error) {
                                                    console.error(\"Error parsing JSON:\", error);
                                                    jsonData = {}; // Default ke objek kosong jika parsing gagal
                                                }
                                            let content = \"<b>Log Aktifitas:</b><br><ol>\";
                                            if (Array.isArray(jsonData) && jsonData.length > 0) {
                                                jsonData.forEach(item => {
                                                    let auditIcon = item.isAudit == \"1\"
                                                    ? '<b style=\"color: lime;\">(Audit)</b> '
                                                    : '';
                                                    content += \"<li>\"+auditIcon + item.dtime_check.substring(0, 19)+\" - \"+item.checker_nama+\"</li>\";
                                                });                                                
                                            }
                                            else {
                                                content += \"<li>Data tidak tersedia</li>\";
                                            }
                                        ...
                                        jQuery.each(arrayFooter, function(i,d){
                                            ...
                                            jQuery.each( $(api.column(id_n_index, { page: 'current'}).data() ), function(ii = '', obj = ''){
                                                ...
                                                var objStr = obj.toString().replace(/<br>/g, '\\n');
                                                var extractedValue = 0;
                                                var tempElement = $('<div>').html(objStr);

                                                if (tempElement.find('.btncheck').length) {
                                                    extractedValue = intVal(objStr.split('<span')[0].trim());
//                                                    console.log('kondisi 2: ' + extractedValue);
//                                                    console.log(objStr.split('<span')[0].trim());
                                                }
                                                else if (tempElement.find('span').length) {
                                                    extractedValue = intVal(tempElement.find('span').first().text());
//                                                    console.log('kondisi 3: ' + extractedValue);
//                                                    console.log(tempElement.find('span').first().text());
                                                }
                                                else {
                                                    extractedValue = intVal(objStr.split('<span')[0].trim());
//                                                    console.log('kondisi 1: ' + extractedValue);
//                                                    console.log(objStr.split('<span')[0].trim());
                                                }

                                                dpageTotal[id_n_index] += extractedValue;

//                                                var obj = obj.replace(/<br>/g, '\\n');
//                                                var pos = obj.indexOf('<');
//                                                if(pos!==-1){
//                                                    dpageTotal[id_n_index] += intVal( $(obj).html() );
//                                                }
...
//                                                }
        ...
        $strTbl .= "<script>
                $('.table-responsive.tblid_$tbl_id').off();
                $('.table-responsive.tblid_$tbl_id').scroll(
                    delay_v2(function () {
                        $('table#$tbl_id').DataTable().fixedHeader.adjust();
                    }, 200)
    ...
    case "indek":
         ...
         * penampil di browser LIMA
         * ---------------------------------------------------------------------------------------------*/
        if (isset($sum_lima) && strlen($sum_lima) > 42) {
            $str = "";
            $str .= "<div id='sum_lima'></div>";
            $str .= "<script>$('#sum_lima').load('$sum_lima');</script>";
            $content .= $str;
            ...
            $str = "";
            $str .= "<div id='sum_lima'><div class='text-uppercase' style='display: table;vert-align: middle;line-height: 25px;font-size: 20px;margin-left: 40px;'>$sum_lima</div></div>";
        ...
        if (isset($sum_null)) {
            // mati_disini();
            $str = "";
            $str .= "<div id='sum_null'>Mempersiapkan Data...  <div style='margin-left:50%;'>$loader_indikator</div></div>";
    ...
    /*-----cloningan indek untuk controler viewBlKas-----*/
    case "view_rek_kas":
        $p = New Layout("$title", "$subTitle", MODUL_TEMPLATE_PATH . "template/laporan_indek.html");
