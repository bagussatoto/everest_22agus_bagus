...
switch ($mode) {
    ...
    case "produk_bulanan_per":
        ...
        $bln_awal = formatTanggal($date1, "Y-m");
        $tahun = formatTanggal($date1, "Y");
        // cekHere($date1 . " $bln_awal");
        ...
        $strHead .= "<td colspan='3'><b class=''>$title_now $judul_lap</b></td>";
        // $strHead .= "<td rowspan='2' class='text-center' style='vertical-align: bottom !important;' id='unit'><b>total unit</b><span id='pg_unit'></span></td>";
        // $strHead .= "<td colspan='3'><b>Nilai Rata-rata Bulanan</b> <r class='font-size-1-2 pull-right'>$margin_avg_f</r></td>";
        ...
        /*----sum kanan----*/
        $strHead .= "<tr>";
        // $strHead .= "<th rowspan='2' class='text-center' title='saldo_awal'>Saldo Estimasi Awal $tahun</th>";
        for ($i = 1; $i <= $tgl_max; $i++) {
            ...
            if ($url_bulan == true) {
                $link_rincian = "laporan/Biaya/viewDetilBl?bl=$i&th=$thn_max";
                $linkDetile = base_url() . $link_rincian . "";
                $linkModal = modalDialogBtn("Biaya $bln_nama $thn_max", $linkDetile);
                $bln_nama_l = "<a href='javascript:void(0);' onclick=\"$linkModal\" title='lihat rincian'>$bln_nama</a>";
            }
            else {
                $bln_nama_l = "$bln_nama";
            }
            ...
            $strHead .= "<th colspan='3' class='text-center' title='$tgl_key'>$bln_nama_l</th>";
        }
        // --------------------
        $strHead .= "<th rowspan='2' class='text-center' style='vertical-align: bottom !important;' id='unitt'><b>Total Biaya</b><span id='pg_unitt'></span></th>";
        $strHead .= "<th rowspan='2' id='nilait'>Total Batal<span id='pg_nilait'></span></th>";
        $strHead .= "<th rowspan='2'>Total saldo</th>";
        ...
        for ($i = 1; $i <= $tgl_max; $i++) {
            $tgl_key = str_pad($i, 2, 0, STR_PAD_LEFT);
            $strHead .= "<th>Biaya</th>";
            $strHead .= "<th id='ttgl_$tgl_key'>Batal</th>";
            $strHead .= "<th>Saldo</th>";
        }
        ...
        foreach ($master_data as $produk_id => $master_datum) {
            ...
            $saldo_awal = $child_data[$produk_id][$bln_awal]['saldo_awal'];
            $saldo_awal_f = number_format($saldo_awal, 0);
            // $strBody .= "<td class='text-right bg-warning' data-order='$saldo_awal'>$saldo_awal_f</td>";
            // arrPrintHijau($child_data);
            $show_rincian = true;
            $pembagi = 1;
            $sum_kanan_qty = 0;
            $sum_kanan = 0;
            ...
            //             arrPrintKuning();
            for ($i = 1; $i <= $tgl_max; $i++) {
                $childDatas = $child_data[$produk_id];
                // arrPrintKuning($childDatas);
                $bln_key = str_pad($i, 2, 0, STR_PAD_LEFT);
                $bln_nama = $bulan_namas[$tgl_key] . " $label_now";
                $dtLabel = "$thn_max-$bln_key";
                // cekHere("$dtLabel");
                $link_rincian = "laporan/Penjualan/viewRincian?pid=$produk_id&bl=$i&th=$thn_max";
                $chNilai = isset($child_data[$produk_id]) ? (isset($child_data[$produk_id][$dtLabel]) ? ($child_data[$produk_id][$dtLabel]['sum_kredit'] * 1) / $pembagi : 0) : 0;
                $chNilai_f = number_format($chNilai, 0);
                $chNilai_l = $chNilai_f;
                if ($show_rincian == true) {
                    $heading_mdl = "$nilai_f $dtLabel";
                    $linkDetile = base_url() . $link_rincian . "";
                    $linkModal = modalDialogBtn("$heading_mdl", $linkDetile);
                    // $chNilai_l = "<a href='$link_rincian'>$chNilai_f</a>";
                    $chNilai_l = "<a href='javascript:void(0);' onclick=\"$linkModal\" title='lihat rincian'>$chNilai_f</a>";
                }

                $chQty = isset($child_data[$produk_id]) ? (isset($child_data[$produk_id][$dtLabel]) ? ($child_data[$produk_id][$dtLabel]['sum_debet'] * 1) : 0) : 0;
                $chQty_f = number_format($chQty, 0);
                $chSaldo = $chQty - $chNilai;
                $chSaldo_f = number_format($chSaldo, 0);

                if (is_array($url_detil)) {
                    $url_detil["produk_id"] = $produk_id;
                    $url_detil["bln"] = $dtLabel;
                    $query = http_build_query($url_detil);

                    $linkDetile = MODUL_PATH . "Biaya/viewDetil?$query";
                    $linkModal = modalDialogBtn("$heading_mdl", $linkDetile);
                    $nilai_link = "<a href='JavaScript:Void(0);' onclick=\"$linkModal\" title='$link_title'>$chSaldo_f</a>";
                }
                else {
                    $nilai_link = $chSaldo_f;
                }


                // $chSaldo_l = "<a href='$link_detil' onclick=\"\">$chSaldo_f</a>";
                $chSaldo_l = "$nilai_link";

                $strBody .= "<td class='text-right' data-order='$chQty'>$chQty_f</td>";
                $strBody .= "<td class='text-right' data-order='$chNilai'>$chNilai_f</td>";
                $strBody .= "<td class='text-right' data-order='$chSaldo'>$chSaldo_l</td>";

                $sum_kanan += $chNilai;
                $sum_kanan_qty += $chQty;
                $sum_kanan_saldo += $chSaldo;
            ...
            /*----summary kanan----------*/
            $sum_kanan_f = number_format($sum_kanan, 0);
            $harian_qty = $sum_kanan_qty;
            $sum_kanan_saldo_f = number_format($sum_kanan_saldo, 0);
            // $strBody .= "<td class='text-right bg-warning' data-order='$sum_kanan'>$sum_kanan_f</td>";
            $avg_margin = isset($margin_data[$produk_id]) ? $margin_data[$produk_id] : 0;
            ...
            $strBody .= "<td class='text-right bg-warning' data-order='$harian_qty'>$harian_qty_f</td>";
            $strBody .= "<td class='text-right bg-warning' data-order='$sum_kanan'>$sum_kanan_f</td>";
            $strBody .= "<td class='text-right bg-warning' data-order='$sum_kanan_saldo'>$sum_kanan_saldo_f</td>";
            // -----------------------------------------------------------
            // $strBody .= "<td class='text-right bg-success' data-order='$avg_harian_qty'>$avg_harian_qty_f</td>";
            // $strBody .= "<td class='text-right bg-success' data-order='$avg_harian'>$avg_harian_f</td>";
            // $strBody .= "<td class='text-right bg-info' data-order='$avg_margin'>$avg_margin_l</td>";
        ...
        $strTbl .= "<div class='table-responsive tblid_$tbl_id'>";
        $strTbl .= "<table border='1' class='table table-condensade table-striped table-hover-color-red table-bordered' id='$tbl_id'>";
        $strTbl .= "<thead>";
        $strTbl .= $strHead;
        $strTbl .= "</thead>";
        $strTbl .= "<tbody>";
    ...
    case "langsung_simple":
        ...
        if (isset($summariLabels)) {
            ...
            foreach ($summariNilais as $kei_1 => $sumValues) {
                ...
                $sum_btn .= "<button type='button' class='btn btn-info text-uppercase' onclick=\"$('#$kei_1').fadeToggle();\">$kei_1_label</button>";
    ...
    case "langsung_indek":
        ...
        $strBody = "";
        $no = 0;
        $modul_path = isset($modul_path) ? $modul_path : base_url() . "biaya/";
        $jenistr = isset($jenisTr) ? $jenisTr : "582";
        $strGet = $_SERVER['QUERY_STRING'];
        $pengenal_kolom = "";
        $pengenal_link = "";
        ...
        foreach ($master_data as $master_datum) {
            ...
            foreach ($arrHeaders as $kolom => $attrs) {
                ...
                $attr = str_replace("{pengenal}", $kolom . $data_id, $attr);
                $format_key = isset($attrs['format_key']) ? $attrs['format_key'] : $kolom;
                $nilai_f = isset($attrs['format']) ? ($nilai != 0 ? $attrs['format']($format_key, $nilai, $jenistr, $modul_path) : $nilai * 1) : $nilai;
                $str_logic = "";
                if (isset($attrs['logics'])) {
                    $ltexs = isset($attrs['logics']['text']) ? $attrs['logics']['text'] : "";
                    $lnilai = $attrs['logics']['nilai']; // tidak berguna ni, coba cari cari yg lain
    ...
    case "langsung_detail_bl":
        foreach ($master_data as $jenis => $master_datums) {

            // cekHijau($jenis);
            // arrPrint($master_datums);
            // arrPrintHijau(count($master_datums));
            $strHead = "";
            $strHead .= "<tr class='text-uppercase'>";
            $strHead .= "<td>no</td>";
            foreach ($arrHeaders as $kolom => $arrHeader) {
            ...
            // $jenistr = isset($jenisTr) ? $jenisTr : "582";
            // $strGet = $_SERVER['QUERY_STRING'];
            foreach ($master_datums as $master_datum_0) {
                $master_datum = (array)$master_datum_0;
                // cekHere(count($master_datum));
            ...
            $tbl_id = isset($data_id) ? $data_id. "_$jenis"  : "$jenis";
            $strTbl = "";
