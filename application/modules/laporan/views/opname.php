    ...
    case "langsung_simple":
        ...
        foreach ($master_data as $master_datum) {
            ...
            foreach ($arrHeaders as $kolom => $attrs) {
                ...
                elseif (isset($attrs['link_blanks'])) {
                    $produk_id = $master_datum['produk_id'];
                    $arrayString = $master_datum['produk_nama'];
                    $arrayblob = blobEncode($arrayString);
                    $link_params = $attrs['link_blanks'];
                    $linking = base_url() . $link_params['target'];
                    $linkCbid = $link_params['cabang_id'];
                    $linking_h = $linking . "$produk_id?o=$linkCbid&blob_ext=$arrayblob";
                    $nilai_link = "<a href='$linking_h' target='_blank' $attr title='testing'>$nilai_f</a>";
                }
                elseif (isset($attrs['collapsible'])) {
                    $layout_in = isset($attrs['collapsible']['layout']) ? $attrs['collapsible']['layout'] : "";
                    $layout_str = isset($attrs['collapsible']['layout']) ? "&layout=$layout_in" : "";
        ...
        if (isset($summary_on_top)) {
            ...
            foreach ($sum_bawah_data as $kei => $sum_bawah_datum) {
                ...
                $label_link = $label;
                if (isset($sum_bawah_datum['links'])) {
                    $paramLinks = $sum_bawah_datum['links'];
                    $heading_key = isset($paramLinks['heading']) ? $paramLinks['heading'] : "";
                    $heading_mdl = isset($master_datum[$heading_key]) ? urlencode($master_datum[$heading_key]) : $nilai;
                    $link_title = isset($paramLinks['title']) ? $paramLinks['title'] : "";
                    $reqKey = isset($paramLinks['key']) ? $paramLinks['key'] : "";
                    $reqValue = isset($master_datum[$reqKey]) ? $master_datum[$reqKey] : "none";
                    $fungsi = isset($paramLinks['fungsi']) ? $paramLinks['fungsi'] : "";
                    $linking = isset($paramLinks['target']) ? $paramLinks['target'] . "?$strGet" . "&$reqKey=$reqValue&ky=$reqKey" : "";
                    $linkDetile = base_url() . $linking . "";
                    // $linkModal = modalDialogBtn("$heading_mdl", $linkDetile);
                    // $label_link = isset($paramLinks['target']) ? "<a href='JavaScript:Void(0);' onclick=\"$linkModal\" title='$link_title'>$label</a>" : $label;
                    $label_link = isset($paramLinks['target']) ? "<a href=\'javascript:void(0);\' onclick=\"$fungsi(\'$nilai_key\',\'$linkDetile\',\'Detil $label\');\" >$label</a>" : $label;
                }
                ...
                $sum_atas .= "<div class=\'info-box-content\'>";
                $sum_atas .= "<span class=\'info-box-text\'>$label_link</span>";
            ...
            $strTbl .= "<script>
                ...
                function bukaModal(key,url,judul) {
                  console.log('key:', key);
                  console.log('url:', url);
