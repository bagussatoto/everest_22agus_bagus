...
switch ($mode) {
    ...
    case "produk_bulanan_per":
        ...
        for ($i = 1; $i <= $tgl_max; $i++) {
            ...
            $strHead .= "<th colspan='3' class='text-center' title='$tgl_key'>$bln_nama</th>";
        }
        // --------------------
        $strHead .= "<th rowspan='2' class='text-center' style='vertical-align: bottom !important;' id='unitt'><b>Total Estimasi Masuk</b><span id='pg_unitt'></span></th>";
        $strHead .= "<th rowspan='2' id='nilait'>Total Realisasi<span id='pg_nilait'></span></th>";
        $strHead .= "<th rowspan='2'>Total saldo estimasi</th>";
        ...
        for ($i = 1; $i <= $tgl_max; $i++) {
            $tgl_key = str_pad($i, 2, 0, STR_PAD_LEFT);
            $strHead .= "<th>Estimasi masuk</th>";
            $strHead .= "<th id='ttgl_$tgl_key'>Realisasi</th>";
            $strHead .= "<th>Saldo estimasi</th>";
        }
        ...
        foreach ($master_data as $produk_id => $master_datum) {
            ...
            for ($i = 1; $i <= $tgl_max; $i++) {
                ...
                $strBody .= "<td class='text-right' data-order='$chNilai'>$chNilai_f</td>";
                $strBody .= "<td class='text-right' data-order='$chSaldo'>$chSaldo_f</td>";
