...
class PenjualanCompare extends MX_Controller
{
    ...
    public function cekSellerBl2()
    {
        ...
        $sub_title = "Periode 01 Januari s/d 31 desember $req_thn";
    ...
    * ----------------------------------------------------*/
    ...
    public function viewepenjualan()
    {
       ...
       * -----------------------------------------------------------------------------------------------*/
        ...
        $data = array(
            ...
            "date_max"     => dtimeNow('Y-m-d'),
            // "sum_null"    => base_url() . "laporan/Crm/maintenance" . "$strGet",
            "sum_satu"     => base_url() . "laporan/PenjualanCompare/cekSellerBl2" . "$strGet",
            // "sum_satu_2"   => base_url() . "laporan/PenjualanCompare/cekSellerBl2" . "$strGet",
            // "sum_tiga"    => base_url() . "laporan/Penjualan/cekpenjualancustomer" . "$strGet",
            // "sum_empat"   => base_url() . "laporan/Penjualan/cekpenjualanproduk" . "$strGet",
    ...
    public function test()
    {
        ...
        $src_datas = $this->callpenjualanTransaksi($thn, $bln, $seller_id);
    ...
    protected function callpenjualanTransaksi($req_thn, $req_bln = "", $req_seller = "")
    {
        $reg_bln = array();
        if ($req_bln != "") {
            $reg_bln = array(
        ...
        $req_seller = $req_seller != "" ? array("seller_id" => $req_seller) : array();
        ...
        $tbl = "z_sales_return_salesman_transaksi_cache";
        $condites = array(
                "periode" => "bulanan",
                "thn"     => $req_thn,
            ) + $reg_bln  + $req_seller;
        ...
        $this->db->where_in("rekening", $rekenings);
        // $this->db->group_by("seller_id,bln,thn");
        $src_return = $this->db->get($tbl)->result();
        // showLast_query("biru");

        // ---------------------------------------------------------------batal
        $tbl = "z_sales_batal_salesman_transaksi_cache";
        if ($req_bln != "") {
            $reg_bln = array(
                "bln" => $req_bln
            );
        ...
        $this->db->where_in("master_jenis", $rekenings);
        // $this->db->group_by("seller_id,bln,thn");
        $src_batal = $this->db->get($tbl)->result();
        // showLast_query("merah");
        // arrPrintKuning($src_datas);

        $src_all = array_merge($src_datas, $src_batal, $src_return);
        // cekBiru(sizeof($src_all) . "=" . sizeof($src_datas) . "=" . sizeof($src_batal) . "=" . sizeof($src_return));
        foreach ($src_all as $item) {
            $rekening = $item->rekening;
            $new_datas = (array)$item;
            if ($rekening == "982") {
                $new_datas["return_nilai"] = $item->saldo_kirim;
                $new_datas["saldo_kirim"] = 0;
            }
            $src_00[] = (object)$new_datas;
        }
        // arrPrintPink($src_00);
        $src_finals = array();
        foreach ($src_00 as $item) {
            $net_saldo_kirim = $item->saldo_kirim;
            $net_batal_nilai = $item->batal_nilai;
            $net_return_nilai = $item->return_nilai;

            $net_saldo = $net_saldo_kirim - $net_batal_nilai - $net_return_nilai;
            $net_datas = (array)$item;
            $net_datas["saldo_netto"] = $net_saldo;

            $src_finals[] = (object)$net_datas;
        }
