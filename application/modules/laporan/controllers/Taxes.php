...
class Taxes extends MX_Controller
{
    ...
    public function callPaymentSource()
    {
        ...
        //        $this->db->join($tbl_2,"$tbl_1.extern_id=$tbl_2.id","right");
    ...
    public function cekRow()
    {
        ...
        $data = array(
            "mode"        => "langsung_simple",
            "title"       => "Rekap potongan pph 23 " . $judul_lap,
            "subTitle"    => "Raw data pembelian",
            "modul_path"  => $this->modul_path,
            // "color_bar"   => "box-info",
            // "jenisTr"     => $this->jenisTr,
    ...
    public function cekSumRow()
    {
        ...
        $segmen_4 = url_segment(4);
        //        $date1 = $get_date1 = isset($_GET['date1']) ? $_GET['date1'] : dtimeNow('Y-m-01');
        $date1 = $get_date1 = isset($_GET['date1']) ? $_GET['date1'] : "2024-03-01";
        $date2 = $get_date2 = isset($_GET['date2']) ? $_GET['date2'] : dtimeNow('Y-m-d');
        $mainConfig = $this->config->item("heTransaksi_ui")["462"];
        //$modulConfig = $this->load->config->item()
        //arrPrint(MODUL_PATH);
        $jenis = "462";
        $tbl_1 = "__rek_pembantu_pph__2030030";
        //        $wheres = "jenis='$jenis' AND DATE(dtime) >= '$date1' AND DATE(dtime) <= '$date2'";
        $ky = $_GET['ky'];
        $ky_value = isset($_GET[$ky]) ? $_GET[$ky] : "";
        ...
        foreach ($tmpA as $item) {
            ...
            $tagNilai = isset($tagihan['sisa']) ? $tagihan['sisa'] : 0;
            $itemtambahan['sisa_tagihan'] = $tagNilai;
            $itemtambahan['nomer'] = isset($tagihan['nomer']) ? $tagihan['nomer'] : "";
            $itemtambahan['pihak_nama'] = isset($tagihan['extern_nama']) ? $tagihan['extern_nama'] : "";
            $itemtambahan['total_tagihan'] = isset($tagihan['tagihan']) ? $tagihan['tagihan'] : 0;
            $itemtambahan['total_terbayar'] = isset($tagihan['terbayar']) ? $tagihan['terbayar'] : 0;
            $itemtambahan['dpp_pph'] = isset($tagihan['extern_nilai2']) ? $tagihan['extern_nilai2'] : 0;
            $itemtambahan['total_pph'] = isset($tagihan['tagihan']) ? $tagihan['tagihan'] : 0;
            $itemtambahan['transaksi_tanggal'] = $transaksi_tanggal;
            $itemtambahan['transaksi_jam'] = $transaksi_jam;
        ...
        * ---------------------------------------------*/
        ...
        $data = array(
            // "mode"        => "langsung_indek",
            "mode"                 => "langsung_simple",
            "title"                => "Rekap Rincian Potongan PPH-23  &nbsp;" . $judul_lap,
            "subTitle"             => "",
            "modul_path"           => $this->modul_path,
            // "color_bar"   => "box-info",
            // "jenisTr"     => $this->jenisTr,
    ...
    public function perindekshow()
    {
        ...
        $data = array(
            ...
            // "sum_satu"    => base_url() . "laporan/" . get_class($this) . "/cekSumRow" . "$strGet",
    ...
    * ----------------------------------------------------*/
    public function view()
    {
       ...
       * -----------------------------------------------------------------------------------------------*/
        ...
        $data = array(
            ...
            "date_min"    => 1,
            "date_max"    => dtimeNow('Y-m-d'),
            "sum_satu"    => base_url() . "laporan/Taxes/viewHarian" . "$strGet",
        );
        $this->load->view("laporan", $data);
    }
    ...
    {
        $koloms = array(
            "sum(tagihan) as 'sum_tagihan'",
            "sum(sisa) as 'sum_sisa'",
        );
        // $this->db->select($koloms);
        // $this->db->group_by("extern_id");
        $paymentSources = $this->callPaymentSource();
        // arrPrintKuning($paymentSources);
        $koloms = array(
            "extern_id",
            "extern_nama",
        );
        ...
        foreach ($paymentSources as $paymentSource) {
            ...
            $supplier_id = $paymentSource['extern_id'];
            foreach ($koloms as $kolom) {
                $data_bentukans[$supplier_id][$kolom] = $paymentSource[$kolom];
            }

            foreach ($kolom_sums as $kolom) {
                if (!isset($data_bentukans[$supplier_id][$kolom])) {
                    $data_bentukans[$supplier_id][$kolom] = 0;
                }
                $data_bentukans[$supplier_id][$kolom] += $paymentSource[$kolom];

            }

            $master_datas[$supplier_id] = $data_bentukans[$supplier_id];

        }

        $targetJenis = '462';
        $configPath = "../../modules/pembayaran/config/";
        $this->load->config($configPath . "coTransaksiUi");
        $this->configUi = $this->config->item("coTransaksiUi");
        // arrPrintPink($master_datas);
        $shoppingCartReferenceExternFields = (isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFields"])) ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFields"] : array();
        // arrPrintKuning($shoppingCartReferenceExternFields);
        ...
        $arrHeaders = array(
            "extern_nama" => array(
                ...
                "link_loader" => array(
                    "key"    => "extern_id",
                    "target" => MODUL_PATH . get_class($this) . "/cekSumRow/1"
                ),
            ),
            "tagihan"     => array(
                // "label"  => "tagihan",
                "label"  => isset($shoppingCartReferenceExternFields['tagihan']) ? $shoppingCartReferenceExternFields['tagihan'] : "tagihan",
                "format" => "formatField_he_format",
            ),
            "terbayar"    => array(
                "label"  => isset($shoppingCartReferenceExternFields['terbayar']) ? $shoppingCartReferenceExternFields['terbayar'] : "terbayar",
                "format" => "formatField_he_format",
            ),
            "diskon"      => array(
                "label"  => isset($shoppingCartReferenceExternFields['diskon']) ? $shoppingCartReferenceExternFields['diskon'] : "diskon",
                "format" => "formatField_he_format",
            ),
            "sisa"        => array(
                // "label"  => "tagihan",
                "label"  => isset($shoppingCartReferenceExternFields['sisa']) ? $shoppingCartReferenceExternFields['sisa'] : "sisa",
                "format" => "formatField_he_format",
            ),
        );
        $data = array(
            // "mode"        => "langsung_indek",
            "mode"        => "cekTransaksi",
            "title"       => "Rekap PPH23" . $judul_lap,
            // "subTitle"             => "",
            // "modul_path"           => $this->modul_path,
            // // "color_bar"   => "box-info",
            // // "jenisTr"     => $this->jenisTr,
            // "jenisTr"              => "462",
            "data_id"     => "belum_bayar" . dtimeToSecond(dtimeNow()) . randomNumber(3),
            // "layout"               => isset($_GET['layout']) ? $_GET['layout'] : true,
            "arrHeaders"  => $arrHeaders,
            ...
            // "date1"                => $date1,
            // "date2"                => $date2,
            "color_bar"   => "box-solid box-danger",
            "date_min"    => 1,
            "date_max"    => dtimeNow('Y-m-d'),
            "loader_div"  => "sum_null",
            // "sum_satu"    => base_url() . "laporan/Crm/produkoutstandingblnseller" . "$strGet",
    ...
    public function perindekshowpph()
    {
        $this->load->helper("he_mass_table");
        $this->load->model("Coms/ComRekeningTransaksiPembantu");
        $ps = new ComRekeningTransaksiPembantu();
        ...
        $data = array(
            "mode"        => "indek",
            "title"       => "Rekap PPH-23" . callMenuLabel_he_menu(),
            "subTitle"    => "Raw Potongan pph23",
            "modul_path"  => $this->modul_path,
            // "jenisTr"     => $this->jenisTr,
            "jenisTr"     => "582",
            ...
            // --------------------------------------------------
            "sum_null"    => base_url() . "laporan/" . get_class($this) . "/$ygditampilkan" . "$strGet",
            "sum_satu"    => base_url() . "laporan/" . get_class($this) . "/cekSumRow/1" . "$strGet",
            // "sum_dua"     => base_url() . "laporan/Penjualan/produksotransaksi" . "$strGet",
            // "sum_tiga"    => base_url() . "laporan/Penjualan/produksoproduk" . "$strGet",
            // "sum_empat"   => base_url() . "laporan/Penjualan/produksocustomer" . "$strGet",
            // "sum_lima"    => base_url() . "laporan/Crm/produkoutstandingcabang" . "$strGet",
    ...
    public function perindekshowppnmasukan()
    {
     ...
     * laporan ppn masukan
     * ---------------------------------------*/
    public function viewRealisasi()
    {
        //        arrprint($_GET);
        //        arrPrint($this->uri->segment_array());
        $currYear = isset($_GET['year']) ? $_GET['year'] : date("Y");
        $curentDate1 = isset($_GET['date1']) ? $_GET['date1'] : date("Y-m") . "-01";
        $curentDate2 = isset($_GET['date2']) ? $_GET['date2'] : date("Y-m-d");
        $selectedTarget = "0000";
        $defaultItemSrc = array(
            //                "extern_date2" => "date e-faktur",
            "dtime"         => "tgl<br>e-faktur",
        ...
        $tr->addFilter("DATE(dtime)>='$curentDate1");
        $tr->addFilter("DATE(dtime)<='$curentDate2");
        $tr->addFilter("cabang_id='" . $this->session->login['cabang_id'] . "'");
        //        $tr->addFilter("sisa>1000");
        $tr->addFilter("label='ppn realisasi'");
        ...
        if (count($tmpSrc) > 0) {
            foreach ($tmpSrc as $row) {
                ...
                $aa = $tr->lookupDetailTransaksi($row->transaksi_id);
                $reg = $tr->lookupBaseDataRegistries($row->transaksi_id)->result();
                $baseMain = blobDecode($reg[0]->main);
                $listItems = "";
                if (count($aa) > 0) {
                    foreach ($aa[$row->transaksi_id] as $aaItems) {
                        if ($aaItems->produk_jenis == "invoice") {
                            $listItems .= formatField_he_format("nomer", $aaItems->produk_nama, "pembelian") . "<br>";
                            //                                $listItems .= formatField_he_format("nomer",$aaItems->produk_nama,"pembelian")."<br>";
                        }
                        else {
                            //
                            $listItems = formatField_he_format("nomer", $baseMain["efakturSource"], "pembelian");
                        }
                    }
                }
                else {
                    //                    $listItems = $row->efaktur_source;
                }
                $tmp = array();
                $jenis = $row->jenis;
                $transaksi_id = $row->id;
        ...
        $data = array(
            ...
            //            "arrHeaders" => $defaultItemSrc,
            "arrHeaders"  => $header,
            "master_data" => $itemsSrc,

            "recapsLabel"      => $defaultItemSrc,
            "recaps"           => $itemsSrc,
            "jenisTr"          => $this->jenisTr,
    ...
    public function cekPpnKeluaranYtd()
    {
        $this->load->helper("he_mass_table");
        $this->load->model("Coms/ComRekeningPpnPeriode");
        $ps = new ComRekeningPpnPeriode();

        $reqThn = dtimeNow('Y');

        $ps->addFilter("thn='$reqThn'");
        $ps->addFilter("periode='bulanan'");
        $ps->addFilter("cabang_id='1'");
        $srcs = $ps->lookupAll()->result();
        // showLast_query("biru");
        // arrPrintPink($srcs);

        foreach ($srcs as $src) {
            $kredit = $src->kredit;
            $srcDatas[$src->thn][$src->bln] = $src->kredit;
            if(!isset($srcDatas[$src->thn]['summ'])){
                $srcDatas[$src->thn]['summ'] = 0;
            }
            $srcDatas[$src->thn]['summ'] += $src->kredit;
        }
        $master_datas = $srcDatas;
        // arrPrintHijau($srcDatas);

        $arrHeaders = array();
        $bulans = namaBulan();
        // arrPrintKuning($bulans);
        $btn_now = dtimeNow('m');
        $thn_now = dtimeNow('Y');
        for ($s = 1; $s <= $btn_now; $s++) {
            $bln = digit_2($s);
            $bln_nama = $bulans[$bln];
            $arrHeaders[$bln] = array(
                "label" => $bln_nama,
                "format" => "formatField_he_format",
                "format_key" => "harga",
            );
        }
        $arrHeaders['summ'] = array(
            "label" => "YTD $thn_now",
            "format" => "formatField_he_format",
            "format_key" => "harga",
        );
    ...
    /*----ppn keluaran--*/
    public function viewppnkeluaran()
    {
        ...
        $date1 = $get_date1 = isset($_GET['date1']) ? $_GET['date1'] : $_GET['date1'] = "";
        ...
        $data = array(
            ...
            // --------------------------------------------------
            // "sum_null" => base_url() . "laporan/" . get_class($this) . "/$ygditampilkan" . "$strGet",
            "sum_null"    => base_url() . "laporan/Taxes/cekPpnKeluaranYtd" . "$strGet&o=1",
            "sum_satu"    => base_url() . "Ledger/viewMoveDetailsPPN/Rekening/2030060" . "$strGet&o=1",
            // "sum_satu"    => base_url() . "laporan/" . get_class($this) . "/cekSumRow" . "$strGet",
            // "sum_dua"     => base_url() . "laporan/Penjualan/produksotransaksi" . "$strGet",
            // "sum_tiga"    => base_url() . "laporan/Penjualan/produksoproduk" . "$strGet",
            // "sum_empat"   => base_url() . "laporan/Penjualan/produksocustomer" . "$strGet",
