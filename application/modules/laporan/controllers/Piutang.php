...
class Piutang extends MX_Controller
{
    ...
    public function cekRow()
    {
        ...
        if(isset($date1) && $date1 != $date_start){
            cekMerah($_GET['date1']);
            $tange_req = array(
                "date(dtime) >=" => $date1,
                "date(dtime) <=" => $date2,
        ...
        // arrPrintKuning($tagihanDuedates);
...
// arrPrintHijau($_GET);
        ...
        foreach ($tagihans as $item) {
            ...
            if ($pembayaran_nama == "cash") {
                ...
                $dueDate = $tagihanDuedate['due_date'];
                $umur_d = umurDay($dueDate);
                $itemtambahan['due_date'] = isset($tagihanDuedate['due_date']) ? $dueDate : "---";
                $itemtambahan['umur_now'] = $umur_d;
            }
            if ($transaksi_id == '27137') {
        ...
        $data = array(
            "mode"                 => "langsung_simple",
            "title"                => "Laporan umur piutang " . $judul_lap,
            "subTitle"             => "Raw data pembelian",
            "modul_path"           => $this->modul_path,
            // "color_bar"   => "box-info",
            // "jenisTr"     => $this->jenisTr,
    ...
    public function perindekshow()
    {
        ...
        $add_td = "<td id='tombol_bulan'>$btn_td</td>";
        // -----------------------------------------------------------------------------------------------
