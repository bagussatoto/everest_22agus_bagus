...
//        arrPrint($items);
        ...
        foreach ($items as $iSpec) {
            ...
            if ($showItems) {
                ...
                echo "<tr style='background-color: #c5e9ff;font-weight: 700;' id='tr_" . $iSpec['id'] . "' bgcolor=$bgColor>";
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            echo "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control form-control-sm text-right' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
