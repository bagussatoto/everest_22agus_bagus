...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        //--------------------------------
        $shoppingCartAdvanceSubValidator = isset($this->configUi[$this->jenisTr]["shoppingCartAdvanceSubValidator"]) ? $this->configUi[$this->jenisTr]["shoppingCartAdvanceSubValidator"] : array();
        //--------------------------------
...
//                                $errFields[$id][] = $field;
        ...
        //---------------------------
        if(sizeof($shoppingCartAdvanceSubValidator)>0){
            $pph23MethodPotongan = isset($_SESSION[$cCode]["main"]["pph23MethodPotongan"]) ? $_SESSION[$cCode]["main"]["pph23MethodPotongan"] : "none";
            if(isset($shoppingCartAdvanceSubValidator[$pph23MethodPotongan]) && (sizeof($shoppingCartAdvanceSubValidator[$pph23MethodPotongan])>0)){
                // main
                foreach ($shoppingCartAdvanceSubValidator[$pph23MethodPotongan] as $keyy => $labell){
                    $dpp_pph = isset($_SESSION[$cCode]["main"][$keyy]) ? $_SESSION[$cCode]["main"][$keyy] : 0;
                    if($dpp_pph < 10000){
                        $msg = $labell . " code:" . __LINE__;
                        mati_disini($msg);
                    }
                }
                // items
                foreach ($_SESSION[$cCode]["items"] as $pidd => $specc){
                    if(isset($specc["pph"]) && ($specc["pph"] == 1)){
                        if(isset($_SESSION[$cCode]["items2"][$pidd])){
                            foreach ($_SESSION[$cCode]["items2"][$pidd] as $spec){
                                foreach ($shoppingCartAdvanceSubValidator[$pph23MethodPotongan] as $keyy => $labell){
                                    $dpp_pph = isset($spec[$keyy]) ? $spec[$keyy] : 0;
                                    if($dpp_pph < 10000){
                                        $msg = $labell . " code:" . __LINE__;
                                        mati_disini($msg);
                                    }
