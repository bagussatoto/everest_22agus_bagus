...
//            }
        ...
        if ($id == "eFaktur") {
            $ppn_nilai = $_SESSION[$cCode]["main"]["ppn"];
            if ($ppn_nilai > 0) {
                $this->load->model("Mdls/MdlPaymentSource");
                $pp = New MdlPaymentSource();
                $pp->addFilter("target_jenis='0000'");
                $pp->addFilter("extern_label2=$val");
                $ppTmp = $pp->lookupAll()->result();
                if (sizeof($ppTmp) > 0) {
                    $msg = "Nomer eFaktur yang anda input tidak valid karena sudah pernah dipakai. Silahkan dikoreksi lagi.";
                    mati_disini($msg);
                }
            }
