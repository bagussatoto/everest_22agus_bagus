                ...
                foreach ($items as $iSpec) {
                    if (strlen($iSpec['nama']) > 0) {
                        ...
                        if (isset($iSpec['nama']) && (strlen($iSpec['nama']) > 3)) {
                            $label = isset($iSpec['label']) ? $iSpec['label'] : "";
//                            $strContent .= isset($iSpec['nama']) ? $iSpec['nama'] . " &nbsp;" . $label : "";
                            if (isset($iSpec["color"])) {
                                $name = isset($iSpec['nama']) ? $iSpec['nama'] . " &nbsp; " . $label : "";
                                $str_name = "<span class='ttext-bold' style='color: red;font-size: medium;font-style: italic;'>" . $name . "</span>";
                            }
                            else {
                                $newFields = in_array('nama', arrAvailFields()) ? formatNota('nama', $iSpec['nama']) : $iSpec['nama'];
                                $str_name = isset($iSpec['nama']) ? $newFields . " &nbsp; " . $label : "";
                            }
//                            $strContent .= isset($iSpec['nama']) ? $iSpec['nama'] . " &nbsp; *" . $label."***" : "";
                            $strContent .= $str_name;
                        }
                        else {
                            $strContent .= "<span label='" . $iSpec['label'] . "'>" . $iSpec['label'] . "</span>";
                        }

                        if (isset($_SESSION[$cCode]['items'][$iSpec['id']])) {
                            $strContent .= "<span pid='" . $iSpec['id'] . "' id='itmCheck_" . $iSpec['id'] . "' class='itemCheck pull-right'> <i class='fa fa-check-square text-success ttext-bold'></i> </span>";
                        }
                        else {
