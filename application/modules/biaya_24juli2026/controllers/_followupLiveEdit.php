...
//    private $jenisTr;
    ...
    public function removeItem()
    {
        ...
        $id = isset($_GET['id']) ? $_GET['id'] : $_GET['mainid'];
        ...
        if (isset($_SESSION[$cCode]['items'][$id])) {
            ...
            if (isset($_GET['childid'])) {
                ...
                $_SESSION[$cCode]['items2'][$mainid][$childid] = null;
                unset($_SESSION[$cCode]['items2'][$mainid][$childid]);
            }
            else {
                //----
                $rekening_item = $_SESSION[$cCode]['items'][$id]['rekening'];
                $rekening_item_0 = str_replace(" ", "_", $rekening_item);
                $_SESSION[$cCode]['main'][$rekening_item_0] = null;
                unset($_SESSION[$cCode]['main'][$rekening_item_0]);
                //----
...
//            unset($_SESSION[$cCode]['main']['harga']);
        ...
        if (sizeof($_SESSION[$cCode]['items']) > 0) {
            foreach ($_SESSION[$cCode]['items'] as $xid => $iSpec) {
                $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
//                $_SESSION[$cCode]['items'][$xid]["harga_anggaran"] = $iSpec['harga']*1;
            }
        }
...
//                    $subcat["piutang_cabang"] += $data_1["subtotal"]*1;
                    $total_by_id += $data_1["subtotal"] * 1;
                }
//                $_SESSION[$cCode]["items"][$by_id]["harga_anggaran"] = $_SESSION[$cCode]["items"][$by_id]["harga"]*1;
                $_SESSION[$cCode]["items"][$by_id]["harga"] = $total_by_id / $_SESSION[$cCode]["items"][$by_id]["jml"];
                $_SESSION[$cCode]["items"][$by_id]["subtotal"] = $total_by_id;
            }
...
//            }
        ...
        if (sizeof($_SESSION[$cCode]['items']) > 0) {
            foreach ($_SESSION[$cCode]['items'] as $xid => $iSpec) {
                $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
                if ($iSpec['harga'] * 1 == 0) {
                    $_SESSION[$cCode]["items"][$xid]["harga"] = $iSpec['harga_anggaran'] * 1;
                    $_SESSION[$cCode]["items"][$xid]["subtotal"] = ($iSpec['jml'] * $iSpec['harga_anggaran']);
                }
            }
        }
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    public function rebuildValues()
    {
        ...
        if (sizeof($_SESSION[$cCode]['items']) > 0) {
            $subcat = array();
            foreach ($_SESSION[$cCode]['items'] as $mid => $rows) {
                $total_by_id = 0;
                if (isset($_SESSION[$cCode]["items2"][$mid]) && count($_SESSION[$cCode]["items2"][$mid])) {
                    foreach ($_SESSION[$cCode]["items2"][$mid] as $cid => $data_1) {
                        $catnama = str_replace(" ", "_", $data_1["cat_nama"]);
                        if (!isset($subcat[$catnama])) {
                            $subcat[$catnama] = 0;
                        }
                    ...
                    $_SESSION[$cCode]["items"][$mid]["harga"] = $total_by_id / $_SESSION[$cCode]["items"][$mid]["jml"];
                    $_SESSION[$cCode]["items"][$mid]["subtotal"] = $total_by_id;
                }
                else {
                    $catnama = str_replace(" ", "_", $rows["cat_nama"]);
                    if (!isset($subcat[$catnama])) {
                        $subcat[$catnama] = 0;
                    }
                    $subcat[$catnama] += $rows["subtotal"] * 1;
                    if (!isset($subcat["piutang_cabang"])) {
                        $subcat["piutang_cabang"] = 0;
                    }
                    $subcat["piutang_cabang"] += $rows["subtotal"] * 1;
//                    $total_by_id += $rows["subtotal"]*1;
                }
            }
            foreach ($subcat as $key => $val) {
                $_SESSION[$cCode]["main"][$key] = $val;
