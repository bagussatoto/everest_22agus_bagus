...
//        arrPrint($tmpGaransi);
        ...
        <script>
            function triggerEntriNilai(nilai,jenis='',ket=''){
                ...
                var nilai_incl_ppn = trimNilai*(1+ppnFactor/100)*1;
                if(ket!=''){
                    $('#desc_notes').val('Project: ' + projectName + '<br>- ' + ket + '<br>- ' + '('+ addCommas(Math.round(nilai_incl_ppn)) +') Incl.PPn').trigger('blur');
                }
                if( !$(nilai_entry).prop('disabled') ){
                    $(nilai_entry).val(trimNilai).trigger('blur');
                }
                else{
...
//                    $('#nilai_entry').val(totalCheckRpNonTax).trigger('keyup').trigger('mouseout');
                    console.log('$linkSelectWorkOrder');
                })
top.$('.modal-dialog').addClass('modal-xl').removeClass('modal-lg');
                </script>";
    }
    else {
