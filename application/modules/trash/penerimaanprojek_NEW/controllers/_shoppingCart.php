        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items5_new'])) {
                ...
                foreach ($_SESSION[$cCode]['items5_new'] as $iSpec) {
                    ...
                    $labelInfo = $sisa*1==0 && $progress_percent==100 ? "<div><label class='text-green text-bold'>selesai</label></div>" : ($progress_percent<100 && $sisa*1>0 ? "<div><label><b><r>belum di proses<br>oleh pelaksana ($progress_percent%)</r></b></label></div>" : ($progress_percent==100&&$progress_id==2 ? "<div><label><b><r>belum di QC ($progress_percent%)</r></b></label></div>" : ""));

                    $tmp["checklistTarget"] = "<input $disabledCheck $selected class='checkbox_tasklist' persen='$persen' harga='$sisa' pid='$ids' type='checkbox' id='checklist_$ids'>$labelInfo";
                    $items5[] = $tmp;
                }
            }
        }
...
//         arrPrintPink( json_encode($arrListLockerProjectSupplies));
//         arrPrintWebs( json_encode($arrListLockerProject));
        // arrPrintPink($daftarGudangWo);
        // arrPrintPink($arrTaskListProject);
//         arrPrintWebs("nilai_kontrak: " . $nilai_kontrak);
//         arrPrintWebs("nilaiProgressAll: " . $nilaiProgressAll);
//         arrPrintWebs("persenProgressAll: " . $persenProgressAll);
        // arrPrintWebs($main);
        // arrPrint($main[project_id]);
        // matiHere(__LINE__);
