...
//cekMerah($jenisTr);
        /** ------------------------------------------------
         * digunakan untuk mentrigger replacer label-label
         * ------------------------------------------------*/
        $pajakOption = isset($mainValues["pajakOption"]) ? $mainValues["pajakOption"] : "";
        $pph__tarif = isset($mainValues["pph__tarif"]) ? $mainValues["pph__tarif"] : "";
...
//                        arrPrint($mainElements);
            if (sizeof($mainElements) > 0) {
                foreach ($mainElements as $eKey => $eSpec) {
                    $elementStr = "";
                    if (isset($eSpec['label'])) {
                        $elementStr .= "<div class='panel-heading text-center text-uppercase' title='$eKey'>";
                        switch ($pajakOption) {
                            case "pph21":
                                if ($eKey == "fixedElements") {
                                    $elementStr .= "Otorisasi komisi";
                                }
                                else {
                                    $elementStr .= $eSpec['label'];
                                }
                            ...
                            case "pph23":
                            case "pph23_15":
                                if ($eKey == "customerDetails") {
                                    $elementStr .= "Detil vendor";
                                }
                                else {
                                    if($jenisTr=="1676"){
                                        $elementStr .= $eSpec['label'];
                                        ...
                                        $elementStr .= "otorisasi biaya konsultasi";
                                    }
                                }
...
//        matiHEre(__LINE__);
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                $contentStr .= "<div class='table-responsive' style='border:0px solid red;'>";
                $contentStr .= "<table class='table table-bordered table-condensed' style='width: 100%;'>";
                $contentStr .= "<tr bgcolor='#f5f5f5' class='text-uppercase'>";
                $contentStr .= "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>No.</th>";
                ...
                switch ($jenisTr){
                    ...
                    default:
                        $lebelReplacers = [
                            "pph21" => [
                                "produk_nama" => "dasar komisi",
                                "harga" => "besarnya komisi",
                                "pph__tarif" => "PPh 21 (%)",
                                "nilai_pph_original" => "PPh 21",
                                "nilai_kas_cn" => "jumlah diterima freelancer",
                            ],
                            "pph23" => [
                                "produk_nama" => "detil projek yang dikerjakan",
                                "harga" => "besarnya biaya",
                                "pph__tarif" => "PPh 23 (%)",
                        ...
                        $repLabels = $lebelReplacers[$pajakOption];
                        break;
                }
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    $label_r = isset($repLabels[$key]) ? $repLabels[$key] : $label;
                    $contentStr .= "<th class='text-muted' style='font-weight:bold;' title='$key'>";
                    $contentStr .= $label_r . "";
                    $contentStr .= "</th>";
                }
                $contentStr .= "</tr>";
                ...
                if (isset($sumRows) && sizeof($sumRows) > 0) {
                    ...
                    $replacerLabels = [
                        "pph21" => [
                            "harga" => "total komisi",
                            "nilai_pph_original" => "$pajakOption $pph__tarif%",
                            "nilai_kas_cn" => "jumlah diterima freelancer",
                        ],
                        "pph23" => [
                            "harga" => "total biaya",
                        ...
                        "pph23_15" => [
                            "harga" => "total biaya",
                            "nilai_pph_original" => "PPh23 $pph__tarif%",
                            "nilai_kas_cn" => "jumlah diterima perusahaan",
                        ],
                    ];

                    $repLabels = $replacerLabels[$pajakOption];
                    ...
                    foreach ($sumRows as $key => $label) {
                        ...
                        if (isset($mainValues[$key])) {
                            ...
                            if (sizeof($mainValues[$key]) > 0) {
                                //                                cekHere("$key " . $mainValues[$key]);
                                $contentStr .= "<tr line=" . __LINE__ . ">";
                                $contentStr .= "<td colspan='$colspan2' class='text-right text-uppercase' title='$key'>$label_r </td>";
                                $contentStr .= "<td class='text-right'>";
                                if (isset($mainValues[$key])) {
                                    $contentStr .= formatField($key, $mainValues[$key]);
                                }
...
//                $contentStr4 .= "<div class='panel no-margin'>"; // anakan table
                $contentStr4 .= "<h4 class='text-left'>$label_items2_sum</h4>";
                $contentStr4 .= "<table class='table table-condensed table-striped no-padding no-border'>";
                if (sizeof($itemLabels2_sum) && (is_array($itemLabels2_sum))) {
                    //region header table anakan
                    $contentStr4 .= "<tr>";
                    $contentStr4 .= "<th class='text-muted bg-grey-1 text-center'>";
                    $contentStr4 .= "No";
                    $contentStr4 .= "</th>";
                    foreach ($itemLabels2_sum as $key => $label) {
                        $contentStr4 .= "<th class='text-muted bg-grey-1 text-center text-capitalize'>";
                        $contentStr4 .= $label;
                        $contentStr4 .= "</th>";
                    }
//                    if (!$avoidRemove) {
//                        $contentStr4 .= "<th style='width:1%;' class='text-muted text-center'>";
//                        $contentStr4 .= "x";
//                        $contentStr4 .= "</th>";
//                    }
                    $contentStr4 .= "</tr>";
                ...
                foreach ($items2_sum as $iSpec) {
                    ...
                    $contentStr4 .= "<tr id='tr_" . $iSpec['id'] . "' bgcolor=$bgColor>";
                    $contentStr4 .= "<td width='5%'>";
                    $contentStr4 .= $no;
                    $contentStr4 .= ".</td>";
                    $colCtr = 0;
                    foreach ($itemLabels2_sum as $key => $label) {
                        ...
                        $contentStr4 .= "<td class='$cAlign'>";
                        $tabIndexNum = $colCtr . $no;

                        if (is_numeric($iSpec[$key])) {
                            // $contentStr4 .= "<input type=text autocomplete='off' readOnly id=$colID class='form-control text-right' style='color:$color;' value='" . $iSpec[$key] . "' >";
                            $contentStr4 .= formatField($key, $iSpec[$key]);
                            ...
                            // $contentStr4 .= "<input type=text autocomplete='off' readOnly id=$colID class='form-control' style='color:$color;' value='" . $iSpec[$key] . "' >";
                            $contentStr4 .= formatField($key, $iSpec[$key]);
                        }
...
//                    if (!$avoidRemove) {
//                        $contentStr4 .= "<td width='1%'>";
//                        $contentStr4 .= "<a class='text-red btn' title='remove this item' data-toggle='tooltip' data-placement='left'
...
//                    }
                ...
                //endregion
                $contentStr4 .= "<tr class='bg-grey-01 3'>";
                $contentStr4 .= "<td class='text-right $bottom_borderless valign-m text-uppercase'>-</td>";
                foreach ($itemLabels2_sum as $key => $label) {
                    $contentStr4 .= "<td class='right-borderlesss'>";
                    $contentStr4 .= formatField($key, $summaryItems2_sum[$key]);
                    $contentStr4 .= "</td>";
                }
                $contentStr4 .= "</tr>";

                $contentStr4 .= "</table>";
//                $contentStr4 .= "</div>"; // anakan table
...
//            $str .= "<button class='btn btn-primary btn-sm' onclick=\"$link_tombol\">Tambah Data Freelancer</button>";
...
//arrPrint($elementLabels);
        ...
        if (sizeof($fixedElementsReplace) > 0) {
            foreach ($fixedElementsReplace as $key => $val) {
                if ($val == false) {
                    $arrTags[$key] = "";
                }
            }
