...
//                arrPrintHitam($data_cabang_pos_0);
                ...
                $data_cabang_pos_ .= "<input $checked name='cabang' type='radio' 
                    ...
                    <label>" . $data_cabang_pos_0["label"] . "</label>";
                $data_cabang_pos_ .= "<br>";
            }
...
//        echo $data_cabang_pos_;
        ...
        $arrTags = array(
            ...
            "pilihan_main" => isset($visibility) ? $visibility : "",
            "pihakMainProcessor" => isset($pihakMainProcessor) ? $pihakMainProcessor : "",
            "date_max" => date("Y-m-d"),
        );
        if (sizeof($arrLinkMenuAdd) > 0) {
            $arrTags = $arrTags + $arrLinkMenuAdd;
...
//                echo "<div class='panel no-margin'>"; // anakan table
                echo "<h4 class='text-left'>$label_items2_sum</h4>";
                echo "<table class='table table-condensed table-striped no-padding no-border'>";
                if (sizeof($itemLabels2_sum) && (is_array($itemLabels2_sum))) {
                    //region header table anakan
                    echo "<tr>";
                    echo "<th class='text-muted bg-grey-1 text-center'>";
                    echo "No";
                    echo "</th>";
                    foreach ($itemLabels2_sum as $key => $label) {
                        echo "<th class='text-muted bg-grey-1 text-center text-capitalize'>";
                        echo $label;
                        echo "</th>";
                    }
//                    if (!$avoidRemove) {
...
//                    }
                ...
                $kurangStoks = array();
                $summaryItems2_sum = array();
                foreach ($items2_sum as $iSpec) {
                    $iID = $iSpec['id'];
                    $no++;
                    $bgColor = "transparent";
                    ...
                    foreach ($itemLabels2_sum as $key => $label) {
                        ...
                        if (is_numeric($iSpec[$key])) {
                            ...
                            echo formatField($key, $iSpec[$key]);
                            if(!isset($summaryItems2_sum[$key])){
                                $summaryItems2_sum[$key] = 0;
                            }
                            $summaryItems2_sum[$key] += $iSpec[$key];
                        }
                        else {
...
//                    }
                ...
                echo "<td class='text-right $bottom_borderless valign-m text-uppercase'>-</td>";
                foreach ($itemLabels2_sum as $key => $label) {
                    echo "<td class='right-borderlesss'>";
                    echo formatField($key, $summaryItems2_sum[$key]);
                    echo "</td>";
                }
                echo "</tr>";
