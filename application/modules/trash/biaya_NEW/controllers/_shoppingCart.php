...
class _shoppingCart extends Modul_Controller
{
    ...
    public function viewCart()
    {
        ...
        $editHandlerMethod2_sum = isset($this->configUi[$this->jenisTr]['editHandlerMethod2_sum']) ? $this->configUi[$this->jenisTr]['editHandlerMethod2_sum'] : "";
        ...
        $fieldSrcs2_sum = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc2_sum']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc2_sum'] : array();
        //----------------------------------------------
        $isSettlement = isset($this->configUi[$this->jenisTr]['isSettlement']) ? $this->configUi[$this->jenisTr]['isSettlement'] : false;
        //----------------------------------------------
        $itemLabels2_sum = $shoppingCartFields2_sum = isset($this->configUi[$this->jenisTr]['shoppingCartFields2_sum'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartFields2_sum'][$stepNumber] : array();
        //----------------------------------------------
...
//                $items2_sum_kurang = array();
                ...
                foreach ($_SESSION[$cCode]['items2_sum'] as $iSpec) {
                    ...
                    if (sizeof($fieldSrcs2_sum) > 0) {
                        foreach ($fieldSrcs2_sum as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                        }
                    }
//                    if (sizeof($itemNumLabels) > 0) {
...
//                    }
                    ...
                    if (isset($editHandlerMethod2_sum)) {
                        if (isset($iSpec['handler'])) {
                            $tmp["editTarget"] = base_url() . $iSpec['handler'] . "/" . $editHandlerMethod2_sum . "/" . $this->jenisTr . "?id=" . $id2_sum . "&newQty=";
                            $tmp["removeTarget"] = base_url() . $iSpec['handler'] . "/removeReference/" . $this->jenisTr . "?id=" . $id2_sum;
                        }
                        else {
                            $tmp["editTarget"] = "";
                            $tmp["removeTarget"] = "";
...
//        arrPrint($items2_sum);
//        arrPrintHitam($_SESSION[$cCode]['items2_sum']);
        //item3 sum
        if (isset($_SESSION[$cCode])) {
...
//arrPrintKuning($relElementConfigs);
        $currentValue = "";
...
//            "freelancerTambahBaris" => MODUL_PATH . $freelancerData["link_baris"] . "/" . $this->jenisTr,
            ...
            "itemLabels2_sum" => $itemLabels2_sum,
            "items2_sum" => $items2_sum,
            "label_items2_sum" => isset($this->configUi[$this->jenisTr]['pihakMainReferencelabel']) ? $this->configUi[$this->jenisTr]['pihakMainReferencelabel'] : "",
        );
