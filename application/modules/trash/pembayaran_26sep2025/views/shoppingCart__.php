...
//        arrPrint($items4);
        ...
        if (count($items4) > 0) {
            ...
            $listProduk .= "<div class='panel-body'>";
            $listProduk .= "<div id='overlay'><div id='text'>Loading Content...</div></div>";
            $listProduk .= "<div class='panel-headger'>RINCIAN PENERIMAAN PODUK</div>";
            $listProduk .= "<table class='table table-bordered'>";
            $listProduk .= "<tr>";
            $listProduk .= "<th>No</th>";
...
//            $ppn = $subtotal*0.11;
            ...
            $listProduk .= "</tr>";
            $listProduk .= "<tr>";
            $listProduk .= "<td colspan='$colspan' style='font-size: 16px;' class='text-right'>Total harga produk</td>";
            $listProduk .= "<td colspan='' style='font-size: 16px;padding-right: 40px;' class='text-bold text-meta text-right'>" . number_format($subtotal) . "</td>";
            $listProduk .= "</tr>";
//            $listProduk .= "<tr>";
...
//                cekMerah($fff);
                ...
                if (isset($shopingCartFakturParam["editableFields"][$fff])) {
                    ...
                    $value = "<input type='$inputType' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked onblur=\"eksekutor(this.$vals,this.name)\">";
                } else {
                    $value = formatField($fff, $shopingCartFakturItems[$fff]);
                }
                $faktur .= "<td>$value <span class='text-danger text-bold text-blink'>$labels </span></td>";
            }
            $faktur .= "</tr>";
...
//                    console.log(tipe_transaksi_sumber)
