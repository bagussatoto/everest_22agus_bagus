...
//            "jenisTr" => $this->jenisTr,
    ...
    // ---------------------------------------optimasi versi GPT
    /**
     * Entry point untuk memilih sumber pembayaran eksternal.
     */
    public function selectPaymentExternSrc_ai()
    {
        $this->load->model("MdlTransaksi");
        $targetJenis = $this->uri->segment(4);
        ...
        // Pastikan booking number ada
        $this->_ensureBookingNumber();

        // Ambil config dasar
        $paymentSources = $this->config->item("payment_source") ?: [];
        $readerDueDate  = isset($this->configUi[$targetJenis]['dueDateReader'])
            ? $this->configUi[$targetJenis]['dueDateReader'] : false;
        $pairDepositCfg = isset($this->configUi[$targetJenis]['pairDepositKonsumen'])
            ? $this->configUi[$targetJenis]['pairDepositKonsumen'] : [];

        // Ambil anti-source
        $antiSources = $this->_loadAntiSources();

        // Ambil transaksi yang belum lunas
        $tmpSrc = $this->_loadTransaksiSrc($targetJenis);

        // Ambil aging (jika perlu)
        list($dueEmployee, $premasterAging, $aginglabel2) =
            $readerDueDate ? $this->_loadAging($tmpSrc) : [[], [], []];

        // Ambil customer + deposit konsumen
        $srcCustomers = $this->_loadCustomers();
        list($pairDeposits, $pairMutasi) = $this->_loadPairDeposits($pairDepositCfg);

        // Build items reguler/marketplace
        list($items, $agingData)  = $this->_buildItems($tmpSrc, $dueEmployee, $premasterAging, $antiSources, $srcCustomers, $pairDeposits, $pairMutasi);
        list($items3, $agingData3)= $this->_buildItems3($tmpSrc, $dueEmployee, $premasterAging);

        // arrPrintHijau($agingData);
        // Siapkan data untuk view
        $data = [
            "mode"             => $this->uri->segment(3),
            "title"            => $this->configUi[$targetJenis]["label"],
            "subTitle"         => $this->_getSubTitle($paymentSources, $targetJenis),
            "items"            => $items,
            "srcLabel"         => "", // diisi dalam builder
            "itemLabelTambahans"=> ["a_sisa" => "return", "a_saldo" => "saldo"],
            "itemLabels"       => $this->_getItemLabels($targetJenis),
            "aginglabel2"      => $aginglabel2,
            "jenisTr"          => $this->jenisTr,
            "agingData"        => $agingData,
            "items3"           => $items3,
            "srcLabel3"        => "",
            "itemLabels3"      => $this->_getItemLabels3($targetJenis),
            "aginglabel3"      => $aginglabel2,
            "agingData3"       => $agingData3,
        ];
    ...
    private function _ensureBookingNumber()
    {
        $cCode = $this->cCode;
        if (!isset($_SESSION[$cCode]["main"]["bookingNumber"])) {
            $_SESSION[$cCode]["main"]["bookingNumber"] =
                date("YmdHis");
        }
    }

    private function _loadAntiSources()
    {
        $this->load->model("Mdls/MdlPaymentAntiSource");
        $pas = new MdlPaymentAntiSource();
        $this->db->where([
        ...
        $result = $pas->lookupAll()->result();

        $data = [];
        foreach ($result as $item) {
            $data[$item->extern_id] = [
                "a_extern_id"   => $item->extern_id,
                "a_extern_nama" => $item->extern_nama,
                "a_tagihan"     => $item->tagihan,
                "a_sisa"        => $item->sisa,
            ];
        }
        return $data;
    ...
    private function _loadTransaksiSrc($targetJenis)
    {
        $tr = new MdlTransaksi();
        $tr->setFilters([]);
        $tr->addFilter("transaksi_payment_source.cabang_id='" . $this->session->login['cabang_id'] . "'");
        $tr->addFilter("sisa>1000");
        return $tr->lookupPaymentSrcByJenis($targetJenis)->result();
    }

    private function _loadCustomers()
    {
        $this->load->model("Mdls/MdlCustomer");
        $cus = new MdlCustomer();
        return $cus->callSpecs();
    }

    private function _loadPairDeposits($pairDepositCfg)
    {
        // disingkat: logika fetchBalances & fetchMoves seperti aslinya
        return [[], []];
    }

    // private function _loadAging($tmpSrc)
    // {
    //     // disingkat: logika aging seperti aslinya
    //     return [[], [], []];
    // }

    private function _loadAging_v1($tmpSrc)
    {
        $dueEmployee   = [];
        $premasterAging = [];
        $aginglabel2   = [];
        ...
        $masterPeriode = periodeAging(); // ambil definisi range aging (0–30, 31–60, dll.)

        // header aging untuk view
        foreach ($masterPeriode as $agingID => $agingLabelData) {
            $aginglabel2[$agingID] = $agingLabelData["label"];
        }

        // mapping due date per customer
        $tempDataDues = [];
        foreach ($tmpSrcDue as $row) {
            $cid  = $row->customers_id;
            $trId = $row->transaksi_id;
            $tempDataDues[$cid][$trId] = [
                "due_date"    => $row->due_date,
                "aging_dtime" => $row->dtime,
                "transaksi_id"=> $trId,
            ];
        }
        ...
        $transaksiBelumLunas = [];
        foreach ($tmpSrc as $item) {
            $transaksiBelumLunas[$item->extern_id][] = $item->transaksi_id;
        }

        // mapping data yang belum lunas
        $dataTridBelumLunas = [];
        foreach ($transaksiBelumLunas as $cid => $arrTrid) {
            foreach ($arrTrid as $trid) {
                if (isset($tempDataDues[$cid][$trid])) {
                    $dataTridBelumLunas[$cid][$trid] = $tempDataDues[$cid][$trid];
                }
            }
        }
        ...
        // hitung aging per customer
        foreach ($dataTridBelumLunas as $cid => $trList) {
            $dueVal   = [];
            $dtimeVal = [];

            foreach ($trList as $trid => $spec) {
                $keyIndex = strtotime($spec['due_date']);
                $dueVal[] = $keyIndex;
                $dtimeVal[$keyIndex] = [
                    "due_date" => $spec['due_date'],
                    "aging"    => $spec['aging_dtime'],
                ];
            }
        ...
        // mapping ke master periode aging
        foreach ($dueEmployee as $cid => $cidData) {
            $aging_tmp = $cidData["aging"];
            if ($aging_tmp <= 120) {
                foreach ($masterPeriode as $periode => $periodeData) {
        ...
        return [$dueEmployee, $premasterAging, $aginglabel2];
    }
    ...
    private function _loadAging($tmpSrc)
    {
        ...
        // master periode aging & header (selalu bangun header supaya view tidak error)
        $masterPeriode = periodeAging();
        foreach ($masterPeriode as $agingID => $agingLabelData) {
            $aginglabel2[$agingID] = $agingLabelData["label"];
        }

        // jika tidak ada data due-date, kembalikan header kosong (cegah error)
        if (empty($tmpSrcDue)) {
            log_message('debug', '[_loadAging] no due-date rows found (lookupAllDueDate returned empty).');
            return array($dueEmployee, $premasterAging, $aginglabel2);
        }

        // mapping due-date per customer_id -> transaksi_id
        $tempDataDues = array();
        foreach ($tmpSrcDue as $row) {
            $cid  = $row->customers_id;
            $trId = $row->transaksi_id;
            $tempDataDues[$cid][$trId] = array(
                "due_date"    => $row->due_date,
                "aging_dtime" => $row->dtime,
                "transaksi_id"=> $trId,
            );
        ...
        // buat daftar transaksi belum lunas berdasarkan tmpSrc (cek extern_id & extern3_id)
        $transaksiBelumLunas = array();
        foreach ($tmpSrc as $item) {
            if (!empty($item->extern_id)) {
                $transaksiBelumLunas[$item->extern_id][] = $item->transaksi_id;
            }
            if (!empty($item->extern3_id)) {
                $transaksiBelumLunas[$item->extern3_id][] = $item->transaksi_id;
            }
        }

        // gabungkan data due-date hanya untuk transaksi yang belum lunas
        $dataTridBelumLunas = array();
        foreach ($transaksiBelumLunas as $cid => $arrTrid) {
            foreach ($arrTrid as $trid) {
                if (isset($tempDataDues[$cid][$trid])) {
        ...
        // jika kosong, tulis log dan return header saja
        if (empty($dataTridBelumLunas)) {
            log_message('debug', '[_loadAging] no matching due-date for transaksiBelumLunas. tmpSrc count=' . count($tmpSrc));
            return array($dueEmployee, $premasterAging, $aginglabel2);
        }

        $dtime_now = strtotime(date("Y-m-d"));

        // hitung due_date/aging per customer (atau extern id)
        foreach ($dataTridBelumLunas as $cid => $trList) {
            $dueVal = array();
            $dtimeVal = array();
            ...
            if (empty($dueVal)) continue;

            asort($dueVal);
            $key_index = reset($dueVal);
            $date_due  = $dtimeVal[$key_index]['due_date'];
            $aging_dt  = $dtimeVal[$key_index]['aging'];

            $umurday = $date_due ? umurDay($date_due) : 0;

            $dueEmployee[$cid] = array(
                "due_date"     => formatField_he_format("dtime", $date_due),
                "over_due"     => $umurday,
                "aging"        => (intval(umurDay($aging_dt)) > 0) ? intval(umurDay($aging_dt)) : "0",
                "classMarking" => $dtime_now > $key_index ? "bg-danger" : "",
            );
        }

        // Map ke master periode (premasterAging)
        foreach ($dueEmployee as $cid => $cidData) {
            $aging_tmp = $cidData["aging"];
            if ($aging_tmp <= 120) {
                foreach ($masterPeriode as $periode => $periodeData) {
        ...
        // debug minimal: daftar kunci yang siap dipakai builder
        log_message('debug', '[_loadAging] dueEmployee count=' . count($dueEmployee) . ' premasterAging keys=' . implode(',', array_keys($premasterAging)));

        return array($dueEmployee, $premasterAging, $aginglabel2);
    }


    // private function _buildItems($tmpSrc, $dueEmployee, $premasterAging, $antiSources, $srcCustomers, $pairDeposits, $pairMutasi)
    // {
    //     $items = [];
    //     $agingData = [];
    //     // disingkat: builder array items, mirip blok asli tapi lebih rapih
    //     return [$items, $agingData];
    // }
    ...
    private function _buildItems($tmpSrc, $dueEmployee, $premasterAging, $antiSources, $srcCustomers, $pairDeposits, $pairMutasi)
    {
        ...
        $tagihans = $terbayars = $diskons = $sisas = [];
        $tagihans_valas = $terbayars_valas = $sisas_valas = $diskons_valas = [];
        $qty_nota = [];
        ...
        foreach ($tmpSrc as $row) {
            ...
            // init jika belum ada
            if (!isset($tagihans[$id])) {
                $tagihans[$id] = $terbayars[$id] = $diskons[$id] = $sisas[$id] = 0;
                $tagihans_valas[$id] = $terbayars_valas[$id] = $sisas_valas[$id] = $diskons_valas[$id] = 0;
                $qty_nota[$id] = 0;
            }

            // assign classMarking untuk aging
            $classMarking = isset($dueEmployee[$id]) ? $dueEmployee[$id]["classMarking"] : "";

            // jika item baru, simpan dasar
            if (!isset($items[$id])) {
                $tmp = (array)$row;
                $tmp["link"]        = MODUL_PATH . get_class($this) . "/selectPaymentSrc/" . $this->jenisTr . "/" . $id;
                $tmp["due_date"]    = isset($dueEmployee[$id]['due_date']) ? $dueEmployee[$id]['due_date'] : "-";
                $tmp["aging"]       = isset($dueEmployee[$id]['aging']) ? $dueEmployee[$id]['aging'] : "-";
                $tmp["over_due"]    = isset($dueEmployee[$id]['over_due']) ? $dueEmployee[$id]['over_due'] : "-";
                $tmp["class_marking"] = $classMarking;
                $items[$id] = $tmp;
            }

            // akumulasi nilai
            $tagihans[$id]         += isset($row->tagihan) ? $row->tagihan : 0;
            $terbayars[$id]        += isset($row->terbayar) ? $row->terbayar : 0;
            $sisas[$id]            += isset($row->sisa) ? $row->sisa : 0;
            $diskons[$id]          += isset($row->diskon) ? $row->diskon : 0;
            $tagihans_valas[$id]   += isset($row->tagihan_valas) ? $row->tagihan_valas : 0;
            $terbayars_valas[$id]  += isset($row->terbayar_valas) ? $row->terbayar_valas : 0;
            $sisas_valas[$id]      += isset($row->sisa_valas) ? $row->sisa_valas : 0;
            $diskons_valas[$id]    += isset($row->diskon_valas) ? $row->diskon_valas : 0;
            $qty_nota[$id]         += 1;
        }

        // pasca proses: tambahkan field tambahan
        foreach ($items as $id => $iSpec) {
            $items[$id]['a_sisa']   = isset($antiSources[$id]['a_sisa']) ? $antiSources[$id]['a_sisa'] : 0;
            $items[$id]['a_saldo']  = $sisas[$id] - $items[$id]['a_sisa'];
            $items[$id]['tagihan']  = $tagihans[$id];
            $items[$id]['terbayar'] = $terbayars[$id];
            $items[$id]['diskon']   = $diskons[$id];
            $items[$id]['sisa']     = $sisas[$id];
            $items[$id]['tagihan_valas']  = $tagihans_valas[$id];
            $items[$id]['terbayar_valas'] = $terbayars_valas[$id];
            $items[$id]['diskon_valas']   = $diskons_valas[$id];
            $items[$id]['sisa_valas']     = $sisas_valas[$id];
            $items[$id]['qty_nota']       = $qty_nota[$id];
            $items[$id]['limit']          = isset($srcCustomers[$id]->kredit_limit) ? $srcCustomers[$id]->kredit_limit : "-";
            $items[$id]['tenor']          = isset($srcCustomers[$id]->due_days) ? $srcCustomers[$id]->due_days : "0";

            // aging periode
            if (isset($premasterAging[$id])) {
                $agingKey = $premasterAging[$id];
                $agingData[$id]["aging_periode"][$agingKey] = $sisas[$id];
            }
        }

        // deposit konsumen
        foreach ($pairDeposits as $kkey => $kkSpec) {
            foreach ($kkSpec as $cid => $cSpec) {
                if (isset($items[$cid])) {
                    $items[$cid]["extern_id"]   = $cSpec["id"];
                    $items[$cid]["extern_nama"] = $cSpec["nama"];
            ...
            if (isset($pairMutasi[$kkey])) {
                foreach ($pairMutasi[$kkey] as $cid => $cSpec) {
                    if (isset($items[$cid])) {
                        $items[$cid]["reference_oleh_id"]   = $cSpec["id"];
                        $items[$cid]["reference_oleh_nama"] = $cSpec["nama"];
                    }
    ...
    // private function _buildItems3($tmpSrc, $dueEmployee, $premasterAging)
    // {
    //     $items3 = [];
    //     $agingData3 = [];
    //     // builder untuk marketplace
    //     return [$items3, $agingData3];
    // }
    ...
    private function _buildItems3($tmpSrc, $dueEmployee, $premasterAging)
    {
        ...
        $tagihans3 = $terbayars3 = $diskons3 = $sisas3 = [];
        $tagihans_valas3 = $terbayars_valas3 = $sisas_valas3 = $diskons_valas3 = [];
        $qty_nota3 = [];

        foreach ($tmpSrc as $row) {
            // hanya proses jika ada extern3_id
            if (empty($row->extern3_id)) continue;

            $id = $row->extern3_id;

            if (!isset($tagihans3[$id])) {
                $tagihans3[$id] = $terbayars3[$id] = $diskons3[$id] = $sisas3[$id] = 0;
                $tagihans_valas3[$id] = $terbayars_valas3[$id] = $sisas_valas3[$id] = $diskons_valas3[$id] = 0;
                $qty_nota3[$id] = 0;
            }
            ...
            if (!isset($items3[$id])) {
                $tmp = (array)$row;
                $tmp["link"] = MODUL_PATH . get_class($this) . "/selectPaymentSrc/" . $this->jenisTr . "/" . $id . "/?mp=1";
                $tmp["due_date"]   = isset($dueEmployee[$id]['due_date']) ? $dueEmployee[$id]['due_date'] : "-";
                $tmp["aging"]      = isset($dueEmployee[$id]['aging']) ? $dueEmployee[$id]['aging'] : "-";
                $tmp["over_due"]   = isset($dueEmployee[$id]['over_due']) ? $dueEmployee[$id]['over_due'] : "-";
                $tmp["class_marking"] = $classMarking;
            ...
            // akumulasi
            $tagihans3[$id]       += isset($row->tagihan) ? $row->tagihan : 0;
            $terbayars3[$id]      += isset($row->terbayar) ? $row->terbayar : 0;
            $sisas3[$id]          += isset($row->sisa) ? $row->sisa : 0;
            $diskons3[$id]        += isset($row->diskon) ? $row->diskon : 0;
            $tagihans_valas3[$id] += isset($row->tagihan_valas) ? $row->tagihan_valas : 0;
            $terbayars_valas3[$id]+= isset($row->terbayar_valas) ? $row->terbayar_valas : 0;
            $sisas_valas3[$id]    += isset($row->sisa_valas) ? $row->sisa_valas : 0;
            $diskons_valas3[$id]  += isset($row->diskon_valas) ? $row->diskon_valas : 0;
            $qty_nota3[$id]       += 1;
        }

        foreach ($items3 as $id => $iSpec) {
            $items3[$id]['a_sisa']   = 0; // anti_sources3 belum dipakai di kode asli
            $items3[$id]['a_saldo']  = $sisas3[$id] - $items3[$id]['a_sisa'];
            $items3[$id]['tagihan']  = $tagihans3[$id];
            $items3[$id]['terbayar'] = $terbayars3[$id];
            $items3[$id]['diskon']   = $diskons3[$id];
            $items3[$id]['sisa']     = $sisas3[$id];
            $items3[$id]['tagihan_valas']  = $tagihans_valas3[$id];
            $items3[$id]['terbayar_valas'] = $terbayars_valas3[$id];
            $items3[$id]['diskon_valas']   = $diskons_valas3[$id];
            $items3[$id]['sisa_valas']     = $sisas_valas3[$id];
            $items3[$id]['qty_nota']       = $qty_nota3[$id];

            if (isset($premasterAging[$id])) {
                $agingKey = $premasterAging[$id];
                $agingData3[$id]["aging_periode"][$agingKey] = $sisas3[$id];
            }
        }
    ...
    private function _getSubTitle($paymentSources, $targetJenis)
    {
        return isset($paymentSources[$targetJenis]['externSrc']['extLabel'])
            ? "select " . $paymentSources[$targetJenis]['externSrc']['extLabel'] . "(s) listed below"
            : "";
    }

    private function _getItemLabels($targetJenis)
    {
        $def = [
        ...
        return isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFields"])
            ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFields"]
            : $def;
    }

    private function _getItemLabels3($targetJenis)
    {
        return isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFieldsMarketplace"])
            ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFieldsMarketplace"]
            : [];
    }
    // ---------------------------------------
...
//                matiHEre();
                ...
                $tmp['so_id'] = $row->extern2_id;
                $tmp['so_num'] = $row->extern2_nama;
                $tmp['id_master_top'] = $row->id_master;
                $tmp['targetJenis'] = $targetJenis;
                switch ($jenisMaster) {
                    case "5822":
                        if ($marketplace == 1) {
                            $tmp['dataJenis'] = "marketplace";
                        }
                        else {
                            $tmp['dataJenis'] = "reguler";
                        }
                        $tmp['pj'] = 0;

                        $idsHis = ($row->ids_his != null) ? blobDecode($row->ids_his) : array();
                        $tmp['reference_so_id'] = isset($idsHis[2]["trID"]) ? $idsHis[2]["trID"] : 0;
                        $tmp['reference_so_nomer'] = isset($idsHis[2]["nomer"]) ? $idsHis[2]["nomer"] : 0;
                        break;
                    case "7499":
                        $tmp['dataJenis'] = "project";
                        $tmp['pj'] = 1;


                        $tmp['reference_so_id'] = $row->id_top;
                        $tmp['reference_so_nomer'] = $row->nomer_top;
                        break;
                }
                    ...
                    if ($this->session->login['cabang_id'] > 0) {
                        ...
                        foreach ($elementMutasiTmp as $el_nama => $el_spec) {
                            ...
                            if (in_array($targetJenis, $allowed)) {
                                ...
                                $buttonMutasi .= "
                                <a href='javascript:void(0)' onclick=\"$modalDialog\" ttarget='_blank'>
                                    <span class='btn btn-info'>$label: $nilai_saldo_f</span>
                                </a>";
