...
//        $this->jenisTr = $this->uri->segment(4);
        ...
        $projectValidate = isset($this->configUi[$this->jenisTr]['projectValidate']) ? $this->configUi[$this->jenisTr]['projectValidate'] : array();
        ...
        // cek project id pada items, bila beda dihentikan dulu.
        if (sizeof($projectValidate) > 0) {
            if (isset($projectValidate["enabled"]) && ($projectValidate["enabled"] == true)) {
                $keyCek = $projectValidate["keyCek"];
                $keyMode = $projectValidate["mode"];
                $keyLabel = $projectValidate["label"];
                if (sizeof($_SESSION[$cCode]['items']) && (in_array($_SESSION[$cCode]["main"]["mode"], $keyMode))) {
                    foreach ($_SESSION[$cCode]['items'] as $ixx => $ixxSpec) {
                        if ($ixxSpec[$keyCek] != $initMain[$keyCek]) {
                            mati_disini($keyLabel . " code: " . __LINE__);
                        }
                    }
                }
            }
        ...
        if ($this->jenisTr == "749") {
            //cek sebelum lanjut apakah sudah ada sesi
            if (!isset($_SESSION[$cCode]["main"]["mode"])) {
                $_SESSION[$cCode]["main"]["mode"] = $_GET["mode"];
            }
            if ($_SESSION[$cCode]["main"]["mode"] == $_GET["mode"]) {

            }
            else {
//matiHEre("mode aktif:: ".$_SESSION[$cCode]["main"]["mode"]);
            }
        ...
        if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
            $tmp = array(
                ...
                "src_id" => isset($_GET['id_master_top']) ? $_GET['id_master_top'] : 0,
            );
...
//                    cekHere("$relativeComNameDetails[$key] " . $relativeComNameDetails[$key]);
        ...
        $_SESSION[$cCode]['main']['refPymSrcIDs'][$_GET['id_master_top']] = $_GET['id_master_top'];
        ...
        switch ($_GET['state']) {
            ...
            case "false":
                ...
                if (isset($_SESSION[$cCode]['main']['refPymSrcIDs'][$_GET['id_master_top']])) {
                    $_SESSION[$cCode]['main']['refPymSrcIDs'][$_GET['id_master_top']] = null;
                    unset($_SESSION[$cCode]['main']['refPymSrcIDs'][$_GET['id_master_top']]);
                }
                if (sizeof($_SESSION[$cCode]['items']) > 0) {
...
//                echo "top.$('#switch_pajak').style.display='none';";
            ...
            if (sizeof($showDetailUangMuka) > 0) {
                if (isset($showDetailUangMuka[$_SESSION[$cCode]['main']['mode']]["enabled"]) && ($showDetailUangMuka[$_SESSION[$cCode]['main']['mode']]["enabled"] == true)) {
                    $mdl = $showDetailUangMuka[$_SESSION[$cCode]['main']['mode']]['mdlName'];
                    $mdlFilter = $showDetailUangMuka[$_SESSION[$cCode]['main']['mode']]['mdlFilter'];
                    $gateTarget = $showDetailUangMuka[$_SESSION[$cCode]['main']['mode']]['gateTarget'];
                    $this->load->model("Mdls/$mdl");
                    $this->load->model("MdlTransaksi");
                    $mm = New $mdl();
                    makeFilter($mdlFilter, $_SESSION[$cCode]['main'], $mm);
                    ...
                    if (sizeof($mmTmp) > 0) {
                        foreach ($mmTmp as $ii => $mmSpec) {
                            $mmSpec->transaksi_id_pymsrc = $mmSpec->transaksi_id;
                            $mmSpec->transaksi_nomer_pymsrc = $mmSpec->nomer;
                            $mtr_id = $mmSpec->transaksi_id;
                            $_SESSION[$cCode][$gateTarget][$ii] = (array)$mmSpec;
                            $mtr = New MdlTransaksi();
                            $mtr->setFilters(array());
                            $mtrTmp = $mtr->lookupDetailTransaksiAll($mtr_id);
                            $isi = array();
                            if (sizeof($mtrTmp[$mtr_id]) > 0) {
                                foreach ($mtrTmp[$mtr_id] as $xx => $mtrSpec) {
                                    $isi[$xx] = array(
                                        "trid" => $mtrSpec->transaksi_id,
                                        "id" => $mtrSpec->produk_id,
                                        "nama" => $mtrSpec->produk_nama,
                                        "nilai" => $mtrSpec->produk_ord_hrg,
                                    );
                                }
                            }
                            $_SESSION[$cCode][$gateTarget][$ii]["isi"] = $isi;
                        }
                    }
                }
            }
