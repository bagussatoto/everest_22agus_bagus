...
//            unset($_SESSION[$cCode]["main"]["bookingNumber"]);
            ...
            $iterator_subpreprocessor = array();
            $iterator_mainpreprocessor = array();
            $iterator_subcomponent = array();
            $iterator_maincomponent = array();
            $iterator_subpostprocessor = array();
            $iterator_mainpostprocessor = array();
            switch ($this->jenisTr) {
                case "749":
                    switch ($_SESSION[$cCode]["main"]["mode"]) {
                        case "uangmuka":
                            $iterator_subpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessorProjectUangMuka'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preProcessorProjectUangMuka'][$jenisTrTarget]['detail'] : array();
                            $iterator_mainpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessorProjectUangMuka'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preProcessorProjectUangMuka'][$jenisTrTarget]['master'] : array();
                            $iterator_subcomponent = isset($this->configCore[$this->jenisTr]['componentsProjectUangMuka'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['componentsProjectUangMuka'][$jenisTrTarget]['detail'] : array();
                            $revertedTarget = "";
                            $iterator_maincomponent = isset($this->configCore[$this->jenisTr]['componentsProjectUangMuka'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['componentsProjectUangMuka'][$jenisTrTarget]['master'] : array();
                            $iterator_subpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessorProjectUangMuka'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessorProjectUangMuka'][$jenisTrTarget]['detail'] : array();
                            cekHitam("post procc pakai config core");
                            $iterator_mainpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessorProjectUangMuka'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['postProcessorProjectUangMuka'][$jenisTrTarget]['master'] : array();

                            $jurnalIndex = array(
                                "master" => $iterator_maincomponent,
                                "detail" => $iterator_subcomponent,
                            );
                            $jurnalPostProc = array(
                                "master" => $iterator_mainpostprocessor,
                                "detail" => $iterator_subpostprocessor,
                            );
                            $jurnalPreProc = array(
                                "master" => $iterator_mainpreprocessor,
                                "detail" => $iterator_subpreprocessor,
                            );

                            break;
                        case "project":
                            $iterator_subpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessorProjectInvoice'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preProcessorProjectInvoice'][$jenisTrTarget]['detail'] : array();
                            $iterator_mainpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessorProjectInvoice'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preProcessorProjectInvoice'][$jenisTrTarget]['master'] : array();
                            $iterator_subcomponent = isset($this->configCore[$this->jenisTr]['componentsProjectInvoice'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['componentsProjectInvoice'][$jenisTrTarget]['detail'] : array();
                            $revertedTarget = "";
                            $iterator_maincomponent = isset($this->configCore[$this->jenisTr]['componentsProjectInvoice'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['componentsProjectInvoice'][$jenisTrTarget]['master'] : array();
                            $iterator_subpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessorProjectInvoice'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessorProjectInvoice'][$jenisTrTarget]['detail'] : array();
                            cekHitam("post procc pakai config core");
                            $iterator_mainpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessorProjectInvoice'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['postProcessorProjectInvoice'][$jenisTrTarget]['master'] : array();
                        ...
                        case "reguler":
                            ...
                            if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                                $iterator_subpreprocessor = isset($_SESSION[$cCode]['revert']['preProc']['detail']) ? $_SESSION[$cCode]['revert']['preProc']['detail'] : array();
                                cekMerah(":: iterator preprocc dari gerbang revert ::");
//                    arrPrintWebs($iterator_subpreprocessor);
                                $iterator_mainpreprocessor = isset($_SESSION[$cCode]['revert']['preProc']['master']) ? $_SESSION[$cCode]['revert']['preProc']['master'] : array();
                                $iterator_subcomponent = isset($_SESSION[$cCode]['revert']['jurnal']['detail']) ? $_SESSION[$cCode]['revert']['jurnal']['detail'] : array();
                                $revertedTarget = $_SESSION[$cCode]['main']['pihakExternID'];
                                $iterator_maincomponent = isset($_SESSION[$cCode]['revert']['jurnal']['master']) ? $_SESSION[$cCode]['revert']['jurnal']['master'] : array();
                                $iterator_subpostprocessor = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['detail'] : array();
                                cekHitam("post procc pakai revert");
                                $iterator_mainpostprocessor = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['master'] : array();

                            }
                            else {
                                $iterator_subpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['detail'] : array();
                                $iterator_mainpreprocessor = isset($this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['master'] : array();
                                $iterator_subcomponent = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail'] : array();
                                $revertedTarget = "";
                                $iterator_maincomponent = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master'] : array();
                                $iterator_subpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail'] : array();
                                cekHitam("post procc pakai config core");
                                $iterator_mainpostprocessor = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['master'] : array();
                            }
                            ...
                            break;
                        default:
                            $msg = "Konfigurasi Transaksi Penerimaan AR tidak dikenali. Silahkan hubungi admin. code: " . __LINE__;
                            mati_disini($msg);
                            break;
                    }
                    break;
...
//                    arrPrintWebs($iterator_subpreprocessor);
            ...
            arrPrintCyan($iterator_maincomponent);
            arrPrintHitam($iterator_subcomponent);
...
//            $buildTablesDetail = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail'] : array();
            $buildTablesMaster = $iterator_maincomponent;
            $buildTablesDetail = $iterator_subcomponent;
            ...
            if (sizeof($iterator_subpreprocessor) > 0) {
                ...
                if (sizeof($iterator_subpreprocessor) > 0) {
                    foreach ($iterator_subpreprocessor as $cCtr => $tComSpec) {
            ...
            if (sizeof($iterator_mainpreprocessor) > 0) {
                $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields']) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'] : array();
                if (sizeof($iterator_mainpreprocessor) > 0) {
                    foreach ($iterator_mainpreprocessor as $cCtr => $tComSpec) {
...
//                        "contents_intext" => isset($aSpec['contents_intext']) ? $aSpec['contents_intext'] : "",
            ...
            $componentConfig['detail'] = $iterator_subcomponent;
            if ($pakai_ini_nocli == 1) {
                if (sizeof($iterator_subcomponent) > 0) {
                    foreach ($iterator_subcomponent as $cCtr => $tComSpec) {
                        $tmpOutParams[$cCtr] = array();
                        $gg = 0;
                        $srcGateName = $tComSpec['srcGateName'];
            ...
            //==filter nilai, jika NOL tidak dikirim, sesuai config==
            $compValidators = ($this->config->item('transaksi_value_required_components') != null) ? $this->config->item('transaksi_value_required_components') : array();
            if (sizeof($iterator_maincomponent) > 0) {
                $componentConfig['master'] = $iterator_maincomponent;
                $cCtr = 0;
                foreach ($iterator_maincomponent as $cCtr => $tComSpec) {
                    $cCtr++;
                    $comName = $tComSpec['comName'];
                    if (substr($comName, 0, 1) == "{") {
                        $comName = trim($comName, "{");
            ...
            if (sizeof($iterator_subpostprocessor) > 0) {
                foreach ($iterator_subpostprocessor as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
            ...
            if (sizeof($iterator_mainpostprocessor) > 0) {
                foreach ($iterator_mainpostprocessor as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
...
//            mati_disini(__LINE__ . " SETOP DULU...");
                        ...
                        if ($paymentMethod == "update") {
                            ...
                            if (sizeof($tmpData) > 0) {
                                ...
                                $tr->writePaymentSrc($insertID, array(
                                    ...
                                    "project_id" => makeValue($externSrc['project_id'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], "0"),
                                    "project_nama" => makeValue($externSrc['project_nama'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], "0"),
                                ));
                            }
            ...
            //            cekHitam($this->db->last_query());
            arrPrintHitam($baseRegistries["jurnal_index"]);
...
//            $componentConfig['master'] = array();
            ...
            if (strlen($connector) > 0) {
                ...
                if (isset($this->configUi[$this->jenisTr]['connectoValidate'][$stepNum])) {
                    ...
                    $preVal = isset($_SESSION[$cCode]['main'][$validateValueConnector]) ? $_SESSION[$cCode]['main'][$validateValueConnector] : 0;
...
//mati_disini(__LINE__);
                    ...
                    if (isset($_SESSION[$cCode]['tableIn_master']) && sizeof($_SESSION[$cCode]['tableIn_master']) > 0) {
                        ...
                        $insertID = $tr->writeMainEntries($_SESSION[$cCode]['tableIn_master']);
                        cekUngu($this->db->last_query());
                        cekUngu("[insertID: $insertID] [insertNum: $insertNum]");
...
//                                "contents_intext" => isset($aSpec['contents_intext']) ? $aSpec['contents_intext'] : "",
                ...
                cekKuning("TIDAK ADA connecting");
            }
            //endregion
