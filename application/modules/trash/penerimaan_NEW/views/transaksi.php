...
//                $jenis_src = ($val["jenis_master"] == "7499") ? "project" : "reguler";
                $jenis_src = isset($val["dataJenis"]) ? $val["dataJenis"] : "reguler";
                $jenis_pj = isset($val["pj"]) ? $val["pj"] : 0;
...
//                $strContent .= "<tr line=" . __LINE__ . " name='popovers' idso='$soID' noso='$soNum' linkso='$linkMovePayment' exso='$extern_id' ajax=\"false\" id=\"popover_$soID\" oondblclick=\"openPopover('$soID','$soNum','$linkMovePayment','$extern_id');\"  >";
                ...
                if (sizeof($itemLabels) > 0) {
                    ...
                    if (($pembayaranBerurutan == true) && ($lanjutBerurutan == true)) {
                        $strContent .= "<input line='".__LINE__."' urut='$nob' class='chRadio paymentSrc $urut_class' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc $disabledHoldLocker 
                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "' 
                        ooonclick=\"document.getElementById('result').src='" . MODUL_PATH . "$selectProcessor/$jenisTr" . "?$qstr&mp=$marketplace&pj=$jenis_pj&state='+this.checked;\"
                        oonclick=\"$('#result').load(encodeURI('" . base_url() . $this->modul . "/$selectProcessor/$jenisTr" . "?$qstr&mp=$marketplace&pj=$jenis_pj&state='+this.checked))\"
                        qstr=\"$qstr&mp=$marketplace&pj=$jenis_pj\"
                        >";
                    }
                    else {
//                        $strContent .= "<input class='chRadio paymentSrc' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc $disabledHoldLocker
//                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "'
//                        onclick=\"document.getElementById('result').src='" . MODUL_PATH . "$selectProcessor/$jenisTr" . "?$qstr&mp=$marketplace&state='+this.checked;\"
//                        >";
                        $strContent .= "<input  class='chRadio paymentSrc' 
                        type='checkbox'
                        value='" . $val['transaksi_id'] . "' 
                        id='opt" . $val['transaksi_id'] . "'
                        data-conten='" . base_url() . $this->modul . "/$selectProcessor/$jenisTr?$qstr&mp=$marketplace&pj=$jenis_pj" . "'
                        data-jenis='" . $jenis_src . "'
                        >";
                    }
...
//top.$('.popover.fade.bottom.in').remove();
            ...
            //untuk menampilkan swal

            $strContent .= "<script>
                $(document).off('change', '.chRadio.paymentSrc').on('change', '.chRadio.paymentSrc', function (ee) {
                    let conten = $(this).data('conten');
                    let jenis  = $(this).data('jenis');
                    let target = document.getElementById('result'); // iframe target
                    let state  = this.checked; // <── aman
                    let radio  = this; // simpan referensi radio-nya
                
                    console.log('state: ', state);
                    console.log('jenis: ', jenis);
                
                    if (state && jenis === 'project') {
                        swal({
                            title: 'Pilih jenis transaksi',
                            type: 'warning',
                            html:
                                '<div class=\"header\">Silahkan Pilih Salah satu</div>' +
                                '<button id=\"invoice\" class=\"btn btn-sm btn-success\" style=\"margin:5px;\">Invoice</button>' +
                                '<button id=\"uangmuka\" class=\"btn btn-sm btn-primary\" style=\"margin:5px;\">Uangmuka</button>' +
                                '<button id=\"batal\" class=\"btn btn-sm btn-danger\" style=\"margin:5px;\"><i class=\"fa fa-times\"></i> Batal</button>',
                            showConfirmButton: false,
                            showCancelButton: false
                        }).then(() => {});
                
                        // pakai satu kali listener aja (one-time)
                        $(document).one('click', '#invoice', function () {
                            swal.close();
                            target.src = conten + '&state=' + state + '&mode=' + jenis;
                            console.log('pilih invoice');
                        });
                
                        $(document).one('click', '#uangmuka', function () {
                            swal.close();
                            target.src = conten + '&state=' + state + '&mode=uangmuka';
                            console.log('pilih uang muka');
                        });
                
                        $(document).one('click', '#batal', function () {
                            swal.close();
                            radio.checked = false; // uncheck radio
                            target.src = conten + '&state=false' + '&mode=' + jenis;
                            console.log('batal pilih');
                        });
                
                    } 
                    else if (state && jenis === 'reguler') {
                        target.src = conten + '&state=' + state + '&mode=' + jenis;
                    } 
                    else if (state && jenis === 'marketplace') {
                        target.src = conten + '&state=' + state + '&mode=' + jenis;
                    } 
                    else {
                        target.src = conten + '&state=' + state + '&mode=' + jenis;
...
//        $p->render();
    ...
    case "selectPaymentExternSrc":
        ...
        if (sizeof($items) > 0) {
            ...
            if (sizeof($aginglabel2) > 0) {
                foreach ($aginglabel2 + $itemLabelTambahans as $keys3 => $key3Label) {
                    if (isset($total[$keys3])) {
