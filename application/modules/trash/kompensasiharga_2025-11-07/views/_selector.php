            ...
            if (is_array($items) && (sizeof($items) > 0)) {
                ...
                foreach ($items as $iSpec) {
                    if (strlen($iSpec['nama']) > 0) {
                        ...
                        if (isset($iSpec['transaksi_id'])) {
                            $transaksi_idValue = $iSpec['transaksi_id'];
                            $defVal_trID = "&transaksi_id=$transaksi_idValue";
                        }
                        else {
                            $defVal_trID = "";
                        }
                        if (isset($iSpec['extern_id'])) {
                            $extern_idValue = $iSpec['extern_id'];
                            $defVal_extID = "&extern_id=$extern_idValue";
                        }
                        else {
                            $defVal_extID = "";
                        }
                        ...
                        if (isset($socketURL[$iSpec['id']]) && strlen($socketURL[$iSpec['id']]) > 3) {
                            ...
                            switch ($cCode) {
                                ...
                                case "_TR_461":
                                    ...
                                    break;
                                default:
                                    $strContent .= "<a href='javascript:void(0);' onclick=\"document.getElementById('result').src='" . $iSpec['target'] . "?selector&id=" . $iSpec['id'] . "$defVal$defVal_trID$defVal_extID';$mb_js\">";
                                    break;
                            }
                        }
