...
//        $selectorModel = isset($_SESSION[$cCode]['main']['pihakMdlName']) ? $_SESSION[$cCode]['main']['pihakMdlName'] : isset($this->configUi[$this->jenisTr]['pihakMdlName']) ? $this->configUi[$this->jenisTr]['pihakMdlName'] : $this->configUi[$this->jenisTr]['selectorModel'];
//        $selectorSrcModel = isset($_SESSION[$cCode]['main']['pihakMdlNameSrc']) ? $_SESSION[$cCode]['main']['pihakMdlNameSrc'] : isset($this->configUi[$this->jenisTr]['pihakMdlNameSrc']) ? $this->configUi[$this->jenisTr]['pihakMdlNameSrc'] : $this->configUi[$this->jenisTr]['selectorSrcModel'];

        //-------------------------
        if (isset($_SESSION[$cCode]['main']['pihakMdlName'])) {
            $selectorModel = $_SESSION[$cCode]['main']['pihakMdlName'];
        } else {
            if (isset($this->configUi[$this->jenisTr]['pihakMdlName'])) {
                $selectorModel = $_SESSION[$cCode]['main']['pihakMdlName'];
            } else {
        ...
        //-------------------------
        if (isset($_SESSION[$cCode]['main']['pihakMdlNameSrc'])) {
            $selectorSrcModel = $_SESSION[$cCode]['main']['pihakMdlNameSrc'];
        } else {
            if (isset($this->configUi[$this->jenisTr]['pihakMdlNameSrc'])) {
                $selectorSrcModel = $_SESSION[$cCode]['main']['pihakMdlNameSrc'];
            } else {
...
//        cekHere("[$selectorModel] [$selectorSrcModel]");
...
//cekHere("[$selectorModel] [$selectorSrcModel]");
        ...
        $editableFieldstr = isset($this->configUi[$this->jenisTr]['editableFieldstr'][1]) ? $this->configUi[$this->jenisTr]['editableFieldstr'][1] : array();
...
//         matiHere(__LINE__ . " " .__METHOD__);
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                if ($_SESSION[$cCode]["main"]["pihakMainName"] == "diskon_lain") {
                    if (count($_SESSION[$cCode]['items']) > 0) {
                        if(isset($_GET['idx'])){
                            $id_x =$_GET['idx'];
                        }
                        else{
                            $id_x = count($_SESSION[$cCode]['items']) +1;
...
//                matiHEre(__LINE__."::".$id_x);
                if (!isset($_SESSION[$cCode]['items']) || !array_key_exists($id_x, $_SESSION[$cCode]['items'])) {
                    $tmp = array(
                        "handler" => $this->modul . "/" . $this->uri->segment(2),
                        "id" => $id,
                        "id_x"=>$id_x,//relative id
                        "jml" => $tmpJml,
                        "harga" => 0,
                        "subtotal" => 0,
...
//                    cekBiru("ada id $id tapi tidak ada cCode items");
                    ...
                    if (isset($_GET['newQty'])) {
                        $_SESSION[$cCode]['items'][$id_x]['jml'] = $_GET['newQty'];
                        $_SESSION[$cCode]['items'][$id_x]['subtotal'] = ($_SESSION[$cCode]['items'][$id_x]['jml'] * ($_SESSION[$cCode]['items'][$id_x]['harga'] + $_SESSION[$cCode]['items'][$id_x]['ppn']));
                    } else {

//                        $editableFields

                        $_SESSION[$cCode]['items'][$id_x]['jml'] += $jml;
                        $_SESSION[$cCode]['items'][$id_x]['subtotal'] = ($_SESSION[$cCode]['items'][$id_x]['jml'] * ($_SESSION[$cCode]['items'][$id_x]['harga'] + (isset($_SESSION[$cCode]['items'][$id_x]['ppn']) ? $_SESSION[$cCode]['items'][$id_x]['ppn'] : 0)));
                    }
                    ...
                    if (isset($arrDiscount[$id]) && sizeof($arrDiscount[$id]) > 0) {
                        foreach ($arrDiscount[$id] as $dKey => $dVal) {
                            if (!isset($_SESSION[$cCode]['items'][$id_x]['discount_' . $dKey])) {
                                $_SESSION[$cCode]['items'][$id_x]['discount_' . $dKey] = 0;
                            }
                            $_SESSION[$cCode]['items'][$id_x]['discount_' . $dKey] = $dVal;
                        }
                    }

//                    arrPRint($editableFieldstr);
                    foreach ($editableFieldstr as $keyx => $labelx) {
//                        cekMerah($labelx);
                        if (isset($_GET[$labelx]) && strlen($_GET[$labelx]) > 0) {
                            $newValue = $_GET[$labelx];
                            $tmp[$keyx] = $newValue;
                            $_SESSION[$cCode]['items'][$id_x][$labelx] = $newValue;
                                echo "replacing value for $labelx with " . $newValue . "<br>";
                        }
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        ...
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && strlen($_GET[$key]) > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]['items'][$id_x][$key] = $newValue;
//                                echo "replacing value for $key with " . $newValue . "<br>";
                            }
                        ...
                        if ($subAmountConfig != null) {
                            $tmp['subtotal'] = makeValue($subAmountConfig, $_SESSION[$cCode]['items'][$id_x], $_SESSION[$cCode]['items'][$id_x], 0);
                        } else {
                            $tmp['subtotal'] = 0;
                        }
                        $_SESSION[$cCode]['items'][$id_x]['subtotal'] = $tmp['subtotal'];
                    }
...
//        }
    ...
    public function remove()
    {
        ...
        if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
            ...
            if (isset($_SESSION[$cCode]['items'][$idx])) {
        ...
        //        die();
        if (isset($_SESSION[$cCode]['items'][$idx])) {
            $_SESSION[$cCode]['items'][$idx] = null;
            unset($_SESSION[$cCode]['items'][$idx]);
            $_SESSION[$cCode]['items'][$idx] = null;
            unset($_SESSION[$cCode]['items'][$idx]);
            $_SESSION[$cCode]['out_detail'][$idx] = null;
            unset($_SESSION[$cCode]['out_detail'][$idx]);
            $_SESSION[$cCode]['out_detail2'][$idx] = null;
            unset($_SESSION[$cCode]['out_detail2'][$idx]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail_values'][$idx])) {
            $_SESSION[$cCode]['tableIn_detail_values'][$idx] = null;
            unset($_SESSION[$cCode]['tableIn_detail_values'][$idx]);
        }
    ...
    public function multiSelectBi()
    {
        ...
        // matiHEre(__LINE__ ." | ". my_toko_email());
        $id = 0;
        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        ...
        $vg->setConfigValuesJenis($this->configValuesJenis);
        $vg->setPpnFactor($ppnFactor);
//        $initMasterValues = $vg->initMasterValue();
        // arrPrint($initMasterValues, true);
        // matiHEre(__LINE__ ." | ". my_toko_email());
//        $vg->buildValue($this->jenisTr, $ids, $initMasterValues, $this->modul);
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
...
//            );
    ...
    public function select_diskon_lain()
    {
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();
