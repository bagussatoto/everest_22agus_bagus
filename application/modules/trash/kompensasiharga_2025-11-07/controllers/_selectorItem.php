...
//        cekKuning($items);
    ...
    public function selectItem_2()
    {
        ...
        $preLocker = isset($this->configUi[$jenisTr]['validLocker']) ? $this->configUi[$jenisTr]['validLocker'] : false;
        $selectorOrderBy = isset($this->configUi[$jenisTr]['selectorOrderBy']) ? $this->configUi[$jenisTr]['selectorOrderBy'] : NULL;
        $selectorViewedFieldsAkumulasi = isset($this->configUi[$jenisTr]['selectorViewedFieldsAkumulasi']) ? $this->configUi[$jenisTr]['selectorViewedFieldsAkumulasi'] : array();
        $items = array();
...
//                    arrPrintPink($modelFilter);
            ...
            foreach ($tmpO as $row) {
                $extern_id = isset($row->extern_id) && strlen($row->extern_id) > 0 ? $row->extern_id : 0;
                $satuan = isset($row->satuan) && strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                $pID = isset($row->produk_id) ? $row->produk_id : $row->id;
                /* ------------------------------------------------------
                 * ngelokup dalam foreacht harap dipertimbangkan ni loadtimenya
                ...
                if (sizeof($selectorFields) > 0) {
                    ...
                    foreach ($selectorFields as $f) {
                        ...
                        $color = isset($colors[$nCtr]) ? $colors[$nCtr] : "#000000";
                        if ($f != 'kode' && is_numeric($row->$f)) {
                            if (in_array($f, $selectorViewedFieldsAkumulasi)) {
//                                cekHitam("hahaah");
                                foreach ($selectorViewedFieldsAkumulasi as $vv) {
                                    if (!isset($nilai[$extern_id][$vv])) {
                                        $nilai[$extern_id][$vv] = 0;
                                    }
                                    $nilai[$extern_id][$vv] += $row->$vv;
                                }
                                $tmp['label'] .= "<span style='$fSize ;margin:0px 2px 0px 2px;color:$color;' class='no-padding no-border $align'>" . number_format($nilai[$extern_id][$f]) . "</span>";
                            }
                            else {
                                $tmp['label'] .= "<span style='$fSize ;margin:0px 2px 0px 2px;color:$color;' class='no-padding no-border $align'>" . number_format($row->$f) . "</span>";
                            }
