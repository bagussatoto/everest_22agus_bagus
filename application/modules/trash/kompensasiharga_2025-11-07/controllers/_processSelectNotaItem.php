...
//arrPrintPink($tmpB);
        ...
        //  membaca registry
        $tr_id = $tmpB[0]->transaksi_id;
        $tmpRegistry = $trs->lookupDataRegistriesByMasterID($tr_id)->result();
//showLast_query("biru");
...
//arrPrintPink($tmp);
                ...
                $_SESSION[$cCode]['main']["referenceID"] = $tr_id;

                if (sizeof($items) > 0) {
                    foreach ($items as $iii => $iiiSpec) {
                        $iiiSpec["handler"] = $this->modul . "/_processSelectProduct";
                        $items[$iii] = $iiiSpec;
                    }
                }
...
//                        mati_disini();
                    ...
                    if ($subAmountConfig != null) {
                        $subtotal = makeValue($subAmountConfig, (array)$row, (array)$row, 0);
                        $tmp["subtotal"] = $subtotal;
                    }
                    else {
                        $tmp["subtotal"] = 0;
...
//        mati_disini("sampai disini " . __LINE__);
        /* --------------------------------------------------
