...
// arrPrint($dataOld);
        ...
        //-----------------------
// matiHEre($dataOld[$key]);
        $trID = $_GET['id'];
        if ($dataOld[$key] == $val) {

            if ($val !== '') {
                echo "<script>top.$('#opt$trID').prop('disabled', false);</script>\n";
            }
            else {


                echo "<script>
            if(top.$('#opt$trID').prop('checked')){
                top.$('#opt$trID').click()
            }
            top.$('#opt$trID').prop('disabled', true);</script>\n";
            }

            $msg = "Tidak ada perubahan data.";
            echo "<script>top.$.showNotification({body:'$msg....!!!',type:'warning'})</script>\n";
            die();
        }
        ...
        $data = array(
            "$key" => $val
//            "extern_date2" => date('Y-m-d H:i:s'),
        );
        $tr->updatePaymentSrc($where, $data);

        $tr->setFilters(array());
        $tr->addFilter("transaksi_id='$transaksiID'");
        $tempReg = $tr->lookupDataRegistries()->result();
        $reg = blobDecode($tempReg[0]->main);
//        foreach($reg as )
        switch ($key) {
            case "extern_label2":
                $key2 = "eFaktur";
                if (isset($reg[$key2])) {
                    unset($reg[$key2]);
                    $newreg = $reg + array($key2 => $val);
                    $tonewreg = blobEncode($newreg);
//                    arrPrint($tonewreg);
//                    matiHere(__LINE__);
                    $tr->setFilters(array());
                    $tr->updateDataRegistry(array("transaksi_id" => $transaksiID), array("main" => $newreg));
...
//                    matiHere();
                ...
                break;
            default:
                matiHere("gagal melakukan update faktur. silahkan hubungi admin untuk melakukan pengecekan lebih lanjut.");
                break;
        }
//        cekMErah($reg);
//        cekMErah($transaksiID);
...
//         mati_disini("-- MAINTENANCE --".$transaksiID);
        $this->db->trans_complete() or die("Gagal saat berusaha  commit transaction!");

        $value_lama = $dataOld[$key];
        $value_baru = $val;

        $msg = "Perubahan berhasil disimpan.<br>lama $value_lama -> $value_baru";
        ...
        echo "<script>top.$.showNotification({body:'$msg....!!!',type:'success'})</script>";
//        die();
