...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg()
    {
                    ...
                    foreach ($mongoFields as $param) {
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "tableIn_master"://
                                    $masterTableInParams = unserialize(base64_decode($row->$param));
                                    if (!is_array($masterTableInParams)) {
                                        $masterTableInParams = blobDecode($masterTableInParams);
                                    }
                                    unset($masterTableInParams['dtime']);
                                    break;
                                case "tableIn_detail"://
                                    $childTableInParams = unserialize(base64_decode($row->$param));
                                    if (!is_array($childTableInParams)) {
                                        $childTableInParams = blobDecode($childTableInParams);
                                    }
                                    break;
                                case "tableIn_master_values"://
                                    $masterTableInValueParams = unserialize(base64_decode($row->$param));
                                    if (!is_array($masterTableInValueParams)) {
                                        $masterTableInValueParams = blobDecode($masterTableInValueParams);
                                    }
                                    break;
                                case "tableIn_detail_values"://
                                    $childTableInValueParams = unserialize(base64_decode($row->$param));
                                    if (!is_array($childTableInValueParams)) {
                                        $childTableInValueParams = blobDecode($childTableInValueParams);
                                    }
                                    break;
                                case "main_add_values"://
                                    $masterAddValues = unserialize(base64_decode($row->$param));
                                    if (!is_array($masterAddValues)) {
                                        $masterAddValues = blobDecode($masterAddValues);
                                    }
                                    break;
                                case "main_add_fields"://
                                    $masterAddFields = unserialize(base64_decode($row->$param));
                                    if (!is_array($masterAddFields)) {
                                        $masterAddFields = blobDecode($masterAddFields);
                                    }
                                    break;
                                case "main_elements"://
                                    $mainElements = unserialize(base64_decode($row->$param));
                                    if (!is_array($mainElements)) {
                                        $mainElements = blobDecode($mainElements);
                                    }
                                    break;
                                case "receiptDetailFields"://
                                    $receiptDetailFields = unserialize(base64_decode($row->$param));
                                    if (!is_array($receiptDetailFields)) {
                                        $receiptDetailFields = blobDecode($receiptDetailFields);
                                    }
                                    break;
                                case "receiptSumFields"://
                                    $receiptSumFields = unserialize(base64_decode($row->$param));
                                    if (!is_array($receiptSumFields)) {
                                        $receiptSumFields = blobDecode($receiptSumFields);
                                    }
                                    break;
                                case "receiptDetailFields2"://
                                    $receiptDetailFields2 = unserialize(base64_decode($row->$param));
                                    if (!is_array($receiptDetailFields2)) {
                                        $receiptDetailFields2 = blobDecode($receiptDetailFields2);
                                    }
                                    break;
                                case "receiptSumFields2"://
                                    $receiptSumFields2 = unserialize(base64_decode($row->$param));
                                    if (!is_array($receiptSumFields2)) {
                                        $receiptSumFields2 = blobDecode($receiptSumFields2);
                                    }
                                    break;
                                case "items"://
                                    $itemsRegistries = unserialize(base64_decode($row->$param));
                                    if (!is_array($itemsRegistries)) {
                                        $itemsRegistries = blobDecode($itemsRegistries);
                                    }
                                    break;
                                case "items2"://
                                    $items2Registries = unserialize(base64_decode($row->$param));
                                    if (!is_array($items2Registries)) {
                                        $items2Registries = blobDecode($items2Registries);
                                    }
                                    break;
                                case "items2_sum"://
                                    $items2_sumRegistries = unserialize(base64_decode($row->$param));
                                    if (!is_array($items2_sumRegistries)) {
                                        $items2_sumRegistries = blobDecode($items2_sumRegistries);
                                    }
                                    break;
                                case "items3_sum"://
                                ...
                                case "itemSrc"://
                                    ...
                                    if (!is_array($itemSrc)) {
                                        $itemSrc = blobDecode($itemSrc);
                                    }
                                    break;
                                case "rsltItems":
                                    $receiptRsltItems = unserialize(base64_decode($row->$param));
                                    if (!is_array($receiptRsltItems)) {
                                        $receiptRsltItems = blobDecode($receiptRsltItems);
                                    }
                                    break;
        ...
        if (isset($globalVars['description_special']) && (sizeof($globalVars['description_special']) > 0)) {
            $catatan = $globalVars['description_special'];
            $fixedElements['noteDetails']['contents'][] = "<br />" . $catatan;
        }
        if (isset($globalVars['description_additional']) && (sizeof($globalVars['description_additional']) > 0)) {
            $ii_rslt = "";
            foreach ($globalVars['description_additional'] as $ii => $iiVal) {
...
//arrPrintHitam($items);
    ...
    public function viewReceiptReg_00()
    {
        ...
        //--------------------
        // arrPrint($shoppingCartSubDetailFields);
        if (sizeof($shoppingCartSubDetailFields) > 0) {
    ...
    public function viewReceiptProduksi()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            $itemNumLabels = isset($this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum] : $this->configUi[$this->jenisTr]['shoppingCartNumFields'][$currentStepNum];
            $itemLabelsConfig = isset($this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum] : $this->configUi[$this->jenisTr]['shoppingCartFields'][$currentStepNum];
            $itemLabelsConfig2 = isset($this->configLayout[$this->jenisTr]['receiptDetailFields2'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields2'][$currentStepNum] : array();
            $itemLabelsConfig3 = isset($this->configLayout[$this->jenisTr]['receiptDetailFields3'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields3'][$currentStepNum] : array();
                    ...
                    foreach ($row as $key_reg => $val_reg) {
                        ...
                        switch ($key_reg) {
                            ...
                            case "main_elements"://
                                ...
                                break;
                            case "receiptDetailFields"://
                                $receiptDetailFields = unserialize(base64_decode($val_reg));
                                break;
                            case "receiptSumFields"://
                                $receiptSumFields = unserialize(base64_decode($val_reg));
                                break;
                            case "receiptDetailFields2"://
                                $receiptDetailFields2 = unserialize(base64_decode($val_reg));
                                break;
                            case "receiptSumFields2"://
                                $receiptSumFields2 = unserialize(base64_decode($val_reg));
                                break;
                            case "items"://
                                $itemsRegistries = unserialize(base64_decode($val_reg));
                                break;
                            case "items2"://
                                $items2Registries = unserialize(base64_decode($val_reg));
                                break;
                            case "items2_sum"://
                                $items2_sumRegistries = unserialize(base64_decode($val_reg));
                                break;
                            case "items3_sum"://
                                $items3_sumRegistries = unserialize(base64_decode($val_reg));
                                break;
                            case "main"://
        ...
        $itemLabels = $itemLabels + $itemNumLabels + $itemSubTotal;
        $itemLabels2 = $itemLabels2 + $itemNumLabels2 + array("subtotal" => "Total Price");
        $itemLabels3 = sizeof($itemNumLabels3) > 0 ? $itemLabels3 + $itemNumLabels3 + array("subtotal" => "Total Price") : $itemLabels3 + array("subtotal" => "Total Price");
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number]) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number] == true) {
            unset($itemLabels['subtotal']);
            unset($itemLabels2['subtotal']);
