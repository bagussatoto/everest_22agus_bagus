...
class Transaksi extends Modul_Controller
{
    ...
    public function viewUndoneItemsIndex()
    {
        ...
        // Lepas lock session agar request AJAX lain dari user yang sama bisa jalan paralel.
        session_write_close();
        $scriptBottom = "";
...
//            $historyFields = isset($this->configUi[$jenisTr]['shortHistoryFieldsCek']) ? $this->configUi[$jenisTr]['shortHistoryFieldsCek'] : array();
        }
            ...
            foreach ($tmpHist as $row) {
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    if (is_array($fLabel)) {
                        ...
                        if ($fName == "reference_jenis_master") {

                            $tmp[$fName] = isset($row->$fName) ? $this->config->item("heTransaksi_ui")[$row->$fName]["label"] : "-";
                        }
                        else {
                            $tmp[$fName] = isset($row->$fName) ? formatField_he_format($fName, $row->$fName) : formatField_he_format($fName, 0);
                        }
...
//            "mode" => "viewUndoneItemsIndex",
    ...
    public function viewUndoneItemsIndex_line()
    {
...
//                    matiHere(__LINE__);
                    ...
                    if (sizeof($tmpSrc2) > 0) {
                        ...
                        $tmpReg_result = array();
                        if (sizeof($pairRegistries) > 0 && isset($arrTransID) && sizeof($arrTransID) > 0) {
                            $trReg = new MdlTransaksi();
                            $trReg->setFilters(array());
                            $selectKolom = implode(",", $pairRegistries) . ",transaksi_id";
                            $trReg->setJointSelectFields($selectKolom);
                            ...
                            if (sizeof($tmpReg) > 0) {
                                foreach ($tmpReg as $regRow) {
                                    foreach ($pairRegistries as $registryKey) {
                                        $tmpReg_result[$regRow->transaksi_id][$registryKey] = (isset($regRow->$registryKey) && $regRow->$registryKey != null) ? blobDecode($regRow->$registryKey) : array();
                                    }
                                }
                            }
                        }
...
//                                                    cekHere(":: $k => $v ::");
                            ...
                            if ($pakai_ini == 1) {
                                ...
                                $detail = viewDetailTransaksi($tmpReg_result[$tmpSpec->transaksi_id], $arrItemShow, $tmpSpec->jenis_master);
                                $tblItems = $detail;
                            }
                            $tmp2Datas['item_fields'] = $tblItems;
                            //endregion
...
//        arrPrint($sesionReplacer);
        ...
        $pairRegistries = isset($this->configUi[$jenisTr]['pairRegistries']) ? $this->configUi[$jenisTr]['pairRegistries'] : array();
        $arrExtHistoryFields2 = isset($this->configUi[$jenisTr]["extHistoryFields2"]) ? $this->configUi[$jenisTr]["extHistoryFields2"] : array();
        $arrItemShow = isset($this->configUi[$jenisTr]["shortItemsFields2"]) ? $this->configUi[$jenisTr]["shortItemsFields2"] : array();
        ...
        $followUpLiveEditField = isset($this->configUi[$jenisTr]["followUpLiveEditField2"]) ? $this->configUi[$jenisTr]["followUpLiveEditField2"] : array();
        $allowedPurchaseOrder = isset($this->configUi[$jenisTr]["allowedPurchaseOrder"]) ? $this->configUi[$jenisTr]["allowedPurchaseOrder"] : false;
        $allowedAllSalesOrder = isset($this->configUi[$jenisTr]["allowedAllSalesOrder"]["enabled"]) ? $this->configUi[$jenisTr]["allowedAllSalesOrder"]["enabled"] : false;
        $allowedAllSalesOrderLabel = isset($this->configUi[$jenisTr]["allowedAllSalesOrder"]["label"]) ? $this->configUi[$jenisTr]["allowedAllSalesOrder"]["label"] : "";
        $produkKategoriBlacklist = array(
            "4",// jasa
        );
...
//        $prePreFields['state'] = "status";
        ...
        if (isset($shortRequestFields2Config["enabled"]) && ($shortRequestFields2Config["enabled"] == true)) {
            $labelHeader = $shortRequestFields2Config["labelHeader"];
...
//            cekHijau(sizeof($tmpHist));
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            //membaca transaksi data
//            $trd = new MdlTransaksi();
//            $trd->setFilters(array());
//            $tmpTrData = $trd->lookupDetailTransaksi($arrTransID);
            $tr = new MdlTransaksi();
            $tr->setFilters(array());
            $tr->addFilter("transaksi_id in ('" . implode("','", $arrTransID) . "')");
            $tr->addFilter("produk_ord_kurang>0");
            $tmpTr = $tr->lookupJoined_OLD()->result();
//            cekMerah($this->db->last_query());
            $tmpTrData = array();
            foreach ($tmpTr as $tmpTr_0) {
                $tmpTrData[$tmpTr_0->transaksi_id][] = $tmpTr_0;

                if (!isset($arrTransIDQty[$tmpTr_0->transaksi_id])) {
                    $arrTransIDQty[$tmpTr_0->transaksi_id] = 0;
                }
                $produk_ord_kurang_netto = (($tmpTr_0->produk_ord_kurang - $tmpTr_0->produk_ord_dibeli) < 0) ? 0 : ($tmpTr_0->produk_ord_kurang - $tmpTr_0->produk_ord_dibeli);
                $arrTransIDQty[$tmpTr_0->transaksi_id] += $produk_ord_kurang_netto;
//                if($tmpTr_0->transaksi_id == 504395){
//                    cekHere("produk_ord_kurang: " . $tmpTr_0->produk_ord_kurang);
//                    cekHere("produk_ord_dibeli: " . $tmpTr_0->produk_ord_dibeli);
//                    cekHere("produk_ord_kurang_netto: " . $produk_ord_kurang_netto);
//                }

            }
//            arrPrint($arrTransIDQty);
//            cekHere($arrTransIDQty[504395]);
                        ...
                        foreach ($tmpReg_result[$row->transaksi_id] as $param => $eReg) {
                            switch ($param) {
                                ...
                                case "items":
                                    ...
                                    foreach ($eReg as $ix => $ixSpec){
                                        if (in_array($ixSpec["kategori_id"], $produkKategoriBlacklist)) {
                                            $itemsPaketJasa[$ix] = $ixSpec["nama"];
                                        }
                                    }
                                    break;
                            }
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    if (is_array($fLabel)) {
                        ...
                        if (isset($row->ids_his)) {
                            if ($hisKey == "nomer") {
//                                $returnVal = showHistoriGlobalNumbers($row->ids_his, $hisStep, true, $this->jenisTr);
                                $returnVal = showHistoriGlobalNumbers($row->ids_his, $hisStep, true, $jenisMaster);
                                if ($returnVal == "") {
                                    $tmp[$fName] = "-";
                                }
                                else {
                                    $tmp[$fName] = $returnVal;
//                                    cekHere("[$fName] [$returnVal]");
                                }
                            }
                            else {
                                $ids_his_decode = blobDecode($row->ids_his);
                        ...
                        if ($fName == "jenis_master") {
                            $tmp[$fName] = $this->config->item("heTransaksi_ui")[$row->$fName]["label"];
                        }
                        else {
...
//cekHitam(__LINE__);
                                ...
                                $editField = "<select $disabled_hold class='form-control' title='$add_note_title' id='" . $fName . "_" . $row->id . "' onchange=\"document.getElementById('result').src='" . MODUL_PATH . $selector . "?key=$fName&jenistr=$jenis_master&trid=" . $row->transaksi_id . "&val=' + encodeURIComponent(this.value);\">";
...
//                            cekMerah(__LINE__);
//                                if($fName == "nomer_top"){
//                                    cekMerah(__LINE__ . " [nomer_top] [$jenisMaster] [$modul_path]");
//                                }
//                                if($fName == "nomer_soa"){
//                                    cekMerah(__LINE__ . " [nomer_soa] [$jenisMaster] [$modul_path]");
//                                }
...
//                    arrPrint($row->cancel_packing_source_id);
                ...
                $tblItems = "";
                if (sizeof($arrItemShow) > 0) {
                    if (isset($tmpTrData[$row->transaksi_id])) {
                        $tblItems .= "<table class='table table-bordered'>";
                        $tblItems .= "<tr>";
                        $tblItems .= "<th>No</th>";
                        foreach ($arrItemShow as $k => $iLabel) {
                        ...
                        $tblItems .= "</tr>";
                        $numb = 0;
                        foreach ($tmpTrData[$row->transaksi_id] as $row2) {
                            if ($row2->produk_ord_jml - $row2->valid_qty != 0) {
                                $row->partial = 1;
                            }
                            $numb++;
                            $tblItems .= "<tr>";
                            ...
                            foreach ($arrItemShow as $k => $iLabel) {
                                $class = "";
                                if (isset($row2->$k) && is_numeric($row2->$k)) {
                                    $class = "text-right";
                                }
                                if ($k == 'produk_ord_diterima') {
                                    $row2->produk_ord_diterima = $row2->produk_ord_jml - $row2->valid_qty;
                                }
                                $isi = isset($row2->$k) ? $row2->$k : "-";
                                $tblItems .= "<td class='$class'>" . $isi . "</td>";
                            }
                            $tblItems .= "</tr>";
                        }
...
//                }
                ...
                //-------------------------
                $disabled_qty = "";
                if (isset($arrTransIDQty[$row->transaksi_id])) {
                    $disabled_qty = ($arrTransIDQty[$row->transaksi_id] == 0) ? "disabled" : "";
                }
                //-------------------------
                $tmp['action'] = "<div class='input-group'>";
                $targetFollowupLink = $isCancelPacking == true ? "followupCancelPackingPrePreview" : "followupPrePreview";
                $followupLink = "top.$('#result').load('" . MODUL_PATH . "$link_swap" . "$jenisTr/" . $row->transaksi_id . "');";
                $tmp['action'] .= "<button class='btn btn-primary btn-block' title='turn this entry into $stepLabel' 
                                href='javascript:void(0)' $disabled_button $disabled_qty
                                onClick =\"$followupLink\">tentukan jumlah yang dikirim</button>";

                //---
                // region tombol pre-pembelian/pre-po
                if($allowedPurchaseOrder == true){

                    if($allowedAllSalesOrder == true){
                        $itemsPaketJasa = array();// direset atau dikosongkan
                    }
                    if(sizeof($itemsPaketJasa) == 0){
                        $nomer = $row->nomer;
                        $linkPrePo = MODUL_PATH . "Transaksi/viewPrePo/$jenisTr/" . $row->transaksi_id;
                        $actionTarget = "top.BootstrapDialog.show({
                                title:'preview data $nomer',
                                message: " . 'top.$' . "('<div></div>').load('$linkPrePo'),
                                draggable:true,
                                size:top.BootstrapDialog.SIZE_WIDE,
                                type:top.BootstrapDialog.TYPE_DEFAULT,
                                closable:false,
                        ...
                        $tmp['action'] .= "<button class='btn btn-warning btn-block' title='lakukan pembelian ke supplier' 
                                href='javascript:void(0)' $disabled_button $disabled_qty
                                onClick =\"$actionTarget\">lakukan pembelian ke supplier</button>";
                    }
                    else{
                        $add_note .= "<br><span class='meta'>$allowedAllSalesOrderLabel</span>";
                    }
                }
                // endregion tombol pre-pembelian/pre-po
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
            ...
            "availDbTable" => $availDbTable,
            "arrayHistory" => isset($arrayHistory) ? $arrayHistory : array(),
            "onprogressTitle" => "<span class='glyphicon glyphicon-alert'></span> $labelHeader ",
            "arrayProgressLabels" => $progressFields,
            "arrayOnProgressToPay" => $paymentConfig,
            "itemLabels" => isset($itemLabels) ? $itemLabels : "",
            "srcLabel" => isset($srcLabel) ? $srcLabel : "",
            ...
            "defaultItemTrgEditable" => isset($defaultItemTrgEditable) ? $defaultItemTrgEditable : array(),
            "editItemTrg" => MODUL_PATH . "_followupLiveEdit/editEfaktur/" . $this->jenisTr . "/",
            "bgTransaksiColor" => array("yellow" => "Transaksi dikirim sebagian/partial.", "orange" => "Transaksi Close/Fullfillment."),
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
    ...
    public function viewPrePo()
    {
//        arrPrint($this->uri->segment_array());
//        arrPrintWebs($_GET);
        $transaksi_id = $no = $this->uri->segment(5);
        $pre_session = "PRE_PO";
        $newWarningLabel = NULL;
        $allowedAction = true;
        $produkKategoriBlacklist = array(
            "4",// jasa
        );
        $itemsBlacklist = array();
        $itemsNol = array();
        $itemsDibeli = array();
        ...
        if (!isset($_SESSION[$pre_session])) {
            $tr = new MdlTransaksi();
            $tr->addFilter("transaksi_id in ('$transaksi_id')");
            $tr->addFilter("produk_ord_kurang>0");
            $tr->addFilter("transaksi.trash_4=0");
            $tmpTr = $tr->lookupJoined_OLD()->result();
            ...
            $items = array();
            if (sizeof($tmpTr) > 0) {
                foreach ($tmpTr as $tmpSpec) {
                    $pIDs[$tmpSpec->produk_id] = $tmpSpec->produk_id;
                    $produk_ord_jml = $tmpSpec->produk_ord_jml;
                    $produk_ord_kurang = $tmpSpec->produk_ord_kurang;
                    $produk_ord_dibeli = $tmpSpec->produk_ord_dibeli;
                    $produk_ord_kurang_netto = (($produk_ord_kurang - $produk_ord_dibeli) < 0) ? 0 : ($produk_ord_kurang - $produk_ord_dibeli);
                    $_SESSION[$pre_session]["items"][$tmpSpec->produk_id] = array(
                        "id" => $tmpSpec->produk_id,
                        "nama" => $tmpSpec->produk_nama,
                        "kode" => $tmpSpec->produk_kode,
//                        "jml" => $tmpSpec->produk_ord_kurang,
//                        "qty" => $tmpSpec->produk_ord_kurang,
                        "jml_order" => $produk_ord_jml,
                        "qty_order" => $produk_ord_jml,
                        "jml" => $produk_ord_kurang_netto,// jumlah yang di po kan
                        "qty" => $produk_ord_kurang_netto,// jumlah yang di po kan
                        "jml_dibeli" => $produk_ord_dibeli,
                        "qty_dibeli" => $produk_ord_dibeli,
                        "transaksi_id" => $tmpSpec->transaksi_id,
                        "transaksi_nomer" => $tmpSpec->nomer,
                        "customers_id" => $tmpSpec->customers_id,
                        "customers_nama" => $tmpSpec->customers_nama,
                        "cabang_tujuan_id" => $tmpSpec->cabang_id,
                        "customers_tujuan_nama" => $tmpSpec->cabang_nama,
                    );
                    $_SESSION[$pre_session]["main"] = array(
                        "transaksi_id" => $tmpSpec->transaksi_id,
                        "transaksi_nomer" => $tmpSpec->nomer,
                        "customers_id" => $tmpSpec->customers_id,
                        "customers_nama" => $tmpSpec->customers_nama,
                        "cabang_tujuan_id" => $tmpSpec->cabang_id,
                        "cabang_tujuan_nama" => $tmpSpec->cabang_nama,
                    );
                }

                $pd = New MdlProduk2();
                $pd->addFilter("id in ('" . implode("','", $pIDs) . "')");
                $pdTmp = $pd->lookupAll()->result();
                showLast_query("biru");
                if (sizeof($pdTmp) > 0) {
                    foreach ($pdTmp as $pdSpec) {
//                        arrPrint($pdSpec);
//                        $sIDs[$pdSpec->supplier_id] = $pdSpec->supplier_id;
                        $_SESSION[$pre_session]["items"][$pdSpec->id]["supplier_id"] = $pdSpec->supplier_id;
                        $_SESSION[$pre_session]["items"][$pdSpec->id]["supplier_nama"] = $pdSpec->supplier_nama;
                        $_SESSION[$pre_session]["items"][$pdSpec->id]["jenis"] = $pdSpec->jenis;
                        $_SESSION[$pre_session]["items"][$pdSpec->id]["kategori_id"] = $pdSpec->kategori_id;
                        $_SESSION[$pre_session]["items"][$pdSpec->id]["kategori_nama"] = $pdSpec->kategori_nama;
                        if (in_array($pdSpec->kategori_id, $produkKategoriBlacklist)) {
                            $itemsBlacklist[$pdSpec->id] = $pdSpec->nama;
                        }
                    }
                }
                ...
                $trr = new MdlTransaksi();
                $trr->setFilters(array());
                $trr->addFilter("transaksi_id=$transaksi_id");
                $tmpReg = $trr->lookupDataRegistries()->result();
                if (sizeof($tmpReg) > 0) {
//                    arrPrintCyan($tmpReg[0]);
                    $main_reg = blobDecode($tmpReg[0]->main);
                    $main_elements = blobDecode($tmpReg[0]->main_elements);
//                    arrPrint($main_reg);
//                    arrPrint($main_elements);
                    $requestReference["requestReferenceSoID"] = $tmpTr[0]->transaksi_id;
                    $requestReference["requestReferenceSoNomer"] = $tmpTr[0]->nomer;
                    $requestReference["requestReferenceSoIDTop"] = $tmpTr[0]->id_top;
                    $requestReference["requestReferenceSoNomerTop"] = $tmpTr[0]->nomer_top;
                    $requestReference["requestReferenceSoJenis"] = $tmpTr[0]->jenis;
                    $requestReference["requestReferenceSoJenisTop"] = $tmpTr[0]->jenis_top;
                    $requestReference["requestReferenceSoJenisMaster"] = $tmpTr[0]->jenis_master;
                    $requestReference["requestReferenceCustomerID"] = $tmpTr[0]->customers_id;
                    $requestReference["requestReferenceCustomerName"] = $tmpTr[0]->customers_nama;
                    $requestReference["requestReferenceCabangID"] = $tmpTr[0]->cabang_id;
                    $requestReference["requestReferenceCabangName"] = $tmpTr[0]->cabang_nama;
                    $requestReference["requestReferenceGudangID"] = $tmpTr[0]->gudang_id;
                    $requestReference["requestReferenceGudangName"] = $tmpTr[0]->gudang_nama;
                    $requestReference["requestReferenceDelivery"] = $main_elements["deliveryDetails"];
                    $requestReference["requestReferenceCustomer"] = $main_elements["customerDetails"];
//                    $requestReference["requestReferenceSoDiambilDtime"] = date("Y-m-d H:i:s");
//                    $requestReference["requestReferenceSoDiambilFulldate"] = date("Y-m-d");
//                    arrPrintHitam($requestReference);
                    foreach ($requestReference as $rkey => $rval) {
                        $_SESSION[$pre_session]["main"][$rkey] = $rval;
                    }
                }
            }
            else{
                $msg = "Sales Order yang anda pilih saat ini tidak aktif. Silahkan refresh halaman ini terlebih dahulu. code: " . __LINE__;
                mati_disini($msg);
            }
        }
        else {
            cekMerah("TIDAK BUAT SESSION PRE_PO");
        }


        $_SESSION[$pre_session]["items_blacklist"] = array();
        if (sizeof($_SESSION[$pre_session]["items"])) {
            foreach ($_SESSION[$pre_session]["items"] as $pid => $pSpec) {
                if (in_array($pSpec["kategori_id"], $produkKategoriBlacklist)) {
                    $_SESSION[$pre_session]["items_blacklist"][$pid] = $pSpec;

                    unset($_SESSION[$pre_session]["items"][$pid]);
                }
                else {
                    $sIDs[$pSpec["supplier_id"]] = $pSpec["supplier_id"];
                    $_SESSION[$pre_session]["main"]["supplier_id"] = $pSpec["supplier_id"];
                    $_SESSION[$pre_session]["main"]["supplier_nama"] = $pSpec["supplier_nama"];
                    //----
                    if($pSpec["jml"] > 0){
                        $itemsDibeli[$pid] = $pSpec["jml"];
                    }
                    else{
                        $itemsNol[$pid] = $pSpec["jml"];
                    }
                    //----
                }
            }
        ...
        $disabled_supplier = "";
        $disabled_supplier_warning = "";
        if (isset($sIDs) && (sizeof($sIDs) != 1)) {
            $disabled_supplier = "disabled";
            $disabled_supplier_warning .= "- Supplier dari daftar barang diatas lebih dari 1, PRE-PO dilakukan kepada 1 supplier. Silahkan dikoreksi dahulu.<br>";
        }
        if(sizeof($itemsNol) > 0){
            $disabled_supplier = "disabled";
            $disabled_supplier_warning .= "- Barang dengan jumlah 0, tidak bisa dilakukan Pre-PO. Silahkan dihapus dari daftar.<br>";
        }
        $itemsHeader = array(
            "id" => "pID",
            "kode" => "SKU",
        ...
        $customerDetail = blobDecode($main_elements["customerDetails"]["contents"]);
        $judulBlacklist = "Produk kategori Paket/Jasa tidak bisa dilakukan Pre-PO.";
        $judulReguler = "Produk kategori Unit/Non Unit yang akan dilakukan Pre-PO.";

        $data = array(
            "mode" => "viewPrePo",
            "title" => $no,
            "main" => $_SESSION[$pre_session]["main"],
            "items" => $_SESSION[$pre_session]["items"],
            "items_blacklist" => $_SESSION[$pre_session]["items_blacklist"],
            "itemsHeader" => $itemsHeader,
            "link_remove" => MODUL_PATH . "Transaksi/removePrePo/" . $this->jenisTr . "/$transaksi_id",
            "link_page" => MODUL_PATH . "Transaksi/viewPrePo/" . $this->jenisTr . "/$transaksi_id",
            "actionTarget" => MODUL_PATH . "Transaksi/doPrePo/" . $this->jenisTr . "/$transaksi_id",
            "disabled_supplier" => $disabled_supplier,
            "disabled_supplier_warning" => $disabled_supplier_warning,
            "deliveryDetail" => isset($deliveryDetail) ? $deliveryDetail : array(),
            "customerDetail" => isset($customerDetail) ? $customerDetail : array(),
            //-----
            "disabledLockerTransaksi" => ($allowedAction == true) ? "" : "disabled",
            "newWarningLabel" => $newWarningLabel,
            //-----
            "judulReguler" => $judulReguler,
            "judulBlacklist" => $judulBlacklist,
            "itemsBlacklist" => isset($itemsBlacklist) ? $itemsBlacklist : array(),
        );
        $this->load->view("transaksi", $data);
    }

    public function removePrePo()
    {
        $link_page = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "/" . $this->uri->segment(5);

        $pre_session = "PRE_PO";
        $pre_session_code = "_TR_466";
        $pid = isset($_GET["pid"]) ? $_GET["pid"] : 0;
        $trID = $this->uri->segment(5);

        // region cek locker transaksi
        $this->load->model("Mdls/MdlLockerTransaksi");
        $lt = New MdlLockerTransaksi();
        ...
        // endregion cek locker transaksi

        if ($pid > 0) {
            if (isset($_SESSION[$pre_session]["items"][$pid])) {
                $_SESSION[$pre_session]["items"][$pid] = NULL;
                unset($_SESSION[$pre_session]["items"][$pid]);
            }
            echo "<script>";
            echo "  if(top.document.getElementById('data_so_$pid')){";
            echo "  $('#data_so_$pid').css('visibility', 'collapse');";
            echo "  }";
            echo "</script>";
        }
        else {
            ...
            if ($trID > 0) {
                $this->load->model("Mdls/MdlLockerTransaksi");
                $lt = New MdlLockerTransaksi();
                $lt->execLocker(array(), 0, $trID, NULL);
            }

            if (isset($_SESSION[$pre_session])) {
                $_SESSION[$pre_session] = NULL;
                unset($_SESSION[$pre_session]);
            }
            if (isset($_SESSION[$pre_session_code])) {
                $_SESSION[$pre_session_code] = NULL;
                unset($_SESSION[$pre_session_code]);
            }
        }

        $sIDs = array();
        $itemsNol = array();
        foreach ($_SESSION[$pre_session]["items"] as $pid => $spec) {
            if ($spec["supplier_id"] > 0) {
                $sIDs[$spec["supplier_id"]] = $spec["supplier_nama"];
            }
            if($spec["jml"] == 0){
                $itemsNol[$pid] = $spec["jml"];
            }
        }
        if(sizeof($itemsNol) > 0){
            $allowedAction = false;
        ...
        if (isset($sIDs) && (sizeof($sIDs) == 1)) {
            ...
            if ($allowedAction == true) {
                echo "  if(top.document.getElementById('button_pre_po')){";
                echo "  $('#button_pre_po').removeAttr('disabled');";
                echo "  }";
            }
            echo "  if(top.document.getElementById('disabled_supplier_warning')){";
            echo "  $('#disabled_supplier_warning').css('visibility', 'collapse');";
            echo "  }";
            echo "</script>";
        }
        else {
            echo "  if(top.document.getElementById('button_pre_po')){";
            echo "  $('#button_pre_po').attr('disabled', true);";
            echo "  }";
        }
    }

    public function doPrePo()
    {
        /* yang wajib ada:
         * id sales order
         * konsumen (id, nama, alamat kirim konsumen)
         * cabang tujuan distribusi/pembuat sales order
         * payment method adalah credit
         * jenis PO adalah PO reguler dan dengan PPN
         * delivery address diganti menjadi alamat konsumen
         * produk_ord_kurang dikurangi jumlah yang di pre-po
         * */
//        arrPrintCyan($_SESSION["PRE_PO"]);
//        mati_disini(__LINE__ . " SETOP DULU, masuk eksekutor pre-po...");

        $pre_po_data = $_SESSION["PRE_PO"];
        $arrItemsData = $pre_po_data["items"];
        $arrMainData = $pre_po_data["main"];
        $transaksi_id = $no = $this->uri->segment(5);

        $pids = array();
        foreach ($arrItemsData as $pid => $pSpec) {
            if ($pSpec["qty"] > 0) {
                $pids[$pid] = $pSpec["qty"];
            }
        }
        if (sizeof($pids) == 0) {
            $msg = "Transaksi PRE Purchase Order gagal dilakukan. Silahkan periksa kembali Supplier atau Jumlah masing-masing barang sudah benar.";
            mati_disini($msg . " code: " . __LINE__);
        }
//        mati_disini(__LINE__);
        $pidsEncode = blobEncode($pids);
        //------------------------------------------------------------
        $jenisTr_PO = "466";
        $step_number = 1;
        $configUiMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr_PO, "coTransaksiUi");
        $configCoreMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr_PO, "coTransaksiCore");
        $configLayoutMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr_PO, "coTransaksiLayout");
        $configValuesMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr_PO, "coTransaksiValues");
        $modul_transaksi = $this->config->item("heTransaksi_ui")[$jenisTr_PO]["modul"];
        ...
        heInitGates_he_cart($jenisTr_PO, $initMasterValues);
        foreach ($arrMainData as $key => $val) {
            if (is_array($val)) {
                unset($val["contents_intext"]);
            }
            $_SESSION["_TR_" . $jenisTr_PO]["main"][$key] = $val;
        }
//        arrPrint($arrMainData);

        // bagian select supplier
        $processPihak = $configUiMasterModulJenis["pihakProcessor"];
        $mdlPihak = $configUiMasterModulJenis["pihakModel"];
        $link_supplier = base_url() . "$modul_transaksi/$processPihak/$jenisTr_PO/$mdlPihak?id=" . $arrMainData["supplier_id"] . "&noreload=1";

        // bagian select PPN / NON PPN
        $link_ppn_nonppn = base_url() . "$modul_transaksi/_processSelectProductPpn/select/$jenisTr_PO?ppn=1&ppnTargetItems=ppnFactor&ppnTargetMain=ppnPersenCheck&overWriteMain=ppnFactor&noreload=1";

        // bagian select produk/items
        $processItems = "_processSelectProduct/multiSelectItems";
        $mdlItems = $configUiMasterModulJenis["selectorModel"];
        $link_items = base_url() . "$modul_transaksi/$processItems/$jenisTr_PO/$mdlItems?selector&items=$pidsEncode&noreload=1";

        // bagian select Tipe PO (PO REGULER atau PO TARGET), langsung PO REGULER
        $link_tipe_po = base_url() . "$modul_transaksi/_shoppingCart/fetchElement/$jenisTr_PO/tipePo/MdlPurchasingOrderTipe/?key=1&noreload=1";

        // bagian eksekutor Create/Save
        $link_create_save = base_url() . "$modul_transaksi/Create/save/$jenisTr_PO?&noreload=1";

        // bagian replace session pembelian
        $link_replacer_session = base_url() . "$modul_transaksi/Transaksi/replacerSession/$jenisTr_PO?&noreload=1";
        $link_reload_shoppingcart = base_url() . "$modul_transaksi/_shoppingCart/viewCart/$jenisTr_PO?&noreload=1";

        cekHere($link_supplier);
        cekHere($link_ppn_nonppn);
        cekHere($link_tipe_po);
        cekHere($link_items);
        cekHere($link_create_save);

        $this->load->model("Mdls/MdlLockerTransaksi");
        $lt = New MdlLockerTransaksi();
        $lt->execLocker($_SESSION[$cCode]['main'], 0, $transaksi_id, NULL);

//mati_disini(__LINE__);

        //------------------------------------------------------------
        echo "<script>
        if(top.document.getElementById('result_2')){
            async function loadWithDelay() {
                const iframe = top.document.getElementById('result_2');
                if (!iframe) return;
                const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
                
                iframe.src = '$link_supplier';
                console.log('supplier...');
                await delay(1000);

                iframe.src = '$link_ppn_nonppn';
                console.log('ppn / non ppn...');
                await delay(2000);
                
                iframe.src = '$link_tipe_po';
                console.log('po reguler...');
                await delay(2000);

                iframe.src = '$link_items';
                console.log('items...');
                await delay(2000);
                
                iframe.src = '$link_reload_shoppingcart';
                console.log('replace session...');
                await delay(2000);
                
                iframe.src = '$link_replacer_session';
                console.log('replace session...');
                await delay(2000);
                
                iframe.src = '$link_create_save';
                console.log('save transaksi...');
                await delay(2000);
            ...
            console.log('tidak ada iframe result_2');
            
        }
        </script>";


//        $msg = "doPRE_PO [$jenisTr_PO]";
//        mati_disini($msg . " code: " . __LINE__);
    ...
    public function cek()
    {
        cekHere(count($_SESSION["PRE_PO"]["items"]));
        arrPrint($_SESSION["PRE_PO"]);
        arrPrintCyan($_SESSION["_TR_" . 466]);
    }
