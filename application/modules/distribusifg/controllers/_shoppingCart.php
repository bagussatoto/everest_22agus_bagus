...
class _shoppingCart extends Modul_Controller
{
    ...
    public function viewCart()
    {
        ...
        // $stop =0;
        if ($stop) {
            $html_text = _showNotifActiveSessionSimpel($this->cCode);
            $var = $html_text;

            $sessmain = $_SESSION[$cCode]['main'];
            $sessitems = $_SESSION[$cCode]['items'];
            if ($sessmain['requestReferenceNomer'] && !$sessmain['jml_target_scan']) {
                echo "<script>
            ...
            elseif ($sessmain['jml_target_scan']) {
                echo "<script>
                $('#pihakName').prop('disabled', true);            
                $('#itemKeyword').prop('disabled', true);            
                </script>";

                mati_disini_original('Scaner on <br>Halaman ini tidak bisa digunakan transaksional');
            }
            else {
                echo "<script>

                $('#pihakName').prop('disabled', false);
                $('#itemKeyword').prop('disabled', false);
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "produk_order" => isset($iSpec['produk_order']) ? $iSpec['produk_order'] : "",
                        "produk_sent" => isset($iSpec['produk_sent']) ? $iSpec['produk_sent'] : "",
                    );
        ...
        if (isset($_SESSION[$cCode]["main"]["requestReferenceID"]) && $_SESSION[$cCode]["main"]["requestReferenceID"] > 10) {
            //ada referal pakai yang ada outstandingnya
            if (isset($this->configUi[$this->jenisTr]['shoppingCartFieldRef'][$stepNumber])) {
                $itemLabels = $this->configUi[$this->jenisTr]['shoppingCartFieldRef'][$stepNumber];
            }
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        if ($shopingCartReload) {
            //shopingCartReload
            $_SESSION[$cCode]["main"]["ppnFactor"] = my_ppn_factor();//ini ditambahin karena ada curiga hilang teroverwrite
