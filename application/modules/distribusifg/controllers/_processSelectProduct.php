...
//        }
    ...
    public function multiSelect()
    {
        ...
        $arrQty = isset($_GET['itemqty']) ? unserialize(base64_decode($itemqty)) : array();
        // id_produk => qty
...
//                );
                ...
                if (sizeof($tmpB) > 0) {
                    foreach ($tmpB as $row) {
                        ...
                        $satuan = strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                        $tmpJml = $jmlParam;
                        $tmp_jml_order = $arrQty[$id]["produk_order"];
                        $tmp_jml_sent = $arrQty[$id]["produk_sent"];
                        if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                            cekMerah("masuk locker config");
                        ...
                        if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                            $tmp = array(
                                ...
                                "harga" => 0,
                                "subtotal" => 0,
                                "produk_order"=>$tmp_jml_order,
                                "produk_sent"=>$tmp_jml_sent,
                            );
                        ...
                        if ($pakai_ini == 1) {
                            if ($item_jenis == "item_komposit") {
                                $_SESSION[$cCode]['items6'][$id] = array();
                                $_SESSION[$cCode]['items7'][$id] = array();
...
//                                    $qty_faktor = isset($_GET['newQty']) ? $_GET['newQty'] : 1;//
                                    $qty_faktor = isset($_GET['newQty']) ? $_GET['newQty'] : $jmlParam;//
                                    ...
                                    foreach ($tmpDataProdukKomposisi as $tmpProdukKomposisiPaket) {
                                        ...
                                        if ($jml_serial_paket * 1 == 1) {
                                            ...
                                            $_SESSION[$cCode]['items7'][$id][$tmpProdukKomposisiPaket->id][$d_kode] = array();
                                        }
                                        // matiHere("====|scan_mode:".$tmp['scan_mode']."|====$cCode====|serial:".$tmp['jml_serial']."|====");
                                        ...
                                        if ($produk_jenis_paket == "unit") {
                                            foreach ($arrDataTambahan as $cat => $catSpec) {
                                                foreach ($catSpec as $dkey => $dval) {
                                                    if (isset($tmpProdukKomposisiPaket->$dval) && ($tmpProdukKomposisiPaket->$dval != NULL)) {
                                                        $_SESSION[$cCode]['items7'][$id][$tmpProdukKomposisiPaket->id][$tmpProdukKomposisiPaket->$dval] = array();
                                                        //--------------
                                                        if (!isset($arrCat[$cat])) {
                                                            $arrCat[$cat] = 0;
                                                        }
                                            ...
                                            $_SESSION[$cCode]['items7'][$id][$tmpProdukKomposisiPaket->id][$tmpProdukKomposisiPaket->kode] = array();
                                            $arrCat["barcode"] = 1;
                                        ...
                                        $tmpPaket['produk_paket_id'] = $id;
                                        $tmpPaket['produk_paket_nama'] = $row->nama;

                                        $_SESSION[$cCode]["items6"][$id][$tmpProdukKomposisiPaket->id] = $tmpPaket;
...
//                    cekMerah("tidak ada itemnya!");
                    $bb = new $selectorSrcModel();
                    $bb->setFilters(array());
                    $bb->addFilter("id=$id");
                    $bbTmp = $bb->lookupAll()->result();
                    $pNama = $bbTmp[0]->nama;
                    $pKode = $bbTmp[0]->kode;
                    $msg = "Produk $pNama dengan SKU $pKode sudah dinonaktifkan atau didelete. Silahkan periksa kembali data produk anda. code: " . __LINE__;
//                    mati_disini($msg);
                    // karena tidak ada itemsnya maka hapus sesi-nya
                    //---------------------------
                    unset($_SESSION[$cCode]["main"]["requestReferenceID"]);
                    foreach ($resetorGate as $gate) {
                        $_SESSION[$cCode][$gate] = array();
                    }
                    //---------------------------
