...
//        echo "<script>$actionTarget</script>";
    ...
    public function updateItemField_ori()
    {
...
//            echo(lgShowAlert("NOT replacing $key with $val"));
    ...
    public function updateItemField()
    {
        ...
        if (isset($_SESSION[$cCode]['items'][$id][$key])) {
            ...
            //--------------------------------------------
            //---------------
            if ($mid != NULL) {// dideteksi adalah paket
                cekHijau("paket");
                $_SESSION[$cCode]['items6'][$mid][$id][$key] = ($val);
            }
            else {
                cekHitam("reguler");
                $_SESSION[$cCode]['items'][$id][$key] = $val;
...
//            }
//            else {
//                $_SESSION[$cCode]['items6'][$mid][$id]['jml_scan'] = ($val);
//            }
            if ($mid != NULL) {// dideteksi adalah paket
                $_SESSION[$cCode]['items6'][$mid][$id]['jml_scan'] = ($val);
            }
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    public function updateChecklistReject()
    {
        ...
        $val = is_numeric($_GET['val']) ? $_GET['val'] : htmlspecialchars($_GET['val']);
        $state = $_GET['state'];
        $getmode = $_GET['getmode'];

        switch ($state) {
            case "true":
                $_SESSION[$cCode]["main"][$key] = 1;
                break;
            case "false":
...
//        resetValues($this->jenisTr);
        ...
        echo "<script>";
        echo "  if(top.document.getElementById('followupPreview')){";
        echo "top.$('#followupPreview').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7) . "?rawBuilderURL=$rawBuilderURL&getmode=$getmode');";
        echo "  }";
    ...
    public function patchReg()
    {
//        arrPrint($_GET);
        $key_master = $_GET["key"];
        $val = $_GET["val"];
        $jenisTr = $_GET["jenistr"];
        $element = $this->uri->segment(4);
        $globalUi = loadConfigUiModul()[$jenisTr];
//        arrprint($globalUi);
        $configElement = $globalUi['receiptElements'];
        $configElementPatch = $globalUi['followUpLiveEditField'];


        $trid = $_GET["trid"];

        if (!isset($_GET["trid"])) {
            matiHere("gagal memperbaharui data, karena uniq code transaksi tidak terdeteksi. silahkan hubungi admin utk melakukan tracing");
        }

        /**
         * - cek apakah sudah dilock/ difollowup oleh orang lain
         * - cek apakah ada perubahan status gudang, ini rawan
         * - kalau status pengiriman relative aman tidak berpengaruh ke cabng lain aka pusat/cabang
         */
        $this->db->trans_start();
        ...
        if (sizeof($ltTmp) > 0) {
            /**
             * tambahin alertToGo biar refresh halaman
             */
            $msg = "Transaksi sedang difollowup oleh (" . $ltTmp[0]->oleh_nama . ") update data tidak dapat dilakukan";
            matiHere($msg);
...
//        matiHere(__LINE__);
        ...
        $tr = new MdlTransaksi();
        $tr->setFilters(array());
        $tr->addFilter("transaksi_id='" . $_GET['trid'] . "'");
        $tmpReg = $tr->lookupDataRegistries()->result();
        if (count($tmpReg) > 0) {
            $mainElementOld = blobDecode($tmpReg["0"]->main_elements);
            $preElement = blobDecode($tmpReg["0"]->main_elements);
            $mainOld = blobDecode($tmpReg["0"]->main);
            $main = blobDecode($tmpReg["0"]->main);
            $newElement = array();
            if (isset($mainElementOld[$element])) {

                /**
                 * main_elements
                 */
                $newElement = $mainElementOld[$element];
                $mdl_name = $mainElementOld[$element]["mdl_name"];
                $key = $configElement[$element]["key"];
                $key_labelSrc = $configElement[$element]["labelSrc"];
                $this->load->model("Mdls/" . $mdl_name);
                $usedVal = $configElement[$element]["usedFields"];
                $m = new $mdl_name();
                $tmp = $m->lookUpAll()->result();
                $tmp_conten = array();
                $labelValue = "";
                foreach ($tmp as $tmp_0) {
                    if (isset($tmp_0->$key) && $tmp_0->$key == $val) {
                        $labelValue = $tmp_0->$key_labelSrc;
                        foreach ($usedVal as $k => $alias) {
                            $tmp_conten[$k] = $tmp_0->$k;
                        }
                    }
                }
                $newConten = blobEncode($tmp_conten);

                $newElement["key"] = $val;
                $newElement["labelValue"] = $labelValue;
                $newElement["contents"] = $newConten;
                $newElement["contents_intext"] = print_r($tmp_conten, true);

                /**
                 * main
                 */
                //detector inject main
                $main[$element] = $val;
                $main[$element . "__label"] = $labelValue;
                $main[$element . "__" . $key_labelSrc] = $labelValue;
                if (isset($configElementPatch[$key_master]["keyInjectMain"])) {
                    $main["pihakMainID"] = $val;
                    $main["pihakMainName"] = $labelValue;
//                    $main["pihakMainJenis"] = $labelValue;
                    $main["pihakMainJenis"] = $tmp_conten["jenis"];// update prends, kirim dari pusat/cabang (27 maret 2025)
                }
                $transaksiUpdate = array();
                if (isset($configElementPatch[$key_master]["keyUpdateTransaksi"])) {
                    $transaksiUpdate["gudang_status_id"] = $val;
                    $transaksiUpdate["gudang_status_nama"] = $labelValue;
                    $transaksiUpdate["gudang_status_jenis"] = $tmp_conten["jenis"];
//                    matiHEre(__LINE__);
                }
            }
            $updateElement[$element] = $newElement;

            unset($preElement[$element]);
            $lastElement = $preElement + $updateElement;
        }
        $tr->updateDataRegistry(array("transaksi_id" => $trid), array("main_elements" => $lastElement));
        $tr->updateDataRegistry(array("transaksi_id" => $trid), array("main" => $main));
        if (count($transaksiUpdate) > 0) {
            $tr->setFilters(array());
            $tr->updateData(array("id" => $trid), $transaksiUpdate);
            cekMerah($this->db->last_query());

        }
//        mdlTransaksiKirimHIstory
        $this->load->model("Mdls/MdlTransaksiKirimHistory");
        $h = new MdlTransaksiKirimHistory();
        $insertHistory = array(
            "transaksi_id" => $trid,
            "extern_id" => $val,
            "extern_nama" => $labelValue,
            "label" => $element,
            "oleh_id" => $this->session->login["id"],
            "oleh_nama" => $this->session->login["nama"],
        ...
        $h->addData($insertHistory) or matiHere();
//        matiHere(__LINE__);
        $this->db->trans_complete() or die("Gagal saat berusaha  commit transaction!");
