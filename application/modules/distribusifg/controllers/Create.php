...
//        echo ($time_line);
        ...
        //-----------------------------------------------------
        $shopingCartReferenceValidator = isset($configUi[$this->jenisTr]['shopingCartReferenceValidator']) ? $configUi[$this->jenisTr]['shopingCartReferenceValidator'] : array();
        $requestReferenceDisabled = "";
        if (sizeof($shopingCartReferenceValidator) > 0) {
            if (isset($shopingCartReferenceValidator["enabled"]) && ($shopingCartReferenceValidator["enabled"] == true)) {
                $key_validate = $shopingCartReferenceValidator["key"];
                $fields = $shopingCartReferenceValidator["fields"];
                if (isset($_SESSION[$cCode]["main"][$key_validate]) && ($_SESSION[$cCode]["main"][$key_validate] > 0)) {
                    $requestReferenceDisabled = $fields;
                }
                else {
                    $requestReferenceDisabled = "";
                }
        ...
        * ---------------------------------------------------------------------*/
        ...
        //-target return stok ke dc, jadi auto select target---
        $loadDC = "";
        $autoSelectTarget = isset($configUi[$this->jenisTr]['steps'][1]['autoSelectTarget']) ? $configUi[$this->jenisTr]['steps'][1]['autoSelectTarget'] : false;
        if ($autoSelectTarget == true) {
            $fields = $configUi[$jenisTr]['selectorFields'];
            $mdlName = isset($configUi[$jenisTr]['pihakModel']) ? $configUi[$jenisTr]['pihakModel'] : "";
            $pihakFilters = isset($configUi[$jenisTr]['pihakFilters']) ? $configUi[$jenisTr]['pihakFilters'] : array();
            $processor = base_url() . $this->modul . "/" . $configUi[$jenisTr]['pihakProcessor'] . "/" . "$jenisTr/" . $configUi[$jenisTr]['pihakModel'];
            ...
            $tmpO = $o->lookupAll()->result();
//            showLast_query("biru");
//            cekMerah(sizeof($tmpO));
            if (sizeof($tmpO) == 1) {
                cekMerah("auto load pihak");
                $pihak_id = $tmpO[0]->id;
                $processor_new = $processor . "?id=$pihak_id";
                $loadDC = "\n<script>
                        top.$('#result').load('$processor_new')
                    </script>";
            }
        }
//        else{
//            cekMerah("tidak auto select pihak");
//        }
        //-----------------------------------------------------
...
//                arrPrint($selectorProcessor);
        ...
        $data = array(
            ...
            "specialDescription" => isset($_SESSION[$cCode]['main']['description_special']) ? $_SESSION[$cCode]['main']['description_special'] : "",
            ...
            // ----------------------
            "pihakProjekCallerDelete" => MODUL_PATH . "_processPihak/removeProjek/$jenisTr",
            "pihakProjekCaller" => isset($this->configUi[$jenisTr]["pihakProjekCaller"]) ? MODUL_PATH . $this->configUi[$jenisTr]["pihakProjekCaller"] . "/$jenisTr/" . $this->configUi[$jenisTr]["pihakModelProjek"] : "",
            "pihakProjekLabel" => isset($_SESSION[$cCode]['main']['pihakProjekName']) ? $_SESSION[$cCode]['main']['pihakProjekName'] : isset($this->configUi[$jenisTr]["pihakProjekLabel"]) ? $this->configUi[$jenisTr]["pihakProjekLabel"] : "",
            // ----------------------
...
//            matiHere(__LINE__." :: ".$this->modul."::".$jenisTrTarget);
            ...
            if ($_SESSION[$cCode]['tableIn_master']['gudang_status_jenis'] == "pusat") {
                cekHitam("DIKIRIM DARI DC/PUSAT");
//                if ($_SESSION[$cCode]['main']['requestReferenceID'] > 0) {
//                    $this->load->model("Mdls/MdlLockerTransaksi");
//                    $lt = New MdlLockerTransaksi();
//                    $lt->execLocker($_SESSION[$cCode]['main'], $nextProp['num'], $_SESSION[$cCode]['main']['requestReferenceID'], NULL);
//                }
            }
            else {
                if ($_SESSION[$cCode]['main']['requestReferenceID'] > 0) {
...
//            $this->lastValidate($insertID);
            ...
            // mati_disini("LINE: " . __LINE__ . "Transaksi Normal, under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
...
//                        cekkuning("detecting printing size");
    ...
    public function swapFrom()
    {
        ...
        $stop = cekSessionTransaksi($this->cCode);

        if ($stop == true) {
            $xxx = _showNotifActiveSessionSimpel($this->cCode);
            $xxx .= "<hr><r>Harap hapus semua item dahulu sebelum mengambil joblist lain</r>";
            mati_disini_original($xxx);
        }

        $postTrIDs = (isset($_POST['trID']) && sizeof($_POST['trID']) > 0) ? $_POST['trID'] : array($this->uri->segment(5));
        $needToClear = false;
        if (isset($_SESSION[$cCode]['items']) && sizeof($_SESSION[$cCode]['items']) > 0) {
            $needToClear = true;
        }
        if ($needToClear) {
            die(lgShowAlert("Harap hapus semua item dahulu sebelum mengambil joblist lain."));
        }
        //------------------------------
        $shortRequestFields2Config = isset($this->configUi[$this->jenisTr]['shortRequestFields2Config']) ? $this->configUi[$this->jenisTr]['shortRequestFields2Config'] : array();
        if (sizeof($shortRequestFields2Config) > 0) {
            if (isset($shortRequestFields2Config["enabled"]) && ($shortRequestFields2Config["enabled"] == true)) {
                $referenceValidator = $shortRequestFields2Config["referenceValidator"];
                $val_key = $referenceValidator["key"];
                $val_label = $referenceValidator["labelWarning"];
                if (isset($_SESSION[$cCode]["main"][$val_key]) && ($_SESSION[$cCode]["main"][$val_key] > 0)) {
                    $link = MODUL_PATH . "Create/index/$jenisTr?reset=1";
                    $msg = $val_label;
                    $alert["html"] = $val_label;
                    die(swalAlertGoTo2($alert, $link, "REFRESH"));
                }
            }
        }
        //------------------------------
...
//        if (isset($_POST['trID']) && sizeof($_POST['trID']) > 0) {
        if (sizeof($postTrIDs) > 0) {

            $this->load->model("Mdls/MdlLockerTransaksi");
            $lt = New MdlLockerTransaksi();
            $lt->addFilter("transaksi_id in ('" . implode("','", $postTrIDs) . "')");
            $lt->addFilter("state='hold'");
            $lt->addFilter("jumlah='1'");
            $ltTmp = $lt->lookupAll()->result();
//            showLast_query("biru");
//            cekHere(sizeof($ltTmp));
            $anu = $this->db->last_query();
            $anu = (string)$anu;
            if (sizeof($ltTmp) > 0) {
...
//                    $allowedAction = true;
                    ...
                    $allowedTransaksi_no = $ltTmp[0]->transaksi_no;
                    // cekUngu("transaksi HOLD by orang lain, TIDAK BISA dilanjutkan");
                    if ($allowedTransaksi_no == 0) {
                        $newWarningLabel = "Transaksi ini dikunci karena sedang ditindaklanjuti oleh <span class='uppercase text-bold'>$allowedActionByHold</span>.";
                        $newWarningLabel .= " Silahkan pilih transaksi lainnya.";
                        $arrSwals = array(
                            "type" => "warning",
                ...
                if ($this->session->login['ghost'] == 0) {
                    ...
                    if (sizeof($ltTmp) > 0) {
                        ...
                        $ltHold = array(
                            "state" => "hold",
                            "produk_id" => $postTrIDs[0],
                            "transaksi_id" => $postTrIDs[0],
                            "cabang_id" => $this->session->login['cabang_id'],
                            "oleh_id" => $this->session->login['id'],
                            "oleh_nama" => $this->session->login['nama'],
                            "jenis" => "transaksi",
            ...
            $this->load->model("MdlTransaksi");
            $tr = new MdlTransaksi();
            $tr->addFilter("transaksi_id in ('" . implode("','", $postTrIDs) . "')");
            $tr->addFilter("produk_ord_kurang>0");
            $tmpTr = $tr->lookupJoined_OLD()->result();
            /**
             * untuk menghindari sabotase jika pengiriman diganti oleh cabang tidak jadi dikirim dari pusat,
             * swapfrom di lock, swap from hanya aktif jika gudang_staus pusat, jika cabang di hentikan
             * berarti metode pengiriman terjadi update.
             */

            $requestReference = array();
            if (sizeof($tmpTr) > 0) {
                $defGudang = $tmpTr[0]->gudang_status_jenis;
                if ($defGudang == "pusat") {

                }
                else {
                    $this->load->model("Mdls/MdlTransaksiKirimHistory");
                    $his = new MdlTransaksiKirimHistory();
                    $his->addFilter("label='gudangStatusDetails'");
                    $his->addFilter("transaksi_id='" . $tmpTr[0]->transaksi_id . "'");
                    $this->db->order_by("id", "desc");
                    $this->db->limit(1);
                    $temp = $his->lookUpAll()->result();
                    $change_metode_kirim = $temp[0]->extern_nama;
                    $change_oleh_name = $temp[0]->oleh_nama;
                    $change_dtime = $temp[0]->dtime;
                    $msg = "";
                    $msg = "Metode pengiriman telah diperbaharui<br> dikirim dari $change_metode_kirim <br>oleh : $change_oleh_name <br>pada : $change_dtime<br> Silahkan refresh halaman ini dan pilih transaksi yang lainnya";
                    $alerts = array(
                        "type" => "warning",
                        "html" => "$msg",
                    );
...
//                matiHere(__LINE__);
                ...
                foreach ($tmpTr as $row) {
                    ...
                    $trash_4 = $row->trash_4;
                    if ($trash_4 == 1) {
                        $cancel_dtime = $row->cancel_dtime;
                        $cancel_name = $row->cancel_name;
                        $nomer = $row->nomer;
                        $nomer_f = formatField_he_format("nomer_nolink", $row->nomer);
                        $msg = "Transaksi $nomer_f sudah direject oleh $cancel_name pada $cancel_dtime. Silahkan refresh halaman ini. code: " . __LINE__;
                        $link = MODUL_PATH . "Create/index/$jenisTr?reset=1";
                        $alert["html"] = $msg;
                        die(swalAlertGoTo2($alert, $link, "REFRESH"));
                    }
...
//                        $items[$row->produk_id] += $row->produk_ord_kurang;
//                        $items[$row->produk_id] += $produk_ord_kurang_netto;
                        if($produk_ord_kurang_netto > 0){
                        ...
                        if (!isset($itemQty[$row->produk_id]["produk_order"])) {
                            $itemQty[$row->produk_id]["produk_order"] = 0;
                        }
                        if (!isset($itemQty[$row->produk_id]["produk_sent"])) {
                            $itemQty[$row->produk_id]["produk_sent"] = 0;
                        }
                        $itemQty[$row->produk_id]["produk_order"] += $row->produk_ord_jml;
//                        $itemQty[$row->produk_id]["produk_sent"] += $row->produk_ord_jml - $row->produk_ord_kurang;
                        $itemQty[$row->produk_id]["produk_sent"] += $row->produk_ord_jml - $produk_ord_kurang_netto;
                    }
                    else {
                    ...
                    //-------------------------------
                    $requestReference["requestReferenceID"] = $row->transaksi_id;
                    $requestReference["requestReferenceNomer"] = $row->nomer;
                    $requestReference["requestReferenceIDTop"] = $row->id_top;
                    $requestReference["requestReferenceNomerTop"] = $row->nomer_top;
                    $requestReference["requestReferenceJenis"] = $row->jenis;
                    $requestReference["requestReferenceJenisTop"] = $row->jenis_top;
                    $requestReference["requestReferenceJenisMaster"] = $row->jenis_master;
                    $requestReference["requestReferenceDiambilDtime"] = date("Y-m-d H:i:s");
                    $requestReference["requestReferenceDiambilFulldate"] = date("Y-m-d");
                }
                //==ambil value-gate
                $trr = new MdlTransaksi();
                $trr->setFilters(array());
                $trr->addFilter("transaksi_id in ('" . implode("','", $postTrIDs) . "')");
                ...
                if (sizeof($tmpReg) > 0 && sizeof($swappedKeys) > 0) {
                    ...
                    foreach ($tmpReg as $rowr) {
                        foreach ($swappedKeys as $kr => $vr) {
                            $_SESSION[$cCode]['main'][$kr] = $oldMain[$vr];
                            echo "<script>";
                            echo "if(top.document.getElementById('$kr')){top.document.getElementById('$kr').value='" . $oldMain[$vr] . "'}";
                            echo "</script>";
                        }
                    }
                }
                ...
                $targetProc = MODUL_PATH . $this->configUi[$jenisTr]['itemSwapper'] . "/$jenisTr/?items=" . base64_encode(serialize($items)) . "&itemqty=" . base64_encode(serialize($itemQty));


                if (sizeof($requestReference) > 0) {
                    foreach ($requestReference as $key => $val) {
                        $_SESSION[$cCode]['main'][$key] = $val;
                    }
                }
                ...
                if ($this->session->login['ghost'] == 0) {
                    $reguest_reference_id = $_SESSION[$cCode]['main']['requestReferenceID'];
                    $this->load->model("Mdls/MdlLockerTransaksi");
                    $lt = New MdlLockerTransaksi();
                    $lt->addFilter("transaksi_id=" . $reguest_reference_id);
                    $ltTmp = $lt->lookupAll()->result();
                    if (sizeof($ltTmp) == 0) {
                        $ltActive = array(
                            "state" => "active",
                            "produk_id" => $reguest_reference_id,
                            "transaksi_id" => $reguest_reference_id,
                            "cabang_id" => $this->session->login['cabang_id'],
                            "oleh_id" => 0,
                            "oleh_nama" => "",
                            "jenis" => "transaksi",
    ...
    public function swapDirectFrom()
    {
        $jenisTr = $this->uri->segment(4);
        $cCode = "_TR_" . $this->jenisTr;
...
//                    $requestReference["requestReferenceJenisMaster"] = $row->jenis_master;
                ...
                // region transaksi penjualannya 5822so
                $trID_so = $tmpTr[0]->reference_id;
                $trNomer_so = $tmpTr[0]->reference_nomer;
//                $trID_so_top = $tmpTr[0]->reference_id_top;
//                $trNomer_so_top = $tmpTr[0]->reference_nomer_top;
//                $trID_jenis_so = $tmpTr[0]->reference_jenis;
//                $trID_jenis_so_top = $tmpTr[0]->reference_jenis_top;
//                $trGudangStatus_so_id = $tmpTr[0]->gudang_status_id;
//                $trGudangStatus_so_nama = $tmpTr[0]->gudang_status_nama;
//                $trGudangStatus_so_jenis = $tmpTr[0]->gudang_status_jenis;
                $trr = new MdlTransaksi();
                $trr->setFilters(array());
                $trr->addFilter("id=$trID_so");
                $trrTmp = $trr->lookupAll()->result();
                //-----INI MENJADI SO NYA--------------------------
                $requestReference["requestReferenceID"] = $trrTmp[0]->id;
                $requestReference["requestReferenceNomer"] = $trrTmp[0]->nomer;
                $requestReference["requestReferenceIDTop"] = $trrTmp[0]->id_top;
                $requestReference["requestReferenceNomerTop"] = $trrTmp[0]->nomer_top;
                $requestReference["requestReferenceJenis"] = $trrTmp[0]->jenis;
                $requestReference["requestReferenceJenisTop"] = $trrTmp[0]->jenis_top;
                $requestReference["requestReferenceJenisMaster"] = $trrTmp[0]->jenis_master;
//                arrPrintKuning($trrTmp);
                // endregion transaksi penjualannya 5822so


                $swappedKeys = isset($this->configUi[$jenisTr]['directSwappedKeys']) ? $this->configUi[$jenisTr]['directSwappedKeys'] : array();
                $allowMultiSelect = isset($this->configUi[$jenisTr]['requestCode']['allowMultiSelect']) ? $this->configUi[$jenisTr]['requestCode']['allowMultiSelect'] : false;

//                arrPrintPink(blobDecode($tmpReg[0]->main));
//                arrPrintHijau($requestReference);
//                arrPrintKuning($swappedKeys);
//                mati_disini(__LINE__);
                ...
                $targetProc = MODUL_PATH . $this->configUi[$jenisTr]['itemSwapper'] . "/$jenisTr/?items=" . base64_encode(serialize($items));
                if (sizeof($requestReference) > 0) {
                    foreach ($requestReference as $key => $val) {
                        $_SESSION[$cCode]['main'][$key] = $val;
                    }
    ...
    public function reset()
    {
        ...
        //-------------------------------------------
        $reguest_reference_id = $_SESSION[$cCode]['main']['requestReferenceID'];
        if ($reguest_reference_id > 0) {
            $this->load->model("Mdls/MdlLockerTransaksi");
            $lt = New MdlLockerTransaksi();
            $lt->execLocker($_SESSION[$cCode]['main'], 0, $reguest_reference_id, NULL);
        }
        //-------------------------------------------
