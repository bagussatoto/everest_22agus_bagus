...
//arrPrintWebs($arrSubDetailDataKolom);
        if (sizeof($signHeader) > 0) {
            foreach ($signHeader as $key => $specHeader) {
                $elementHdr = "<div>";
                foreach ($specHeader as $value) {
 ...
 * -------------------------------------------------------*/
        ...
        $tanggal_transaksi .= "<p class='no-margin'>$nomer <i style='color: #FFFFFF'>$trIdKhusus</i></p>";
        ...
        if (sizeof($signValues) > 0) {
            foreach ($signValues as $key => $eSpec) {
                ...
                $signVar .= "<div style='floatr: left;' title='$key' class='$key $tampil text-uppercase'>";
