                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        ...
                        case "text":
                            ...
                            if (is_numeric($fieldVal) && $keyID != 'barcode' && $keyID != 'produk_kode' && $keyID != 'sku') {
//                                echo "<span isNumeric keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:$color;background:#f0f0f0;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                                echo "<span isNumeric keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:$color;background:#f0f0f0;'>" . formatField_he_format($key, $fieldVal) . "$add_data_val</span>";
                            }
                            else {
                                echo "<span isNotNumeric keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;'>" . formatField_he_format($key, $fieldVal) . "$add_data_val</span>";
                            }
...
//                echo "<script> setTimeout( function(){ top.$('.select2').selectpicker(); console.log('dari shopingcart') }, 500 ); </script>";
        ...
        if ((sizeof($itemLabels2) > 0) && (is_array($itemLabels2))) {
        ...
        //region items3
        if ((sizeof($itemLabels3) > 0) && (is_array($itemLabels3))) {
