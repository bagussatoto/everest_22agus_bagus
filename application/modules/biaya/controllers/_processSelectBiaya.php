...
//        }
    ...
    public function selectInv()
    {
        ...
        $lockerConfig = isset($this->configUi[$this->jenisTr]['lockerCheck']) ? $this->configUi[$this->jenisTr]['lockerCheck'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $autoLoadRebate = isset($this->configUi[$this->jenisTr]['autoLoadRebate']) ? $this->configUi[$this->jenisTr]['autoLoadRebate'] : array();
...
// START OF COMPLETE REPEATED LOGIC
            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                $nilai_untung = 0;
                $nilai_rugi = 0;
                foreach ($_SESSION[$cCode]['items'] as $itemId => $iSpec) {
                    $nilai_untung += $iSpec['nilai_untung'];
                    $nilai_rugi += $iSpec['nilai_rugi'];
                }
...
// END OF COMPLETE REPEATED LOGIC
        ...
        // START OF COMPLETE REPEATED LOGIC
        // Post-ValueGate Guard: Re-apply UU HPP Progressive Tariff for ID 78 and 79
        if (isset($_SESSION[$cCode]["main"]["pihakMainID"])) {
            $id_biaya = $_SESSION[$cCode]["main"]["pihakMainID"];
            if ($id_biaya == 78 || $id_biaya == 79) {
                $this->load->helper("he_pph21_progresif");

                // Restore gross cashback if passed in $_GET['harga']
                if (isset($_GET['harga']) && $_GET['harga'] !== '' && isset($_SESSION[$cCode]['items'][$id])) {
                    $_SESSION[$cCode]['items'][$id]['harga'] = floatval(str_replace(',', '', $_GET['harga']));
                }
                ...
                if (sizeof($res_pph) > 0) {
                    ...
                    foreach ($_SESSION[$cCode]["items"] as $ii => $item_spec) {
                        ...
                        $nilai_pph = ($tarif_efektif_total / 100) * $harga;
                        $nilai_neto = $harga - $nilai_pph;

                        $_SESSION[$cCode]["items"][$ii]["harga"]               = $harga;
                        ...
                        $_SESSION[$cCode]["items"][$ii]["pph_21"]              = $nilai_pph;
                        $_SESSION[$cCode]["items"][$ii]["nilai_pph21"]         = $nilai_pph;
                        $_SESSION[$cCode]["items"][$ii]["nilai_pph_original"] = $nilai_pph;
                        $_SESSION[$cCode]["items"][$ii]["nilai_kas_cn"]        = $nilai_neto;
                    }
                }
            }
        }
        ...
        if (sizeof($autoLoadRebate) > 0) {
            if (isset($autoLoadRebate["enabled"]) && ($autoLoadRebate["enabled"] == true)) {
                $link = $autoLoadRebate["link"];
                echo "<script>";
                echo "top.$('#result').load('" . base_url() . $this->modul . "$link')";
                echo "</script>";
            }
        }
