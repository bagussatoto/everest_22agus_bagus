...
class History extends Modul_Controller
{
    ...
    public function viewHistory()
    {
        ...
        // arrPrintKuning($_GET);
        $batal = isset($_GET["batal"]) ? $_GET["batal"] : NULL;
        ...
        if ($dipakai == 1) {
            ...
            if (sizeof($tmpHist) > 0) {
                ...
                //---------------------
                $arrMasterIDs = array();
                $arrTrMasterIDs = array();
                $arrPairBayarMasterIDs = array();
                foreach ($tmpHist as $ii => $row) {
                    $arrMasterIDs[$row->id_master] = $row->id_master;
//                    $arrTrMasterIDs[$row->id] = $row->id_master;
                }
                $tr = New MdlTransaksi();
                $tr->addFilter("id_master in ('" . implode("','", $arrMasterIDs) . "')");
                $trtMasterHist = $tr->lookUpAll()->result();
                foreach ($trtMasterHist as $ii => $row) {
//                $arrMasterIDs[$row->id_master] = $row->id_master;
                    $arrTrMasterIDs[$row->id] = $row->id_master;
                }

                //---------------------
                $trIDs = array_keys($arrTrMasterIDs);
                $tr = New MdlTransaksi();
                $tr->setFilters(array());
                $tr->addFilter("transaksi_id in ('" . implode("','", $trIDs) . "')");
                $trHist = $tr->lookUpAllPaymentSrc()->result();
                if (sizeof($trHist) > 0) {
                    foreach ($trHist as $trPymSpec) {
//                    arrPrintHitam($trPymSpec);
                        $terbayar = $trPymSpec->terbayar;
                        $sisa = $trPymSpec->sisa;
                        $trid = $trPymSpec->transaksi_id;
                        $mid = $arrTrMasterIDs[$trPymSpec->transaksi_id];
                        if ($terbayar > 0) {
                            if (($terbayar > 0) && ($sisa > 0)) {
                                $ket_status = "terbayar: " . number_format($terbayar, 0, ".", ",") . ", KURANG BAYAR: " . number_format($sisa, 0, ".", ",");
                            }
                            ...
                            $ket_status = "BELUM DIBAYAR";
                        }
                        $arrPairBayarMasterIDs[$mid] = array(
                            "pym_src_master_id" => $mid,
                            "pym_src_transaksi_id" => $trid,
                            "pym_src_terbayar" => $terbayar,
                            "pym_src_sisa" => $sisa,
                            "pym_src_status_keterangan" => $ket_status,
                        );
                    }
                }
                //---------------------
                ...
                foreach ($tmpHist as $ii => $row) {
                    ...
                    if (isset($arrPairBayarMasterIDs[$master_id]) && (sizeof($arrPairBayarMasterIDs[$master_id]) > 0)) {
                        foreach ($arrPairBayarMasterIDs[$master_id] as $pym_key => $pym_val) {
                            $row->$pym_key = $pym_val;
                        }
                    }
...
//            "title"                => isset($subCodes) && isset($currentState) ? $subCodes[$currentState] : $this->jenisTrName,
            ...
            "conten_null" => MODUL_PATH . get_class($this) . "/showData/$jenisTr/$currentState/$this_methode?date1=$date1&date2=$date2&jmaster=$jenisTr" . $getBatal,
    ...
    public function showData()
    {
        ...
        $jmaster = isset($_GET['jmaster']) ? $_GET['jmaster'] : "";
        if (!isset($this->session->login['id'])) {
            gotoLogin();
        }
        $starttime = microtime(true);
        ...
        // $backdate_f = formatTanggal(backDate(30), 'Y-m-d');
        $backdate_f = formatTanggal(backDate(10), 'Y-m-d');
        ...
        if (sizeof($steps) > 1) {
            ...
            if (isset($_GET['stID'])) {
                ...
                $currentState = strlen($this->uri->segment(5)) > 0 ? $this->uri->segment(5) : $this->configUi[$jenisTr]['steps'][1]['target'];
                // $currentState = strlen($this->uri->segment(4)) > 0 ? $this->uri->segment(4) : $this->configUi[$jenisTr]['steps'][1]['target'];
            }
        }
        //endregion
...
//            arrPrintCyan($arrPairBayarMasterIDs);
            //---------------------
            //---------------------
            ...
            foreach ($tmpHist as $ii => $row) {
                ...
                if (sizeof($pairRegistries) > 0) {
                    ...
                    $arrItemShow = isset($this->configUi[$jenisTr]["shortItemsFields"]) ? $this->configUi[$jenisTr]["shortItemsFields"] : array();
                    $arrItemShow2 = isset($this->configUi[$jenisTr]["shortItemsFields2"]) ? $this->configUi[$jenisTr]["shortItemsFields2"] : array();
                    $detail = viewDetailTransaksi($tmpReg_result[$row->id], $arrItemShow, $row->jenis_master);
                    $detail2 = viewDetailTransaksiRebate($tmpReg_result[$row->id], $arrItemShow2, $row->jenis_master, true, array(), "items10_sum");
//                    cekHere($detail2);
                }
...
//cekHere("[$fName]");
                    if ($fName == "pym_src_status_keterangan") {
//                        cekHere("[$colValue]");
//                        if($colValue == NULL){
//                            $colValue = "BELUM DIBAYAR";
//
//                        }
                    }
...
//                break;
        ...
        $data = array(
            ...
            "subTitle" => "histories",
            "customButton" => $customButton,
            "customButtonTarget" => isset($currentState) ? "$currentState?date1=$date1&date2=$date2&jmaster=$jmaster" : "",
            "arrayHistoryLabels" => $historyFields,
            "arrayHistory" => $arrayHistory,
            "arrayHistorySumField" => $sumValue,
            "arrayHistoryId" => $arrayHistory_ids,
