    ...
    public function viewCart()
    {
        ...
        switch ($this->jenisTr) {
            case "6677":
                ...
                elseif ($_SESSION[$cCode]["main"]["pihakID"] == 0) {
                    $_SESSION[$cCode]["items"] = array();
                }
                break;
        }
        ...
        // Khusus Opsi 78 & 79 (PPh 21 UU HPP): Jalankan kalkulasi Total Cart dan override editableFields SEBELUM pembuatan array $items
        $autoSelectPPh_check = isset($this->configUi[$this->jenisTr]['autoSelectPPh']) ? $this->configUi[$this->jenisTr]['autoSelectPPh'] : array();
        if (sizeof($autoSelectPPh_check) > 0 && isset($autoSelectPPh_check["enabled"]) && ($autoSelectPPh_check["enabled"] == true)) {
            $key_deteksi_check = $autoSelectPPh_check["key_deteksi"];
            if (isset($_SESSION[$cCode]["main"][$key_deteksi_check])) {
                $id_biaya_check = $_SESSION[$cCode]["main"][$key_deteksi_check];
                if ($id_biaya_check == 78 || $id_biaya_check == 79) {
                    unset($itemNumLabels['pph__tarif']);
                    $editableFields = array("harga");
                    $editableFields2 = array("harga");
        // END OF COMPLETE REPEATED LOGIC

                    $this->load->helper("he_pph21_progresif");
                    $total_cashback_bruto = 0;
                    if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items'])) {
                        foreach ($_SESSION[$cCode]['items'] as $item_spec) {
                            $item_val = isset($item_spec['harga']) ? floatval($item_spec['harga']) : 0;
                            $total_cashback_bruto += $item_val;
                        }
                    }

                    $has_fasilitas = ($id_biaya_check == 78);
                    $res_pph = hitung_pph21_progresif_uuhpp($total_cashback_bruto, $has_fasilitas);
                    $res_pph_has_fasilitas = $res_pph["has_fasilitas"];
                    $res_pph_tarif_efektif_total = $res_pph["tarif_efektif_total"];

                    $_SESSION[$cCode]["main"]["has_fasilitas"] = $res_pph_has_fasilitas;
                    $_SESSION[$cCode]["main"]["tarif_efektif_total"] = $res_pph_tarif_efektif_total;
                    $_SESSION[$cCode]["main"]["nilai_pph_original"] = $res_pph["total_pph"];
                    $_SESSION[$cCode]["main"]["pph__tarif"] = $res_pph_tarif_efektif_total;
                    $_SESSION[$cCode]["main"]["nilai_kas_cn"] = $res_pph["nilai_kas_cn"];
                    $_SESSION[$cCode]["items8_sum"] = (sizeof($res_pph['detail']) > 0) ? $res_pph['detail'] : array();

                    if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items'])) {
                        foreach ($_SESSION[$cCode]["items"] as $iix => $iixSpec){
                            $item_harga = isset($iixSpec['harga']) ? floatval($iixSpec['harga']) : 0;
                            $item_pph = ($res_pph_tarif_efektif_total / 100) * $item_harga;
                            $item_neto = $item_harga - $item_pph;

                            $_SESSION[$cCode]["items"][$iix]["has_fasilitas"] = $res_pph_has_fasilitas;
                            $_SESSION[$cCode]["items"][$iix]["tarif_efektif_total"] = $res_pph_tarif_efektif_total;
                            $_SESSION[$cCode]["items"][$iix]["pph__tarif"] = $res_pph_tarif_efektif_total;
                            $_SESSION[$cCode]["items"][$iix]["pph_21"] = $item_pph;
                            $_SESSION[$cCode]["items"][$iix]["nilai_pph21"] = $item_pph;
                            $_SESSION[$cCode]["items"][$iix]["nilai_pph_original"] = $item_pph;
                            $_SESSION[$cCode]["items"][$iix]["nilai_kas_cn"] = $item_neto;
                        }
                    }
                }
            }
        ...
        // START OF COMPLETE REPEATED LOGIC
        $nestedPool = isset($this->configUi[$this->jenisTr]['nestedElementsPool']) ? $this->configUi[$this->jenisTr]['nestedElementsPool'] : array();
        $nestedRules = isset($this->configUi[$this->jenisTr]['nestedRelativeRules']) ? $this->configUi[$this->jenisTr]['nestedRelativeRules'] : array();
        if (sizeof($nestedRules) > 0 && sizeof($nestedPool) > 0) {
            ...
            foreach ($activeKeys as $actKey) {
                if (isset($nestedPool[$actKey])) {
                    ...
                    $this->configUi[$this->jenisTr]['receiptElements'][$actKey] = $nestedPool[$actKey];
                }
            }
        }
        // END OF COMPLETE REPEATED LOGIC
        ...
        $pairedItemBreakdown = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown'] : array();
        $pairedItemBreakdownEnabled = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']['enabled']) && $this->configUi[$this->jenisTr]['shoppingCartPairedItemBreakDown']['enabled'] == true ? true : false;
        $pairedItemPotong = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong'] : array();
        $pairedItemPotongEnabled = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong']['enabled']) && $this->configUi[$this->jenisTr]['shoppingCartPairedItemPotong']['enabled'] == true ? true : false;
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "inv_new_net1" => isset($iSpec['inv_new_net1']) ? $iSpec['inv_new_net1'] : "",
                        "inv_dpp_pengganti" => isset($iSpec['inv_dpp_pengganti']) ? $iSpec['inv_dpp_pengganti'] : "",
                    ...
                    if ($shoppingCartAmountValue != null) {
                        $tmp['subtotal'] = makeValue($shoppingCartAmountValue, $tmp, $_SESSION[$cCode]['items'][$id], 0);
                    }
                    else {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "biaya/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "biaya/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;

                    /*---*/
                    $stokCache = $stokCachies[$id]['qty_debet'];
                    $stokBooked = $stokCache - $iSpec['stok'];
                    $tmp["stok_booked"] = $stokBooked;
...
//                                cekHitam("[$elKey] [$eName][$currentValue][$rKey]");
//                                arrPrint($relElementConfigs[$eName][$currentValue][$rKey]);
                            }
                        }
                        else {
                        ...
                        if (sizeof($addRowsConfigs[$eName][$currentValue]) > 0) {
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    if (isset($oValSpec['labelAdd'])) {
                                        $anuu = "";
                                        foreach ($oValSpec['labelAddKey'] as $lkey) {
                                            $vall = makeValue($lkey, $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0);
                                            $vall = is_numeric($vall) ? number_format($vall) : $vall;
                                            $anuu .= $vall . "&nbsp;";
                                        }
                                        $addLabel = "<br>" . $oValSpec['labelAdd'] . " : <span class='strong'>$anuu</span>";
                                    }
                            ...
                            case "radio":
                                ...
                                if (sizeof($tmpo) > 0) {
                                    switch ($eName) {
                                        ...
                                        case "cash_account_source_nested":
...
//                                            }
                        ...
                        $editableUsedFields = isset($elementConfigs[$eName]['editableUsedFields']) ? $elementConfigs[$eName]['editableUsedFields'] : array();
                        $hiddenUsedFields = isset($elementConfigs[$eName]['hiddenUsedFields']) ? $elementConfigs[$eName]['hiddenUsedFields'] : array();
                            ...
                            if (isset($elementConfigs[$eName]['usedFields']) && sizeof($elementConfigs[$eName]['usedFields']) > 0) {
                                ...
                                foreach ($elementConfigs[$eName]['usedFields'] as $src => $label) {
                                    ...
                                    if (strlen($fieldLabel) > 0 || $showNull == true) {
                                        if (strlen($label) > 0) {
                                            if (in_array($src, $hiddenUsedFields)) {
                                            ...
                                            if (array_key_exists($src, $editableUsedFields)) {
                                                $usedGerbang = $eName . "__" . $src;
                                                $usedValue = (isset($_SESSION[$cCode]['main'][$usedGerbang]) && ($_SESSION[$cCode]['main'][$usedGerbang] != NULL)) ? $_SESSION[$cCode]['main'][$usedGerbang] : 0;
                                                $selectorTarget = "'" . MODUL_PATH . get_class($this) . "/recordUsedFieldElement/" . $this->jenisTr . "/$eName/$amdlName/$src?val='+this.value";
                                                $edit_tipe = $editableUsedFields[$src]["tipe"];
                                                $newValue_r = "<input type='$edit_tipe' id='$src' name='$src' class='' value='$usedValue'
                                                onclick=\"this.select();\"
                                                onblur=\"top.$('#result').load($selectorTarget);\"
                                                >";
...
// START OF COMPLETE REPEATED LOGIC
        //-------------------------------------------------------------------------------
        if (isset($_SESSION[$cCode]["main"]["pajakOption"]) && $_SESSION[$cCode]["main"]["pajakOption"] == "pph_final_umkm") {
            $_SESSION[$cCode]["main"]["pph23Methode"] = "suket";
            $_SESSION[$cCode]["main"]["pph23Methode__tarif"] = 0.5;
            $_SESSION[$cCode]["main"]["pph23Methode__label"] = "suket PP 55/2022";
            $_SESSION[$cCode]["main"]["pph23Methode__name"] = "suket PP 55/2022";
            $_SESSION[$cCode]["main"]["pph__tarif"] = 0.5;
        }
...
// END OF COMPLETE REPEATED LOGIC
                ...
                if (isset($_SESSION[$cCode]["main"][$key_deteksi])) {
                    ...
                    $label_info_pph = str_replace("{pihakName}", $_SESSION[$cCode]["main"]["pihakName"], $label_info_pph);
                    ...
                    // Rincian Berjenjang Khusus PPh 21 UU HPP (Hanya ID 78 dan 79)
                    $html_rincian_pph21_progresif = NULL;
                    if ($id_biaya == 78 || $id_biaya == 79) {
                        $this->load->helper("he_pph21_progresif");
                        $total_cashback_bruto = 0;
                        if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items'])) {
                            foreach ($_SESSION[$cCode]['items'] as $item_spec) {
                        ...
                        if ($total_cashback_bruto > 0) {
                            $has_fasilitas = ($id_biaya == 78);
                            $res_pph = hitung_pph21_progresif_uuhpp($total_cashback_bruto, $has_fasilitas);

                            // Synchronize session items & view items array
                            $tarif_efektif_total = $res_pph["tarif_efektif_total"];
                            if (isset($_SESSION[$cCode]["items"]) && is_array($_SESSION[$cCode]["items"])) {
                                foreach ($_SESSION[$cCode]["items"] as $ii => $item_spec) {
                                    $harga = isset($item_spec["harga"]) ? floatval($item_spec["harga"]) : 0;
                                    $_SESSION[$cCode]["items"][$ii]["tarif_efektif_total"] = $tarif_efektif_total;
                                    $_SESSION[$cCode]["items"][$ii]["pph__tarif"]          = $tarif_efektif_total;
                                    $_SESSION[$cCode]["items"][$ii]["pph21Methode__tarif"] = $tarif_efektif_total;
                                    $_SESSION[$cCode]["items"][$ii]["nilai_pph21"]         = ($tarif_efektif_total / 100) * $harga;
                                    $_SESSION[$cCode]["items"][$ii]["nilai_pph_original"] = ($tarif_efektif_total / 100) * $harga;
                                    $_SESSION[$cCode]["items"][$ii]["nilai_kas_cn"]        = $harga - $_SESSION[$cCode]["items"][$ii]["nilai_pph21"];
                                }
                            }
                            if (isset($items) && is_array($items)) {
                                foreach ($items as $idx => $item_spec) {
                                    $harga = isset($item_spec["harga"]) ? floatval($item_spec["harga"]) : 0;
                                    $items[$idx]["tarif_efektif_total"] = $tarif_efektif_total;
                                    $items[$idx]["pph__tarif"]          = $tarif_efektif_total;
                                    $items[$idx]["pph21Methode__tarif"] = $tarif_efektif_total;
                                    $items[$idx]["nilai_pph21"]         = ($tarif_efektif_total / 100) * $harga;
                                    $items[$idx]["nilai_pph_original"] = ($tarif_efektif_total / 100) * $harga;
                                    $items[$idx]["nilai_kas_cn"]        = $harga - $items[$idx]["nilai_pph21"];
                                }
                            }

                            $html_rincian = "<div style='border: 1px solid #007bff; background: #ffffff; border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); margin-bottom: 5px; overflow: hidden;'>";
                            $html_rincian .= "<div style='font-weight: bold; font-size: 13px; padding: 7px 12px; background: #007bff; color: #ffffff; text-transform: uppercase;'>Rincian Perhitungan PPh 21 Progresif UU HPP</div>";
                            $html_rincian .= "<table class='table table-bordered table-striped' style='margin: 0; font-size: 13px; background: #ffffff; width: 100%;'>";
                            $html_rincian .= "<thead><tr style='background: #e9ecef;'><th class='text-center' style='padding: 6px 10px;'> Tarif</th><th class='text-center' style='padding: 6px 10px;'>Bruto </th><th class='text-center' style='padding: 6px 10px;'>Tarif Efektif</th><th class='text-center' style='padding: 6px 10px;'>Nominal PPh 21</th></tr></thead><tbody>";

                            foreach ($res_pph['detail'] as $d_layer) {
                                $html_rincian .= "<tr>";
                                $html_rincian .= "<td style='padding: 6px 10px; font-weight: 600;'>" . $d_layer['label'] . "</td>";
                                $html_rincian .= "<td class='text-right' style='padding: 6px 10px;'>" . number_format($d_layer['bruto_layer'], 0, ',', '.') . "</td>";
                                $html_rincian .= "<td class='text-center' style='padding: 6px 10px; font-weight: 600;'>" . ($d_layer['tarif_efektif'] * 100) . "%</td>";
                                $html_rincian .= "<td class='text-right' style='padding: 6px 10px; font-weight: 600; color: #d9534f;'>" . number_format($d_layer['pph_layer'], 0, ',', '.') . "</td>";
                                $html_rincian .= "</tr>";
                            }

                            $html_rincian .= "<tr style='font-weight: bold; background: #d6d8d9; border-top: 2px solid #6c757d;'>";
                            $html_rincian .= "<td style='padding: 8px 10px; font-size: 13px;'>TOTAL PPH 21</td>";
                            $html_rincian .= "<td class='text-right' style='padding: 8px 10px; font-size: 13px;'>" . number_format($res_pph['bruto'], 0, ',', '.') . "</td>";
//                            $html_rincian .= "<td class='text-center' style='padding: 8px 10px; font-size: 13px; color: #007bff;'>" . number_format($res_pph['tarif_efektif_total'], 2, ',', '.') . "%</td>";
                            $html_rincian .= "<td class='text-center' style='padding: 8px 10px; font-size: 13px; color: #007bff;'>-</td>";
                            $html_rincian .= "<td class='text-right' style='padding: 8px 10px; font-size: 13px; color: #d9534f;'>" . number_format($res_pph['total_pph'], 0, ',', '.') . "</td>";
                            $html_rincian .= "</tr>";
                            $html_rincian .= "</tbody></table></div>";

                            $html_rincian_pph21_progresif = $html_rincian;
                        }
                    }
                }
            }
        ...
        //-------------------------------------------------------------------------------
        $freelancerOptionInfo = isset($this->configUi[$this->jenisTr]['freelancerOptionInfo']) ? $this->configUi[$this->jenisTr]['freelancerOptionInfo'] : array();
        if (sizeof($freelancerOptionInfo) > 0) {
            $freelancerOptionKey = isset($_SESSION[$cCode]["main"]["freelancerOption"]) ? $_SESSION[$cCode]["main"]["freelancerOption"] : 0;
            $freelancerOptionInfoLabel = isset($freelancerOptionInfo[$freelancerOptionKey]) ? $freelancerOptionInfo[$freelancerOptionKey] : NULL;
        }
        //-------------------------------------------------------------------------------
        ...
        if (sizeof($freelancerData) > 0) {
            if ($freelancerData["optionFreelancerID"] == $_SESSION[$cCode]["main"]["optionFreelancerID"]) {
                ...
                $freelancerValueValidator = isset($this->configUi[$this->jenisTr]['freelancerValueValidator']) ? $this->configUi[$this->jenisTr]['freelancerValueValidator'] : array();
                if (sizeof($freelancerValueValidator) > 0) {
                    if (isset($freelancerValueValidator["enabled"]) && ($freelancerValueValidator["enabled"] == true)) {
                        $source = $_SESSION[$cCode]["main"][$freelancerValueValidator["source"]];
                        $target = $_SESSION[$cCode]["main"][$freelancerValueValidator["target"]];
                        $selisih = $source - $target;
                        $selisih = ($selisih < 0) ? ($selisih * -1) : $selisih;
                        if ($selisih > 100) {
                            $freelancerMark = array(
//                                "bgcolor" => "red",
                                "bgcolor" => "white",
                            );
                        }
                ...
                $new_key = 1;
                if (!isset($_SESSION[$cCode][$freelancerGate]) || (sizeof($_SESSION[$cCode][$freelancerGate]) == 0)) {
                    foreach ($freelancerHeaders as $key => $label) {
                        $_SESSION[$cCode][$freelancerGate][$new_key][$key] = "";
                        $_SESSION[$cCode][$freelancerGate][$new_key]["qty"] = 1;
                        $_SESSION[$cCode][$freelancerGate][$new_key]["jml"] = 1;
                    }
                }
        ...
        //-------------------------------------------------------------------------------
        $shoppingCartCashbackAddAction = "";
        $shoppingCartCashbackAdd = isset($this->configUi[$this->jenisTr]["shoppingCartCashbackAdd"][1]) ? $this->configUi[$this->jenisTr]["shoppingCartCashbackAdd"][1] : array();
        if ((sizeof($shoppingCartCashbackAdd) > 0) && ($_SESSION[$cCode]["main"]["type_cashback"] == "barang")) {
            $mdlName = $shoppingCartCashbackAdd["mdlName"];
            ...
            if (sizeof($mmTmp) > 0) {
                foreach ($mmTmp as $mmSpec) {
                    $fields[$mmSpec->id] = $mmSpec->nama;
                }
            }
            $shoppingCartCashbackAdd["fields"] = $fields;
            $shoppingCartCashbackAdd["selectedType_valas"] = isset($_SESSION[$cCode]["main"][$shoppingCartCashbackAdd["key"]]) ? $_SESSION[$cCode]["main"][$shoppingCartCashbackAdd["key"]] : 0;
            $shoppingCartCashbackAddAction = MODUL_PATH . $shoppingCartCashbackAdd["selector"] . "/" . $this->jenisTr;
            $labelPajak = $shoppingCartCashbackAdd["labelPajak"];
            $labelPajak = str_replace("{nilai_pajak}", number_format($_SESSION[$cCode]["main"]["nilai_pph_original"], 2, ",", "."), $labelPajak);
        }

        $shoppingCartCashbackAddProductAction = "";
        ...
        if ((sizeof($shoppingCartCashbackAddProduct) > 0) && (isset($_SESSION[$cCode]["main"]["type_cashback"]) && $_SESSION[$cCode]["main"]["type_cashback"] == "produk")) {
            $mdlName = $shoppingCartCashbackAddProduct["mdlName"];
            $this->load->model("Mdls/$mdlName");
            $mp = New $mdlName();
            $shoppingCartCashbackAddProduct["selectedType_produk"] = isset($_SESSION[$cCode]["main"][$shoppingCartCashbackAddProduct["key"]]) ? $_SESSION[$cCode]["main"][$shoppingCartCashbackAddProduct["key"]] : 0;
            $fields = array();
            if ($shoppingCartCashbackAddProduct["selectedType_produk"]) {
                $mpTmp = $mp->lookupByID($shoppingCartCashbackAddProduct["selectedType_produk"])->result();
                if (sizeof($mpTmp) > 0) {
                    $pcode = isset($mpTmp[0]->kode) ? "[" . $mpTmp[0]->kode . "] " : "";
                    $fields[$mpTmp[0]->id] = $pcode . $mpTmp[0]->nama;
                }
            }
            $hak_akses = availHakAkses("7468", 1);
            $shoppingCartCashbackAddProduct["fields"] = $fields;
            $shoppingCartCashbackAddProductAction = MODUL_PATH . $shoppingCartCashbackAddProduct["selector"] . "/" . $this->jenisTr;
            $labelPajak = $shoppingCartCashbackAddProduct["labelPajak"];
            $labelPajak = str_replace("{nilai_pajak}", number_format($_SESSION[$cCode]["main"]["nilai_pph_original"], 2, ",", "."), $labelPajak);
            $labelPajak = str_replace("{hak_akses_uang_pajak}", $hak_akses, $labelPajak);

        }
        if (isset($_SESSION[$cCode]["main"]["freelancerOption"]) && ($_SESSION[$cCode]["main"]["freelancerOption"] == 1)) {//MEMBER PENERIMA KOMISI
            $resetEl = array("kompensasiMethod", "cash_account");
            foreach ($resetEl as $reset_el) {
                $elements[$reset_el] = NULL;
                $elementConfigs[$reset_el] = NULL;
                unset($elements[$reset_el]);
                unset($elementConfigs[$reset_el]);
            }
        }
        ...
        $data = array(
            ...
            "pairedItemBreakdown" => $pairedItemBreakdown,
            "pairedItemBreakdownEnabled" => isset($pairedItemBreakdownEnabled) ? $pairedItemBreakdownEnabled : false,
            ...
            "html_rincian_pph21_progresif" => isset($html_rincian_pph21_progresif) ? $html_rincian_pph21_progresif : NULL,
            ...
            "freelancerShowData" => isset($_SESSION[$cCode][$freelancerData["gate"]]) ? $_SESSION[$cCode][$freelancerData["gate"]] : array(),
...
//            "freelancerTambahBaris" => MODUL_PATH . $freelancerData["link_baris"] . "/" . $this->jenisTr,
            ...
            //-------------------
            "shopingCartCashbackType" => isset($this->configUi[$this->jenisTr]["shoppingCartCashbackType"][1]) ? $this->configUi[$this->jenisTr]["shoppingCartCashbackType"][1] : array(),
            "shoppingCartCashbackFinishing" => isset($this->configUi[$this->jenisTr]["shoppingCartCashbackFinishing"][1]) ? $this->configUi[$this->jenisTr]["shoppingCartCashbackFinishing"][1] : array(),
            "shoppingCartCashbackAdd" => $shoppingCartCashbackAdd,
            "shoppingCartCashbackAddAction" => $shoppingCartCashbackAddAction,
            "shoppingCartCashbackAddProduct" => $shoppingCartCashbackAddProduct,
            "shoppingCartCashbackAddProductAction" => $shoppingCartCashbackAddProductAction,
            "labelPajak" => isset($labelPajak) ? $labelPajak : NULL,
            "cCode" => $cCode,
        );
        ...
        if (sizeof($pairedItem) > 0) {
            if (isset($pairedItem['enabled']) && ($pairedItem['enabled'] == true)) {
                    ...
                    foreach ($tmp as $row) {
                        if (sizeof($srcLabel) > 0) {
                            foreach ($srcLabel as $label) {
                                ...
                                $cat_nama = isset($row->cat_nama) ? $row->cat_nama : "";
                                ...
                                $selItemsCatNama[$row->$srcKey] = $cat_nama;
                            }
                        }
                    }
                }
                ...
                $data['selItemsCatNama'] = $selItemsCatNama;
    ...
    public function getStokCache()
    {
        $tbl_0 = "_rek_pembantu_supplies_cache";
        $wheres = array(
            // "id" => $transaksi_id,
            "periode" => "forever",
            "qty_debet>" => "0",
        ...
        $srcs_2 = $this->db->get($tbl_0)->result_array();
        // showLast_query("hijau");
// arrPrintKuning($srcs_2);
        $datas = array();
        foreach ($srcs_2 as $item) {
    ...
    public function recordUsedFieldElement()
    {
        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;
        $elName = $this->uri->segment(5);
        $src = $this->uri->segment(7);
        $val = ($_GET['val']);
        $elementConfigs = isset($this->configUi[$jenisTr]['receiptElements']) ? $this->configUi[$jenisTr]['receiptElements'] : array();
        $relElementConfigs = isset($this->configUi[$jenisTr]['relativeElements']) ? $this->configUi[$jenisTr]['relativeElements'] : array();
        $configUiJenis = $this->configUi[$this->jenisTr];
        ...
        heRecordUsedElement_modul($this->jenisTr, $elName, $src, $val, $configUiJenis);
        $id = 0;
        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($configUiJenis);
    ...
    public function recordAddRow()
    {
        ...
        switch ($rowName) {
            case "valas__qty":
                if ($val > $_SESSION[$cCode]["main"]["valas__saldo"]) {
                    $msg = "Jumlah valas " . $_SESSION[$cCode]["main"]["selectedType_valas_nama"] . " yang anda isikan melebihi stok. Silahkan dikoreksi lagi. code: " . __LINE__;
                    mati_disini($msg);
                }
                break;
        }
...
//            $_SESSION[$cCode]["items2"][$iID] = null;
//            unset($_SESSION[$cCode]["items2"][$iID]);
//        }
...
//                $id = $row->id;
                ...
                if (!array_key_exists($val, $_SESSION[$cCode]["items2"][$iID])) {
                    $tmp = array(
                        "handler" => $this->uri->segment(1) . "/" . $handler2,
                        "editTarget" => MODUL_PATH . $handler2 . "/" . $this->uri->segment(4),
                        "id" => $iID,
                        "biaya_dasar_id" => $val,
                        "biaya_dasar_nama" => $row->nama,
                        "biaya_id" => $iID,
                        "biaya_nama" => $_SESSION[$cCode]["items"][$iID]["nama"],
                        "nama" => $row->nama,
...
//                        $h->addFilter("jenis_value in ('" . implode("','", $priceConfig['label']) . "')");
                    ...
                    $_SESSION[$cCode]["items2"][$iID][$val] = $tmp;
                    //endregion
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        ...
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                $newValue = $_GET[$key];
                                $tmp[$key] = $newValue;
                                $_SESSION[$cCode]["items2"][$iID][$key] = $newValue;
                        ...
                        foreach ($itemNumLabels as $key => $label) {
                            $_SESSION[$cCode]["items2"][$iID]["sub_" . $key] = ($_SESSION[$cCode]["items2"][$iID][$key] * $_SESSION[$cCode]["items2"][$iID]["jml"]);
                        }
                        $_SESSION[$cCode]["items2"][$iID]['sub_nett'] = ($_SESSION[$cCode]["items2"][$iID]['nett'] * $_SESSION[$cCode]["items2"][$iID]['jml']);
                        $_SESSION[$cCode]["items2"][$iID]['subtotal'] = ($_SESSION[$cCode]["items2"][$iID]['jml'] * $_SESSION[$cCode]["items2"][$iID]['harga']);
                    }
                }
        ...
        if ($valValue != null && $valCol != null) {
            $_SESSION[$cCode]["items2"][$iID][$valCol] = $valValue;
        }
...
//        $vg->setPpnFactor($ppnFactor);
    ...
    public function tambahBarisFreelancer()
    {
        $cCode = $this->cCode;
        $freelancerData = isset($this->configUi[$this->jenisTr]['freelancerData']) ? $this->configUi[$this->jenisTr]['freelancerData'] : array();
        $freelancerHeaders = $freelancerData["headers"];
        ...
        if (isset($_SESSION[$cCode][$freelancerGate])) {
            $arrKeys = array_keys($_SESSION[$cCode][$freelancerGate]);
...
//        mati_disini(__LINE__ . " || [$last_key] [$new_key]");

        if (!isset($_SESSION[$cCode][$freelancerGate][$new_key])) {
            foreach ($freelancerHeaders as $key => $label) {
                $_SESSION[$cCode][$freelancerGate][$new_key][$key] = "";
                $_SESSION[$cCode][$freelancerGate][$new_key]["qty"] = 1;
                $_SESSION[$cCode][$freelancerGate][$new_key]["jml"] = 1;
        ...
        echo "<script>";
        echo "top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?epreketek=yes&populate=1');";
        echo "</script>";

    }

    public function recordFieldAdditional()
    {
        ...
        switch ($rowName) {
            case "nilai_kas_cn_detail":
                $_SESSION[$cCode]['items4_sum'][$id][$rowName] = $val;
                if (isset($_SESSION[$cCode]["main"]["type_cashback"]) && ($_SESSION[$cCode]["main"]["type_cashback"] == "barang")) {
                    $valas_qty = $_SESSION[$cCode]["main"]["valas__qty"];
                    $nilai_kas_cn = $_SESSION[$cCode]["main"]["nilai_kas_cn"];
                    $valas_qty_detail = ($val / $nilai_kas_cn) * $valas_qty;
                    $_SESSION[$cCode]['items4_sum'][$id]["nilai_valas_qty_detail"] = $valas_qty_detail;
                    $_SESSION[$cCode]['items4_sum'][$id]["nilai_valas_detail"] = $val;
                }
                if (isset($_SESSION[$cCode]["main"]["type_cashback"]) && ($_SESSION[$cCode]["main"]["type_cashback"] == "produk")) {// hadiah produk...
                    $_SESSION[$cCode]['items4_sum'][$id]["cashbackCheck"] = $_SESSION[$cCode]["main"]["cashbackCheck"];
                    $_SESSION[$cCode]['items4_sum'][$id]["type_cashback"] = $_SESSION[$cCode]["main"]["type_cashback"];

                }
                break;
        }
//        mati_disini(__LINE__ . " || [$rowName] [$val] [$id]");
    ...
    public function removeBarisFreelancer()
    {
        ...
        if (isset($_SESSION[$cCode]["items4_sum"][$id])) {
            $_SESSION[$cCode]["items4_sum"][$id] = NULL;
            unset($_SESSION[$cCode]["items4_sum"][$id]);
        }
...
// START OF COMPLETE REPEATED LOGIC
    public function recordSuketFile()
    {
        $cCode = $this->cCode;
        $jenisTr = $this->jenisTr;
        $files = $_FILES['file'];
        ...
        if ($files['error'] == 0) {
            $cUrl_result = upload_image($files);
            $_SESSION[$cCode]['main']['file_suket'] = $cUrl_result->full_url;
        }
        ...
        $vg->setPpnFactor(my_ppn_factor());
        $initMasterValues = heInitMasterValues_he_cart($this->jenisTr, 1, $this->configUiJenis);
        $vg->buildValue($this->jenisTr, null, $initMasterValues, $this->modul);
...
// START OF COMPLETE REPEATED LOGIC
    public function searchProductAjax()
    {
        ...
        if (strlen($keyword) >= 3) {
            $this->db->limit(15);
            $mpTmp = $mp->lookupByKeyword($keyword)->result();
        }
        else {
            $this->db->limit(5);
        ...
        if (sizeof($mpTmp) > 0) {
            ...
            foreach ($mpTmp as $mpSpec) {
                $product_ids[] = $mpSpec->id;
            }

            $this->load->model("Mdls/MdlLockerStock");
            $mls = new MdlLockerStock();
            $stockNows = $mls->fetchStates(my_cabang_id(), my_gudang_id(), $product_ids);

            foreach ($mpTmp as $mpSpec) {
                $pcode = isset($mpSpec->kode) ? "[" . $mpSpec->kode . "] " : "";
                $stok = isset($stockNows[$mpSpec->id]["active"]) ? (float)$stockNows[$mpSpec->id]["active"] : 0;
                $satuan = (isset($mpSpec->satuan) && strlen($mpSpec->satuan) > 0) ? $mpSpec->satuan : "unit";

                $color = ($stok >= 10) ? '#0056cd' : '#dd3300'; // Biru jika stok >= 10, Merah jika stok < 10

                if ($stok > 0) {
                    $results[] = array(
                        'id' => $mpSpec->id,
                        'text' => $pcode . $mpSpec->nama . " (" . $stok . " " . $satuan . ")",
                        'text_nama' => $pcode . $mpSpec->nama,
                        'text_stok' => $stok . " " . $satuan,
                        'color' => $color,
                        'disabled' => false
                    );
                }
                    ...
                    $results[] = array(
                        'id' => $mpSpec->id,
                        'text' => $pcode . $mpSpec->nama . " (0 " . $satuan . " - Silahkan minta distribusi stok dari pusat)",
                        'text_nama' => $pcode . $mpSpec->nama,
                        'text_stok' => "0 " . $satuan,
                        'color' => '#dd3300',
                        'tooltip' => 'Silahkan minta distribusi stok dari pusat',
                        'disabled' => true
                    );
                }
            }
