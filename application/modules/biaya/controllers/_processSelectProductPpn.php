...
//        }
    ...
    public function cashback()
    {
        ...
        $this->load->library("FieldCalculator");
        $cal = new FieldCalculator();
        $val = $_GET['val'];// berisi 0 atau 1
        $name = $_GET['name'];
        $key_check = $_GET['key'];
        $cCode = $this->cCode;
        $cCode = "_TR_" . $this->jenisTr;
        $resetGerbangCashback = isset($this->configUi[$this->jenisTr]['resetGerbangCashback'][$val]) ? $this->configUi[$this->jenisTr]['resetGerbangCashback'][$val] : array();
        if(sizeof($resetGerbangCashback)>0){
            foreach ($resetGerbangCashback as $reset){
                $_SESSION[$cCode]["main"][$reset] = NULL;
                unset($_SESSION[$cCode]["main"][$reset]);
            }
        }
        if ($val != 3) {
            $_SESSION[$cCode]["items10_sum"] = NULL;
            unset($_SESSION[$cCode]["items10_sum"]);
        }
        //-----------------------------------------------------

        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("undefine ppn factor, please logout and login again. code: " . __LINE__);
        $_SESSION[$cCode]["main"][$key_check] = $val;
        $_SESSION[$cCode]["main"]["type_cashback"] = $name;
...
//        }
    ...
    public function finishing()
    {
        ...
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("undefine ppn factor, please logout and login again. code: " . __LINE__);
        $_SESSION[$cCode]["main"][$key_check] = $val;
        $_SESSION[$cCode]["main"]["finishing_label"] = $name;
