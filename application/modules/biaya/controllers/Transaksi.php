...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $pakai_ini = 0;
        // Validasi kelengkapan scan nomor serial untuk transaksi Cashback Penjualan (6677)
        switch ($this->jenisTr) {
            case "6677":
            case "66772":
                $kodee = ($this->jenisTr == "6677") ? "hadiah" : "rebate";
                if ($pakai_ini == 1) {
                    if (isset($_SESSION[$cCode]["items10_sum"]) && is_array($_SESSION[$cCode]["items10_sum"])) {
                        $this->load->model("Mdls/MdlProduk");
                        $mp_val = new MdlProduk();
                        foreach ($_SESSION[$cCode]["items10_sum"] as $product_id => $item) {
                            $mpTmp = $mp_val->lookupByID($product_id)->result();
                            if (sizeof($mpTmp) > 0) {
                                $jml_serial = isset($mpTmp[0]->jml_serial) ? intval($mpTmp[0]->jml_serial) : 0;
                                if ($jml_serial > 0) {
                                    $qty = isset($item['qty']) ? intval($item['qty']) : 1;
                                    $required_serial_count = $qty * $jml_serial;

                                    $scanned_count = 0;
                                    if (isset($_SESSION[$cCode]['items9_sum']) && is_array($_SESSION[$cCode]['items9_sum'])) {
                                        foreach ($_SESSION[$cCode]['items9_sum'] as $scan) {
                                            if (isset($scan['id']) && $scan['id'] == $product_id) {
                                                $scanned_count++;
                                            }
                                        }
                                    }

                                    if ($scanned_count < $required_serial_count) {
                                        $product_name = isset($item['nama']) ? $item['nama'] : "Produk";
                                        $errMsgs[] = "Nomor serial untuk produk <b>" . htmlspecialchars($product_name) . "</b> belum lengkap (Di-scan: $scanned_count / $required_serial_count).";
                                    }
                                }
                            }
                        }
                    ...
                    if (isset($_SESSION[$cCode]["main"]["type_cashback"]) && ($_SESSION[$cCode]["main"]["type_cashback"] == "produk")) {
                        if (isset($_SESSION[$cCode]["items10_sum"]) && is_array($_SESSION[$cCode]["items10_sum"])) {
                            if (sizeof($_SESSION[$cCode]["items10_sum"]) == 0) {
                                $msg = "Barang $kodee WAJIB dipilih sebelum melanjutkan. code: " . __LINE__;
                                $errMsgs[] = $msg;
                            }
                        }
                        else {
                    ...
                    if ((isset($iSpec['disabled']) && $iSpec['disabled'] == "0") || !isset($iSpec['disabled'])) {
                        ...
                        foreach ($fieldValidatorRules as $field => $label) {
                            ...
                            if (!is_numeric($iSpec[$field])) {
                                $errMsgs[] = "$label harus berupa angka.";
                                $errLines[] = $id;
                                $errFields[$id][] = $field;
                            }
        ...
        if (sizeof($freelancerValueValidator) > 0) {
            if (isset($freelancerValueValidator["enabled"]) && ($freelancerValueValidator["enabled"] == true)) {
                if ($freelancerValueValidator["optionFreelancerID"] == $_SESSION[$cCode]["main"]["optionFreelancerID"]) {
                    ...
                    if ($selisih > 100) {
                        $msg = "Anda memilih Freelancer Member/Terdaftar. ";
                        $msg .= "Pastikan jumlah total komisi masing-masing Freelancer sama dengan jumlah total komisi.";
                        mati_disini($msg . " code: " . __LINE__);
                    }
                }
                if (isset($_SESSION[$cCode]["items4_sum"])) {
                    foreach ($_SESSION[$cCode]["items4_sum"] as $ii => $iiSpec) {
                        $ii_id = $iiSpec["id"];
                        $ii_nilai = $iiSpec["nilai_kas_cn_detail"];
                        if ($ii_id == NULL) {
                            $msg = "Freelancer wajib dipilih. Bila belum ada silahkan tambah data freelancer dulu.";
                            mati_disini($msg . " code: " . __LINE__);
                        }
                        if ($ii_nilai == NULL) {
                            $msg = "Nilai komisi masing-masing freelancer wajib diisi.";
                            mati_disini($msg . " code: " . __LINE__);
                        }
                    }
                }
        ...
        if (!isset($_SESSION[$cCode][$gateNameDetail]) || sizeof($_SESSION[$cCode][$gateNameDetail]) == 0) {
            ...
            $msg = "Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu and masukkan data yang benar supaya transaksi bisa dilanjutkan. @" . __LINE__ . __METHOD__;
            mati_disini($msg);
        }
        else {
            foreach ($_SESSION[$cCode][$gateNameDetail] as $spec) {
                if (!isset($spec['valid_qty'])) {
                    ...
                    $msg = "Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu and masukkan data yang benar supaya transaksi bisa dilanjutkan. $hidden_msg";
                    mati_disini($msg);
                }
                else {
                    if ($spec['valid_qty'] == 0) {
...
//            $linkMultiOtorisasi = MODUL_PATH . "FollowUp/followupMultiPrePreview/2677/2/1";
            $bottomBtn = "<button class='btn btn-warning' onclick=\"top.open_holdon();$linkMultiOtorisasi\">approve request</button>";
        }
...
//                                $link_edit = MODUL_PATH . "FollowUp/doPreEdit/" . $this->jenisTr . "/$transaksiID_edit/$currentStepNum?mode=autoPrePreview";
                            ...
                            $actionLabel_edit = "Edit";
                            $tmp['action'] .= "<button disabled class='btn btn-danger btn-block btn-xs' title='turn this entry into $stepLabel' href='javascript:void(0)' 
                                style='background-color:#000000;color:#ffffff;'>" . $actionLabel_edit . " </button>";
                        }
//                        }
                    }
                }
...
//                                $link_reject = MODUL_PATH . "FollowUp/doRevert/" . $this->jenisTr . "/$transaksiID_reject/$masterRevertStep/$childRevertStep/$currentStepNum?mode=autoPrePreview";
                                ...
                                $actionLabel_edit = "Reject 1 step/Cancel/Undo";
                                $tmp['action'] .= "<button class='btn btn-danger btn-block btn-xs' title='turn this entry into $stepLabel' href='javascript:void(0)' 
                                $disabled_hold $disabled_button_reject
...
//                                $link_reject_all = MODUL_PATH . "FollowUp/doRevertAll/" . $this->jenisTr . "/$transaksiID_reject/$currentStepNum?mode=autoPrePreview";
                                ...
                                $actionLabel_edit = "Reject all step/Void";
                                $tmp['action'] .= "<button class='btn btn-danger btn-block btn-xs' title='turn this entry into $stepLabel' href='javascript:void(0)' 
                                style='background-color:#000000;color:#ffffff;'
                                $disabled_hold $disabled_button_reject
                                onclick =\"document.getElementById('result').src='$link_reject_all'\">" . $actionLabel_edit . " </button>";
...
// matiHEre("hoop".__LINE__);
    ...
    public function index()
    {
        ...
        $arrJenisTrBiayaCek = array("2677", "2675", "3675", "3674", "16677", "16678");
        if (!in_array($jenisTr, $arrJenisTrBiayaCek)) {
            redirect(MODUL_PATH . "Create/index/" . $this->jenisTr);
        }
...
//                            arrPrint($tmpSpec);
                            ...
                            foreach ($stepHistoryFields as $fName => $fLabel) {
                                ...
                                if (is_numeric($tmpSpec->$fName)) {
                                    if (!isset($sumFooter[$fName])) {
                                        $sumFooter[$fName] = 0;
                                    }
                                    $sumFooter[$fName] += isset($tmpSpec->$fName) ? $tmpSpec->$fName : 0;
                                }
                            }
                            ...
                            if ($pakai_ini == 1) {
                                ...
                                $detail = viewDetailTransaksi($tmpReg_result[$tmpSpec->transaksi_id], $arrItemShow, $row->jenis_master);
                                $tblItems = $detail;
                            }
...
//        arrPrint($sumFooter);
        $data = array(
            "data" => $arrayOnProgressView,
            "footer" => isset($sumFooter) ? $sumFooter : array(),
        );
//        return $arrayOnProgressView;
    ...
    public function viewKepoinItems()
    {
        ...
        $jenisTr = $this->jenisTr;
        $shortKepoinLabel = isset($this->configUi[$jenisTr]['shortKepoinLabel']) ? $this->configUi[$jenisTr]['shortKepoinLabel'] : "transaksi belum selesai (belum disetujui kantor pusat)";
        $kepoinFields = isset($this->configUi[$jenisTr]['shortKepoinFields']) ? $this->configUi[$jenisTr]['shortKepoinFields'] : array();
        $connectTo = isset($this->configUi[$jenisTr]['connectTo']) ? $this->configUi[$jenisTr]['connectTo'] : "";
        ...
        if (strlen($connectTo) > 0) {
            ...
            // arrPrint($cek);
            // matiDisini(__LINE__);
            $arrayOnProgressView = $tmp2["data"];
            $sumFooter = $tmp2["footer"];
        }
