...
//         arrPrint($tmpTr);
                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "items2"://
                                    ...
                                    break;
                                case "items7":// serial
                                    $items7Registries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items2_sum"://
                                    $items2_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items3_sum"://
                                    ...
                                    break;
                                case "items4_sum"://
                                    $items4_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items5_sum"://
                                    $items5_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items8_sum"://
                                    $items8_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items9_sum"://
                                    $items9_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items10_sum"://
                                    $items10_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "main"://
                                    $main = unserialize(base64_decode($row->$param));
        ...
        $elements = $elements + $fixedElements + $companyProfile;
        $footer = isset($this->configLayout[$tmpTr[0]->jenis_master]['staticFooter'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['staticFooter'][$row->step_number] : "";
        $fixedElementsSwitch = isset($this->configLayout[$tmpTr[0]->jenis_master]['fixedElementsSwitch']) ? $this->configLayout[$tmpTr[0]->jenis_master]['fixedElementsSwitch'] : array();
        if (sizeof($fixedElementsSwitch) > 0) {
            $gate = $fixedElementsSwitch["gate"];
            $gateVal = isset($globalVars[$gate]) ? $globalVars[$gate] : NULL;
            if ($gateVal != NULL) {
                $fixedElementsReplace = $fixedElementsSwitch["key"][$gateVal];
            }
        ...
        //--------------------
        $receiptSumFieldsUi = isset($this->configUi[$tmpTr[0]->jenis_master]['shoppingCartSumFields'][$row->step_number]) ? $this->configUi[$tmpTr[0]->jenis_master]['shoppingCartSumFields'][$row->step_number] : array();
        ...
        $pajakOption = isset($main["pajakOption"]) ? $main["pajakOption"] : "";
        if ($pajakOption == "pph21") {
            $title = $this->configUi[$jenisTr]["steps"][$currentStepNum]["label_nota21"];
        }
        elseif ($pajakOption == "pph23") {
            $title = $this->configUi[$jenisTr]["steps"][$currentStepNum]["label_nota23"];
        }
        elseif ($pajakOption == "pph23_15") {
            $title = $this->configUi[$jenisTr]["steps"][$currentStepNum]["label_nota23"];
        }
        else {
            $title = $this->configUi[$jenisTr]["steps"][$currentStepNum]["label"];
        ...
        $itemLabels2 = isset($this->configLayout[$tmpTr[0]->jenis_master]["itemLabels2"]) ? $this->configLayout[$tmpTr[0]->jenis_master]["itemLabels2"] : array();
        //-------------------------------------------------------------------------------
        ...
        if (sizeof($freelancerData) > 0) {
            if ($freelancerData["optionFreelancerID"] == $main["optionFreelancerID"]) {
                ...
                $freelancerShowData = isset($items4_sumRegistries) ? $items4_sumRegistries : array();
            }
        }
        ...
        $autoSelectPPh_check = isset($this->configUi[$tmpTr[0]->jenis_master]['autoSelectPPh']) ? $this->configUi[$tmpTr[0]->jenis_master]['autoSelectPPh'] : array();
        if (sizeof($autoSelectPPh_check) > 0 && isset($autoSelectPPh_check["enabled"]) && ($autoSelectPPh_check["enabled"] == true)) {
            ...
            if (isset($main[$key_deteksi_check])) {
                $id_biaya_check = $main[$key_deteksi_check];
                if ($id_biaya_check == 78 || $id_biaya_check == 79) {
                    unset($itemLabels['pph__tarif']);
                }
            }
        }
//arrPrint($itemLabels);
        $data = array(
            ...
            "items" => $items,
            // "items2" => isset($items2_sumRegistries) ? $items2_sumRegistries : $items2,
            "items2" => isset($items10_sumRegistries) ? $items10_sumRegistries : $items2,
            "itemsRegistries" => $itemsRegistries,
            "items7Registries" => $items7Registries,
...
//            "sumRows"                  => sizeof($receiptSumFieldsLayout) > 0 ? $receiptSumFieldsLayout : $receiptSumFieldsUi,
            ...
            "fixedElementsReplace" => isset($fixedElementsReplace) ? $fixedElementsReplace : array(),
...
//            "freelancerMark" => isset($freelancerMark) ? $freelancerMark : array(),
        ...
        $shoppingCartUuHpp = isset($this->configUi[$tmpTr[0]->jenis_master]["shoppingCartUuHpp"][$row->step_number]) ? $this->configUi[$tmpTr[0]->jenis_master]["shoppingCartUuHpp"][$row->step_number] : array();
        $data['shoppingCartUuHpp'] = $shoppingCartUuHpp;
        $data['items8_sum'] = isset($items8_sumRegistries) ? $items8_sumRegistries : array();
