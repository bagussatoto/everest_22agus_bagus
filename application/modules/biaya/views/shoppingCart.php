...
// END OF COMPLETE REPEATED LOGIC
        ...
        foreach ($items as $iSpec) {
            ...
            if ($showItems) {
                ...
                $allItemFields = array_merge((array)$itemLabels, isset($numLabels) ? (array)$numLabels : array());
                foreach ($allItemFields as $key => $label) {
                    $listMode[$key] = "input";
                    $keyupEvent[$key] = "";
                    $keyUpStr[$key] = "";
                    if (array_key_exists($key, $keyUpEvents)) {
                        ...
                        // Support both input.value and span.innerHTML/innerText in JS calculations
                        if (strlen($keyupEvent[$key]) > 0) {
                            $keyupEvent[$key] = preg_replace(
                                "/document\.getElementById\('([^']+)'\)\.value\s*=/",
                                "(function(id,val){var e=document.getElementById(id);if(e){if(e.tagName==='INPUT'||e.tagName==='TEXTAREA'){e.value=addCommas(val);}else{e.innerHTML=addCommas(val);}}})('$1',",
                                $keyupEvent[$key]
                            );
                            if (strpos($keyupEvent[$key], "(function(id,val)") !== false) {
                                $keyupEvent[$key] .= ")";
                            }
                        }
                    }
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            $combinedOnkeyup = "";
                            if (isset($keyUpStr[$key]) && strlen($keyUpStr[$key]) > 0) {
                                $combinedOnkeyup .= $keyUpStr[$key];
                            }
                            $combinedOnkeyup .= "if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }";
                            ...
                            value='" . niceDecimal($fieldVal) . "' onclick='this.select()' ";
                            ...
                            $pemicuGerbang .= "*onmouseout=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$queryParams';}\" ";
...
// END OF COMPLETE REPEATED LOGIC
                    ...
                    if (isset($html_rincian_pph21_progresif) && strlen($html_rincian_pph21_progresif) > 0) {
                        $left_cols = 5;
                        $rem_cols = sizeof($itemLabels) - $left_cols;
                        if ($rem_cols < 1) { $rem_cols = 1; }

                        if ($nr == 1) {
                            $total_sum_rows = sizeof($sumRows);
                            echo "<td colspan='$left_cols' rowspan='$total_sum_rows' valign='top' style='padding:4px; text-align:left;'>";
                            echo $html_rincian_pph21_progresif;
                            echo "</td>";
                        }
                        echo "<td colspan='$rem_cols' class='text-right $bottom_borderless valign-m text-uppercase'>$label</td>";
                    } else {
