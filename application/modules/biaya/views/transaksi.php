...
//            pageResize: true,
        ...
        if (sizeof($linkMenu) > 0) {
            foreach ($linkMenu as $ii => $iiSpec) {
                $arrLinkMenuAdd["link_" . $ii] = MODUL_PATH . $iiSpec["link"];
...
//    arrprint($scriptBottom);
        $propDisplay = "block";
        $altDisplay = "none";

        if (isset($showHiddenCode) && (sizeof($showHiddenCode) > 0)) {
//            arrPrint($showHiddenCode);
            $linkAction = MODUL_PATH . $showHiddenCode["targetAction"] . "$jenisTr/" . $showHiddenCode["targetKey"] . "/?state=";
            $checked = $showHiddenCodeCek == 1 ? "checked" : "";
            $strCode = "<input class='chRadio' type='checkbox' $checked
                    onclick=\"document.getElementById('result').src='$linkAction'+this.checked;\">";
            $strCode .= "&nbsp;&nbsp;<span style='font-size:15px;'>" . $showHiddenCode["label"] . "</span>";
...
//            cekHere($strCode);
        ...
        // opsi status freelancer MEMBER/NON MEMBER, bagian kiri
        $data_cabang_pos_ = "";
        if (count($data_cabang_pos) > 0) {
            $data_cabang_pos_ .= "<h4 style='text-transform: capitalize;'>$optionFreelancerLabel</h4>";
            $data_cabang_pos_ .= "<div id='data_cabang_pos' class='alert alert-warning text-left $visibility' sstyle='visibility:$visibility;'>";
            foreach ($data_cabang_pos as $data_cabang_pos_0) {
                $checked = $optionFreelancerIDSelected == $data_cabang_pos_0["id"] ? "checked" : "";
                $data_cabang_pos_ .= "<input $checked name='cabang'type='radio' id='freelancer_" . $data_cabang_pos_0["id"] . "' 
                    onclick=\"document.getElementById('result').src='" . $data_cabang_pos_0["target"] . "?id=" . $data_cabang_pos_0["id"] . "'\">
                    <label for='freelancer_" . $data_cabang_pos_0["id"] . "'>" . $data_cabang_pos_0["label"] . "</label>";
                $data_cabang_pos_ .= "<br>";
        ...
        $arrTags = array(
            ...
            //----
            "strCode" => isset($strCode) ? $strCode : "",
            "pihakRulesVisibility" => isset($rulesVisibility) ? $rulesVisibility : "",

            "data_cabang_pos" => $data_cabang_pos_,
            "pihak_disabled" => isset($pihak_disabled) ? $pihak_disabled : "",
            "pihak_visibility" => isset($pihak_visibility) ? $pihak_visibility : "",
            //----
            "time_line" => $time_line,
        );
        if (sizeof($arrLinkMenuAdd) > 0) {
...
// START OF COMPLETE REPEATED LOGIC
        echo "<style>
            .preview-font-sm, .preview-font-sm table, .preview-font-sm td, .preview-font-sm th {
                font-size: 11px !important;
            }
            .preview-font-md, .preview-font-md table, .preview-font-md td, .preview-font-md th {
                font-size: 13px !important;
            }
            .preview-font-lg, .preview-font-lg table, .preview-font-lg td, .preview-font-lg th {
                font-size: 16px !important;
            }
            .preview-font-xl, .preview-font-xl table, .preview-font-xl td, .preview-font-xl th {
                font-size: 19px !important;
            }
        </style>";

        echo "<div class='preview-controls no-print' style='margin-bottom: 15px; display: flex; align-items: center; background: #f9f9f9; padding: 10px; border: 1px solid #eee; border-radius: 4px;'>";
        echo "  <span style='font-weight: bold; margin-right: 15px; font-size: 12px; color: #555;'>Pilihan Tampilan:</span>";
        echo "  <div class='btn-group' role='group'>";
        echo "      <button type='button' id='btnViewNormal' onclick='changePreviewMode(\"normal\")' class='btn btn-primary btn-sm'>Normal</button>";
        echo "      <button type='button' id='btnViewWide' onclick='changePreviewMode(\"wide\")' class='btn btn-default btn-sm'>lebar+</button>";
        echo "      <button type='button' id='btnViewFont2' onclick='changePreviewMode(\"font2\")' class='btn btn-default btn-sm'>font++</button>";
        echo "      <button type='button' id='btnViewFont3' onclick='changePreviewMode(\"font3\")' class='btn btn-default btn-sm'>font+++</button>";
        echo "  </div>";
        echo "</div>";

        echo "<script>
        function changePreviewMode(mode) {
            var modalDialog = $('#btnViewNormal').closest('.modal-dialog');
            if (modalDialog.length === 0) {
                modalDialog = top.$('.modal-dialog');
            }
            var modalContent = $('#btnViewNormal').closest('.modal-content');
            if (modalContent.length === 0) {
                modalContent = top.$('.modal-content');
            }

            // Reset class active tombol
            // Reset modal size & font classes
            $('#btnViewNormal, #btnViewWide, #btnViewFont2, #btnViewFont3').removeClass('btn-primary').addClass('btn-default');
            modalDialog.removeClass('modal-lg modal-xl');
            modalContent.removeClass('preview-font-sm preview-font-md preview-font-lg preview-font-xl');

            if (mode === 'normal') {
                modalDialog.addClass('modal-lg');
                modalContent.addClass('preview-font-md');
                $('#btnViewNormal').removeClass('btn-default').addClass('btn-primary');
            } else if (mode === 'wide') {
                modalDialog.addClass('modal-xl');
                modalContent.addClass('preview-font-md');
                $('#btnViewWide').removeClass('btn-default').addClass('btn-primary');
            } else if (mode === 'font2') {
                modalDialog.addClass('modal-xl');
                modalContent.addClass('preview-font-lg');
                $('#btnViewFont2').removeClass('btn-default').addClass('btn-primary');
            } else if (mode === 'font3') {
                modalDialog.addClass('modal-xl');
                modalContent.addClass('preview-font-xl');
                $('#btnViewFont3').removeClass('btn-default').addClass('btn-primary');
            }
        }
        </script>";
...
//                }
            ...
            if (isset($items8_sum) && (sizeof($items8_sum) > 0)) {
                echo "<div style='margin-top: 20px; margin-bottom: 20px;'>";
                echo "<div style='font-weight: bold; text-align: center; background-color: #f5f5f5; border: 1px solid #ddd; border-bottom: none; padding: 6px; text-transform: uppercase; color: #333;'>Rincian Perhitungan PPh 21 Progresif UU HPP</div>";
                ...
                foreach ($shoppingCartUuHpp as $fieldKey => $fieldLabel) {
                    echo "<th style='color: #555;'>$fieldLabel</th>";
                }
                echo "</tr>";
                $no_p = 0;
                foreach ($items8_sum as $pid => $pSpec) {
                    $no_p++;
                    echo "<tr>";
                    echo "<td style='text-align: center; vertical-align: middle; font-weight: bold;'>$no_p</td>";
                    foreach ($shoppingCartUuHpp as $fieldKey => $fieldLabel) {
                        if(is_numeric($pSpec[$fieldKey])){
                            if(!isset($total_bawah[$fieldKey])){
                                $total_bawah[$fieldKey] = 0;
                            }
                            $total_bawah[$fieldKey] += $pSpec[$fieldKey];
                        }
                        if($fieldKey == "tarif_efektif"){
                            $fieldVal = isset($pSpec[$fieldKey]) ? $pSpec[$fieldKey] * 100 : "";
                        }
                        else{
                            $fieldVal = isset($pSpec[$fieldKey]) ? formatField_he_format($fieldKey, $pSpec[$fieldKey]) : "";
                        }
                ...
                echo "<tr class='bg-grey-2 text-uppercase' style='font-weight: bold;'>";
                echo "<th style='width: 5%; text-align: center; color: #555;'>-</th>";
                foreach ($shoppingCartUuHpp as $fieldKey => $fieldLabel) {
                    if(isset($total_bawah[$fieldKey])){
                        $nilai_bawah = formatField_he_format($fieldKey, $total_bawah[$fieldKey]);
                    }
                    else{
                        $nilai_bawah = "-";
                    }
                    ...
                    echo "<th style='color: #555;'>$nilai_bawah</th>";
                }
                echo "</tr>";
                echo "</table>";
                echo "</div>";
            ...
            if (isset($items10_sum) && (sizeof($items10_sum) > 0)) {
                ...
                echo "</tr>";
                $no_p = 0;
                foreach ($items10_sum as $pid => $pSpec) {
                    $no_p++;
                    echo "<tr>";
                    echo "<td style='text-align: center; vertical-align: middle; font-weight: bold;'>$no_p</td>";
                    foreach ($cashbackFields as $fieldKey => $fieldLabel) {
                    ...
                    echo isset($pSpec['qty']) ? htmlspecialchars($pSpec['qty']) : "1";
                    echo "</td>";
                    echo "</tr>";

// START OF COMPLETE REPEATED LOGIC
                    $cCode_tmp = isset($cCode) ? $cCode : (isset($this->cCode) ? $this->cCode : "");
                    if (empty($cCode_tmp) && isset($this->jenisTr)) {
                        $cCode_tmp = "_TR_" . $this->jenisTr;
                    }

                    if (isset($pSpec['jml_serial']) && $pSpec['jml_serial'] > 0) {
                        $serials = array();
                        $items9_sum_data = isset($items9_sum) ? $items9_sum : (isset($_SESSION[$cCode_tmp]['items9_sum']) ? $_SESSION[$cCode_tmp]['items9_sum'] : array());
                        if (is_array($items9_sum_data)) {
                            foreach ($items9_sum_data as $item) {
                                $match = false;
...
// START OF COMPLETE REPEATED LOGIC
                        if (sizeof($serials) > 0) {
                            echo "<tr>";
                            echo "<td>&nbsp;</td>";
                            echo "<td colspan='" . (sizeof($cashbackFields) + 1) . "' style='padding: 2px 15px;'>";
                            echo "<ol style='padding-left: 15px; margin: 0px; font-size: 11px; color: #555; font-family: monospace; font-weight: bold;'>";
                            foreach ($serials as $sNum) {
                                $sNum_esc = htmlspecialchars($sNum);
                                echo "<li style='margin-bottom: 2px;'>$sNum_esc</li>";
                            }
                            echo "</ol>";
                            echo "</td>";
                            echo "</tr>";
...
// END OF COMPLETE REPEATED LOGIC
            ...
            if (isset($freelancerShow) && ($freelancerShow == true)) {
                $str = "<div class='col-lg-12 no-padding text-left' >";
                $str .= "<h4 class='text-left'> $freelancerLabel </h4>";
                $str .= "<table class='table table-condensed'>";
                ...
                foreach ($freelancerShowData as $ii => $iiSpec) {
                    ...
                    $str .= "<td>$no</td>";
                    foreach ($freelancerHeaders as $key => $label) {
                        $valx = isset($iiSpec[$key]) ? formatField_he_format($key, $iiSpec[$key]) : 0;
                        $str .= "<td>$valx </td>";
...
//            }
            ...
            if (isset($items10_sum) && (sizeof($items10_sum) > 0)) {
                ...
                echo "<div style='margin-top: 20px; margin-bottom: 20px;'>";
                echo "<div style='font-weight: bold; text-align: center; background-color: #f5f5f5; border: 1px solid #ddd; border-bottom: none; padding: 6px; text-transform: uppercase; color: #333;'>Produk Cashback yang Dipilih</div>";
                echo "<table class='table table-condensed table-bordered no-margin' style='background-color: #fff;'>";
                echo "<tr class='bg-grey-2 text-uppercase' style='font-weight: bold;'>";
                echo "<th style='width: 5%; text-align: center; color: #555;'>No</th>";
                foreach ($cashbackFields as $fieldKey => $fieldLabel) {
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        if ($allowEdit == true && in_array($key, $editableFields)) {
                            ...
                            $strVal = "<input type=$inputType $disabled_field name='$key" . "_" . "$id' class='form-control text-right' value='" . number_format($val) . "' onclick='this.select()' $addEvent>";
                    ...
                    if (sizeof($items2[$id]) > 0) {
                        echo "<span id='btnToggle_$id' onclick=\"$('#child_$id').toggleClass('hidden');$('i', $(this)).toggleClass('fa-minus', 'fa-plus')\" title='buka untuk lihat details biaya' class='btn btn-xs btn-default'><i class='fa fa-plus'></i></span>";
                    }
                    ...
                    if ($allowEdit == true) {//==delete item
                        if ($allowRemove == false) {
                            ...
                            echo "<a class='pull-right' href='javascript:void(0)' onclick=\"document.getElementById('result').src='$removeItemTarget?id=$id&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='fa fa-remove text-danger'></span></a>";
                        }
                    }
                    ...
                    $colspanItms2 = count($itemLabels) + 1;

                    if (isset($items2[$id]) && sizeof($items2[$id]) > 0) {
                        echo "<tr id='child_$id' class='bg-info hidden'>";
                        echo "<td>-</td>";
                        echo "<td colspan='$colspanItms2'>";
                        echo "<div class=''>";
                        echo "<table class='table dataTable compact display table-bordered'>";
...
//                        echo "</tr>";
                        ...
                        foreach ($items2[$id] as $iSpec) {
                            ...
                            if ($allowEdit == true) {//==delete item
                                if ($allowRemove == false) {
                                    ...
                                    echo "<td>";
                                    echo "<a href='javascript:void(0)' onclick=\"document.getElementById('result').src='$removeItemTarget?mainid=$id&childid=$childID&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='glyphicon glyphicon-remove text-danger'></span></a>";
                                    echo "</td>";
                                }
                            }
                ...
                if (isset($sumRows) && sizeof($sumRows) > 0) {
                    ...
                    echo "<tr line=" . __LINE__ . ">";
                    echo "<td colspan='" . (count($colspanSum) + 2) . "'>&nbsp;</td>";
                    echo "</tr>";
                    ...
                    foreach ($sumRows as $key => $label) {
                        ...
                        echo "<tr line=" . __LINE__ . ">";
                        echo "<td colspan='" . count($colspanSum) . "' class='text-right'>$label</td>";
                        echo "<td class='text-right'>";
                        if (isset($mainValues[$key])) {
                            echo formatField($key, $mainValues[$key]);
                        }
...
//                            cekBiru("BAWAH");
                ...
                if ((isset($itemLabels2)) && (sizeof($itemLabels2) > 1)) {
                    if (isset($items2) && sizeof($items2) > 0) {
                        echo "<div class='table-responsive'>";
                        echo "<table class='table table-bordered table-condensed' border='1px'>";
                        echo "<tr bgcolor='#f5f5f5'>";
                        echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
                        foreach ($itemLabels2 as $key => $label) {
                            echo "<th class='text-muted' style='font-weight:bold;'>";
...
//                            arrPrintPink($iSpec2);
                            ...
                            foreach ($itemLabels2 as $key2 => $label2) {
                                ...
                                echo "<td>";
                                if (isset($items3[$ii][$key2])) {
                                    echo formatField($key2, $items3[$ii][$key2]);
                                }
                                else {
                                    echo "";
                                }
                            ...
                            echo "</tr>";
                            // ----- subItems milik items2
                            echo "<tr line=" . __LINE__ . ">";
                            echo "<td align='right'>&nbsp;</td>";
                            $colspan = count($itemLabels2) - 1;
                            echo "<td align='right' colspan='$colspan'>";

                            echo "<table class='table table-bordered table-condensed' style='margin-top:0px;'>";
                            echo "<tr bgcolor='#f5f5f5'>";
                            foreach ($itemSubLabels2 as $iikey2 => $iilabel2) {
                                echo "<th  class='text-muted' style='font-weight:bold;'>";
                                echo $iilabel2;
                                echo "</th>";
                            }
                            echo "</tr>";
                            foreach ($iSpec2 as $iiSpec2) {

                                echo "<tr>";
                                foreach ($itemSubLabels2 as $iikey2 => $iilabel2) {
                                    echo "<td>";
                                    if (isset($iiSpec2[$iikey2])) {
                                        echo formatField($iikey2, $iiSpec2[$iikey2]);
                                    }
                                    else {
                                        echo "";
                                    }
                        ...
                        if (isset($sumRows2) && sizeof($sumRows2) > 0) {
                            foreach ($sumRows2 as $key => $label) {
                                echo "<tr line=" . __LINE__ . ">";
                                echo "<td colspan='" . sizeof($itemLabels2) . "' class='text-right'>$label</td>";
                                echo "<td class='text-right'>";
                                if (isset($mainValues[$key])) {
                                    echo formatField($key, $mainValues[$key]);
                                }
...
//                }
    ...
    case "viewKepoinItems":
        ...
        if (sizeof($kepoinFields) > 0) {
            ...
            $str .= "<button type='button' class='btn btn-box-tool btn-primary' onclick=\"$('#kepoinList').load('$link_undone');\"><i class='fa fa-refresh'></i> refresh</button> &nbsp;";
            $str .= "<button type='button' class='btn btn-box-tool btn-warning' data-widget='collapse'><i class='fa fa-minus'></i></button>";
            $str .= "</div>";
            ...
            if (sizeof($arrayOnProgressView) > 0) {
                ...
                * ----------------------------------------*/
                ...
                $str .= "<table id='kepoin_items' class='table table-condensed table-hover-color-red table-striped'>";

                $str .= "<thead>";
                $str .= "<tr line=" . __LINE__ . " class='bg-danger text-uppercase'>";
                $str .= "<th class='bg-primary'>No.</th>";
...
//                        if(is_numeric($pSpec[$k])){
//                            if(!isset($sumFooter[$k])){
//                                $sumFooter[$k] = 0;
//                            }
//                            $sumFooter[$k] += isset($pSpec[$k]) ? $pSpec[$k] : 0;
//                        }
                ...
                $str .= "</tbody>";
//arrPrintWebs($sumFooter);
                $str .= "<tfoot>";
                if (isset($sumFooter) && sizeof($sumFooter) > 0) {
                    $str .= "<tr line=" . __LINE__ . " class='bg-grey-1 text-bold'>";
                    $str .= "<td>-</td>";
                    if (sizeof($kepoinFields) > 0) {
                        foreach ($kepoinFields as $key => $label) {
                            $str .= "<td>";
                            if (isset($sumFooter) && isset($sumFooter[$key])) {
                                $str .= formatField_he_format($key, $sumFooter[$key]);
                            }
                            else {
                                $str .= "-";
                            }
            ...
            $str .= "<script>
                    $(document).ready( function(){
                        var table = $('#kepoin_items').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: true,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
