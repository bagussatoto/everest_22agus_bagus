...
//                                        unset($_SESSION[$cCode]["main_elements"][$relName]['contents_intext']);
        ...
        $actionTarget = "top.BootstrapDialog.closeAll();top.BootstrapDialog.show(
                                   {
                                       title:'Followup preview',
                                       message: " . 'top.$' . "('<div></div>').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->jenisTr . "/" . $this->uri->segment(7) . "/" . $this->uri->segment(8) . "/" . $this->uri->segment(9) . "?rawBuilderURL=$rawBuilderURL'),
                                        size:top.BootstrapDialog.SIZE_WIDE,
                                        draggable:false,
                                        closable:true,
                                        type:top.BootstrapDialog.TYPE_SUCCESS,
        ...
        echo "<script>";
        echo "  if(top.document.getElementById('followupPreview')){";
        echo "  top.$('#followupPreview').load('" . MODUL_PATH . "FollowUp/followupPreview/" . $this->jenisTr . "/" . $this->uri->segment(7) . "/" . $this->uri->segment(8) . "/" . $this->uri->segment(9) . "?rawBuilderURL=$rawBuilderURL');";
        echo "  }";
        echo "  else{";
        echo " $actionTarget";
        echo "  }";
...
//                    showLast_query("biru");
                    ...
                    if ($eName == "cash_account") {
                        ...
                        $defValue = "";
                        if (isset($_SESSION[$cCode]['main_elements'][$eName]['key']) && isset($_SESSION[$cCode]['main_elements'][$eName]['contents'])) {
                        ...
                        $labelSrcFields = isset($eSpec['labelSrcFields']) ? $eSpec['labelSrcFields'] : array();
                        $allowElementEdit = isset($eSpec['allowEdit']) ? $eSpec['allowEdit'] : true;
                        if (sizeof($labelSrcFields) == 0) {
                            $labelSrcFields = array($labelSrc);
                        }

                        $disabled = $allowElementEdit == true ? "" : "disabled='disabled'";
                        if ($allowElementEdit == false) {
                            $editStr = "";
                            $addStr = "";
                        }
                        $arrBankRekening = array();
                        $elStr[$eName] .= "<div class='box-body'>";
                        if (sizeof($tmpo) > 0) {
                            foreach ($tmpo as $row) {
                                $arrBank[$row->folders] = isset($row->folders_nama) ? $row->folders_nama : "";
                                $labelValue = "";
                                foreach ($labelSrcFields as $col) {
                                ...
                                if ($labelValue == "") {
                                    $labelValue = isset($row->$labelSrc) ? $row->$labelSrc : "-";
                                }
                                ...
                                $arrBankRekening[$row->folders][] = "<input type='radio' name='" . $eName . "' value='" . $row->$keySrc . "' $selected $disabled onclick=\"top.$('#result').load($selectorTarget);\"> " . $labelValue;
                                $arrBankRekening[$row->folders][] = "</label><br>\n";
                            }
                        }
                        $elStr[$eName] .= "<div style='text-align: left;line-height:1.4;'>";
                        if (isset($arrBank) && (sizeof($arrBank) > 0)) {
                            foreach ($arrBank as $fid => $fnama) {
                                if ($fnama != NULL) {
                                    $elStr[$eName] .= "<span class='text-bold' style='font-size:15px;'>" . $fnama . "</span><br>";
                                }
