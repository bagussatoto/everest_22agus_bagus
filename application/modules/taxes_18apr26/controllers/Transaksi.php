                    ...
                    if (sizeof($inputFakturTransaksi) > 0) {
                        if (isset($inputFakturTransaksi[$row->next_substep_num]["enabled"]) && ($inputFakturTransaksi[$row->next_substep_num]["enabled"] == true)) {
                            if (isset($addMainSourceEditable[$fName])) {
                                ...
                                $tmp[$fName] = "<input id='$idd' type='$type_input'
                                    name='$fname' value='$defaultValue' 
                                    class='form-control text-right text-red text-bold bg-lime'>";
                            }
                        }
                    }
...
//        $this->db->where('per_customers.npwp', '');
        ...
        if (isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
            $this->db->group_start();
            $this->db->like('per_customers.npwp', $_POST['search']['value']);
            $this->db->or_like('transaksi.customers_nama', $_POST['search']['value']);
            $this->db->or_like('transaksi.dtime', $_POST['search']['value']);
            $this->db->or_like('transaksi.nomer', $_POST['search']['value']);
            $this->db->or_like('transaksi.nomer_top', $_POST['search']['value']);
            $this->db->or_like('transaksi.nomer2', $_POST['search']['value']);
            $this->db->group_end();
        }
        ...
        if (isset($_POST["order"][0]["column"]) && $_POST["order"][0]["column"] > 0) {
            $column_db = $keyConvert[$_POST["order"][0]["column"]];
            $directional = $_POST["order"][0]["dir"];
            $table = "transaksi";
            switch ($column_db) {
                case "npwp":
                    $table = "per_customers";
...
//                    $table = "transaksi";
            ...
            $this->db->order_by("$table.$column_db", $directional);
        }
        else {
            // $this->db->order_by('transaksi.id', 'asc');
        }
...
//        $tmpHist_0 = $tr->lookupRecentUndoneEntries_joined($sesionReplacer)->result();
        ...
        $items = $arrayOnprogressGroup[$step];
        $itemJml = count($items);
        $jmlPerhalaman = isset($_POST['length']) && $_POST['length'] != -1 ? $_POST['length'] : $itemJml;
//        $jmlPerhalaman = 100;
