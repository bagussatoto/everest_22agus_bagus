...
//            cekOrange("lolos validator @" . __LINE__ . " " . __METHOD__);
            ...
            $stop_mtime = microtime(true);
            $exec_mtime = $stop_mtime - $start_mtime;
            cekMerah("$mdlName:: exec time: <br>$stop_mtime<br>$start_mtime<hr>$exec_mtime");
            // matiHere(__LINE__);
            ...
            if (sizeof($iterator) > 0) {
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    cekBiru("kiriman komponem $mdlName pada " . dtimeNow());
                    // arrPrint($tmpOutParams);
...
//                $trr->addFilter("param='main'");
                $trr->setJointSelectFields("main, transaksi_id");
                ...
                $allowMultiSelect = isset($configUiMasterModulJenis['requestCode']['allowMultiSelect']) ? $configUiMasterModulJenis['requestCode']['allowMultiSelect'] : false;
                $oldMain = "";
                ...
                $targetProc = MODUL_PATH . $configUiMasterModulJenis['itemSwapper'] . "/$jenisTr/?items=" . base64_encode(serialize($items)) . "&main=" . base64_encode(serialize($oldMain)) . "&trs=" . base64_encode(serialize($trIDs));
        ...
        elseif (isset($_POST['prdID']) && sizeof($_POST['prdID']) > 0) {
            ...
            $configUiMasterModulJenis = loadConfigModulJenis_he_misc($origJenisTr, "coTransaksiUi");
            $configCoreMasterModulJenis = loadConfigModulJenis_he_misc($origJenisTr, "coTransaksiCore");
            $configLayoutMasterModulJenis = loadConfigModulJenis_he_misc($origJenisTr, "coTransaksiLayout");

            $jenisTr = isset($configUiMasterModulJenis['aliasMainTrans']) ? $configUiMasterModulJenis['aliasMainTrans'] : $origJenisTr;
...
//                $trr->addFilter("param='main'");
                ...
                $targetProc = MODUL_PATH . $configUiMasterModulJenis['itemSwapper'] . "/$origJenisTr/?items=" . base64_encode(serialize($items)) . "&main=" . base64_encode(serialize($oldMain)) . "&trs=" . base64_encode(serialize($trIDs));
