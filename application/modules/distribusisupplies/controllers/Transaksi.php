...
//                    }
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            foreach ($tmpHist as $row) {
                ...
                // ini belum ambil transaksi_data atau registry items
                // masih ambil dari registry main, ya keluarnya hanya 1 item saja
                //region show items
                $tblItems = "";
                if (sizeof($arrItemShow) > 0) {
                    $tblItems .= "<table class='table table-bordered' sstyle='border:1px solid red;'>";
                    $tblItems .= "<tr>";
                    $tblItems .= "<th>No</th>";
                    ...
                    $tblItems .= "</tr>";
                    $prefRegistriesItems = $tmpReg_result[$row->transaksi_id]["items"];
                    // arrPrint($prefRegistriesItems);
                    $numb = 0;
                    foreach ($prefRegistriesItems as $ii => $rowItem) {
                        $numb++;
                        $tblItems .= "<tr>";
                        $tblItems .= "<th>$numb</th>";
                        foreach ($arrItemShow as $k => $iLabel) {
                            $class = "";
                            if (isset($rowItem[$k]) && (is_numeric($rowItem[$k]))) {
                                $class = "text-center";
                            }
                            $row_items = isset($rowItem[$k]) ? $rowItem[$k] : "";
                            $tblItems .= "<td class='$class'>" . $row_items . "</td>";
                        }
                        $tblItems .= "</tr>";
                    }
...
//cekbiru(":: $jenisTr ::");
        ...
        $configCoreMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr, "coTransaksiCore");
        $configLayoutMasterModulJenis = loadConfigModulJenis_he_misc($jenisTr, "coTransaksiLayout");


        $historyFields = isset($configUiMasterModulJenis['shortHistoryFields']) ? $configUiMasterModulJenis['shortHistoryFields'] : array();
        $tabHistoryFields = isset($configUiMasterModulJenis['tabHistoryFields']) ? $configUiMasterModulJenis['tabHistoryFields'] : array();
        $tabFieldsItems = isset($configUiMasterModulJenis['tabFieldsItems']) ? $configUiMasterModulJenis['tabFieldsItems'] : array();
        $pairRegistries = isset($configUiMasterModulJenis['pairRegistries']) ? $configUiMasterModulJenis['pairRegistries'] : array();
        $dueDateAllow = isset($configUiMasterModulJenis['updateDueDate']) ? $configUiMasterModulJenis['updateDueDate'] : array();
        ...
        if ($isMob) {
            $historyFields = isset($configUiMasterModulJenis['compactHistoryFields']) ? $configUiMasterModulJenis['compactHistoryFields'] : array();
        }
        $glanceHistoryFields = isset($configUiMasterModulJenis['glanceHistoryFields']) ? $configUiMasterModulJenis['glanceHistoryFields'] : array("nomer" => "receipt no");
        $glanceHistoryFields2 = array("nomer" => "receipt no"); //init;
...
//cekHere(":: " . $rows->produk_id . " :: " . $rows->valid_qty . " ::");
