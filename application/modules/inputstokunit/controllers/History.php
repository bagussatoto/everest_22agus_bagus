...
//                            arrPrintWebs($row->references);
                            foreach ($row->references as $ref_number_id) {
                                $itr = New MdlTransaksi();
                                $itr->setJointSelectFields(array("nomer"));
                                $itr->addFilter("id='$ref_number_id'");
                                $itrTemp = $itr->lookupAll()->result();
                                $ref_number = $itrTemp[0]->nomer;
                                $ref_jenis_master = $itrTemp[0]->jenis_master;
                                $modul = $this->masterConfigUi[$ref_jenis_master]['modul'];
                                $modul_path = base_url() . "$modul/";
//                                cekHere($modul);

                                $ref_number_format = formatField_he_format("nomer", $ref_number, $ref_jenis_master, $modul_path);
                                if ($ref_result == "") {
