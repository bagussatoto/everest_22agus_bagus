...
class _shoppingCart extends Modul_Controller
{
    ...
    public function viewCart()
    {
        ...
        //----------------------------------------------
        $shoppingCartKodeBahanBaku = isset($this->configUi[$this->jenisTr]['shoppingCartTargetModal']) ? $this->configUi[$this->jenisTr]['shoppingCartTargetModal'] : array();
        //----------------------------------------------
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "banking/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "banking/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
            ...
            if (isset($_SESSION[$cCode]['items2_sum'])) {
                ...
                foreach ($_SESSION[$cCode]['items2_sum'] as $iSpec) {
                    ...
                    $tmp = array("id" => $iSpec['id'],
                        ...
                        "nilai_target" => isset($iSpec['nilai_target']) ? $iSpec['nilai_target'] : "",
                        "sub_nilai_target" => isset($iSpec['sub_nilai_target']) ? $iSpec['sub_nilai_target'] : "",
                    );
                    if ($noteEnabled) {
                        $tmp['note'] = isset($iSpec['note']) ? $iSpec['note'] : "";
                    }
                    ...
                    if (isset($editHandlerMethod2)) {
                        if (isset($iSpec['handler'])) {
                            ...
                            $tmp["removeTarget"] = base_url() . $iSpec['handler'] . "/removePaired/" . $this->jenisTr . "?id=" . $id2;
                        }
                        else {
                            $tmp["editTarget"] = "";
                            $tmp["removeTarget"] = "";
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount']) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$stepNumber] == true) {
            ...
            if (sizeof($shoppingCartAmountValue) > 0) {
                foreach ($shoppingCartAmountValue as $key => $valTmp) {
                    unset($itemLabels[$key]);
                }
            }
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        if (sizeof($shoppingCartKodeBahanBaku) > 0) {
            if (isset($shoppingCartKodeBahanBaku["enabled"]) && ($shoppingCartKodeBahanBaku["enabled"] == true)) {
                $shoppingCartKodeBahan = $shoppingCartKodeBahanBaku;
            }
        }
        ...
        $data = array(
            ...
            "shoppingCartKodeBahanBaku" => isset($shoppingCartKodeBahan) ? $shoppingCartKodeBahan : array(),
        );
...
//        arrPrint($_GET);
    ...
    //-----------------------------------
    public function recordProduksi()
    {
        ...
        $configUiJenis = $this->configUi[$this->jenisTr];
        $configCoreJenis = $this->configCore[$this->jenisTr];
        $configValuesJenis = $this->configValues[$this->jenisTr];
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        ...
        if ($mains["fase_id"] == 1) {
            ...
            $psnTmp = $psn->dataCek($val);
            if (sizeof($psnTmp) > 0) {
                $produk_nama = $psnTmp[0]->produk_nama;
                $msg = "Nomer seri produk sudah dipakai $produk_nama. Silahkan mengisi dengan nomer seri yang lain.";
                mati_disini($msg);
            }
        }
        ...
        $vg->setConfigUiJenis($configUiJenis);
        $vg->setConfigCoreJenis($configCoreJenis);
        $vg->setConfigValuesJenis($configValuesJenis);
        $initMasterValues = array(
            "olehID" => my_id(),
            "olehName" => my_name(),
            "sellerID" => my_id(),
...
//
//        echo "<script>";
//        echo "top.$('#shopping_cart').load('" . base_url() . $this->modul . "/" . "_shoppingCart/viewCart/" . $jenisTr . "?');";
//        echo "</script>";
    }

    public function recordProduksiEdit()
    {
        ...
        $psn = New MdlProdukPerSerialNumber();
        $psnTmp = $psn->dataCek($lastVal);
        if (sizeof($psnTmp) > 0) {
            $id_tbl = $psnTmp[0]->id;
            $produk_nama = $psnTmp[0]->produk_nama;
            $data_new = array();
            foreach ($psnTmp[0] as $keyy => $vall) {
                if ($keyy != "id") {
                    $data_new[$keyy] = $vall;
                }
            }
            $data_new["serial_number"] = $val;
            $data_new["dtime"] = date("Y-m-d H:i:s");
            $data_new["oleh_id"] = my_id();
            $data_new["oleh_nama"] = my_name();
            // update yang lama dengan trash menjadi 1
            $data = array(
                "trash" => 1
            );
            $where = array(
            ...
            // insert baru...
            $psn->addData($data_new);
            showLast_query("kuning");


            $tr = New MdlTransaksiModul();
            $tr->setPrefix($prefix);
            $tr->addFilter("kode_produksi='$lastVal'");
            ...
            if (sizeof($trTmp) > 0) {
                foreach ($trTmp as $trSpec) {
                    ...
                    $tr->updateMainEntries($where, $data);
                    showLast_query("orange");
                }
            }


            $mi = New MdlManufacturIdentity();
            $mi->addFilter("kode_produksi='$lastVal'");
...
//        echo "</script>";
    ...
    //-----------------------------------
    public function recordProduksiBahanBaku()
    {

        $jenisTr = $this->jenisTr;
        $cCode = $this->cCode;
        $iID = $this->uri->segment(5);
        $rowName = $this->uri->segment(6);
        $val = ($_GET['val']);
        $mains = $_SESSION[$cCode]['main'];
        $mainGates = $_SESSION[$cCode];
//        $total_diskon = $mains['akumDisc'] + $val;
        ...
        $shoppingCartKodeBahanBaku = isset($this->configUi[$this->jenisTr]['shoppingCartTargetModal']) ? $this->configUi[$this->jenisTr]['shoppingCartTargetModal'] : array();
        if (sizeof($shoppingCartKodeBahanBaku) > 0) {
            $targetGate = $shoppingCartKodeBahanBaku["gateTarget"];
        }
...
//        }
        ...
        $nilai_source = isset($_SESSION[$cCode]["main"]["nilai_source"]) ? $_SESSION[$cCode]["main"]["nilai_source"] : 0;
        $nilai_target = 0;
        foreach ($_SESSION[$cCode][$targetGate] as $spec){
            $nilai_target += $spec["nilai_target"];
        }
        $total_nilai_target = $nilai_target + $val;

        $total_nilai_target_f = number_format($total_nilai_target, "0", ".", ",");
        $nilai_source_f = number_format($nilai_source, "0", ".", ",");
        if($total_nilai_target > $nilai_source){
            $msg = "Total nilai rincian sebesar $total_nilai_target_f melebihi nilai yang ditentukan sebesar $nilai_source_f. Silahkan dikoreksi kembali.";
            mati_disini($msg);
        }

//        mati_disini(__LINE__);
        if (!isset($_SESSION[$cCode][$targetGate][$iID][$rowName])) {
            $_SESSION[$cCode][$targetGate][$iID][$rowName] = null;
        }
        $_SESSION[$cCode][$targetGate][$iID][$rowName] = $val;
