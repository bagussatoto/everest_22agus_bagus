...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $fieldEquivalent = isset($this->configUi[$this->jenisTr]['shoppingCartFieldEquivalent']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldEquivalent'] : array();
                        ...
                        case "dataField":
                            if (strlen($_SESSION[$cCode]['main_elements'][$eName]['value']) < 0.5) {
                                if ((isset($aSpec['noValidate'])) && ($aSpec['noValidate'] == true)) {
                                    ...
                                    $msg = isset($aSpec['labelValidate']) ? $aSpec['labelValidate'] : "must be filled with one entry!";
                                    $errMsgs[] = ($aSpec['label'] . " $msg");
                                    echo "<script>";
                                    echo "if(top.document.getElementById('elTitle_$eName')){top.document.getElementById('elTitle_$eName').className='box-headers text-red text-left'};";
                                    echo "</script>\n";
                                }
...
//                                    $errMsgs[] = "$label $name must be > 0";
        ...
        if (sizeof($fieldEquivalent) > 0) {
            if (isset($fieldEquivalent["enabled"]) && ($fieldEquivalent["enabled"] == true)) {
                $key_source = $fieldEquivalent["source"]["key"];
                $key_target = $fieldEquivalent["target"]["key"];
                $key_source_nilai = $_SESSION[$cCode]["main"][$key_source];
                $key_target_nilai = $_SESSION[$cCode]["main"][$key_target];
                $selisih = $key_source_nilai - $key_target_nilai;
                $selisih = ($selisih < 0) ? ($selisih * -1) : $selisih;
                $key_source_nilai_f = number_format($key_source_nilai, "0", ".", ",");
                $key_target_nilai_f = number_format($key_target_nilai, "0", ".", ",");
                if ($selisih > 1) {
                    $msg = "Total nilai rincian sebesar $key_target_nilai_f tidak sama dengan nilai yang ditentukan sebesar $key_source_nilai_f. Silahkan dikoreksi kembali.";
                    $errMsgs[] = $msg;
                }
            }
        }
...
// matiHEre("hoop".__LINE__);
    ...
    public function index()
    {
        ...
        $link_paymentList = MODUL_PATH . "Transaksi/selectPaymentExternSrc/$jenisTr/?gr=cGVtYmVsaWFu&ohyes=ohno&step=$getStep";
// matiHEre();
        $scriptBottom = "";
        $scriptBottom .= "<script>
            ...
            if (isset($returnRoutes[$jenisSrc])) {
                ...
                if (sizeof($tmpTr) > 0) {
                    ...
                    if (sizeof($mainReg) > 0) {
                        $trr = new MdlTransaksi();
                        $trr->setFilters(array());
                        $trr->addFilter("id in ('" . implode("','", $mainReg) . "')");
                        $tmpReg = $trr->lookupRegistries()->result();
                        foreach ($tmpReg as $rSpec) {
                            $mainRegResult[$rSpec->transaksi_id] = blobDecode($rSpec->values);
                        }
                    }
                }
