...
class _processSelectRekening extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        $autoBuildTarget = isset($this->configUi[$this->jenisTr]['autoBuildTarget']) ? $this->configUi[$this->jenisTr]['autoBuildTarget'] : array();
...
//                            mati_disini();
                ...
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    ...
                    if (sizeof($autoBuildTarget) > 0) {
                        if ($autoBuildTarget["enabled"] == true) {
                            $cc = new $selectorSrcModel();
                            $tmpCC = $cc->lookupAll()->result();
                            if (sizeof($tmpCC) > 0) {
                                foreach ($tmpCC as $rowCC) {
                                    if ($rowCC->id != $id) {
                                        ...
                                        foreach ($fieldSrcs as $key => $src) {
                                            ...
                                            if (sizeof($tmpEx) > 1) {//===berarti mengandung karakter simbol perhitungan
                                                ...
                                                foreach ($tmpEx as $key2 => $val2) {
                                                    if (!is_numeric($val2)) {
                                                        if (isset($tmp[$val2]) && $tmp2[$val2] > 0) {
                                                            $newSrc = str_replace($val2, $tmp2[$val2], $newSrc);
                                                        }
                                                        else {
                                                            $newSrc = str_replace($val2, 0, $newSrc);
                                                        }
                                        ...
                                        $_SESSION[$cCode][$autoBuildTarget["targetGate"]][$id2] = $tmp2;
                    ...
                    if (isset($_SESSION[$cCode]['items2'][$id])) {
                        ...
                        $_SESSION[$cCode]['items2'][$id]['jml'] = $_SESSION[$cCode]['items'][$id]['jml'];
                    }
...
//mati_disini($_SESSION[$cCode]['main']['harga']);
        ...
        // karena ada telat gerbang maka diload ulang gerbangnya
        //----------------------------------------------------------------
        ...
        //
        if ($_SESSION[$cCode]['items'][$id]['saldo_akhir'] < 0) {
//            mati_disini($_SESSION[$cCode]['items'][$id]['saldo_akhir']);
            $iNama = $_SESSION[$cCode]['items'][$id]['nama'];
            $saldo_akhir_f = number_format($_SESSION[$cCode]['items'][$id]['saldo_akhir'], "0", ".", ",");
            $msg = "Saldo akhir modal $iNama salah karena kurang dari 0, sebesar $saldo_akhir_f. Silahkan dikoreksi kembali.";
            echo lgShowWarning("Peringatan", $msg);
            echo "<script>";
            echo "top.$('#result').load('" . MODUL_PATH . get_class($this) . "/select/" . $this->jenisTr . "?id=$id&newQty=1&nilai_source=0');";
            echo "</script>";
        }
...
//mati_disini(__LINE__);
        ...
        echo "<script>";
        echo "top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?epreketek=yes');";
        echo "</script>";
    }
}