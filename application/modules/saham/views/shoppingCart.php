...
//                }
            ...
            foreach ($items2 as $iSpec) {
                ...
                foreach ($itemLabels2 as $key => $label) {
                    ...
                    $isi_kolom = isset($iSpec[$key]) ? $iSpec[$key] : "";
                    if (is_numeric($isi_kolom)) {
                        // echo "<input type=number autocomplete='off' readOnly id=$colID class='form-control text-right' style='color:$color;' value='$isi_kolom' >";
//                        echo formatField($key, $iSpec[$key]);
                        // echo $iSpec[$key];
//
                        if (sizeof($shoppingCartKodeBahanBaku) > 0) {
                            $elLink = $shoppingCartKodeBahanBaku["link"];
                            $jenisTr = $shoppingCartKodeBahanBaku["jenisTr"];
                        }

                        if (in_array($key, $editableFields2)) {
//                            echo "editable...";
                            echo "<input type=number autocomplete='off' class='form-control' style='color:$color;' value='$isi_kolom'
                                    onblur=\"$('#result').load('" . MODUL_PATH . "$elLink" . "$jenisTr/$iID/$key?val='+this.value);\"
                                    >";
                        }
                        else {
                        ...
                        if (!isset($summary2[$key])) {
                            $summary2[$key] = 0;
                        }
                        $summary2[$key] += $iSpec[$key];
                    }
                    else {
                        ...
                        if (in_array($key, $editableFields2)) {
//                            echo "---editable...";
                            echo "<input type=text autocomplete='off' class='form-control' style='color:$color;' value='$isi_kolom'
                                    onblur=\"$('#result').load('" . MODUL_PATH . "$elLink" . "$jenisTr/$iID/$key?val='+this.value);\"
                                    >";
                        }
                        else {
...
//                }
            ...
            $colspann = sizeof($itemLabels2)-1;
            $cAlign = is_numeric($iSpec[$key]) ? "text-right" : "text-left";
            echo "<tr class='bg-grey-1 3'>";
            echo "<td ccolspan='$colspann' class='text-right valign-m text-uppercase'>TOTAL</td>";
            foreach ($itemLabels2 as $key => $label) {
//                if ($key == "sub_nilai") {
                    echo "<td class='right-borderlesss $cAlign'>";
                    echo formatField_he_format($key, $summary2[$key]);
                    echo "</td>";
//                }
            }
            echo "</tr>";
