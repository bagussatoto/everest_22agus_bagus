...
//                            case "combo":
//                                $elStr[$eName] .= "<select class='form-control selectpicker' data-style='btn-primary' data-live-search='true' data-size='10' data-container='body'
//                                onchange=\"hiliteDiv(this);document.getElementById('result').src=$selectorTarget;\">";
                        ...
                        switch ($eSpec['inputType']) {
                            ...
                            case "radio":
                                ...
                                if (sizeof($tmpo) > 0) {
                                    if ($eName == "cash_account") {
                                        foreach ($tmpo as $row) {
                                            $arrBank[$eName][$row->folders] = $row->folders_nama;
                                            $ex = explode("/", $elementConfigs[$eName]['labelSrc']);
                                            if (sizeof($ex) > 1) {
                                                $labelValue = "";
                                                foreach ($ex as $col) {
                                                ...
                                                $arrBankRekening[$eName][$row->folders][] = "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
...
//
                                        ...
                                        if (isset($arrBank[$eName]) && (sizeof($arrBank[$eName]) > 0)) {
                                            $anu = "";
                                            foreach ($arrBank[$eName] as $fid => $fnama) {
                                                if ($fnama != NULL) {
                                                    $anu .= "<span class='text-bold' style='font-size:15px;'>$fnama</span><br>";
                                                }
                                                if (isset($arrBankRekening[$eName][$fid]) && (sizeof($arrBankRekening[$eName][$fid]) > 0)) {
                                                    foreach ($arrBankRekening[$eName][$fid] as $fdata) {
                                                        $anu .= $fdata;
                                                    }
                                                }
                                            }
                                                ...
                                                case "cash_account_target":
                                                    foreach ($tmpo as $row) {
                                                        ...
                                                        if (sizeof($ex) > 1) {
                                                            ...
                                                            $selected = isset($_SESSION[$cCode]['items'][$eName][$helpName]) && $_SESSION[$cCode]['items'][$eName][$helpName] == $row->$keySrc ? "checked" : "";
                                                            $arrBankRekening[$row->folders][] = "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'><input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected onclick=\"hiliteDiv(this);top.$('#result').load($selectorTarget);\">" . $labelAccount . "</label>\n";
                                                        }
                                                    }
                                                    $elStr[$eName] .= "<div style='text-align: left;'>";
