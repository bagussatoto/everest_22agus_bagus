...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
         ...
         * ------------------------------*/
        $total_project = $_SESSION[$cCode]['main']['harga_nppn']*1;
        ...
        $allow_inchoate_termin = isset($_SESSION[$cCode]['main']['allow_inchoate_termin']) ? $_SESSION[$cCode]['main']['allow_inchoate_termin'] : 0;
        ...
        if(empty($items3) && !$allow_empty_termin){
            ...
            if ($pengaturan_termin && floor($total_harga_termin) != floor($total_project) && !$allow_inchoate_termin) {
                $debug  = "<div class='meta bg-yellow'>pengaturan_termin: $pengaturan_termin</div>";
                $debug .= "<div class='meta bg-yellow'>total_harga_termin != total_project ($total_harga_termin!=$total_project)</div>";
                $debug .= "<div class='meta bg-yellow'>allow term incomplete ? $allow_inchoate_termin || 0= false / 1= allowed</div>";
                $dtime = "<div class='meta'>".date("d,M-Y H:i")."</div>";
                $alerts = array("type" => "warning",
                    "title" => "Termin Belum Lengkap",
                    "html" => "($allow_empty_termin) Termin belum di lengkapi, apakah anda akan melewatkan setingan ini..??$debug$dtime",
                );
                echo swalAlert($alerts);
                die();
            }
...
////                    cekMerah( $row->nomer );
////                    arrPrintWebs( $activeProject[$row->nomer] );
//                    }
//                else{
//                    cekMerah( $row->nomer );
//                    arrPrintWebs( $activeProject[$row->nomer] );
...
//                $linkRencana = MODUL_PATH . "Printing/viewReceiptReg/$jenisTr/".$row->nomer;
                ...
                if( $activeProject[$row->nomer]->project_start == 1 ){
                    ...
                    $tmp['rencana'] .= "<a class='btn btn-sm btn-warning' title='' data-toggle='tooltip' href='javascript:void(0);' onclick =\"window.open('$newLink');\"><i class='fa fa-send'></i> Show Progress</a>";
                    $tmp['rencana'] .= "</div>";
                }
                else{
                ...
                if ($allowFollowup) {
                    ...
                    if( $activeProject[$row->nomer]->project_start == 1 ){
                        ...
                        if( $activeProject[$row->nomer]->persen_progress == 100 ){
                            $tmp['action'] .= "<button id='prj_id_".$activeProject[$row->nomer]->id."' type='button' class='btn btn-sm btn-success' title='Project Bisa Dilanjutkan untuk Finishing' data-toggle='tooltip'><i class='fa fa-check-square'></i> PROGRESS COMPLETE 100% </button>";
                            $tmp['action'] .= "<button type='button' class='btn btn-sm btn-info' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick =\"top.open_holdon();$followupLink\"><i class='fa fa-check'></i> <b>FINISH PROJECT</b></button>";
                        }
                        else{
                            $persentase = 0+ $activeProject[$row->nomer]->persen_progress*1;
                            $tmp['action'] .= "<button id='prj_id_".$activeProject[$row->nomer]->id."' type='button' class='btn btn-sm btn-danger' title='Project Bisa Dilanjutkan setelah Progress 100%...' data-toggle='tooltip'><i class='fa fa-gear fa-spin'></i> On Progress ".number_format($activeProject[$row->nomer]->persen_progress*1)."% </button>";
                            if($persentase*1>0){
                                $tmp['action'] .= "
                                    <script>
                        ...
                        if ($allowJoin) {
                            $tmp['action'] .= "<span class='input-group-addon'>";
                            $tmp['action'] .= "<a title='process many items at once' href='" . MODUL_PATH . "Transaksi/viewIncomplete/" . $this->jenisTr . "/$currentStepNum'><i class='fa fa-dedent'></i></a>";
                            $tmp['action'] .= "</span>";
                        }
                        $tmp['action'] .= "</div>";
                    }
                        ...
                        if( $activeProject[$row->nomer]->lock == 1 && $activeProject[$row->nomer]->quot_status == 0 ){
                            ...
                            if($activeProject[$row->nomer]->lock==1&&$activeProject[$row->nomer]->quot_status==1){
                                $tmp['action'] = "<div class='btn-group-sm btn-group-vertical'>";
                                $tmp['action'] .= "<button type='button' class='btn btn-sm btn-info' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick =\"top.open_holdon();$followupLink\"><i class='fa fa-check'></i> START PROJECT</button>";
                                if ($allowJoin) {
                                    $tmp['action'] .= "<span class='input-group-addon'>";
                                    $tmp['action'] .= "<a title='process many items at once' href='" . MODUL_PATH . "Transaksi/viewIncomplete/" . $this->jenisTr . "/$currentStepNum'><i class='fa fa-dedent'></i></a>";
                                    $tmp['action'] .= "</span>";
                                ...
                                if ( $nextStepNum == 3 ) {
                                    ...
                                    $tmp['action'] .= "<button type='button' class='btn btn-sm btn-primary' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick =\"top.open_holdon();$followupLink\"><i class='fa fa-check'></i> " . $actionLabel . "</button>";
                                    $tmp['action'] .= "<span class='input-group-addon'>";
                                    $tmp['action'] .= "<a class='btnx btn-smx btn-infox btn-blockx' title='Buat BOM' data-toggle='tooltip' href='javascript:void(0);' onclick =\"top.open_holdon();window.location.href='$newLink'\"><i class='fa fa-hourglass'></i> CREATE BOM </a>";
                                    $tmp['action'] .= "</span>";
                                    $tmp['action'] .= "</div>";
                                }
                                else {
...
//       arrPrint( $this->db->last_query() );
        ...
        if (count($tmp) > 0) {
            if( !in_array("o_project_spv", $mems) ){
                ...
                <h3><i class='fa fa-table'></i> My Project <div class='btn btn-sm btn-info pull-right' onclick=\"viewundoneList();\"><i class='fa fa-refresh'></i> REFRESH</div></h3>
...
//            arrprint($tmp);
            foreach ($tmp as $trmp_0) {
                ...
                foreach ($viewFields as $key => $label) {
                    ...
                    $value = "<a href='$newLink' onclick=\"closePopup();\">" . formatField_he_format($key, $trmp_0->$key) . "</a>";
                    }
                    if (in_array($key, $isLinkModal)) {
                        $value = "<a href='javascript:void(1);' class='link-with-icon' onclick=\"view_data('$newLink');\" title='klik untuk melihat detail'>" . formatField_he_format($key, $trmp_0->$key) . " <i class='fa fa-eye text-red'></i></a>";
                    }
                    $dataView .= "<td data-key='$key'>$value</td>";
                }
                $dataView .= "</tr>";
            }
            $dataView .= "</tbody>";
