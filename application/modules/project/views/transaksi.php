...
//            pageResize: true,
        ...
        $p->addTags(
            array(
                ...
                "dasboardLists" => "<div class='box-body' style=\"background-image: url('{base}/demo_project/public/images/sys/123.gif');height: 250px;background-repeat: no-repeat;\"></div>",
            )
        );
        ...
        $str .= "
                <script>
                    var undoneitems_table = $('#undoneitems_table').dataTable({
                        ...
                        buttons: [
                            {
                                text: 'SO BELUM DIAPPROVE',
                                action: function ( e, dt, node, config ) {
                                    dt
                                    .columns(10)
                                    .search('preview quotation')
                                    .draw();
                            }
                            },
                            {
                            ...
                            {
                                text: 'RAB BELUM DIAPPROVE',
                                action: function ( e, dt, node, config ) {
                                    dt
                                    .columns(10)
                                    .search('start project')
                                    .draw();
                                }
                            },
//                            {
...
//            echo "</li>";
        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            echo "<h2 id='showTasklist'>Status Keseluruhan Project <r>(".$mainValues['projectName'].")</r></h2>";
            echo "<h4>Work Order / Tasklist <span id='reloadTasklistModal' onclick=\"top.open_holdon();top.$('#result').load('$btnReloadTaskList#showTasklist');\" class='pull-right btn btn-xs btn-danger'><i class='fa fa-refresh'></i> REFRESH</span></h4>";
                        ...
                        switch($kk){
                            case "progress_nama":
                                if($row->progress_id == 3){
                                    ...
                                    $val_ .= "<span style='margin-left: 3px;' data-id='create-$tsk_id-$produk_id' onclick='fnTasklist.create(this)' id='' class='btn btn-xs btn-info'><i class='fa fa-send'></i> Lakukan QC</span>";
                                }
                                else{
...
//                                        var jml = $(b).val();
                                    ...
                                    var linkUpdate      = '" . base_url() . "master_project/masterData/exeSubTasklist/';
                                    var debug = 0;
...
//                                console.log(imgBank);
                            ...
                            function loadEditSpk(id,prd_id){
                                ...
                                var linkCreate      = '" . base_url() . "master_project/masterData/showCreateSubTasklist/';
...
//                                size: 'custom-width',
                    ...
                    function check_status_project(){
                        ...
                        var announcement = '';
                        if(stok_gantung==0 && belum_qc==0){
                            top.$('#approvalButton').prop('disabled', false);
                        }
                        else{
                            top.$('#approvalButton').prop('disabled', true);
                            top.$('#checklist_trx').prop('checked', false);

                            if(stok_gantung || belum_qc){
                                announcement += ''
                            }
                            if(stok_gantung){
                    ...
                    // JSON yang berisi data produk untuk setiap nota/invoice
                    var productData = ".json_encode($arrMaterialDist).";
                    var productHold = ".json_encode($arrMaterialDistHold).";
                    ...
                    $('.btn-tooltip').tooltip({
                        ...
                        title: function() {
                            ...
                            var product = productData[productId].produk;

                            // Konten HTML untuk tooltip
                            var tooltipContent = '<table class=\"table dataTable compact table-bordered\">';
                            tooltipContent += '<thead><tr><th>ID</th><th>Nama Produk</th><th>Jumlah</th></tr></thead>';
                            tooltipContent += '<tbody><tr><td>' + product.id + '</td><td>' + product.nama + '</td><td>' + product.jumlah + '</td></tr></tbody>';
                ...
                <script>
                            function loadBahanTasklist(id,prd_id){
                                ...
                                var linkCreate      = '" . base_url() . "master_project/masterData/showCreateSubTasklist/';
                                var linkUploadPhoto = '" . base_url() . "master_project/masterData/uploadPhoto/';
                                var linkUpdateQc    = '" . base_url() . "master_project/masterData/exeSubTasklistQc/';
                                var json      = ".json_encode($mainValues).";
                                var prodID      = '".$mainValues['projectID']."';
...
//                                                                        console.log('reload dimatikan dulu');
                                                                        if( top.$('#reloadTasklistModal') ){
                                                                            top.$('#reloadTasklistModal').click();
                                                                        }
                                                                        else{
...
//                        $msgNote = "Lanjutkan Close Project dengan menutup $total_items item, " . $sumSubItems['qty'] . " unit";
//                        $checklistNoteEncode = blobEncode($msgNote);
//                        echo "<input type='checkbox' value='' onclick=\"document.getElementById('result').src='" . $checklistNotePaired . "?checklistnote=$checklistNoteEncode';\">";
//                        echo "&nbsp; <span>$msgNote</span>";
//                        echo "</div>";
//                    }
//                    else {
...
//                        $checklistNoteEncode = blobEncode($msgNote);
//                        $checked = isset($checklistnote_cek) && ($checklistnote_cek == 1) ? "checked" : "";
//                        echo "<input type='checkbox' value='' $checked onclick=\"document.getElementById('result').src='" . $checklistNotePaired . "?checklistnote=$checklistNoteEncode';\">";
//                        echo "&nbsp; <span>$msgNote</span>";
//                        echo "</div>";
//                    }
//                }
...
//            else {
                ...
                echo "<div style='padding: 10px;' id='check_lanjut_closing' class='panel bg-red hidden'>";

                $msgNote = "Checklist Lanjutkan Close Project";
                $checklistNoteEncode = blobEncode($msgNote);
                $checked = isset($checklistnote_cek) && ($checklistnote_cek == 1) ? "checked" : "";

                echo "<div class='text-rightx'>";
                echo "<label for='checklist_trx'>";
                echo "<input id='checklist_trx' type='checkbox' value='' $checked onclick=\"check_status_project();document.getElementById('result').src='" . $checklistNotePaired . "?checklistnote=$checklistNoteEncode';\">";
                ...
                echo "<div style='font-size: 16px;'>1. Pastikan tidak ada stok yang masih gantung, jika masih ada harap di proses return.</div>";
                echo "<div style='font-size: 16px;'>2. Pastikan tidak ada WO yang belum di QC, lakukan QC jika masih ada.</div>";
                echo "</div>";
