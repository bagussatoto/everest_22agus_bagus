            ...
            if ($showItems) {
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            echo "<div class='row'>";
                            // echo "<div class='col-md-10' style='padding-right: 0'>";
                            echo "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id='$colID'  class='hidden form-control text-right' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
                            ...
                            // echo "</div>";
// arrPrintHijau($iSpec);
                            $nilaiBulat =$iSpec['harga_bulat'];
                            $niceBulat = niceDecimal($nilaiBulat);
                            $niceDesimal =$iSpec['harga_desimal'];
                            echo "<div class='col-md-10' style='padding-right: 0'>";
                            echo "<input type='text' title='masukan hanya angka saja, tanpa titik, maupun koma' class='form-control text-right' id='bulat_$colID' placeholder='biangan bulat' oninput=\"this.value = this.value.replace(/[^0-9]/g, '');\" onclick=\"this.select();\" onblur=\"gerbang_bulat('$colID', this.value);\" value='$niceBulat'>";
                            echo "</div>";
                            echo "<div class='col-md-2' style='padding-left: 0'>";
                            echo "<input type='number' title='masukan angka bulat sebagai desimal, makimal 3 digid' class='form-control' id='desimal_$colID' placeholder='desimal' max='99' min='0' oninput=\"if(this.value.length > 3) this.value = this.value.slice(0,3);\"  onclick=\"this.select();\" onblur=\"gerbang_desimal('$colID', this.value);\" value='$niceDesimal'>";
                            echo "</div>";
                            echo "</div>";
                            break;
                        case "text":
...
//                arrPrint($addRowHiddens[$k]);
        ...
        echo "<script>
            function gerbang_bulat(target,nilai) {
              let desimal = $('#desimal_' + target).val();
              let nilai_riil = desimal > 0 ? nilai +'.'+ desimal : nilai;
              // console.log('desi:', desimal);
              // console.log('nilai_riil:', nilai_riil);
              
              // if(localStorage.getItem('bulat_' + target) === null){
                  localStorage.setItem('bulat_' + target, nilai)
              // }
                            
              $('#' + target).val(nilai_riil).trigger('blur');              
            }
                                    
            function gerbang_desimal(target,nilai) {
              let bulat = $('#bulat_' + target).val();
              let nilai_riil =  bulat +'.'+ nilai;
              // console.log('bulat:', bulat);
              // console.log('nilai_riil:', nilai_riil);
              
              localStorage.setItem('desimal_' + target, nilai)
            ...
            function populateInputsFromLocalStorage() {
                for (let i = 0; i < localStorage.length; i++) {
                    ...
                    const nilaiTersimpan = localStorage.getItem(key);

                    const inputElement = document.getElementById(key);
                    if (inputElement) {                        
                        // $('#' + key).val(addCommas(nilaiTersimpan)).css('background-color', 'yellow');
