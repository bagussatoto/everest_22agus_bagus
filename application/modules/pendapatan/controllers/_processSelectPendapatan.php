...
class _processSelectPendapatan extends Modul_Controller
{
...
//                            mati_disini();
                ...
                if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    ...
                    unset($_SESSION[$cCode]['main']);
                    unset($_SESSION[$cCode]['tableIn_master']);
                    unset($_SESSION[$cCode]['tableIn_detail']);
                    unset($_SESSION[$cCode]['tableIn_detail_values']);
                    unset($_SESSION[$cCode]['tableIn_master_values']);
                    $tmp = array(
...
//                        $h->addFilter("jenis_value in (" . implode(",", $priceConfig['label']) . ")");
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        ...
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && $_GET[$key] > 0) {
                                ...
                                $pphps4_2s = pembulatan_pajak($newValue,20);
                                $_SESSION[$cCode]['items'][$id]['pphps4_2_bulat'] = $pphps4_2s["ppn"];
                                $_SESSION[$cCode]['items'][$id]['harga_bulat'] = (int)$newValue;
                                $parts = explode('.', (string) $newValue);
                                $decimalPart = isset($parts[1]) ? $parts[1] : 0;
                                $_SESSION[$cCode]['items'][$id]['harga_desimal'] = $decimalPart;
            ...
            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                $_SESSION[$cCode]['main']['rekName'] = array();

                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    $_SESSION[$cCode]['main']['rekName'] = $iSpec['nama'];
