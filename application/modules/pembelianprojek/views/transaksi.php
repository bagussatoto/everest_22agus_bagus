...
//            $strOnprogFooter = "<a class='btn btn-default' href='" . base_url() . $this->uri->segment(1) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
        ...
        $scriptBottom .= isset($str_scriptBottom) ? $str_scriptBottom : "";
        ...
        $p->addTags(
            array(
                ...
                "mode_disabled_edit" => ($modeedit != NULL) ? "disabled" : "",
...
//                            cekMerah(":: $key NOT editable ::");
                        ...
                        if (sizeof($subItemLabels) > 0) {
                            if (isset($subItemLabels)) {
                                if (sizeof($subItemLabels) && (is_array($subItemLabels))) {
                                    $colspan2 = $imageEnabled == true ? 1 : -1;
                                    $colspan = sizeof($subItemLabels) - $colspan2;
                                    $linkSubAdd = $shoppingCartAdvanceItemsAdd . "/$iID/$sub_key";
                                    echo "<tr>";
                                    ...
                                    foreach ($subAdvanceItems2 as $subNo => $iiSpec) {
                                        ...
                                        foreach ($subItemLabels as $key => $label) {
                                            ...
                                            echo "<input type='$input_type' id='$subcolID' class='form-control $text_align' $disabled value='$subValue'";
                                            echo " onclick='this.select()' ";
                                            echo " onblur=\"document.getElementById('result').src='" . $linkSubEditable . "?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&$thisValue\"";
                                            echo " >";
                                            echo "</td>";
                                        ...
                                        if ($allowEdit == true) {//==delete item
                                            if ($allowRemove == true) {
                                                $linkSubRemove = $shoppingCartAdvanceItemsRemove . "/$iID/$subNo/?&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL";
                                                echo "<td style='width:1%;' class='text-muted text-center'>";
                                                echo "<a class='text-red btn' title='remove sub item' data-toggle='tooltip' data-placement='left'
                                            onclick=\"document.getElementById('result').src='$linkSubRemove';\"
                                        >";
...
//                arrPrint($tabFieldsItems);
        $currentSegment = isset($currentSegment) ? $currentSegment : 0;
        $str = "";
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            if (sizeof($tabHistoryFields) > 0) {
                //                cekKuning("::");
...
//                arrPrint($tabHistoryFields);
                $link = base_url() . "pembelian/Transaksi/viewRequestItems/461";
...
//                }
                if ($currentSegment > 0) {
                    ...
                    $arrayOnProgress2 = $arrayOnProgress2Wo[$currentSegment];
                }


                foreach ($tmpHeaderRequest as $ky => $arrLab) {
                    $active = $ky == $currentSegment ? "active text-bold text-green" : "text-bold";

//                    $str .= "<li class=\"$active\"><a href=\"#tab_$ky\" data-toggle=\"tab\" aria-expanded=\"false\">" . $arrLab['label'] . "</a></li>";
                    $str .= "<li class=\"$active\"><a href='JavaScript:void(0)' onclick=\"" . $arrLab['link'] . "\" data-toggle=\"tab\" aria-expanded=\"false\">" . $arrLab['label'] . "</a></li>";
                    $no1++;
                }
...
//cekHere(":: $woID == $currentSegment ::");
                        $str .= "<tr line=" . __LINE__ . " sstyle='$display'>";
...
//                                                console.log(arrCheck);
                                                ...
                                                if(arrCheck.length > 0){
                                                    jQuery.each(arrCheck, function(i, b){
                                                        ...
                                                        var ponama = $(b).attr('ponama');
                                                        var woid = $(b).attr('woid');
                                                        var wonama = $(b).attr('wonama');
                                                        var projid = $(b).attr('projid');
                                                        var projnama = $(b).attr('projnama');
                                                        var id = $(b).attr('id');
                                                        if( $(b).is(':checked') ){
                                                            arrValid[id] = povalue;
                                                            arrValidNama[id] = ponama;
                                                            arrWoid[id] = woid;
                                                            arrWoidNama[id] = wonama;
                                                            arrProjid[id] = projid;
                                                            arrProjidNama[id] = projnama;
                                                        }
                                                    })
                                                }
                                                   ...
                                                   $('#btnCreateShoppingCart')
                                                    ...
                                                    .on('click', function(){
                                                        $.ajax({
                                                            ...
                                                            data: { 
                                                                items: btoa(JSON.stringify(arrValid)),
                                                                itemsNama: btoa(JSON.stringify(arrValidNama)),
                                                                itemsWoid: btoa(JSON.stringify(arrWoid)),
                                                                itemsWoNama: btoa(JSON.stringify(arrWoidNama)),
                                                                itemsProjid: btoa(JSON.stringify(arrProjid)),
                                                                itemsProjNama: btoa(JSON.stringify(arrProjidNama))  
                                                                },
