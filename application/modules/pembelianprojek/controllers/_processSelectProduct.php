...
//        arrPrintPink($_POST);
//        cekHere($_POST["items"]);
//        arrPrint(blobDecode($_POST["items"]));
        $arrItems = $itemsTrans = json_decode(base64_decode($_POST["items"]), true);
        $arrItemsNama = json_decode(base64_decode($_POST["itemsNama"]), true);
        $arrItemsWoid = $itemsTrans = json_decode(base64_decode($_POST["itemsWoid"]), true);
        $arrItemsWoNama = json_decode(base64_decode($_POST["itemsWoNama"]), true);
        $arrItemsProjid = $itemsTrans = json_decode(base64_decode($_POST["itemsProjid"]), true);
        $arrItemsProjNama = json_decode(base64_decode($_POST["itemsProjNama"]), true);
...
//arrPrint($arrItems);
//arrPrint($arrItemsNama);
//arrPrint($arrItemsWoid);
//arrPrint($arrItemsWoNama);
//arrPrint($arrItemsProjid);
//arrPrint($arrItemsProjNama);
...
//        $arrItems = isset($_GET['items']) ? unserialize(base64_decode($items)) : array();
        ...
        // region items BIAYA/JASA sementara tembak dulu...........
        $tmpItems = array(
            "handler" => $this->uri->segment(1) . "/_processSelectProductException",
            "id" => 28,
            "jml" => 1,
            "qty" => 1,
            "nama" => "BIAYA/JASA",
        ...
        if (!array_key_exists($tmpItems["id"], $_SESSION[$cCode]['items'])) {
            $tmp = $tmpItems;
            foreach ($fieldSrcs as $key => $src) {
                $tmpEx = $cal->multiExplode($src);
                arrPrint($tmpEx);
            ...
            $tmp["subtotal"] = $subtotal;
            $_SESSION[$cCode]['items'][$tmpItems["id"]] = $tmp;


            $this->load->helper("he_value_builder");
            fillValues_he_value_builder($this->jenisTr, 0, 1, $this->configCoreJenis, $this->configUiJenis, $this->configValuesJenis, $_SESSION[$cCode]['main']['ppnFactor']);
...
//            arrPrintHijau($_SESSION[$cCode]['items']);
            ...
            $key_result = isset($iSpec[$shoppingCartAdvanceItemsKey]) ? $iSpec[$shoppingCartAdvanceItemsKey] : 1;
...
//            arrPrintWebs($advanceSubNumItemsLabel);
            if (sizeof($arrItems) > 0) {
                foreach ($arrItems as $idx => $jmlParam) {
                    // project id dan workorder id, masuk ke main disini
                    $this->load->model("Mdls/MdlProdukProject");
                    $b = new MdlProdukProject();
                    $tmpB = $b->lookupByID($arrItemsProjid[$idx])->result();
//                    showLast_query("biru");
//arrPrintPink($tmpB);
                    $_SESSION[$cCode]['main']['projectID'] = $arrItemsProjid[$idx];
                    $_SESSION[$cCode]['main']['projectName'] = $arrItemsProjNama[$idx];
                    $_SESSION[$cCode]['main']['projectNama'] = $arrItemsProjNama[$idx];
                    $_SESSION[$cCode]['main']['pihakProjekID'] = isset($tmpB[0]->id) ? $tmpB[0]->id : "";
                    $_SESSION[$cCode]['main']['pihakProjekMasterID'] = isset($tmpB[0]->id_master) ? $tmpB[0]->id_master : "";
                    $_SESSION[$cCode]['main']['pihakProjekName'] = isset($tmpB[0]->nama) ? $tmpB[0]->nama : "";
                    ...
                    $_SESSION[$cCode]['main']['pihakProjekGudangID'] = getDefaultWarehouseProject($arrItemsProjid[$idx], $_SESSION[$cCode]['main']['pihakProjekName'])["gudang_id"];
                    $_SESSION[$cCode]['main']['pihakProjekGudangName'] = getDefaultWarehouseProject($arrItemsProjid[$idx], $_SESSION[$cCode]['main']['pihakProjekName'])["gudang_nama"];
                    $_SESSION[$cCode]['main']['pihakProjekGudangNama'] = getDefaultWarehouseProject($arrItemsProjid[$idx], $_SESSION[$cCode]['main']['pihakProjekName'])["gudang_nama"];
                    //-------
                    $_SESSION[$cCode]['main']['pihakProjekWorkOrderID'] = isset($arrItemsWoid[$idx]) ? $arrItemsWoid[$idx] : "";
                    $_SESSION[$cCode]['main']['pihakProjekWorkOrderNama'] = isset($arrItemsWoNama[$idx]) ? $arrItemsWoNama[$idx] : "";
                    //-------
//arrPrintPink($_SESSION[$cCode]['main']);
//mati_disini(__LINE__);
                    if (!isset($_SESSION[$cCode]['items2'][$iID])) {
                        $_SESSION[$cCode]['items2'][$iID] = array();
                        $_SESSION[$cCode]['items2'][$iID][$idx]['jml'] = "1";
                        $_SESSION[$cCode]['items2'][$iID][$idx]['qty'] = "1";
                        $_SESSION[$cCode]['items2'][$iID][$idx]['pph_persen'] = $_SESSION[$cCode]['main']['pihakPph23'];
                        $_SESSION[$cCode]['items2'][$iID][$idx]['ppnFactor'] = $this->ppnFactor;
                        if (sizeof($advanceSubItemsLabel) > 0) {
                            foreach ($advanceSubItemsLabel as $skey => $sval) {
                                $_SESSION[$cCode]['items2'][$iID][$idx][$skey] = "";
                            }
                        }
                        ...
                        $_SESSION[$cCode]['items2'][$iID][$idx]["harga"] = $jmlParam;
                        $_SESSION[$cCode]['items2'][$iID][$idx]["nama"] = isset($arrItemsNama[$idx]) ? $arrItemsNama[$idx] : "";
                        $_SESSION[$cCode]['items2'][$iID][$idx]["name"] = isset($arrItemsNama[$idx]) ? $arrItemsNama[$idx] : "";
