...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        //--------------------------------
        $shoppingCartAdvanceFieldValidators = isset($this->configUi[$this->jenisTr]["shoppingCartAdvanceFieldValidators"]) ? $this->configUi[$this->jenisTr]["shoppingCartAdvanceFieldValidators"] : array();
...
//                            $errFields[$id] = array();
//                        }
                            foreach ($shoppingCartAdvanceFieldValidators as $field => $label) {
                                if (!isset($iSpec[$field])) {
                                    $errMsgs[] = "$label wajib diisi";
//                                $errLines[] = $id;
//                                $errFields[$id][] = $field;
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            foreach ($tmpHist as $row) {
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    $modul = isset($this->masterConfigUi[$row->jenis_master]['modul']) ? $this->masterConfigUi[$row->jenis_master]['modul'] : "";
                    ...
                    if (is_array($fLabel)) {
                        ...
                        if (isset($row->ids_his)) {
                            $returnVal = showHistoriGlobalNumbers($row->ids_his, $hisStep, true, $jenis_master);
                            if ($returnVal == "") {
                                $tmp[$fName] = "-";
                            }
                            else {
                        ...
                        $tmp[$fName] = isset($row->$fName) ? formatField_he_format($fName, $row->$fName, $jenis_master, $modul_path) : formatField_he_format($fName, 0, $jenis_master, $modul_path);
                    }

                    if ($fName == "no") {
                        $tmp[$fName] = formatField_he_format($fName, $numb, $jenis_master, $modul_path);
                    }
                }
...
//                    cekHitam("punya relasi uang muka");
                    ...
                    $tmp['keterangan'] .= "<span style='color:red;font-weight:bolder;'><br>segera SRN, jika sudah tiba.</span>";
                }
                //----
...
//                cekMerah(__LINE__ . " -/- " . $this->db->last_query());
                        ...
                        foreach ($tabHistoryFields as $key => $row) {
                            ...
                            switch ($key) {
                                ...
                                case "transaksi_id":
                                    ...
                                    $produk_btn = "<span href='" . MODUL_PATH . "ViewDetails/index/" . $rows->transaksi_id . "' name='qtips' class='btn btn-sm'> berisi " . sizeof($sumPrdOrdJmlTR[$rows->transaksi_id]) . " item, Total " . $totalSumPrdOrdJmlTR . " unit  </span>";
                                    $clickEvent = "";
    ...
    public function viewResume()
    {
        ...
        $pairRegistry = isset($this->configUi[$jenisMasterTrans]["pairRegistries"]) ? $this->configUi[$jenisMasterTrans]["pairRegistries"] : array("main", "items");
        ...
        if (sizeof($tmpEP) > 0) {
            foreach ($tmpEP as $ii => $row) {
                ...
                $epValues['sign_' . $row->id . '_' . $row->step_number] = array(
                    ...
                    "modul" => $this->masterConfigUi[$jenis_master]["modul"],
                    "modul_path" => base_url() . $this->masterConfigUi[$jenis_master]["modul"] . "/",
                );
            }
        }
...
//arrPrintCyan($sSpec);
                ...
                $modul = $sSpec['modul'];
                $jenis_master = $sSpec['jenis_master'];
                $modul_path = $sSpec['modul_path'];
...
//                $signStr .= "<td>" . ($sSpec['title']);
                ...
                $signStr .= formatField_he_format("print_label", $nomerOrigNota, $jenis_master, $modul_path);
                $signStr .= "</td>";
...
// matiHEre();
    ...
    //--------------------------------
    public function viewRequestItems__OLD()
    {
...
//            arrPrintPink($arrDataRequest);
                ...
                $pw = New MdlProjectWorkOrder();
                $pkw = New MdlProjectKomposisiWorkorder();

                $pp->addFilter("uang_muka_approved>0");
                $ppTmp = $pp->lookupAll()->result();
                if (count($ppTmp) > 0) {
                    $arrProject = array();
                    $arrProjectID = array();
                    $arrWorkOrder = array();
                    $arrWorkOrderID = array();
                    foreach ($ppTmp as $ppSpec) {
                        $arrProject[$ppSpec->id] = $ppSpec;
                        $arrProjectID[] = $ppSpec->id;
                    }

                    // workorder
                    $pw->addFilter("produk_id in ('" . implode("','", $arrProjectID) . "')");
                    $pwTmp = $pp->lookupAll()->result();
                    if (count($pwTmp) > 0) {
                        foreach ($pwTmp as $pwSpec) {
                            $arrWorkOrder[$pwSpec->id] = $pwSpec;
                            $arrWorkOrderID[] = $pwSpec->id;
                        }
                    }
                }
                    ...
                    foreach ($tabHistoryFields as $key => $row) {
                        switch ($key) {
                            case "produk_id":
                                ...
                                // $qty_purchase_l = "<a href='voi' onclick=\"$btn_po\">88</a>";
                                ...
                                $produk_id = array(
                                    // "select" => "<input type=$inputType name='prdID[]' value='" . $rows->produk_id . "' id='select_" . $key . "_" . $reqCtr . "' $clickEvent $clickEventMulti>",
                                    // "select" => "<input type=$inputType name='prdID[]' value='" . $rows->id . "' id='select_" . $key . "_" . $reqCtr . "' $clickEvent $clickEventMulti>",
...
//cekHijau(":: $origJenisTr :: $cCode :: $cCodeOrig ::");
        ...
        $url = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/$jenisTr";
        $tmpByReq = array();
        $tmpHeaderRequest = array();
        $tmpHeaderRequest[0] = array(
            "label" => "ALL",
            "link" => "$('#requestItems').load('$url/0');",
        );
        if (isset($this->configUi[$this->uri->segment(4)]['tabRequestCode'])) {
            $masterRefCode = $this->configUi[$this->uri->segment(4)]['tabRequestCode']['masterCode'];
            $stateRefCode = $this->configUi[$this->uri->segment(4)]['tabRequestCode']['stateCode'];
...
//            cekHere("$masterRefCode :: $stateRefCode :: $stateRefNum");
            ...
            if ($pakai_ini == 1) {
                ...
                //------------------------------------------------------
                $this->load->model("Coms/ComTransaksiJasa");
//                $this->load->model("Coms/ComRekeningPembantuProduk");
//                $this->load->model("Mdls/MdlProduk");
                //------------------------------------------------------
                $ct = New ComTransaksiJasa();
                $ct->addFilter("qty_kredit>0");
//                if ($segment5 > 0) {
//                    $ct->addFilter("extern_id='$segment5'");// select per-work order........
//                }
//
                $ct->addFilter("rekening in ('" . implode("','", $arrJenis) . "')");
                $ctTmp = $ct->lookupAll()->result();
...
//                //            arrPrintWebs($stockSupplies);
                ...
                if (sizeof($ctTmp) > 0) {
                    foreach ($ctTmp as $spec) {
                        ...
                        if ($spec->rekening == "h582so") {
                            $spTmp[] = array(
                                "extern_id" => $spec->extern_id,
                                "extern_nama" => $spec->extern_nama,
                                "extern2_id" => $spec->extern2_id,
                                "extern2_nama" => $spec->extern2_nama,
                                "extern3_id" => $spec->extern3_id,
                                "extern3_nama" => $spec->extern3_nama,
                                "extern4_id" => $spec->extern4_id,
                                "extern4_nama" => $spec->extern4_nama,
                                "qty_kredit" => $spec->qty_kredit,
                                "qty_debet" => $spec->qty_debet,
                                "kredit" => $spec->kredit,
                                "debet" => $spec->debet,
                            );
                            $tmpHeaderRequest[$spec->extern_id] = array(
                                "label" => $spec->extern_nama,
                                "link" => "$('#requestItems').load('$url/$extID');",
                            );
                        }
                    }
                }
...
//arrPrintPink($tmpHeaderRequest);
            if (sizeof($spTmp) > 0) {
                $arrIds = array();
                $reqCtr = 0;
                foreach ($spTmp as $rows) {
                    $rID = $rows["extern_id"];
                    $rNama = $rows["extern_nama"];
                    $rJasaID = $rows["extern2_id"];
                    $rJasaNama = $rows["extern2_nama"];
                    $projectID = $rows["extern3_id"];
                    $projectNama = $rows["extern3_nama"];
//                    $arrIds[] = $rows["extern_id"];
//                    arrPrintHijau($rows);
                    $reqCtr++;
                    foreach ($tabHistoryFields as $key => $row) {
                        switch ($key) {
...
//                                $qty_sales_order = isset($arrDataRequest['h582so'][$rID]['kredit']) ? $arrDataRequest['h582so'][$rID]['kredit'] : 0;
                                $qty_sales_order = isset($rows['kredit']) ? $rows['kredit'] : 0;
                                $qty_stock = isset($stockSupplies[$rID]) ? $stockSupplies[$rID] : 0;
                                $qty_purchase = isset($arrDataRequest['1467'][$rID]['kredit']) ? $arrDataRequest['1467'][$rID]['kredit'] : 0;
                                $qty_purchase_request = isset($arrDataRequest['1466'][$rID]['kredit']) ? $arrDataRequest['1466'][$rID]['kredit'] : 0;
                                $qty_buffer = 0;
                                // $qty_new_order = ($qty_sales_order - $qty_stock + $qty_buffer) - $qty_purchase;
                                $qty_new_order = $qty_stock - $qty_sales_order + $qty_purchase - $qty_buffer + $qty_purchase_request;
                                if ($qty_new_order > 0) {
                                ...
                                $produk_id = array(
                                    // "select" => "<input type=$inputType name='prdID[]' value='" . $rows->produk_id . "' id='select_" . $key . "_" . $reqCtr . "' $clickEvent $clickEventMulti>",
                                    // "select" => "<input type=$inputType name='prdID[]' value='" . $rows->id . "' id='select_" . $key . "_" . $reqCtr . "' $clickEvent $clickEventMulti>",
                                    "select" => "<input type='$inputType' id='$rJasaID' name='prdID[]' class='' ponama='$rJasaNama' 
                                        povalue='$qty_new_order' woid='$rID' wonama='$rNama' 
                                        projid='$projectID' projnama='$projectNama'  
                                        onclick='initBtn();resetCalcCheckAll()' 
                                        onclicks=\"document.getElementById('result').src='$link_ceklist'\">",
                                    "id" => $rID,
                                    // "dtime" => formatField_he_format('dtime', $rows->dtime) . "<br><span class='meta'>" . timeSince(strtotime($rows->dtime)) . "</span>",
                                    "nama" => isset($rNama) ? $rNama : "-",
                                    "nama_jasa" => isset($rJasaNama) ? $rJasaNama : "-",
                                    "satuan" => "-",
//                                    "satuan" => isset($rows->satuan) ? $rows->satuan : "-",
                                    // "nomer_top" => implode("<br>", $listingTrx),
                                    // "produk_ord_jml" => $sumPrdOrdJmlID[$rows->produk_id],
                                ...
                                $tmpTabData[$key][] = $produk_id;
                                $tmpTabData2[$rID][$key][] = $produk_id;
                                break;
                        }
                    }
                }
//                arrPrintPink($tmpTabData);
                $arrayOnprogress2 = $tmpTabData;
                $arrayOnprogress2Wo = $tmpTabData2;
            }
...
//                arrPrintWebs($arrayOnprogress2);
        //        cekBiru("DAFTAR REQUEST PEMBELIAN BAHAN BAKU");
//arrPrintPink($arrayOnprogress2Wo);
        $btnShopCrt = MODUL_PATH . "_processSelectProduct/multiSelectBi/$jenisTr";
//        $url = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/$jenisTr/";
        $data = array(
            "mode" => $this->uri->segment(3),
            ...
            "arrayOnProgress" => $arrayOnprogress,
            "arrayOnProgress2" => $arrayOnprogress2,
            "arrayOnProgress2Wo" => $arrayOnprogress2Wo,
            "needToClear" => $needToClear,
            "reqFormTarget" => $reqFormTarget,
            "allowMultiSelect" => $allowMultiSelect,
            "overDue" => $arrOver_due,
            ...
            "supplier_id" => $supplier_id,
            //---------------
            "tmpHeaderRequest" => $tmpHeaderRequest,
            "currentSegment" => isset($segment5) ? $segment5 : 0,
        );
