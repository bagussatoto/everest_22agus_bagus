...
//            arrPrintPink($main);
    ...
    public function viewTimelinePdf()
    {
        ...
        // 2. Gateway Resolution via transaksi_payment_source (ISO Precision Scope Locker)
        $visitedIds = array();

        if (isset($startTrx->id) && $startTrx->id > 0) {
            $visitedIds[(int)$startTrx->id] = true;
            ...
            if (!empty($psRow) && !empty($psRow->project_id)) {
                ...
                foreach ($psAllRows as $pR) {
                    if (!empty($pR->transaksi_id) && is_numeric($pR->transaksi_id)) {
        ...
        $collectIdsFromRow = function($row) use (&$visitedIds) {
            ...
            if (isset($row->id) && $row->id > 0) { $visitedIds[$row->id] = true; }
            if (isset($row->id_master) && $row->id_master > 0) { $visitedIds[$row->id_master] = true; }
            if (isset($row->id_top) && $row->id_top > 0) { $visitedIds[$row->id_top] = true; }

            if (!empty($row->ids_prev)) {
                $prevArr = explode("|", str_replace(",", "|", $row->ids_prev));
                foreach ($prevArr as $pId) {
                    $cleanPId = trim($pId);
                    if (is_numeric($cleanPId) && $cleanPId > 0) { $visitedIds[$cleanPId] = true; }
                }
            }
            ...
            if (!empty($row->ids_ref)) {
                ...
                if (is_array($decodedRef)) {
                    foreach ($decodedRef as $rKey => $rVal) {
                        if (is_numeric($rKey) && $rKey > 0) { $visitedIds[$rKey] = true; }
                        if (is_numeric($rVal) && $rVal > 0) { $visitedIds[$rVal] = true; }
                    }
                } else if (is_numeric($row->ids_ref) && $row->ids_ref > 0) {
                    $visitedIds[$row->ids_ref] = true;
                }
            }
            ...
            if (!empty($row->ids_his)) {
                ...
                if (is_array($decodedHis)) {
                    foreach ($decodedHis as $stepKey => $stepVal) {
                        if (is_array($stepVal)) {
                            if (isset($stepVal['trID']) && is_numeric($stepVal['trID']) && $stepVal['trID'] > 0) {
                                $visitedIds[$stepVal['trID']] = true;
                            }
                            if (isset($stepVal['id']) && is_numeric($stepVal['id']) && $stepVal['id'] > 0) {
                                $visitedIds[$stepVal['id']] = true;
                            }
                        }
                    }
                }
        ...
        // Fetch parent/linked rows to discover master_project IDs transitively (Batch Query Optimized - Zero MdlTransaksi/Registry Calls!)
        $processedIds = array();
        while (true) {
            $unprocessed = array();
            foreach (array_keys($visitedIds) as $vK) {
                if (is_numeric($vK) && (int)$vK > 0 && !isset($processedIds[$vK])) {
                    $unprocessed[] = (int)$vK;
                }
            }
            if (empty($unprocessed)) break;

            foreach ($unprocessed as $uId) {
                $processedIds[$uId] = true;
            }

            $this->db->select("transaksi.id, transaksi.id_master, transaksi.id_top, transaksi.ids_prev, transaksi.ids_ref, transaksi.ids_his");
            $this->db->from("transaksi");
            $this->db->where_in("transaksi.id", $unprocessed);
            $pQ = $this->db->get();
            $pRows = is_object($pQ) ? $pQ->result() : array();

            foreach ($pRows as $pR) {
                $collectIdsFromRow($pR);
            }
        }

        // If this is a project transaction, discover all sibling termins referencing the root Master Project ID
        $isProject = false;
        ...
        if ($isProject && sizeof($visitedIds) > 0) {
            $parentChainIds = array();
            foreach (array_keys($visitedIds) as $vK) {
                if (is_numeric($vK) && (int)$vK > 0) {
                    $parentChainIds[] = (int)$vK;
                }
            }

            if (sizeof($parentChainIds) > 0) {
                $rootLookup = array_flip($parentChainIds);

                $this->db->select("transaksi.id, transaksi.id_master, transaksi.id_top, transaksi.ids_prev, transaksi.ids_ref");
                $this->db->from("transaksi");
                ...
                $this->db->where("(transaksi.id_master IN (" . implode(",", $parentChainIds) . ") OR transaksi.id_top IN (" . implode(",", $parentChainIds) . ") OR transaksi.id IN (" . implode(",", $parentChainIds) . "))", null, false);
                $this->db->where("(transaksi.jenis = '7499' OR transaksi.jenis_master = '588' OR transaksi.jenis_master = '7499')", null, false);
                $sibQ = $this->db->get();
                $sibRows = is_object($sibQ) ? $sibQ->result() : array();

                foreach ($sibRows as $sR) {
                    if (empty($sR->id) || isset($visitedIds[$sR->id])) continue;

                    $matched = false;
                    if (!empty($sR->id_master) && isset($rootLookup[$sR->id_master])) {
                        $matched = true;
                    } elseif (!empty($sR->id_top) && isset($rootLookup[$sR->id_top])) {
                        $matched = true;
                    } elseif (!empty($sR->ids_prev)) {
                        $pArr = explode("|", str_replace(",", "|", $sR->ids_prev));
                        foreach ($pArr as $pVal) {
                            $cP = trim($pVal);
                            if (!empty($cP) && isset($rootLookup[$cP])) {
                                $matched = true;
                                break;
                            }
                        }
                    } elseif (!empty($sR->ids_ref)) {
                        $decodedRef = blobDecode($sR->ids_ref);
                        if (is_array($decodedRef)) {
                            foreach ($decodedRef as $rK => $rV) {
                                if (isset($rootLookup[$rK]) || isset($rootLookup[$rV])) {
                                    $matched = true;
                                    break;
                                }
                            }
        ...
        // Discover forward payment receipt & DP transactions referencing the active project parent chain (Fast Indexed Query)
        if (sizeof($allTargetIds) > 0) {
            $targetLookupIds = isset($parentChainIds) && sizeof($parentChainIds) > 0 ? $parentChainIds : $allTargetIds;
            $targetLookup = array_flip($targetLookupIds);

            $this->db->select("transaksi.id, transaksi.id_master, transaksi.id_top, transaksi.ids_ref, transaksi.ids_prev, transaksi.jenis");
            $this->db->from("transaksi");
            $this->db->where("transaksi.trash", "0");
            $this->db->where("transaksi.trash_4", "0");
            $this->db->where("(transaksi.cancel_id IS NULL OR transaksi.cancel_id = '0' OR transaksi.cancel_id = '')", null, false);
            $this->db->where("transaksi.link_id", "0");
            $this->db->where("(transaksi.id_master IN (" . implode(",", $targetLookupIds) . ") OR transaksi.id_top IN (" . implode(",", $targetLookupIds) . "))", null, false);

            $fwParts = array(
                "transaksi.jenis = '749'", "transaksi.jenis_master = '749'", "(transaksi.jenis LIKE '%749%' AND transaksi.jenis NOT LIKE '%7499%')",
                "transaksi.jenis = '4467'", "transaksi.jenis_master = '4467'",
                "transaksi.jenis = '583'", "transaksi.jenis_master = '583'"
            );
            $this->db->where("(" . implode(" OR ", $fwParts) . ")", null, false);

            $fwQuery = $this->db->get();
            $fwRows = is_object($fwQuery) ? $fwQuery->result() : array();

            foreach ($fwRows as $fwR) {
                if (empty($fwR->id)) continue;
                $visitedIds[(int)$fwR->id] = true;
            }
        }

        $allTargetIds = array_keys($visitedIds);

        // 3. Query main transaction headers (link_id = '0') across all discovered IDs
        $this->db->select("transaksi.*");
        $this->db->from("transaksi");
        $this->db->where("transaksi.trash", "0");
        $this->db->where("transaksi.trash_4", "0");
        $this->db->where("(transaksi.cancel_id IS NULL OR transaksi.cancel_id = '0' OR transaksi.cancel_id = '')", null, false);
        $this->db->where("transaksi.link_id", "0"); // Prevents 2x duplicate rows!
        ...
        foreach ($timelineSteps as $step) {
            if (!isset($seenIds[$step->id])) {
                ...
                // Identify payment receipt & DP steps vs invoice steps (7499 is Termin Invoice, 749 is A/R Receipt, 4467/583 is DP)
                $isReceiptStep = false;
                if (isset($step->jenis)) {
                ...
                if ($isReceiptStep) {
                    // Pembayaran penerimaan kas/bank: transaksi_nilai SUDAH INCL PPN! JANGAN ditambah 11% lagi!
                    $nomIncl = $net > 0 ? $net : $nilai;
                    $ppnStep = $ppnVal > 0 ? $ppnVal : ($nomIncl - ($nomIncl / 1.11));
                } else {
                    // Invoice / SPK: transaksi_nilai adalah DPP (Excl PPN). Hitung Incl PPN!
                    if ($net > 0) {
                        $nomIncl = $net;
        ...
        if ($totalTagihan == 0 && isset($startTrx->transaksi_nilai)) {
            ...
            $sPpn = isset($startTrx->transaksi_ppn) ? (float)$startTrx->transaksi_ppn : 0;
            if ($sNet > 0) {
                $totalTagihan = $sNet;
