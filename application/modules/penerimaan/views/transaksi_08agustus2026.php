...
//        $cPrint    .= "<SMALL>------------------------------------------";
    ...
    case "selectPaymentSrc":
        ...
        if (sizeof($items) > 0) {
            ...
            $hiddenDefaultCol = array(4, 7, 8, 9, 10, 14);
            $cIdx = 1;
            foreach ($itemLabels as $key => $label) {
                $lName = is_array($label) ? $label["label"] : $label;
                $chkStr = !in_array($cIdx, $hiddenDefaultCol) ? "checked='checked'" : "";
...
//                $strContent .= "<tr line=" . __LINE__ . " name='popovers' idso='$soID' noso='$soNum' linkso='$linkMovePayment' exso='$extern_id' ajax=\"false\" id=\"popover_$soID\" oondblclick=\"openPopover('$soID','$soNum','$linkMovePayment','$extern_id');\"  >";
                ...
                if (sizeof($itemLabels) > 0) {
                    ...
                    if (($pembayaranBerurutan == true) && ($lanjutBerurutan == true)) {
                        ...
                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "' 
                                            onclick=\"if(!validatePaymentSrcCheckbox(this)){this.checked=false;return false;}document.getElementById('result').src='" . MODUL_PATH . "$selectProcessor/$jenisTr" . "?$qstr&mp=$marketplace&state='+this.checked;\"
                        >";
                    }
                   ...
                   var table = $('#payment-xxx').DataTable({
                                   ...
                                   footerCallback: function ( row, data, start, end, display ) {
                                       ...
                                       jQuery.each( $(api.column(7, { page: 'current'}).data()), function(i, obj){
                                           pageTotal7 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 7 ).footer() ).html(\"<div class='text-right text-primary text-bold'>\" + addCommas(pageTotal7) + \"</div>\");

                                       var pageTotal8=0;
                                       jQuery.each( $(api.column(8, { page: 'current'}).data()), function(i, obj){
                                           pageTotal8 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 8 ).footer() ).html(\"<div class='text-right text-warning text-bold'>\" + addCommas(pageTotal8) + \"</div>\");

                                       var pageTotal9=0;
                                       jQuery.each( $(api.column(9, { page: 'current'}).data()), function(i, obj){
                                           pageTotal9 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 9 ).footer() ).html(\"<div class='text-right text-info text-bold'>\" + addCommas(pageTotal9) + \"</div>\");

                                       var pageTotal10=0;
                                       jQuery.each( $(api.column(10, { page: 'current'}).data()), function(i, obj){
                                           pageTotal10 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 10 ).footer() ).html(\"<div class='text-right text-info text-bold'>\" + addCommas(pageTotal10) + \"</div>\");

                                       var pageTotal11 = 0;
                                       jQuery.each( $(api.column(11, { page: 'current'}).data()), function(i, obj){
                                           pageTotal11 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 11 ).footer() ).html(\"<div class='text-right text-primary text-bold'>\" + addCommas(pageTotal11) + \"</div>\");

                                       var pageTotal12 = 0;
                                       jQuery.each( $(api.column(12, { page: 'current'}).data()), function(i, obj){
                                           pageTotal12 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 12 ).footer() ).html(\"<div class='text-right text-success text-bold'>\" + addCommas(pageTotal12) + \"</div>\");

                                       var pageTotal13 = 0;
                                       jQuery.each( $(api.column(13, { page: 'current'}).data()), function(i, obj){
                                           pageTotal13 += intVal( typeof obj === 'string' ? obj : $(obj).html() );
                                       });
                                       $( api.column( 13 ).footer() ).html(\"<div class='text-right text-danger text-bold'>\" + addCommas(pageTotal13) + \"</div>\");
                                   }
                               });
...
//                    });
    ...
    // END OF COMPLETE REPEATED LOGIC
        $strContent .= "<script>
                            function validatePaymentSrcCheckbox(el){
                                if(!el.checked){
                                    return true;
                                }
