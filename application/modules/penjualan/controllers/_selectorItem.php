...
class _selectorItem extends Modul_Controller
{
    ...
    public function selectItem()
    {
        ...
        $showStockBadge = isset($this->configUi[$jenisTr]['selectorStockBadge']) ? ($this->configUi[$jenisTr]['selectorStockBadge'] == true) : false;
        $items = array();
...
// START OF COMPLETE REPEATED LOGIC
            ...
            foreach ($tmpO as $prodItems) {
                $prodIds[] = isset($prodItems->produk_id) ? $prodItems->produk_id : $prodItems->id;
            }
...
// START OF COMPLETE REPEATED LOGIC
            // Query Stok Fisik & Booking per Produk untuk Gudang/Cabang Aktif (Hanya jika konfigurasi selectorStockBadge aktif)
            $arrStokFisik = array();
            $lsb_datas = array();
            $gudang_status_id = 1;
            if ($showStockBadge) {
                if (sizeof($prodIds) > 0) {
                    ...
                    $cs->setFilters(array());
                    $cs->addFilter("state='active'");
                    $cs->addFilter("jenis='produk'");
                    if (isset($cID) && $cID != "" && $cID != "0" && $cID != "-555") {
                        $cs->addFilter("cabang_id='$cID'");
                    }
                    if (isset($gID) && $gID != "" && $gID != "0" && $gID != "-555") {
                        $cs->addFilter("gudang_id='$gID'");
                    }
                    $cs->addFilter("produk_id in ('" . implode("','", $prodIds) . "')");
                    $tmpStokRows = $cs->lookupAll()->result();
                    if (sizeof($tmpStokRows) > 0) {
                        foreach ($tmpStokRows as $sRow) {
                            if (!isset($arrStokFisik[$sRow->produk_id])) {
                                $arrStokFisik[$sRow->produk_id] = 0;
                            }
                            $arrStokFisik[$sRow->produk_id] += ($sRow->jumlah * 1);
                        }
                    }
                }
                ...
                $gudang_status_id = isset($tableIn_master['gudang_status_id']) ? $tableIn_master['gudang_status_id'] : 1;
            }
...
//                $pID = isset($row->$selectorParamFields['id']) ? $row->$selectorParamFields['id'] : $row->id;
                ...
                // Perhitungan Stok Fisik, Booking, dan Available (hanya jika selectorStockBadge aktif)
                $badgeStok = "";
                if ($showStockBadge) {
                    $stokFisik = isset($arrStokFisik[$pID]) ? ($arrStokFisik[$pID] * 1) : 0;
                    $stokBooking = 0;
                    if (isset($lsb_datas[$cID][$pID][$gudang_status_id]['sum_valid_qty'])) {
                        $stokBooking = $lsb_datas[$cID][$pID][$gudang_status_id]['sum_valid_qty'] * 1;
                    } elseif (isset($lsb_datas[$pID][$gudang_status_id]['sum_valid_qty'])) {
                        $stokBooking = $lsb_datas[$pID][$gudang_status_id]['sum_valid_qty'] * 1;
                    }
                    $stokAvailable = $stokFisik - $stokBooking;

                    $kategoriId = isset($row->kategori_id) ? $row->kategori_id : (isset($tmpP[$pID]->kategori_id) ? $tmpP[$pID]->kategori_id : 0);
                    if ($kategoriId == 4) {
                        $badgeStok = "<span class='badge bg-blue pull-right' style='font-size:0.85em;margin-left:6px;background-color:#007bff;color:#fff;'>Jasa</span>";
                    } else {
                        if ($stokAvailable > 0) {
                            $badgeStok = "<span class='badge bg-green pull-right' style='font-size:0.85em;margin-left:6px;background-color:#28a745;color:#fff;' title='Fisik: $stokFisik | Booking: $stokBooking'>Stok: " . number_format($stokAvailable) . "</span>";
                        } else {
                            $badgeStok = "<span class='badge bg-red pull-right' style='font-size:0.85em;margin-left:6px;background-color:#dc3545;color:#fff;' title='Fisik: $stokFisik | Booking: $stokBooking'>Stok: " . number_format($stokAvailable) . " (Habis)</span>";
                        }
                    }
                }
                ...
                if (sizeof($selectorFields) > 0) {
                    ...
                    $tmp['label'] = ($tmp['label2']) . $serial_number . ($badgeStok != "" ? " " . $badgeStok : "");
