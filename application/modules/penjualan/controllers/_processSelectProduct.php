...
class _processSelectProduct extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        $cekStok = isset($this->configUi[$this->jenisTr]['cekStok'][1]) ? $this->configUi[$this->jenisTr]['cekStok'][1] : array();
        $pairMakers = isset($this->configUi[$this->jenisTr]['pairMakers'][1]) ? $this->configUi[$this->jenisTr]['pairMakers'][1] : array();
        $pairInjectors = isset($this->configUi[$this->jenisTr]['pairInjectors'][1]) ? $this->configUi[$this->jenisTr]['pairInjectors'][1] : array();
...
//        matiHere(__LINE__ . " $ppnFactorInclude");
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                $tmpJml = 1;
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true && $produk_jenis_id != 4) {
                ...
                if ($cekStok == true) {
                    if (sizeof($pairMakers) > 0) {
                        foreach ($pairMakers as $pmaker => $pmakerSpec) {
                            $cekStok = $this->cekStok($produk_id, $pmakerSpec["params"]);
                            if (isset($pairInjectors[$pmaker])) {
                                foreach ($pairInjectors[$pmaker] as $gate => $gateSpec) {
                                    $targetKolom = $gateSpec["targetColumn"];
                                    if (sizeof($cekStok) > 0) {
                                        foreach ($cekStok as $pid => $jml_stok) {
                                            if (isset($_SESSION[$cCode][$gate][$pid])) {
                                                $_SESSION[$cCode][$gate][$pid][$targetKolom] = $jml_stok;
                                            }
                                        }
                                    }
...
//                mati_disini(__LINE__);
                ...
                $activeCabangID = isset($this->session->login['cabang_id']) ? $this->session->login['cabang_id'] : 0;
                $stok_booking = 0;
                if($cekStokBooking == true){
                    if (isset($lsb_datas[$activeCabangID][$id][$gudang_status_id]['sum_valid_qty'])) {
                        $stok_booking = $lsb_datas[$activeCabangID][$id][$gudang_status_id]['sum_valid_qty'];
                        cekKuning("ATAS: $stok_booking || [$activeCabangID][$id][$gudang_status_id]");
                    }
//                    elseif (isset($lsb_datas[$id][$gudang_status_id]['sum_valid_qty'])) {
//                        $stok_booking = $lsb_datas[$id][$gudang_status_id]['sum_valid_qty'];
//                        cekKuning("BAWAH: $stok_booking");
//                    }
                    else{
                        $stok_booking = 0;
                        cekKuning("KOSONG: $stok_booking");
                    }
                }
                ...
                /* ---------------------------------------------------------------------------
                 * Pengecekan stok ketersediaan untuk penentuan harga otomatis termahal jika stok 0
                 * ---------------------------------------------------------------------------*/
                $pihak_kategori_kirim = isset($_SESSION[$cCode]["main"]["pihakMainJenis"]) ? $_SESSION[$cCode]["main"]["pihakMainJenis"] : "";
//                $stok_lokasi = ($pihak_kategori_kirim == "cabang") ? (isset($tmp['stok']) ? $tmp['stok'] : 0) : (isset($tmp['stok_center']) ? $tmp['stok_center'] : 0);
                if($pihak_kategori_kirim == "cabang"){
                    $stok_lokasi = isset($tmp['stok']) ? $tmp['stok'] : 0;
                }
                elseif($pihak_kategori_kirim == "pusat"){
                    $stok_lokasi = isset($tmp['stok_center']) ? $tmp['stok_center'] : 0;
                }
                else{
                    $stok_lokasi = 0;
                }

                $stok_ready = $stok_lokasi - $stok_booking;
                $stok_total = isset($tmp['stok_total']) ? $tmp['stok_total'] : (isset($tmp['stok']) ? $tmp['stok'] : 0);
                $stok_total_booking = $stok_total - $stok_booking;

                $arrHargaPilihan = array(
                    isset($pro_harga) ? $pro_harga : 0,
                    isset($pro_harga_reseller) ? $pro_harga_reseller : 0,
                    isset($pro_harga_online) ? $pro_harga_online : 0
                );
                $hargaMaxTermahal = max($arrHargaPilihan);
                cekKuning("[pihak_kategori: $pihak_kategori] [stok_booking: $stok_booking] [stok_ready: $stok_ready] [stok_lokasi: $stok_lokasi]");
//
//                if (($stok_ready <= 0 || $stok_total <= 0) && ($hargaMaxTermahal > 0)) {
//                if (($stok_total <= 0) && ($hargaMaxTermahal > 0)) {
//                if (($stok_total_booking <= 0) && ($hargaMaxTermahal > 0) && ($produk_jenis_id != 4)) {
                if (($stok_ready <= 0) && ($hargaMaxTermahal > 0) && ($produk_jenis_id != 4)) {
                    $harga_yg_dipakai = $hargaMaxTermahal;
                    $jual_dipakai = $hargaMaxTermahal;
                    cekHitam("TERMAHAL [$stok_total_booking] [$produk_jenis_id] [stok_total: $stok_total] [stok_booking: $stok_booking]");
                }
                else {
                    cekHitam("STANDAR");
                    /** ---------------------------------------------------------------------------
...
//                mati_disini("[stok_lokasi: $stok_lokasi] [stok_ready: $stok_ready] [stok_total: $stok_total] [jual_dipakai: $jual_dipakai] [harga_yg_dipakai: $harga_yg_dipakai]");
...
// START OF COMPLETE REPEATED LOGIC
        if (isset($_GET['minValue']) || isset($_GET['rowid']) || isset($_GET['newQty'])) {
            echo "<script>";
            echo "top.console.log('shoppingcart direload');";
...
// END OF COMPLETE REPEATED LOGIC
    ...
    public function remove()
    {
        ...
        if (isset($_GET['id'])) {
            ...
            $hadiahTmp = $ld->callHadiahPenjualanProduk($_SESSION[$cCode]['items']);
            if (count($hadiahTmp) > 0) {
                if (isset($hadiahTmp["detail"][$id])) {
                    $rel_id = $hadiahTmp["detail"][$id]["produk_rel_id"];
                    unset($_SESSION[$cCode]['items'][$rel_id]);
                    //buat test
                    ...
                    if (count($tmpB) > 0) {
                        foreach ($tmpB as $d_h => $dataProdukHadiah) {
                            $tmpHadiah = array(
                                ...
                                "harga" => $hadiahData["produk_rel_harga"],
                                "subtotal" => $hadiahData["jml"] * $hadiahData["produk_rel_harga"],
                                "satuan" => strlen($dataProdukHadiah->satuan) > 0 ? $dataProdukHadiah->satuan : "n/a",
                                "disc_percent" => 100,
                                "discount_qty" => 0,
                                "harga_jasa" => 0,
                                "disc" => $hadiahData["produk_rel_harga"],
...
//                            arrprint($fieldSrcs);
                            foreach ($fieldSrcs as $key => $src) {
                                if (is_array($src) && sizeof($src) > 0) {
                                    ...
                                    $tmpHadiah[$key] = makeValue($src, $tmpHadiah, $tmpHadiah, isset($dataProdukHadiah->$src) ? $dataProdukHadiah->$src : 0);
                                    //                            cekHere("hasilnya $key -> " . $tmp[$key]);
                                }
                            }
                            //                            arrPrint($tmpHadiah);
...
//                $_SESSION[$cCode]["items5_sum"]=$hadiahTmp;
    ...
    public function cekStok($pid, $paramsFilter)
    {
