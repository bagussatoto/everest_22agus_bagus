...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        $shoppingCartSumEditableFields = isset($this->configUi[$this->jenisTr]['shoppingCartSumEditableFields'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartSumEditableFields'][$stepNumber] : array();
        ...
        $shoppingCartStokKurang = isset($this->configUi[$this->jenisTr]['shoppingCartStokKurang']) ? $this->configUi[$this->jenisTr]['shoppingCartStokKurang'] : array();
        $cekStokDiskonDisabled = isset($this->configUi[$this->jenisTr]['cekStokDiskonDisabled'][1]) ? $this->configUi[$this->jenisTr]['cekStokDiskonDisabled'][1] : false;
        $cekStokDiskonDisabledFields = isset($this->configUi[$this->jenisTr]['cekStokDiskonDisabledFields'][1]) ? $this->configUi[$this->jenisTr]['cekStokDiskonDisabledFields'][1] : array();
        $cekStokBooking = isset($this->configUi[$this->jenisTr]['cekStokBooking'][1]) ? $this->configUi[$this->jenisTr]['cekStokBooking'][1] : false;
        //----------------------------------------------
         ...
         * session shopngcart
         * ----------------------------------------------------------------*/
        $sessCcode = $_SESSION[$cCode];
        $sesMains = $sessMain = $sessCcode['main'];
        $sessItems = $sessCcode['items'];
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($sessItems as $id => $iSpec) {
                    ...
                    if ($cekStokBooking == true) {
                        $stok_booking = isset($tmp['stok_booking']) ? ($tmp['stok_booking'] * 1) : 0;
                    }
                    else {
                        $stok_booking = 0;
                        $tmp['stok_booking'] = 0;
                    ...
                    if (isset($tmp['kategori_id']) && $tmp['kategori_id'] == 4) {
                        $tmp['stok_ready'] = 0;
                        $tmp['stok_ready_input'] = 0;
                        $tmp['stok'] = 0;
                    }
                    else {
                        $itemJmlSession = isset($iSpec['jml']) ? ($iSpec['jml'] * 1) : 0;
                        $isPusat = false;
                        $pihakMainJenis = isset($sesMains['pihakMainJenis']) ? strtolower(trim($sesMains['pihakMainJenis'])) : "";
                        $pihakMainName = isset($sesMains['pihakMainName']) ? strtolower(trim($sesMains['pihakMainName'])) : "";
                        if ($pihakMainJenis == "pusat" || $pihakMainJenis == "center" ||
                            $pihakMainJenis == "dc" || strpos($pihakMainName, "pusat") !== false || strpos($pihakMainName, "center") !== false || strpos($pihakMainName, "dc") !== false) {
                            $isPusat = true;
                        }
                        if ($isPusat) {
                            $stokFisik = isset($tmp['stok_center']) ? ($tmp['stok_center'] * 1) : 0;
                            $tmp['stok_ready_input'] = ($stokFisik - $stok_booking) - $itemJmlSession;
                            $tmp['stok_center'] = $tmp['stok_center'];//stok_ready_input
                            $tmp['stok'] = $tmp['stok'];//stok_ready_input
                            $tmp['stok_ready'] = $tmp['stok_ready_input'];
                        }
                        else {
                            $stokFisik = isset($tmp['stok']) ? ($tmp['stok'] * 1) : 0;
                            $tmp['stok_ready_input'] = ($stokFisik - $stok_booking) - $itemJmlSession;
                            $tmp['stok'] = $tmp['stok'];//stok_ready_input
                            $tmp['stok_ready'] = $tmp['stok_ready_input'];
                        }
                    }
                    $stok_total_booking = (isset($tmp['stok_total']) ? $tmp['stok_total'] : 0) - $stok_booking - (isset($iSpec["jml"]) ? $iSpec["jml"] : 0);
                    if ($tmp['stok_ready_input'] < 0) {
                        if (isset($tmp["kategori_id"]) && $tmp["kategori_id"] != 4) {
                            $arrProdukStokKurang[$iSpec["id"]] = array(
                                "id" => $iSpec["id"],
                                "nama" => $iSpec["nama"],
                                "jumlah" => $tmp['stok_ready_input'],
                            );
                            $disabledDiskonItems[$iSpec["id"]] = $iSpec["id"];
                        }
                    }
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "penjualan/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "penjualan/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
                    $tmp["allowedEdit"] = isset($iSpec["allowEdit"]) ? $iSpec["allowEdit"] : true;
...
//cekHere("-----------------");
//cekHere(count($arrProdukStokKurang));
//cekHere(count($disabledDiskonItems));
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items2_sum'])) {
                ...
                $valItem = 0;
                $valDiscCek = 0;
                //reset disckon dari item biar tidak dihiting duakali karena dihitung dari sub item
                unset($main["disc"]);
...
//                            $main[$key] = $valItem;
        ...
        //region diskon event diskon tambahan
        /* ---------------------------------------------------------------------------------
        ...
        * ---------------------------------------------------------------------------------*/
        ...
        if (isset($_SESSION[$cCode]['main']['pihakLevel'])) {
            ...
            // arrPrintHijau($data_mains);
            $pihakLevel = $data_mains['pihakLevel'];
            // $harga_nppn = isset($data_mains['harga_nett1']) ? $data_mains['harga_nett1'] : 0;
            ...
            // cekKuning($data_mains["harga_net"]);
            $macem_diskons = array();
            ...
            //             arrPrintKuning($macem_diskons);
            // arrPrintKuning($macem_diskons["diskon"]);
            ...
            // arrPrintHijau($diskon_cashbacks);
            ...
            //            arrPrintWebs($info_diskon_cashbacks);
        ...
        // $_SESSION[$cCode]['main']['nett1_bulat'] = 9999;
...
//                                        mati_disini();
                                    ...
                                    if (sizeof($tmpo) == 1) {
                                        $usedKey = $eSpec['key'];
                                        $defValueSrc = makeValue($tmpo[0]->$usedKey, $_SESSION[$cCode]["main"], $_SESSION[$cCode]["main"], $tmpo[0]->$usedKey);
        ...
        if (sizeof($elementConfigs) > 0) {
            ...
            if (sizeof($receiptElementsReplacer) > 0) {
                if (isset($receiptElementsReplacer["enabled"]) && ($receiptElementsReplacer["enabled"] == 1)) {
                    if (isset($_SESSION[$cCode]["main"]["pihakTipeID"]) && ($_SESSION[$cCode]["main"]["pihakTipeID"] == $receiptElementsReplacer["tipe_penjualan"])) {
                        ...
                        if (sizeof($addRowsConfigs[$eName][$currentValue]) > 0) {
                            ...
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                ...
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    $addRows[$oValueName] = "<input " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " type=text autocomplete='off' id='$oValueName' class='form-control text-right' style='font-size:17px;' $disabled placeholder='$oValueName' value='" . number_format($defVal) . "' $minValStr $maxValStr
                                ...
                                if (sizeof($exFilter) > 1) {
                                    if (substr($exFilter[1], 0, 1) == ".") {
                                        ...
                                        if (isset($_SESSION[$cCode]['main'][$exFilter[1]])) {
                                            ...
                                            $addLink .= "?reqField=" . $exFilter[0] . "&reqVal=" . $_SESSION[$cCode]['main'][$exFilter[1]] . "&pihakId=" . $_SESSION[$cCode]['main'][$exFilter[1]] . "&pId=yes";
                                        }
                                        else {
                                    ...
                                    foreach ($tmpo as $row) {
                                        ...
                                        if (sizeof($ex) > 1) {
                                            ...
                                            $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                                                ...
                                                onclick=\"hiliteDiv(this);top.$('#result').load($selectorTarget);\">" . $labelValue . "</label>\n";
                                        }
                                        else {
                                            ...
                                            $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                                                <input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected 
                                                onclick=\"hiliteDiv(this);top.$('#result').load($selectorTarget);\">" . $row->$labelSrc . "</label>\n";
                                        }
...
//                                                $newValue_r = $newSaldo > 0 ? "<span class='text-bold text-green'>" . number_format($newSaldo) . "</span>" : "<span class='text-bold text-red'>SALDO KOSONG</span>";
                    ...
                    top.BootstrapDialog.show(
                        ...
                        //------------
                        $reloadStr = "";
                        if (isset($eSpec['reloadLink'])) {
                            $reloadLink = MODUL_PATH . $eSpec['reloadLink'] . $this->jenisTr . "/$amdlName?id=" . $_SESSION[$cCode]['main_elements'][$eName]['key'];
                            $reloadStr = "<a " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " href='javascript:void(0)' class='btn btn-tool' 
                            onclick=\"btn_result('$reloadLink');\"><span class='glyphicon glyphicon-refresh'></span></a>";
                        }
                        //------------
                        $elements[$eName] = array(
                            ...
                            "editStr" => $editStr,
                            "addStr" => $addStr,
                            "reloadStr" => isset($reloadStr) ? $reloadStr : null,
...
//                        $initValue = isset($eSpec['defaultValue']) ? makeValue($eSpec['defaultValue'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], $eSpec['defaultValue']) : "";
//                        cekHere("masuk disini [$eName] " . __LINE__);
                        $defaultValue = isset($_SESSION[$cCode]['main'][$eName]) ? $_SESSION[$cCode]['main'][$eName] : 0;
//                        $defaultValue = isset($_SESSION[$cCode]['main'][$eName]) ? makeValue($_SESSION[$cCode]['main'][$eName], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'][$eName]) : 0;
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        $main_session = $_SESSION[$cCode]['main'];
...
//            cekHijau($pernyataanNote);
        ...
        //--------------------------
        $shoppingCartHideGudangStatus = isset($this->configUi[$this->jenisTr]['shoppingCartHideGudangStatus']) ? $this->configUi[$this->jenisTr]['shoppingCartHideGudangStatus'] : array();
        $shoppingCartHideGudangStatusKey = isset($this->configUi[$this->jenisTr]['shoppingCartHideGudangStatusKey']) ? $this->configUi[$this->jenisTr]['shoppingCartHideGudangStatusKey'] : "none";
        if (sizeof($shoppingCartHideGudangStatus) > 0) {
            if (isset($_SESSION[$cCode]["main"][$shoppingCartHideGudangStatusKey])) {
                $hiddenKey = $_SESSION[$cCode]["main"][$shoppingCartHideGudangStatusKey];
                if (isset($shoppingCartHideGudangStatus[$hiddenKey])) {
//                    arrPrintKuning($shoppingCartHideGudangStatus[$hiddenKey]);
                    foreach ($shoppingCartHideGudangStatus[$hiddenKey] as $hidden_val) {
                        if (array_key_exists($hidden_val, $itemLabels)) {
                            unset($itemLabels[$hidden_val]);
                        }
                    }
                }
            }
        ...
        $allowCredit = true;
        $customerName = $main_session["customerName"];
        $grandTotal = $main_session["grandTotal"];
        $pihakKreditSaldo = $main_session["pihakKreditSaldo"];
        $pihakKreditActive = isset($main_session["pihakKreditActive"]) ? $main_session["pihakKreditActive"] * 1 : 0;
        $pihakKreditActive_f = number_format($pihakKreditActive);
        $pihakKreditLimit = isset($main_session["pihakKreditLimit"]) ? $main_session["pihakKreditLimit"] * 1 : 0;
        $pihakKreditAllowed = $pihakKreditLimit - $pihakKreditActive;
        // $pihakKreditAllowed = 7500000;
        $pihakKreditAllowed_f = number_format($pihakKreditAllowed);
        // cekBiru("$pihakKreditLimit - $pihakKreditActive = $pihakKreditAllowed_f");
        //        cekAlert("$pihakKreditAllowed_f");
        $customElementPayment = "";
        if ($pihakKreditLimit == 0) {
            $allowCredit = false;
        }
        elseif ($main_session["pihakKreditAllowed"] == 0) {
            $allowCredit = false;
        }
        elseif (isset($main_session["grandTotal"]) && $main_session["grandTotal"] > $pihakKreditAllowed) {
            if (isset($_SESSION[$cCode]['main_elements']['paymentMethod']['key']) && $_SESSION[$cCode]['main_elements']['paymentMethod']['key'] != "cash") {
                $msgWarning = "Limit kredit <b>$customerName</b><br>";
                $msgWarning .= "saat ini tersisa <b><r>Rp $pihakKreditAllowed_f</r></b>.<br><br>";
                $msgWarning .= "Pesanan saat ini sebesar <span class='text-orange'><b>Rp " . number_format($main_session["grandTotal"]) . "</b></span> tidak bisa diproses karena melebihi limit.<br><br>";
                $msgWarning .= "<r>*SOLUSI CEPAT:</r><br>";

                //jika kredit limit < grandTotal
                if ($pihakKreditLimit < $main_session["grandTotal"]) {
                    $msgWarning .= "Kurangi nilai pesanan maksimal sebesar Kredit Limit <r>Rp " . number_format($pihakKreditLimit) . "</r><br>";
                }
                elseif ($pihakKreditLimit > $main_session["grandTotal"] && $pihakKreditAllowed < $main_session["grandTotal"]) {
                    $msgWarning .= "👉 Silakan lakukan pelunasan minimal <span class='text-green'><b>Rp " . number_format($main_session["grandTotal"] - $pihakKreditAllowed) . "</b></span> dari tunggakan sebelumnya, untuk menambah limit kredit, sehingga limit Anda cukup untuk pesanan baru saat ini.<br>";
                }
                else {

                }

                // cekAlert("$msgWarning", "⚠️ PERHATIAN", "warning", "600px");
                //                $allowCredit = false;
                $customElementPayment .= "\n
//                    selectDefaultCashElement();
                    console.log('::init selectDefaultCashElement::');
                ";
            }
        ...
        /** -----------------------------------------------
         * membuat variabel js disertai consolelog
         * */
        // $customElementPayment .= "\n
        //     console.log('::update pihakNama ');
        //     pihakNama = '" . $_SESSION[$cCode]['main']['pihakName'] . "';
        //     console.log('::menjadi: '+pihakNama);
        //     console.log('::update pihakKreditActive '+top.pihakKreditActive+' menjadi: ' + addCommas(" . json_encode($pihakKreditActive) . "));
        //     pihakKreditActive = $pihakKreditActive;
        //     console.log('::update pihakKreditLimit '+top.pihakKreditLimit+' menjadi: ' + addCommas(" . json_encode($pihakKreditLimit) . "));
        //     pihakKreditLimit = $pihakKreditLimit;
        //     console.log('::update pihakKreditAllowed '+top.pihakKreditAllowed+' menjadi: " . number_format(($pihakKreditLimit > 0 ? $pihakKreditLimit - $pihakKreditActive : 0), 0) . "');
        //     pihakKreditAllowed = pihakKreditLimit*1 > 0 ? (pihakKreditLimit*1) - (pihakKreditActive*1) : 0;
        // ";
        ...
        ";
        //--------------------------
        if (sizeof($shoppingCartStokKurang) > 0) {
            if (isset($shoppingCartStokKurang["enabled"]) && ($shoppingCartStokKurang["enabled"] == true)) {
                if (isset($arrProdukStokKurang) && (sizeof($arrProdukStokKurang) > 0)) {
// START OF COMPLETE REPEATED LOGIC
                    $detailKurang = "<br><br><b>Rincian Kekurangan Stok:</b><ul style='text-align: left; margin-top: 5px; color: #d9534f;'>";
                    foreach ($arrProdukStokKurang as $pKurang) {
                        $detailKurang .= "<li>[" . $pKurang['id'] . "] " . $pKurang['nama'] . " (Kekurangan: " . abs($pKurang['jumlah']) . ")</li>";
                    }
                    $detailKurang .= "</ul>";

                    $fixedNote = $fixedNoteTop = $labelKurangStok = $shoppingCartStokKurang["label"] . $detailKurang;
// END OF COMPLETE REPEATED LOGIC
                }
            }
        }
//arrPrint($arrProdukStokKurang);
//arrPrint($disabledDiskonItems);
        $data = array(
            "main_session" => $main_session,
            "main" => $main,
            //-----------------------------------
            "defaultTermOfPayment" => $defaultTermOfPayment,
            "allowCredit" => $allowCredit,
            "pihakKreditLimit" => $pihakKreditLimit,
            "pihakKreditActive" => $pihakKreditActive,
            // "customer_info"                         => $customer_info,
            "pihakKreditAllowed" => $pihakKreditAllowed,
            "pihakKreditSaldo" => $pihakKreditSaldo,
            "grandTotal" => $grandTotal,
            "customer_info" => MODUL_PATH . "_shoppingCart/customerInfo/" . $this->jenisTr,
            // "kreditValidation" => true,
            "kreditValidation" => isset($this->configUi[$jenisTr]["kreditValidation"]) ? $this->configUi[$jenisTr]["kreditValidation"] : false,
            //-----------------------------------
            "showScheme" => $arrCons,
            "showItems" => $showItems,
            ...
            "arrHeaderElementSelector" => isset($this->configUi[$this->jenisTr]['shoppingCartHeaderElementSelector']) ? $this->configUi[$this->jenisTr]['shoppingCartHeaderElementSelector'] : "",
            ...
            "shoppingCartSumEditableFields" => $shoppingCartSumEditableFields,
            "shoppingCartSumEditableFieldsRecorder" => MODUL_PATH . get_class($this) . "/recordAddRow/" . $this->jenisTr,
            "link_info_diskon" => MODUL_PATH . "Create/infoDiskon/" . $this->jenisTr,
            "customElementPayment" => $customElementPayment,
            //--------------
            "labelKurangStok" => isset($labelKurangStok) ? $labelKurangStok : "",
            "disabledDiskonItems" => isset($disabledDiskonItems) ? $disabledDiskonItems : array(),
            "cekStokDiskonDisabled" => $cekStokDiskonDisabled,
            "cekStokDiskonDisabledFields" => $cekStokDiskonDisabledFields,
        );
...
//                "dtime" => "date",
        ...
        if ($shopingCartReload) {
            ...
            if (isset($_GET['stop1']) && $_GET['stop1'] == 1) {
                ...
                echo "top.trig_HitungSubtotal();";
                echo "</script>";
            }
            else {
                echo "<script>";
                echo "top.$('div#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=0&stop1=1');";
                echo "</script>";
            }
        }
    ...
    public function fetchElement()
    {
        ...
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("gagal mendapatkan data ppn, silahkan relogin untuk membersihkan sesi atau ubah qty produk");
        $stepNum = 1;

        $key = $_GET['key'];

//mati_disini(":: $jenisTr, $elName, $mdlName, $key ::");
        if (isset($this->configUiJenis["receiptElements"][$elName]["kreditLimitValidate"][$stepNum]["enabled"]) && ($this->configUiJenis["receiptElements"][$elName]["kreditLimitValidate"][$stepNum]["enabled"] == true)) {
...
//        );
    ...
    public function resetElement()
    {
        ...
        $configCoreJenis = $this->configCore[$this->jenisTr];
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("gagal mendapatakan data ppn, silahkan relogin untuk membesihkan sesi, atau coba klik icon refresh pada shopingcart sebelah kanan atas <span class='glyphicon glyphicon-refresh'></span>");
 ...
 * ---------------------------*/ // arrPrintPink($_SESSION[$cCode]);
    ...
    public function fetchItemsElement()
    {
        ...
        $configCoreJenis = $this->configCore[$this->jenisTr];
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("gagal mendapatakan data ppn, silahkan relogin untuk membesihkan sesi, atau coba klik refresh pada shopingcart sebelah kanan atas <span class='glyphicon glyphicon-refresh'></span>");
...
//        arrPrint($_GET);
    ...
    public function buildValues()
    {
        ...
        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("gagal mendapatkan data ppn, silahkan relogin untuk membesihkan sesi");
...
//        );
    ...
    public function customerInfo()
    {
         ...
         * info customer
         * ----------------------------------*/
        $pihakID = $main_session["pihakID"];
        $customer_info_keis = array(
            "customerName" => array(
                "label" => "consumen",
                // "format" => "number_format",
            ),
            "pihakKreditAllowed" => array(
                "label" => "allowed kredit",
                // "format" => "number_format",
            ),
            "defaultTermOfPayment" => array(
                "label" => "TOP/tenor",
                "format" => "number_format",
            ),
            ...
            "pihakKreditLimit" => array(
                "label" => "Limit",
                "format" => "number_format",
            ),
            "pihakKreditActive" => array(
                "label" => "hutang aktive",
                "format" => "number_format",
            ),
            "pihakKreditUmurTertua" => array(
                "label" => "umur hutang tertua",
                // "format" => "formatField_he_format",
            ),
            "customerDetails__kredit_limit" => array(
                "label" => "sisa kredit limit",
                "format" => "number_format",

            ),
            "pihakKreditSaldo" => array(
                "label" => "sisa kredit limit",
                "format" => "number_format",
        ...
        $customer_info .= "<style>
            .alert-danger{
                    background-color: #a71301 !important;
            }
            .alert:hover span, .btn-warning:hover span {
                opacity: 1 !important;
                visibility: visible !important;
            }
        </style>";
        foreach ($customer_info_keis as $customer_info_kei => $params) {
            $label = isset($params['label']) ? $params['label'] : $customer_info_kei;
            $info = $$customer_info_kei = $main_session[$customer_info_kei];
            $format_key = isset($params["format_key"]) ? $params["format_key"] : $customer_info_kei;
            if (isset($params["format"])) {
                $info_f = $params["format"]($info);
            }
            else {
                $info_f = $info;
            }
        ...
        $kreditSaldoMinGrandTotal = $pihakKreditSaldo - $grandTotal;
        // cekKuning("[$kreditSaldoMinGrandTotal = $pihakKreditSaldo - $grandTotal]");
        // matiHere(__LINE__);

        $button_reload = "<button type='button' class='btn btn-warning' onclick=\"document.getElementById('result').src='" . base_url() . "penjualan/_processPihak/select/5822/MdlCustomer?id=$pihakID'\" 
                    style='width: 40px; height: 40px; border-radius: 50%; padding: 0; align-items: center; justify-content: center;'
                    title='Reload Data Baru'>
        ...
        $msgInfo = "";
        if ($pihakKreditLimit > 0) {
            $alert_warna = "alert-danger";
            $msgInfo .= "<div class='overflow-h'>";
            $msgInfo .= "<div class='col-md-2'>TOP/Kredit limit:<br>" . number_format($pihakKreditLimit) . "/" . number_format($defaultTermOfPayment) . " hari</div>";
            $msgInfo .= "<div class='col-md-2'>Kredit terpakai<br> " . number_format($pihakKreditActive) . "</div>";


            if (!$pihakKreditAllowed) {
                // $msgInfo .= "<div class='col-md-2'>Saldo kredit<br> <del><i>" . number_format($pihakKreditSaldo) . "</i></del>/Overdue " . number_format($defaultTermOfPayment) . " hari</div>";
                $msgInfo .= "<div class='col-md-2'>Saldo kredit<br> <del><i>" . number_format($pihakKreditSaldo) . "</i></del>/Overdue " . number_format($pihakKreditUmurTertua) . " hari</div>";
                $note = "Silahkan melakukan pelunasan";
                if ($pihakKreditUmurTertua > $defaultTermOfPayment) {
                    $note .= " atau melakukan transaksi dari POS";
                }

                $msgInfo .= "<div class='col-md-6'>Note $customerName<br> $note</div>";

                echo "<script>
                    $('#tic_lanjut').prop('disabled', true).prop('checked', false).trigger('change');
                </script>";
            }
            else {
                $msgInfo .= "<div class='col-md-2'>Saldo kredit<br> " . number_format($pihakKreditSaldo) . "</div>";
                $msgInfo .= "<div class='col-md-5'>Note $customerName<br> </div>";
                $msgInfo .= "<div class='col-md-1'>$button_reload</div>";

                if ($pihakKreditActive < 100) {
                    ...
                    if ($main_session["paymentMethod"] != "credit") {
                        /// pppp
                    }
                    else {

                        echo "<script>
                        console.log('------------------------------okko $pihakKreditActive');
                        var container = $('.col-md-6.col-lg-6').has('#elTitle_paymentMethod');
                        container.find('input[name=\"paymentMethod\"][value=\"credit\"]').prop('checked', true).trigger('change');
                    </script>";
                    }
                }
            }
            ...
            /** -------------------------------------------------
             * untuk customer CASH ONLY
             * -------------------------------------------------*/
            $alert_warna = "alert-info";
            $msgInfo = "";
            $msgInfo .= "<div class='overflow-h'>";
            $msgInfo .= "<div class='col-md-11'>Cash<br>$customerName</br></div>";
            $msgInfo .= "<div class='col-md-1'>$button_reload</div>";
            $msgInfo .= "</div>";
            echo "<script>
                    $('#tic_lanjut').prop('disabled', false);
                    setTimeout(function() {
                        var container = $('.col-md-6.col-lg-6').has('#elTitle_paymentMethod');
                        container.find('input[name=\"paymentMethod\"]:not([value=\"cash\"])')
                                .prop('checked', false)
                                .prop('disabled', true);                    
                    }, 200);
                </script>";
        ...
        $customer_info .= "<div class='alert $alert_warna' style='margin-top: 5px;margin-bottom: unset; $bg_warna'>";
        $customer_info .= $msgInfo;
        $customer_info .= "</div>";
        ...
        $msgWarning = "";
        $msgWarning .= "Limit kredit <b>$customerName</b><br>";
        $msgWarningSolusi = "<hr><r>SOLUSI CEPAT:</r><br>";

        if (!$pihakKreditAllowed) {

            if ($pihakKreditActive > 100) {
                $msgWarning .= "";
                $msgWarningSolusi .= "👉 Silakan lakukan pelunasan minimal <span class='text-green'><b>Rp " . number_format($main_session["grandTotal"] - $pihakKreditAllowed) . "</b></span> dari tunggakan sebelumnya, untuk menambah limit kredit, sehingga limit Anda cukup untuk pesanan baru saat ini.<br>";
                $msgWarningSolusi .= "👉 Bisa melakukan transaksi dari menu Penjualan POS<br>";
            }
            else {
                $msgWarningSolusi .= "👉 Kurangi nilai pesanan maksimal sebesar Kredit Limi <span class='text-green'><b>Rp " . number_format($main_session["grandTotal"] - $pihakKreditAllowed) . "</b></span> dari tunggakan sebelumnya, untuk menambah limit kredit, sehingga limit Anda cukup untuk pesanan baru saat ini.<br>";
            }

            if ($pihakKreditLimit > 0) {
                echo cekAlert($msgWarning . $msgWarningSolusi);
            }
        }
        else {
            if ($kreditSaldoMinGrandTotal > 0) {
                echo "<script>
                    $('#tic_lanjut').prop('disabled', false);
                </script>";
            }
                ...
                if ($defaultPaymentMethod != "cash") {
                    ...
                    $msgWarningSolusi .= "👉 Kurangi nilai pesanan maksimal sebesar Kredit Limi <span class='text-green'><b>Rp " . number_format($pihakKreditLimit) . "<br>";

                    echo cekAlert($msgWarning . $msgWarningSolusi);
                    echo "<script>
                        $('#tic_lanjut').prop('disabled', true).prop('checked', false);
        ...
        // if ($pihakKreditLimit > 0 || $kreditSaldoMinGrandTotal < 0) {
        ...
        // if($grandTotal > $pihakKreditSaldo){
        //     echo "<script>
        //     $('#tic_lanjut').prop('disabled', true);
        // </script>";
        // }
