...
//         arrPrint($itemLabels);
        ...
        foreach ($items as $pids => $item) {
            ...
            $arrItemsRegistries[$id] = isset($itemsRegistries[$id]) ? $itemsRegistries[$id] : array();
            $items[$id] = array_merge(array_filter($items[$id]), array_filter($arrItemsRegistries[$id]));
            if(isset($receiptKeyTambahan) && (sizeof($receiptKeyTambahan)>0)){
                foreach ($receiptKeyTambahan as $key_tambahan){
                    $items[$id][$key_tambahan] = isset($arrItemsRegistries[$id][$key_tambahan]) ? $arrItemsRegistries[$id][$key_tambahan] : "";
                }
            }
//                        arrPrint($arrItemsRegistries[$id]);
//                        $item = $items[$id];
//                        arrPrintKuning($items[$id]);
            ...
            foreach ($itemLabels as $itemkey => $itemLabel) {
                ...
                $tanda_stok_kurang = "";
                if(isset($receiptKeyIsiTambahan[$itemkey]) && ($receiptKeyIsiTambahan[$itemkey] == true)){
                    $tanda_stok_kurang = ($items[$id]["stok_total"] > 0) ? "" : "-";
                }
                $tblShow .= "<td title='$itemkey' $attr class='$cssClass'><span class='$mainClass'>$nilai_f  &nbsp; $tanda_stok_kurang</span> $add_data_val $alt_nilai</td>";
            }
            $tblShow .= "</tr>";
            //-----------------------
            if (isset($arrSubDetailDataKolom["nama"][$id])) {
