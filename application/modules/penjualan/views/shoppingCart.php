...
/* ------------------------------------------------------------------------
 * pembersih session shopingcart bocoran dari doFollowUp
 * ------------------------------------------------------------------------*/
if (isset($main["transaksi_id"]) && $main["transaksi_id"] > 0) {
    $id_exist = isTransaksiIdExist_misc($main["transaksi_id"]);
    // showLast_query("biru");
    // arrPrintHijau($id_exist);
    // cekOrange("session dr followup " . $main["transaksi_id"]);
    ...
    if ($id_exist > 0) {
        ...
        $alerts = array(
            "type" => "error",
            "title" => "Opss....",
            "html" => "shoping cart tidak valid, tunggu sesaat lagi untuk bisa digunakan <br>atau klik button untuk membersihkan",
            // "html"  => $main["transaksi_id"] . " || $id_exist",
        );
        // echo swalAlert($alerts);
        // echo swalAlertGoTo2($alerts, $resetLink, "Bersihkan shopingcart sekarang");
        //
        // echo "<script>$('#result').load('$resetLink');</script>";
    }
}
// -----------------------------------------------------------------------------end-----
...
// START OF COMPLETE REPEATED LOGIC
    // Top alert moved to sumRows summary section next to Total Bruto
    /*
        ...
        if (sizeof($arrHeaderElement) > 0) {
            foreach ($arrHeaderElement as $el => $eDetails) {
                ...
                foreach ($eDetails['subElements'] as $sels => $seDetails) {
                    ...
                    $selsItemsTarget = isset($seDetails['srcItem']) ? $seDetails['srcItem'] : "";
                    $mainOverwrite = isset($seDetails['overWriteMain']) ? $seDetails['overWriteMain'] : "";
...
//                          </label>";
                    ...
                    echo "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'>
                              <input type='radio' name='switch_ppn' value='$selsValue' $checked 
                              onclick=\"$('#result').load('" . MODUL_PATH . "$arrHeaderElementSelector/$jenisTr?tp='+this.value);\">
                              <span>$selsLabel</span>
                          </label>";
...
//        arrPrint($disabledDiskonItems);
        ...
        foreach ($items as $iSpec) {
            ...
            if ($showItems) {
                //pembeda hadiah dan reguler
                $style_hadiah = isset($iSpec["type_produk"]) && $iSpec["type_produk"] == "hadiah" ? "background: #c5b117;color: #af601a;font-style: italic" : "";
                $style_icon = isset($iSpec["type_produk"]) && $iSpec["type_produk"] == "hadiah" ? "fa fa-gift" : "";
                $iID = $iSpec['id'];
                ...
                echo "<tr id='tr_" . $iSpec['id'] . "' bgcolor=$bgColor style='$style_hadiah'>";
                echo "<td style='vertical-align:middle; width:1%' class='text-center '><span class='$style_icon'></span>";
                echo $no;
                echo "</td>";
                $colCtr = 0;
                $queryParams = "";
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    $bglime = "bg-lime";
                    $disabled_diskon = "";
                    if ($cekStokDiskonDisabled == true) {
                        // cekStokDiskonDisabledFields
                        if ((in_array($key, $cekStokDiskonDisabledFields)) && isset($disabledDiskonItems[$iSpec["id"]])) {
                            $disabled_diskon = isset($disabledDiskonItems[$iSpec["id"]]) ? "disabled" : "";
                            $bglime = "";
                        }
                    }
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            echo "<input $disabled_diskon type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  
                            class='form-control text-right text-red text-bold $bglime' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
...
//                            $pemicuGerbangAsli = "onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';} \" $keyupData";
                            $pemicuGerbangAsli = "onblur=\"hiliteDiv(this);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams'; \" $keyupData";
                            $pemicuGerbangAsli .= "oonmouseout=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';}\" ";
                            $pemicuGerbangAsli .= "onkeyup=\"this.value = addCommas(removeCommas(this.value))\"";
...
//                            $pemicuGerbang = "onblur=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';}\" $keyupData ";
                            $pemicuGerbang = "onblur=\"hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';\" $keyupData ";
                            $pemicuGerbang .= "oonmouseout=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';}\" ";
...
// START OF COMPLETE REPEATED LOGIC
                        case "text":
                            ...
                            if (is_numeric($fieldVal) && $keyID != 'barcode' && $keyID != 'produk_kode') {
                                $baseStokAttr = "";
                                if ($keyID == 'stok_ready' || $keyID == 'stok' || $keyID == 'stok_center') {
                                    $stokFisikBase = isset($iSpec['stok_ready_input']) && isset($iSpec['jml']) ? ($iSpec['stok_ready_input'] + $iSpec['jml']) : $fieldVal;
                                    $baseStokAttr = " base_stok='$stokFisikBase' is_stok_col='1' ";
                                }
                                echo "<span isNumeric $baseStokAttr keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:#$color;background:#f0f0f0;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                                echo "<script>
    var nilai_text = $('#$colID').text();
    // console.log(anu);
    if(nilai_text < 0){
        $('#$colID').css('background-color', '#fd31009c');
    }
    </script>";
                            }
                            else {
//                            if (strlen($fieldVal) > 10) {
                                echo "<span isNotNumeric fieldVal lebih besar dari 10 keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
//                            }
//                            else {
//                                echo "<span isNotNumeric keyid=$keyID noid=$noID id=$colID class='form-control' style='color:$color;border:0px;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
//                            }
                            }
                            break;
// END OF COMPLETE REPEATED LOGIC
...
// START OF COMPLETE REPEATED LOGIC
        ...
        if (isset($sumRows) && sizeof($sumRows) > 0) {
            $nr = 0;
            $notifMsg = isset($fixedNoteTop) ? $fixedNoteTop : (isset($fixedNote) ? $fixedNote : null);
            $hasNotif = (isset($notifMsg) && strlen($notifMsg) > 0) ? true : false;
            $totalSumRows = sizeof($sumRows);
            $colspanLeftNotif = sizeof($itemLabels) > 1 ? (sizeof($itemLabels) - 1) : 1;
...
//                cekHere(":: $key ::");
                $msg = "";
                if ($key == "ongkir_ui") {
                    $msg .= "<br><span class='meta text-lowercase'>";
                    $msg .= "ini hanya tampilan. untuk mengisi nilai shipping service, silahkan mengisi di baris warna putih.";
                    $msg .= "</span>";
                }
                if ($showItems) {
                    ...
                    if ($hasNotif) {
                        if ($nr == 1) {
                            echo "<td colspan='" . $colspanLeftNotif . "' rowspan='" . $totalSumRows . "' class='valign-t' style='vertical-align: top; padding: 10px;'>";
                            echo "<div class='alert alert-danger' style='margin: 0px; font-size: 16px; font-weight: bold;'>";
                            echo "<span>$notifMsg</span>";
                            echo "</div>";
                            echo "</td>";
                        }
                        echo "<td class='text-right $bottom_borderless valign-m text-uppercase'>$label $msg</td>";
                    }
                    else {
                        echo "<td colspan='" . sizeof($itemLabels) . "' class='text-right $bottom_borderless valign-m text-uppercase'>$label $msg</td>";
                    }
                    echo "<td colspan='3' class='right-borderlesss'>";
                    if (in_array($key, $shoppingCartSumEditableFields)) {
                        $link = isset($shoppingCartSumEditableFieldsRecorder) ? $shoppingCartSumEditableFieldsRecorder : "";
                        echo "<input type='number' id='$key' class='form-control text-right' value='$val' 
                            onclick='select();'
                            onblur=\"$('#result').load('" . $link . "/$key?val='+this.value);\"
                        >";
                    }
                    else {
...
//                cekHere(":: $k => $label ::");
        ...
        echo "</div>"; // responsive
        echo "<div id='customer_info'></div>";

// START OF COMPLETE REPEATED LOGIC
        if (isset($fixedNote) && (!isset($hasNotif) || !$hasNotif)) {
...
//            echo "</div>";
    ...
    /** ----------------------------------------------------------------
     * link link_info_diskon dikirim dr co shopingcart
     * ----------------------------------------------------------------*/
    // $link_info_diskon = MODUL_PATH . "Create/infoDiskon/5822";
    // echo $link_info_diskon;
    // cekHere("link_info_diskon:: $link_info_diskon");

    if (isset($_GET['stop1'])) {
        echo "<script>$('#container_information').load('$link_info_diskon');</script>";
    }

    $kategori_nama = "";
    // arrPrintHijau($main);
    if (isset($main_session['kategoriNama'])) {
        if ($main_session['kategoriNama'] == "distributor") {
            // $titles = "harga tidak bisa diganti";
            /** ------------------------------------
             * fitur dimatikan, kalu menghidupkan...
             * berkaitan dengan proses_select pihak dan lib diskon
             * ------------------------------------*/
            $titles = "bisa memilih harga";
        }
        else {
            $titles = "bisa memilih harga";
        }
        ...
        $kategori_nama .= "<r>(";
        $kategori_nama .= (isset($main_session['kategoriNama']) && strlen($main_session['kategoriNama']) > 3) ? strtoupper($main_session['kategoriNama']) : "REGULER";
        $kategori_nama .= ")</r> ";
    }
    echo "<script>$('#kategori_pihak').html('$kategori_nama ');</script>";
    echo "<script>$('#keterangan_pihak').html('$titles ');</script>";
    ...
    // cekAlert("$allowCredit || $pihakKreditLimit ||| $pihakKreditActive");

    echo "<script>
    // console.log('---- limit');
    var defaultTermOfPayment = '$defaultTermOfPayment';
    var allowCredit = '$allowCredit';
    
    // console.log('defaultTermOfPayment: ', defaultTermOfPayment);
    // console.log('allowCredit: ', allowCredit);
    console.log('grandTotal-----------------: ', '$grandTotal');
    console.log('pihakKreditSaldo-----------------: ', '$pihakKreditSaldo');
    ...
    if ($kreditValidation) {
        ...
        if ($allowCredit == false) {
            echo "<script>


            // $('#customer_info').load('$customer_info');

            console.log('limit: $pihakKreditLimit');
            if (!window.paymentInit) {
                var container = $('.col-md-6.col-lg-6').has('#elTitle_paymentMethod');
                // container.find('input[name=\"paymentMethod\"][value=\"cash\"]').trigger('click');
                // Setelah reload selesai → disable yang lain
                // setTimeout(function() {
                //     var container = $('.col-md-6.col-lg-6').has('#elTitle_paymentMethod');
                //     container.find('input[name=\"paymentMethod\"]:not([value=\"cash\"])')
                //         .prop('checked', false)
                //         .prop('disabled', true);
                // }, 2000); // kasih delay sedikit supaya kontainer selesai reload
                window.paymentInit = true; // flag global
            }
            
             setTimeout(function() {
                    var container = $('.col-md-6.col-lg-6').has('#elTitle_paymentMethod');
                    // container.find('input[name=\"paymentMethod\"]:not([value=\"cash\"])')
                    container.find('input[name=\"paymentMethod\"]')
                        .prop('checked', false)
                        .prop('disabled', true);
        ...
        if ($customElementPayment) {
            echo "<script>$customElementPayment</script>";
        }
    }

// START OF COMPLETE REPEATED LOGIC
//    echo "||| " . count($disabledDiskonItems) . " |||";
    if (isset($disabledDiskonItems) && (sizeof($disabledDiskonItems) > 0)) {
        $jsPids = json_encode(array_values($disabledDiskonItems));
        echo "<script>
        (function() {
            var pids = $jsPids;
            
            function lockZeroStockRadios() {
                $.each(pids, function(index, pid) {
                    var \$radios = $('input[name=\"harga_pilihan_' + pid + '\"]');
                    \$radios.prop('disabled', true);
                    console.log('tombol mati...');
                    \$radios.closest('label').addClass('text-muted').css('cursor', 'not-allowed');
                });
            }

            lockZeroStockRadios();

            // Matikan dan uncheck tic_lanjut serta disable tombol continue/proses
            $('#tic_lanjut').prop('checked', false).prop('disabled', true);
            $('#btnProcess').prop('disabled', true);
            if (typeof top !== 'undefined' && top.$) {
                top.$('#tic_lanjut').prop('checked', false).prop('disabled', true);
                top.$('#btnProcess').prop('disabled', true);
            }

            $(document).off('ajaxComplete.lockDiskon').on('ajaxComplete.lockDiskon', function(event, xhr, settings) {
                if (settings.url && settings.url.indexOf('infoDiskon') !== -1) {
                    lockZeroStockRadios();
                }
            });
        })();
        ...
        echo "<script>
            // Aktifkan kembali tic_lanjut jika seluruh stok mencukupi
            $('#tic_lanjut').prop('disabled', false);
            if (typeof top !== 'undefined' && top.$) {
                top.$('#tic_lanjut').prop('disabled', false);
            }
        </script>";
    }
    ...
    echo "<script>
    (function() {
        $(document).off('keyup.stokLiveCalc').on('keyup.stokLiveCalc', 'input[keyid=jml]', function() {
            var noid = $(this).attr('noid');
            var currentJml = parseFloat(removeCommas($(this).val())) || 0;
            var hasMinus = false;
            var \$spans = $('span[is_stok_col=1][noid=' + noid + ']');
            if (\$spans.length) {
                \$spans.each(function() {
                    var \$span = $(this);
                    var baseStok = parseFloat(\$span.attr('base_stok'));
                    if (!isNaN(baseStok)) {
                        var sisa = baseStok - currentJml;
                        \$span.text(addCommas(sisa));
                        if (sisa < 0) {
                            \$span.css('background-color', '#fd31009c');
                            hasMinus = true;
                        } else {
                            \$span.css('background-color', '#f0f0f0');
                        }
                    }
                });
            }
