...
$('#uplodXls').on('submit',function() {
    ...
    var arrProduk = JSON.parse(localStorage.getItem('items'));
        ...
        // region detail dikirim dari...
        $data_cabang_pos_ = "";
        if (count($data_cabang_pos) > 0) {
            $data_cabang_pos_ .= "<div class='alert-info'>";
            $data_cabang_pos_ .= "<h4 style='text-transform: capitalize;'>$pihakMainLabel</h4>";
            foreach ($data_cabang_pos as $data_cabang_pos_0) {
                $checked = $pihakMainName == $data_cabang_pos_0["label"] ? "checked" : "";
                $data_cabang_pos_ .= "<input $checked name='cabang'type='radio' 
                onclick=\"document.getElementById('result').src='" . $data_cabang_pos_0["target"] . "?id=" . $data_cabang_pos_0["id"] . "'\"><label>" . $data_cabang_pos_0["label"] . "</label>";
                $data_cabang_pos_ .= "&nbsp;&nbsp;";
            }
            $data_cabang_pos_ .= "</div>";
        }
        // endregion detail dikirim dari...


        // region tipe penjualan
        $data_tipe_penjualan_ = "";
        if (count($data_tipe_penjualan) > 0) {
            $data_tipe_penjualan_ .= "<div id='data_tipe_penjualan' class='alert-info'>";
            $data_tipe_penjualan_ .= "<span class='text-bold' style='text-transform: capitalize;'>$pihakTipeLabel: </span><br>";
            foreach ($data_tipe_penjualan as $data_tipe_penjualan_0) {
                $checked = ($pihakTipeID == $data_tipe_penjualan_0["id"]) ? "checked" : "";
                $data_tipe_penjualan_ .= "<input $checked name='tipe_penjualan' type='radio' 
                onclick=\"document.getElementById('result').src='" . $data_tipe_penjualan_0["target"] . "?id=" . $data_tipe_penjualan_0["id"] . "'\"><label>" . $data_tipe_penjualan_0["label"] . "</label>";
                $data_tipe_penjualan_ .= "&nbsp;&nbsp;";
            }
            $data_tipe_penjualan_ .= "</div>";
        }
        // endregion tipe penjualan


        // region detail marketplace
        $data_marketplace_ = "";
//        if ((count($data_marketplace) > 0) && ($pihakTipeID > 0)) {
        if (count($data_marketplace) > 0) {
            $display = ($pihakTipeID > 0) ? "block" : "none";
            $data_marketplace_ .= "<div id='data_marketplace'  class='alert-warning' style='display:$display;'>";
            $data_marketplace_ .= "<span class='text-bold' style='text-transform: capitalize;'>$pihakMarketplaceLabel: </span><br>";
            foreach ($data_marketplace as $data_marketplace_0) {
                $checked = ($pihakMarketplaceID == $data_marketplace_0["id"]) ? "checked" : "";
                $data_marketplace_ .= "<input $checked name='marketplace' type='radio' 
                onclick=\"document.getElementById('result').src='" . $data_marketplace_0["target"] . "?id=" . $data_marketplace_0["id"] . "'\"><label>" . $data_marketplace_0["label"] . "</label>";
                $data_marketplace_ .= "&nbsp;&nbsp;";
            }
            $data_marketplace_ .= "</div>";
        }
        // endregion detail marketplace

        if (isset($description_label) && ($description_label != NULL)) {
            $description_label_str = "<span style='font-size:15px;font-weight:bold;'>";
            $description_label_str .= $description_label;
            $description_label_str .= "</span>";
        }
        ...
        $p->addTags(
            array(
                ...
                "selector_caller_delete2" => $pihakMainCallerDelete2,
                ...
                "selector_name2" => isset($pihakMain2Name) ? $pihakMain2Name : "",
                ...
                "pihak_main_name" => isset($pihakMainName) ? $pihakMainName : "",
                ...
                "data_cabang_pos" => $data_cabang_pos_,
                "ppnFactorInclude" => isset($ppnFactorInclude) ? $ppnFactorInclude : "",
                //----
                "data_tipe_penjualan" => isset($data_tipe_penjualan_) ? $data_tipe_penjualan_ : "",
                "data_marketplace" => isset($data_marketplace_) ? $data_marketplace_ : "",

                "description_label" => isset($description_label_str) ? $description_label_str : "",
                // ----------------------------------------------------------------
                "pihakKreditActive" => isset($pihakKreditActive) ? $pihakKreditActive : 0,
                "pihakKreditLimit" => isset($pihakKreditLimit) ? $pihakKreditLimit : 0,
                "session_main" => isset($session_main) ? json_encode($session_main) : array(),
                "customer_info" => $customer_info,
                "kreditValidation" => $kreditValidation,
                //----
                "time_line" => $time_line,
            )
        );
    ...
    case "viewUndoneItems":
        ...
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            ...
            if (sizeof($arrayOnProgress) > 0) {
                ...
                // arrPrint($arrBlock);
                // arrPrint($arrayProgressLabels);
                // arrPrint($arrayProgressLabels_2);
                // arrPrint($arrayOnProgress);
                $str .= "<div class='table-responsive step'>";
                $str .= "<table id='arrayOnProgress_step' class='table table-condensed table-hover-color-red table-striped table-bordered'>";
                $str .= "<thead>";
                ...
                $str .= "<tbody>";
                $noi = 0;
                // unset($arrayProgressLabels['state']);
                ...
                $str .= "<script>
                    $(document).ready( function(){
                        var table = $('#arrayOnProgress_step').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: true,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
                            ...
                            footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var intVal = function ( i ) {
                                            return typeof i === 'string' ?
                                                ...
                                                var arrayFooter = $('#arrayOnProgress_step>tfoot>tr>th');
                                        var dpageTotal = [];
                                ...
                                $('.table-responsive.step').floatingScroll();
                                $('.table-responsive.step').scroll(function () {
                                    setTimeout(function () {
                                        $('#arrayOnProgress_step').DataTable().fixedHeader.adjust();
                                    }, 400);
                            });
                    </script>";
            }
    ...
    case "viewUndoneItemsIndex":
        ...
        $stepper = isset($_GET['step']) ? ($isMobile == true ? 2 : $_GET['step']) : 1;
        $presteper = isset($_GET['prestep']) ? ($isMobile == true ? 2 : $_GET['prestep']) : 2;
        if (isset($_GET['step'])) {
            //            arrPrint($steps[$_GET['step']]);
        }
...
// arrPrint($preStep);
            if (sizeof($preStep) >= 1) {
                $jenispreStep = url_segment(4);
                // arrPrint($arrayOnprePreGroup);
                // cekLime(count($arrayOnprePreGroup));
                $strOnprog .= "<ul class='nav nav-tabs'>";
                foreach ($preStep as $tpStep => $stepData) {
                    $isiBadge = isset($arrayOnprePreGroup[$tpStep]) ? "<span class='badge bg-red'>" . sizeof($arrayOnprePreGroup[$tpStep]) . "</span>" : "";
                    $actives = $tpStep == $presteper ? "active" : "";
                    $trSelesai = count($preStep) == $tpStep ? "SELESAI<br>" : "";
                    $visibility = count($preStep) == $tpStep ? "visibility:hidden;" : "";
                    ...
                    $cssSelesai = count($preStep) == $preStep ? "style='padding-top: 0;padding-bottom: 0;'" : "";
                    $strOnprog .= "<li id='prenav_$tpStep' class='none $actives'>";
                    $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?prestep=$tpStep";

                    $varUrl = $tpStep == $presteper ? "javascript:void(0);" : "if($('#undoneList>div>div.box-body')){ $('#undoneList').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#undoneList').load('$undoneLinkIndex'); }else{ console.log('tidak ada container undoneList'); }";

                    $strOnprog .= "<a $cssSelesai id='prea_$tpStep' class='nav-link btn' onclick=\"$varUrl;localStorage.setItem('steppreJenis', '$jenispreStep');localStorage.setItem('steppre', '$tpStep');\">";
                ...
                // cekBiru(current_url());
                $baseUrl = current_url();
                $getUrlstep = isset($_GET['step']) ? $_GET['step'] : 1;
                $getUrlprestep = isset($_GET['prestep']) ? $_GET['prestep'] : 2;
                // $getUrl = $_SERVER['QUERY_STRING'];
                $strOnprog .= "<script>               
                var baseurl = '$baseUrl';
                var geturlstep = '$getUrlstep';
                var geturlprestep = '$getUrlprestep';

                var jenisSteppre = '$jenispreStep';
                var stepNow = localStorage.getItem('step');
                var stepNowpre = localStorage.getItem('steppre');
                var stepJenisNowpre = localStorage.getItem('steppreJenis');
                // console.log('stepNowpre:',stepNowpre);
                
               var prestepGet = geturlprestep;
               var stepGet = geturlstep;
               
               var strStep = '';
               if(!stepGet){
                   strStep = 'step=' + stepNow; 
               }
               else {
                   if(!stepNow){                       
                        strStep = 'step=' + stepGet;
                   }
                   else {
                        strStep = 'step=' + stepNow;    
                   }
               ...
               var strPreStep = '';
               if(!prestepGet){
                   strPreStep = 'prestep=' + stepNowpre;
               }
               else {
                   if(!stepNowpre){
                   strPreStep = 'prestep=' + prestepGet;
               }
                   else {
                        strPreStep = 'prestep=' + stepNowpre;   
                   }
                ...
                var urlNew = baseurl + '?' + strStep + '&' + strPreStep; 
                // console.log('urlNew:',urlNew);                
               
               if (!prestepGet || !stepGet) {
                   // console.log(\" ------ Salah satu atau kedua parameter tidak ditemukan. Halaman akan di-reload ke URL:\", urlNew);
                   // console.log(\"prestep:\", prestepGet);
                   //  console.log(\"step:\", stepGet);
                    open_holdon();
                   $('#undoneList').load(urlNew);
               } 
               else {
                   close_holdon();
                    // console.log(\">>> Kedua parameter ditemukan, tidak ada aksi:\");
                    // console.log(\"prestep:\", prestepGet);
                    // console.log(\"step:\", stepGet);
                }
               
              // if(document.getElementById('prenav_' + stepNowpre) && stepJenisNowpre == jenisSteppre){
                   // $('#prenav_'+ stepNowpre).addClass('active').trigger('click');                    
                   // $('#prea_'+ stepNowpre).trigger('click');                                       
               // }
            ...
            $strOnprog .= "<table id='onprogress_prestep' class='table datatables stripe compact nowarp order-column table-condensed table-bordered no-padding' style='border:solid red 0px;'>";
            $strOnprog .= "<thead>";
            $strOnprog .= "<tr class='text-uppercase' line=" . __LINE__ . ">";
            if (sizeof($arrayProgressPreLabels) > 0) {
                $strOnprog .= "<th class=''>No.</th>";
                foreach ($arrayProgressPreLabels as $key => $label) {
                    $strOnprog .= "<th class=''>";
                    if (is_array($label)) {
                        $strOnprog .= isset($label['label']) ? $label['label'] : "-";
                    }
            ...
            $strOnprog .= "<tbody>";
            $no = 0;
            foreach ($arrayOnprePreGroup[$presteper] as $key => $val) {
                //----------------------
                $background_color = isset($arrayOnprogressMarking[$key]['style']) ? $arrayOnprogressMarking[$key]['style'] : "";
            ...
            $strOnprog .= "<script>
                    $(document).ready( function(){
                        var table = $('#onprogress_prestep').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: true,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
                            ...
                            footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var intVal = function ( i ) {
                                            return typeof i === 'string' ?
                                                ...
                                                var arrayFooter = $('#onprogress_prestep>tfoot>tr>th');
                                        var dpageTotal = [];
                                        jQuery.each(arrayFooter, function(i,d){
        ...
        if (sizeof($arrayOnProgress) > 0) {
            ...
            if ($isMobile == true & $stepper == 2) {
                ...
                foreach ($arrayOnProgress as $key => $val) {
                    ...
                    $strOnprogMbl .= $val['dtime'];
                    $strOnprogMbl .= "</div>";

                    $strOnprogMbl .= "<div class='box-body'>";
                    $strOnprogMbl .= "<p>" . $val['nomer_soa'] . "</p>";
                    $strOnprogMbl .= "<p>Konsumen: " . $val['customers_nama'] . "</p>";
        ...
        if ($isMobile == true & $stepper == 2) {
            ...
            $str = "<div class='text-uppercase'><h3>pre packing list <span class='pull-right'>$link</span></h3></div>";
            $str .= $strOnprogMbl;
        }
         ...
         * -----------------------------*/
        if ($isMobile == true) {
            $str .= "<div id='job_kurir' style='margin-top: 20px;'></div>";
            $str .= "<script>$('#job_kurir').load('$link_undoneList_kurir');</script>";
        }
    ...
    case "preview":
        ...
        if (isset($items) && sizeof($items) > 0) {
            ...
            echo "</table>";
            //region untuk handling hadiah /free produk penjualan
            if (count($itemHadiah) > 0) {
                echo "<table class='table table-bordered table-condensed' style='background:#ffffff;'>";
                echo "<tr bgcolor='#f0f0f0'>";
                echo "<td colspan='" . (sizeof($itemHadiahFields) + 1) . "' bgcolor=#f0f0f0>";
                echo "<span class='fa fa-gift' style='font-size: large;'> Hadiah</span> ";
                echo "</td>";
                echo "</tr>";
                echo "<tr>";
                echo "<td>No</td>";
                ...
                foreach ($itemHadiah as $itemHadiah_0) {
                    ...
                    echo "<td>" . $i_hadiah . "</td>";
                    foreach ($itemHadiahFields as $h_key => $h_label) {
                        echo "<td>" . formatField($h_key, $itemHadiah_0[$h_key]) . "</td>";
                    }
                    echo "<tr>";
                }
            ...
            if (strlen($description) > 0) {
                ...
                echo "<span class='' style='font-style:italic;font-family:Monaco, Menlo, Consolas, \"Courier New\", monospace;'>tidak ada catatan</span><br>";
            }
            echo "</td>";
            echo "</tr>";
            echo "</table>";
    ...
    case "followupPreview":
        ...
        $disabledKurangStok = "";
        if (isset($labelKurangStok) && ($labelKurangStok != NULL)) {
            $disabledKurangStok = "disabled";
            echo "<div class='alert alert-danger text-center'>";
            echo "<span style='font-size:20px;'>$labelKurangStok</span>";
            echo "</div class='alert alert-warning'>";
//
//            $arrSwals = array(
//                "type" => "warning",
...
//                    arrPrintWebs($iSpec);
                    $colorStyle = isset($iSpec["type_produk"]) && $iSpec["type_produk"] == "hadiah" ? "background: #c5b117;color: #af601a;font-style: italic" : "";
                    $span_icon = isset($iSpec["type_produk"]) && $iSpec["type_produk"] == "hadiah" ? "<span class='fa fa-gift'>" : "";
                    ...
                    $no++;
                    echo "<tr line=" . __LINE__ . " style='$colorStyle'>";
                ...
                if (sizeof($mainElements) > 0) {
                    ...
                    foreach ($mainElements as $elName => $aSpec) {
                        ...
                        if (array_key_exists($elName, $elementConfig)) {
                            ...
                            $arrHiddenVal = array();
                            if ($showw == 1) {
                                if (isset($elementConfig[$elName]["optionPoints"])) {
                                    //                                    $nextStep = 5;
                                    $nextStep = $mainValues["step_number"];
                                    if ($elementConfig[$elName]["optionPoints"]["step"] == $nextStep) {
                                        $key_option = "$elName" . "_optionPoints";
                                        $default_value_option = isset($mainValues[$key_option]) ? $mainValues[$key_option] : 0;
                                        $arrHiddenVal[$elName] = isset($elementConfig[$elName]["optionPoints"]["hidden"][$default_value_option]) ? $elementConfig[$elName]["optionPoints"]["hidden"][$default_value_option] : array();

                                        echo "<br>OPTION: &nbsp;&nbsp;";
                                        foreach ($elementConfig[$elName]["optionPoints"]["label"] as $label_key => $label_val) {
                                            $labelTitle = $elementConfig[$elName]["optionPoints"]["labelTitle"][$label_key];
                                            $checked = $default_value_option == $label_val ? "checked" : "";
                                            echo "<input type='radio' name='switch_delivery_detail' value='$label_val' required $checked title='$labelTitle'
                                              onclick=\"$('#result').load('" . $elementOptionTarget . "?val='+this.value+'&key=$key_option');\">
                                              <span>$label_key</span>&nbsp;&nbsp;";
                                        }
                                        foreach ($elementConfig[$elName]["optionPoints"]["labelTitle"] as $labelTitle_key => $labelTitle_val) {
                                            echo "<br><span class='meta'>$labelTitle_key: $labelTitle_val</span>";
                                        }
                                    }
                                }
                            }
                                ...
                                case "dataModel":
                                    ...
                                    if (sizeof($elContents) > 0) {
                                        ...
                                        foreach ($elContents as $label => $val) {
                                            if ($val != "") {
                                                $strLabel = isset($elementConfig[$elName]['usedFields'][$label]) ? $elementConfig[$elName]['usedFields'][$label] : "";
                                                if ((!in_array($label, $arrHiddenVal[$elName]))) {
                                                    $arrContents[$strLabel] = $val;
                                                    //                                                    echo "<tr line=" . __LINE__ . ">";
                                                    //                                                    if (strlen($strLabel) > 0) {
                                                    //                                                        echo "<td align='left' class='text-muted'>" . $strLabel . "</td>";
                                                    //                                                    }
                                        ...
                                        if (sizeof($arrContents) > 0) {
                                            foreach ($arrContents as $contents_key => $contents_val) {
                                                echo "<tr line=" . __LINE__ . ">";
                                                if (strlen($contents_key) > 0) {
                                                    echo "<td align='left' class='text-muted'>" . $contents_key . "</td>";
                                                }
                                                echo "<td align='left' class='text-black'>$contents_val</td>";
                                                echo "</tr>";
                                            }
                                        }
                                        else {
                                            echo "<tr line=" . __LINE__ . ">";
                                            echo "<td align='left' class='text-black'>None</td>";
                                            echo "</tr>";
                                        }
...
//                    <span class='fa fa-pencil'></span> " . $editSpec['label'] . "</button>";
                    ...
                    echo "<button type='button' class='btn btn-default margin' style='border:1px #ff7700 solid;color:#ff7700;' 
                    onclick=\"btn_alert_result('Peringatan', '" . $editSpec['warning'] . "', '" . $editSpec['targetUrl'] . "')\">
                    <span class='fa fa-pencil'></span> " . $editSpec['label'] . "</button>";
                ...
                if ((isset($extBtns) && sizeof($extBtns) > 0) || (isset($payBtns) && sizeof($payBtns) > 0)) {
                    ...
                    // -------------------------------------------------------------
                    echo "<button type='button' disabled $disabledKurangStok class='btn btn-success margin' style='border:1px #008800 solid;color:#ffffff;'>
                        <span class='fa fa-play'></span> " . $approvalSpec['label'] . "</button>";
                    // echo "</div>";
                }
                else {
                    ...
                    if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                        ...
                        $disabled_button_kreditlimit $disabled_button $disabled_checker $disabledKurangStok
                        onclick=\"if(confirm('" . $approvalSpec['warning'] . "')==1){this.disabled=true;document.getElementById('f1').action='" . $approvalSpec['targetUrl'] . "';
                        document.getElementById('f1').submit();top.open_holdon();}\">
                        <span class='glyphicon glyphicon-ok'></span> " . $approvalSpec['label'] . "</button>";
                    }
                    else {
                        echo "&nbsp;";
                    }
                ...
                if (isset($definitionButton) && sizeof($definitionButton) > 0) {
                    ...
                    echo "<div class='row' style='margin-top: 10px;margin-bottom:-10px;font-size: larger;'>";
                    echo "<div class='panel-body'>";
                    echo "<div class='col-md-12 text-center alert' style='border:1px #cccccc dotted;background:#e5e5e5;line-height:16px;'>";
                    if (isset($beforeStepWarning) && ($beforeStepWarning != NULL)) {
                ...
                if (isset($referenceImages) && count($referenceImages) > 0) {
                    echo "<div class='row' style='margin-top: 10px;margin-bottom:-30px;font-size: larger;'>";
                    echo "<div class='panel-body'>";
                    echo "<div class='col-md-12 text-center alert' style='border:1px #cccccc dotted;background:#e5e5e5;line-height:16px;'>";
                    $image_link = $referenceImages["link_img"];
                    $image_label = $referenceImages["nama"];
//                arrPrint($referenceImages);
                    echo "<div class='image-cojntainer'>";
                    echo "<img width='100%' src='$image_link'  alt='$image_label' >";
                    echo "</div>";
                    echo "</div>";
                    echo "</div>";
                    echo "</div>";
    ...
    case "followupCancelPackingPreview":
        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                //arrPrint($items2);
                if (isset($itemLabels2) && sizeof($itemLabels2) > 0) {
        ...
        if (isset($detil_item_rslt) && sizeof($detil_item_rslt) > 0) {
            ...
            if (isset($detail_items) && sizeof($detail_items) > 0) {
                if (isset($itemLabels) && (sizeof($itemLabels) > 1)) {
                    ...
                    echo "<tr bgcolor='#f5f5f5'>";
                    echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
                    if (count($customHeaderResume) > 0) {
                        foreach ($customHeaderResume as $key => $label) {
                            echo "<th class='text-muted' style='font-weight:bold;'>";
                            if (is_array($label)) {
                                echo $label["label"];
                            }
                        ...
                        echo "<th>Dikirim(qty)</th>";
                        echo "<th>fulfill(qty)</th>";
                        echo "<th>subtotal awal(RP)</th>";
                        echo "<th>fulfill(RP)</th>";
                        echo "<th>subtotal akhir(RP)</th>";
                    }
                    else {
                        foreach ($itemLabels as $key => $label) {
                            echo "<th class='text-muted' style='font-weight:bold;'>";
                    ...
                    foreach ($detail_items as $iSpec) {
                        ...
                        if (count($customHeaderResume) > 0) {
                            ...
                            if (count($shipment) > 0) {
                                $val_kirim = $shipment[$id]["jml"];
                            }
                            else {
                                $val_kirim = 0;
                            }
                            if (count($fullfillment) > 0) {
                                $val_fullfil = $fullfillment[$id]["jml"];
                                $subtotal_fullfil = $val_fullfil * $iSpec["nett1_ppn"];
                            }
                            else {
                                $val_fullfil = 0;
                                $subtotal_fullfil = 0;
                            }
                            $netto = $iSpec["subtotal"] - $subtotal_fullfil;
                            $grandtotal += $netto;
                            echo "<td>" . formatField_he_format("jml", $val_kirim) . "</td>";
                            echo "<td>" . formatField_he_format("jml", $val_fullfil) . "</td>";
                            echo "<td>" . formatField_he_format("subtotal", $iSpec["subtotal"]) . "</td>";
                            echo "<td>" . formatField_he_format("subtotal", $subtotal_fullfil) . "</td>";
                            echo "<td>" . formatField_he_format("subtotal", $netto) . "</td>";
                        }
                        else {
                            foreach ($itemLabels as $key => $label) {
                                echo "<td>";
                    ...
                    if (count($customHeaderResume) > 0) {
                        $listAddRows .= "<tr>";
                        $valAddRows = isset($detail_main[$key]) ? $detail_main[$key] : "0";
                        $listAddRows .= "<td colspan='10' align='right'>grand total</td>";
//                        $listAddRows .= "<td colspan='10'>" . formatField($key, $valAddRows) . "</td>";
                        $listAddRows .= "<td colspan='' >" . formatField_he_format("subtotal", $grandtotal) . "</td>";
                        $listAddRows .= "</tr>";
                        echo $listAddRows;
                    }
                    else {
...
////        matiHere(__LINE__);
        ...
        break;
    case "viewResumeImages":
        //        cekHitam("Here i am" . __LINE__);
        //        cekHere("MBELEKKK");
        //        arrPrint($referenceImages);
        ...
        echo "<div class='row'>";
        echo "<div class='col-md-6 overflow-h' style='overflow: hidden;' >";
        $pakaiini = 0;
        ...
        if ($pakaiini == 0) {
            echo "<div style=''>";
            if (isset($signStr)) {
                echo "<div style='' class='table-responsive'>";
                echo $signStr;
                echo "</div>";
            }
            if (isset($historiEfaktur)) {
            ...
            if (sizeof($items) > 0) {
                foreach ($items as $cabangID => $subItems) {
                    ...
                    if (sizeof($subItems) > 0) {
                        ...
                        echo "<div class='table-responsive'>";
                        echo "<table class='table table-condensed' " . __LINE__ . ">";
...
$('.modal-dialog').addClass('modal-xl')
    ...
    case "viewResumeDetails":
        ...
        if (isset($itemsLabel) && sizeof($itemsLabel) > 0) {
            $contents .= "<div class='table-responsive text-center'>";
            $contents .= "<span style='font-weight: bold;font-size: 20px;'>";
            $contents .= $mainFields["jenisTrName"];
            $contents .= "</span>";
