    ...
    public function viewHistory()
    {
        ...
        if (sizeof($tmpHist) > 0) {
            if (sizeof($pairRegistries) > 0) {
                ...
                if (sizeof($tmpReg) > 0) {
                    foreach ($tmpReg as $regRow) {
                        foreach ($pairRegistries as $param) {
                            $params_decode = blobDecode($regRow->$param);
                            $tmpReg_result[$regRow->transaksi_id][$param] = $params_decode;

                            switch($param){
                                case "items":
                                    $arrTrItems = array_keys($params_decode);
                                    foreach ($arrTrItems as $trItems){
                                        $arrTransItemsID[$trItems] = $trItems;
                                        $arrTransItemsID_trid[$trItems] = $regRow->transaksi_id;
                ...
                if (sizeof($arrTransItemsID) > 0) {
                    $tr = new MdlTransaksi();
                    $tr->setFilters(array());

                    $tr->addFilter("id in ('" . implode("','", $arrTransItemsID) . "')");
                    $tmpTransHistRef = $tr->lookUpMainTransaksi()->result();
                    if (sizeof($tmpTransHistRef) > 0) {
                        foreach ($tmpTransHistRef as $histSpec) {
                            $hist_id = $histSpec->id;// id so project
                            $hist_id_termin = $arrTransItemsID_trid[$hist_id];// id termin
                            $hist = blobDecode($histSpec->ids_his);
                            foreach ($hist as $step => $hisSpec) {
                                $arrIdsHistRef[$hist_id_termin]["referenceSO_ID__".$step] = $hisSpec["trID"];
                                $arrIdsHistRef[$hist_id_termin]["referenceSO_Nomer__".$step] = $hisSpec["nomer"];
                                $arrIdsHistRef[$hist_id_termin]["referenceSO_Step__".$step] = $step;
                                $arrIdsHistRef[$hist_id_termin]["referenceSO_Jenis"] = $histSpec->jenis_master;
                                $arrIdsHistRef[$hist_id_termin]["referenceSO_idhist"] = $hist;
                            }
                        }
                    }
                }
//                arrPrintCyan($arrIdsHistRef);
            }
            ...
            foreach ($tmpHist as $ii => $row) {
                ...
                if(isset($arrIdsHistRef[$row->id]) && (sizeof($arrIdsHistRef[$row->id])>0)){
                    foreach ($arrIdsHistRef[$row->id] as $rkey => $rval){
                        $row->$rkey = $rval;
                    }
                }
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    if (is_array($fLabel)) {
                        ...
                        elseif($hisKey == "nomer_reference_so"){
//                            cekHere("masuk disini...");
                            $modul_path_ref_so = base_url().$fLabel['modul']."/";
//                            $colValue = isset($row->referenceSO_idhist) ? showHistoriGlobalNumbers($row->referenceSO_idhist, $hisStep, true, $row->referenceSO_Jenis) : "-";
                            $colValue = formatField_he_format("nomer", $row->$fName, $row->referenceSO_Jenis, $modul_path_ref_so);// jenis($row->referenceSO_Jenis), modul_path (base_url().$fLabel['modul'])
//                            $colValue = formatField_he_format("nomer_nolink", $row->$fName);// jenis($row->referenceSO_Jenis), modul_path (base_url().$fLabel['modul'])
                        }
                        else {
...
//                    cekHere(__LINE__);
//                    $part4 = microtime(true);
//                    cekKuning($part4-($starttime+$part3+$part2+$part1));
                    //                    $tr = new MdlTransaksi();
                    //                    $tr->setFilters(array());
                    //                    $tr->addFilter("id in ('" . implode("','", $arrTransHist) . "')");
...
//                            cekHere("masuk disini...");
//                            $colValue = isset($row->referenceSO_idhist) ? showHistoriGlobalNumbers($row->referenceSO_idhist, $hisStep, true, $row->referenceSO_Jenis) : "-";
                            $colValue = formatField_he_format("nomer_nolink", $row->$fName);
                        }
                        else {
                            if (isset($fLabel['transaksi_jenis2'][$row->transaksi_jenis2])) {
                                $getKey = $fLabel['transaksi_jenis2'][$row->transaksi_jenis2];
