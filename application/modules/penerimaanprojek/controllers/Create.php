...
class Create extends Modul_Controller
{
    ...
    /**
     * Menentukan key tipe payment untuk payment source project (7499).
     *
     * @param string $stepCode
     * @param array $paymentSrcConfig
     * @param string $sessionCode
     * @param string $connectorCode
     * @return string
     */
    private function resolvePaymentSourceKey($stepCode, $paymentSrcConfig, $sessionCode, $connectorCode = "")
    {
        $targetJenis = isset($paymentSrcConfig['jenisTarget']) ? (string)$paymentSrcConfig['jenisTarget'] : "";
        $stepCode = (string)$stepCode;
        $connectorCode = (string)$connectorCode;

        if ($targetJenis !== "7499" && $stepCode !== "7499" && $connectorCode !== "7499") {
            return "";
        }

        $paymentType = isset($_SESSION[$sessionCode]['main']['type_payment']) ? $_SESSION[$sessionCode]['main']['type_payment'] : "";
        return $this->normalizeProjectPaymentType($paymentType, false);
    }

    /**
     * Normalisasi type payment project.
     *
     * @param string $rawType
     * @param bool $fallbackTermin
     * @return string
     */
    private function normalizeProjectPaymentType($rawType, $fallbackTermin = false)
    {
        $paymentType = strtolower(trim((string)$rawType));

        if ($paymentType === "uang muka" || $paymentType === "uang_muka" || $paymentType === "downpayment") {
            $paymentType = "dp";
        }
        else if ($paymentType === "garansi") {
            $paymentType = "retensi";
        }

        if (!in_array($paymentType, array("dp", "termin", "retensi"))) {
            return $fallbackTermin ? "termin" : "";
        }

        return substr($paymentType, 0, 32);
    }

    /**
     * Ambil alokasi DPP per type payment dari session project setting.
     *
     * @param string $sessionCode
     * @param string $paymentType
     * @return float
     */
    private function getProjectTypeAllocationDpp($sessionCode, $paymentType)
    {
        $ppnFactor = isset($_SESSION[$sessionCode]['main']['ppnFactor']) ? ($_SESSION[$sessionCode]['main']['ppnFactor'] * 1) : 11;
        $divisorPpn = 1 + ($ppnFactor / 100);
        if ($divisorPpn <= 0) {
            $divisorPpn = 1;
        }

        $projectTermin = isset($_SESSION[$sessionCode]['items3']) && is_array($_SESSION[$sessionCode]['items3']) ? $_SESSION[$sessionCode]['items3'] : array();
        $projectDp = isset($_SESSION[$sessionCode]['items4']) && is_array($_SESSION[$sessionCode]['items4']) ? $_SESSION[$sessionCode]['items4'] : array();
        $projectRetensi = isset($_SESSION[$sessionCode]['items5']) && is_array($_SESSION[$sessionCode]['items5']) ? $_SESSION[$sessionCode]['items5'] : array();

        $allocationInclPpn = 0;
        if ($paymentType === "dp") {
            $allocationInclPpn = (!empty($projectDp) && isset($projectDp[0]['harga'])) ? ($projectDp[0]['harga'] * 1) : 0;
        }
        else if ($paymentType === "retensi") {
            $allocationInclPpn = (!empty($projectRetensi) && isset($projectRetensi[0]['harga'])) ? ($projectRetensi[0]['harga'] * 1) : 0;
        }
        else {
            if (!empty($projectTermin)) {
                foreach ($projectTermin as $terminSpec) {
                    $allocationInclPpn += (isset($terminSpec['harga']) ? ($terminSpec['harga'] * 1) : 0);
                }
            }
        }

        $allocationDpp = $allocationInclPpn > 0 ? ($allocationInclPpn / $divisorPpn) : 0;
        if ($allocationDpp < 0) {
            $allocationDpp = 0;
        }

        return $allocationDpp;
    }

    /**
     * Ambil total issued DPP net per type payment dari payment source.
     *
     * Catatan:
     * - Basis validasi alokasi menggunakan DPP (bukan Incl.PPN).
     * - Kolom returned tersimpan basis Incl.PPN, sehingga return DPP dihitung proporsional
     *   terhadap perbandingan returned/tagihan di row yang sama.
     *
     * @param int $projectId
     * @param string $paymentType
     * @param int $excludeTransaksiId
     * @param bool $lockForUpdate
     * @return float
     */
    private function getIssuedProjectTypeDpp($projectId, $paymentType, $excludeTransaksiId = 0, $lockForUpdate = false)
    {
        $hasTrashColumn = $this->db->field_exists('trash', 'transaksi_payment_source');
        $whereTrash = $hasTrashColumn ? " AND trash = 0" : "";

        $sql = "SELECT COALESCE(SUM(
                    CASE
                        WHEN IFNULL(dpp_ppn, 0) <= 0 THEN 0
                        WHEN IFNULL(tagihan, 0) <= 0 THEN IFNULL(dpp_ppn, 0)
                        WHEN IFNULL(returned, 0) <= 0 THEN IFNULL(dpp_ppn, 0)
                        WHEN IFNULL(returned, 0) >= IFNULL(tagihan, 0) THEN 0
                        ELSE GREATEST(
                            IFNULL(dpp_ppn, 0) - (IFNULL(dpp_ppn, 0) * (IFNULL(returned, 0) / NULLIF(IFNULL(tagihan, 0), 0))),
                            0
                        )
                    END
                ), 0) AS total_issued
                ...
                WHERE project_id = ?
                  ...
                  AND target_jenis = ?
                  AND _key = ?{$whereTrash}";
        $params = array($projectId, "7499", "749", $paymentType);

        if ($excludeTransaksiId * 1 > 0) {
            $sql .= " AND transaksi_id <> ?";
            $params[] = $excludeTransaksiId;
        }
        ...
        $row = $query->row();
        return isset($row->total_issued) ? ($row->total_issued * 1) : 0;
    }

    /**
     * Validasi type payment dan plafon alokasi per type untuk transaksi termin (7499).
     *
     * @param string $sessionCode
     * @param int $excludeTransaksiId
     * @param bool $lockForUpdate
     * @return array
     */
    private function validateProjectTypePaymentQuota($sessionCode, $excludeTransaksiId = 0, $lockForUpdate = false)
    {
        if ((string)$this->jenisTr !== "7499") {
            return array("valid" => true, "message" => "");
        }

        $rawType = isset($_SESSION[$sessionCode]['main']['type_payment']) ? $_SESSION[$sessionCode]['main']['type_payment'] : "";
        $paymentType = $this->normalizeProjectPaymentType($rawType, false);
        if ($paymentType === "") {
            return array("valid" => false, "message" => "Type payment wajib dipilih (DP / Termin / Retensi).");
        }

        $_SESSION[$sessionCode]['main']['type_payment'] = $paymentType;

        $projectId = isset($_SESSION[$sessionCode]['main']['projectID']) ? ($_SESSION[$sessionCode]['main']['projectID'] * 1) : 0;
        if ($projectId <= 0) {
            return array("valid" => false, "message" => "Project ID tidak valid untuk proses termin.");
        }

        $currentDpp = isset($_SESSION[$sessionCode]['main']['dpp_ppn']) ? ($_SESSION[$sessionCode]['main']['dpp_ppn'] * 1) : 0;
        if ($currentDpp <= 0) {
            $currentDpp = isset($_SESSION[$sessionCode]['main']['nilai_entry']) ? ($_SESSION[$sessionCode]['main']['nilai_entry'] * 1) : 0;
        }
        if ($currentDpp <= 0) {
            return array("valid" => false, "message" => "Nilai tagihan tidak valid, silahkan isi nominal penerbitan.");
        }

        $allocationDpp = $this->getProjectTypeAllocationDpp($sessionCode, $paymentType);
        $issuedDpp = $this->getIssuedProjectTypeDpp($projectId, $paymentType, $excludeTransaksiId, $lockForUpdate);
        $afterIssuedDpp = $issuedDpp + $currentDpp;
        $tolerance = 1;

        if (($afterIssuedDpp - $allocationDpp) > $tolerance) {
            $msg = "Penerbitan " . strtoupper($paymentType) . " melebihi alokasi. ";
            $msg .= "Alokasi DPP: " . number_format($allocationDpp, 2) . ", ";
            $msg .= "Sudah diterbitkan (net): " . number_format($issuedDpp, 2) . ", ";
            $msg .= "Input saat ini: " . number_format($currentDpp, 2) . ".";
...
//            unset($_SESSION[$cCode]["main"]["bookingNumber"]);
            ...
            $quotaValidation = $this->validateProjectTypePaymentQuota($cCode, 0);
            if (!$quotaValidation['valid']) {
                mati_disini($quotaValidation['message']);
            }
            ...
            $this->db->trans_start();
            $quotaValidationTx = $this->validateProjectTypePaymentQuota($cCode, 0, true);
            if (!$quotaValidationTx['valid']) {
                $this->db->trans_rollback();
                mati_disini($quotaValidationTx['message']);
            }
                    ...
                    foreach ($payConfigs[1] as $paymentSrcConfig) {
                        ...
                        if ($paymentMethod == "update") {
                            ...
                            if (sizeof($tmpData) > 0) {
                                ...
                                $paymentSourceKey = $this->resolvePaymentSourceKey($stepCode, $paymentSrcConfig, $cCode);
                                if ($paymentSourceKey != "") {
                                    $data["_key"] = $paymentSourceKey;
                                }
...
//                                cekUngu(":: " . $externSrc['project_id']);
                                $tr->writePaymentSrc($insertID, array(
                                    ...
                                    "_key" => $this->resolvePaymentSourceKey($stepCode, $paymentSrcConfig, $cCode),
                                    "tagihan" => isset($_SESSION[$cCode]['main'][$valueSrc]) ? $_SESSION[$cCode]['main'][$valueSrc] : 0,
                                    "terbayar" => 0,
                                    "payment_source_keterangan" => $_SESSION[$cCode]['main']['description'],
...
//                            cekKuning($externSrc['ppn_sisa']);
//                            cekKuning($_SESSION[$cCode]["main"][$externSrc['ppn_sisa']]);
...
//                            matiHEre();
            ...
            //==========================================================================================================
//            mati_disini("LINE: " . __LINE__ . " TRANSAKSI BERHASIL, saat ini sedang maintenance. tunggu beberapa saat lagi yaa.., INV: $tmpNomorNota || TRID: $insertID");
            ...
            if (strlen($connector) > 0) {
                $transaksiReferenceID = $insertID;
                $transaksiReferenceNomer = $insertNum;
                ...
                if ($stepNum == sizeof($steps)) {
                    ...
                    $masterReplacers = array(
                        ...
                        "reference_id" => $transaksiReferenceID,
                        "reference_nomer" => $transaksiReferenceNomer,
                    );
...
//                                "contents_intext" => isset($aSpec['contents_intext']) ? $aSpec['contents_intext'] : "",
            ...
            if ($_SERVER['REMOTE_ADDR'] == "202.65.117.72") {
                // mati_disini("LINE: " . __LINE__ . " TRANSAKSI BERHASIL (mode maintenance), tunggu beberapa saat lagi yaa.., TRID: $insertID<br>MAINTENANCE KARENA IP LOKAL: " . $_SERVER['REMOTE_ADDR']);
                mati_disini("LINE: " . __LINE__ . " FROM:IP.72 | TRANSAKSI BERHASIL (mode maintenance), tunggu beberapa saat lagi yaa.., TRID: $insertID<br>IP: ". $_SERVER['REMOTE_ADDR']);
            }
//             mati_disini("LINE: " . __LINE__ . " TRANSAKSI BERHASIL, saat ini sedang maintenance. tunggu beberapa saat lagi yaa.., INV: $tmpNomorNota || TRID: $insertID");
            ...
            if (!empty($items5_sum)) {
                ...
                $prdTsk = new MdlTasklistProject();

                $totalBayarGlobal = $_SESSION[$cCode]["main"]['nilai_bayar'] * 1;
                cekLime("totalBayarGlobal: $totalBayarGlobal");
                foreach ($items5_sum as $idTsk => $taskData) {

                    cekHitam("totalBayarGlobal: $totalBayarGlobal");
                    $aftotalBayarGlobal = $totalBayarGlobal;
                    $thisSisa = $taskData['sisa'] * 1;
                    $thisDebet = $taskData['debet'] * 1;
                    $sisaGlobal = $totalBayarGlobal - $thisSisa;

                    $afDebet = $sisaGlobal * 1 > 0 ? $taskData['debet'] + $thisSisa : ($sisaGlobal * 1 == 0 ? $taskData['debet'] + $thisSisa : $taskData['debet'] + $aftotalBayarGlobal);
                    cekMerah("Debet: $thisDebet");
                    cekMerah("afDebet: $thisDebet+$thisSisa = $afDebet");

                    $afSisa = $sisaGlobal * 1 > 0 ? $thisSisa - $thisSisa : ($sisaGlobal * 1 == 0 ? $thisSisa - $thisSisa : $thisSisa - $aftotalBayarGlobal);
                    cekMerah("sisa: $thisSisa");
                    cekMerah("afSisa: $thisSisa - $thisSisa  = $afSisa");

                    $totalBayarGlobal = $sisaGlobal;
                    cekMerah("totalBayarGlobal: $aftotalBayarGlobal - $thisSisa = $totalBayarGlobal");

                    if ($aftotalBayarGlobal >= 0) {

                        $invoiced = $taskData['invoiced'];
                        $inv_trx_id = $taskData['inv_trx_id'];
                        $inv_dtime = $taskData['inv_dtime'];

                        $ddupState = $prdTsk->updateData(
                            array(
                                "id" => $idTsk
                            ),
                            array(
                                "debet" => $afDebet,
                                "sisa" => $afSisa,
                                "invoiced" => $invoiced != "" ? $invoiced . "," . $tmpNomorNota : $tmpNomorNota,
                                "inv_trx_id" => $inv_trx_id != "" ? $inv_trx_id . "," . $insertID : $insertID,
                                "inv_dtime" => $inv_dtime != "" ? $inv_dtime . "," . date('Y-m-d H:i:s') : date('Y-m-d H:i:s'),
                            )
                        ) or die("Failed to update tr next-state!");
                        showlast_query('biru');
                    }
                        ...
                        cekUngu("totalBayarGlobal: $totalBayarGlobal");
                    }
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., INV: $tmpNomorNota || TRID: $insertID");
...
//        }
    ...
    public function doEdit()
    {
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            $editMasterID = 0;
            if (isset($_SESSION[$cCode]['mode']['edit'])) {
                if (is_array($_SESSION[$cCode]['mode']['edit'])) {
                    $editMasterID = isset($_SESSION[$cCode]['mode']['edit'][0]) ? ($_SESSION[$cCode]['mode']['edit'][0] * 1) : 0;
                }
                else {
                    $editMasterID = $_SESSION[$cCode]['mode']['edit'] * 1;
                }
            }
            $quotaValidation = $this->validateProjectTypePaymentQuota($cCode, $editMasterID);
            if (!$quotaValidation['valid']) {
                mati_disini($quotaValidation['message']);
            }
            ...
            $this->db->trans_start();
            $quotaValidationTx = $this->validateProjectTypePaymentQuota($cCode, $editMasterID, true);
            if (!$quotaValidationTx['valid']) {
                $this->db->trans_rollback();
                mati_disini($quotaValidationTx['message']);
            }
                            ...
                            if (sizeof($payConfigs) > 0) {
                                foreach ($payConfigs as $paymentSrcConfig) {
                                    ...
                                    $tr->writePaymentSrc($insertID, array(
                                        ...
                                        "nomer" => $_SESSION[$cCode]['main']['nomer'],
                                        "label" => $paymentSrcConfig['label'],
                                        "_key" => $this->resolvePaymentSourceKey($stepCode, $paymentSrcConfig, $cCode, $connector),
                                        "tagihan" => $_SESSION[$cCode]['main'][$valueSrc],
                                        "terbayar" => 0,
                                        "sisa" => $_SESSION[$cCode]['main'][$valueSrc],
                                        "cabang_id" => $_SESSION[$cCode]['main']['placeID'],
