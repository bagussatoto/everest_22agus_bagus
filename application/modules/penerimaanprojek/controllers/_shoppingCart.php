...
class _shoppingCart extends Modul_Controller
{
     ...
     * PURPOSE   : Menyiapkan data keranjang penagihan proyek dengan pembatasan status DP dan validasi sisa plafon riil.
     * COMPLIANCE: PSAK 72 / IFRS 15, ISO 9001:2015, ISO 27001
     * LOG_EXPIRE: 2026-11-04
     * [/AGENT_LOG]
     */
...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        $itemLabels4 = isset($this->configUi[$this->jenisTr]['shoppingCartFields4'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartFields4'][$stepNumber] : array();
        ...
        $itemNumLabels4 = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields4'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields4'][$stepNumber] : array();
        $itemNumLabels5 = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields5'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields5'][$stepNumber] : array();
        $itemLabels5 = isset($this->configUi[$this->jenisTr]['shoppingCartFields5'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartFields5'][$stepNumber] : array();
...
//                            $main[$key] = $valItem;
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items4_new'])) {
                $no = 0;
                foreach ($_SESSION[$cCode]['items4_new'] as $iSpec) {
                    $no++;
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => isset($iSpec['nama']) ? $iSpec['nama'] : "",
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items4_sum'])) {
                $no = 0;
                foreach ($_SESSION[$cCode]['items4_sum'] as $iSpec) {
                    $no++;
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => isset($iSpec['nama']) ? $iSpec['nama'] : "",
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items5_new'])) {
                $currentTypePayment = isset($_SESSION[$cCode]['main']['type_payment']) ? strtolower(trim($_SESSION[$cCode]['main']['type_payment'])) : "";
                if ($currentTypePayment === "uang muka" || $currentTypePayment === "uang_muka" || $currentTypePayment === "downpayment") {
                    $currentTypePayment = "dp";
                }
                else if ($currentTypePayment === "garansi") {
                    $currentTypePayment = "retensi";
                }
                $isModeDp = $currentTypePayment === "dp";

                $no = 0;
                foreach ($_SESSION[$cCode]['items5_new'] as $iSpec) {
                    $no++;
                    $ids = $iSpec['id'];
                    $harga = $iSpec['nilai_sub_fase'];
                    $persen = $iSpec['persen_sub'];
                    $invoiced = $iSpec['debet'];
                    $debetNominal = $invoiced * 1;
                    $sisa = $iSpec['sisa'];
                    $progress_percent = $iSpec['progress_percent'];
                    $progress_id = $iSpec['progress_id'];
                    ...
                    $selected = isset($_SESSION[$cCode]['items5_sum'][$ids]) ? "checked" : "";

                    if ($isModeDp) {
                        $disabledCheck = $sisa * 1 == 0 ? "disabled" : "";
                        $dpTerpakaiNominal = (isset($iSpec['dp_terpakai']) ? $iSpec['dp_terpakai'] * 1 : (isset($iSpec['debet_dp']) ? $iSpec['debet_dp'] * 1 : 0));
                        if ($sisa * 1 == 0) {
                            $labelInfo = "<div><label class='text-green text-bold'>sudah ditagih</label></div>";
                        }
                        else if ($dpTerpakaiNominal > 0) {
                            $labelInfo = "<div><label class='text-warning text-bold'>sudah dipakai DP sebagian</label></div>";
                        }
                        else {
                            $labelInfo = "<div><label class='text-info text-bold'>siap ditagih untuk DP project</label></div>";
                        }
                    }
                    else {
                        $disabledCheck = $sisa * 1 == 0 || $progress_percent != 100 ? "disabled" : "";
                        $disabledCheck = $progress_percent == 100 && $progress_id == 2 ? "disabled" : $disabledCheck;
                        if ($sisa * 1 == 0 && $progress_percent == 100) {
                            $labelInfo = "<div><label class='text-green text-bold'>sudah ditermin</label></div>";
                        }
                        else if ($progress_percent == 100 && $debetNominal > 0) {
                            $labelInfo = "<div><label class='text-warning text-bold'>sudah ditermin sebagian</label></div>";
                        }
                        else if ($progress_percent < 100 && $sisa * 1 > 0) {
                            $labelInfo = "<div><label><b><r>belum di proses<br>oleh pelaksana ($progress_percent%)</r></b></label></div>";
                        }
                        else if ($progress_percent == 100 && $progress_id == 2) {
                            $labelInfo = "<div><label><b><r>belum di QC<br>(Progress = $progress_percent%)</r></b></label></div>";
                        }
                        else {
                            $labelInfo = "";
                        }
                    ...
                    $disabledCheckClass = $disabledCheck == "disabled" ? "disabled_" : "";

                    $tmp["checklistTarget"] = "<input $disabledCheck $selected class='" . $disabledCheckClass . "checkbox_tasklist' persen='$persen' harga='$sisa' pid='$ids' type='checkbox' id='checklist_$ids'>$labelInfo";
                    $items5[] = $tmp;
                }
            }
        }
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items5_sum'])) {
                $no = 0;
                foreach ($_SESSION[$cCode]['items5_sum'] as $iSpec) {
                    $no++;
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => isset($iSpec['nama']) ? $iSpec['nama'] : "",
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartSubamount2']) && $this->configUi[$this->jenisTr]['shoppingCartSubamount2'][$stepNumber] == true) {
            $itemLabels = $itemLabels + $itemNumLabels + array("subtotal" => "Total");
            $itemLabels = $itemLabels + array("sub_harga" => "Total<br><r>(excl)</r><br><r>(VAT & DISC)</r>");
            $itemLabels2 = $itemLabels2 + $itemNumLabels2 + array("subtotal" => "Total");
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartSubamount4']) && $this->configUi[$this->jenisTr]['shoppingCartSubamount4'][$stepNumber] == true) {
            $itemLabels4 = $itemLabels4 + $itemNumLabels4 + array("subtotal" => "Total");
        }
        else {
            $itemLabels4 = $itemLabels4 + $itemNumLabels4;
        }

        if (isset($this->configUi[$this->jenisTr]['shoppingCartSubamount5']) && $this->configUi[$this->jenisTr]['shoppingCartSubamount5'][$stepNumber] == true) {
            $itemLabels5 = $itemLabels5 + $itemNumLabels5 + array("subtotal" => "Total");
        }
        else {
            $itemLabels5 = $itemLabels5 + $itemNumLabels5;
        }
                            ...
                            foreach ($relOptionConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                ...
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    $inputs[$oValueName] = "<input " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " type=text id='$oValueName' class='form-control text-center' placeholder='$oValueName' value='" . number_format($defVal) . "' min='$minValue' max='$maxValue' onfocus='this.select()'
                                        ...
                                        onblur=\"if(removeCommas(this.value)!=removeCommas(this.defaultValue)){hiliteDiv(this);top.$('#result').load($relInputTarget);}\"
                                        onmouseouts=\"if(removeCommas(this.value)!=removeCommas(this.defaultValue)){hiliteDiv(this);top.$('#result').load($relInputTarget);}\">";
                        ...
                        if (sizeof($addRowsConfigs[$eName][$currentValue]) > 0) {
                            ...
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                ...
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    $relInputTargetTail = "";
                                    if ((string)$this->jenisTr === "7499" && $oValueName === "nilai_entry") {
                                        $relInputTargetTail = "+'&valCol=description&valValue='+encodeURIComponent((typeof resolveDescNotesValueForRequest === 'function' ? resolveDescNotesValueForRequest() : ''))";
                                    }
                                    $relInputTarget = "'" . MODUL_PATH . get_class($this) . "/recordAddRow/" . $this->jenisTr . "/$oValueName/?val='+removeCommas(this.value)$relInputTargetTail";
                                    ...
                                    $keyupStr = "";
                                    if ($maxValue != "") {
                                        $keyupStr .= "if(parseInt(removeCommas(this.value))>" . round($maxValue) . "){this.value='" . number_format($origDefValue) . "';this.select();}else{if(parseInt(removeCommas(this.value))>0){this.value=addCommas(this.value)}else{this.value=0}}";
                                    }
                                    $keyupStr .= $keyupAct;
                                    ...
                                    $bgClassInputColor = $disabled == "" ? " bg-lime text-black text-bold" : "";


                                    $blurStr = "";
                                    if ($minValue != "") {
                                        $blurStr = "if(removeCommas(this.value)!=removeCommas(this.defaultValue)){if(parseInt(removeCommas(this.value))>=" . round($minValue) . "){hiliteDiv(this);top.$('#result').load($relInputTarget);}else{this.value='$minValue';this.focus();}}";
                                    }
                                    else {
                                        $blurStr = "if(removeCommas(this.value)!=removeCommas(this.defaultValue)){hiliteDiv(this);top.$('#result').load($relInputTarget);}";
                                    ...
                                    if (isset($addRowsConfigs[$eName][$currentValue][$oValueName]['role']) && $addRowsConfigs[$eName][$currentValue][$oValueName]['role'] == "minus") {
                                        $defVal = "(" . number_format($defVal) . ")";
                                    }
                                    $addRows[$oValueName] = "<input " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " type=text autocomplete='off' id='$oValueName' class='form-control$bgClassInputColor text-right' style='font-size:17px;' $disabled placeholder='$oValueName' value='" . $defVal . "' $minValStr $maxValStr
...
//                $_SESSION[$cCode]["main"]["skip_faktur"]="true";
        ...
        $taskListProject = new MdlTasklistProject();
        $this->db->where("produk_id", $main['project_id']);
        $arrTaskListProject = $taskListProject->lookupAll()->result();

        $daftarGudangWo = array();
        $nilaiProgressAll = 0;
        $nilai_kontrak = 0;

        if (!empty($arrTaskListProject)) {
            foreach ($arrTaskListProject as $taskData) {
                $nilai_kontrak = $taskData->nilai_kontrak;
                $daftarGudangWo[] = $taskData->gudang_wo;
                $nilaiProgressAll += ($taskData->nilai_sub_fase * $taskData->progress_percent) / 100;
            }
        }

        $persenProgressAll = round(($nilaiProgressAll / $nilai_kontrak) * 100, 2);

        $arrListLockerProject = array();
        if (!empty($daftarGudangWo)) {
            $this->load->model("Mdls/MdlLockerStock");
            $taskListLockerProject = new MdlLockerStock();
            $this->db->where_in("gudang_id", $daftarGudangWo);
            $this->db->where("jumlah > 0");
            $arrListLockerProject = $taskListLockerProject->lookupAll()->result();
        }

        $arrListLockerProjectSupplies = array();
        if (!empty($daftarGudangWo)) {
            $this->load->model("Mdls/MdlLockerStockSupplies");
            $taskListLockerProjectSupplies = new MdlLockerStockSupplies();
            $this->db->where_in("gudang_id", $daftarGudangWo);
            $this->db->where("jumlah > 0");
            $arrListLockerProjectSupplies = $taskListLockerProjectSupplies->lookupAll()->result();
        }

        $receiptAddSDppTax = isset($this->configLayout[$this->jenisTr]['receiptAddDpp'][$stepNumber]) ? $this->configLayout[$this->jenisTr]['receiptAddDpp'][$stepNumber] : array();
        $sumDppTax = array();
        ...
        $terminproject = array();
        $terminprojectDp = array();
        $terminprojectTermin = array();
        $terminprojectRetensi = array();

// START OF COMPLETE REPEATED LOGIC
        $projectPid = isset($main['project_id']) ? ($main['project_id'] * 1) : 0;
        if ($projectPid <= 0 && isset($_SESSION[$cCode]['main']['projectID'])) {
            $projectPid = $_SESSION[$cCode]['main']['projectID'] * 1;
        }
        if ($projectPid > 0) {
            $this->load->model("MdlTransaksi");
            $paymentData = $this->MdlTransaksi->getProjectPaymentSummaryCI3($projectPid);
            $terimabayarproject = isset($paymentData["terimabayarproject"]) ? $paymentData["terimabayarproject"] : array();
            $terminproject = isset($paymentData["terminproject"]) ? $paymentData["terminproject"] : array();
            $terminprojectDp = isset($paymentData["terminproject_dp"]) ? $paymentData["terminproject_dp"] : array();
            $terminprojectTermin = isset($paymentData["terminproject_termin"]) ? $paymentData["terminproject_termin"] : $terminproject;
            $terminprojectRetensi = isset($paymentData["terminproject_retensi"]) ? $paymentData["terminproject_retensi"] : array();
        }

        // Fetch histori DP 4467 dari __rek_pembantu_customer_project__2010050
        $historiDP = array();
        $customerPid = isset($main['customers_id']) ? ($main['customers_id'] * 1) : (isset($_SESSION[$cCode]['main']['pihakID']) ? ($_SESSION[$cCode]['main']['pihakID'] * 1) : 0);
        if ($projectPid > 0) {
            $this->db->select("
                t.id AS transaksi_dp_id,
                t.jenis AS jenis,
                t.nomer AS no_kwitansi_dp,
                t.dtime AS tanggal_terima,
                t.oleh_nama AS penerima_nama,
                t.customers_id AS customer_id,
                t.customers_nama AS customer_nama,
                t.project_id AS project_id,
                t.transaksi_nilai AS total_dp_gross,
                t.transaksi_net AS dpp_dp,
                t.ppn_nilai AS ppn_dp,
                t.bank_nama AS bank_nama,
                t.bank_rekening_nama AS bank_rekening_nama,

                rp.kredit AS dp_diterima,
                rp.debet AS dp_terpakai_lalu,
                (rp.kredit - rp.debet) AS sisa_dp_active
            ", FALSE);
//            t.faktur_pajak_nomer AS no_faktur_pajak,
            $this->db->from("transaksi t");
            $this->db->join("__rek_pembantu_customer_project__2010050 rp", "t.id = rp.transaksi_id", "inner");
            $this->db->where("t.transaksi_jenis", "4467");
            $this->db->where("t.trash", 0);
            if ($customerPid > 0) {
                $this->db->where("t.customers_id", $customerPid);
            }

            $this->db->group_start();
                $this->db->where("t.project_id", $projectPid);
//                $this->db->or_where("t.referensi_so_project", $projectPid);
            $this->db->group_end();

            $this->db->order_by("t.dtime", "ASC");
            $historiDP = $this->db->get()->result_array();
        }
...
//            "itemLabels4" => $itemLabels4,
            "itemLabels5" => $itemLabels5,
            ...
            "linkSelectWorkOrder" => MODUL_PATH . get_class($this) . "/selectWorkOrder/" . $this->jenisTr,
            "minValues" => $minValue,
            "addRows" => $addRows,
            ...
            "project_termin" => isset($_SESSION[$cCode]['items3']) ? $_SESSION[$cCode]['items3'] : array(),
            "project_dp" => isset($_SESSION[$cCode]['items4']) ? $_SESSION[$cCode]['items4'] : array(),
            "project_garansi" => isset($_SESSION[$cCode]['items5']) ? $_SESSION[$cCode]['items5'] : array(),
            "items7" => isset($_SESSION[$cCode]['items7']) ? $_SESSION[$cCode]['items7'] : array(),
            "projectName" => isset($_SESSION[$cCode]['main']['projectName']) ? $_SESSION[$cCode]['main']['projectName'] : "unknown project",
            "projectHarga" => isset($_SESSION[$cCode]['main']['projectHarga']) ? $_SESSION[$cCode]['main']['projectHarga'] : "unknown project",
            "projectNilai" => isset($_SESSION[$cCode]['main']['projectNilai']) ? $_SESSION[$cCode]['main']['projectNilai'] : "unknown project",
            "ppnFactor" => isset($_SESSION[$cCode]['main']['ppnFactor']) ? $_SESSION[$cCode]['main']['ppnFactor'] : 11,
            "terimabayarproject" => $terimabayarproject,
            "terminproject" => $terminproject,
            "terminprojectDp" => $terminprojectDp,
            "terminprojectTermin" => $terminprojectTermin,
            "terminprojectRetensi" => $terminprojectRetensi,
            ...
            "typePaymentDefault" => isset($_SESSION[$cCode]['main']['type_payment']) ? $_SESSION[$cCode]['main']['type_payment'] : "",
            ...
            "daftarGudangWo" => $daftarGudangWo,
            "persenProgressAll" => $persenProgressAll * 1,
...
//        arrPrint($data);
    ...
    public function recordAddRow()
    {
        ...
        $val = ($_GET['val']);
        $valCol = isset($_GET['valCol']) ? $_GET['valCol'] : "";
...
//        arrPrint($_GET);
    ...
    public function selectWorkOrder()
    {
        $cCode = "_TR_" . $this->jenisTr;
        $id = $_GET['transaksi_id'];

        $data = $_REQUEST['data'];
        $totalCheckRp = $_REQUEST['totalCheckRp'];
        $totalCheckPercent = $_REQUEST['totalCheckPercent'];

        if (!empty($data) && $totalCheckRp * 1 > 0 && $totalCheckPercent * 1 > 0) {
            if (isset($_SESSION[$cCode]['items5_sum'])) {
                unset($_SESSION[$cCode]['items5_sum']);
            }
            foreach ($data as $woID => $woData) {
                if (!empty($_SESSION[$cCode]['items5_new'][$woID])) {
                    $_SESSION[$cCode]['items5_sum'][$woID] = array_merge($_SESSION[$cCode]['items5_new'][$woID], $woData);
                }
            }
        }
        else {
    ...
    public function recordColumn()
    {
        ...
        $cCode = $this->cCode;
        $colName = $this->uri->segment(5);
        if (strlen((string)$colName) < 1) {
            $colName = $this->uri->segment(6);
        }
        $val = urldecode($_GET['val']);
        $valCol = isset($_GET['valCol']) ? $_GET['valCol'] : null;
        $valValue = isset($_GET['valValue']) ? urldecode($_GET['valValue']) : null;
        $noRefresh = isset($_GET['norefresh']) ? ($_GET['norefresh'] * 1) : 0;

        if ($colName === "type_payment") {
            $val = strtolower(trim((string)$val));
            if ($val === "uang muka" || $val === "uang_muka" || $val === "downpayment") {
                $val = "dp";
            }
            else if ($val === "garansi") {
                $val = "retensi";
            }
            if (!in_array($val, array("", "dp", "termin", "retensi"))) {
                $val = "";
            }
        }
