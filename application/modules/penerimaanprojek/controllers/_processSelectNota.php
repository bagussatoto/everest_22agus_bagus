...
//        $this->jenisTr = $this->uri->segment(4);
        ...
        // Tentukan kolom filter berdasarkan prefix
        // Pola: 588so -> quot_nomer, 588st -> project_start_nomer
        if (substr($nomer, 0, 5) === '588so') {
            $columnFilter = "quot_nomer";
        }
        elseif (substr($nomer, 0, 5) === '588st') {
            $columnFilter = "project_start_nomer";
        }
        ...
        $prdprj->addFilter("$columnFilter=$nomer");
        $prevProdukProject = $prdprj->lookUpAll()->result();

        //project sudah closing
        if($prevProdukProject[0]->closing_status == 99 || $prevProdukProject[0]->trash > 0 ){
...
//        arrPrintWebs($prevProdukProject);
//        cekMerah("__CEK STATUS PROJECT__");
//        matiHere(__LINE__);
...
//        $prdprj = new MdlProdukProject();
//
//        $prdprj->addFilter("quot_nomer='".$_GET['nomer']."'");
//        $prevProdukProject = $prdprj->lookUpAll()->result();
//
...
//        arrPrint($prevProdukProject);
        ...
        if(!empty($prevProdukProject)){
            ...
            //bahan baku dikerjakan (tasklist / WO)
            $arrBahanBakuWo=array();
            $this->db->select("*");
            $this->db->where("status='1' and trash='0' and produk_id='$id_project' and jenis_transaksi='sub_wo'");
            $bahanBakuTasklist = $this->db->get("project_sub_tasklist_komposisi")->result();
            if(!empty($bahanBakuTasklist)){
                foreach($bahanBakuTasklist as $ky => $dtask){
                    $arrBahanBakuWo[$dtask->no_spk]['produk'][$dtask->produk_dasar_id] = (array)$dtask;
                }
            }
            ...
            if(!empty($bahanBakuBOM)){
                foreach($bahanBakuBOM as $k => $tskSub){
                    $arrBahanBaku[$tskSub->no_spk][$tskSub->jenis][$tskSub->produk_dasar_id] = (array)$tskSub;
                    if( isset($arrBahanBakuWo[$tskSub->no_spk][$tskSub->jenis][$tskSub->produk_dasar_id]) ){
                        $jmlFromBOM = $arrBahanBaku[$tskSub->no_spk][$tskSub->jenis][$tskSub->produk_dasar_id]['jml'];
                        $jmlFromWO = $arrBahanBakuWo[$tskSub->no_spk][$tskSub->jenis][$tskSub->produk_dasar_id]['jml'];

                        if($jmlFromWO>$jmlFromBOM){
                            $kelebihan[$tskSub->no_spk][$tskSub->jenis][$tskSub->produk_dasar_id] = array(
                                "jmlFromBOM" => $jmlFromBOM,
                                "jmlFromWO" => $jmlFromWO,
                            );
                        }
            ...
            $prdTsk->addFilter("produk_id='$id_project'");
//            $prdTsk->addFilter("invoiced=0");
//            $prdTsk->addFilter("sisa>0");
//            $prdTsk->addFilter("progress_percent=100");
//            $prdTsk->addFilter("progress_id=3");
            $prevProdukTasklist = $prdTsk->lookUpAll()->result();
            $lastQ = $this->db->last_query();
            if(!empty($prevProdukTasklist)){
                foreach($prevProdukTasklist as $ky => $tsk){
//                    $unDoneTask[$id_project][$tsk->id] = $tsk;
                    $tsk->bahan_baku = isset($arrBahanBaku[$tsk->no_spk]) ? $arrBahanBaku[$tsk->no_spk] : array();
                    $tsk->kelebihan = isset($kelebihan[$tsk->no_spk]) ? $kelebihan[$tsk->no_spk] : array();
                    $unDoneTask[$tsk->id] = (array)$tsk;
                }
            }
//            $unDoneTask[$id_project]['query'] = $lastQ;
        }
        ...
        $initMain = array(
            ...
            "projectNilai" => isset($_GET['extern_nilai2']) && sizeof($_GET['extern_nilai2']) > 0 ? $_GET['extern_nilai2'] : "",
        );
...
//matiHere(__LINE__);
                        ...
                        foreach ($row as $key_reg => $val_reg) {
                            switch ($key_reg) {
                                case "items":
                                    ...
                                    break;
                                case "items2":
                                    $items2Registries = blobDecode($val_reg);
                                    break;
                                case "items5":
                                    $items5Registries = blobDecode($val_reg);
                                    break;
                                case "items7":
                                    $items7Registries = blobDecode($val_reg);
                                    break;
                                case "items4":
                                    $items4Registries = blobDecode($val_reg);
                                    break;
                                case "items3":
                                    $items3Registries = blobDecode($val_reg);
                                    break;
                            }
                        }
                    }
        ...
        $_SESSION[$cCode]['items3'] = $items3Registries;
        $_SESSION[$cCode]['items4'] = $items4Registries;
        $_SESSION[$cCode]['items5'] = $items5Registries;
        $_SESSION[$cCode]['items7'] = $items7Registries;
            ...
            case "true":
                ...
                if (sizeof($componentAssConfig) > 0) {
                    if (!array_key_exists($id, $_SESSION[$cCode]['items2'])) {
                        $_SESSION[$cCode]['items2'][$id] = $items2Registries;
                    }
                }
                break;
            case "false":
        ...
        if (sizeof($_SESSION[$cCode]['items2']) > 0) {
            ...
            foreach ($_SESSION[$cCode]['items2'] as $pID => $pSpec) {
                foreach ($pSpec as $eKey => $eSpec) {
                    foreach($eSpec as $jn => $jSpec){
                        if (!isset($_SESSION[$cCode]['items2_sum'][$eKey][$jn])) {
//                            $_SESSION[$cCode]['items2_sum'][$eKey][$jn] = $jSpec;
//                            $_SESSION[$cCode]['items2_sum'][$eKey][$jn]['jml'] = 0;
//                            $_SESSION[$cCode]['items2_sum'][$eKey][$jn]['harga'] = 0;
//                            $_SESSION[$cCode]['items2_sum'][$eKey][$jn]['produk_ids'] = array();
                        }
                    }
//                    if (!isset($_SESSION[$cCode]['items2_sum'][$eKey])) {
//                        $_SESSION[$cCode]['items2_sum'][$eKey] = $eSpec;
////                        $_SESSION[$cCode]['items2_sum'][$eKey]['jml'] = 0;
////                        $_SESSION[$cCode]['items2_sum'][$eKey]['harga'] = 0;
////                        $_SESSION[$cCode]['items2_sum'][$eKey]['produk_ids'] = array();
//                    }

//                    $_SESSION[$cCode]['items2_sum'][$eKey]['jml'] += $eSpec['jml'];
//                    $_SESSION[$cCode]['items2_sum'][$eKey]['harga'] += $eSpec['harga'];
//                    $_SESSION[$cCode]['items2_sum'][$eKey]['produk_ids'][$pID] = $pID;
//                    if (sizeof($relativeComNameDetails) > 0) {
//                        $key = isset($_SESSION[$cCode]['main']['pihakMainName']) ? $_SESSION[$cCode]['main']['pihakMainName'] : "";
//                        cekHere($key);
//                        cekHere("$relativeComNameDetails[$key] " . $relativeComNameDetails[$key]);
//                        $_SESSION[$cCode]['items2_sum'][$eKey]['relativeCom'] = isset($relativeComNameDetails[$key]) ? $relativeComNameDetails[$key] : "";
//                        $_SESSION[$cCode]['items2_sum'][$eKey]['rekName'] = isset($_SESSION[$cCode]['main']['pihakMainName']) ? $_SESSION[$cCode]['main']['pihakMainName'] : "";
//                    }
...
//                            cekHere("hasilnya $key -> " . $tmp[$key]);
    ...
    public function generateNilai(){
        $this->load->model("Mdls/MdlTasklistProject");
        $prdTsk = new MdlTasklistProject();
        $prdTsk->addFilter("kredit=0");
        $prevProdukTasklist = $prdTsk->lookUpAll()->result();
        if(!empty($prevProdukTasklist)){
            foreach($prevProdukTasklist as $k => $row){
                $prdTsk->updateData(
                    array(
                        "id" => $row->id
                    ),
                    array(
                        "debet" => 0,
                        "sisa" => $row->nilai_sub_fase,
                        "kredit" => $row->nilai_sub_fase,
                    ));
            }
        }
    }
