...
class Printing extends Modul_Controller
{
    ...
    public function viewProformaReceipt()
    {
        ...
        //                        arrPrint($tmpTr);
        if (sizeof($tmpTr) > 0) {
            $this->jenisTr = $tmpTr[0]->jenis;
            if (!isset($this->configUi[$this->jenisTr])) {
                $this->configUi[$this->jenisTr] = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiUi");
            }
            if (!isset($this->configLayout[$this->jenisTr])) {
                $this->configLayout[$this->jenisTr] = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiLayout");
            }
            if (!isset($this->configCore[$this->jenisTr])) {
                $this->configCore[$this->jenisTr] = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiCore");
            }
...
//            $itemLabels = sizeof($receiptDetailFields) > 0 ? $receiptDetailFields : $itemLabels;
            ...
            if ($mainJenis == "paket") {
                ...
                foreach ($tmpTr as $row) {
                    ...
                    //--- tanbahan fields, sementara manual disini
                    if(isset($itemsRegistries[$id]) && (sizeof($itemsRegistries[$id])>0)){
                        $rawItems[$row->produk_id]['project_id'] = isset($itemsRegistries[$id]['project_id']) ? $itemsRegistries[$id]['project_id'] : "";
                        $rawItems[$row->produk_id]['project_nama'] = isset($itemsRegistries[$id]['project_nama']) ? $itemsRegistries[$id]['project_nama'] : "";
                    }
...
//                    $fixedElements['fixedElements']['label'] = $globalVars['jenis_label'];
                    $fixedElements['fixedElements']['label'] = $globalVars['title_nota'];
                }
                else {
...
//                    $fixedElements['fixedElements']['label'] = $globalVars['jenis_label'];
        ...
        $receiptSumFields = isset($this->configLayout[$this->jenisTr]['receiptSumFields'][$row->step_number]) ? $this->configLayout[$this->jenisTr]['receiptSumFields'][$row->step_number] : array();
        ...
        $itemNumLabels5 = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields5'][$row->step_number]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields5'][$row->step_number] : array();
        $itemLabels5 = isset($this->configUi[$this->jenisTr]['detailsPrintFields5'][$row->step_number]) ? $this->configUi[$this->jenisTr]['detailsPrintFields5'][$row->step_number] : array();
...
//            "title" => $this->configUi[$jenisTr]["steps"][$currentStepNum]["label"],
    ...
    public function viewReceiptReg()
    {
        ...
        // Jika jenisTr bukan milik modul saat ini, template berada di folder modul pemilik
        ...
        if ($modul_transaksi != $this->modul) {
            // path lengkap ke modul pemilik transaksi (melewati MODUL_TEMPLATE_PATH)
