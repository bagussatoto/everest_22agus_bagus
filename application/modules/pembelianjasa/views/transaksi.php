...
//        }
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                $badgeCount = 0;
                if (isset($arrayOnprogressCountGroup[$tStep])) {
                    $badgeCount = $arrayOnprogressCountGroup[$tStep];
                }
                elseif (isset($arrayOnprogressGroup[$tStep])) {
                    $badgeCount = sizeof($arrayOnprogressGroup[$tStep]);
                }
                $isiBadge = $badgeCount > 0 ? "<span class='badge bg-red'>" . $badgeCount . "</span>" : "";
                $actives = $tStep == $stepper ? "active" : "";
                $trSelesai = count($steps) == $tStep ? "SELESAI<br>" : "";
...
//            arrPrintWebs($shopingcartAddDpp);
...
//                        cekLime($keyAdd);
                    ...
                    echo "<tr line=" . __LINE__ . ">";
                    if(isset($shopingcartAddDpp[$keyAdd])){
//                    arrPrint($shopingcartAddDpp[$keyAdd]);
                        $colspan_i = sizeof($itemLabels) - 2;
                        foreach($shopingcartAddDpp[$keyAdd] as $ik =>$ik_label){
                            if(isset($main[$ik])){
                                $colspan_r = "1";
