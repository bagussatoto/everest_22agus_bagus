...
//             mati_disini("LOLOS, tidak kena VALIDATE :: $prevAction");
    ...
    public function viewUndoneItemsIndex()
    {
        ...
        $arrFilters = array();
        $akses_target = array();
        $arrayOnprogressCountGroup = array();
...
//                        }
    ...
    public function viewUndoneItemsIndex_all()
    {
        ...
        $maxInlineItemsOnUndoneAll = isset($this->configUi[$jenisTr]["maxInlineItemsOnUndoneAll"]) ? (int)$this->configUi[$jenisTr]["maxInlineItemsOnUndoneAll"] : 50;
        // Batas aman: detail item inline hanya untuk jumlah row kecil (<= 50).
        if ($maxInlineItemsOnUndoneAll <= 0) {
            $maxInlineItemsOnUndoneAll = 50;
        }
        if ($maxInlineItemsOnUndoneAll > 50) {
            $maxInlineItemsOnUndoneAll = 50;
        }
        $liteItemsOnUndoneAll = (isset($_GET['lite']) && ((int)$_GET['lite'] == 1)) ? true : false;
        // Default tampilkan badge semua step agar user langsung melihat antrian.
        // Bisa dimatikan sementara via query string ?badge=0 jika perlu troubleshooting.
        $loadBadgeAllOnUndoneAll = true;
        if (isset($_GET['badge']) && ((int)$_GET['badge'] == 0)) {
            $loadBadgeAllOnUndoneAll = false;
        }
        ...
        $prePreFields['action'] = "action";
        $swapJenisTr = isset($this->configUi[$jenisTr]['requestCode']['masterCode']) ? $this->configUi[$jenisTr]['requestCode']['masterCode'] : array();
        $enablePrePreOnUndoneAll = isset($this->configUi[$jenisTr]['enablePrePreOnUndoneAll']) ? $this->configUi[$jenisTr]['enablePrePreOnUndoneAll'] : false;
        $arrayOnprePre = array();
        $arrayOnprePreGroup = array();
        if ($enablePrePreOnUndoneAll && sizeof($swapJenisTr) > 0) {
            $steps = $this->configUi[$swapJenisTr]['steps'];
            $arrFilters = array();
            if (sizeof($steps) > 1) {
                $this->load->model("MdlTransaksi");
...
//            cekHere("all step dibaca semua...");
        ...
        if ($loadBadgeAllOnUndoneAll) {
            // Hitung badge semua step via query agregat ringan.
            $bq = $this->db;
            $bq->select("transaksi_data.sub_step_number as sub_step_number, COUNT(DISTINCT CONCAT(transaksi_data.transaksi_id, '|', transaksi_data.next_substep_code)) as total", false);
            $bq->from("transaksi");
            $bq->join("transaksi_data", "transaksi_data.transaksi_id = transaksi.id");
            // Samakan default filter bawaan MdlTransaksi.
            $bq->where("transaksi.status", "1");
            $bq->where("transaksi.trash", "0");
            $bq->where("transaksi.link_id", "0");
            $bq->where("transaksi.div_id", $this->session->login['div_id']);
            $bq->where("transaksi.jenis_top", $steps[1]['target']);
            $bq->where("transaksi_data.trash", "0");
            $bq->where("transaksi_data.next_substep_code !=", "");
            $bq->where("transaksi_data.sub_step_number >", 0);
            $bq->where("transaksi_data.valid_qty >", 0);
            if (sizeof($akses_target) > 0) {
                $bq->where_in("transaksi.next_step_code", array_values($akses_target));
            }
            if ($this->session->login['employee_type'] == "employee_freelance") {
                $bq->where("transaksi.seller_id", $this->session->login['id']);
            }
            if (sizeof($sesionReplacer) > 0) {
                $cab = isset($sesionReplacer['cabang_id']) ? $sesionReplacer['cabang_id'] : null;
                $gud = isset($sesionReplacer['gudang_id']) ? $sesionReplacer['gudang_id'] : null;
                if (($cab !== null) && ($cab !== "")) {
                    $bq->group_start();
                    $bq->where("transaksi.cabang_id", $cab);
                    $bq->or_where("transaksi.cabang2_id", $cab);
                    $bq->group_end();
                }
                if (($gud !== null) && ($gud !== "")) {
                    $bq->group_start();
                    // Samakan dengan lookupRecentUndoneEntries_joined(): filter gudang dari transaksi (main).
                    $bq->where("transaksi.gudang_id", $gud);
                    $bq->or_where("transaksi.gudang2_id", $gud);
                    $bq->group_end();
                }
            }
            $bq->group_by("transaksi_data.sub_step_number");
            $tmpBadge = $bq->get();
            if ($tmpBadge && ($tmpBadge->num_rows() > 0)) {
                foreach ($tmpBadge->result() as $bgSpec) {
                    $stepKey = (int)$bgSpec->sub_step_number;
                    $arrayOnprogressCountGroup[$stepKey] = (int)$bgSpec->total;
                }
            }
        }
        if (!$liteItemsOnUndoneAll) {
            $stepCurrentCount = isset($arrayOnprogressCountGroup[(int)$jenisTrsub]) ? (int)$arrayOnprogressCountGroup[(int)$jenisTrsub] : 0;
            if (($maxInlineItemsOnUndoneAll > 0) && ($stepCurrentCount > $maxInlineItemsOnUndoneAll)) {
                $liteItemsOnUndoneAll = true;
            }
        }
        ...
        $tr->addFilter("next_substep_code<>''");
        $tr->addFilter("sub_step_number>0");
        $tr->addFilter("sub_step_number='" . $jenisTrsub . "'");
        $tr->addFilter("valid_qty>0");
        if ($this->session->login['employee_type'] == "employee_freelance") {
            $tr->addFilter("seller_id='" . $this->session->login['id'] . "'");
        }
...
//        showLast_query("biru");
        ...
        if (!$liteItemsOnUndoneAll && ($maxInlineItemsOnUndoneAll > 0) && (sizeof($tmpHist) > $maxInlineItemsOnUndoneAll)) {
            $liteItemsOnUndoneAll = true;
        }
        $arrayOnprogress = array();
        $arrayOnprogressGroup = array();
        ...
        $extact = "";
        $selectProcessor = "";
        // Pastikan badge step aktif selalu identik dengan jumlah isi tabel step aktif.
        $arrayOnprogressCountGroup[(int)$jenisTrsub] = sizeof($tmpHist);
        if (sizeof($tmpHist) > 0) {
            $arrTransID = array();
            $arrTransTopID = array();
            $arrIdsHist = array();
...
//                    arrPrintWebs($hist);
            ...
            $tmpReg_result = array();
            $pairRegistriesOnUndoneAll = $pairRegistries;
            if ($liteItemsOnUndoneAll) {
                $pairRegistriesOnUndoneAll = array_values(array_diff($pairRegistriesOnUndoneAll, array("items", "items2_sum", "items3_sum", "items4", "items5_sum")));
            }
            if (sizeof($pairRegistriesOnUndoneAll) > 0) {
                $selectKolom = implode(",", $pairRegistriesOnUndoneAll) . ", transaksi_id";
                $trReg = new MdlTransaksi();
                $trReg->setFilters(array());
                //            $trReg->addFilter("param='main'");
                $trReg->setJointSelectFields($selectKolom);
...
//            arrprint($arrIdsHistByTrans);
            ...
            foreach ($tmpHist as $row) {
                ...
                if ($pakai_ini == 1) {
                    ...
                    if ($liteItemsOnUndoneAll) {
                        $detailLink = MODUL_PATH . "Transaksi/viewInlineItemsByTransaksiId/" . $this->jenisTr . "/" . $row->transaksi_id;
                        $tblItems = "<a class='btn btn-default btn-xs' href='javascript:void(0)' onclick=\"var box=$(this).siblings('.inline-detail-box');if(box.is(':visible')){box.slideUp(120);}else{if(box.data('loaded')!=1){box.html('<span class=\\'text-muted\\'>Loading detail...</span>');box.load('$detailLink',function(){box.data('loaded',1);});}box.slideDown(120);}return false;\">lihat detail</a>";
                        $tblItems .= "<div class='inline-detail-box' style='display:none;margin-top:2px;max-height:320px;overflow:auto;'></div>";
                        $free_produk = "";
                    }
                    else {
                        $detail = viewDetailTransaksi($tmpReg_result[$row->transaksi_id], $arrItemShow, $row->jenis_master);
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
            ...
            "arrayOnProgress" => (isset($arrayOnprogress) && sizeof($arrayOnprogress) > 0) ? $arrayOnprogress : array(),
            "arrayOnprogressGroup" => (isset($arrayOnprogressGroup) && (sizeof($arrayOnprogressGroup) > 0)) ? $arrayOnprogressGroup : array(),
            "arrayOnprogressCountGroup" => (isset($arrayOnprogressCountGroup) && (sizeof($arrayOnprogressCountGroup) > 0)) ? $arrayOnprogressCountGroup : array(),
    ...
    public function viewInlineItemsByTransaksiId()
    {
        if (!isset($this->session->login['id'])) {
            echo "<div class='alert alert-warning'>Session login tidak ditemukan.</div>";
            return;
        }

        $jenisTr = $this->uri->segment(4);
        $transaksiID = (int)$this->uri->segment(5);
        if (($transaksiID < 1) || (!isset($this->configUi[$jenisTr]))) {
            echo "<div class='alert alert-warning'>Parameter detail tidak valid.</div>";
            return;
        }
        ...
        $this->load->model("MdlTransaksi");
        $trMain = new MdlTransaksi();
        $trMain->setFilters(array());
        $trMain->addFilter("id='" . $transaksiID . "'");
        $tmpMain = $trMain->lookupAll()->result();
        if (sizeof($tmpMain) < 1) {
            echo "<div class='alert alert-warning'>Data transaksi tidak ditemukan.</div>";
            return;
        }

        $jenisMaster = isset($tmpMain[0]->jenis_master) ? $tmpMain[0]->jenis_master : $jenisTr;
        $regDecoded = array();
        if (sizeof($pairRegistries) > 0) {
            $selectKolom = implode(",", $pairRegistries) . ", transaksi_id";
            $trReg = new MdlTransaksi();
            $trReg->setFilters(array());
            $trReg->setJointSelectFields($selectKolom);
            $trReg->addFilter("transaksi_id='" . $transaksiID . "'");
            $tmpReg = $trReg->lookupDataRegistries()->result();
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $regRow) {
                    foreach ($regRow as $key_reg => $val_reg) {
                        ...
                        $regDecoded[$regRow->transaksi_id][$key_reg] = blobDecode($val_reg);
                    }
                }
            }
        }

        if (!isset($regDecoded[$transaksiID])) {
            echo "<div class='alert alert-warning'>Detail item tidak ditemukan.</div>";
            return;
        }
        if (!isset($regDecoded[$transaksiID]["items"]) || !is_array($regDecoded[$transaksiID]["items"])) {
            echo "<div class='alert alert-warning'>Detail item belum tersedia.</div>";
            return;
        }

        $html = viewDetailTransaksi($regDecoded[$transaksiID], $arrItemShow, $jenisMaster);
        echo $html;
    }
