        ...
        foreach ($items as $iSpec) {
            ...
            if ($showItems) {
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    if (in_array($key, $editableFieldstr)) {
...
//                        $queryParams .= "&$key='+removeCommasStr(document.getElementById('" . $colID[$key] . "').value)+'";
                    }
                    if ($listMode[$key] == "string") {
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "string":
                            echo "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control text-right' style='color:$color;' value='" . $fieldVal . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
                    ...
                    if (isset($noteType)) {
                        switch ($noteType) {
                            case "textarea":
                                ...
                                onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $noteRecorder . "?val='+encodeURIComponent(this.value)+'&iid=$iID&idx=$xID';}\"
                                onmouseout=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $noteRecorder . "?val='+encodeURIComponent(this.value)+'&iid=$iID&idx=$xID';}\"
                                >$noteVal</textarea>";
                                break;
                            case "text":
                            default:
...
//                arrPrint($addRowHiddens[$k]);
        ...
        if (isset($continue_shoppingcart) && ($continue_shoppingcart == false)) {
            if (isset($continue_shoppingcart_label)) {
                echo "<div class='alert alert-danger' style='margin-top: 10px;font-size: 15px;'>";
                echo "<span>$continue_shoppingcart_label</span>";
                echo "</div>";
            }
        }
