...
//            $strOnprogFooter = "<a class='btn btn-default' href='" . base_url() . $this->uri->segment(1) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
        ...
        $arrTags = array(
            ...
            "pihak_main_value" => $pihakMainValue,
        ...
        if (sizeof($arrayOnProgress) > 0) {
            if ($isMobile == true & $stepper == 2) {
                ...
                foreach ($arrayOnProgress as $key => $val) {
                    ...
                    $strOnprogMbl .= "<p>" . $val['nomer_po'] . "</p>";
                    $strOnprogMbl .= "<p>" . $val['suppliers_nama'] . "</p>";
                    $strOnprogMbl .= "</div>";
...
//                $strOnprogFooter = "";
        ...
        if ($isMobile == true & $stepper == 2) {
            ...
            $str .= "<div style='float: right;font-size: 1.7em;margin-right: 10px;'><a href='JavaScript:void(0);' onclick=\"getScan();\" ><i class='fa fa-qrcode'></i></a></div>";
            $str .= "</div>";
            $str .= $strOnprogMbl;
        }
        echo $str;
...
//        }
        ...
        if (sizeof($arrPihakData) > 0) {
//            cekHere(count($arrPihakData));
            asort($arrPihakData);
            $link = $linkDataSelected . "?sid=";
            $strOnprog .= "<span>Pilih Supplier: </span>";
            $strOnprog .= "<select class='sselectpicker btn btn-box-tool btn-warning' onchange=\"$('#klaimList').load('$link'+this.value);\">";
            $strOnprog .= "<option class='' value='0' style='color:#ffffff;'>--- pilih supplier ---</option>";
            foreach ($arrPihakData as $sid => $snama) {
                $selected = ($sid == $pihakDataSelected) ? "selected" : "";
                $strOnprog .= "<option class='' style='color:#ffffff;' value='$sid' $selected> $snama </option>";
            }
            $strOnprog .= "</select>";
        }
        $strOnprog .= "</div>";
        ...
        if (isset($arrPihakData) && (count($arrPihakData) > 0) && (count($arrPihakData) <= $addTabHeaderLimit)) {
//        if (isset($arrPihakData) && (count($arrPihakData) > 0)) {
            $arrPihakData[0] = "ALL";
            ksort($arrPihakData);
            $content = "<ul class='nav nav-tabs'>";
            foreach ($arrPihakData as $step => $label) {
                $color = $step == $pihakDataSelected ? "#454549" : "#999999";
                $borderColor = $step == $pihakDataSelected ? "#cccccc" : "#ffffff";
                $bgColor = $step == $pihakDataSelected ? "#ffffff" : "#f0f0f0";
                $linkStep = $linkDataSelected . "?sid=$step";
                $content .= "<li class='nav-item'>";
//                $varUrl = $step == $pihakDataSelected ? "javascript:void(0);" : "if($('#klaimList>div.box-body')){ $('#klaimList>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#klaimList').load('$linkStep'); }else{ console.log('tidak ada container klaimList'); }";
                $varUrl = $step == $pihakDataSelected ? "javascript:void(0);" : "if($('#klaimList>div.box-body')){ $('#klaimList').load('$linkStep'); }else{ console.log('tidak ada container klaimList'); }";
                ...
                foreach ($arrayOnProgress as $key => $val) {
                    ...
                    if (sizeof($arrayProgressLabels) > 0) {
                        foreach ($arrayProgressLabels as $key => $label) {
                            ...
                            $add_label = "";
                            $klaim_label = "";
                            if (isset($historySumFields[$key])) {
                                $key_alias = $key . "_status";
                                $key_alias_val = $val[$key_alias];
                                $key_alias_label = $labelAlias[$key_alias_val];
                                $add_label = "<span class='meta'>$key_alias_label</span>";
                                $nilai_val = ($key_alias_val == 2) ? "$add_label" : "$nilai_val<br>$add_label";
//                                cekHere("[$key] [$key_alias] [$key_alias_val] [$key_alias_label] [$add_label]");
                            }
                            if($key == "nilai_total_diklaim"){
                                $klaim_label = "<br>" . $val[$key."_label"];
                            }
                            $strOnprog .= "<td>";
                            $strOnprog .= $nilai_val . $klaim_label;
                            $strOnprog .= "</td>";
                        }
                    }
                    $strOnprog .= "</tr>";
                ...
                if (isset($arraySumField) && sizeof($arraySumField) > 0) {
                    $strOnprog .= "<tfoot>";
                    $strOnprog .= "<tr line=" . __LINE__ . ">";
                    $strOnprog .= "<th>-</th>";
//
...
//                    }
                    if (sizeof($arrayProgressLabels) > 0) {
                        foreach ($arrayProgressLabels as $key => $label) {
                            $strOnprog .= "<th>";
                            if (isset($arraySumField) && isset($arraySumField[$key])) {
                                $strOnprog .= formatField_he_format($key, $arraySumField[$key]);
                            }
                            else {
                                $strOnprog .= "-";
                            }
