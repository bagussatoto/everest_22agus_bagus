...
//                                    $receiptDetailFields = unserialize(base64_decode($row->$param));
                                ...
                                case "rsltItems":
                                    ...
                                    break;
                                case "items6_sum":
                                    $items6_sum = unserialize(base64_decode($row->$param));
                                    break;
                            }
                        }
                    }
            ...
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582")) {
                ...
                if ($main["pihakMainName"] == "diskon_lain") {
                    $tmp = array();
//                    arrprint($itemLabels);
                    foreach ($itemsRegistries as $ix => $items) {
                        if (sizeof($itemLabels) > 0) {
                            foreach ($itemLabels as $key => $val) {
                                if (strpos($key, '+') !== false) {//==mengandung penggabungan (+)
                                    $chars = explode("+", $key);
                                    $colValue = "";
                                    foreach ($chars as $key2) {
                                        $colValue .= isset($items[$key2]) ? $items[$key2] . "<br>" : "";
                                    }
                                }
                                else {
                                    $colValue = isset($items[$key]) ? $items[$key] : "";
                                }
                                $tmp[$key] = $colValue;
                                if (!isset($childTableInValueParams[$ix][$key])) {
                                    $childTableInValueParams[$ix][$key] = $colValue;
                                }
                            }
                            $tmp["produk_nama"] = $items["nama"];
                            $tmp["produk_ord_jml"] = $items["qty"];
                            foreach ($itemNumLabels as $k => $kLabel) {
                                $tmp[$k] = $items[$k];
                            }
                            foreach ($itemSubTotal as $kk => $vv) {
                                $tmp[$kk] = $items[$kk];
                            }
                        ...
                        if ($subAmountConfig != null) {
                            $subtotal = makeValue($subAmountConfig, $childTableInValueParams[$ix], $childTableInValueParams[$ix], 0);
                        }
                        else {
                            $subtotal = 0;
                        }
...
//        arrPrintKuning($globalVars);
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number]) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number] == true) {
            unset($itemLabels['subtotal']);
            unset($itemLabels2['subtotal']);
            unset($itemLabels['sub_nett1']);
            unset($itemLabels2['sub_nett1']);
            unset($itemLabels['sub_harga']);
            unset($itemLabels2['sub_harga']);
//            cekHere("hidden");
        }
        $zeroAllowed = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFieldsZeroAllowed'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFieldsZeroAllowed'][$row->step_number] : array();
...
//                    cekHere(":: $gerbang :: [$ii] $key_data_number ::");
//                    arrPrint($sdata);
        ...
        //--------------------
        $receiptSwictItems = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptSwictItems']) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptSwictItems'] : false;
        if($receiptSwictItems == true){
            $items = $itemsRegistries;
        }
        $shoppingCartUnsetHeader = isset($this->configUi[$tmpTr[0]->jenis_master]['shoppingCartUnsetHeader'][$main["pihakMainReferenceJenis"]]) ? $this->configUi[$tmpTr[0]->jenis_master]['shoppingCartUnsetHeader'][$main["pihakMainReferenceJenis"]] : array();
