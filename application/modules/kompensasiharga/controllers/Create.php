...
//                arrPrint($selectorProcessor);
        ...
        if (sizeof($sessionBackSave) > 0) {
            if (isset($sessionBackSave["enabled"]) && ($sessionBackSave["enabled"] == true)) {
                ...
                arrPrintHitam($_SESSION[$gateName]);
                if (isset($_SESSION[$gateName]) && (sizeof($_SESSION[$gateName]) > 0)) {
                    foreach ($_SESSION[$gateName] as $key => $val) {
                        $_SESSION[$cCode]["main"][$key] = $val;
                    }
                }
                $gate = $sessionBackSave["gate"];
                foreach ($gate as $key => $val) {
                    $_SESSION[$gateName][$key] = isset($_SESSION[$cCode]["main"][$val]) ? $_SESSION[$cCode]["main"][$val] : "";
                }
                $pihakID = isset($_SESSION[$gateName]["pihakID"]) ? $_SESSION[$gateName]["pihakID"] : 0;
                $pihakMainID = isset($_SESSION[$gateName]["pihakMainID"]) ? $_SESSION[$gateName]["pihakMainID"] : 0;
            }
        }
        //-----------------------------
        $pihakID = isset($_SESSION[$cCode]["main"]["pihakID"]) ? $_SESSION[$cCode]["main"]["pihakID"] : $pihakID;
        $pihakMainID = isset($_SESSION[$cCode]["main"]["pihakMainID"]) ? $_SESSION[$cCode]["main"]["pihakMainID"] : $pihakMainID;
            ...
            if ($pihakMainID > 0) {
                ...
                if (sizeof($loaderTrigger) > 0) {
                    if (isset($loaderTrigger["enabled"]) && ($loaderTrigger["enabled"] == true)) {
                        $selectorSrcModel = isset($this->configUi[$this->jenisTr]['selectorModel']) ? $this->configUi[$this->jenisTr]['selectorModel'] : NULL;
                        $modelFilter = isset($this->configUi[$this->jenisTr]['selectorFilters']) ? $this->configUi[$this->jenisTr]['selectorFilters'] : array();
                        if ($selectorSrcModel != NULL) {
                            $this->load->model("Mdls/" . $selectorSrcModel);
                            $b = new $selectorSrcModel();
                            if (sizeof($modelFilter) > 0) {
                                makeFilter($modelFilter, $_SESSION[$cCode]["main"], $b);
                            }
                            $tmpO = $b->lookupAll()->result();
                            if (sizeof($tmpO) > 0) {
                                ...
                                foreach ($arrUnset as $set) {
                                    unset($_SESSION[$cCode]["main"][$set]);
                                }
                            }
                        }
                    }
...
//            "pihakMainLabel" => (isset($_SESSION[$cCode]["main"]["pihakMainName"])) ? $_SESSION[$cCode]["main"]["pihakMainName"] : isset($this->configUi[$jenisTr]["pihakMainLabel"]) ? $this->configUi[$jenisTr]["pihakMainLabel"] : "",
            "pihakMainLabel" => $this->configUi[$jenisTr]["pihakMainLabel"],
            "pihakMainValue" => (isset($_SESSION[$cCode]["main"]["pihakMainName"])) ? $_SESSION[$cCode]["main"]["pihakMainName"] : "",
...
//            "pihakExternLabel" => isset($_SESSION[$cCode]['main']['pihakExternLabel']) ? $_SESSION[$cCode]['main']['pihakExternName'] . "(" . $this->configUi[$jenisTr]["pihakExternLabel"] . ")" : isset($this->configUi[$jenisTr]["pihakExternLabel"]) ? $this->configUi[$jenisTr]["pihakExternLabel"] : "",
    ...
    public function preview()
    {
        ...
        $itemSrcLabels = $itemSrcLabels + $itemSrcNumLabels;
        $itemLabels = (sizeof($itemLabels) > 0) ? $itemLabels + array("subtotal" => "sub-amount") : array();
        $itemLabels2 = (sizeof($itemLabels2) > 0) ? $itemLabels2 + array("subtotal" => "sub-amount") : array();
        $itemLabels3 = (sizeof($itemLabels3) > 0) ? $itemLabels3 + array("subtotal" => "sub-amount") : array();
        $itemSrcLabels = (sizeof($itemSrcLabels) > 0) ? $itemSrcLabels + array("subtotal" => "sub-amount") : array();
        ...
        $data = array(
            ...
            "shopingCartEmbededTaxItem" => isset($shopingCartEmbededTaxItem) ? $shopingCartEmbededTaxItem : array(),
        );
...
//            matihere();
            ...
            if (isset($_SESSION[$cCode]['items3_sum']) && count($_SESSION[$cCode]['items3_sum']) > 0) {
                foreach ($_SESSION[$cCode]['items3_sum'] as $ii => $iiItems3) {
                    if (isset($cContent["stepCode"][$jenisTrTarget])) {
                        $_SESSION[$cCode]['items3_sum'][$ii]["curent_transaksi_count"] = $cContent["stepCode"][$jenisTrTarget];
                    }
//                    foreach($cContent[""])
//                    arrPrint($iiItems3);
                }
                ...
                if (sizeof($payConfigs) > 0) {
                    foreach ($payConfigs[1] as $paymentSrcConfig) {
                        ...
                        if ($paymentMethod == "update") {
                            ...
                            if (($_SESSION[$cCode]['main'][$valueSrc]) > 10) {
...
//            $this->lastValidate($insertID);
            ...
            if ($_SERVER['REMOTE_ADDR'] == "202.65.117.72") {
                ...
                mati_disini("LINE: " . __LINE__ . " REALISASI KLAIM DISKON sementara tidak bisa digunakan (maintenance), tunggu beberapa saat lagi yaa.., TRID: $insertID");
            }
//            mati_disini("LINE: " . __LINE__ . " TRANSAKSI BERHASIL, REALISASI KLAIM DISKON sementara sedang maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
            ...
            //region connecting ppn masukan belum faktur
...
//                        cekkuning("detecting printing size");
    ...
    public function infoDiskon()
    {
        ...
        * -----------------------------------------------------*/
        ...
        $strDiskon = $dp->cekDiskonPembelianSaldo($sesItems, $cabang_id, $toko_id);
        // print_r($strDiskon);
        echo $strDiskon['html'];

        // $strDiskonKategori = $dp->cekDiskonProdukKategori($sesItems);
        // echo $strDiskonKategori['html'];
