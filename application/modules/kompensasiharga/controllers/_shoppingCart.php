...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        $editableFieldstr = isset($this->configUi[$this->jenisTr]['editableFieldstr'][$stepNumber]) ? $this->configUi[$this->jenisTr]['editableFieldstr'][$stepNumber] : array();
        ...
        $shoppingCartKlaimDiskonInfo = isset($this->configUi[$this->jenisTr]['shoppingCartKlaimDiskonInfo']) ? $this->configUi[$this->jenisTr]['shoppingCartKlaimDiskonInfo'] : array();
        //----------------------------------------------
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $idx => $iSpec) {
                    $pihakID = $id;
                    $id = $iSpec["id"];
                    $no++;
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "referensi_item_supplier" => isset($iSpec['referensi_item_supplier']) ? $iSpec['referensi_item_supplier'] : "",
                        "id_x" => isset($iSpec['id_x']) ? $iSpec['id_x'] : "",
                    );
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pembelian/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?idx=$idx&id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pembelian/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?idx=$idx&id=" . $id;
//arrPrintPink($tmp);
                    $items[] = $tmp;
...
//                            $main[$key] += isset($iSpec[$key]) ? ($iSpec['jml'] * $iSpec[$key]) : 0;
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartSubamount2']) && $this->configUi[$this->jenisTr]['shoppingCartSubamount2'][$stepNumber] == true) {
            ...
            $itemLabels = (sizeof($itemLabels) > 0) ? $itemLabels + array("subtotal" => "subtotal") : array();
            $itemLabels2 = (sizeof($itemLabels2) > 0) ? $itemLabels2 + array("subtotal" => "subtotal") : array();
            $itemLabels3 = (sizeof($itemLabels3) > 0) ? $itemLabels3 + array("subtotal" => "subtotal") : array();
        }
...
//                                $initValueKey = isset($eSpec['defaultValueKey']) ? $eSpec['defaultValueKey'] : NULL;
//                                if($initValueKey != NULL){
//                                    $defValueSrc = makeValue($eSpec['defaultValue'], $_SESSION[$cCode][$initValueKey][$eSpec['defaultValue']], $_SESSION[$cCode][$initValueKey][$eSpec['defaultValue']], 0);
//                                    cekHitam("[$initValue] [$initValueKey] || " . $eSpec['defaultValue']);
//                                }
//                                else{
//                                    $defValueSrc = isset($eSpec['defaultValue']) ? $eSpec['defaultValue'] : "";
//                                    cekHere("[$initValue] [$initValueKey] || " . $eSpec['defaultValue']);
//                                }
...
//                                        $keyupStr .= "if(parseInt(removeCommas(this.value))>$maxValue){this.value='" . number_format($origDefValue) . "';this.select();}else{if(parseInt(removeCommas(this.value))>0){this.value=addCommas(this.value)}else{this.value=0}}";
                                        $keyupStr .= "if(parseInt(removeCommas(this.value))>$maxValue){this.value='" . $origDefValue . "';this.select();}else{if(parseInt((this.value))>0){this.value=(this.value)}else{this.value=0}}";
                                    }
                                    $keyupStr .= $keyupAct;
                                    ...
                                    //                                    $defVal = $origDefValue;
                                    $addRows[$oValueName] = "<input " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " type=text autocomplete='off' id='$oValueName' 
                                    class='form-control text-right' style='font-size:17px;' $disabled placeholder='$oValueName' value='" . $defVal . "' $minValStr $maxValStr
...
//                        $initValueKey = isset($eSpec['defaultValueKey']) ? $eSpec['defaultValueKey'] : NULL;
//                        if($initValueKey != NULL){
//                            $initValue = makeValue($eSpec['defaultValue'], $_SESSION[$cCode][$initValueKey][$eSpec['defaultValue']], $_SESSION[$cCode][$initValueKey][$eSpec['defaultValue']], 0);
//                            cekHitam("[$initValue] [$initValueKey] || " . $eSpec['defaultValue']);
//                        }
//                        else{
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
                            ...
                            case "number":
                                ...
                                $elStr[$eName] .= "<input " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " type=text class='form-control' value='$defaultValue' 
                                onfocus='this.select()' $maxValStr $maxValValidator 
                                oonclick=\"this.value='$defaultValue';\"
                                onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
                                break;
                            case "date":
...
//            cekHijau($pernyataanNote);
        ...
        //-----------------------------
        if (isset($shoppingCartKlaimDiskonInfo) && (sizeof($shoppingCartKlaimDiskonInfo) > 0)) {
            $label_replacer = $shoppingCartKlaimDiskonInfo["replacer"];
            $label_info = $shoppingCartKlaimDiskonInfo["label_info"];
        ...
        //-----------------------------
        $shoppingCartUnsetHeader = isset($this->configUi[$this->jenisTr]['shoppingCartUnsetHeader'][$_SESSION[$cCode]["main"]["pihakMainReferenceJenis"]]) ? $this->configUi[$this->jenisTr]['shoppingCartUnsetHeader'][$_SESSION[$cCode]["main"]["pihakMainReferenceJenis"]] : array();
        $klaimValidator = isset($this->configUi[$this->jenisTr]['klaimValidator']) ? $this->configUi[$this->jenisTr]['klaimValidator'] : array();
        if (sizeof($shoppingCartUnsetHeader) > 0) {
            foreach ($shoppingCartUnsetHeader as $vUnset) {
                if (array_key_exists($vUnset, $itemLabels)) {
                    unset($itemLabels[$vUnset]);
                }
            }
        }
        //-----------------------------

        cekHere("MAKSIMAL KLAIM: " . $_SESSION[$cCode]["main"]["diskon_total_maksimal"]);
        if (sizeof($klaimValidator) > 0) {
            if (isset($klaimValidator["enabled"]) && ($klaimValidator["enabled"] == true)) {
                $source = $klaimValidator["source"];
                $target = $klaimValidator["target"];
                $label = $klaimValidator["label"];
                $diskon_total_maksimal = isset($_SESSION[$cCode]["main"][$target]) ? $_SESSION[$cCode]["main"][$target] : 0;
                $diskon_total_klaim = isset($_SESSION[$cCode]["main"][$source]) ? $_SESSION[$cCode]["main"][$source] : 0;
                $diskon_selisih = $diskon_total_klaim - $diskon_total_maksimal;
//                if ($diskon_total_klaim > $diskon_total_maksimal) {
                if ($diskon_selisih > 100) {
                    $continue_shoppingcart = false;
                    $continue_shoppingcart_label = $label;
                }
            }
        }
        ...
        if (count($shopingCartFaktur) > 0) {
            if ($_SESSION[$cCode]["main"]["produk_rel_harga_dpp"] == 0) {
...
//                    $dataFaktur[$key_faktur] = isset($_SESSION[$cCode]["main"][$key_faktur]) ? round($_SESSION[$cCode]["main"][$key_faktur]) : "";
                    if(isset($_SESSION[$cCode]["main"][$key_faktur])){
                        if(is_numeric($_SESSION[$cCode]["main"][$key_faktur])){
                            $dataFaktur[$key_faktur] = round($_SESSION[$cCode]["main"][$key_faktur]);
                        }
                        else{
                            $dataFaktur[$key_faktur] = $_SESSION[$cCode]["main"][$key_faktur];
                        }
                    }
                }
...
//                        $_SESSION[$cCode][$gateTarget][0] = $dataFaktur;
            ...
            foreach ($arrRound as $ii){
                $_SESSION[$cCode]["main"][$ii] = round($_SESSION[$cCode]["main"][$ii]);
            }
...
//            "sumRows2" => $sumRows2,
            ...
            "editableFieldstr" => $editableFieldstr,
            ...
            //--------------
            "continue_shoppingcart" => isset($continue_shoppingcart) ? $continue_shoppingcart : true,
            "continue_shoppingcart_label" => isset($continue_shoppingcart_label) ? $continue_shoppingcart_label : NULL,
 ...
 * ---------------------------*/ // arrPrintPink($_SESSION[$cCode]);
    ...
    public function recordItemColumn()
    {
        ...
        if (isset($_SESSION[$cCode]["main"]["pihakMainName"]) && $_SESSION[$cCode]["main"]["pihakMainName"] == "diskon_lain") {
            $id = $iID = $_GET['idx'];
        }
        else {
            $id = $iID = $_GET['iid'];
