...
class _processSelectNota extends Modul_Controller
{
    ...
    public function select_OLD()
    {
        ...
        if (!array_key_exists($id, $_SESSION[$cCode]['items'])) {
            cekMerah("MASUK akan memasukkan ITEMS");
            $tmp = array(
...
//mati_disini(__LINE__);
    ...
    public function remove()
    {
        ...
        if (isset($_SESSION[$cCode]['main']['refIDs'][$id_reference])) {
            $_SESSION[$cCode]['main']['refIDs'][$id_reference] = null;
            unset($_SESSION[$cCode]['main']['refIDs'][$id_reference]);
        }
...
//                            cekHere("hasilnya $key -> " . $tmp[$key]);
    ...
    public function select()
    {
        ...
        $getID = $_GET['id'];
//        $id = $_GET['transaksi_id'];
        $id = $transaksi_id = $_GET['transaksi_id'];
//        $id = $_GET['id'];
        //reset dulu semua items
        ...
        $referenceGateFields = isset($this->configUi[$this->jenisTr]['referenceGateFields']) ? $this->configUi[$this->jenisTr]['referenceGateFields'] : array();
        //---------------------------------------
        $selectorModel = isset($this->configUi[$this->jenisTr]['selectorModel']) ? $this->configUi[$this->jenisTr]['selectorModel'] : "MdlProduk";
        $selectorSrcModel = isset($this->configUi[$this->jenisTr]['selectorSrcModel']) ? $this->configUi[$this->jenisTr]['selectorSrcModel'] : "MdlProduk";
        $selectorParamFields = isset($this->configUi[$this->jenisTr]['selectorParamFields']) ? $this->configUi[$this->jenisTr]['selectorParamFields'] : array();
        //---------------------------------------
...
//        arrprint($_GET);
        ...
        $initMain = array(
            "pihakID" => isset($_GET['extern_id']) ? $_GET['extern_id'] : $_SESSION[$cCode]["main"]["pihakID"],
            "pihakName" => isset($_GET['extern_nama']) ? $_GET['extern_nama'] : $_SESSION[$cCode]["main"]["pihakName"],
            "valasDetails" => isset($_GET['valas_id']) ? $_GET['valas_id'] : "",
            "pihakMainName" => isset($_GET['extern_label2']) && sizeof($_GET['extern_label2']) > 0 ? $_GET['extern_label2'] : $_SESSION[$cCode]["main"]["pihakMainName"],
            "other" => isset($_GET['extern_nilai4']) && sizeof($_GET['extern_nilai4']) > 0 ? $_GET['extern_nilai4'] : "",
            "relativeComName" => sizeof($relativeComNameDetails) > 0 ? $relativeComNameDetails[$_GET['extern_label2']] : "",
            "pph_23" => isset($_GET['pph_23']) ? $_GET['pph_23'] : 0,
            "terbayar_pph23" => isset($_GET['terbayar_pph23']) ? $_GET['terbayar_pph23'] : 0,
        ...
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $componentAssConfig = isset($this->configUi[$this->jenisTr]['componentsAss']) ? $this->configUi[$this->jenisTr]['componentsAss'] : array();
        $selectorToMain = isset($this->configUi[$this->jenisTr]['selectorToMain']) ? $this->configUi[$this->jenisTr]['selectorToMain'] : array();
...
//                    cekHere("$relativeComNameDetails[$key] " . $relativeComNameDetails[$key]);
            ...
            if (sizeof($selectorParamFields) > 0) {
                $this->load->model("Mdls/" . $selectorModel);
                $o = new $selectorModel();
                $tmpO = $o->lookupByID($getID)->result();
//                showLast_query("kuning");
                if (sizeof($tmpO) > 0) {
                    foreach ($tmpO as $row) {
                        ...
                        foreach ($fieldSrcs as $key => $src) {
                            $tmp[$key] = makeValue($src, (array)$row, (array)$row, 0);
                        }

                        if (sizeof($selectorToMain) > 0) {
                            if (isset($selectorToMain["enabled"]) && ($selectorToMain["enabled"] == true)) {
                                foreach ($selectorToMain["gate"] as $akey => $aval) {
                                    $_SESSION[$cCode]['main'][$akey] = isset($row->$aval) ? $row->$aval : "";
                                    $tmp[$akey] = isset($row->$aval) ? $row->$aval : "";
                                }
                            }
                        }
            ...
            $tmp["handler"] = $this->uri->segment(1) . "/" . $this->uri->segment(2);
            $_SESSION[$cCode]['items'][$id] = $tmp;
        }
        else {
            cekMerah("TIDAK akan memasukkan ITEMS");
...
//            cekBiru("bulding summary item_result...");
    ...
    public function select_2()
    {
        ...
        $fieldSrcs = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc'] : array("nama" => "nama");
        $shoppingCartFieldAkumulasiSrc = isset($this->configUi[$this->jenisTr]['shoppingCartFieldAkumulasiSrc']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldAkumulasiSrc'] : array();
        $subAmountConfig = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValue'][1] : null;
        $componentAssConfig = isset($this->configUi[$this->jenisTr]['componentsAss']) ? $this->configUi[$this->jenisTr]['componentsAss'] : array();
        $relativeComNameDetails = isset($this->configUi[$this->jenisTr]['relativeComNameDetails']) ? $this->configUi[$this->jenisTr]['relativeComNameDetails'] : array();
        $isRadioSelect = isset($this->configUi[$this->jenisTr]['isPaymentRadioSelect']) ? $this->configUi[$this->jenisTr]['isPaymentRadioSelect'] : false;
...
//            "pihakName" => isset($_GET['extern_nama']) ? $_GET['extern_nama'] : $_SESSION[$cCode]["main"]["pihakName"],
        ...
        $componentAssConfig = isset($this->configUi[$this->jenisTr]['componentsAss']) ? $this->configUi[$this->jenisTr]['componentsAss'] : array();
        $selectorToMain = isset($this->configUi[$this->jenisTr]['selectorToMain']) ? $this->configUi[$this->jenisTr]['selectorToMain'] : array();
        $selectorProccessFilters = isset($this->configUi[$this->jenisTr]['selectorProccessFilters']) ? $this->configUi[$this->jenisTr]['selectorProccessFilters'] : array();
        $selectorToMode = isset($this->configUi[$this->jenisTr]['selectorToMode']['mode']) ? $this->configUi[$this->jenisTr]['selectorToMode']['mode'] : 1;
        $switcherKeyItems = isset($this->configUi[$this->jenisTr]['switcherKeyItems']) ? $this->configUi[$this->jenisTr]['switcherKeyItems'] : array();
...
//                    cekHere("$relativeComNameDetails[$key] " . $relativeComNameDetails[$key]);
            ...
            $this->load->model("Mdls/" . $selectorModel);
            $o = new $selectorModel();
            if (sizeof($selectorProccessFilters) > 0) {
                makeFilter($selectorProccessFilters, $_SESSION[$cCode]['main'], $o);
            }
            if ($extern_id > 0) {
                $o->addFilter("produk_id='$extern_id'");
            }
            $tmpO = $o->lookupAll()->result();
            showLast_query("kuning");
            arrPrintCyan($tmpO);
//            mati_disini(__LINE__ . " --- [$selectorToMode]");
            if (sizeof($tmpO) > 0) {
                foreach ($tmpO as $row) {
                    $row_id = $row->id;// id tabel
                    $row_extern_id = $row->extern_id;// id diskon (bila skema diskon), id_produk (diskon free produk).
                    $row_extern2_id = $row->extern2_id;// id extern2, id_produk/barang
                    $row_produk_id = $row->produk_id;// id diskon utama
                    cekHere("[$row_extern_id] [$row_extern2_id] [$row_produk_id]");
                    switch ($selectorToMode) {
                        case "2":
                            if ((sizeof($switcherKeyItems) > 0) && (isset($switcherKeyItems["key"][$row_produk_id]))) {
                                $key_header_x = $switcherKeyItems["key"][$row_produk_id];
                                $key_header = $row->$key_header_x;
                            }
                            else {
                                $key_header = $row_produk_id;
                            }

                            if (!array_key_exists($key_header, $_SESSION[$cCode]['items'])) {
                                $tmp = array(
                                    "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                    "id" => $row_id,
                                    "refID" => $transaksi_id,
                                ...
                                $tmp["handler"] = $this->uri->segment(1) . "/" . $this->uri->segment(2);
                                $_SESSION[$cCode]['items'][$key_header] = $tmp;
                                $_SESSION[$cCode]["items"][$key_header]["diskon_supplier_nilai_disc"] = 0;
                                $_SESSION[$cCode]["items"][$key_header]["diskon_supplier_nilai_netto"] = $_SESSION[$cCode]['items'][$key_header]["diskon_supplier_nilai"];

                            }
                            else {
                                if (sizeof($shoppingCartFieldAkumulasiSrc) > 0) {
                                    foreach ($shoppingCartFieldAkumulasiSrc as $key => $src) {
                                        $nilai = makeValue($src, (array)$row, (array)$row, 0);
                                        $_SESSION[$cCode]['items'][$key_header][$key] += $nilai;
                                    }
                                }
                            }

                            // build items2, isi rincian dari per-diskon
                            if (!isset($_SESSION[$cCode]["items2"][$key_header])) {
                                $_SESSION[$cCode]["items2"][$key_header] = array();
                            }
                            if (!array_key_exists($row_id, $_SESSION[$cCode]["items2"][$key_header])) {
                                $tmp2 = array(
                                    "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                    "id" => $row_id,
                                    "refID" => $transaksi_id,
                                ...
                                foreach ($fieldSrcs as $key => $src) {
                                    $tmp2[$key] = makeValue($src, (array)$row, (array)$row, 0);
                                }
                                foreach ($selectorParamFields as $key => $src) {
                                    $tmp2[$key] = isset($row->$src) && $row->$src != "" ? $row->$src : "$key - null ";
                                }
                                $_SESSION[$cCode]["items2"][$key_header][$row_id] = $tmp2;
                            }

                            break;
                        default:
                            if (!array_key_exists($row_id, $_SESSION[$cCode]['items'])) {
                                $tmp = array(
                                    "handler" => $this->uri->segment(1) . "/" . $this->uri->segment(2),
                                    "id" => $row_id,
                                    "refID" => $transaksi_id,
                                ...
                                $tmp["handler"] = $this->uri->segment(1) . "/" . $this->uri->segment(2);
                                $_SESSION[$cCode]['items'][$row_id] = $tmp;
                            }
                            else {
