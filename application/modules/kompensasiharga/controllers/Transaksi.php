...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $ppnCekValidate = isset($this->configUi[$this->jenisTr]["ppnCekValidate"]) ? $this->configUi[$this->jenisTr]["ppnCekValidate"] : array();
        ...
        $kompensasiMethodValidator = isset($this->configUi[$this->jenisTr]["kompensasiMethodValidator"]) ? $this->configUi[$this->jenisTr]["kompensasiMethodValidator"] : array();
...
//                            }
        ...
        //----------------------
        if (sizeof($ppnCekValidate) > 0) {
            foreach ($ppnCekValidate as $key => $label) {
        ...
        //----------------------
        if (sizeof($kompensasiMethodValidator) > 0) {
            $cek = $_SESSION[$cCode]["main"]["kompensasiMethod"];
            if (isset($kompensasiMethodValidator[$cek]) && (sizeof($kompensasiMethodValidator[$cek]) > 0)) {
                if (isset($kompensasiMethodValidator[$cek]["enabled"]) && ($kompensasiMethodValidator[$cek]["enabled"] == true)) {
                    foreach ($kompensasiMethodValidator[$cek]["kolom"] as $kolom_cek => $kolom_label) {
                        if (!isset($_SESSION[$cCode]['main'][$kolom_cek])) {
                            $errMsgs[] = "$kolom_label";
                        }
                        elseif ($_SESSION[$cCode]['main'][$kolom_cek] == NULL) {
                            $errMsgs[] = "$kolom_label";
                        }
                        elseif ($_SESSION[$cCode]['main'][$kolom_cek] == 0) {
                            $errMsgs[] = "$kolom_label";
                        }
                    }
                }
...
//                        }
    ...
    public function viewUndoneItemsIndex()
    {
        ...
        if ($isMob) {
            // dimatikan klo hidup data yg dimobil terbatas
            // $historyFields = isset($this->configUi[$jenisTr]['compactHistoryFields']) ? $this->configUi[$jenisTr]['compactHistoryFields'] : array();
        }
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            foreach ($tmpHist as $row) {
                ...
                if ($allowFollowup) {
                    ...
                    //                        }
                    /*-----------------print QrCode---------------------*/
                    if ($allowPrintQr == true) {
                        $link_qr = base_url() . "addons/Qr/doPrint?jn=$jenisTr&mid=" . $row->id_master . "&FromTransaksi=" . $row->transaksi_id;
                        $tmp['action'] .= "<button type='button' class='btn btn-info btn-block hidden-xs hidden-sm' onclick=\"popBig('$link_qr');\"><i class='fa fa-qrcode'></i> Print QR</button>";
                    }
                    $tmp['action'] .= "</div class='input-group'>";
...
//                        }
    ...
    public function viewRequestItems()
    {
        ...
        if (isset($this->configUi[$this->uri->segment(4)]['tabRequestCode'])) {
            ...
            $reqFormTarget = MODUL_PATH . "Create/swapFrom/" . $this->uri->segment(4);
...
//                $tmpByReq = $tr->lookupRecentHistories()->result();
                ...
                $tr->addFilter("transaksi_data.valid_qty>0");
                $tmpByReq = $tr->lookupJoined_OLD()->result();
            }
            else {
                $tr->addFilter("transaksi_data.next_substep_code='" . $masterRefCode . "'");
                $tr->addFilter("transaksi_data.produk_jenis='supplies'");
                        ...
                        foreach ($tabHistoryFields as $key => $row) {
                            ...
                            switch ($key) {
                                ...
                                case "transaksi_id":
                                    ...
                                    $produk_btn = "<span href='" . MODUL_PATH . "ViewDetails/nomer/" . $this->jenisTr . "/" . $rows->nomer . "' name='qtips' class='btn btn-sm'> berisi " . sizeof($sumPrdOrdJmlTR[$rows->transaksi_id]) . " item, Total " . $totalSumPrdOrdJmlTR . " unit  </span>";
                                    $clickEvent = "";
...
//                    $dTmp = $m->lookUpDetailTransaksi($transaksiTopID)->result();//error
...
//                        $itemsNotApprove = $items_reg;
        ...
        //--------------------
        $shopingCartFaktur = isset($this->configUi[$jenisMasterTrans]['addMainSource'][1]) ? $this->configUi[$jenisMasterTrans]['addMainSource'][1] : array();
        if (count($shopingCartFaktur) > 0) {
            if (isset($shopingCartFaktur["gateTarget"])) {
                $gateTarget = $shopingCartFaktur["gateTarget"];
            }
        }
//        cekHitam($jenisMasterTrans);
//        arrPrintHitam($shopingCartFaktur);
        //--------------------
        $data = array(
            "mode" => $this->uri->segment(3),
            "title" => $no,
            ...
            //---------------------
            "formulirFaktur" => isset($items6_sum_reg) ? $items6_sum_reg : array(),
            "cloneFormulirFaktur" => "",
            "cloneFormulirFakturDelete" => "",
...
//                        "kode" => $row->produk_kode,
                        "label" => strlen($row->produk_label) > 2 ? $row->produk_label : $row->produk_nama,
                        "kode" => strlen($row->produk_kode) > 2 ? $row->produk_kode : "",
            ...
            // arrPrint($mainDataOutstandingItems);
...
//arrPrint($headerFieldLabel);
    ...
    public function index()
    {
        ...
        $link_klaimList = MODUL_PATH . "Transaksi/viewKlaimSupplierIndex/$jenisTr/?gr=cGVtYmVsaWFu&ohyes=ohno&step=$getStep";
        $link_historyList = MODUL_PATH . "History/showData/$jenisTr/?limit=10";
        $link_stokLimit = base_url() . "Bi/showStokLimit/$jenisTr/?limit=10";
        $scriptBottom = "";
        $scriptBottom .= "<script>
...
\n\n
                    var viewhistoryList = function () {
                     ...
                     var viewklaimList = function () {
                        top.$('#klaimList').html(\" <div class='row' style='margin-top:14px;padding-bottom:14px;'><span class='col-lg-12 text-center'><img width='5%' src='$baseUrl/assets/images/loading_16_p.gif'></span></div> \");
                        setTimeout(function () {
                              if (top.$('#klaimList').find('img').length > 0) {
                                  top.$('#klaimList').load('$link_klaimList', function(){viewklaimList})
                         }
                        },1000);
                     }\n
                     viewklaimList()\n
...
// matiHEre();
    ...
    public function viewKlaimSupplierIndex()
    {
        $jenisTrsub = isset($_GET['step']) ? $_GET['step'] : 1;
        $starttime = microtime(true);
        if (!isset($this->session->login['id'])) {
...
//        $connectTo = isset($this->configUi[$jenisTr]['connectTo']) ? $this->configUi[$jenisTr]['connectTo'] : "";
        ...
        $allowPrintQr = isset($this->configUi[$jenisTr]["steps"][$jenisTrsub]["allowPrintQr"]) ? $this->configUi[$jenisTr]["steps"][$jenisTrsub]["allowPrintQr"] : 0;
        //------------------------
        $viewKlaimSupplier = isset($this->configUi[$jenisTr]["viewKlaimSupplier"]) ? $this->configUi[$jenisTr]["viewKlaimSupplier"] : array();
        if (sizeof($viewKlaimSupplier) > 0) {
            $historyFields = isset($viewKlaimSupplier["historyFields"][1]) ? $viewKlaimSupplier["historyFields"][1] : array();
            $historySumFields = isset($viewKlaimSupplier["historySumFields"]) ? $viewKlaimSupplier["historySumFields"] : array();
            $jenisSource = isset($viewKlaimSupplier["jenisSource"]) ? $viewKlaimSupplier["jenisSource"] : array();
            $historyKlaimFields = isset($viewKlaimSupplier["historyKlaimFields"]) ? $viewKlaimSupplier["historyKlaimFields"] : array();
            $labelAlias = isset($viewKlaimSupplier["labelAlias"]) ? $viewKlaimSupplier["labelAlias"] : array();
            $kolomRegistry = isset($viewKlaimSupplier["kolom_registry"]) ? $viewKlaimSupplier["kolom_registry"] : array();
            $kolomTransaksi = isset($viewKlaimSupplier["kolom_transaksi"]) ? $viewKlaimSupplier["kolom_transaksi"] : array();
        }
        //------------------------
        $isMob = isMobile_he_misc();
        if ($isMob) {
        ...
        $stepCodes = array();
        $jmlStep = count($steps);
        if (isset($jenisSource) && (sizeof($jenisSource) > 0)) {
...
//                $tr->addFilter("id>");
            ...
            if (isset($_GET["sid"]) && ($_GET["sid"] > 0)) {
                $tr->addFilter("suppliers_id=" . $_GET["sid"]);
            }
            if (isset($kolomTransaksi) && (sizeof($kolomTransaksi) > 0)) {
                $this->db->select(implode(",", $kolomTransaksi));
            }
            $tr->addFilter("jenis in ('" . implode("','", $jenisSource) . "')");
...
//            arrPrint($tmpHist);
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            $arrNextAction = array();
            foreach ($tmpHist as $row) {
                $arrTransID[] = $row->id;// kode transaksi 467
                $arrTransTopID[] = $row->id_top;
                $arrNextAction[$row->transaksi_id] = array(
                    "next_step_num" => $row->next_substep_num,
                    "next_step_code" => $row->next_substep_code,
            ...
            //-----------------
            if (isset($viewKlaimSupplier["pairedModel"])) {
                $arrDiskonData = array();
                $arrDiskonDataByTransaksi = array();
                $mdlName = $viewKlaimSupplier["pairedModel"]["comName"];
                $filter = $viewKlaimSupplier["pairedModel"]["filter"];
                $this->load->model("Coms/$mdlName");
                $mn = New $mdlName();
                foreach ($filter as $ff) {
                    $mn->addFilter($ff);
                }
                $mn->addFilter("extern_id in ('" . implode("','", $arrTransID) . "')");
                $mnTmp = $mn->lookupAll()->result();
//                showLast_query("kuning");
                if (sizeof($mnTmp) > 0) {
                    foreach ($mnTmp as $ii => $mnSpec) {
                        $arrDiskonData[$mnSpec->extern_id][$mnSpec->extern2_id] = array(
                            "transaksi_id" => $mnSpec->extern_id,
                            "qty_debet" => $mnSpec->qty_debet,
                            "debet" => $mnSpec->debet,
                            "suppliers_id" => $mnSpec->extern3_id,
                            "suppliers_nama" => $mnSpec->extern3_nama,
                            "extern2_id" => $mnSpec->extern2_id,
                            "extern2_nama" => $mnSpec->extern2_nama,
                        );
                        if (!isset($arrDiskonDataByTransaksi[$mnSpec->extern_id]["sisa_diskon"])) {
                            $arrDiskonDataByTransaksi[$mnSpec->extern_id]["sisa_diskon"] = 0;
                        }
                        $arrDiskonDataByTransaksi[$mnSpec->extern_id]["sisa_diskon"] += $mnSpec->debet;
                    }
                }
            }
            if (isset($viewKlaimSupplier["addPairedModelLocker"])) {
                $arrDiskonLockerData = array();
                $arrDiskonLockerDataDiklaim = array();
                $mdlName = $viewKlaimSupplier["addPairedModelLocker"]["mdlName"];
                $filter = $viewKlaimSupplier["addPairedModelLocker"]["filter"];
                $this->load->model("Mdls/$mdlName");
                $mnl = New $mdlName();
                foreach ($filter as $ff) {
                    $mnl->addFilter($ff);
                }
                $mnl->addFilter("transaksi_id in ('" . implode("','", $arrTransID) . "')");
                $mnlTmp = $mnl->lookupAll()->result();
//                showLast_query("kuning");
                if (sizeof($mnlTmp) > 0) {
                    foreach ($mnlTmp as $ii => $mnSpec) {
                        if (!isset($arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_klaim"])) {
                            $arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_klaim"] = 0;
                        }
                        $arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_klaim"] += $mnSpec->nilai;
                        if (!isset($arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_diklaim"])) {
                            $arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_diklaim"] = 0;
                        }
                        $arrDiskonLockerData[$mnSpec->transaksi_id][$mnSpec->produk_id]["nilai_diklaim"] += $mnSpec->nilai_diklaim;
                        if (!isset($arrDiskonLockerDataDiklaim[$mnSpec->transaksi_id]["nilai_total_diklaim"])) {
                            $arrDiskonLockerDataDiklaim[$mnSpec->transaksi_id]["nilai_total_diklaim"] = 0;
                        }
                        $arrDiskonLockerDataDiklaim[$mnSpec->transaksi_id]["nilai_total_diklaim"] += $mnSpec->nilai_diklaim;
                    }
                }
            }
            if (isset($viewKlaimSupplier["addPairedModelKlaim"])) {
                $arrTransaksiDataKlaim = array();
                $mdlName = $viewKlaimSupplier["addPairedModelKlaim"]["mdlName"];
                $filter = $viewKlaimSupplier["addPairedModelKlaim"]["filter"];
                $mnl = New $mdlName();
                foreach ($filter as $ff) {
                    $mnl->addFilter($ff);
                }
                $mnl->addFilter("reference_id in ('" . implode("','", $arrTransID) . "')");
                $mnlTmp = $mnl->lookupAll()->result();
                if (sizeof($mnlTmp) > 0) {
                    foreach ($mnlTmp as $ii => $mnSpec) {
                        $arrTransaksiDataKlaim[$mnSpec->reference_id][$ii] = array(
                            "id" => $mnSpec->id,
                            "nomer" => $mnSpec->nomer,
                            "jenis" => $mnSpec->jenis,
                            "jenis_master" => $mnSpec->jenis_master,
                            "oleh_nama" => $mnSpec->oleh_nama,
                            "dtime" => $mnSpec->dtime,
                            "fulldate" => $mnSpec->fulldate,
                        );
                        $tridKlaim[$mnSpec->id] = $mnSpec->id;
                    }
                    $tbl = "__rek_pembantu_subpiutangsuppliertrans__1010020030";
                    $this->db->where_in("transaksi_id", $tridKlaim);
                    $query = $this->db->get($tbl)->result();
//                    showLast_query("kuning");
                    if (sizeof($query) > 0) {
                        foreach ($query as $spec) {
                            if (!isset($arrDiskonLockerDataDiklaim[$spec->extern_id]["nilai_total_diklaim_mutasi"])) {
                                $arrDiskonLockerDataDiklaim[$spec->extern_id]["nilai_total_diklaim_mutasi"] = 0;
                            }
                            $arrDiskonLockerDataDiklaim[$spec->extern_id]["nilai_total_diklaim_mutasi"] += $spec->kredit;
//                            if (!array_key_exists($spec->extern_id, $arrDiskonLockerDataDiklaim)) {
//                            }
                        }
                    }
                }
                ...
                if ($pakai_ini == 1) {
                    if (sizeof($pairRegistries) > 0) {
                        if ((sizeof($tmpReg_result) > 0) && (isset($tmpReg_result[$row->id]))) {
                            ...
                            $row->diskon_nilai_total_freeproduk = $row->diskon_nilai_total + $row->produk_rel_harga;
                        }
                    }
                }
                if (isset($arrDiskonData[$row->id]) && (sizeof($arrDiskonData[$row->id]) > 0)) {
                    foreach ($arrDiskonData[$row->id] as $diskon_id => $diskonSpec) {
                        $new_key = "diskon_" . $diskon_id . "_nilai";
                        $row->$new_key = $diskonSpec["debet"];
                        foreach ($diskonSpec as $kk => $vv) {
                            $new_kk = "rek_" . $diskon_id . "_" . $kk;
                            $row->$new_kk = $vv;
                        }
                    }
                }
                if (isset($arrDiskonLockerData[$row->id]) && (sizeof($arrDiskonLockerData[$row->id]) > 0)) {
                    foreach ($arrDiskonLockerData[$row->id] as $diskon_id => $klaimSpec) {
                        foreach ($klaimSpec as $kk => $vv) {
                            $new_kk = $kk . "_" . $diskon_id;
                            $row->$new_kk = $vv;
                        }
                    }
                }
                if (isset($arrDiskonLockerDataDiklaim[$row->id]) && (sizeof($arrDiskonLockerDataDiklaim[$row->id]) > 0)) {
                    foreach ($arrDiskonLockerDataDiklaim[$row->id] as $kky => $vval) {
                        $row->$kky = $vval;
                    }
                    if($row->nilai_total_diklaim == 0){
                        $row->nilai_total_diklaim = $row->nilai_total_diklaim_mutasi;
                    }
                }
                if (isset($arrDiskonDataByTransaksi[$row->id]) && (sizeof($arrDiskonDataByTransaksi[$row->id]) > 0)) {
                    foreach ($arrDiskonDataByTransaksi[$row->id] as $kky => $vval) {
                        $row->$kky = $vval;
                    }
                }
                //---------------
                $jenis_trans = $row->jenis_master;
                $modules = isset($this->masterConfigUi[$jenis_trans]["modul"]) ? $this->masterConfigUi[$jenis_trans]["modul"] : false;
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    if (is_array($fLabel)) {
                        ...
                        if (isset($row->ids_his)) {
                            if ($hisKey == "nomer") {
//                                $returnVal = showHistoriGlobalNumbers($row->ids_his, $hisStep, true, $this->jenisTr);
                                $returnVal = showHistoriGlobalNumbers($row->ids_his, $hisStep, true, $jenis_trans);
                                if ($returnVal == "") {
                                    $tmp[$fName] = "-";
                                    $tmpByPihakID[$row->suppliers_id][$fName] = "-";
                                }
                                else {
                                    $tmp[$fName] = $returnVal;
                                    $tmpByPihakID[$row->suppliers_id][$fName] = $returnVal;
                                }
                            }
                            else {
                                $ids_his_decode = blobDecode($row->ids_his);
                                if (isset($ids_his_decode[$hisStep][$hisKey])) {
                                    $tmp[$fName] = $ids_his_decode[$hisStep][$hisKey];
                                    $tmpByPihakID[$row->suppliers_id][$fName] = $ids_his_decode[$hisStep][$hisKey];
                                }
                                else {
                                    $tmp[$fName] = "-";
                                    $tmpByPihakID[$row->suppliers_id][$fName] = "-";
                        ...
                        $tmp[$fName] = isset($row->$fName) ? formatField_he_format($fName, $row->$fName, $jenis_trans, $modules_path) : "0";
                        if (array_key_exists($fName, $historySumFields)) {
                            ...
                            // jumlah bawah per-supplier
                            if (!isset($tmpByPihakID[$row->suppliers_id][$fName])) {
                                $tmpByPihakID[$row->suppliers_id][$fName] = 0;
                            }
                            $tmpByPihakID[$row->suppliers_id][$fName] += $row->$fName;
                            // jumlah kanan per-supplier
                            if (!isset($tmpByPihakIDRight[$row->suppliers_id])) {
                                $tmpByPihakIDRight[$row->suppliers_id] = 0;
                            }
                            $tmpByPihakIDRight[$row->suppliers_id] += $row->$fName;
                            // jumlah kanan per-nota
                            if (!isset($tmp['sub_total'])) {
                                $tmp['sub_total'] = 0;
                            }
                            $tmp['sub_total'] += $row->$fName;

                            //-----------------------------------
                            $key_status = $fName . "_status";
                            if (isset($row->$fName)) {
                                $sisa_klaim = floor($row->$fName);
                                $key_diklaim = isset($historyKlaimFields[$fName]) ? $historyKlaimFields[$fName] : 0;
                                $nilai_diklaim = floor($row->$key_diklaim);
                                if (($sisa_klaim > 0) && ($nilai_diklaim == 0)) {
                                    $tmp[$key_status] = 3;
                                }
                                elseif (($sisa_klaim > 0) && ($nilai_diklaim > 0)) {
                                    $tmp[$key_status] = 4;
                                }
                                elseif (($sisa_klaim == 0) && ($nilai_diklaim > 0)) {
                                    $tmp[$key_status] = 5;
...
//                                cekHijau($row->nomer." || sisa klaim: $sisa_klaim, diklaim: $nilai_diklaim");
                            }
                            else {
                                $tmp[$key_status] = 2;
                            }
...
//                    if (is_numeric(isset($row->$fName) ? $row->$fName : "")) {
                    ...
                    if ($fName == "nilai_total_diklaim") {
                        if (isset($arrTransaksiDataKlaim[$row->id])) {
                            $hasil = "";
                            foreach ($arrTransaksiDataKlaim[$row->id] as $data) {
                                $modules = isset($this->masterConfigUi[$data["jenis_master"]]["modul"]) ? $this->masterConfigUi[$data["jenis_master"]]["modul"] : false;
                                $modul_path_ref = base_url() . $modules . "/";
                                $nota = formatField_he_format("nomer", $data["nomer"], $data["jenis_master"], $modul_path_ref);
                                if ($hasil == "") {
                                    $hasil = "<span class='meta'>klaim: " . $data["fulldate"] . ", " . $nota . ", " . $data["oleh_nama"] . "</span>";
                                }
                                else {
                                    $hasil .= "<br><span class='meta'>klaim: " . $data["fulldate"] . ", " . $nota . ", " . $data["oleh_nama"] . "</span>";
                                }
                            }
                            $tmp[$fName . "_label"] = $hasil;
                        }
                ...
                if (sizeof($row->cancel_packing_source_id) > 0) {
                    ...
                    $trx->addFilter("id='" . $row->cancel_packing_source_id . "'");
                    $tmpTrx = $trx->lookupAll()->result();
                    $tmp['nomer_top'] = formatField_he_format("nomer_top", $tmpTrx[0]->nomer, $jenis_trans, $modules_path);
                }
                ...
                //-------------------
                $sisa_diskon_hitung = $row->diskon_nilai_total_freeproduk - $row->nilai_total_diklaim;
                $row->sisa_diskon_hitung = (reformatExponent($sisa_diskon_hitung) < 1) ? 0 : reformatExponent($sisa_diskon_hitung);
//                arrPrint($row);
                if (($row->sisa_diskon_hitung == 0) && ($row->sisa_diskon > 0)) {
                    $arrGrnUpdateID[$row->id] = $row->id;
                }
                if ($row->diskon_nilai_total_freeproduk != $row->sisa_diskon) {
                    if ($row->nilai_total_diklaim == 0) {
                        $arrGrnUpdateID_cek[$row->suppliers_id][$row->id][1] = $tmpReg_result[$row->id]["main"]["diskon_1_nilai"];
                        $arrGrnUpdateID_cek[$row->suppliers_id][$row->id][2] = $tmpReg_result[$row->id]["main"]["diskon_2_nilai"];
                        $arrGrnUpdateID_cek[$row->suppliers_id][$row->id][3] = $tmpReg_result[$row->id]["main"]["diskon_3_nilai"];
                        $arrGrnUpdateID_cek[$row->suppliers_id][$row->id][4] = $tmpReg_result[$row->id]["main"]["diskon_4_nilai"];
                        $arrGrnUpdateID_cek[$row->suppliers_id][$row->id][5] = $tmpReg_result[$row->id]["main"]["diskon_5_nilai"];
                    }
                }
                //-------------------
            }
...
//        arrPrintCyan($arrGrnUpdateID_cek);

//        arrPrintPink($arrDiskonLockerData);
//        arrPrintPink($tmpByPihakID);
//        arrPrintKuning($tmpByPihakIDRight);
//        arrPrintWebs($arrayOnprogress);
//

        if (sizeof($arrGrnUpdateID) > 0) {
//            $this->cekSaldoDiskonGrn($arrGrnUpdateID);
        }
        if (sizeof($arrGrnUpdateID_cek) > 0) {
//            $this->cekSaldoDiskonGrnCache($arrGrnUpdateID_cek);
        }
        ...
        //-----------------------------------------------------
        if (isset($viewKlaimSupplier["addPairedModel"])) {
            $arrPihakData = array();
            $mdlName = $viewKlaimSupplier["addPairedModel"]["mdlName"];
            $this->load->model("Mdls/$mdlName");
            $mnm = New $mdlName();
            $mnmTmp = $mnm->lookupAll()->result();
            if (sizeof($mnmTmp) > 0) {
                foreach ($mnmTmp as $ii => $mnmSpec) {
                    $arrPihakData[$mnmSpec->id] = $mnmSpec->nama;
                }
            }
        }
        if (isset($_GET["sid"]) && ($_GET["sid"] > 0)) {
            $getSupplierNama = (isset($_GET["sid"])) ? "(" . $arrPihakData[$_GET["sid"]] . ")" : "";
            unset($progressFields["suppliers_nama"]);
        }
        //-----------------------------------------------------
        ...
        $data = array(
            ...
            "availDbTable" => $availDbTable,
            "arrayHistory" => isset($arrayHistory) ? $arrayHistory : array(),
            "onprogressTitle" => "<span class='glyphicon glyphicon-alert'></span> DAFTAR KLAIM KE SUPPLIER &nbsp; $getSupplierNama",
            "arrayProgressLabels" => $progressFields,
            "arrayOnProgressToPay" => $paymentConfig,
            "itemLabels" => isset($itemLabels) ? $itemLabels : "",
            "srcLabel" => isset($srcLabel) ? $srcLabel : "",
            ...
            "arraySumField" => isset($sumValue) ? $sumValue : array(),
            "arrPihakData" => isset($arrPihakData) ? $arrPihakData : array(),
            "pihakDataSelected" => isset($_GET["sid"]) ? $_GET["sid"] : 0,
            "linkDataSelected" => MODUL_PATH . "Transaksi/viewKlaimSupplierIndex/" . $this->jenisTr . "/",
            "addTabHeaderLimit" => isset($viewKlaimSupplier["addTabHeaderLimit"]) ? $viewKlaimSupplier["addTabHeaderLimit"] : 0,
            "historySumFields" => isset($historySumFields) ? $historySumFields : array(),
            "labelAlias" => isset($labelAlias) ? $labelAlias : array(),
        );

        $this->load->view("transaksi", $data);
    }

    public function cekSaldoDiskonGrn($kiriman)
    {
        $tbl = "__rek_pembantu_subpiutangsuppliertrans__1010020030";
        ...
        foreach ($kiriman as $id) {
            $this->db->select('*');
            $this->db->where("extern_id", $id);
            $this->db->where("rekening", "1010020030");
            $this->db->where("periode", "forever");
            $query = $this->db->get($tbl1)->result();
            foreach ($query as $query_spec) {
                $tbl_id = $query_spec->id;
                $data = array(
                    "debet" => 0,
                    "harga" => $query_spec->debet,
                );
                $this->db->where('id', $tbl_id);
                $this->db->update($tbl1, $data);
...
//        cekHijau("<h3>DONE...</h3>");
    ...
    public function cekSaldoDiskonGrnCache($kiriman)
    {
        $tbl = "__rek_pembantu_subpiutangsuppliertrans__1010020030";
        $tbl1 = "_rek_pembantu_subpiutangsuppliertrans_cache";
        $tbl2 = "stock_locker_diskon";
        ...
        foreach ($kiriman as $supplier_id => $spec) {
            foreach ($spec as $trid => $specc){
                foreach ($specc as $diskon_id => $nilai){

                    $this->db->select('*');
                    $this->db->where("extern_id", $trid);
                    $this->db->where("extern2_id", $diskon_id);
                    $this->db->where("extern3_id", $supplier_id);
                    $this->db->where("rekening", "1010020030");
                    $this->db->where("periode", "forever");
                    $query = $this->db->get($tbl1)->result();
                    foreach ($query as $query_spec) {
