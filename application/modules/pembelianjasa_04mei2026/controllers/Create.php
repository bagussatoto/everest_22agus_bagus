...
//                        cekkuning("detecting printing size");
    ...
    public function dataApprove()
    {
        ...
        $jenisTr = $this->jenisTr;
        $selectorModel = isset($this->configUi[$jenisTr]['selectorModel']) ? $this->configUi[$jenisTr]['selectorModel'] : array();
        $pihakModel = isset($this->configUi[$jenisTr]['pihakModel']) ? $this->configUi[$jenisTr]['pihakModel'] : array();
//        cekHere(":: $selectorModel :: $pihakModel ::");
        $arrMdl = array($selectorModel, $pihakModel);
        $arrDataApprove = array();
        $arrDataApproveLabel = array();
        foreach ($arrMdl as $mdl) {

            $dt = New MdlDataTmp();
//        $dt->addFilter("mdl_name in ('".implode("','", $arrMdl)."')");
            $dt->addFilter("mdl_name=$mdl");
            $dtTmp = $dt->lookupAll()->result();
//            showLast_query("biru");
//            cekHere(":: $mdl = " . sizeof($dtTmp));

            if (sizeof($dtTmp) > 0) {
                foreach ($dtTmp as $spec) {
//                    arrPrint($spec);
                    $mdl_tmp = $spec->mdl_name;
                    $label_tmp = $spec->mdl_label;
                    $content_decode = blobDecode($spec->content);
//                    arrPrintWebs($content_decode);
                    $nama = $content_decode["nama"];

                    $arrDataApprove[$mdl_tmp][] = $nama;
                    $arrDataApproveLabel[$mdl_tmp] = $label_tmp;
                }
            }
        }

//        arrPrintHijau($arrDataApproveLabel);
//        arrPrintHijau($arrDataApprove);

        $data = array(
            "mode" => "data_approve",
            "items" => $arrDataApprove,
            "label" => $arrDataApproveLabel,
//            "refresh" => MODUL_PATH . get_class($this) . "/dataApprove/$jenisTr",
        );
        $this->load->view("transaksi", $data);
    }
}
