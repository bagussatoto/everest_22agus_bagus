...
class _processPihak extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        $shopingCartErrorEditable = isset($this->configUi[$this->jenisTr]['shopingCartErrorEditable']) ? $this->configUi[$this->jenisTr]['shopingCartErrorEditable'] : array();
        $cCode = $this->cCode;
...
//        mati_disini("==========");
//region handling error tidak bisa edit dpp pengganti
        $error_label = "";
        if(count($shopingCartErrorEditable)>0){
            arrPrint($shopingCartErrorEditable);
            foreach ($shopingCartErrorEditable as $ky =>$labels){
                arrPrint($_SESSION[$cCode]["main"]);
                if(isset($_SESSION[$cCode]["main"][$ky])){

                    if(strlen($_SESSION[$cCode]["main"][$ky])<10){
                        cekMerah("adadada");
                        $error_label .= $labels;
                    }
                    else{
                        cekBiru(count($_SESSION[$cCode]["main"][$ky]));
                        cekHitam("lolos");
                    }
                }
            }
...
//        echo count($shopingCartErrorEditable);
        ...
        echo "<script>";
        echo "  top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "top.$('#error_label').text('$error_label')";
        echo "</script>";
    }
