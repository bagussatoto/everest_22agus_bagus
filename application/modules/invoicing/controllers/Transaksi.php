...
//        arrPrintWebs($_GET);
        ...
        $feederTransaksiJenis = isset($this->configUi[$jenisTr]["feederTransaksiJenis"]) ? $this->configUi[$jenisTr]["feederTransaksiJenis"] : array();
        $feederTransaksiReplacerStepJenis = isset($this->configUi[$jenisTr]["feederTransaksiReplacerStepJenis"]) ? $this->configUi[$jenisTr]["feederTransaksiReplacerStepJenis"] : array();
        ...
        /* ------------------------------------------------
         * DISCLAIMER untuk masing2 step ui ada di coUi
         * ------------------------------------------------*/
        $diskripsi = "";
        if (sizeof($feederTransaksiJenis) > 0) {
//            arrPrintWebs($feederTransaksiJenis);
            if (isset($_GET["step"]) && ($_GET["step"] > 0)) {
                $historyFields = $feederTransaksiJenis[$_GET["step"]]["header"];
                $diskripsi = isset($feederTransaksiJenis[$_GET["step"]]["diskripsi"]) ? $feederTransaksiJenis[$_GET["step"]]["diskripsi"]['text'] : "";
            }
            else {
                $historyFields = $feederTransaksiJenis[1]["header"];
            }
        }
...
//        $progressFields['state'] = "status";
        $progressFields['action'] = "action";
//        $steps = $this->configUi[$jenisTr]['steps'];
        $steps = $this->configUi[$jenisTr]['feederTransaksiJenis'];
        foreach ($steps as $sk => $vspec) {
            $arrJenisRefINV[$sk] = $vspec["target"];
        }
        $arrFilters = array();
...
//        }
        ...
        $tr->addFilter("div_id='" . $this->session->login['div_id'] . "'");
//        $tr->addFilter("jenis in ('".implode("','", $arrJenisRefINV)."')");
        $tr->addFilter("transaksi.status_inv='0'");
        $tr->addFilter("transaksi.trash_4='0'");

//        $tr->addFilter("transaksi.pembayaran='credit'");
//        $tr->addFilter("next_substep_code<>''");
//        $tr->addFilter("sub_step_number>0");
//        $tr->addFilter("jenis_top='" . $steps[1]['target'] . "'");
//        $tr->addFilter("valid_qty>0");
...
//        $this->db->group_end();
        ...
        // ----------------------------------------------------------
        // $this->db->group_start();
        // $where_1 = "transaksi.jenis='5822spd' and transaksi.pembayaran in ('credit','cod')";// penjualan kredit
        // $this->db->where($where_1);
        // $where_0 = "transaksi.jenis='5822spo' and transaksi_data.valid_qty > 0";// penjualan so
        // $this->db->where($where_0);
        // $this->db->group_end();
        // ------------------------------
        $this->db->group_start();
        // $where_0 = "transaksi.jenis='5822spo' and transaksi_data.valid_qty > 0";// penjualan so
        // $this->db->or_where($where_0);
        $where_1 = "transaksi.jenis='5822spd' and transaksi.pembayaran in ('credit','cod')";// penjualan kredit
        $this->db->or_where($where_1);

        $where_0 = "transaksi.jenis='5822spo' and transaksi_data.valid_qty > 0";// penjualan spo
        $this->db->or_where($where_0);

        $where_4 = "transaksi.jenis='5822so' and transaksi_data.valid_qty > 0";// penjualan spo
        $this->db->or_where($where_4);

        $this->db->group_end();
        //-------------------------

        $where_2 = "transaksi.jenis='4464'";// penjualan tunai
            ...
            if (sizeof($pairRegistries) > 0) {
                ...
                if (sizeof($tmpReg) > 0) {
                    ...
                    if ($pakai_ini == 1) {
                        ...
                        $referenceNumber_current_explode = ($tmpReg_result[$regRow->transaksi_id]["main"]["jenisTr"]);
...
//                        arrPrintCyan($tmpReg_result[$regRow->transaksi_id]["main"]);
                    }
                }
            }
            ...
            foreach ($tmpHist as $row) {
//                arrPrint($row);
                $arrTransID_master[$row->id_master] = $row->id_master;
...
//                }
                ...
                if($pakai_ini==1){
                    if ($row->sub_step_number > 0) {
                        $allowFollowup = true;
                        $actionLabel = $this->configUi[$jenisTr]['steps'][$currentStepNum]['label'];
                    }
                    if (isset($this->configUi[$jenisTr]['steps'][$nextStepNum])) {
                        if (isset($this->accessList[$jenisTr])) {
                            if (isset($this->accessList[$jenisTr][$nextStepNum][$nextStepCode]["allowFollowUp"])) {
                    ...
                    $allowFollowup = true;
                    $actionLabel = $this->configUi[$jenisTr]['steps'][1]['actionLabel'];
                    // replacer $currentStepNum
                    $currentStepNum = $feederTransaksiReplacerStepJenis[$row->jenis];
                }
...
//                    $targetFollowupLink = ($isCancelPacking == true) ? "followupCancelPackingPrePreview" : "followupPrePreview";
                    $targetFollowupLink = "followupPrePreviewInvoicing";
...
//                        }
                ...
                // $arrayOnprogress[] = $tmp;
                if(array_key_exists($row->jenis, $feederTransaksiReplacerStepJenis)){
        ...
        $data = array(
            ...
            "onprogressTitle" => "transaksi in process ",
            "arrayProgressLabels" => $progressFields,
            ...
            "feederTransaksiJenis" => isset($feederTransaksiJenis) ? $feederTransaksiJenis : array(),
