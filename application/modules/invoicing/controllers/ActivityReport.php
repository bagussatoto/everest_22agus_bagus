...
class ActivityReport extends Modul_Controller
{
    ...
    public function viewMonthly()
    {
        ...
        /* -----------------------------
         * pengaturan group akses
         * -----------------------------*/
        $allowReportPusat = array(
            "c_holding",
            "c_owner",
            "o_export_spv",
            "c_special",
        );
        $allowReportCabang = array(
            "c_owner",
            "o_export_spv",
            // "o_seller_spv",
        );
...
//arrPrintWebs(my_memberships());
        ...
        if ($this->session->login['cabang_id'] > 0) {
            ...
            $nilai = 0;
            foreach ($allowReportCabang as $groupAkses) {
                if (in_array($groupAkses, my_memberships())) {
                    $nilai++;
                }
            }
            if ($nilai == 0) {
                $tr->addFilter("oleh_id='" . my_id() . "'");
            }
        }
        else {
            $selectedCabang = "transaksi.cabang_id<>-1";
            $nilai = 0;
            foreach ($allowReportPusat as $groupAkses) {
                if (in_array($groupAkses, my_memberships())) {
                    $nilai++;
                }
            }
            if ($nilai == 0) {
                ...
                // tambahan untuk hanya olehID == id_login
...
//         showLast_query("pink");
            ...
            foreach ($registries as $registry) {
                ...
                foreach ($regs as $reg) {
                    ...
                    if (isset($reg['nett1'])) {
                        ...
                        if (!isset($subNettRekap[$trDtime_m][$sellers_id][$xsID])) {
                            $subNettRekap[$trDtime_m][$sellers_id][$xsID] = 0;
                        }
        ...
        // matiHEre();
//        arrPrintKuning($this->configUi[$this->jenisTr]['steps']);
//        cekHere("$names, $nameMasterAlias, $months");
        $label_target = "";
        foreach ($this->configUi[$this->jenisTr]['steps'] as $stp => $stpSpec){
            if($stpSpec["target"] == $selectedStep){
                $label_target = $stpSpec["label"];
            }
        }
        $data = array(
            "mode" => "recap",
//            "title" => $this->configUi[$this->jenisTr]['label'] . " report",
//            "title" => "Monthly " . $label_target . " Report ",
            "title" => "Monthly " . $label_target . " ",
            "subTitle" => "",
//            "subTitle" => "monthly, " . $currYear,
            ...
            "thisPage" => MODUL_PATH . get_class($this) . "/" . $this->uri->segment(3) . "/" . $this->jenisTr,
            ...
            "allowReportPusat" => $allowReportPusat,
            "allowReportCabang" => $allowReportCabang,
            "allowReportAkses" => $nilai,
            "currYear" => $currYear,
            "dtime" => formatTanggal(date(), "d F Y H:i:s"),
        );
    ...
    public function viewSalesMonthly()
    {
        ...
        $r->addFilter("periode='bulanan'");
        //$r->addFilter("rekening in ('penjualan','return penjualan')");
        $r->addFilter("rekening in ('penjualan','return penjualan','penjualan projek')");
        $r->setTableName("_rek_master_cache");
        $rekValuesTmp = $r->lookUpAll()->result();
        cekLime($this->db->last_query());
        $rekValues = array();
        ...
        foreach ($recaplistAll as $sID => $sidData) {
            foreach ($sidData as $time => $timeData) {
                ...
                $src3 = isset($timeData['7499']) ? $timeData['7499'] : 0;
                $srcF1 = isset($timeData['982']) ? $timeData['982'] : 0;
                $srcF2 = isset($timeData['9912']) ? $timeData['9912'] : 0;
                $srcF3 = isset($timeData['999']) ? $timeData['999'] : 0;
                $net = ($src + $src2 + $src3) - $srcF1 - $srcF2 - $srcF3;
                $recaplistAll[$sID][$time]['netto'] = $net;
                // foreach ($selectedTrans as $jenis => $alias) {
                //     if (!isset($recaplistAll[$sID][$time][$jenis])) {
                //         $recaplistAll[$sID][$time][$jenis] = 0;
    ...
    public function viewSalesOrderMonthlySql()
    {
        ...
        // arrPrintPink($arrPenjualan_idMaster);
        // arrPrintWebs($regReferensiId);
        ...
        foreach ($tmp as $row) {
            //            arrPrintWebs($row);
            //            $sellID = $listedCustomer[$row->id_master];
            $sellID = isset($listedCustomer[$row->id]) ? $listedCustomer[$row->id] : "";
            $trDtime = $trIdDts[$row->id]["dtime"];
            $trDtime_m = formatTanggal($trDtime, "Y-m");
