...
$('#uplodXls').on('submit',function() {
    ...
    case "viewUndoneItemsIndex":
        ...
        $stepper = isset($_GET['step']) ? ($isMobile == true ? 3 : $_GET['step']) : 3;
        if (isset($_GET['step'])) {
            //            arrPrint($steps[$_GET['step']]);
        }
// cekHere($stepper);
        ...
        $arrayOnprogressGroup_jml = 0;
        foreach ($arrayOnprogressGroup as $item => $data_steps) {
            $arrayOnprogressGroup_jml += count($data_steps);
        }
        $inproses_jml = $arrayOnprogressGroup_jml > 0 ? $arrayOnprogressGroup_jml : "";
        // $inproses_jml = 10;
        $strOnprog = "";

        $strOnprog .= "<div id='box_pemantau' class=\"box box-success box-solid collapsed-box $cslass_hidden\" style='margin-bottom: 10px !important;'>";
        $strOnprog .= "<div class=\"box-header with-border ttext-red\">";
        $strOnprog .= "<h4 class=\"box-title text-uppercase\">$onprogressTitle <span class='badge bg-yellow' id='inproses_jml'></span> <small style='color: yellow;' id='inproses_warning'></small></h4>";
        $strOnprog .= "<div class='box-tools pull-right'>";
        $strOnprog .= "<button type='button' class='btn btn-box-tool' data-widget='collapse' id='btn_collapsible'><i class='fa fa-plus'></i></button>";
        $strOnprog .= "<button type='button' class='btn btn-box-tool' id='btn_refresh' title='perbarui data'><i class='fa fa-refresh'></i></button>";
        $strOnprog .= "</div>";
        $strOnprog .= "</div>";

        $strOnprog .= "<div class=\"box-body\" style='display: none;'>";
        ...
        if (sizeof($steps) >= 1) {
            $strOnprog .= "<ul class='nav nav-tabs'>";
            foreach ($steps as $tStep => $stepData) {
                $feederTransaksiJenisParams = $feederTransaksiJenis[$tStep];
                $attr = $feederTransaksiJenisParams['attr'];
                $isiBadge = isset($arrayOnprogressGroup[$tStep]) ? sizeof($arrayOnprogressGroup[$tStep]) : "";
                $actives = $tStep == $stepper ? "active" : "";
//                $trSelesai = count($steps) == $tStep ? "SELESAI<br>" : "";
...
//                $cssSelesai = count($steps) == $tStep ? "style='padding-top: 0;padding-bottom: 0;'" : "";
                $cssSelesai = "";
                $strOnprog .= "<li class='$actives' $attr>";
                ...
                $strOnprog .= "<span class='text-uppercase text-bold' style='$visibility;'>$print_qr $trSelesai" . $stepData['label'] . "</span>  <span class='badge bg-red'>$isiBadge</span> </a>";
                $strOnprog .= "</li>";
            }
            $strOnprog .= "</ul>";
            $strOnprog .= "<div class='clearfix'>&nbsp;</div>";
        ...
        if (strlen($diskripsi) > 10) {
            $strOnprog .= "<div class='alert alert-info' style='padding: 10px !important;'><div class='text-uppercase text-red'>disclaimer </div>$diskripsi</div>";
        }
        if (sizeof($arrayOnProgress) > 0) {
            // $isMobile = true;
            ...
            if ($isMobile == true & $stepper == 2) {
                ...
                if (strlen($diskripsi) > 10) {
                    // $strOnprog .= "<div class='alert alert-info '><div class='text-uppercase text-red'>disclaimer </div>$diskripsi</div>";
                }
                ...
                $strOnprog .= "<thead>";
                unset($arrayProgressLabels['action']);
                // arrPrintHijau($arrayProgressLabels);
...
//                $strOnprog .= "</div>";
        ...
        $url_sendiri = base_url() . uri_string();
        // cekHere($url_sendiri);
        $strOnprog .= "<script>
              let jml_inproses = '$inproses_jml';
              $('#inproses_jml').text(jml_inproses);
              if(jml_inproses > 0){                  
                   $('#btn_collapsible').trigger('click');
                   // $('#inproses_warning').html('oe oe').addClass('blink');
                   // $('#box_pemantau').addClass('box-info').removeClass('box-success');
              }
              else {
                   $('#box_pemantau').removeClass('box-success');
              }
              
              $('#btn_refresh').click(function() {
                $('#undoneList').load('$url_sendiri');
                $('#box_pemantau').addClass('box-info').removeClass('box-success');
              });
        </script>";
...
//    cekHere(":: $link_scan_mobile ::");
    ...
    case "preview":
        ...
        if (isset($newWarningLabel) && ($newWarningLabel != null)) {
            echo "<div class='alert alert-danger text-center'>";
            echo $newWarningLabel;
            echo "</div class='alert alert-warning'>";
        }
...
//            echo "<a class='btn btn-block btn-default' data-dismiss='modal'><span class='glyphicon glyphicon-chevron-left'></span> cancel</a>";
            echo "<button type='button' class='btn btn-default margin' data-dismiss='modal' 
                onclick=\"document.getElementById('result').src='$clearContentTarget';\"><span class='glyphicon glyphicon-chevron-left'></span> cancel </button>";
            echo "</div class='col-md-6'>";

            if ($hiddenButtonAction == false) {

                echo "<div class='col-md-6'>";
                echo "<a class='btn btn-block btn-success' 
            onclick=\"if(confirm('your selected items will be processed. Continue saving?')==1){document.getElementById('result').src='" . $actionTarget . "';this.style.visibility='hidden';}\">
