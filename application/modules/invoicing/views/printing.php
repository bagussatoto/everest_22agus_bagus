                ...
                if ($opsi_cetak == 3 || $opsi_cetak == 4 || $opsi_cetak == 6) {
                    ...
                    if (isset($iSpec['bahan_baku']['biaya'])) {
                        foreach ($iSpec['bahan_baku']['biaya'] as $prdData) {
                            ...
                            if ($opsi_cetak == 4 || $opsi_cetak == 6) { $global_jasa += $saldo; }
                        }
                    }
...
// START OF COMPLETE REPEATED LOGIC
            ...
            // Set colspan dinamis
            $colspan_footer = 2;
            if ($opsi_cetak == 1) { $colspan_footer = 2; }
            elseif ($opsi_cetak == 3 || $opsi_cetak == 2) { $colspan_footer = 3; }
            elseif ($opsi_cetak == 4) { $colspan_footer = 4; }
            elseif ($opsi_cetak == 5) { $colspan_footer = 5; }
            elseif ($opsi_cetak == 6) { $colspan_footer = 2; }
            else { $colspan_footer = is_array($itemLabels5) ? count($itemLabels5) : 3; }
// END OF COMPLETE REPEATED LOGIC
...
//        cekMerah("statusInvoice: $statusInvoice");
        ...
        /**
         * [AGENT_LOG]
         * PURPOSE: Tombol pemicu untuk mengakses Amandemen Invoice (Clean)
         * [/AGENT_LOG]
         */
        if ($jenisTr == '4822' || $jenisTr == '4822_mod') {
            $amandemen_link = base_url() . "amandemen_invoice/Create/index/" . $transaksi_id;
            //MATIKAN DULU JIKA PERLU DINYALAKAN UNTUK DIMONITORING
//            $option .= "<div class='hidden-on-print' style='margin-bottom:10px; margin-top:5px; text-align:right;'>";
//            $option .= "<a href='#' onclick=\"window.open('$amandemen_link', 'Amandemen', 'width=1100,height=800,scrollbars=yes'); return false;\" class='btn btn-warning btn-sm' style='font-weight:bold; border-radius:3px; box-shadow: 0px 2px 4px rgba(0,0,0,0.2);'><i class='fa fa-edit'></i> Buat Amandemen Invoice (Clean)</a>";
//            $option .= "</div>";
        }
