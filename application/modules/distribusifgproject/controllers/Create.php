...
//                arrPrint($selectorProcessor);
        ...
        // region GUDANG PROJECT DI PUSAT---------------------------------
        $this->load->model("Mdls/MdlGudang");
        $cb = New MdlGudang();
        $cb->addFilter("jenis='gudang project'");
        if (my_cabang_id() == CB_ID_PUSAT) {
            $cb->addFilter("cabang_id=" . my_cabang_id());
        }
        else {
            $cb->addFilter("cabang_id=" . CB_ID_PUSAT);
        }
        $cbTmp = $cb->lookupAll()->result();
        if (sizeof($cbTmp) > 0) {
            $_SESSION[$cCode]["main"]["gudangProjectID"] = $cbTmp[0]->id;
            $_SESSION[$cCode]["main"]["gudangProjectName"] = $cbTmp[0]->nama;
            $_SESSION[$cCode]["main"]["gudangProjectNama"] = $cbTmp[0]->nama;
        }
        else {
            $_SESSION[$cCode]["main"]["gudangProjectID"] = 0;
            $_SESSION[$cCode]["main"]["gudangProjectName"] = "";
            $_SESSION[$cCode]["main"]["gudangProjectNama"] = "";
        }
        //endregion---------------------------------
        // region pilih cabang, kalau 1, langsung build session main pihak
        $this->load->model("Mdls/MdlCabang");
        $cb = New MdlCabang();
        $cb->addFilter("jenis='project'");
        if (my_cabang_id() == CB_ID_PUSAT) {
            $cbTmp = $cb->lookupAll()->result();
            if (sizeof($cbTmp) > 0) {
                $_SESSION[$cCode]["main"]["pihakID"] = $cbTmp[0]->id;
                $_SESSION[$cCode]["main"]["pihakName"] = $cbTmp[0]->nama;
                $_SESSION[$cCode]["main"]["pihakNama"] = $cbTmp[0]->nama;
                $_SESSION[$cCode]["main"]["pihakName2"] = $cbTmp[0]->nama;
            }
        }
        // endregion pilih cabang, kalau 1, langsung build session main pihak
        ...
        $this->load->view("transaksi", $data);
            ...
            echo $_SESSION[$cCode]["main"]["bookingNumber"];
        }
    ...
    {
        $returnJson = (isset($_GET['return_json']) && $_GET['return_json'] == 1) || (defined('SILENT_TRANSAKSIONAL') && SILENT_TRANSAKSIONAL);
        if ($returnJson) {
            ob_start();
        }

        if (!isset($_GET['json']) && !$returnJson) {
...
//            unset($_SESSION[$cCode]["main"]["bookingNumber"]);
            if (!isset($_SESSION[$cCode]["main"]["bookingNumber"])) {
                ...
                mati_disini($msg);
                    ...
                    echo json_encode($_SESSION[$cCode]);
                    die();
                }
            }
            ...
            if (!isset($_SESSION[$cCode]['items'])) {
                if ($returnJson) {
                    if (ob_get_level() > 0) { @ob_end_clean(); }
                    echo json_encode(array("success" => false, "error" => "belum ada item yang dipilih"));
                    return;
                }
            ...
            if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo("now processing your transaction..<br>"); }
            ...
            if (sizeof($iterator) > 0) {
                ...
                if (sizeof($iterator) > 0) {
                    foreach ($iterator as $cCtr => $tComSpec) {
                        ...
                        if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "sub-preproc: $comName, initializing values <br>"; }
                ...
                if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo("no processor defined. skipping preprocessor..<br>"); }
            }
            //endregion
...
//            cekOrange("lolos validator @" . __LINE__ . " " . __METHOD__);
            ...
            if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "<div style='background:#ff7766;'>"; }
            ...
            if (isset($_SESSION[$cCode]['tableIn_master']) && sizeof($_SESSION[$cCode]['tableIn_master']) > 0) {
                ...
                $_SESSION[$cCode]['tableIn_master']['project_id'] = $_SESSION[$cCode]['main']['pihakProjekID'];
                $_SESSION[$cCode]['tableIn_master']['project_nama'] = $_SESSION[$cCode]['main']['pihakProjekName'];

                $_SESSION[$cCode]['tableIn_master']['referensi_id'] = $_SESSION[$cCode]['main']['pihakProjekWorkorderSubGudangID'];
                $_SESSION[$cCode]['tableIn_master']['referensi_nomer'] = $_SESSION[$cCode]['main']['pihakProjekNoSpk'];
                $_SESSION[$cCode]['tableIn_master']['extern_nomer'] = $_SESSION[$cCode]['main']['pihakProjekNoSubSpk'];
                $_SESSION[$cCode]['tableIn_master']['referensi_jenis'] = $_SESSION[$cCode]['main']['pihakProjekWorkOrderSubNama'];
            ...
            if (sizeof($iterator) > 0) {
                ...
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
                    if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "component # $cCtr: $comName<br>"; }
            ...
            if (sizeof($iterator) > 0) {
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
                    if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "[$cCtr] sub-postProcessor: $comName, gate: $srcGateName, initializing values <br>"; }
                    $tmpOutParams[$cCtr] = array();
                ...
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    if (isset($_SESSION[$cCode][$srcGateName])) {
                        if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "[$cCtr] sub-postProcessor: $comName, sending values <br>"; }
                ...
                if (sizeof($relPaymentSrc) > 0) {
                    ...
                    if (sizeof($tmpRelPay) > 0) {
                        ...
                        foreach ($iterator as $cCtr => $tComSpec) {
                            ...
                            //                            $srcGateName = $tComSpec['srcGateName'];
                            //                            $srcRawGateName = $tComSpec['srcRawGateName'];
                            if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "sub-postProcessor: $comName, sending values <br>"; }
            ...
            if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                ...
                if (($stepnumAvail - $stepnum) > 0) {
                    //                    mati_disini("masukk".$nextStepCode);
...
//            echo json_encode($_SESSION[$cCode]['items']);
//mati_disini(__LINE__);
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
            ...
            if ($returnJson) {
                if (ob_get_level() > 0) { @ob_end_clean(); }
                echo json_encode(array("success" => false, "error" => "session tidak valid"));
                return;
            }
...
//                mati_disini("TEST EDIT...");
            ...
            //mati_disini($_SESSION[$cCode]['main']['nomer']);
            $iterator = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail'] : array();
            if (sizeof($iterator) > 0) {
                foreach ($iterator as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
                    if (!defined("SILENT_TRANSAKSIONAL") && !isset($_GET["return_json"]) && !isset($_GET["json"])) { echo "sub-postProcessor: $comName, initializing values <br>"; }
                    $tmpOutParams[$cCtr] = array();
                    foreach ($_SESSION[$cCode][$srcGateName] as $xid => $dSpec) {
                        $id = $dSpec['id'];
                        $subParams = array();
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (strlen($connector) > 0) {
                ...
                if (sizeof($steps) == 1) {
                    ...
                    $tCodeTargetJenisTransaksi = $this->configUi[$connector]["connectToReject"][$stepNumCurrent]["connectTo"];
                    $modul_transaksi = $this->modul;
                    //==replace pertama
                    $masterReplacersO = array(
                        "jenisTr" => $connector,
