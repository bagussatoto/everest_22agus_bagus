...
class _processSelectValas extends Modul_Controller
{
...
//mati_disini(__LINE__);
        ...
        if ($pihakModel == "MdlBankAccount_in") {
            ...
            $this->load->model("Mdls/" . $pihakModel);
            $a = new $pihakModel();
            $tmpA = $a->lookupByID($extern_id)->result();
            if (sizeof($tmpA) > 0) {
                $_SESSION[$cCode]['main']['pihakID'] = $extern_id;
                $_SESSION[$cCode]['main']['pihakName'] = isset($tmpA[0]->nama) ? $tmpA[0]->nama : "";
                $tmpPihakName = isset($tmpA[0]->nama) ? $tmpA[0]->nama : "";
                if (isset($tmpA[0]->name)) {
                    $tmpPihakName = $tmpA[0]->name;
                }
                ...
                if (sizeof($pihakMainValueSrc) > 0) {
                    foreach ($pihakMainValueSrc as $key => $src) {
                        $_SESSION[$cCode]['main'][$key] = $tmpA[0]->$src;
                    }
                }
            }
        }
...
//                                echo "replacing value for $key with " . $newValue . "<br>";
                    ...
                    cekHere("masuk sini maka qty nambah 1");
                }
            }
        }
        else {
            cekMerah("tidak ada itemnya! " . __LINE__);
            die();
        }
