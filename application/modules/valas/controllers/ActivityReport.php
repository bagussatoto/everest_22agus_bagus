...
// arrPrint($historyFields);
    ...
    public function viewMonth()
    {
        ...
        */
        ...
        if (sizeof($compDatas) > 0) {
            ...
            foreach ($compDatas as $trJenis => $items) {
                ...
                foreach ($items as $transaksi_id => $item) {
                    ...
                    // region sales
                    $specs['sales']['value'] = $referenceID == 0 ? formatField_he_format("number", $nett1) : 0;
                    $specs['sales']['attr'] = "class='text-right'";
                    $referenceID == 0 ? $sumSale += $nett1 : $sumSale = 0;
                    // endregion sales

                    // region return
                    $specs['return']['value'] = $referenceID > 0 ? formatField_he_format("number", $nett1) : 0;
                    $specs['return']['attr'] = "class='text-right'";
                    $referenceID > 0 ? $sumReturn += $nett1 : $sumReturn = 0;
                    // endregion return

                    // region netto berjalan
                    $specs['netto']['value'] = formatField_he_format("number", $netto);
        ...
        $footers[formatField_he_format('number', $sumSale)] = "class='bg-info text-right'";
        $footers[formatField_he_format('number', $sumReturn)] = "class='bg-info text-right'";
        $footers[formatField_he_format('number', $sumNetto . ",0")] = "class='bg-info text-right'";
        // arrPrint($rSpecs);
        // arrPrint($spdSpecs);
        // arrPrint($bodies);
        // arrPrint($footers);
