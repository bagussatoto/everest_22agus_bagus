...
switch ($mode) {
    ...
    case "viewHistoryOld":
        ...
        if (isset($_GET['date1'])) {
            $date1_f = formatField_he_format("fulldate", $_GET['date1']);
            $date2_f = formatField_he_format("fulldate", $_GET['date2']);
        ...
        if (sizeof($trItems) > 0) {
            ...
            foreach ($trItems as $iSpec) {
                ...
                foreach ($trHeaders as $key => $label) {
                    ...
                    $content .= ("<td>");
                    $content .= formatField_he_format($key, $iSpec[$key]);
                    $content .= ("</td>");
            ...
            foreach ($trHeaders as $key => $label) {
                $content .= ("<td>");
                if (isset($totals[$key])) {
                    $content .= formatField_he_format($key, $totals[$key]);
                }
                $content .= ("</td>");
            }
            $content .= ("</tr>");
        ...
        if (sizeof($arrayHistory) > 0) {
            ...
            foreach ($arrayHistoryLabels as $key => $label) {
                ...
                if(isset($summary[$key])){
                    $content .= formatField_he_format("debet", $summary[$key]);
                }
                else{
                    $content .= "--";
                }
...
var pos = obj.indexOf('<');
    ...
    case "salesKonsolidate":
        ...
        if (sizeof($arrayHistory) > 0) {
            ...
            $content .= ("</th>");
            foreach ($arrayHistoryLabels as $key => $label) {
                $sumNilai = is_numeric($summary[$key]) ? formatField_he_format('debet',$summary[$key]) : "-";
                $content .= ("<th>");
                $content .= $sumNilai;
                $content .= ("</th>");
            }
