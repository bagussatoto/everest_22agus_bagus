...
//            $strByTrans .= "<h3>$labelByTransaksi</h3>";
            ...
            foreach ($settlementByTransaksi as $trNomor => $spec) {
                ...
                if (isset($headerMainByTransaksi) && sizeof($headerMainByTransaksi) > 0) {
                    foreach ($headerMainByTransaksi as $key => $val) {
                        ...
                        if ($val == NULL) {
                            $strByTrans .= formatField_he_format($key, $mhVal);
                        }
                        else {
                            $strByTrans .= "$val : " . formatField_he_format($key, $mhVal);
                        }
                        $strByTrans .= "<br>";
                    }
                }
                ...
                if (isset($itemTrans[$trNomor]) && sizeof($itemTrans[$trNomor]) > 0) {
                    ...
                    foreach ($itemTrans[$trNomor] as $iSpec) {
                        ...
                        foreach ($headerByTransaksi as $key => $val) {
                            $colVal = isset($iSpec[$key]) ? formatField_he_format($key, $iSpec[$key]) : "";
                ...
                if (isset($summaryByTransaksi) && sizeof($summaryByTransaksi) > 0) {
                    foreach ($summaryByTransaksi as $key => $val) {
                        ...
                        $strByTrans .= "<td>" . formatField_he_format($key, $sumVal) . "</td>";
                        $strByTrans .= "</tr>";
                    }
                }
            ...
            $strByTrans .= "<div><span sstyle='color:#000000;font-size:18px;'>TOTAL BILL: </span><span class='pull-right'>" . formatField_he_format("debet", sizeof($settlementByTransaksi)) . "</span></div>";
            $strByTrans .= "<div><span sstyle='color:#000000;font-size:18px;'>TOTAL BY NOTA/BILL: </span><span class='pull-right'>" . formatField_he_format("debet", $total) . "</span></div>";
            $strByTrans .= "</div>";
            $strByTrans .= "<br><br>";
    ...
    if (isset($settlementByMenu) && sizeof($settlementByMenu) > 0) {
        ...
        foreach ($settlementByMenu as $mSpec) {
            ...
            foreach ($headerByMenu as $key => $val) {
                ...
                $colVal = isset($mSpec[$key]) ? formatField_he_format($key, $mSpec[$key]) : "";
        ...
        foreach ($headerByMenu as $key => $val) {
            if (isset($total_menu[$key])) {
                $colVal = formatField_he_format($key, $total_menu[$key]);
                $strByMenu .= "<td>$colVal</td>";
            }
        }
        $strByMenu .= "</tr>";
