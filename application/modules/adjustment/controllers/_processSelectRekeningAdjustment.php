...
class _processSelectRekeningAdjustment extends Modul_Controller
{
    private $rels = array();
    ...
    public function __construct()
    {
        ...
        $accountChildConfig = $this->config->item("accountChilds");
//        if(sizeof($accountChildConfig)>0){
//            foreach($accountChildConfig as $rek=>$rel){
//
//            }
//        }
        $this->rels = $accountChildConfig;
...
//        $id = 1;
        ...
        if (!isset($_SESSION[$cCode]["main"]["ppnFactor"])) {
            if (!isset($this->session->login["ppnFactor"])) {
                matiHEre("undefine ppn factor, please logout and login again*");
            }
            else {
...
//                        $_SESSION[$cCode]["$sSName"][$iID] = null;
//                        unset($_SESSION[$cCode]["$sSName"][$iID]);
//                    }
//                }
//            }
        }
        ...
        $selectorSrcModel = $index == 1 ? $this->configUi[$this->jenisTr]['selectorSrcModel'] : $this->configUi[$this->jenisTr]['selectorSrcModel2'];
        $prefix = $index == 1 ? "src" : "target";
...
//                $tmpJml = 1;
                ...
                if (!array_key_exists($index, $_SESSION[$cCode]['items'])) {
                    $tmp = array(
                    ...
                    $_SESSION[$cCode]['main'][$prefix . 'Account'] = $row->id;
                    $_SESSION[$cCode]['out_master'][$prefix . 'Account'] = $row->id;

                    $_SESSION[$cCode]['main'][$prefix . 'Position'] = $row->defPosition;
                    $_SESSION[$cCode]['out_master'][$prefix . 'Position'] = $row->defPosition;

                    $rel = isset($this->rels[$row->id]) ? $this->rels[$row->id] : "";

                    $_SESSION[$cCode]['main'][$prefix . 'Rel'] = $rel;
                    $_SESSION[$cCode]['out_master'][$prefix . 'Rel'] = $rel;

                    foreach ($fieldSrcs as $key => $src) {
                        $tmp[$key] = makeValue($src, $tmp, $tmp, "");
...
//                    $_SESSION[$cCode]['items'][$id] = $tmp;
                    $_SESSION[$cCode]['items'][$index] = $tmp;
...
//                                echo "replacing value for $key with " . $newValue . "<br>";
                                ...
                                $_SESSION[$cCode]['items'][$index][$key] = $newValue;
                                echo "replacing value for $key with " . $newValue . "<br>";
                            }
                        }
                        ...
                        if ($subAmountConfig != null) {
                            $tmp['subtotal'] = makeValue($subAmountConfig, $_SESSION[$cCode]['items'][$index], $_SESSION[$cCode]['items'][$index], 0);
                        }
                        else {
                            $tmp['subtotal'] = 0;
                        }
//                        $_SESSION[$cCode]['items'][$id]['subtotal'] = $tmp['subtotal'];
                        $_SESSION[$cCode]['items'][$index]['subtotal'] = $tmp['subtotal'];
                    }
                    else {
                        cekbiru("itemNumLabels TIDAK ada");
                    }
...
//        echo "top.document.getElementById('pilihan_outlet').innerHTML='';";
        echo "top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=');";
        echo "</script>";
    }
...
//        $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHEre("undefine ppn factor, please logout and login again*");
        $ppnFactor = my_ppn_factor();
        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel2'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel2'];
...
//                $tmpJml = 1;
                ...
                if (!array_key_exists($index, $_SESSION[$cCode]['items'])) {
                    ...
                    $_SESSION[$cCode]['main']['targetAccount'] = $row->id;
                    $_SESSION[$cCode]['out_master']['targetAccount'] = $row->id;

                    $_SESSION[$cCode]['main']['targetPosition'] = $row->defPosition;
                    $_SESSION[$cCode]['out_master']['targetPosition'] = $row->defPosition;
...
//            }
        ...
        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel3'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel3'];

        $prefix = $index == 1 ? "src3" : "target3";
...
//                $tmpJml = 1;
                ...
                if (!array_key_exists($index, $_SESSION[$cCode]['items'])) {
                    ...
                    $_SESSION[$cCode]['main']['target3Account'] = $row->id;


                    $_SESSION[$cCode]['main']['target3Position'] = $row->defPosition;
...
//            }
        ...
        $selectorModel = $this->configUi[$this->jenisTr]['selectorModel4'];
        $selectorSrcModel = $this->configUi[$this->jenisTr]['selectorSrcModel4'];

        $prefix = $index == 1 ? "src4" : "target4";
...
//                $tmpJml = 1;
                ...
                if (!array_key_exists($index, $_SESSION[$cCode]['items'])) {
                    ...
                    $_SESSION[$cCode]['main']['target4Account'] = $row->id;


                    $_SESSION[$cCode]['main']['target4Position'] = $row->defPosition;
...
//                    echo "replacing value for $key with " . $newValue . "<br>";
            ...
            //==defining defValue
            cekmerah("defining defValue");
            $defValue = 0;
            switch ($_SESSION[$cCode]['main'][$prefix . 'Position']) {
                case "debet":
                    cekmerah("defining defValue from debet value");
                    if ($_SESSION[$cCode]['items'][$index]['debet'] > 0) {
                        $defValue = $_SESSION[$cCode]['items'][$index]['debet'];
                    }
                    else {
                        if ($_SESSION[$cCode]['items'][$index]['kredit'] > 0) {
                            $defValue = -($_SESSION[$cCode]['items'][$index]['kredit']);
                        }
                    }
                    break;
                case "kredit":
                    cekmerah("defining defValue from kredit value");
                    if ($_SESSION[$cCode]['items'][$index]['kredit'] > 0) {
                        $defValue = $_SESSION[$cCode]['items'][$index]['kredit'];
                    }
                    else {
            ...
            $_SESSION[$cCode]['main'][$prefix . 'DefValue'] = $defValue;

            cekmerah($prefix . "DefValue is now $defValue");
