...
if (isset($items)) {
    ...
    if (sizeof($items) > 0) {
        ...
        //-------------------------
        if (isset($shoppingcart_warning)) {
            echo "<div class='text-left'>";
            echo "<span class='meta'>$shoppingcart_warning</span>";
            echo "</div>";
        }
        //-------------------------
...
//        }
        ...
        foreach ($items as $iSpec) {
            if ($showItems) {
                ...
                if (isset($iSpec["background_color"])) {
                    $background = $iSpec["background_color"];
                }
                else {
                    $background = "";
                }

                echo "<tr id='tr_" . $iSpec['id'] . "' bgcolor=$bgColor style='background-color:$background;'>";
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            if (isset($iSpec["disabled"])) {
                                $disabled_input = $iSpec["disabled"];
                            }
                            else {
                                $disabled_input = "";
                            }

                            echo "<input type='text' $disabled_input min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control text-right' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
...
//                                echo "<span keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:$color;background:#f0f0f0;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                                echo "<span keyid=$keyID noid=$noID id=$colID class='text-right' style='color:$color;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                            }
                            else {
                ...
                if (isset($items2[$byID]) && sizeof($items2[$byID]) > 0) {
                    ...
                    if (sizeof($itemLabels2) && (is_array($itemLabels2)) && $showItems) {
                        ...
                        echo "<tr>";
                        echo "<td style='width:1%;' class='text-muted bg-grey-1 text-center'>";
                        echo "No";
                        echo "</td>";
                        ...
                        if (!$avoidRemove) {
                            echo "<th style='width:1%;' class='text-muted bg-grey-1 text-center'>";
                            echo "x";
                            echo "</th>";
                        }
                    ...
                    $kurangStoks = array();
                    foreach ($items2[$byID] as $iSpec2) {
                        $bid = $byID; // << biaya idnya
                        $iID = $iSpec2['id']; // << produkid nya
                        $no2++;
                        $bgColor = "transparent";
                        ...
                        if (isset($_SESSION['errLines'])) {
                            if (in_array($iSpec2['id'], $_SESSION["errLines"])) {
                                $bgColor = "#ffff77";
                            }
                        }
                        echo "<tr id='tr_" . $iSpec2['id'] . "' bgcolor=$bgColor>";
                        echo "<td width='5%'>";
                        echo $no2;
                        echo ".</td>";
                        $colCtr = 0;
...
//                            $qtyParam = "+removeCommas(document.getElementById('".$bid."_jml_$no2').value)"; //ori
                            $qtyParam = "+removeCommas(this.value)"; //perpendekan script
                        }
                        foreach ($itemLabels2 as $key => $label) {
                            $listMode[$key] = "input";
                            $keyupEvent[$key] = "";
                            ...
                            if (in_array($key, $editableFields2)) {
                                $readOnly[$key] = "";
                                if (isset($iSpec2["jml"]) && $iSpec2["jml"] < 1) {
                                    $readOnly[$key] = "readonly_xz";
                                }
                                if (isset($paramsForceEditable[$key])) {
                                    if ($paramsForceEditable[$key] == true) {
                            ...
                            $colID[$key] = $bid . "_" . $key . "_" . $no2;
                            if ($listMode[$key] == "input") {
                                $queryParams .= "&$key='+removeCommas(this.value)+'"; //perpendekan script
//                                $queryParams .= "&$key='+removeCommas(document.getElementById('" . $colID[$key] . "').value)+'"; //original
                            }
                        }
                        ...
                        foreach ($itemLabels2 as $key => $label) {
                            $colCtr++;
                            $color = "343434";
                            if (isset($_SESSION['errFields'][$iSpec2['id']])) {
                                if (in_array($key, $_SESSION['errFields'][$iSpec2['id']])) {
                                    $color = "#dd3300";
                                }
                            }
                            echo "<td align='left'>";
                            $colID = $bid . "_" . $key . "_" . $no2;
                            $keyID = $key;
                            $noID = $no2;
                            $tabIndexNum = $colCtr . $no2;
                            $fieldVal = "";
                            if (substr($key, 0, 1) == "*") {
                                $key_p = str_replace("*", "", $key);
                                $key_ex = explode("#", $key_p);
                                ...
                                if (isset($iSpec2[$key])) {
                                    if (is_numeric($iSpec2[$key])) {
                                        $fieldVal = $iSpec2[$key] + 0;
                                    }
                                    else {
                                        $fieldVal = $iSpec2[$key];
                                    }
                            ...
                            $keyupData = (($key == "qty" || $key == "jml") && $validateKey_up == true) ? "onkeydown=\"if(parseInt(this.value)<$moq){setTimeout(function(){ this.value='" . $iSpec2[$key] . "'}, 1000);} \"" : "";

                            switch ($listMode[$key]) {
                                case "input":
                                    if (isset($iSpec2["disabled"])) {
                                        $disabled_input = $iSpec2["disabled"];
                                    }
                                    else {
                                        $disabled_input = "";
                                    }
                                    ...
                                    echo "<input type='text' $disabled_input min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control text-right' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
                                    $baseInputName = isset($unionSelectors['base']) ? "document.getElementById('" . $unionSelectors['base'] . "_" . $no2 . "')" : "this";
                                    $pemicuGerbangAsli = "onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';} \" $keyupData";
                                    $pemicuGerbangAsli .= "*onmouseout=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';}\" ";
                                    $pemicuGerbang = "onblur=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';}\" $keyupData ";
                                    $pemicuGerbang .= "*onmouseout=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';}\" ";
                                    $pemicuGerbangUnion = "onchange=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';} \" ";
                                    ...
                                    if (isset($keyupAction) && $keyupAction == true) {
                                        echo "onkeyup=\"document.getElementById('result').src='" . $iSpec2['editTarget'] . "'$qtyParam+'$queryParams';if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }\"";
                                    }
                                    else {
                                        echo "onkeyup=\"delay( function(){ $('#shopping_cart').trigger('change') }, 400, this );if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }\"";
                                    }
                                ...
                                case "text":
                                    $add_data_val = "";
                                    if (is_array($label)) {
                                        $add_data_val = isset($iSpec2[$label["addKey"]]) ? "<br>" . $iSpec2[$label["addKey"]] : "";
                                    }
                                    if (is_numeric($fieldVal)) {
                                        echo "<span keyid=$keyID noid=$noID id=$colID class='text-right' style='color:$color;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                                    }
                                        ...
                                        if (strlen($fieldVal) > 10) {
                                            ...
                                            echo "<span keyid=$keyID noid=$noID id=$colID class='form-control no-border' style='color:$color;border:0px;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
                                        }
                                    }
                                    break;
                            }
                        ...
                        if (!$avoidRemove) {
                            echo "<td width='1%'>";
                            echo "<a class='text-red btn' title='remove this item' data-toggle='tooltip' data-placement='left' onclick=\"document.getElementById('result').src='" . $iSpec2['removeTarget'] . "';\"><span class='glyphicon glyphicon-remove'></span></a>";
                            echo "</td>";
                        }
                        //endregion
...
//                arrPrint($addRowHiddens[$k]);
        ...
        if (sizeof($items2_sum) > 0) {
            if (sizeof($itemLabels2_sum) && (is_array($itemLabels2_sum))) {
                echo "<div class='panel no-margin bborder-cek' style='padding-top:10px;'>"; // anakan table
                echo "<div class='text-left'><h4>SUMMARY SUPPLIES</h4></div>"; // anakan table
                echo "<table class='table table-condensed table-striped no-padding no-border'>";
...
//                        }
        ...
        $anu = "- <strong>belum ada item/produk yang dipilih.</strong> -<br>";
        $anu .= "<small>silahkan pilih item/produk pada formulir disamping.</small><br>";
        echo tplNoData($anu);
