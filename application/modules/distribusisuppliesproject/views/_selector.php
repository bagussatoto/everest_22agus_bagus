...
//                    arrPrintWebs($iSpec['is_spk_tmb']);
                    if (strlen($iSpec['nama']) > 0) {
                        ...
                        $isGrey = $iSpec['qty_distribute_persentase']*1 >= 100 ? "is-grey" : "";
                        $isGreyTitle = $iSpec['qty_distribute_persentase']*1 >= 100 ? "SPK ini (".$iSpec['no_spk'].")sudah komplit distribusinya..." : "";
                        ...
                        if (isset($socketURL[$iSpec['id']]) && strlen($socketURL[$iSpec['id']]) > 3) {
                            if($iSpec['qty_distribute_persentase']*1 >= 100){
                        ...
                        $strContent .= "
                            <div style='margin-bottom: 0px;' class='progress active bg-red'>
                ...
                $strContent .= "- tidak ada items yang cocok -<br>REASON: SPK kemungkinan tidak ada suppliesnya...??<br>SPK sudah selesai dikerjakan (QC)...??";
                $strContent .= "</div>";
            }
        }
        else {
    ...
    case "viewReturnTasklist":
        ...
        if (isset($items) && sizeof($items) > 0) {
            if (is_array($items) && (sizeof($items) > 0)) {
                ...
                $groupBiaya = array();
                foreach ($items as $iSpec) {
                    $groupBiaya[$iSpec['biaya_id']][] = $iSpec;
                    $nameBiaya[$iSpec['biaya_id']] = $iSpec['biaya_nama'];
                }
//                arrPrint($groupBiaya);
//                $groupBiaya = array();

                foreach($groupBiaya as $biyID => $arrSpec){

                    $strContent .= "<li class='list-group-item bg-blue'>";
                    $strContent .= "<div class='text-bold text-uppercase'>".$nameBiaya[$biyID]."</div>";
                    $strContent .= "</li>";

                    foreach($arrSpec as $iSpec){
                        if (strlen($iSpec['nama']) > 0) {
                            if (isset($iSpec['minValue'])) {
                                $defaultValue = $iSpec['minValue'];
                                $defVal = "&minValue=$defaultValue";
                            ...
                            if (isset($socketURL[$iSpec['id']]) && strlen($socketURL[$iSpec['id']]) > 3) {
                                ...
                                switch ($cCode) {
                                    default:
                                        if (isset($iSpec['target']) && ($iSpec['target'] != NULL)) {
                                            $strContent .= "<a href='javascript:void(0);' onclick=\"document.getElementById('result').src='" . $iSpec['target'] . "?biaya_id=$biyID&spk=".$iSpec['no_spk']."&selector&id=" . $iSpec['id'] . "$defVal';$mb_js\">";
                                        }
                                        break;
                                }
                            }
                            ...
                            $strContent .= isset($iSpec['nama']) ? "<div style='font-weight: 700;font-size: 16px;' class='text-uppercase'>" . $iSpec['nama'] . "</div>" : "";
                            $strContent .= isset($iSpec['label']) ? "<div>" . $iSpec['label'] . "</div>" : "";

//                            $strContent .= "
//                            <div style='margin-bottom: 0px;' class='progress active bg-red'>
//                                <div class='progress-bar progress-bar-success progress-bar-striped' role='progressbar' aria-valuenow='".$iSpec['qty_distribute_persentase']."' aria-valuemin='0' aria-valuemax='100' style='width: ".$iSpec['qty_distribute_persentase']."%'>
//                                     <i class='fa fa-truck'></i> &nbsp; <span style='font-size: 14px;' class='text-bold'>".number_format($iSpec['qty_distribute_persentase'], 0)."</span> <i class='fa fa-percent'></i>
//                                </div>
//                            </div>
//                        ";
...
//                        }
//
////                        if( !isset($groupBiaya[$iSpec['biaya_id']]) ){
////                            $strContent .= "<li class='list-group-item'>";
////                            $strContent .= "<div class='text-bold text-upprecase'>".$iSpec['biaya_nama']."</div>";
////                            $strContent .= "</li>";
////                            $groupBiaya[$iSpec['biaya_id']] = $iSpec['biaya_nama'];
////                        }
//
//                        $strContent .= "<li class='list-group-item'>";
//                        if ($socketURL[$iSpec['id']] != "") {
