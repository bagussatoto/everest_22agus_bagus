...
// cekHitam($mdlName);
            ...
            foreach ($tmpO as $row) {
                ...
                if (strlen($tmpName) > 1) {
                    ...
                    $items[] = array(
                        ...
                        "label" => $newTmpName,
                        "target" => $processor,
                        "label_view" => isset($row->$pihakView) ? $row->$pihakView : "$pihakView",
    ...
    public function selectPihakProjek()
    {
        ...
        $fields = $this->configUi[$jenisTr]['selectorFields'];
        $pihakFilters = isset($this->configUi[$jenisTr]['pihakProjekFilters']) ? $this->configUi[$jenisTr]['pihakProjekFilters'] : array();
        $pihakAddStaticEntry = isset($this->configUi[$jenisTr]['pihakAddStaticEntry']) ? $this->configUi[$jenisTr]['pihakAddStaticEntry'] : array();
        $pihakProjekViewedFields = isset($this->configUi[$jenisTr]['pihakProjekViewedFields']) ? $this->configUi[$jenisTr]['pihakProjekViewedFields'] : array();
...
//                        $o->addFilter($f_ex[0] . "='" . $this->session->login[$f_ex[1]] . "'");
        ...
        $processor = base_url() . $this->modul . "/" . $this->configUi[$jenisTr]['pihakProjekProcessor'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['pihakModelProjek'];
        if (sizeof($tmpO) > 0) {
            foreach ($tmpO as $row) {
                $tmpName = isset($row->$selectColumn) ? $row->$selectColumn : "";
...
//arrPrintPink($items);
    ...
    public function selectPihakProjekTaskList()
    {
        ...
        $fields = $this->configUi[$jenisTr]['selectorFields'];
        $pihakFilters = isset($this->configUi[$jenisTr]['pihakProjekTaskListFilters']) ? $this->configUi[$jenisTr]['pihakProjekTaskListFilters'] : array();
        $pihakAddStaticEntry = isset($this->configUi[$jenisTr]['pihakAddStaticEntry']) ? $this->configUi[$jenisTr]['pihakAddStaticEntry'] : array();
        $pihakProjekViewedFields = isset($this->configUi[$jenisTr]['pihakProjekTasklistViewedFields']) ? $this->configUi[$jenisTr]['pihakProjekTasklistViewedFields'] : array();
        $key = isset($_GET['search']) ? $_GET['search'] : "";
        ...
        $arrCekKolom = array(
            ...
            "name" => array("pihakExternName"),
            "nama" => array("pihakExternName"),
            "produk_nama" => array("pihakTasklistName"),
            "employee_nama" => array("pihakExternName"),
        );
...
//        $this->db->limit(20);
        ...
        if($mdlName=="MdlTasklistProject"){
            ...
            $tmpO__ = $o->lookupByKeyword($key)->result();

            $tmpO = array_merge($tmpO, $tmpO__);
        }

        // showLast_query("biru");
        // arrPrintWebs($tmpO);

        $processor = base_url() . $this->modul . "/" . $this->configUi[$jenisTr]['pihakProjekProcessorTasklist'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['pihakModelProjekTaskList'];

        if (sizeof($tmpO) > 0) {
            foreach ($tmpO as $row) {
                $spk = $row->no_spk; //002/SPK-TMB/108/979/002/XI/2025 | 166/SPK-INT/108/979/166/X/2025
                $is_spk_tmb = (strpos($spk, 'SPK-TMB') !== false) ? 1 : 0;
