...
//                "stepCode" => isset($this->configUiJenis['steps'][$stepNum]['target']) ? $this->configUiJenis['steps'][$stepNum]['target'] : 0,
        ...
        echo "top.document.getElementById('itemKeyword').click();";
        echo "  top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=$id');";
        echo "</script>";
    }

    public function selectProjekTasklist()
    {
        $cCode = "_TR_" . $this->jenisTr;
        $id = isset($_GET['id']) ? $_GET['id'] : 0;
        ...
        $b = new $mdlName();
        if( isset($_GET['spk']) ){
            $this->db->where("no_spk", trim($_GET['spk']));
        }
        $tmpB = $b->lookupByID($id)->result();
...
//        arrPrint($tmpB);
        if(empty($tmpB)){
            ...
            $b = new MdlTasklistProjectTambahan();
            if( isset($_GET['spk']) ){
                $this->db->where("no_spk", trim($_GET['spk']));
            }
            $tmpB = $b->lookupByID($id)->result();
...
//            "nama" => array("pihakProjekName"),
        ...
        if (sizeof($tmpB) > 0) {
            ...
            $pihakProjekID = $tmpB[0]->produk_id;
            $pihakCabangID = $tmpB[0]->cabang_id;
            $fase_id = $tmpB[0]->fase_id;
            $sub_fase_id = $tmpB[0]->sub_fase_id;

            $ambil_angka_depan_spk = explode("/", $tmpB[0]->no_spk)[0];
            $gudang_project = "$pihakProjekID" . "$fase_id" . "$sub_fase_id" . "$ambil_angka_depan_spk";


            $_SESSION[$cCode]['main']['pihakProjekTasklistID'] = $id;
            $_SESSION[$cCode]['main']['pihakProjekTasklistName'] = isset($tmpB[0]->produk_nama) ? $tmpB[0]->produk_nama : "";
            $_SESSION[$cCode]['main']['pihakProjekTasklistSPK'] = isset($tmpB[0]->no_spk) ? $tmpB[0]->no_spk : "";
            $_SESSION[$cCode]['main']['pihakProjekNoSubSpk'] = $sub_spk;
            $_SESSION[$cCode]['main']['pihakProjekTasklistEmployeeId'] = isset($tmpB[0]->employee_id) ? $tmpB[0]->employee_id : "";
            $_SESSION[$cCode]['main']['pihakProjekTasklistEmployeeName'] = isset($tmpB[0]->employee_nama) ? $tmpB[0]->employee_nama : "";
            $_SESSION[$cCode]['main']['pihakProjekTasklistFaseID'] = isset($tmpB[0]->fase_id) ? $tmpB[0]->fase_id : "";
            $_SESSION[$cCode]['main']['pihakProjekTasklistSubFaseID'] = isset($tmpB[0]->sub_fase_id) ? $tmpB[0]->sub_fase_id : "";
            $_SESSION[$cCode]['main']['gudangProject2ID'] = $gudang_project;
            $_SESSION[$cCode]['main']['cabangProjectID'] = $pihakCabangID;

            if (isset($tmpB[0]->produk_nama)) {
                $tmpPihakName = $tmpB[0]->produk_nama;
            }
            if (isset($tmpB[0]->no_spk)) {
                $tmpNoSPK = $tmpB[0]->no_spk;
            }
        ...
        echo "<script>";
        echo "top.document.getElementById('pihakProjekTaskListName').value='" . $tmpPihakName . " ($tmpNoSPK)';";
        echo "top.document.getElementById('pilihan_project_tasklist').innerHTML='';";
        echo "</script>";
    }
