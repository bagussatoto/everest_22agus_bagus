...
class _shoppingCart extends Modul_Controller
{
    ...
    public function viewCart()
    {
        ...
        $itemLabels2_sum = isset($this->configUi[$this->jenisTr]['shoppingCartFields2_sum'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartFields2_sum'][$stepNumber] : array();
        ...
        $shoppingCartItemMode = isset($this->configUi[$this->jenisTr]['shoppingCartItems'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartItems'][1] : array();
        //----------------------------------------------
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "jml_wo" => isset($iSpec['jml_wo']) ? $iSpec['jml_wo'] : "",
                        ...
                        "biaya_nama" => isset($iSpec['biaya_nama']) ? $iSpec['biaya_nama'] : "",
                    );
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "distribusifg/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "distribusifg/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
                    //----------------------
//                    if (sizeof($shoppingCartItemMode) > 0) {
//                        if (isset($shoppingCartItemMode["enabled"]) && ($shoppingCartItemMode["enabled"] == true)) {
//                            $source_jml = makeValue($shoppingCartItemMode["source"], $iSpec, $iSpec, 0);
//                            $target_jml = makeValue($shoppingCartItemMode["target"], $iSpec, $iSpec, 0);
//                            if ($source_jml == $target_jml) {
//                                $tmp["disabled"] = NULL;
//                                $tmp["background_color"] = "#a3ffa3";
//                                $shoppingcart_warning = $shoppingCartItemMode["label_warning"];
//                            }
//                            else if($source_jml < $target_jml){
//                                $tmp["disabled"] = NULL;
//                                $tmp["background_color"] = "yellow";
//                                $shoppingcart_warning = $shoppingCartItemMode["label_warning"];
//                            }
//                            else{
//                                $tmp["disabled"] = NULL;
//                                $tmp["background_color"] = NULL;
//                            }
//                        }
//                    }
                    //----------------------
        ...
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items2'])) {
                $no = 0;
                $sesItems = $_SESSION[$cCode]['items2'];
                foreach ($_SESSION[$cCode]['items2'] as $biaya_id => $iSpecData) {
                    foreach ($iSpecData as $id => $iSpec) {
                        $pihakID = $id;
                        $no_spk = $iSpec["no_spk"];
                        $no++;
                        $pihakName = isset($sesMains['pihakName']) ? $sesMains['pihakName'] : "";
                        ...
                        $tmp['sub_harga'] = isset($iSpec['sub_harga']) ? $iSpec['sub_harga'] : 0;
                        //direwrite karena ada transaksi lama yg non modul, seingga selector nyasar
                        $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "distribusifg/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&biaya_id=" . $biaya_id . "&spk=$no_spk&newQty=";
                        $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "distribusifg/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id . "&biaya_id=" . $biaya_id;
                        //----------------------
                        if (sizeof($shoppingCartItemMode) > 0) {
                            if (isset($shoppingCartItemMode["enabled"]) && ($shoppingCartItemMode["enabled"] == true)) {
                                $source_jml = makeValue($shoppingCartItemMode["source"], $iSpec, $iSpec, 0);
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);top.$('#result').load($selectorTarget);}\">";
        ...
        $data = array(
            ...
            //--------------------------------------------
            "shoppingcart_warning" => isset($shoppingcart_warning) ? $shoppingcart_warning : "",
        );
    ...
    public function reset()
    {
        ...
        if (isset($_SESSION[$cCode]['items2'])) {
            ...
            foreach ($_SESSION[$cCode]['items2'] as $biyID => $itemBiy) {
                foreach ($itemBiy as $id => $item) {
...
//                            cekHijau($pre__gudang_id);
                        ...
                        $this->db->trans_start();
//cekBiru($mdlName);
                        // cekBiru("biayaID::" . $biyID);
                        // cekBiru("biayaID::" . $mdlName);
                        $c = new $mdlName();
                        $array_hold_sebelumnya = $c->cekLoker($pre__cabang_id, $id, "hold", $this->session->login['id'], "0", $pre__gudang_id, $biyID);
...
//        echo "top.fillBoxes();";
        ...
        echo "if( top.$('#shopping_cart').length > 0 ){
