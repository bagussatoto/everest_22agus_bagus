        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    $no++;
                    $arrItemsRegistries[$id] = isset($itemsRegistries[$id]) ? $itemsRegistries[$id] : array();
                    $iSpec = $iSpec + $arrItemsRegistries[$id];
//                    arrPrintKuning($arrItemsRegistries[$id]);
...
//                arrPrint($sumRows2);
            ...
            if (isset($receiptEfakturFields) && sizeof($receiptEfakturFields) > 0) {
                $no = 0;
                $contenStr5 .= "<div class='table-responsive' style='border:0px solid red;'>";
                $contenStr5 .= "<table class='table table-bordered table-condensed' style='width: 100%;'>";
                $contenStr5 .= "<tr bgcolor='#f5f5f5'>";
                foreach ($receiptEfakturFields as $key => $label) {
                    $contenStr5 .= "<th class='text-muted' style='font-weight:bold;'>";
                    $contenStr5 .= $label;
                    $contenStr5 .= "</th>";
                }
                $contenStr5 .= "</tr>";
                ...
                if (isset($formulirFaktur) && (sizeof($formulirFaktur)) > 0) {
                    foreach ($formulirFaktur as $ctt => $fSpec) {
                        ...
                        foreach ($receiptEfakturFields as $fff => $f_labels) {
                            ...
                            $contenStr5 .= "<td id='td_$fff'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                            $contenStr5 .= "</td>";
                    ...
                    if (sizeof($formulirFaktur) > 1) {
                        ...
                        $contenStr5 .= "<tr style='background-color:$bgcolor;'>";
                        foreach ($receiptEfakturFields as $fff => $f_labels) {
                            $value = isset($sub_total_bawah[$fff]) ? formatField($fff, $sub_total_bawah[$fff]) : "";
                            $contenStr5 .= "<td id='td_$fff' class='text-bold' style='font-size:15px;'>$value";
                            $contenStr5 .= "</td>";
                        }
                        $contenStr5 .= "</tr>";
                    }
                    ...
                    foreach ($receiptEfakturFields as $key => $label) {
                        ...
                        $contenStr5 .= "<td>";
                        $contenStr5 .= formatField_he_format($key, $val);
                        $contenStr5 .= "</td>";
                    }
                    $contenStr5 .= "</tr>";
                }
