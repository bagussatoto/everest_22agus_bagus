        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                if (sizeof($valValidate_items) > 0) {
                    //                    $this->debugArrPrint($valValidate_items);
                ...
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    ...
                    if (sizeof($tmpC) > 0) {
                        ...
                        foreach ($tmpC as $row) {
                            ...
                            //                            $this->debugArrPrint($array_hold_sebelumnya);
                            //                            mati_disini();
                            ...
                            if (!$this->finalizeSafeTransaction("Gagal saat menyimpan perubahan produk.")) {
                                return;
                            }
                ...
                if (sizeof($connectedDiscountConfig) > 0) {
                    if ($connectedDiscountConfig['enabled'] == 1) {
                        ...
                        foreach ($tmpDr as $drSpec) {
                            ...
                            //                            $this->debugArrPrint($tmpSr);
                            foreach ($tmpSr as $srSpec) {
                                $this->debugArrPrint($srSpec);
...
//        }
    ...
    public function multiSelect()
    {
        ...
        $arrItems = $this->decodeSafeBase64ArrayParam('items');
        // id_produk => qty

        $arrTrID = $this->decodeSafeBase64ArrayParam('trs');

        $arrMain = $this->decodeSafeBase64ArrayParam('main');
