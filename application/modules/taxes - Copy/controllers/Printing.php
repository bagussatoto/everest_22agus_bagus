...
class Printing extends Modul_Controller
{
    ...
    public function viewProformaReceipt()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            // $this->debugArrPrint($cc);
            // $this->debugArrPrint($cb);
            if (sizeof($tmpTop)) {
            ...
            foreach ($tmpTr as $row) {
                ...
                //                $this->debugArrPrint($subAmountConfig);
                ...
                //                $this->debugArrPrint($rawItems);
                //                $this->debugArrPrint($childTableInValueParams[$row->produk_id]);
                $tmp["subtotal"] = $subtotal;
                //                $this->debugArrPrint($tmp);
                //                $this->debugArrPrint($childTableInValueParams[$id]);
                //                $this->debugArrPrint($childTableInValueParams);
                ...
                //$this->debugArrPrint($receiptDetailFieldsReplacerConfig);
        ...
        //         $this->debugArrPrint($globalVars);
        //endregion
        //        $this->debugArrPrint($masterTableInParams);
                            ...
                            foreach ($eSpec as $key => $val) {
                                ...
                                if (is_array($vTmp)) {
                                    ...
                                    if (isset($relElementType[$vTmp])) {
                                        // $this->debugArrPrint($relElementType[$vTmp]);
        ...
        //region downpayment
        //        $this->debugArrPrint($masterTableInValueParams);
        ...
        // $this->debugArrPrint($elementFixedConfig);
        ...
        //        $this->debugArrPrint($itemSubTotal);
    ...
    public function viewReceipt()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            //endregion
            //cekBiru(count($tmpTr));
            //$this->debugArrPrint($itemsRegistries);
            //$this->debugArrPrint($main);
            //$this->debugArrPrint($masterTableInValueParams);

            $itemLabels = sizeof($receiptDetailFields) > 0 ? $receiptDetailFields : $itemLabels;
            $itemLabels2 = $receiptDetailFields2;
            if ($mainJenis == "paket") {
                //                $this->debugArrPrint($itemLabels);
                $rawItems[] = array(
                    "produk_kode" => "",
        ...
        if (isset($globalVars["ids_his"])) {
            ...
            // $this->debugArrPrint($tmpTr[0]->ids_prev);
                    ...
                    if (isset($globalVars[$key])) {
                        if (is_array($globalVars[$key])) {
                            if (sizeof($globalVars[$key])) {
                                //                                $this->debugArrPrint($globalVars[$key]);
                                $ii_result = "";
        ...
        //        $this->debugArrPrint($itemNumLabels);
    ...
    public function viewReceiptReg()
    {
        ...
        $gunggunganModeTransaksi = $tmpTr[0]->gunggungan_mode;
        $cabang_id_transaksi = $tmpTr[0]->cabang_id;
...
//        $this->debugShowLastQuery("biru");
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            switch($this->jenisTr){
                case "115":
                    $this->configUi[$this->jenisTr] = $configUiMasterModulJenis = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiUi");
                    $this->configCore[$this->jenisTr] = $configCoreMasterModulJenis = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiCore");
                    $this->configLayout[$this->jenisTr] = $configLayoutMasterModulJenis = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiLayout");
                    $this->configValues[$this->jenisTr] = $configValuesMasterModulJenis = loadConfigModulJenis_he_misc($this->jenisTr, "coTransaksiValues");
                    $modul_transaksi = $this->config->item("heTransaksi_ui")[$this->jenisTr]["modul"];
                    $itemLabels = isset($this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum] : array();
//                    cekMerah("[$modul_transaksi]");
//                    arrPrintCyan($configLayoutMasterModulJenis);
            ...
            if (in_array($this->jenisTr, $this->jenisException)) {
                ...
                // $this->debugArrPrint($injectedFields);
            ...
            if (sizeof($injectedFields) > 0) {
                ...
                // $this->debugArrPrint($valMainInject);
                // matiHere();
            }

            $itemLabels = (isset($itemLabels)) ? $itemLabels : $receiptDetailFields;
            $itemLabels2 = $receiptDetailFields2;
            ...
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582")) {
                ...
                foreach ($itemsRegistries as $tmpItem) {
                    ...
                    //                    $this->debugArrPrint($injectParam);
...
//        arrPrintKuning($itemLabels);
        ...
        //        $this->debugArrPrint((array)$mainProp);
        //        $this->debugArrPrint($globalVars);
        //endregion
        ...
        //        $this->debugArrPrint($fixedElements['fixedElements']);
        ...
        //region add fields2 main
        $receiptEfakturFields = $addMainData2Fields = isset($this->configLayout[$this->jenisTr]["receiptAddMain"][$row->step_number]) ? $this->configLayout[$this->jenisTr]["receiptAddMain"][$row->step_number] : array();
        $addMainData2 = array();
        if (sizeof($addMainData2Fields) > 0) {
            foreach ($addMainData2Fields as $keys => $keysLabel) {
                $addMainData2[$keys] = isset($globalVars[$keys]) ? $globalVars[$keys] : "";
        ...
        //--------------------
        $shopingCartFaktur = isset($this->configUi[$tmpTr[0]->jenis_master]['addMainSource'][$row->step_number]) ? $this->configUi[$tmpTr[0]->jenis_master]['addMainSource'][$row->step_number] : array();
        ...
        //--------------------
        if ($gunggunganModeTransaksi == 1) {
            switch ($tmpTr[0]->jenis_master) {
                case "110":
                    $itemLabels = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFieldsGunggung'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptDetailFieldsGunggung'][$row->step_number] : array();
                    break;
                case "111":
                    $itemLabels = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptMultiItemFields'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptMultiItemFields'][$row->step_number] : array();
                    $itemNumLabels = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptMultiItemNumFields'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptMultiItemNumFields'][$row->step_number] : array();
                    $itemLabels = $itemLabels + $itemNumLabels;
                    break;
            }


//            arrPrintCyan($itemLabels);
            if (isset($itemsRegistries)) {
                foreach ($itemsRegistries as $id => $iiSpec) {
                    foreach ($itemLabels as $key => $labels) {
                        if (isset($iiSpec[$key])) {
                            $items[$id][$key] = $iiSpec[$key];
                        }
                    }
                }
            }
...
//        arrPrintKuning($receiptSumFieldsLayout);
//        arrPrintKuning($signHeader);
...
//        arrPrintKuning($itemsRegistries);
        $data = array(
            ...
            "receiptEfakturFields" => isset($receiptEfakturFields) ? $receiptEfakturFields : array(),
    ...
    public function viewReceiptCashIn()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            //           $this->debugArrPrint($receiptDetailFields);
            //region tabel2 tarikan untuk kolom2 nilai (hpp, ppn, dll)
            $tmpVal_main = $tr->lookupMainValuesByTransID($trID)->result();
            $tmpVal_detail = $tr->lookupDetailValuesByTransID($trID)->result();
            //endregion
            ...
            //            $this->debugArrPrint($main2);
            //            matiHere();
            $itemLabels = $receiptDetailFields;
            $itemLabels2 = $receiptDetailFields2;
            //            $this->debugArrPrint($itemLabels2);
            foreach ($tmpTr as $row) {
            ...
            foreach ($tmpTri as $row2) {
                ...
                //                $this->debugArrPrint($childTableInValueParams2[$id]);
                if ($subAmountConfig != null) {
                    $subAmountConfig = str_replace("jml", "produk_ord_jml", $subAmountConfig);
                    $subAmountConfig = str_replace("produk_ord_produk_ord_jml", "produk_ord_jml", $subAmountConfig);
        ...
        if (isset($globalVars['nomer_top'])) {
            ...
            //            $this->debugArrPrint($globalVars['countersTrIDS']);
            //            $addTopCounter = isset($globalVars['countersTop']['stepCode']) ? "-" . digit_5($globalVars['countersTop']['stepCode']) : "";
            // $addTopCounter = isset($globalVars['countersTop']['stepCode|placeID']) ? "-" . digit_5($globalVars['countersTop']['stepCode|placeID']) : "";
            // $globalVars['nomer_top'] = $globalVars['nomer_top'] . $addTopCounter;
        ...
        //region fixed signature
        $fixedSignConfig = isset($this->configLayout[$tmpTr[0]->jenis_master]['fixedSignatures'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['fixedSignatures'][$currentStepNum] : array();
        //       $this->debugArrPrint($fixedSignConfig);
        if (sizeof($fixedSignConfig) > 0) {
            foreach ($fixedSignConfig as $key => $eSpec) {
                if (substr($eSpec['label'], 0, 1) == ".") {
                    $label = str_replace(".", "", $eSpec['label']);
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            if ($mainJenis == "paket") {
                ...
                foreach ($itemsReg as $ctr => $row) {
                    ...
                    //                    $rowCtr = $id;
                    $rowCtr = $ctr;
                    //                    $this->debugArrPrint($itemsRegistries[$id]);
                    if ($itemsRegs == true) {
    ...
    public function viewSmallReceipt()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            //            $this->debugArrPrint($masterGates);
