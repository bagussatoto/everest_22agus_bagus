...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $shopingCartReturnValidators = isset($this->configUi[$this->jenisTr]["shopingCartReturnValidators"][1]) ? $this->configUi[$this->jenisTr]["shopingCartReturnValidators"][1] : array();
        ...
        //----------------------
        if (sizeof($shopingCartReturnValidators) > 0) {
            if ($shopingCartReturnValidators["enabled"] == true) {
                $no = 0;
                foreach ($_SESSION[$cCode]['items'] as $xid => $iSpec) {
                    $no++;
                    $nama = $iSpec["nama"];
                    $barcode = $iSpec["barcode"];
                    $stok = $iSpec["stok"];
                    $jml = $iSpec["jml"];
                    if ($jml > $stok) {
                        $msg = "Stok produk $nama $barcode tidak cukup untuk direturn. Silahkan diperiksa baris nomer $no.";
                        $errMsgs[] = $msg;
                    }
                }
            }
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
    ...
    public function viewResume()
    {
        ...
        $referensiSoID = $tmp1[0]->reference_id;
        $referensiSoNomer = $tmp1[0]->reference_nomer;
        $referensiSoID_top = $tmp1[0]->reference_id_top;
        $referensiSoNomer_top = $tmp1[0]->reference_nomer_top;
...
//        showLast_query("biru");
        ...
        if ($referensiSoID > 0) {
            ...
            $trep->addFilter("link_id>0");
            $trep->addFilter("id>$indTransksiID");
            $trep->addFilter("reference_id_top='$referensiSoID_top'");
            $tmpEP = $trep->lookupAll()->result();
//            showLast_query("biru");
            if (sizeof($tmpEP) > 0) {
//                arrPrint($tmpEP);
                foreach ($tmpEP as $ii => $row) {
                    ...
                    $counterGlobal = $counterDecode['stepCode|placeID'][$counterjenis];
                    $cGlobals = digit_5($counterGlobal);
                    $epKey = "sign_" . $row->id . "_" . $row->step_number;
                    if (!array_key_exists($epKey, $epValues)) {
                        $epValues[$epKey] = array(
                            "title" => isset($this->configUi[$row->jenis_master]['steps'][$row->step_number]['label']) ? $this->configUi[$row->jenis_master]['steps'][$row->step_number]['label'] : "",
                            "cabang_nama" => $row->cabang_nama,
                            "label" => "AUTO " . $row->jenis_label,
                            "dtime" => $row->dtime,
                            "nomer" => formatField_he_format("nomer_nolink", $row->nomer),
                            "nomer_orig" => $row->nomer,
                            "oleh" => $row->oleh_nama,
        ...
        if (sizeof($signArray) > 0) {
            ...
            foreach ($signArray as $k => $sSpec) {
                ...
                $background_color = isset($sSpec["background_color"]) ? $sSpec["background_color"] : "";

                $signStr .= "<tr style='background-color:$background_color;'>";
...
//                    $dTmp = $m->lookUpDetailTransaksi($transaksiTopID);
        ...
        $data = array(
            ...
            "items" => $lItems,
            "itemsShadow" => isset($lItems_shadow) ? $lItems_shadow : array(),
...
\n\n
    ...
    public function replacerSession()
    {
        $this->jenisTr = $this->uri->segment(4);
        $cCode = "_TR_" . $this->jenisTr;
        $pre_po_data = $_SESSION["PRE_PO"];
        $arrMainData = $pre_po_data["main"];
        $delivery_address = $arrMainData["requestReferenceDelivery"];
//        arrPrint($delivery_address);

        // region replace delivery_details
        if (isset($_SESSION[$cCode]["main_elements"])) {
            $_SESSION[$cCode]["main_elements"]["deliveryDetails"] = $delivery_address;

            $_SESSION[$cCode]["main"]["deliveryDetails"] = $delivery_address["key"];
            $_SESSION[$cCode]["main"]["deliveryDetails__label"] = $delivery_address["labelValue"];
            $_SESSION[$cCode]["main"]["deliveryDetails__extern_name"] = $delivery_address["labelValue"];
            $contens_decode = blobDecode($delivery_address["contents"]);
            foreach ($contens_decode as $contents_key => $contents_val) {
                $_SESSION[$cCode]["main"]["deliveryDetails__" . $contents_key] = $contents_val;
            }
        }
        // endregion replace delivery_details
