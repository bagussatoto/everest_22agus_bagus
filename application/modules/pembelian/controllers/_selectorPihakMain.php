...
//                        "label" => $newTmpName,
    ...
    public function selectPihakGudang()
    {
...
//        $mdlName = $this->uri->segment(5);
        $fields = $this->configUi[$jenisTr]['selectorFields'];
        $mdlName = isset($this->configUi[$jenisTr]['pihakExternMain']) ? $this->configUi[$jenisTr]['pihakExternMain'] : NULL;
        $pihakFilters = isset($this->configUi[$jenisTr]['pihakExternFilters']) ? $this->configUi[$jenisTr]['pihakExternFilters'] : array();
        $pihakExternFiltersIN = isset($this->configUi[$jenisTr]['pihakExternFiltersIN']) ? $this->configUi[$jenisTr]['pihakExternFiltersIN'] : array();
        $pihakAddStaticEntry = isset($this->configUi[$jenisTr]['pihakAddStaticEntry']) ? $this->configUi[$jenisTr]['pihakAddStaticEntry'] : array();
        $selectorFields = isset($this->configUi[$jenisTr]['pihakMainViewedFields']) ? $this->configUi[$jenisTr]['pihakMainViewedFields'] : array();
        $configPair = isset($this->configUi[$jenisTr]["pihakMainPair"]) ? $this->configUi[$jenisTr]["pihakMainPair"] : array();
        $key = isset($_GET['search']) ? $_GET['search'] : "";
...
////                }
//            }
            $o = makeFilter($pihakFilters, $_SESSION[$cCode]["main"], $o);
        }
        if(sizeof($pihakExternFiltersIN)>0){
            $fval = array();
            $pihakExternFiltersINValue = $pihakExternFiltersIN["value"];
            $pihakExternFiltersINKey = $pihakExternFiltersIN["key"];
            foreach ($pihakExternFiltersINValue as $xxx => $fff){
                $fval[$xxx] = makeValue($fff, $_SESSION[$cCode]["main"], $_SESSION[$cCode]["main"], 0);
            }
            $o->addFilter("$pihakExternFiltersINKey in ('".implode("','", $fval)."')");
        }
        $this->db->limit(20);
        $tmpO = $o->lookupByKeyword($key)->result();
        showLast_query("biru");
...
//                        $o->addFilter($f_ex[0] . "='" . $this->session->login[$f_ex[1]] . "'");
        ...
        $processor = MODUL_PATH . $this->configUi[$jenisTr]['pihakExternProcessor'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['pihakExternMain'];
        cekHere("$processor");
        if (sizeof($tmpO) > 0) {
            if (sizeof($configPair) > 0) {
                foreach ($tmpO as $row) {
