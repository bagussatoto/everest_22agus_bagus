...
class _followupLiveEdit extends Modul_Controller
{
    ...
    public function removeItem()
    {
        ...
        //----------------------------
        $closedRequest = isset($this->configUi[$this->jenisTr]['closedRequest'][$intoStep]) ? $this->configUi[$this->jenisTr]['closedRequest'][$intoStep] : array();
...
//        echo "<script>$actionTarget</script>";
    ...
    public function updateItemField()
    {
        ...
        $allowScaner = isset($this->configUi[$this->jenisTr]['allowScaner'][$intoStep]) ? $this->configUi[$this->jenisTr]['allowScaner'][$intoStep] : false;
        $allowEditSerial = isset($this->configUi[$this->jenisTr]['serialLiveEdit'][$intoStep]) ? $this->configUi[$this->jenisTr]['serialLiveEdit'][$intoStep] : array();
        //----------------------------
        $closedRequest = isset($this->configUi[$this->jenisTr]['closedRequest'][$intoStep]) ? $this->configUi[$this->jenisTr]['closedRequest'][$intoStep] : array();
...
//            }
                $produk_jenis = $tmp["kategori_nama"];
...
//                            if (isset($tmp[$dval]) && ($tmp[$dval] != NULL) && ($tmp[$dval] != 0)) {
                            if (isset($tmp[$dval]) && (strlen($tmp[$dval]) >= 3)) {
//                        $_SESSION[$cCode]['items2'][$produk_id][$rows->$dval] = array();
                    ...
                    //handle serial 1
                    $jml_serial = $tmp["jml_serial"];
//                    $tmp['jml_serial'] = $jml_serial;
                    $tmp['scan_mode'] = $jml_serial > 0 ? "serial" : "simple";
                    if ($jml_serial * 1 == 1) {
                        $d_kode = $tmp["produk_kode"];
                        //$_SESSION[$cCode]['items2'][$produk_id][$d_kode] = array();

                        $tmp[$d_kode] = $_SESSION[$cCode]['items'][$id]["jml"];
//                                                                matiHere($jml_serial." ".$_SESSION[$cCode]['items'][$id]["jml"]);
                    }
                }
            ...
            if ($val < $_SESSION[$cCode]['items'][$id][$key]) {
                ...
                if (in_array($key_triger, $arrPartialKey)) {
                    $_SESSION[$cCode]['main']['partial'] = 1;
                }
                //----------------------------------------------------------------
            }
        ...
        if (isset($_SESSION[$cCode]['items_child'][$id]) && sizeof($_SESSION[$cCode]['items_child'][$id])) {
...
//            arrPrint($_SESSION[$cCode]['items_child'][$id]);
        ...
        //region untuk koreksi serial
        if (count($allowEditSerial) > 0) {

            $srcGate = $allowEditSerial["srcGate"];
            $srcTarget = $allowEditSerial["srcTarget"];
            $srcSerialMaster = $allowEditSerial["srcSerialMaster"];
            $srcSerialMasterItems = $allowEditSerial["srcSerialMasterItems"];
            $keyTriger = $allowEditSerial["keyTriger"];

            // Validasi
            if (isset($_SESSION[$cCode][$srcGate][$id]) &&
                $key_triger == $keyTriger) {
                // Inisialisasi array cancel
                if (!isset($_SESSION[$cCode]['items7_cancel'])) {
                    $_SESSION[$cCode]['items7_cancel'] = array();
                }

                // Ambil qty lama
                $qtyLama = $_SESSION[$cCode][$srcSerialMaster][$id]['valid_qty'];
                $qtyBaru = $val;
//                matiHere($qtyLama."vs".$qtyBaru);
                // Lakukan koreksi
                if ($qtyLama > $qtyBaru) {

                    $_SESSION[$cCode]['items7_cancel'] = $this->koreksiSerialWithHistory(
                        $_SESSION[$cCode],
                        $cCode,
                        $allowEditSerial,
                        $id,
                        $qtyBaru,
                        $_SESSION[$cCode]['items7_cancel']
                    );
                }
            }
...
//        matiHere();
    ...
    public function updateMainField()
    {
        ...
        $mb = New MobileDetect();
        $isMob = $mb->isMobile();
        if ($isMob == true || isset($_GET['from']) && $_GET['from'] == "mobile") {
            $link_mobile = MODUL_PATH . "FollowUp/shopingCart/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $this->uri->segment(6) . "/" . $this->uri->segment(7);
            $actionTarget = "top.$('#shopingcart_mobile').load('$link_mobile')";
            echo "<script>top.close_holdon();$actionTarget</script>";
        }
...
//
    ...
    public function diskonSupplierOption()
    {
    ...
    public function gantiGudangTujuan()
    {
        $this->jenisTr = $this->uri->segment(4);
        $fromStep = $this->uri->segment(7);
        $intoStep = $this->uri->segment(6);
        ...
        switch ($key){
            case "gantiGudangTujuan":
                $_SESSION[$cCode]['main']['gudangTujuan'] = $val;
                $_SESSION[$cCode]['main']['gudangTujuanID'] = $val;
                $_SESSION[$cCode]['main']['gudangTujuanNama'] = $label;
                $_SESSION[$cCode]['main']['gudangTujuanName'] = $label;
                $_SESSION[$cCode]['main']['gudangTujuanLabel'] = $label;
                break;
        }
    ...
    // Fungsi untuk mengurangi qty produk dengan menghapus elemen terakhir
    function koreksiSerialWithHistory(&$session, $cCode, $config, $produkId, $qtyBaru, &$array3 = array())
    {
        arrPrint($config);
        $srcGate = $config['srcGate'];
        $srcTarget = $config['srcTarget'];
        $srcSerialMaster = $config['srcSerialMaster'];
        $srcSerialMasterItems = $config['srcSerialMasterItems'];
        $keyTriger = $config['keyTriger'];

        $qtyLama = $session[$srcSerialMaster][$produkId]["valid_qty"];
        $selisih = $qtyLama - $qtyBaru;
//        arrPrint($session);
//matiHere($qtyLama."::".$selisih);
        if ($selisih <= 0) {
            return $array3;
        }

        // Simpan snapshot untuk rollback
        $snapshot = $session[$srcGate][$produkId];
        $serialDihapus = array();

        foreach ($session[$srcGate][$produkId] as $tipe => &$dataSerial) {
            for ($i = 0; $i < $selisih; $i++) {
                $serial = array_pop($dataSerial);
                if ($serial !== null) {
                    $serialDihapus[] = $serial;
                }
            }
        }
        unset($dataSerial);

//        arrPrint($serialDihapus);
//        matiHere(__LINE__);

        if (empty($serialDihapus)) {
            return $array3;
        }

        // Pindahkan data
        $dataDipindah = array();
        foreach ($session[$srcTarget] as $key => $item) {
            if (isset($item['produk_serial_number_2']) &&
                in_array($item['produk_serial_number_2'], $serialDihapus)) {
                $dataDipindah[] = $item;
                unset($_SESSION[$cCode][$srcTarget][$key]);
//                unset($_SESSION[$cCode]["items3_sum"][$key]);
                cekmerah("diunset[".$srcTarget."".$key."]".$item['produk_serial_number_2']."");
            }
        }
//matiHEre();
        $_SESSION[$cCode][$srcTarget] = array_values($session[$srcTarget]);
        $array3 = array_merge($array3, $dataDipindah);

        // Update qty
//        $session[$cCode][$srcSerialMaster][$srcSerialMasterItems][$produkId][$keyTriger] = $qtyBaru;

        // Simpan history di session
        if (!isset($session[$cCode]['history_serial'])) {
            $_SESSION[$cCode]['history_serial'] = array();
        }
        $_SESSION[$cCode]['history_serial'][] = array(
            'produk_id' => $produkId,
            'qty_lama' => $qtyLama,
    ...
    function rollbackSerialByConfig(&$session, $cCode)
    {
        if (empty($session[$cCode]['history_serial'])) {
            return false;
        }

        $last = array_pop($session[$cCode]['history_serial']);
        $config = $last['config'];
        ...
        $produkId = $last['produk_id'];

        // 1. Kembalikan data ke srcGate dari snapshot
        $session[$cCode][$srcGate][$produkId] = $last['snapshot'];

        // 2. Kembalikan qty
        $session[$cCode][$srcSerialMaster][$srcSerialMasterItems][$produkId][$keyTriger] = $last['qty_lama'];

        // 3. Kembalikan data dari array3 ke srcTarget
        // Hapus dari items7_cancel dan kembalikan ke srcTarget
        if (isset($session[$cCode]['items7_cancel'])) {
            foreach ($session[$cCode]['items7_cancel'] as $key => $item) {
                if (isset($item['produk_id']) && $item['produk_id'] == $produkId) {
                    $session[$cCode][$srcTarget][] = $item;
                    unset($session[$cCode]['items7_cancel'][$key]);
                }
            }
            $session[$cCode]['items7_cancel'] = array_values($session[$cCode]['items7_cancel']);
        }

        // 4. Reindex srcTarget
        $session[$cCode][$srcTarget] = array_values($session[$cCode][$srcTarget]);

        return true;
    }

    function removeSerial($id,$cCode){
        arrPrint($this->uri->segment_array());
        $intoStep = $this->uri->segment(6);
        $allowEditSerial = isset($this->configUi[$this->jenisTr]['serialLiveEdit'][$intoStep]) ? $this->configUi[$this->jenisTr]['serialLiveEdit'][$intoStep] : array();
        if(count($allowEditSerial)>0){
            arrPrint($allowEditSerial);
            $srcGate = $allowEditSerial["srcGate"];
            $srcTarget = $allowEditSerial["srcTarget"];

            if(isset($_SESSION[$cCode][$srcGate][$id])){
                foreach ($_SESSION[$cCode][$srcGate][$id] as $tipe => &$dataSerial) {
                    foreach ($dataSerial as $ii =>$serial){
                        $serialDihapus[] = $serial;
                    }
                }
                unset($_SESSION[$cCode][$srcGate][$id]);

}
            if(count($serialDihapus)>0){
                foreach ($_SESSION[$cCode][$srcTarget] as $key => $item) {
                    if (isset($item['produk_serial_number_2']) &&
                        in_array($item['produk_serial_number_2'], $serialDihapus)) {
                        $dataDipindah[] = $item;
                        unset($_SESSION[$cCode][$srcTarget][$key]);
