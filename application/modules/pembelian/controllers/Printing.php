                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "items5_sum":
                                    ...
                                    break;
                                case "items7_sum":
                                    $items7_sum_reg = unserialize(base64_decode($row->$param));
                                    break;
                            }
                        }
                    }
...
//        arrPrint($_SESSION[]);
        ...
        if (sizeof($shoppingCartSubDetailFields) > 0) {
            ...
            $psnData = array();
            $psnDataTabel = array();
//            arrPrint($items7_sum_reg);
            if(count($items7_sum_reg)>0){
                //untuk serial yang digeser sat GRN pengakuannya
                cekHitam(__LINE__);
                $psnDataTabel=$items7_sum_reg;
            }
            else{
                //jalan reguler ketika akan prepacking belum punya
                cekKuning(__LINE__);
            $this->load->model("Mdls/MdlProdukPerSerialNumber");
            $psn = New MdlProdukPerSerialNumber();
            $psn->addFilter("transaksi_id='$transaksi_id_serial'");
