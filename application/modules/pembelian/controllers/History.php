...
//        showLast_query("biru");
        ...
        $sumValue = array();
        $arrPairTransaksiHist = array();
        $arrRefNumberData = array();

        if (sizeof($tmpHist) > 0) {
            $arrPairIds = array();
            $arrRefNumberIds = array();
            foreach ($tmpHist as $tmpRowHist) {
                if (isset($tmpRowHist->referenceID) && ($tmpRowHist->referenceID > 0)) {
                    $arrPairIds[$tmpRowHist->referenceID] = $tmpRowHist->referenceID;
                }
                if (isset($tmpRowHist->references) && is_array($tmpRowHist->references) && (sizeof($tmpRowHist->references) > 0)) {
                    foreach ($tmpRowHist->references as $tmpRefNumberId) {
                        if ($tmpRefNumberId > 0) {
                            $arrRefNumberIds[$tmpRefNumberId] = $tmpRefNumberId;
                        }
                    }
                }
            }

            if (sizeof($arrPairIds) > 0) {
                $trPairBulk = new MdlTransaksi();
                $trPairBulk->addFilter("id in ('" . implode("','", $arrPairIds) . "')");
                $tmpPairRows = $trPairBulk->lookupMainTransaksi()->result();
                if (sizeof($tmpPairRows) > 0) {
                    foreach ($tmpPairRows as $tmpPairRow) {
                        $arrPairTransaksiHist[$tmpPairRow->id] = isset($tmpPairRow->ids_his) ? blobDecode($tmpPairRow->ids_his) : array();
                    }
                }
            }

            if (sizeof($arrRefNumberIds) > 0) {
                $trRefBulk = new MdlTransaksi();
                $trRefBulk->addFilter("id in ('" . implode("','", $arrRefNumberIds) . "')");
                $tmpRefRows = $trRefBulk->lookupAll()->result();
                if (sizeof($tmpRefRows) > 0) {
                    foreach ($tmpRefRows as $tmpRefRow) {
                        $arrRefNumberData[$tmpRefRow->id] = array(
                            "nomer" => isset($tmpRefRow->nomer) ? $tmpRefRow->nomer : "",
                            "jenis_master" => isset($tmpRefRow->jenis_master) ? $tmpRefRow->jenis_master : "",
                        );
                    }
                }
            }
...
// cekBiru($row->id." ".$row->nomer);
                ...
                $vendorIds_his_global = isset($counterIds_his['stepCode|supplierID']["$jenisTrtop|$supplier_id"]) ? $counterIds_his['stepCode|supplierID']["$jenisTrtop|$supplier_id"] : "";
                $cGlobals = digit_5($counterGlobal);
                $cGlobal_spo = digit_5($counterIds_his_global);
                $cGlobal_vendor = digit_5($vendorIds_his_global);
                ...
                if (sizeof($pairRegistries) > 0) {
                    ...
                    if (sizeof($pairTransaksi) > 0) {
                        if (isset($row->referenceID) && ($row->referenceID > 0) && isset($arrPairTransaksiHist[$row->referenceID])) {
                            $hisTr = $arrPairTransaksiHist[$row->referenceID];
                    ...
                    $free_produk = viewFreeProduk($tmpReg_result[$row->id], $produkUnitDisc["viewFields"]);
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    //                    if ($fName == "ids_his") {
                    if (is_array($fLabel)) {
                        $fLabel_label = $fLabel['label'];
                        $hisStep = $fLabel['step'];
                        $hisKey = $fLabel['key'];
                        //                        $tNomer = $id_hist[$hisStep][$hisKey];
                        if (is_array($hisKey)) {
                            $anu = "";
                            foreach ($hisKey as $kkk) {
                                if ($anu == "") {
                                    $anu = formatField_he_format($kkk, $row->$kkk);
                                }
                                else {
                                    $anu .= formatField_he_format($kkk, $row->$kkk);
                                }
                    ...
                    if ($fName == "references_num") {
                        $ref_result = "";
                        if (isset($row->references) && is_array($row->references) && (sizeof($row->references) > 0)) {
//                            arrPrintWebs($row->references);
                            foreach ($row->references as $ref_number_id) {
                                if (isset($arrRefNumberData[$ref_number_id])) {
                                    $ref_number = $arrRefNumberData[$ref_number_id]['nomer'];
                                    $ref_jenis_master = $arrRefNumberData[$ref_number_id]['jenis_master'];
                                    $modulRef = isset($this->masterConfigUi[$ref_jenis_master]['modul']) ? $this->masterConfigUi[$ref_jenis_master]['modul'] : "";
                                    $modulPathRef = $modulRef != "" ? base_url() . $modulRef . "/" : base_url();
                                    $ref_number_format = formatField_he_format("nomer", $ref_number, $ref_jenis_master, $modulPathRef);
                                    if ($ref_result == "") {
                                        $ref_result = "$ref_number_format";
                                    }
                                    else {
                                        $ref_result .= ", $ref_number_format";
                                    }
                            ...
                            // masuk disini adalah PO MANUAL, minta dikosongkan, blank, tanpa keterangan
                            // 28 april 2022
                            $ref_result = "";
                        }
                        $colValue = $ref_result;
