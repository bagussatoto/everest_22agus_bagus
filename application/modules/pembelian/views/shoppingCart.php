                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            ...
                            $pemicuGerbangAsli = "pemicuGerbangAsli onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';} \" $keyupData";
                            ...
                            $pemicuGerbang = "pemicuGerbang onblur=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);document.getElementById('result').src='" . $iSpec['editTarget'] . "'$qtyParam+'$queryParams';}\" $keyupData ";
...
//                arrPrint($addRowHiddens[$k]);
        ...
        echo "<script>
        $('input[keyid=jml]').off();
        $('input[keyid=jml]').on('keyup', function(){
            
            if(top.$('#tic_lanjut').is(':checked')){
                top.$('#tic_lanjut').prop('checked',false);
                top.$('#btnProcess').attr('disabled',true);
            }
        });
        
                $('input[keyid=harga]').off();
        $('input[keyid=harga]').on('keyup', function(){
...
//            echo "</div>";
    ...
    switch ($jenisTr) {
        case "466":// pembelian fg
            $link_info_diskon = MODUL_PATH . "Create/infoDiskon/$jenisTr";
