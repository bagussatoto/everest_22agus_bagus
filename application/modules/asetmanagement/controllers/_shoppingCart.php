        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/","asetmanagement/",$iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/","asetmanagement/",$iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//                "dtime" => "date",
        ...
        if ($shopingCartReload) {
            ...
            if (isset($_GET['stop1']) && $_GET['stop1'] == 1) {
                ...
                echo "<script>";
                echo "setTimeout(function(){top.$('div#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=0&stop1=1');}, 500) ";
                echo "</script>";
            }
        }
...
//        cekhere($configUiJenis);
//        cekhere($configCoreJenis);
...
//        arrPrint($_GET);
    ...
    public function buildValues()
    {
        ...
        $configValueJenis = $this->configValues[$jenisTr];
        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($configUiJenis);
        $vg->setConfigCoreJenis($configCoreJenis);
        $vg->setConfigValuesJenis($configValueJenis);
//        $initMasterValues = array(
//            "olehID" => my_id(),
//            "olehName" => my_name(),
//            "placeID" => my_cabang_id(),
...
//        );
    ...
    public function toggleDtp() {
        $jenisTr = $this->uri->segment(3);
        $cCode = "_TR_" . $jenisTr;
        $status = isset($_GET['status']) ? (int)$_GET['status'] : 0;

        $_SESSION[$cCode]['main']['pajak_ditanggung_pemerintah'] = $status;

        // Trigger ValueGate (sama persis dengan kode Anda)
        $this->load->library("ValueGate");
        $vg = new ValueGate();
        $vg->setConfigUiJenis($this->configUiJenis);
        $vg->setConfigCoreJenis($this->configCoreJenis);
        $vg->buildValue($jenisTr, 0, array(
            "jenisTr" => $jenisTr,
            "stepNumber" => 1,
            "dtime" => dtimeNow()
        ), $this->modul);

        echo "<script>top.$('#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $jenisTr . "');</script>";
    }
