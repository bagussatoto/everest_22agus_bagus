...
class _selectorItemAktiva extends Modul_Controller
{
...
//        $cCode = "_TR_" . $this->jenisTr;
    ...
    public function selectItem()
    {
        ...
        $modelFilter2 = isset($this->configUi[$jenisTr]['selectorFilters2']) ? $this->configUi[$jenisTr]['selectorFilters2'] : array();
        ...
        $selectorView = isset($this->configUi[$jenisTr]['selectorView']) ? $this->configUi[$jenisTr]['selectorView'] : "_selectorAktiva";
        ...
        if ($preLocker) {
            $mdlPreLocker = $this->configUi[$jenisTr]["lockerCheck"]["lockerCheck"]["mdlName"];
...
//        arrPrint($selectedAsset);
        ...
        //region lihat ada dicabang mana varang yang dicari

        $itemAll = array();
        if (count($tmp2) == 0) {
            //call data allcabang untuk labeling
            $allCabang = getCabangData();
//            arrPrint($allCabang);
            //lihat barng yang dicari ada dicabang mana
            $o->setFilters(array());
            $o->addFilter("state='active'");
            $o->addFilter("jenis='aktiva'");
            $o->addFilter("jumlah>'0'");
            $tmp3 = $o->lookupByKeyword($key)->result();

            foreach ($tmp3 as $tmp3_0) {
                if (sizeof($selectorParamFields) > 0) {
                    $tmp3_1=array();
                    foreach ($selectorParamFields as $key => $src) {
                        $tmp3_1[$key] = $tmp3_0->$src != "" ? $tmp3_0->$src : "$key - null ";
                    }
                    $tmp3_1['lokasi'] = $allCabang[$tmp3_0->cabang_id]["nama"];
                    $tmp3_1['curent_lokasi'] = $allCabang[$_SESSION['login']['cabang_id']]["nama"];
                    $tmp3_1['link'] = base_url()."Katalog/viewAktivaDetail";
