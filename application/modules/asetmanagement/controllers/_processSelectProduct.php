...
class _processSelectProduct extends Modul_Controller
{
    ...
    public function select()
    {
        ...
        if(isset($_GET['dpp_persen_pengganti'])){
            $dpp_persen_pengganti = $_GET['dpp_persen_pengganti'];

            if($dpp_persen_pengganti > 100){
                mati_disini_original("Anda mengisi DPP PENGANTI (%) sebesar $dpp_persen_pengganti <br> Prosentase maksimal yang diizinkan 100%");
            }
        }
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                if (!isset($_SESSION[$cCode]['items']) || !array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    ...
                    cekBiru("ada id $id tapi tidak ada cCode items [cCode:$cCode]");


//                    matiHere($_SESSION[$cCode]['items'][$id]['harga']);
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        ...
                        //resiprokal manual
                        if(isset($_GET['key_source']) && $_GET['key_source'] == "dpp_persen_pengganti"){
                            $_SESSION[$cCode]['items'][$id]['dpp_nilai_pengganti'] = $_SESSION[$cCode]['items'][$id]['harga']*$_GET['dpp_persen_pengganti']/100;
                        }
                        elseif(isset($_GET['key_source']) && $_GET['key_source'] == "dpp_nilai_pengganti"){
                            $_SESSION[$cCode]['items'][$id]['dpp_persen_pengganti'] = $_GET['dpp_nilai_pengganti']/$_SESSION[$cCode]['items'][$id]['harga']*100;
                        }
                        elseif(isset($_GET['key_source']) && $_GET['key_source'] == "hargapluspajak"){
                            if($_SESSION[$cCode]['main']['pihakMainRulesID'] == "pm"){
                                $_SESSION[$cCode]['items'][$id]['harga'] = $_GET['hargapluspajak']/$_SESSION[$cCode]['main']['ppnFactorInclude'];
                                $_SESSION[$cCode]['items'][$id]['dpp_persen_pengganti'] = isset($_GET['dpp_persen_pengganti']) && $_GET['dpp_persen_pengganti'] == 0 ? 100 : $_GET['dpp_persen_pengganti'];
                                $_SESSION[$cCode]['items'][$id]['dpp_nilai_pengganti'] = $_SESSION[$cCode]['items'][$id]['harga']*$_SESSION[$cCode]['items'][$id]['dpp_persen_pengganti']/100;
                            }
                            else{
                                $_SESSION[$cCode]['items'][$id]['harga'] = $_GET['hargapluspajak'];
                                $_SESSION[$cCode]['items'][$id]['dpp_nilai_pengganti'] = 0;
                            }
                        }
                        elseif(isset($_GET['key_source']) && $_GET['key_source'] == "harga"){
                            if($_SESSION[$cCode]['main']['pihakMainRulesID'] == "pm"){
                                $_SESSION[$cCode]['items'][$id]['hargapluspajak'] = $_GET['harga']*$_SESSION[$cCode]['main']['ppnFactorInclude'];
                            }
                            else{
                                $_SESSION[$cCode]['items'][$id]['hargapluspajak'] = $_GET['harga'];
                            }
                        }
