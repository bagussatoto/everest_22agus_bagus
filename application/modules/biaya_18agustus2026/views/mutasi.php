...
switch ($mode) {
    ...
    case "langsung_simple":
        ...
        $no = 0;
        $modul_path = isset($modul_path) ? $modul_path : base_url() . "penjualan/";
        $jenistr = isset($jenisTr) ? $jenisTr : "5822";
        $strGet = $_SERVER['QUERY_STRING'];
        $pengenal_kolom = "";
        $pengenal_link = "";
        $pengenal_masterid = "";
        ...
        foreach ($master_data as $master_datum) {
            ...
            foreach ($arrHeaders as $kolom => $attrs) {
                ...
                if(isset($attrs['data_order'])){
                    $nilai_order = $master_datum[$attrs['data_order']];
                }
                $data_order = isset($attrs['data_order']) ? "data-order='$nilai_order'" : "$nilai";
                // ----------------------------------------------------------------------
                // data-order='$chNilai'
    ...
    case "indek":
        $p = New Layout("$title", "$subTitle", MODUL_TEMPLATE_PATH . "template/mutasi.html");
    ...
    case "pivot":
        ...
        foreach ($master_data as $master_datum) {
            ...
            foreach ($arrHeaders as $kolom => $attrs) {
                ...
                if (!is_array($nilai)) {
                    $strBody .= "<td rowspan='$jmlRincianBaris'>$nilai_f</td>";
                    // $strBody .= "<td data-datatable-multi-row-rowspan='$jmlRincianBaris'>$nilai_f</td>";
                }
                else {
                    // $strBody .=  "<td></td>";
                }
