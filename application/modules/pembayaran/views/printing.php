...
//                $contenStr5 .= "</div>";
    ...
    case "viewReceiptReg":
        ...
        if (isset($mainElements)) {
            ...
            if (sizeof($mainElements) > 0) {
                foreach ($mainElements as $eKey => $eSpec) {
                    $elementStr = "";
                    if (isset($eSpec['label'])) {
                        $elementStr .= "<div class='panel-heading text-center text-uppercase'>";
                        $elementStr .= $eSpec['label'];
                        $elementStr .= "</div>";
                    }
                    if (sizeof($eSpec['contents'])) {
                        ...
                        foreach ($eSpec['contents'] as $e => $val) {
                            if (!empty($val)) {
                                ...
                                if (!is_numeric($e)) {
                                    $elementStr .= $colLabel != "" ? "<td style='width: 1em;white-space: nowrap;vertical-align: top;' class='text-capitalize'>$colLabel </td><td style='width: 1em;white-space: nowrap;vertical-align: top;'>&nbsp; : </td><td style='vertical-align: top;' class='text-uppercase'>" . $val . "</td>" : "<td colspan='3'>" . $val . "</td>";
                                    /* ==============================================
                                     * format helper diaturdr controler
                                     * ==============================================*/
                                }
        ...
        if (sizeof($itemSrc) > 0) {
            ...
            $item_src .= "<table class='table table-bordered table-condensed' style='width: 100%;'>";
            $item_src .= "<tr bgcolor='#f5f5f5'>";
            $item_src .= "<th class='text-muted text-uppercase' style='font-weight:bold;' align='right' width='5'>No.</th>";

            foreach ($itemSrcLabel as $ky => $srcLabel) {
                $item_src .= "<th class='text-muted text-uppercase' style='font-weight:bold;'>";
                $item_src .= $srcLabel . "";
                $item_src .= "</th>";
            }
            $item_src .= "</tr>";
            ...
            foreach ($itemSrc as $itemSrc0) {
                ...
                foreach ($itemSrcLabel as $ky => $srclabel) {
                    $val = isset($itemSrc0[$ky]) ? $itemSrc0[$ky] : "";
                    $item_src .= "<td title='$ky'>";
                    $item_src .= formatField_he_format($ky, $val);
                    $item_src .= "</td>";

                    if(is_numeric($val)){
                        if(!isset($subtotal[$ky])){
                            $subtotal[$ky] = 0;
                        }
                        $subtotal[$ky] += $val;
                    }
            ...
            $item_src .= "<tr>";
            $item_src .= "<td class='text-right' colspan='8'>Jumah</td>";
            $item_src .= "<td></td>";
            $item_src .= "</tr>";
        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                $contentStr .= "<table class='table table-bordered table-condensed' style='width: 100%;'>";
                $contentStr .= "<tr bgcolor='#f5f5f5'>";
                $contentStr .= "<th class='text-muted text-uppercase' style='font-weight:bold;' align='right' width='5'>No.</th>";


                foreach ($itemLabels as $key => $label) {
//                    cekHere("$key");
                    $contentStr .= "<th class='text-muted text-uppercase' style='font-weight:bold;'>";
                    $contentStr .= $label . "";
                    $contentStr .= "</th>";
                }
                $contentStr .= "</tr>";
                    ...
                    foreach ($sumRows as $key => $label) {
                        ...
                        if (isset($mainValues[$key])) {
                            ...
                            if (sizeof($mainValues[$key]) > 0) {
                                //                                cekHere("$key " . $mainValues[$key]);
                                $contentStr .= "<tr line=" . __LINE__ . ">";
                                $contentStr .= "<td colspan='$colspan2' class='text-right text-uppercase'>$label</td>";
                                $contentStr .= "<td class='text-right'>";
                                if (isset($mainValues[$key])) {
            ...
            if (isset($receiptEfakturFields) && sizeof($receiptEfakturFields) > 0) {
                ...
                $contentStr7 .= "<div class='ppanel-header' style='border:0px solid red;'>FAKTUR PAJAK MASUKAN</div>";
                ...
                if (isset($formulirFaktur) && (sizeof($formulirFaktur)) > 0) {
                    foreach ($formulirFaktur as $ctt => $fSpec) {
                        $contentStr7 .= "<tr>";
                        foreach ($receiptEfakturFields as $fff => $f_labels) {
                            $value = formatField($fff, $fSpec[$fff]);
                            $contentStr7 .= "<td id='td_$fff'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                            $contentStr7 .= "</td>";
                    ...
                    if (sizeof($formulirFaktur) > 1) {
                        ...
                        $contentStr7 .= "<tr style='background-color:$bgcolor;'>";
                        foreach ($receiptEfakturFields as $fff => $f_labels) {
                            $value = isset($sub_total_bawah[$fff]) ? formatField($fff, $sub_total_bawah[$fff]) : "";
                            $contentStr7 .= "<td id='td_$fff' class='text-bold' style='font-size:15px;'>$value";
                            $contentStr7 .= "</td>";
                        }
                        $contentStr7 .= "</tr>";
                    }
...
//            arrPrintWebs($formulirFaktur);
//            arrPrint($formulirInsentifFaktur);
            $contentStr8="";
            if(isset($receiptEfakturInsentifFields)){
                $no = 0;
                $contentStr8 .= "<div class='table-responsive' style='border:0px solid red; padding-top: 5px'>";
                $contentStr8 .= "<div class='panel-header' style='border:0px solid red;'>FAKTUR PAJAK DITANGGUNG PEMERINTAH(DTP)</div>";
                $contentStr8 .= "<table class='table table-bordered table-condensed' style='width: 100%;'>";
                $contentStr8 .= "<tr bgcolor='#f5f5f5'>";
                foreach ($receiptEfakturInsentifFields as $key => $label) {
                    $contentStr8 .= "<th class='text-muted' style='font-weight:bold;'>";
                    $contentStr8 .= $label;
                    $contentStr8 .= "</th>";
                }
                $contentStr8 .= "</tr>";

                if (isset($formulirInsentifFaktur) && (sizeof($formulirInsentifFaktur)) > 0) {
//                    arrPrint($formulirInsentifFaktur);
                    foreach ($formulirInsentifFaktur as $ctt => $fSpec) {
                        $contentStr8 .= "<tr>";
                        foreach ($receiptEfakturInsentifFields as $fff => $f_labels) {
                            $value = formatField($fff, $fSpec[$fff]);
//                            $value = $fSpec[$fff];
                            $contentStr8 .= "<td id='td_$fff'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                            $contentStr8 .= "</td>";
                            if (is_numeric($fSpec[$fff])) {
                                if (!isset($sub_total_bawah[$fff])) {
                                    $sub_total_bawah[$fff] = 0;
                    ...
                    if (sizeof($formulirInsentifFaktur) > 1) {
                        $bgcolor = isset($formulirFakturStyle["bgcolor"]) ? $formulirFakturStyle["bgcolor"] : "";
                        $contentStr8 .= "<tr style='background-color:$bgcolor;'>";
                        foreach ($receiptEfakturInsentifFields as $fff => $f_labels) {
                            $value = isset($sub_total_bawah[$fff]) ? formatField($fff, $sub_total_bawah[$fff]) : "";
                            $contentStr8 .= "<td id='td_$fff' class='text-bold' style='font-size:15px;'>$value";
                            $contentStr8 .= "</td>";
                        }
                        $contentStr8 .= "</tr>";
                    }
                    ...
                    $contentStr8 .= "<tr line=" . __LINE__ . ">";
                    foreach ($receiptEfakturFields as $key => $label) {
                        $val = isset($mainValues[$key]) ? $mainValues[$key] : "";
                        $contentStr8 .= "<td>";
                        $contentStr8 .= formatField_he_format($key, $val);
                        $contentStr8 .= "</td>";
                    }
                    $contentStr8 .= "</tr>";
                }
            ...
            $elementLabels["content_8"] = $contentStr8;
        }
