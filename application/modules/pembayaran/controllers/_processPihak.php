...
//                $ppn_final = ($ppnFactor / 100) * $value;
                ...
                $_SESSION[$cCode]["items6_sum"][$detail_key]["tagihan_bayar"] = $subtotal;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_sudah_faktur"] = $ppn_final;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["dpp_pengganti"] = $dpp_pengganti;

                $reloadShoppingcart = 1;
            }
            if ($key == "dpp_non_insentif_reguler") {
                $dpp_pengganti = (11 / 12) * $value;
//                $ppn_final = ($ppnFactor / 100) * $value;
                $ppn_final = (12 / 100) * $dpp_pengganti;
                $subtotal = $value + $ppn_final;
                ...
                $_SESSION[$cCode]["items6_sum"][$detail_key]["ppn_sudah_faktur"] = $ppn_final;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["dpp_pengganti"] = $dpp_pengganti;
                $_SESSION[$cCode]["items6_sum"][$detail_key]["dpp_non_insentif"] = $dpp_pengganti;
...
//        arrPrint($_SESSION[$cCode]["items6_sum"]);
    ...
    public function addTaxDataInsentif()
    {
//        arrPrint($this->uri->segment_array());
//        arrPrint($_GET);
//        arrPrint($_GET);
        ...
        $detail_key = $this->uri->segment(5);
//        $key = $_GET["nama"]."_insentif";
        $key = $_GET["nama"];
        $value = $_GET["nilai"];
        $key2 = $_GET["nama"] . "_insentif";
        $cCode = $this->cCode;
        $ppnFactor = $_SESSION[$cCode]["main"]["ppnFactor"];
        $reloadShoppingcart = 0;
//cekHitam($key);
...
//            $_SESSION[$cCode]["main"][$key] = $value;
        ...
        if (isset($_SESSION[$cCode]["items7_sum"][$detail_key])) {
            if ($key == "dpp_non_insentif_reguler") {
                $dpp_pengganti = (11 / 12) * $value;
//                $ppn_final = ($ppnFactor / 100) * $value;
                $ppn_final = (12 / 100) * $dpp_pengganti;
                $subtotal = $value + $ppn_final;
                //----
                $_SESSION[$cCode]["items7_sum"][$detail_key][$key] = $value;
                $_SESSION[$cCode]["items7_sum"][$detail_key][$key2] = $value;
                $_SESSION[$cCode]["items7_sum"][$detail_key]["ppn_final"] = $ppn_final;
                $_SESSION[$cCode]["items7_sum"][$detail_key]["nilai_entry"] = $subtotal;
                $_SESSION[$cCode]["items7_sum"][$detail_key]["tagihan_bayar"] = $subtotal;
                $_SESSION[$cCode]["items7_sum"][$detail_key]["ppn_sudah_faktur"] = $ppn_final;
                $_SESSION[$cCode]["items7_sum"][$detail_key]["dpp_pengganti"] = $dpp_pengganti;

                $reloadShoppingcart = 1;
            }
            if ($key == "dateFaktur") {
                $_SESSION[$cCode]["items7_sum"][$detail_key][$key] = $value;
                $_SESSION[$cCode]["main"][$key . "_insentif"] = $value;
            }
            if ($key == "eFaktur") {
                $_SESSION[$cCode]["items7_sum"][$detail_key][$key] = $value;
                $_SESSION[$cCode]["main"][$key . "_insentif"] = $value;
