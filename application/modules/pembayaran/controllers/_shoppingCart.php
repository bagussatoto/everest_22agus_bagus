    ...
    public function viewCart()
    {
        ...
        switch ($_SESSION[$cCode]["main"]["paymentmode"]) {
            case "valas":
                ...
                $shoppingCartStockValasNotif = isset($this->configUi[$this->jenisTr]['shoppingCartStockValasNotif']) ? $this->configUi[$this->jenisTr]['shoppingCartStockValasNotif'] : array();
                if (isset($shoppingCartStockValasNotif["enabled"]) && ($shoppingCartStockValasNotif["enabled"] == true)) {
                    $stokValasNotif = $shoppingCartStockValasNotif["enabled"];
                }
                break;
            default:
            case "reguler":
                    ...
                    switch ($_SESSION[$cCode]["main"]["paymentmode"]) {
                        case "valas":
                            ...
                            if ($shopingCartPaymentItemsColorEnabled == true) {
                                if ($iSpec['valas_new_sisa'] > 1) {
                                    if (isset($iSpec['valas_nilai_bayar']) && ($iSpec['valas_nilai_bayar'] > 0)) {
                                        // dibayar, tidak lunas
                                        $tmp['background_pembayaran'] = $shopingCartPaymentItemsColor['colorCode'][1]['color'];
                                    }
                                    elseif (isset($iSpec['valas_nilai_bayar']) && ($iSpec['valas_nilai_bayar'] == 0)) {
                                        // tidak dibayar
...
//            arrPrint($this->configUi[$this->jenisTr]['referenceFakturReceive']);
//            matiHere(__LINE__);
...
//                $_SESSION[$cCode]["main"]["skip_faktur"]="true";
                ...
                if (isset($shopingCartFaktur["gateTarget"])) {
                    ...
                    // Sync fixed fields while preserving eFaktur, dateFaktur, & skip_faktur if already present in items6_sum
                    if (count($_SESSION[$cCode][$gateTarget]) >= 1) {
                        foreach ($dataFaktur as $aa => $bb) {
                            if (in_array($aa, array("eFaktur", "dateFaktur", "skip_faktur"))) {
                                $existingVal = isset($_SESSION[$cCode][$gateTarget][0][$aa]) ? trim($_SESSION[$cCode][$gateTarget][0][$aa]) : "";
                                if (strlen($existingVal) > 6 && $existingVal != "0000-00-00") {
                                    continue;
                                }
                            }
...
//            "pajak_ditanggung_pemerintah" => isset($this->config[]["pajak_ditanggung_pemerintah"]) ? $_SESSION[$cCode]["main"]["optionPembayaran"] : 0,
            ...
            "stokValasNotif" => isset($stokValasNotif) ? $stokValasNotif : NULL,
        );
...
//                        $msg .= " Saldo saat ini $saldo_f, anda mengisi sebesar $val_f. Silahkan dikoreksi.";
                        $msg = "Ada transaksi yang tidak sesuai. Mohon mengirim pesan ini ke Admin.<br>";
                        $msg .= __CLASS__ . " line: " . __LINE__;
                        die(lgShowAlertMerah($msg));
                    }
...
//                    }
                    switch ($jenisTr) {
                        case "1424":
                            if ($val > $_SESSION[$cCode]['main']['tagihan_bayar_after_pph23']) {
                                $saldo = $_SESSION[$cCode]['main']['tagihan_bayar_after_pph23'];
                                $saldo_f = number_format($saldo, 0, ",", ".");
//                                $msg = "Penggunaan Credit Note " . $_SESSION[$cCode]['main']['pihakName'] . " melebihi tagihan yang akan dibayar.";
//                                $msg .= " Nilai tagihan $saldo_f, anda mengisi sebesar $val_f. Silahkan dikoreksi.";
//                                $msg = "Ada transaksi yang tidak sesuai. Mohon mengirim pesan ini ke Admin.<br>";
//                                $msg .= __CLASS__ . " line: " . __LINE__;
                                $msg = notifTransaksiError(__CLASS__, __LINE__);
                                die(lgShowAlertMerah($msg));
                            }
                            break;
                        default:
...
//                    $msg .= __CLASS__ . " line: " . __LINE__;
                ...
                switch ($jenisTr) {
                    case "1424":
                        if ($val > $_SESSION[$cCode]['main']['tagihan_bayar_after_titipan_relasi']) {
                            $selisih = $val - $_SESSION[$cCode]['main']['tagihan_bayar_after_titipan_relasi'];
                            $selisih = ($selisih < 0) ? ($selisih * -1) : $selisih;
                            if ($selisih > 1) {
                                $saldo = $_SESSION[$cCode]['main']['tagihan_bayar_after_titipan_relasi'];
                                $saldo_f = number_format($saldo, 0, ",", ".");
...
//                                $msg .= " Nilai tagihan <b class='blink'>$saldo_f</b>, anda mengisi sebesar $val_f. Silahkan dikoreksi, menjadi $saldo_f<br>";
//                                $msg .= __CLASS__ . " line: " . __LINE__;
...
//Diskon anda yyy tidak diijinkan karena > dari xxx"
            ...
            case "nilai_entry":
                if (($this->jenisTr == "1488") && ($_SESSION[$cCode]["main"]["valas_id"] > 0)) {
                    $_SESSION[$cCode]["main"]["additional"] = 0;

                }
                break;

            case "valas_nilai_stock":
                if ($val > $_SESSION[$cCode]["main"]["valas_account__saldo"]) {
                    $msg = "Jumlah valas " . $_SESSION[$cCode]["main"]["valas_account__label"] . " yang anda isikan melebihi stok. Silahkan dikoreksi lagi. code: " . __LINE__;
                    mati_disini($msg);
                }
                break;
        }
...
//                $_SESSION[$cCode]["main"]["diskon_khusus_persen"] = 0;//tetap ditulis biar tidak error, value diabaikan
        ...
        if (isset($_SESSION[$cCode]['items6_sum']) && is_array($_SESSION[$cCode]['items6_sum'])) {
            foreach ($_SESSION[$cCode]['items6_sum'] as $idx => $item6) {
                if (isset($item6['is_predefined_faktur']) && $item6['is_predefined_faktur'] == 1) {
                    $fieldsToBackup = array('dpp_final', 'dpp_pengganti', 'ppn_final', 'tagihan_bayar');
                    foreach ($fieldsToBackup as $f) {
                        if (isset($item6[$f])) {
                            $asliKey = $f . '_asli';
                            if (isset($item6[$asliKey])) {
                                $asliBackups[$idx][$asliKey] = $item6[$asliKey];
                            } else {
                                $asliBackups[$idx][$asliKey] = $item6[$f];
                            }
                        }
                    }
                }
        ...
        if (!empty($asliBackups)) {
            foreach ($asliBackups as $idx => $backup) {
                foreach ($backup as $k => $v) {
                    $_SESSION[$cCode]['items6_sum'][$idx][$k] = $v;
                }
            }
        }
