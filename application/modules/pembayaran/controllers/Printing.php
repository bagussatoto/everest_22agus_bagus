...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            //            $itemNumLabels = isset($this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum] : $this->configUi[$this->jenisTr]['shoppingCartNumFields'][$currentStepNum];
            $itemNumLabels = isset($this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receipNumFields'][$currentStepNum] : (isset($this->configUi[$this->jenisTr]['shoppingCartNumFields'][$currentStepNum]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'][$currentStepNum] : array());
            if (isset($this->configLayout[$this->jenisTr]['receipCartNumFields2'][$currentStepNum])) {
                $itemNumLabels2 = $this->configLayout[$this->jenisTr]['receipCartNumFields2'][$currentStepNum];
            }
            elseif (isset($this->configUi[$this->jenisTr]['shoppingCartNumFields2'][$currentStepNum])) {
            ...
            $receiptGlobalStepVar = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptGlobalStepVar'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptGlobalStepVar'][$currentStepNum] : array();
            //region tabel2 tarikan untuk kolom2 nilai (hpp, ppn, dll)
            $tmpVal_main = $tr->lookupMainValuesByTransID($trID);
            $tmpVal_detail = $tr->lookupDetailValuesByTransID($trID);
            //endregion

            $itemNumFieldsCustom = isset($this->configUi[$this->jenisTr]['shoppingCartNumFieldsCustom'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFieldsCustom'][1] : array();
            $itemNumKodePajak = isset($this->configUi[$this->jenisTr]['shoppingCartKodePajak'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartKodePajak'][1] : array();
            ...
            $receiptSumFields2 = array();
            $itemsRegistries = array();
            $items6_sumRegistries = array();
            $items7_sumRegistries = array();
            $itemSrc = array();
                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "items2_sum"://
                                    ...
                                    break;
                                case "items6_sum"://
                                    $items6_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "items7_sum"://
            ...
            if ($pakai_ini == 1) {
                ...
                $itemLabels = isset($this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields'][$currentStepNum] : array();
                $itemLabels2 = isset($this->configLayout[$this->jenisTr]['receiptDetailFields2'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptDetailFields2'][$currentStepNum] : array();
            }

//            arrPrint($itemKodePajak);
//            arrPrint($itemNumKodePajak);
            ...
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582")) {
                ...
                foreach ($tmpTr as $row) {
                    ...
                    if (isset($itemsRegistries[$id])) {
                        ...
                        foreach ($itemNumLabels as $key => $labels) {
                            if (isset($itemsRegistries[$id][$key])) {
                                $tmp[$key] = $itemsRegistries[$id][$key];
                            }
                        }
                        if (sizeof($itemNumKodePajak) > 0) {
                            foreach ($itemNumKodePajak as $kk => $vv) {
                                if(isset($itemsRegistries[$id][$kk])){
                    ...
                    $rawItems[$id] = $tmp;
                    $rawItems[$id] = array_replace(array_filter($childTableInValueParams[$id]), array_filter($rawItems[$id]));

                    if ((is_array($receiptDetailFieldsReplacerConfig) && (sizeof($receiptDetailFieldsReplacerConfig) > 0))) {
                        foreach ($receiptDetailFieldsReplacerConfig as $key => $vSpec) {
                            if (array_key_exists($key, $rawItems[$id])) {
                                $rawItems[$id][$key] = isset($vSpec[$rawItems[$id][$key]]) ? $vSpec[$rawItems[$id][$key]] : "";
                            }
                        }
                    }
                    ...
                    foreach ($availValueKeys as $ak) {
                        //                        echo "checking $ak: ";
                        if (isset($rawItems[$id][$ak]) && is_numeric($rawItems[$id][$ak])) {
                            //                        echo "ada ";
                            $valueKey = $ak;
                        }
                        else {
        ...
        $globalVars = array_merge($globalVars, $arrCompanyProfile, $masterTableInParams, $receiptGlobalStepVar);
        ...
        if (sizeof($elementFixedConfig) > 0) {
                ...
                if (is_array($label)) {
                    ...
                    if (isset($globalVars[$key])) {
                        if (is_array($globalVars[$key])) {
                            ...
                            /** ----------------------------------
                             * format ebillingDate tidak jalan jalan bosss
                             * sementaun nongo l apa adanya
                             */
                            // cekHijau($key ."---". $globalVars[$key] ."----". strtotime($globalVars[$key]) . "====" . date('d F Y',strtotime($globalVars[$key])) . "+++" . formatField_he_format(trim('ebillingDatef'), trim($globalVars[$key])));
                            $fixedElements['fixedElements']['contents'][$label] = formatField_he_format(trim($key), $globalVars[$key]);
                        }
                    }
                    else {
                        $fixedElements['fixedElements']['contents'][$label] = "";
            ...
            /**
             * memasukan elemen ke contenfixelenen
             */
            // foreach ($elements as $keyElement => $element) {
            // $fixedElements['fixedElements']['contents'][$keyElement] = $element['value'];
            // }
...
//            cekHere($companyStr);
        ...
        $itemSubTotal2 = isset($this->configLayout[$this->jenisTr]['receiptSumDetailFields2'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptSumDetailFields2'][$currentStepNum] : array("subtotal" => "Subtotal");
        $itemLabels = $itemLabels + $itemNumLabels + $itemSubTotal;
        $itemLabels2 = $itemLabels2 + $itemNumLabels2 + $itemSubTotal2;
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number]) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount'][$row->step_number] == true) {
            unset($itemLabels['subtotal']);
            unset($itemLabels2['subtotal']);
        }
        ...
        //--------------------
        $receiptSumFieldsLayout = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFields'][$currentStepNum]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptSumFields'][$currentStepNum] : array();
        ...
        //--------------------
        $receiptEfakturFields = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturFields'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturFields'][$row->step_number] : array();
        $receiptEfakturInsentifFields = isset($this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturInsentifFields'][$row->step_number]) ? $this->configLayout[$tmpTr[0]->jenis_master]['receiptEfakturInsentifFields'][$row->step_number] : array();
        //--------------------
        ...
        if (count($shopingCartFaktur) > 0) {
            if (isset($shopingCartFaktur["gateTarget"])) {
                $gateTarget = $shopingCartFaktur["gateTarget"] . "Registries";
            }
            if (isset($shopingCartInsentifFaktur["gateTarget"])) {
                $gateInsentifTarget = $shopingCartInsentifFaktur["gateTarget"] . "Registries";
            }
        }
//        arrPrint($$gateInsentifTarget);
//        matiHere($gateInsentifTarget);
        //--------------------
        $template = $this->configLayout[$jenisTr]["receiptTemplate"][$currentStepNum];
        //--------------------
        if(sizeof($itemNumKodePajak)>0){
            ...
            $itemLabels = $itemLabels + $itemNumKodePajak + $itemNumFieldsCustom;
        }
        //--------------------


         cekHere("$template || $mode");
        arrPrint($receiptEfakturFields);
//        arrPrint($itemsRegistries);
        $data = array(
            "mode" => $mode,
            "template" => $template,
            ...
            "mainData2Fields" => $addMainData2Fields,
            "mainData2" => $addMainData2,
            "receiptEfakturFields" => $receiptEfakturFields,
            "receiptEfakturInsentifFields" => $receiptEfakturInsentifFields,

            "formulirFaktur" => isset($$gateTarget) ? $$gateTarget : array(),
            "formulirInsentifFaktur" => isset($$gateInsentifTarget) ? $$gateInsentifTarget : array(),
