...
//                arrPrint($selectorProcessor);
    ...
    public function preview()
    {
        ...
        $itemSrcLabels = isset($configUi[$this->jenisTr]['shoppingCartFieldItem'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartFieldItem'][$stepNumber] : array();
        ...
        //------------------------------------------
        $shoppingCartMode = isset($configUi[$this->jenisTr]['mode']) ? $configUi[$this->jenisTr]['mode'] : NULL;
        ...
        switch ($_SESSION[$cCode]["main"]["paymentmode"]) {
            case "valas":
                $itemLabels = isset($this->configUi[$this->jenisTr]['shoppingCartFieldsValas'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartFieldsValas'][$stepNumber] : array();
                $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFieldsValas'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartNumFieldsValas'][$stepNumber] : array();
                $shoppingCartAmountValue = isset($this->configUi[$this->jenisTr]['shoppingCartAmountValueValas'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartAmountValueValas'][$stepNumber] : array();
                $elementConfigs = isset($this->configUi[$this->jenisTr]['receiptElementsValas']) ? $this->configUi[$this->jenisTr]['receiptElementsValas'] : array();
                $addRowsConfigs = isset($this->configUi[$this->jenisTr]['additionalRowsValas']) ? $this->configUi[$this->jenisTr]['additionalRowsValas'] : array();
                break;
            default:
            case "reguler":
                break;
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['itemsSrc1'])) {
                foreach ($_SESSION[$cCode]['itemsSrc1'] as $iSpec) {
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => $iSpec['nama'],
                        "satuan" => isset($iSpec['satuan']) ? $iSpec['satuan'] : "n/a",
            ...
            if (isset($_SESSION[$cCode]['itemsTarget1'])) {
                foreach ($_SESSION[$cCode]['itemsTarget1'] as $iSpec) {
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => $iSpec['nama'],
                        "satuan" => isset($iSpec['satuan']) ? $iSpec['satuan'] : "n/a",
            ...
            if (isset($_SESSION[$cCode]['itemSrc'])) {
                foreach ($_SESSION[$cCode]['itemSrc'] as $iSpec) {
                    $tmp = array(
                        ...
                        "serial_no" => isset($iSpec['serial_no']) ? $iSpec['serial_no'] : "",
                        "kode" => isset($iSpec['kode']) ? $iSpec['kode'] : "",
                        "dtime_entry" => isset($iSpec['dtime_entry']) ? $iSpec['dtime_entry'] : "",
                        "extern_date2" => isset($iSpec['extern_date2']) ? $iSpec['extern_date2'] : "",
                        "extern_label2" => isset($iSpec['extern_label2']) ? $iSpec['extern_label2'] : "",
        ...
        //endregion
        /**
         * sementara hidden dulu itemtarget untuk setor ppn
         */
        if ($jenisTr == "1148") {
            unset($itemsSrc);
        }
        ...
        $itemsLabelTarget1 = $itemSrcLabels + $itemSrcNumLabels + array("subtotal" => "sub-amount");

        $itemsSrc1Label1 = array(
            "extern_date2" => "tgl PIB",
            "extern_label2" => "Nomor PIB",
            "nama" => "????",
            "extern_nama" => "No PO Import",
            "sisa" => "PIB NILAI",
        );

        $srcItemsSrc1Label = $itemsSrc1Label1 + array("subtotal" => "sub-amount");
        ...
        if (isset($configUi[$this->jenisTr]['shoppingCartHideSubamount']) && $configUi[$this->jenisTr]['shoppingCartHideSubamount'][$stepNumber] == true) {
            ...
            unset($itemSrcLabels['subtotal']);
            unset($itemsLabelTarget1['subtotal']);
        }
...
//                    $advanceAmountItemsLabel = isset($shoppingCartAdvanceAmountValue[$key_result][$stepNumber]) ? $shoppingCartAdvanceAmountValue[$key_result][$stepNumber] : array();
        ...
        //untuk faktur insentif
        $shopingCartInsentifFaktur = isset($this->configUi[$this->jenisTr]['addMainSourceInsentif'][1]) ? $this->configUi[$this->jenisTr]['addMainSourceInsentif'][1] : array();

        if (count($shopingCartInsentifFaktur) > 0) {
            if (isset($shopingCartInsentifFaktur["gateTarget"])) {
                $gateInsentifTarget = $shopingCartInsentifFaktur["gateTarget"];
        ...
        if (sizeof($shoppingCartNoFaktur) > 0) {
            $gateSource = $shoppingCartNoFaktur["sourceGate"];
            $pairedItemTarget = $gateTarget = $shoppingCartNoFaktur["targetGate"];
            $fieldsCek = $shoppingCartNoFaktur["fields"];
            $shoppingCartNoFakturLabel = $shoppingCartNoFaktur["label"];
            $mainKey = isset($shoppingCartNoFaktur["mainKey"]) ? $shoppingCartNoFaktur["mainKey"] : array();
            if (sizeof($mainKey) > 0) {
                foreach ($mainKey as $mmKey => $mmVal) {
                    $_SESSION[$cCode]["main"][$mmKey] = 0;
                }
            }
            if (isset($_SESSION[$cCode][$gateSource]) && (sizeof($_SESSION[$cCode][$gateSource]) > 0)) {
                $_SESSION[$cCode][$gateTarget] = array();
                foreach ($_SESSION[$cCode][$gateSource] as $ii => $iiSpec) {
//                    if (!isset($iiSpec[$fieldsCek]) || ($iiSpec[$fieldsCek] == NULL) || ($iiSpec[$fieldsCek] == 0)) {
                    if (!isset($iiSpec[$fieldsCek]) || ($iiSpec[$fieldsCek] === NULL)) {
                        $_SESSION[$cCode][$gateTarget][$ii] = $iiSpec;
                        if (sizeof($mainKey) > 0) {
                            foreach ($mainKey as $mmKey => $mmVal) {
                                if (!isset($_SESSION[$cCode]["main"][$mmKey])) {
                                    $_SESSION[$cCode]["main"][$mmKey] = 0;
                                }
                                $_SESSION[$cCode]["main"][$mmKey] += (isset($iiSpec[$mmVal]) ? $iiSpec[$mmVal] : 0);
                            }
                        }
            ...
            $main = array_merge($main, array_filter($_SESSION[$cCode]['main']));
        }
        //--------------------------
        $sessionPairedItemTarget = isset($_SESSION[$cCode][$pairedItemTarget]) ? $_SESSION[$cCode][$pairedItemTarget] : array();
        //--------------------------
        ...
        $data = array(
            ...
            "srcItemsSrc1Label" => $srcItemsSrc1Label,
            "itemsSrc1" => $itemsSrc1,

            "itemsLabelTarget1" => $itemsLabelTarget1,
...
//            "items2" => $pairedItemTarget,
            "items2" => $sessionPairedItemTarget,
            "items3" => $items3_sum,
            "itemsSrc" => $itemsSrc,
            ...
            "sumRows2" => isset($configUi[$this->jenisTr]['shoppingCartSumFields2'][1]) ? $configUi[$this->jenisTr]['shoppingCartSumFields2'][1] : array(),
            ...
            "shoppingCartAdvanceItemsSelector" => isset($shoppingCartAdvanceItemsSelector) ? MODUL_PATH . $shoppingCartAdvanceItemsSelector . "/" . $this->jenisTr : NULL,
            "shoppingCartAdvanceItemsRemove" => isset($shoppingCartAdvanceItemsRemove) ? MODUL_PATH . $shoppingCartAdvanceItemsRemove . "/" . $this->jenisTr : NULL,
            "shoppingCartAdvanceItemsAdd" => isset($shoppingCartAdvanceItemsAdd) ? MODUL_PATH . $shoppingCartAdvanceItemsAdd . "/" . $this->jenisTr : NULL,
            "subAdvanceItems" => isset($_SESSION[$cCode]['items4'][$_SESSION[$cCode]['main']['refID']]) ? $_SESSION[$cCode]['items4'][$_SESSION[$cCode]['main']['refID']] : array(),
            //----------------------
            ...
            "formulirInsentifFaktur" => isset($_SESSION[$cCode][$gateInsentifTarget]) ? $_SESSION[$cCode][$gateInsentifTarget] : array(),
            "fakturParam" => $shopingCartFaktur,
            "fakturItems" => isset($dataFaktur) ? $dataFaktur : array(),
            "fakturInsentifParam" => $shopingCartInsentifFaktur,
            "fakturInsentifItems" => isset($dataInsentifFaktur) ? $dataInsentifFaktur : array(),
            "shoppingCartNoFakturLabel" => isset($shoppingCartNoFakturLabel) ? $shoppingCartNoFakturLabel : array(),
            "shopingCartTaxSrc" => isset($_SESSION[$cCode]["itemSrc"]) ? $_SESSION[$cCode]["itemSrc"] : array(),
            "shopingCartTaxtarget" => isset($_SESSION[$cCode]["itemtarget"]) ? $_SESSION[$cCode]["itemtarget"] : array(),
    ...
    public function previewMulti()
    {
        ...
        $itemNumLabels3 = isset($configUi[$this->jenisTr]['shoppingCartNumFields3'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartNumFields3'][$stepNumber] : array();
        $itemSrcNumLabels = isset($configUi[$this->jenisTr]['shoppingCartNumFieldItemSrc'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartNumFieldItemSrc'][$stepNumber] : array();
        $itemNumFieldsCustom = isset($configUi[$this->jenisTr]['shoppingCartNumFieldsCustom'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartNumFieldsCustom'][$stepNumber] : array();
        $itemNumKodePajak = isset($configUi[$this->jenisTr]['shoppingCartKodePajak'][$stepNumber]) ? $configUi[$this->jenisTr]['shoppingCartKodePajak'][$stepNumber] : array();
        ...
        $shoppingCartMode = isset($configUi[$this->jenisTr]['mode']) ? $configUi[$this->jenisTr]['mode'] : NULL;
        $shopingCartEmbededTaxItem = isset($this->configUi[$this->jenisTr]['shopingCartEmbededTaxItem']) ? $this->configUi[$this->jenisTr]['shopingCartEmbededTaxItem'] : array();
        $shoppingCartKodePajak = isset($this->configUi[$this->jenisTr]['shoppingCartKodePajak'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartKodePajak'][1] : array();
        $itemKodePajak = isset($this->configUi[$this->jenisTr]['kodePajak'][1]) ? $this->configUi[$this->jenisTr]['kodePajak'][1] : array();
        //------------------------------------------
        //------------------------------------------
        ...
        $items3_sum = array();
        $itemsSrc = array();
        $itemNumKodePajakTerlibat = array();
        if (isset($_SESSION[$cCode])) {
            if (isset($_SESSION[$cCode]['items'])) {
                foreach ($_SESSION[$cCode]['items'] as $ii => $iSpec) {
                    ...
                    if (($iSpec["sisa"] <= 1) && ($iSpec["nilai_ppn_keluaran_didelete"] > 0)) {
                        $tmp["background_color"] = "grey";
                    }
                    //-------------------------------------
                    if (sizeof($itemKodePajak) > 0) {
                        $kpj = kodePajak($itemKodePajak, $iSpec["extern_label2"], $iSpec["sisa"]);
                        foreach ($kpj as $kk => $vv) {
                            $tmp[$kk] = $vv;
                            $_SESSION[$cCode]['items'][$ii][$kk] = $vv;
                            $itemNumKodePajakTerlibat[$kk] = $kk;
                        }

                    }
                    $items[] = $tmp + $iSpec;
                }
                foreach ($itemNumKodePajak as $kk => $vv) {
                    if (!array_key_exists($kk, $itemNumKodePajakTerlibat)) {
                        unset($itemNumKodePajak[$kk]);
                    }
                }
            }
            if (isset($_SESSION[$cCode]['itemsSrc1'])) {
            ...
            if (isset($_SESSION[$cCode]['itemSrc'])) {
//                arrPrintCyan($itemSrcNumLabels);
                foreach ($_SESSION[$cCode]['itemSrc'] as $isrc => $iSpec) {
                    $tmp = array(
                        "id" => $iSpec['id'],
                        "nama" => $iSpec['nama'],
                        "satuan" => isset($iSpec['satuan']) ? $iSpec['satuan'] : "n/a",
                    ...
                    if (sizeof($itemSrcNumLabels) > 0) {
                        foreach ($itemSrcNumLabels as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                            if (!isset($_SESSION[$cCode]['itemSrc'][$isrc][$key])) {
                                $_SESSION[$cCode]['itemSrc'][$isrc][$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                            }
                            if (!isset($main[$key])) {
                                $main[$key] = 0;
                            }
                    ...
                    //-------------------------------------
                    $tmp["background_color"] = "";
                    if (($iSpec["sisa"] <= 1) && ($iSpec["nilai_ppn_masukan_didelete"] > 0)) {
                        $tmp["background_color"] = "grey";
                    }
                    //-------------------------------------
                    $tmp["editTarget"] = base_url() . $iSpec['handler'] . "/select/" . $this->jenisTr . "?id=" . $iSpec['id'] . "&newQty=";
...
//                arrPrint($itemsSrc);
        ...
        $itemLabels = $itemLabels + $itemNumLabels + $itemNumKodePajak + $itemNumFieldsCustom + array("subtotal" => "sub-amount");
        $itemLabels2 = $itemLabels2 + $itemNumLabels2 + array("subtotal" => "sub-amount");
        $itemLabels3 = $itemLabels3 + $itemNumLabels3 + array("subtotal" => "sub-amount");
        $itemSrcLabels = $itemSrcLabels + $itemSrcNumLabels + array("subtotal" => "sub-amount");
        $itemsLabelTarget1 = $itemSrcLabels + $itemSrcNumLabels + array("subtotal" => "sub-amount");
                        ...
                        if (sizeof($addRowsConfigs[$eName][$currentValue]) > 0) {
                            ...
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                ...
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    if (!isset($oValSpec['hideRow'])) {
                                        $addRowLabels[$oValueName] = $oValSpec['label'];
                                        if (isset($oValSpec["addKolom"]) && (sizeof($oValSpec["addKolom"]) > 0)) {
                                            foreach ($oValSpec["addKolom"] as $add_kolom => $add_label) {

                                                $addRowKolomLabels[$oValueName][$add_kolom] = $add_label;
                                            }
                                        }
                                    }
                                }
        ...
        //endregion
//arrPrintCyan($addRowLabels);
//arrPrint($addRowKolomLabels);
...
//                    if (!isset($iiSpec[$fieldsCek]) || ($iiSpec[$fieldsCek] == NULL) || ($iiSpec[$fieldsCek] == 0)) {
        ...
        $qstrLabelsMasukan = isset($this->configUi[$this->jenisTr]["qstrLabelsMasukan"]) ? $this->configUi[$this->jenisTr]["qstrLabelsMasukan"] : array();
        $selectProcessorRemove = array(
            "items" => MODUL_PATH . $this->configUi[$this->jenisTr]["selectorRemove"] . "/" . $this->jenisTr,
            "itemsSrc" => MODUL_PATH . $this->configUi[$this->jenisTr]["selectorRemove2"] . "/" . $this->jenisTr,
            "itemsTarget1" => MODUL_PATH . $this->configUi[$this->jenisTr]["selectorRemove3"] . "/" . $this->jenisTr,
            "itemsSrc1" => MODUL_PATH . $this->configUi[$this->jenisTr]["selectorRemove4"] . "/" . $this->jenisTr,
        );
        ...
        $data = array(
            ...
            "shopingCartEmbededTaxItem" => $shopingCartEmbededTaxItem,
            //----
            "addRowKolomLabels" => $addRowKolomLabels,
            "qstrLabelsKeluaran" => $qstrLabelsKeluaran,
            "qstrLabelsMasukan" => $qstrLabelsMasukan,
            "selectProcessorRemove" => $selectProcessorRemove,
        );
            ...
            switch ($_SESSION[$cCode]["main"]["paymentmode"]) {
                case "valas":
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPreProcessorDetail = isset($_SESSION[$cCode]['revert']['preProc']['detail']) ? $_SESSION[$cCode]['revert']['preProc']['detail'] : array();
                    }
                    else {
                        $iteratorPreProcessorDetail = isset($this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget]['detail'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPreProcessorMaster = isset($_SESSION[$cCode]['revert']['preProc']['master']) ? $_SESSION[$cCode]['revert']['preProc']['master'] : array();
                    }
                    else {
                        $iteratorPreProcessorMaster = isset($this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget]['master'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorComponentDetail = isset($_SESSION[$cCode]['revert']['jurnal']['detail']) ? $_SESSION[$cCode]['revert']['jurnal']['detail'] : array();
                        $revertedTarget = $_SESSION[$cCode]['main']['pihakExternID'];
                    }
                    else {
                        $iteratorComponentDetail = isset($this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget]['detail'] : array();
                        $revertedTarget = "";
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorComponentMaster = isset($_SESSION[$cCode]['revert']['jurnal']['master']) ? $_SESSION[$cCode]['revert']['jurnal']['master'] : array();
                    }
                    else {
                        $iteratorComponentMaster = isset($this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget]['master'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPostProcessorDetail = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['detail'] : array();
                    }
                    else {
                        $iteratorPostProcessorDetail = isset($this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['detail'] : array();
                    }
                    $iteratorPostProcessorSubDetail = isset($this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['sub_detail']) ? $this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['sub_detail'] : array();
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPostProcessorMaster = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['master'] : array();
                    }
                    else {
                        $iteratorPostProcessorMaster = isset($this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]['master'] : array();
                    }

                    //====registri value-gate
                    if (isset($this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget]) && sizeof($this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget])) {
                        $jurnalIndex = $this->configCore[$this->jenisTr]['componentsValas'][$jenisTrTarget];
                    }
                    else {
                    ...
                    //---------------------------------------------------
                    if (isset($this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget]) && sizeof($this->configCore[$this->jenisTr]['postProcessorValas'][$jenisTrTarget])) {
                    ...
                    //---------------------------------------------------
                    if (isset($this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget]) && sizeof($this->configCore[$this->jenisTr]['preProcessorValas'][$jenisTrTarget])) {
                ...
                case "reguler":
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        ...
                        $iteratorPreProcessorDetail = isset($this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['detail'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPreProcessorMaster = isset($_SESSION[$cCode]['revert']['preProc']['master']) ? $_SESSION[$cCode]['revert']['preProc']['master'] : array();
                    }
                    else {
                        $iteratorPreProcessorMaster = isset($this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['master'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorComponentDetail = isset($_SESSION[$cCode]['revert']['jurnal']['detail']) ? $_SESSION[$cCode]['revert']['jurnal']['detail'] : array();
                        $revertedTarget = $_SESSION[$cCode]['main']['pihakExternID'];
                        ...
                        $iteratorComponentDetail = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail'] : array();
                        $revertedTarget = "";
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorComponentMaster = isset($_SESSION[$cCode]['revert']['jurnal']['master']) ? $_SESSION[$cCode]['revert']['jurnal']['master'] : array();
                        ...
                        $iteratorComponentMaster = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['master'] : array();
                    }
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPostProcessorDetail = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['detail'] : array();
                    }
                    else {
                        $iteratorPostProcessorDetail = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['detail'] : array();
                    }
                    $iteratorPostProcessorSubDetail = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['sub_detail']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['sub_detail'] : array();
                    if (isset($this->configCore[$this->jenisTr]['relativeComponets']) && $this->configCore[$this->jenisTr]['relativeComponets'] == true) {
                        $iteratorPostProcessorMaster = isset($_SESSION[$cCode]['revert']['postProc']['detail']) ? $_SESSION[$cCode]['revert']['postProc']['master'] : array();
                    }
                    else {
                        $iteratorPostProcessorMaster = isset($this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['master']) ? $this->configCore[$this->jenisTr]['postProcessor'][$jenisTrTarget]['master'] : array();
                    }
...
//            $buildTablesDetail = isset($this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail']) ? $this->configCore[$this->jenisTr]['components'][$jenisTrTarget]['detail'] : array();
            $buildTablesMaster = $iteratorComponentMaster;
            $buildTablesDetail = $iteratorComponentDetail;
            ...
            if (sizeof($iteratorPreProcessorDetail) > 0) {
                ...
                if (sizeof($iteratorPreProcessorDetail) > 0) {
                    foreach ($iteratorPreProcessorDetail as $cCtr => $tComSpec) {
            ...
            if (sizeof($iteratorPreProcessorMaster) > 0) {
                $itemNumLabels = isset($this->configUi[$this->jenisTr]['shoppingCartNumFields']) ? $this->configUi[$this->jenisTr]['shoppingCartNumFields'] : array();
                if (sizeof($iteratorPreProcessorMaster) > 0) {
                    foreach ($iteratorPreProcessorMaster as $cCtr => $tComSpec) {
                        $comName = $tComSpec['comName'];
                        $srcGateName = $tComSpec['srcGateName'];
                        $srcRawGateName = $tComSpec['srcRawGateName'];
                        $resultParams = isset($tComSpec['resultParams']) ? $tComSpec['resultParams'] : array();
...
//                        "contents_intext" => isset($aSpec['contents_intext']) ? $aSpec['contents_intext'] : "",
            ...
            $componentConfig['detail'] = $iteratorComponentDetail;
            if ($pakai_ini_cli == 0) {
                if (sizeof($iteratorComponentDetail) > 0) {
                    foreach ($iteratorComponentDetail as $cCtr => $tComSpec) {
                        $tmpOutParams[$cCtr] = array();
                        $gg = 0;
                        $srcGateName = $tComSpec['srcGateName'];
                    ...
                    foreach ($iteratorComponentDetail as $cCtr => $tComSpec) {
                        ...
                        if (isset($_SESSION[$cCode][$srcGateName]) && (sizeof($_SESSION[$cCode][$srcGateName]) > 0)) {
                            ...
                            if ($tobeExecuted) {
                                cekMerah("$comName dieksekusiii src gate" . $srcGateName);
                                arrPrint($tmpOutParams[$cCtr]);
            ...
            if (sizeof($iteratorComponentMaster) > 0) {
                $componentConfig['master'] = $iteratorComponentMaster;
                $cCtr = 0;
                foreach ($iteratorComponentMaster as $cCtr => $tComSpec) {
                    $cCtr++;
                    $comName = $tComSpec['comName'];
                    if (substr($comName, 0, 1) == "{") {
                        $comName = trim($comName, "{");
            ...
            if (sizeof($iteratorPostProcessorDetail) > 0) {
                foreach ($iteratorPostProcessorDetail as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
                    ...
                    if (isset($_SESSION[$cCode][$srcGateName]) && (sizeof($_SESSION[$cCode][$srcGateName]) > 0)) {
                        ...
                        foreach ($_SESSION[$cCode][$srcGateName] as $xid => $dSpec) {
                            ...
                            if (isset($tComSpec['static'])) {
                                ...
                                /**
                                 * tambahan untuk handling E-Billing pajak biar paymentsource gak puyeng buat laporan setor pajak
                                 */
                                if (isset($subParams['static']["setor_id"])) {
                                    $subParams['static']["setor_id"] = $insertID;
                                }
                                if (isset($subParams['static']["setor_nomer"])) {
                                    $subParams['static']["setor_nomer"] = $insertNum;
                                }
                                if (isset($subParams['static']["setor_ebilling"])) {
                                    $subParams['static']["setor_ebilling"] = $_SESSION[$cCode]["main"]["ebilling"];
                                }
                                if (isset($subParams['static']["setor_date"])) {
                                    $subParams['static']["setor_date"] = $_SESSION[$cCode]["main"]["ebillingDate"];
                                }
//                                arrPrint($subParams);
//                                matiHEre();
            ...
            if (sizeof($iteratorPostProcessorSubDetail) > 0) {
                $tmpOutParams = array();
                foreach ($iteratorPostProcessorSubDetail as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
            ...
            if (sizeof($iteratorPostProcessorMaster) > 0) {
                foreach ($iteratorPostProcessorMaster as $cCtr => $tComSpec) {
                    $comName = $tComSpec['comName'];
                    $srcGateName = $tComSpec['srcGateName'];
                    $srcRawGateName = $tComSpec['srcRawGateName'];
...
//                        cekBiru($valueSrc);
                        ...
                        if ($paymentMethod == "update") {
                            ...
                            if (sizeof($tmpData) > 0) {
                                ...
                                cekPink("extern_nilai2 " . $_SESSION[$cCode]['main'][$externSrc['extern_nilai2']]);
                            }
                            ...
                            if (($_SESSION[$cCode]['main'][$valueSrc]) > 1000) {
                                $arrDataPym = array(
                                    "jenis" => $stepCode,
                                    "target_jenis" => $paymentSrcConfig['jenisTarget'],
                                    "reference_jenis" => $paymentSrcConfig['jenisSrc'],
                                    ...
                                    "extern_label2" => (isset($externSrc['extern_label2']) && ($_SESSION[$cCode]['main'][$externSrc['extern_label2']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_label2']] : "",
                                    "extern_nilai2" => (isset($externSrc['extern_nilai2']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai2']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai2']] : 0,
                                    "extern_nilai3" => (isset($externSrc['extern_nilai3']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai3']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai3']] : 0,
                                    "extern_nilai4" => (isset($externSrc['extern_nilai4']) && ($_SESSION[$cCode]['main'][$externSrc['extern_nilai4']])) ? $_SESSION[$cCode]['main'][$externSrc['extern_nilai4']] : 0,
                                    ...
                                    "extern3_id" => makeValue($externSrc['extern3_id'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                    "extern3_nama" => makeValue($externSrc['extern3_nama'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                    "extern4_id" => makeValue($externSrc['extern4_id'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                    "extern4_nama" => makeValue($externSrc['extern4_nama'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                    "extern5_id" => makeValue($externSrc['extern5_id'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                    "extern5_nama" => makeValue($externSrc['extern5_nama'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                );
                                // arrPrintWebs($arrDataPym);
                                // cekPink("extern_nilai2 ".$_SESSION[$cCode]['main'][$externSrc['extern_nilai2']]." ->>>>>".$externSrc['extern2_id']);
            ...
            $baseRegistries = array(
                ...
                'itemsTarget1' => isset($_SESSION[$cCode]['itemsTarget1']) ? $_SESSION[$cCode]['itemsTarget1'] : array(),
...
// START OF COMPLETE REPEATED LOGIC
    public function saveFaktur()
    {
        ...
        $dateFaktur = isset($_GET['dateFaktur']) ? $_GET['dateFaktur'] : "";
        $eFaktur = isset($_GET['eFaktur']) ? $_GET['eFaktur'] : "";
        $skipFaktur = isset($_GET['skip_faktur']) && $_GET['skip_faktur'] == "true" ? true : false;

        if ($dateFaktur != "" && isset($_SESSION[$cCode]['items6_sum'][0])) {
            $_SESSION[$cCode]['items6_sum'][0]['dateFaktur'] = $dateFaktur;
        }
        if ($eFaktur != "" && isset($_SESSION[$cCode]['items6_sum'][0])) {
            $_SESSION[$cCode]['items6_sum'][0]['eFaktur'] = $eFaktur;
        }
        if ($skipFaktur) {
        ...
        if ($this->transaksiMaintenance === true) {
            $msg = isset($this->transaksiMaintenanceMsg['title']) ? $this->transaksiMaintenanceMsg['title'] . "<br>" . $this->transaksiMaintenanceMsg['mesage'] : "Maintenance Mode";
            mati_disini($msg);
        }
        $allowCreateFaktur = isset($configUi[$jenisTr]['allowCreateFaktur']) ? $configUi[$jenisTr]['allowCreateFaktur'] : false;
        if (!$allowCreateFaktur) {
            mati_disini("Transaksi ini tidak mengizinkan simpan E-Faktur saja. Silahkan hubungi admin untuk aktifasi");
        }

        $fakturRef = isset($configUi[$jenisTr]['creteFakturReference']) ? $configUi[$jenisTr]['creteFakturReference'] : array();
        $targetCode = isset($fakturRef['target_code']) ? $fakturRef['target_code'] : "4892";
        $validateFields = isset($fakturRef['validate']) ? $fakturRef['validate'] : array('eFaktur', 'dateFaktur');

        if (!isset($_SESSION[$cCode])) {
            mati_disini("Sesi transaksi tidak ditemukan atau telah kadaluarsa.");
        }
        $formulirFaktur = isset($_SESSION[$cCode]['items6_sum']) ? $_SESSION[$cCode]['items6_sum'] : (isset($_SESSION[$cCode]['items6']) ? $_SESSION[$cCode]['items6'] : array());
        $skipFaktur = isset($_SESSION[$cCode]['main']['skip_faktur']) && $_SESSION[$cCode]['main']['skip_faktur'] == "true" ? true : false;

        $fakturValid = true;
        if (!$skipFaktur) {
            if (sizeof($formulirFaktur) > 0) {
                foreach ($formulirFaktur as $fSpec) {
                    foreach ($validateFields as $vField) {
                        if (!isset($fSpec[$vField]) || trim($fSpec[$vField]) == "") {
                            $fakturValid = false;
                            break 2;
                        }
                    }
        ...
        if (!$fakturValid) {
            $msgWarning = 'Silahkan lengkapi Nomor E-Faktur dan Tanggal Terbitnya terlebih dahulu.';
            echo "<script>
                var msg = " . json_encode($msgWarning) . ";
                if (typeof top !== 'undefined' && typeof top.swal === 'function') {
                    top.swal({ type: 'warning', title: 'E-Faktur Belum Lengkap', text: msg });
                } else if (typeof swal === 'function') {
                    swal({ type: 'warning', title: 'E-Faktur Belum Lengkap', text: msg });
                } else if (typeof top !== 'undefined' && typeof top.alert === 'function') {
                    top.alert(msg);
                } else {
                    alert(msg);
                }
        ...
        // 1. Sanitasi input
        $eFakturClean = trim(strip_tags($eFaktur));
        $dateFakturClean = trim(strip_tags($dateFaktur));

        // 2. Proteksi Duplikasi Nomor E-Faktur Global (ISO Standard: Unik Mutlak di Seluruh Tabel)
        if (!$skipFaktur && strlen($eFakturClean) >= 10) {
            $mDup = new MdlPaymentSource();
            $this->db->where('extern_label2', $eFakturClean);
//            $this->db->or_where('extern_kode2', $eFakturClean);
            $this->db->limit(1);
            $dupSources = $mDup->lookupAll()->result();

            if (sizeof($dupSources) > 0) {
                $dSrc = $dupSources[0];
                $infoNomer = isset($dSrc->nomer) && trim($dSrc->nomer) != "" ? trim($dSrc->nomer) : (isset($dSrc->transaksi_id) ? "ID Transaksi: " . $dSrc->transaksi_id : "-");
                $infoOleh = isset($dSrc->oleh_nama) && trim($dSrc->oleh_nama) != "" ? trim($dSrc->oleh_nama) : (isset($dSrc->oleh_id) ? "User ID: " . $dSrc->oleh_id : "-");
                $infoDtime = isset($dSrc->dtime) && trim($dSrc->dtime) != "" ? trim($dSrc->dtime) : "-";

                $htmlContent = "<div style='text-align: left; background: #fff5f5; border: 1px solid #ffcdd2; padding: 14px; border-radius: 8px; margin-top: 10px; font-size: 13px; color: #333; line-height: 1.5;'>" .
                    "<div style='margin-bottom: 10px; font-weight: bold; color: #c62828;'>🛑 Nomor E-Faktur ('" . $eFakturClean . "') sudah terdaftar pada sistem!</div>" .
                    "<table style='width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 13px;'>" .
                    "  <tr><td style='padding: 3px 0; width: 140px; color: #555;'>• Digunakan Pada</td><td style='font-weight: bold;'>: " . $infoNomer . "</td></tr>" .
                    "  <tr><td style='padding: 3px 0; color: #555;'>• Di-entry Oleh</td><td style='font-weight: bold;'>: " . $infoOleh . "</td></tr>" .
                    "  <tr><td style='padding: 3px 0; color: #555;'>• Waktu Input</td><td style='font-weight: bold;'>: " . $infoDtime . "</td></tr>" .
                    "</table>" .
                    "<div style='font-size: 12px; color: #666; border-top: 1px dashed #ffcdd2; padding-top: 8px; margin-top: 8px;'>" .
                    "<i>Satu Nomor E-Faktur tidak boleh digunakan pada transaksi lain.</i>" .
                    "</div>" .
                    "</div>";

                echo "<script>
                    var msgHtml = " . json_encode($htmlContent) . ";
                    setTimeout(function() {
                        if (typeof top !== 'undefined' && typeof top.swal === 'function') {
                            top.swal({ type: 'warning', title: 'Duplikasi Nomor E-Faktur', html: msgHtml, confirmButtonColor: '#d33' });
                        } else if (typeof swal === 'function') {
                            swal({ type: 'warning', title: 'Duplikasi Nomor E-Faktur', html: msgHtml, confirmButtonColor: '#d33' });
                        } else {
                            alert('Nomor E-Faktur sudah terdaftar pada transaksi lain!');
                        }
                    }, 400);
                    if (typeof top !== 'undefined' && typeof top.close_holdon === 'function') { top.close_holdon(); }
                </script>";
                exit();
            }
        }

        // 3. Proteksi Single Nota Re-entry
        if (sizeof($formulirFaktur) > 0) {
            foreach ($formulirFaktur as $fSpec) {
                $notaId = isset($fSpec['transaksi_id']) ? $fSpec['transaksi_id'] : 0;
                if ($notaId > 0) {
                    $mPay = new MdlPaymentSource();
                    $mPay->addFilter("transaksi_id='$notaId'");
                    $mPay->addFilter("extern_kode2!='0'");
                    $this->db->limit(1);
                    $existingSources = $mPay->lookupAll()->result();

                    if (sizeof($existingSources) > 0) {

                        $exSrc = $existingSources[0];
                        $infoNomer = isset($exSrc->nomer) && trim($exSrc->nomer) != "" ? trim($exSrc->nomer) : "ID Nota: " . $notaId;
                        $infoOleh = isset($exSrc->oleh_nama) && trim($exSrc->oleh_nama) != "" ? trim($exSrc->oleh_nama) : (isset($exSrc->oleh_id) ? "User ID: " . $exSrc->oleh_id : "-");
                        $infoDtime = isset($exSrc->dtime) && trim($exSrc->dtime) != "" ? trim($exSrc->dtime) : "-";
                        $infoFaktur = isset($exSrc->extern_kode2) && trim($exSrc->extern_kode2) != "" ? trim($exSrc->extern_kode2) : (isset($exSrc->extern_kode2) ? trim($exSrc->extern_kode2) : "-");

                        $htmlContent = "<div style='text-align: left; background: #fff5f5; border: 1px solid #ffcdd2; padding: 14px; border-radius: 8px; margin-top: 10px; font-size: 13px; color: #333; line-height: 1.5;'>" .
                            "<div style='margin-bottom: 10px; font-weight: bold; color: #c62828;'>🛑 Nota ini sudah memiliki E-Faktur terdaftar!</div>" .
                            "<table style='width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 13px;'>" .
                            "  <tr><td style='padding: 3px 0; width: 140px; color: #555;'>• Nomor Transaksi</td><td style='font-weight: bold;'>: " . $infoNomer . "</td></tr>" .
                            "  <tr><td style='padding: 3px 0; color: #555;'>• Nomor E-Faktur</td><td style='font-weight: bold;'>: " . $infoFaktur . "</td></tr>" .
                            "  <tr><td style='padding: 3px 0; color: #555;'>• Di-entry Oleh</td><td style='font-weight: bold;'>: " . $infoOleh . "</td></tr>" .
                            "  <tr><td style='padding: 3px 0; color: #555;'>• Waktu Input</td><td style='font-weight: bold;'>: " . $infoDtime . "</td></tr>" .
                            "</table>" .
                            "<div style='font-size: 12px; color: #666; border-top: 1px dashed #ffcdd2; padding-top: 8px; margin-top: 8px;'>" .
                            "<i>E-Faktur tidak dapat disimpan ulang untuk nota ini.</i>" .
                            "</div>" .
                            "</div>";
                        ...
                        echo "<script>
                            ...
                            setTimeout(function() {
                                if (typeof top !== 'undefined' && typeof top.swal === 'function') {
                                    top.swal({ type: 'warning', title: 'Transaksi Sudah Memiliki E-Faktur', html: msgHtml, confirmButtonColor: '#d33' });
                                } else if (typeof swal === 'function') {
                                    swal({ type: 'warning', title: 'Transaksi Sudah Memiliki E-Faktur', html: msgHtml, confirmButtonColor: '#d33' });
                                } else {
                                    alert('Nota ini sudah memiliki E-Faktur terdaftar!');
                                }
                            }, 400);
                            if (typeof top !== 'undefined' && typeof top.close_holdon === 'function') { top.close_holdon(); }
                        </script>";
        ...
        $pakai_ini = 1;
        if ($pakai_ini) {
            $connector = $targetCode;
//        matiHere($connector);
            $oldCode = $cCode;
            $cCode = $cCode = "_TR_" . $connector;
            $this->jenis = $jenisTr = $connector;
            $configUiMasterModulJenis = loadConfigModulJenis_he_misc($connector, "coTransaksiUi");
            $configCoreMasterModulJenis = loadConfigModulJenis_he_misc($connector, "coTransaksiCore");
            $configLayoutMasterModulJenis = loadConfigModulJenis_he_misc($connector, "coTransaksiLayout");
            $configValuesMasterModulJenis = loadConfigModulJenis_he_misc($connector, "coTransaksiValues");
            $modul_transaksi = $this->config->item("heTransaksi_ui")[$connector]["modul"];
            $tCodeTargetJenisTransaksi = $configUiMasterModulJenis['steps'][1]['target'];
//        arrPrint($configCoreMasterModulJenis);
//        matiHere();
            $iteratorComponentMaster = $iteratorComponentMaster = isset($configCoreMasterModulJenis['components'][$tCodeTargetJenisTransaksi]['master']) ? $configCoreMasterModulJenis['components'][$tCodeTargetJenisTransaksi]['master'] : array();
            $iteratorComponentDetail = isset($configCoreMasterModulJenis['components'][$tCodeTargetJenisTransaksi]['detail']) ? $configCoreMasterModulJenis['components'][$tCodeTargetJenisTransaksi]['detail'] : array();
            $iteratorPostProcessorDetail = isset($configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['detail']) ? $configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['detail'] : array();
            $iteratorPostProcessorMaster = isset($configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['master']) ? $configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['master'] : array();
            $iteratorPreProcessorDetail = isset($configCoreMasterModulJenis['preProcessor'][$tCodeTargetJenisTransaksi]['detail']) ? $configCoreMasterModulJenis['preProcessor'][$tCodeTargetJenisTransaksi]['detail'] : array();
            $jurnalPostProc = $configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi];
            $jurnalIndex = $configCoreMasterModulJenis['components'][$tCodeTargetJenisTransaksi];
            $jurnalPreProc = $configCoreMasterModulJenis['preProcessor'][$tCodeTargetJenisTransaksi];
            $iteratorPreProcessorMaster = isset($configCoreMasterModulJenis['preProcessor'][$tCodeTargetJenisTransaksi]['master']) ? $configCoreMasterModulJenis['preProcessor'][$tCodeTargetJenisTransaksi]['master'] : array();
            $iteratorPostProcessorSubDetail = isset($configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['sub_detail']) ? $configCoreMasterModulJenis['postProcessor'][$tCodeTargetJenisTransaksi]['sub_detail'] : array();
            if (sizeof($configUiMasterModulJenis) == 0) {
                die("kode connector tidak dikenali!");
            }
            ...
            $_SESSION[$cCode] = array(
                ...
                "tableIn_detail_values" => isset($_SESSION[$oldCode]['tableIn_detail_values2_sum']) ? $_SESSION[$oldCode]['tableIn_detail_values2_sum'] : (isset($_SESSION[$oldCode]['tableIn_detail_values']) ? $_SESSION[$oldCode]['tableIn_detail_values'] : array()),
                "tableIn_detail_values_rsltItems" => isset($_SESSION[$oldCode]['tableIn_detail_values_rsltItems']) ? $_SESSION[$oldCode]['tableIn_detail_values_rsltItems'] : array(),
                'tableIn_detail_values_rsltItems2' => isset($_SESSION[$oldCode]['tableIn_detail_values_rsltItems2']) ? $_SESSION[$oldCode]['tableIn_detail_values_rsltItems2'] : array(),
                'tableIn_detail_values2_sum' => isset($_SESSION[$oldCode]['tableIn_detail_values2_sum']) ? $_SESSION[$oldCode]['tableIn_detail_values2_sum'] : array(),
            );
...
//arrPrint($_SESSION[$cCode]["main"]);
            ...
            if (sizeof($iteratorPreProcessorMaster) > 0) {
                ...
                $this->load->helper("he_value_builder");
                fillValues_he_value_builder($tCodeTargetJenisTransaksi, 1, 1, $configCoreMasterModulJenis, $configUiMasterModulJenis, $configValuesMasterModulJenis);
            } else {
                // no processor defined
            }
            //endregion

            //jalankan ulang value builder
            fillValues_he_value_builder($tCodeTargetJenisTransaksi, 1, 1, $configCoreMasterModulJenis, $configUiMasterModulJenis, $configValuesMasterModulJenis);
//            mati_disini(__LINE__);
            $arrAlert = array(
                "type" => "success",
...
//                    }
            ...
            //==filter nilai, jika NOL tidak dikirim, sesuai config==
            $compValidators = ($this->config->item('transaksi_value_required_components') != null) ? $this->config->item('transaksi_value_required_components') : array();
//arrPrint($iteratorComponentMaster);
//matiHere();
            if (sizeof($iteratorComponentMaster) > 0) {
//            matiHEre(__LINE__);
                $componentConfig['master'] = $iteratorComponentMaster;
                ...
                foreach ($iteratorComponentMaster as $cCtr => $tComSpec) {
                    ...
                    if (isset($tComSpec['static2'])) {
                        ...
                        $tmpOutParams['static2']["fulldate"] = date("Y-m-d");
                        $tmpOutParams['static2']["dtime"] = date("Y-m-d H:i:s");
                        $tmpOutParams['static2']["keterangan"] = $configUiMasterModulJenis['steps'][$tCodeTargetJenisTransaksi]['label'] . " nomor " . $tmpNomorNota . " oleh " . $this->session->login['nama'];
...
//                                matiHEre();
            ...
            if (sizeof($iteratorPostProcessorMaster) > 0) {
                foreach ($iteratorPostProcessorMaster as $cCtr => $tComSpec) {
                    ...
                    if (isset($tComSpec['static2'])) {
                        ...
                        $tmpOutParams['static2']["fulldate"] = date("Y-m-d");
                        $tmpOutParams['static2']["dtime"] = date("Y-m-d H:i:s");
                        $tmpOutParams['static2']["keterangan"] = $configUiMasterModulJenis['steps'][1]['label'] . " nomor " . $tmpNomorNota . " oleh " . $this->session->login['nama'];
            ...
            $doWriteReg = $tr->writeDataRegistries($insertID, $baseRegistries) or die(lgShowError("Ada kesalahan", "Gagal saat berusaha  write base params into registries"));
//matiHEre(__LINE__."Transaksi berhasil disimpan");
            $this->db->trans_complete() or die("Gagal saat berusaha  commit transaction!");
        }
        if (isset($_SESSION[$cCode])) {
            unset($_SESSION[$cCode]);
        ...
        $this->session->errMsg = "E-Faktur PPN Masukan berhasil disimpan dan Jurnal PPN Masukan telah dicatat (Kas/Bank Rp 0).";
        $msgSuccess = $this->session->errMsg;
        echo "<script>
            var msg = " . json_encode($msgSuccess) . ";
            setTimeout(function() {
                if (typeof top !== 'undefined' && typeof top.swal === 'function') {
                    top.swal({
                        type: 'success',
                        title: 'E-Faktur Berhasil Disimpan',
                        text: msg,
        ...
        unset($this->session->errMsg);
        echo "<script>close_holdon();</script>";
        exit();

    }

    public function checkDuplicateEfakturAjax()
    {
        $eFaktur = isset($_GET['eFaktur']) ? trim(strip_tags($_GET['eFaktur'])) : "";
        if (strlen($eFaktur) < 10) {
            echo json_encode(array("is_duplicate" => false));
            exit();
        }

        $this->load->model("Mdls/MdlPaymentSource");
        $mDup = new MdlPaymentSource();
        $this->db->where('extern_label2', $eFaktur);
//        $this->db->or_where('extern_kode2', $eFaktur);
        $this->db->limit(1);
        $dupSources = $mDup->lookupAll()->result();

        if (sizeof($dupSources) > 0) {
            $dSrc = $dupSources[0];
            $infoNomer = isset($dSrc->nomer) && trim($dSrc->nomer) != "" ? trim($dSrc->nomer) : (isset($dSrc->transaksi_id) ? "ID: " . $dSrc->transaksi_id : "-");
            $infoOleh = isset($dSrc->oleh_nama) && trim($dSrc->oleh_nama) != "" ? trim($dSrc->oleh_nama) : (isset($dSrc->oleh_id) ? "User ID: " . $dSrc->oleh_id : "-");
            $infoDtime = isset($dSrc->dtime) && trim($dSrc->dtime) != "" ? trim($dSrc->dtime) : "-";
