...
//        arrPrint($this->session->login);
    ...
    public function validate()
    {
        ...
        $transaksiValidator = isset($this->configUi[$this->jenisTr]["transaksiValidator"]) ? $this->configUi[$this->jenisTr]["transaksiValidator"] : array();
...
//                                    $errMsgs[] = "$label $name must be > 0";
        ...
        if (sizeof($transaksiValidator) > 0) {
            $keycek = $transaksiValidator["key"];
            $keycekmode = isset($_SESSION[$cCode]["main"][$keycek]) ? $_SESSION[$cCode]["main"][$keycek] : "reguler";
            $keycekvalidatePaymentItemsValidator = isset($transaksiValidator["mode"][$keycekmode]["shopingCartPaymentItemsValidator"]) ? $transaksiValidator["mode"][$keycekmode]["shopingCartPaymentItemsValidator"] : array();
            $keycekvalidateRowNumValidators = isset($transaksiValidator["mode"][$keycekmode]["shoppingCartRowNumValidators"]) ? $transaksiValidator["mode"][$keycekmode]["shoppingCartRowNumValidators"] : array();
            if (sizeof($keycekvalidatePaymentItemsValidator) > 0) {
                foreach ($keycekvalidatePaymentItemsValidator as $key => $spec) {
                    $target_key = $spec["target"];
                    $label_cek = $spec["label"];
                    $label2_cek = $spec["label2"];
                    if (isset($_SESSION[$cCode]['main'][$target_key]) && ($_SESSION[$cCode]['main'][$target_key] < 1)) {
            ...
            if (sizeof($keycekvalidateRowNumValidators) > 0) {
                foreach ($keycekvalidateRowNumValidators as $field => $label) {
                    if (!isset($_SESSION[$cCode]['main'][$field])) {
                        $errMsgs[] = "$label wajib dilengkapi";
                        $errRows[] = $field;
                    }
...
//            arrPrintCyan($total_item_sumber_dana);
         ...
         * PURPOSE   : Mencegah race condition validasi e-Faktur dengan Upfront Request Sync (Three-Layer Protection).
         * COMPLIANCE: ISO 9001, ISO 27001 Annex A.8.2, ISO/IEC 25010, PSAK, Coretax DJP
         * LOG_EXPIRE: 2026-11-12
         * [/AGENT_LOG]
         */
        if (isset($_REQUEST['eFaktur']) && trim($_REQUEST['eFaktur']) !== '') {
            $eFakturReq = trim($_REQUEST['eFaktur']);
            $_SESSION[$cCode]['main']['eFaktur'] = $eFakturReq;
            if (isset($_SESSION[$cCode]['items6_sum']) && is_array($_SESSION[$cCode]['items6_sum'])) {
                foreach ($_SESSION[$cCode]['items6_sum'] as $iKey => $iVal) {
                    $_SESSION[$cCode]['items6_sum'][$iKey]['eFaktur'] = $eFakturReq;
                }
            }
        }
        if (isset($_REQUEST['dateFaktur']) && trim($_REQUEST['dateFaktur']) !== '') {
            $dateFakturReq = trim($_REQUEST['dateFaktur']);
            $_SESSION[$cCode]['main']['dateFaktur'] = $dateFakturReq;
            if (isset($_SESSION[$cCode]['items6_sum']) && is_array($_SESSION[$cCode]['items6_sum'])) {
                foreach ($_SESSION[$cCode]['items6_sum'] as $iKey => $iVal) {
                    $_SESSION[$cCode]['items6_sum'][$iKey]['dateFaktur'] = $dateFakturReq;
                }
            }
        }
                            ...
                            if (isset($_SESSION[$cCode]["main"][$vall]) && $_SESSION[$cCode]["main"][$vall] > 0) {
                                foreach ($koloms as $keyy => $labell) {
                                    if (!isset($_SESSION[$cCode]["main"][$keyy]) || ($_SESSION[$cCode]["main"][$keyy] == NULL)) {
                                        //cek souirce dulu
                                        if(isset($_SESSION[$cCode][$gateSource])){
                                            foreach ($_SESSION[$cCode][$gateSource] as $ii => $iiSpec){
                                                if (!isset($iiSpec[$keyy]) || ($iiSpec[$keyy] == NULL)){
                                                    $errMsgs[] = "$labell **$keyy*";
                                                }
                                            }
                                        }
...
//        echo($_SERVER['REMOTE_ADDR']);
        ...
        $link_undoneListValas = MODUL_PATH . "Transaksi/selectPaymentExternSrcValas/$jenisTr/?gr=cGVtYmVsaWFu&ohyes=ohno&step=$getStep";
...
\n\n
        ...
        if ($this->jenisTr == "1488") {
            $scriptBottom .= "<script>
                    var viewundoneListValas = function () {
                         top.$('#undoneListValas').html(\"<div class='row' style = 'margin-top: 14px; padding-bottom: 14px;' > <span class='col-lg-12 text-center' ><img width = '5%' src = '$baseUrl/assets/images/loading_16_p.gif'></span></div>\");
                    setTimeout(function () {
                                      if (top.$('#undoneListValas').find('img').length > 0) {
                                          top.$('#undoneListValas').load('$link_undoneListValas', function(){viewundoneListValas})
                                          }
                                      },1000);
                    }\n
                    viewundoneListValas()
    ...
    public function selectPaymentExternSrc()
    {
        ...
        $shoppingCartReferenceExternFilter = (isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFilterReguler"])) ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFilterReguler"] : array();
        $this->load->model("MdlTransaksi");
...
//                    $tr->addFilter("sisa>1000");
            ...
            if (sizeof($shoppingCartReferenceExternFilter) > 0) {
                foreach ($shoppingCartReferenceExternFilter as $ff) {
                    $tr->addFilter($ff);
                }
            }
...
//            cekMerah($this->db->last_query());
            ...
            if (sizeof($tmpSrc) > 0) {
                foreach ($tmpSrc as $row) {
                    ...
                    if (!in_array($row->extern_id, $externs)) {
                        ...
                        $tmp = (array)$row;
                        $tmp["link"] = MODUL_PATH . get_class($this) . "/selectPaymentSrc/$targetJenis/" . $row->extern_id . "/?paymentmode=reguler";
            ...
            $data = array(
                ...
                "jenisTr" => $this->jenisTr,
                "link_top" => $link_top,
                "labelOngoingReguler" => ($this->configUi[$targetJenis]["labelOngoingReguler"] != NULL) ? $this->configUi[$targetJenis]["labelOngoingReguler"] : "",
            );
            ...
            //deteksi bulan dan tahun pajak untuk filter tgl efaktur (keluaran & masukan)-------
            ...
            elseif (isset($_SESSION["_TR_" . $targetJenis]['main']['date_keluaran']) && $_SESSION["_TR_" . $targetJenis]['main']['date_keluaran']) {
            ...
            $bulan_masukan = isset($_GET["date_masukan"]) ? $_GET["date_masukan"] : date("Y-m");
            if (isset($_GET["date_masukan"])) {
                $bulan_masukan = $_GET["date_masukan"];
            }
            elseif (isset($_SESSION["_TR_" . $targetJenis]['main']['date_masukan']) && $_SESSION["_TR_" . $targetJenis]['main']['date_masukan']) {
                $bulan_masukan = $_SESSION["_TR_" . $targetJenis]['main']['date_masukan'];
            }
            else {
                $bulan_masukan = date("Y-m");
            }
            $bulan_masukan_ex = explode("-", $bulan_masukan);
            $bulan_masukan_ex_1 = $bulan_masukan_ex[0];// tahun
            $bulan_masukan_ex_2 = $bulan_masukan_ex[1];// bulan
...
//            }
            ...
            if ($filter_bulan == true) {
                $tr->addFilter("month(transaksi_payment_source.extern_date2)='$bulan_masukan_ex_2'");
                $tr->addFilter("year(transaksi_payment_source.extern_date2)='$bulan_masukan_ex_1'");
            }
...
//
                    ...
                    switch ($jenis) {
                        ...
                        case "4892"://realisasi ppn masukan dari pembayaran
...
//            $this->db->limit(20);
            ...
            // START OF COMPLETE REPEATED LOGIC
            if ($filter_bulan == true) {
                $_SESSION["_TR_" . $targetJenis]['main']['date_masukan'] = $bulan_masukan;
                $_SESSION["_TR_" . $targetJenis]['main']['date_keluaran'] = $bulan_keluaran;
                if (!isset($_SESSION["_TR_" . $targetJenis]['main']['date_keluaran'])) {
...
//                "bulanMundur" => backCustomMonths(date("Y-m"), 3),
    ...
    public function selectPaymentSrc()
    {
        ...
        $_SESSION[$cCode]["main"]["paymentmode"] = $paymentmode;
        //-----------------------------------------------------
...
//        cekHitam($this->uri->segment(3));
        $qstrLabels = array(
            ...
            "extern_date2" => "extern_date2",//tgl faktur ppn masukan
            "extern_kode2" => "extern_kode2",//nomer faktur ppn masukan
        ...
        $data = array(
            ...
            "paymentSrcDisabled" => isset($paymentSrcDisitemLabelsabled) ? $paymentSrcDisabled : array(),
...
//        matiHere(__LINE__);
    ...
    public function selectPaymentExternSrcValas()
    {
        ...
        $itemLabels = (isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFieldsValas"])) ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFieldsValas"] : $defItemLabels;
        $shoppingCartReferenceExternFilter = (isset($this->configUi[$targetJenis]["shoppingCartReferenceExternFilterValas"])) ? $this->configUi[$targetJenis]["shoppingCartReferenceExternFilterValas"] : array();
...
//            cekMerah($this->db->last_query());
            ...
            if (sizeof($tmpSrc) > 0) {
                foreach ($tmpSrc as $row) {
                    ...
                    if (!in_array($row->extern_id, $externs)) {
                        ...
                        $tmp = (array)$row;
                        $tmp["link"] = MODUL_PATH . get_class($this) . "/selectPaymentSrcValas/$targetJenis/" . $row->extern_id . "/?paymentmode=valas";
                        $tmp["due_date"] = isset($dueEmployee[$row->extern_id]['due_date']) ? formatField_he_format("dtime", $dueEmployee[$row->extern_id]['due_date']) : "-";
                        $tmp["aging"] = isset($dueEmployee[$row->extern_id]['aging']) ? $dueEmployee[$row->extern_id]['aging'] : "-";
                        $tmp["over_due"] = isset($dueEmployee[$row->extern_id]['over_due']) ? $dueEmployee[$row->extern_id]['over_due'] : "-";
                        $tmp["class_marking"] = $classMarking;
            ...
            $link_top = NULL;
            if ($showAll == true) {
                $link_top = MODUL_PATH . get_class($this) . "/selectPaymentSrcValas/$targetJenis/";
            }
            ...
            $data = array(
                ...
                "jenisTr" => $this->jenisTr,
                "link_top" => $link_top,
                "labelOngoingValas" => ($this->configUi[$targetJenis]["labelOngoingValas"] != NULL) ? $this->configUi[$targetJenis]["labelOngoingValas"] : "",
            );
    ...
    public function selectPaymentSrcValas()
    {
        if (!isset($this->session->login['id'])) {
            gotoLogin();
        }
        ...
        $tr->addFilter("transaksi_payment_source.valas_id>0");
        $this->db->order_by("transaksi_payment_source.dtime", "asc");
        $tmpSrc = $tr->lookupPaymentSrcByJenis_joined($master_target)->result();
        cekHitam($this->db->last_query());
        if ($readerDueDate) {
...
//                                arrPrintHitam($detail);
        ...
        $itemTransaksiIDs_blob = blobEncode($itemTransaksiIDs);
        if ($externID > 0) {
            $itemLabels = isset($this->configUi[$targetJenis]["shoppingCartReferenceFieldsValas"]) ? $this->configUi[$targetJenis]["shoppingCartReferenceFieldsValas"] : $defaultItemLabels;
            $subtitle = $externName;
        }
        else {
            $itemLabels = isset($this->configUi[$targetJenis]["shoppingCartReferenceFieldsAll"]) ? $this->configUi[$targetJenis]["shoppingCartReferenceFieldsAll"] : $defaultItemLabels;
