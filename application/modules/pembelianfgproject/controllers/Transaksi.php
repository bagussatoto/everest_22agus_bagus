...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $shoppingCartFieldStokValidators = isset($this->configUi[$this->jenisTr]["shoppingCartFieldStokValidators"]) ? $this->configUi[$this->jenisTr]["shoppingCartFieldStokValidators"] : array();
            ...
            if (isset($_SESSION[$cCode]['items']) && sizeof($_SESSION[$cCode]['items']) > 0) {
                foreach ($_SESSION[$cCode]['items'] as $xid => $iSpec) {
                    ...
                    if ((isset($iSpec['disabled']) && $iSpec['disabled'] == "0") || !isset($iSpec['disabled'])) {
                        ...
                        if (sizeof($shoppingCartFieldStokValidators) > 0) {
                            foreach ($shoppingCartFieldStokValidators as $iii => $kkk) {
                                if ($iSpec[$iii] > $iSpec["stok"]) {
                                    $errMsgs[] = "$nama $kkk. ";
                                }
                            }
                        }
    ...
    {
        cekHere("DISINI.. viewUndoneItemsIndex");
...
//                        else{
                        if ($allowFollowup) {
                            ...
                            $evCom = evaluateComponents_he_menu($this->jenisTr, $currentStepNum, $this->configCoreJenis, $this->configUiJenis);
                            $evMaster = evaluateMain_he_menu($this->jenisTr, $currentStepNum, $this->configCoreJenis, $this->configUiJenis);
                            cekHitam("evPreProcc: $evPre, evPostProcc: $evPost, evCom: $evCom, evMaster: $evMaster");
                            if ($evPre == null && $evPost == null && $evCom == null && $evMaster == null) {
                                $transaksiID_edit = $row->transaksi_id;
                                $transaksiCustomerNama_f = "<strong>" . $row->customers_nama . "</strong>";
                                $transaksiNomer = $row->nomer;
...
//                                $link_edit = MODUL_PATH . "FollowUp/doPreEdit/" . $this->jenisTr . "/$transaksiID_edit/$currentStepNum?mode=autoPrePreview";
                                ...
                                $actionLabel_edit = "Edit";
                                $tmp['action'] .= "<button class='btn btn-danger btn-block btn-xs' disabled title='turn this entry into $stepLabel' href='javascript:void(0)' 
                                style='background-color:#000000;color:#ffffff;'
                                >" . $actionLabel_edit . " </button>";
                            }
                        }
...
//arrPrintHitam($tmp['action']);
    ...
    public function viewUndoneItemsIndex_all()
    {
        cekHere("DISINI.. viewUndoneItemsIndex_all");
        $jenisTrsub = isset($_GET['step']) ? $_GET['step'] : 1;
        $starttime = microtime(true);
        if (!isset($this->session->login['id'])) {
            redirect(base_url() . "Login");
        ...
        $allowPrintQr = isset($this->configUi[$jenisTr]["steps"][$jenisTrsub]["allowPrintQr"]) ? $this->configUi[$jenisTr]["steps"][$jenisTrsub]["allowPrintQr"] : 0;
        $arrItemShow = isset($this->configUi[$jenisTr]["shortItemsFields"]) ? $this->configUi[$jenisTr]["shortItemsFields"] : array();
        $produkUnitDisc = isset($this->configUi[$jenisTr]["produkUnitDisc"]) ? $this->configUi[$jenisTr]["produkUnitDisc"] : array();
...
//        arrPrintCyan($this->accessList[$jenisTr]);
...
//                    arrPrintWebs($hist);
                    foreach ($hist as $hisSpec) {
                        ...
                        $arrIdsHistByTrans[$row->transaksi_id][$hisSpec['step']] = array(
                            "step" => $hisSpec['step'],
                            "trID" => $hisSpec['trID'],
                            "nomer" => $hisSpec['nomer'],
                        );
                    ...
                    if (isset($hist[2])) {
                        $arrID_po[$hist[2]["trID"]] = $hist[2]["trID"];
                    }
                }
            }
            ...
            $relasi_uang_muka = array();
            if (sizeof($arrID_po) > 0) {
                cekHere("cek po dengan relasi uang muka");
                // region uang muka relasi po
//                $this->load->model("Mdls/MdlSupplierUangMukaRek");
//                $tr = new MdlSupplierUangMukaRek();
//                $tr->setFilters(array());
//                $tr->addFilter("periode='forever'");
//                $tr->addFilter("extern2_id in ('" . implode("','", $arrID_po) . "')");
...
//                    }
//                }
                // endregion uang muka relasi po

                // region uang muka relasi po dan ppn
                ...
                if (sizeof($trTmpSrc) > 0) {
                    foreach ($trTmpSrc as $trTmpSrcSpec) {
                        ...
                        $relasi_uang_muka[$trTmpSrcSpec->extern2_id] = array(
//                        $transaksiReferensiUangMukaPpn[$trTmpSrcSpec->extern2_id] = array(
                ...
                // endregion uang muka relasi po dan ppn
            }
...
//            arrprint($this->accessList[$jenisTr]);
//            arrprint($arrIdsHistByTrans);
            $numb = 0;
            foreach ($tmpHist as $row) {
                //----
                foreach ($arrIdsHistByTrans[$row->transaksi_id] as $st => $stSpec) {
                    $nk = "transaksi_id_ref_" . $st;
                    $row->$nk = $stSpec["trID"];
                }
                //----
                $extHistoryFields2 = isset($arrExtHistoryFields2[$row->step_number]) ? $arrExtHistoryFields2[$row->step_number] : array();
                ...
                if ($pakai_ini == 1) {
                    ...
                    $tblItems = $detail;
                    // ini free produk
                    $free_produk = viewFreeProduk($tmpReg_result[$row->transaksi_id], $produkUnitDisc["viewFields"]);
                }

                $tmp['free_produk'] = $free_produk;
                $tmp['item_fields'] = $tblItems;
                $tmp['next_pic'] = "-";
                if (sizeof($arrNextPIC) > 0) {
                    if (isset($arrNextPIC[$row->next_substep_code][$row->next_substep_num])) {
...
//                    }
                    ...
                    $tmp['action'] .= "<button class='btn btn-default btn-block' title='turn this entry into $stepLabel' href='javascript:void(0)'
                                ><span class='meta'>" . $actionLabel . "</span></button>";
...
//                                $label_image = preg_replace('/\d{10}/', '...', $relImages[$row->id_master]["nama"]);
                ...
                if ($row->requestReferenceSoID > 0) {
//                    cekHere("dikirim langsung ke konsumen " . $row->transaksi_id);
                    $tmpMark['style'] = "background-color:pink;";
                    $tmp['keterangan'] .= "<span style='color:red;'>-</span>";
                }

                //----
                if (array_key_exists($row->transaksi_id_ref_2, $relasi_uang_muka)) {
//                    cekHitam("punya relasi uang muka");
                    $tmpMark['style'] = ($row->partial == 1) ? "background-color:yellow;" : "background-color:#20fc03;";
                    $tmp['keterangan'] .= "<span style='color:red;font-weight:bolder;'><br>segera GRN, jika sudah tiba.</span>";
                }
                //----
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
            ...
            "reloadLink" => MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$jenisTrsub",
        );

        $this->load->view("transaksi", $data);
    }

    public function viewRequestItems__()
    {
            ...
            if (sizeof($tmp) > 0) {
                ...
                foreach ($tmp as $row) {
                    ...
                    if ($row->debet > 0 || $row->kredit > 0) {
                        $strLink = $defLink;
                        if ($row->urut != "99") {// urut 99 maka dihidden

                            $lItems[$row->cabang_id][$row->urut][] = array(
                                "dtime" => $row->dtime,
                                "rekening_coa" => $row->rekening,
                            ...
                            $lItems_99[$row->cabang_id][$row->urut][] = array(
                                "dtime" => $row->dtime,
                                "rekening_coa" => $row->rekening,
                                "rekening" => $row->debet > 0 ? $rekening_alias : "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . $rekening_alias,
                                "debet" => $row->debet > 0 ? $row->debet : "",
...
//                    arrPrintPink($spec);
                ...
                foreach ($dsTmp as $spec) {
                    $dsData[$spec->id] = $spec->label;
                    $dsDataCek[$spec->id] = $spec->label;
                }
                unset($dsDataCek[6]);
                unset($dsDataCek[7]);
                $dsDataCekKeys = array_keys($dsDataCek);
                $lsd = New MdlLockerStockDiskonVendor();
                $lsd->addFilter("transaksi_id='$indTransksiID'");
                $lsd->addFilter("extern_id in ('" . implode("','", $dsDataCekKeys) . "')");
                $lsdTmp = $lsd->lookupAll()->result();
//                showLast_query("biru");
                $arrDiskonData = array();
                if (sizeof($lsdTmp) > 0) {
                    foreach ($lsdTmp as $lsdSpecc) {
                        $lsdSpec = (array)$lsdSpecc;
                        $arrDiskonData[$lsdSpec["extern_id"]]["id"] = $lsdSpec["extern_id"];
                        $arrDiskonData[$lsdSpec["extern_id"]]["nama"] = $lsdSpec["extern_nama"];
                        $arrDiskonData[$lsdSpec["extern_id"]]["alias"] = isset($dsData[$lsdSpec["extern_id"]]) ? $dsData[$lsdSpec["extern_id"]] : $lsdSpec["extern_nama"];
                        $arrDiskonData[$lsdSpec["extern_id"]]["diskon_nilai"] = $lsdSpec["diskon_nilai"];
                        if (!isset($arrDiskonData[$lsdSpec["extern_id"]]["sub_diskon_nilai"])) {
                            $arrDiskonData[$lsdSpec["extern_id"]]["sub_diskon_nilai"] = 0;
                        }
                        $arrDiskonData[$lsdSpec["extern_id"]]["sub_diskon_nilai"] += $lsdSpec["nilai_unit"];
                    }
                }
            }
//            arrPrintHijau($arrDiskonData);
        ...
        arrPrintCyan($lItems);
        arrPrintHitam($lItems_99);
        if ((ipadd() == "202.65.117.72") && (sizeof($lItems_99) > 0)) {
            $lItems = $lItems + $lItems_99;
        }
...
//            showLast_query("pink");
            ...
            $itemNotApprove = isset($this->configUi[$jenisMasterTrans]['closedRequest'][1]['enabled']) ? $this->configUi[$jenisMasterTrans]['closedRequest'][1]['enabled'] : true;
            ...
            if (sizeof($tmpReg) > 0) {
                ...
                //----------------------------------
                cekHere("itemNotApprove: [$itemNotApprove]");
                if ($itemNotApprove == true) {
                    $m->setFilters(array());
//                    $dTmp = $m->lookUpDetailTransaksi($transaksiTopID);
                    $dTmp = $m->lookupDetailTransaksiNoJenis($transaksiTopID)->result();
                    showLast_query("kuning");
                    $notApprove = array();
        ...
        //-------------------------------------
        $produkUnitDisc = isset($this->configUi[$jenisMasterTrans]["produkUnitDisc"]) ? $this->configUi[$jenisMasterTrans]["produkUnitDisc"] : array();
    ...
    public function index()
    {
        ...
        $connectTo = isset($this->configUi[$jenisTr]['connectTo']) ? $this->configUi[$jenisTr]['connectTo'] : "";
        $undoneItemsIndexAll = isset($this->configUi[$jenisTr]['undoneItemsIndexAll']) ? $this->configUi[$jenisTr]['undoneItemsIndexAll'] : false;
        $steps = $this->configUi[$jenisTr]['steps'];
        //region link to add new transaction
        ...
        if ($undoneItemsIndexAll == true) {
//            cekHere("undoneItemsIndexAll [$undoneItemsIndexAll]");
            $link_undoneList = MODUL_PATH . "Transaksi/viewUndoneItemsIndex_all/$jenisTr/?gr=cGVtYmVsaWFu&ohyes=ohno&step=$getStep";
        }
        else {
//            cekHitam("undoneItemsIndexAll [$undoneItemsIndexAll]");
        }
...
\n\n
                    var viewhistoryList = function () {
                     ...
                     viewhistoryList()\n
                    //  var viewStokLimit = function () {
                    //     top.$('#stok_limit').html(\" < div class='row' style = 'margin-top: 14px; padding-bottom: 14px;' > <span class='col-lg-12 text-center' ><img width = '8%' src = '$baseUrl/assets/images/loading_16_p.gif' ><div class='text-center text-uppercase text-bold' > LOAD Stok Limit...</div ></span > </div > \");
                    //     setTimeout(function () {
                    //         if (top.$('#stok_limit').find('img').length > 0) {
                    //             top.$('#stok_limit').load('$link_stokLimit', function(){viewStokLimit})
                    //         }
                    //     },1000);
                    // }\n 
                    // viewStokLimit()
...
//arrPrintPink($_GET);
        ...
        $cCode = "_TR_" . $this->jenisTr;
        $cCodeOrig = "_TR_" . $origJenisTr;
//cekHijau(":: $origJenisTr :: $cCode :: $cCodeOrig ::");
        $historyFields = isset($this->configUi[$jenisTr]['shortHistoryFields']) ? $this->configUi[$jenisTr]['shortHistoryFields'] : array();
        $tabHistoryFields = isset($this->configUi[$jenisTr]['tabHistoryFields']) ? $this->configUi[$jenisTr]['tabHistoryFields'] : array();
        $tabFieldsItems = isset($this->configUi[$jenisTr]['tabFieldsItems']) ? $this->configUi[$jenisTr]['tabFieldsItems'] : array();
        $pairRegistries = isset($this->configUi[$jenisTr]['pairRegistries']) ? $this->configUi[$jenisTr]['pairRegistries'] : array();
        ...
        $needToClear = false;
        $allowMultiSelect = isset($this->configUi[$origJenisTr]['tabRequestCode']['allowMultiSelect']) ? $this->configUi[$origJenisTr]['tabRequestCode']['allowMultiSelect'] : false;
        $selectProduk = isset($this->configUi[$origJenisTr]['tabRequestCode']['selectProduk']) ? $this->configUi[$origJenisTr]['tabRequestCode']['selectProduk'] : false;
        ...
        if (isset($this->configUi[$this->uri->segment(4)]['tabRequestCode'])) {
            ...
            //            cekHijau(__LINE__ . "<br>" . $this->db->last_query());
            // supplier Id
            $mainGates = isset($_SESSION[$cCode]['main']) ? $_SESSION[$cCode]['main'] : array();
            $supplier_id = isset($mainGates['supplierID']) ? $mainGates['supplierID'] : "";
            $supplier_nama = isset($mainGates['supplierName']) ? $mainGates['supplierName'] : "";
            $str_supplier_nama = isset($mainGates['supplierName']) ? " ke vendor <b class='text-uppercase'>$supplier_nama</b>" : "";

            // $this->load->model("Mdls/MdlSuppliesPerSupplier");
            // $sps = new MdlSuppliesPerSupplier();
            // $prodPerSuppliers = array();
            // $prodPerSuppliers = $sps->callSpecs($supplier_id);
            // arrPrintPink($prodPerSuppliers);

            //------------------------------------------------------
            $arrJenis = array(
                "1466",// sudah diorder dan belum diapprove
                "1467",// sudah diorder dan belum diterima/GRN
                "h582so",// bahan baku total so
            );
            //------------------------------------------------------
            $this->load->model("Coms/ComTransaksiProduk");
            ...
            //------------------------------------------------------
            $ct = New ComTransaksiProduk();
            $ct->addFilter("qty_kredit>0");
            $ct->addFilter("rekening in ('" . implode("','", $arrJenis) . "')");
            $ctTmp = $ct->lookupAll()->result();
//             showLast_query("kuning");
            //------------------------------------------------------
            $sp = New MdlProduk();
            $spTmp = $sp->lookupAll()->result();
//             showLast_query("hitam");
            // arrPrintKuning($spTmp);
//            if (isset($mainGates['supplierID'])) {
//                $spTmp = sizeof($prodPerSuppliers > 0) ? $prodPerSuppliers : array();
//            }

            $spp = New MdlProdukPerSupplier();
            $sppTmp = $spp->lookupAll()->result();
            $arrProdukSupplier = array();
            if (sizeof($sppTmp) > 0) {
                foreach ($sppTmp as $sppSpec) {
//                    arrPrint($sppSpec);
                    $arrProdukSupplier[$sppSpec->produk_id] = $sppSpec->suppliers_id;
                }
            }
            $sup = New MdlSupplier();
            $supTmp = $sup->lookupAll()->result();
            $arrSupplier = array();
//            $arrSupplier[-100] = "Semua Data";
            $arrSupplier[0] = "Belum terelasi Supplier";
            if (sizeof($supTmp) > 0) {
                foreach ($supTmp as $supSpec) {
//                    arrPrint($sppSpec);
                    $arrSupplier[$supSpec->id] = $supSpec->nama;
                }
            }

            //------------------------------------------------------
            $rs = New ComRekeningPembantuProduk();
            $rs->addFilter("periode='forever'");
            $rs->addFilter("rekening='1010030030'");
            $rsTmp = $rs->lookupAll()->result();
//            showLast_query("biru");
            $stockSupplies = array();
            if (sizeof($rsTmp) > 0) {
                foreach ($rsTmp as $rsSpec) {
                    if (!isset($stockSupplies[$rsSpec->extern_id])) {
                        $stockSupplies[$rsSpec->extern_id] = 0;
                    }
                    $stockSupplies[$rsSpec->extern_id] += $rsSpec->qty_debet;
                }
            }
            //            arrPrintWebs($stockSupplies);
            //------------------------------------------------------
            // arrPrintKuning($ctTmp);
            $arrDataRequest = array();
            if (sizeof($ctTmp) > 0) {
                foreach ($ctTmp as $spec) {
                    $arrDataRequest[$spec->rekening][$spec->extern_id] = array(
                        "extern_id" => $spec->extern_id,
                        "extern_nama" => $spec->extern_nama,
                        //                        "qty_kredit" => $spec->qty_kredit,
                    );
                    if (!isset($arrDataRequest[$spec->rekening][$spec->extern_id]['qty_kredit'])) {
                        $arrDataRequest[$spec->rekening][$spec->extern_id]['qty_kredit'] = 0;
                    }
                    $arrDataRequest[$spec->rekening][$spec->extern_id]['qty_kredit'] += $spec->qty_kredit;
                }
            }
//            arrPrintPink($arrDataRequest);
            ...
            $_SESSION['undoneQty'][$this->jenisTr] = (isset($tmpByReq) && sizeof($tmpByReq) > 0) ? sizeof($tmpByReq) : "";


            if (sizeof($spTmp) > 0) {
                $arrIds = array();
                $reqCtr = 0;
                foreach ($spTmp as $rows) {
                    $arrIds[] = $rows->id;
                    $reqCtr++;
                    foreach ($tabHistoryFields as $key => $row) {
                        switch ($key) {
                            case "produk_id":
                                ...
                                $tmpTabData[$key][$rows->id] = $produk_id;
                                break;
                            case "supplier_id":
                                $inputType = $allowMultiSelect == true ? "checkbox" : "radio";
                                $clickEvent = "";
...
//                                cekHere($rows->id . " :: $supplier_id ::");
            ...
            cekMerah(":: tidak ada tabRequestCode ::");
        }

        //endregion

        // arrPrintWebs($tabFieldsItems);
        // arrPrintWebs($arrayOnprogress2);
        // cekBiru("DAFTAR REQUEST PEMBELIAN BAHAN BAKU");
        // arrPrintPink($this->uri->segment_array());
        ...
        $data = array(
            ...
            "supplier_nama" => $str_supplier_nama,
            "supplier_id" => isset($_GET["supplier_id"]) ? $_GET["supplier_id"] : $supplier_id,
            "supplier_data" => $arrSupplier,
            "supplier_produk" => $arrProdukSupplier,
            "requestSupplier" => $requestSupplier,
            "selectProduk" => $selectProduk,
        );
    ...
    public function getKebutuhanProject()
    {

        $lp = array();
        $this->db->select("id,satuan_nilai");
        $this->db->where("jenis='item'");
        $tmpP = $this->db->get("produk")->result();

        if (!empty($tmpP)) {
            foreach ($tmpP as $k => $pRow) {
                $lp[$pRow->id] = $pRow->satuan_nilai;
            }
        }

        $this->db->select("produk_dasar_id,produk_dasar_nama,qty_debet,qty_kredit,qty_saldo");
        $this->db->where("jenis='produk'");
        $p = $this->db->get("project_komposisi_sub_workoder")->result();

        $kebutuhan = array();
        if (!empty($p)) {
            foreach ($p as $k => $row) {
//                if(!isset($kebutuhan[$row->produk_dasar_id]['qty_debet'])){
//                    $kebutuhan[$row->produk_dasar_id]['qty_debet'] = 0;
//                }
//                if(!isset($kebutuhan[$row->produk_dasar_id]['qty_kredit'])){
//                    $kebutuhan[$row->produk_dasar_id]['qty_kredit'] = 0;
//                }
//                if(!isset($kebutuhan[$row->produk_dasar_id]['qty_saldo'])){
//                    $kebutuhan[$row->produk_dasar_id]['qty_saldo'] = 0;
//                }
                if (!isset($kebutuhan[$row->produk_dasar_id]['qty'])) {
                    $kebutuhan[$row->produk_dasar_id]['qty'] = 0;
                }
//                $kebutuhan[$row->produk_dasar_id]['qty_debet'] += $row->qty_debet;
//                $kebutuhan[$row->produk_dasar_id]['qty_kredit'] += $row->qty_kredit;
//                $kebutuhan[$row->produk_dasar_id]['qty_saldo'] += $row->qty_saldo;
                $kebutuhan[$row->produk_dasar_id]['produk_id'] = $row->produk_dasar_id;
                $kebutuhan[$row->produk_dasar_id]['produk_nama'] = $row->produk_dasar_nama;
                $kebutuhan[$row->produk_dasar_id]['satuan_nilai'] = $lp[$row->produk_dasar_id];
                $kebutuhan[$row->produk_dasar_id]['qty'] += $row->qty_saldo;

                if (isset($lp[$row->produk_dasar_id]) && $lp[$row->produk_dasar_id] * 1 > 0) {
                    $kebutuhan[$row->produk_dasar_id]['satuan_total'] = $kebutuhan[$row->produk_dasar_id]['qty'] * $kebutuhan[$row->produk_dasar_id]['satuan_nilai'];
                }
                else {
                    $kebutuhan[$row->produk_dasar_id]['satuan_total'] = 0;
                }
            }
        }

        $this->db->truncate("project_kebutuhan_produk");
        $this->db->insert_batch("project_kebutuhan_produk", $kebutuhan);

        arrPrint($kebutuhan);

    }

    function bi_pembelian_project()
    {
        $this->getKebutuhanProject();
    }
