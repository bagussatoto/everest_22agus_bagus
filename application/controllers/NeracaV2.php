...
class NeracaV2 extends CI_Controller
{
    protected $koloms;
    ...
    public function viewBalanceSheet_OLD()
    {
            ...
            foreach ($tmp as $row) {
                if (sizeof($accountAkumulasiPenyusutan) > 0 && !in_array($row['rekening'], $accountAkumulasiPenyusutan)) {
                    ...
                    if (isset($accountChilds[$row['rekening']])) {
                        ...
                        $link = base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row['rekening']] . "/" . $row['rekening'];
...
//            "items" => $rekenings,
            ...
            "inspectTarget_rincian" => base_url() . "LedgerV2/viewBalances_l1/",
    ...
    public function viewBalanceSheet()
    {
        ...
        $accountChilds = $this->config->item("accountChilds") != null ? $this->config->item("accountChilds") : array();
        //        $accountAlias = $this->config->item("accountAlias") != null ? $this->config->item("accountAlias") : array();
        $accountAlias = $as->lookUpTransactionStructureLabel();
        //        $blockRekenings = $this->config->item("accountStructure");
        $blockRekenings = $as->lookUpTransactionStructure();
        // arrPrintPink($blockRekenings);
        // arrPrintPink($accountAlias);
        $accountAkumulasiPenyusutan = $this->config->item("accountAkumulasiPenyusutan") != null ? $this->config->item("accountAkumulasiPenyusutan") : array();
        $accountCatException = $this->config->item("accountCatOppositeExceptions") != null ? $this->config->item("accountCatOppositeExceptions") : array();
        $accountRekeningSort = $this->config->item("accountRekeningSort") != null ? $this->config->item("accountRekeningSort") : array();
...
//        arrPrint($tmp);
        ...
        /** -------------------------------------------------------------------------------
         * penulisan rekening dalam huruf kecil semua
         * */
        $arrRekeningDetils = [
            "hutang usaha" => [
                // "title" => "detil",
                "icon" => "",
                "link" => "laporan/Hutang/viewhr/cekSumMutasi/",
            ],
            // "hutang biaya" => [
            //     "link" => "laporan/Hutang/viewhr/cekSumMutasi/",
            // ],
        ...
        if (sizeof($tmp) > 0) {
            foreach ($tmp as $row) {
                if (sizeof($accountAkumulasiPenyusutan) > 0 && !in_array($row['rekening'], $accountAkumulasiPenyusutan)) {
                    ...
                    $rekening_key = strtolower($rekening_name);
                    if (array_key_exists($rekening_key, $arrRekeningDetils)) {
                        $link_detil = $arrRekeningDetils[$rekening_key]["link"];
                        $detil_title = isset($arrRekeningDetils[$rekening_key]["title"]) ? $arrRekeningDetils[$rekening_key]["title"] : "detil $rekening_name";
...
//        arrPrintWebs($struktureRekening);
        //        arrPrint($akumulasiPenyusutan);
        ...
        $cpSrc = $cp->callDatas();
        $neracaStatus = isset($cpSrc->neraca_ok) ? $cpSrc->neraca_ok : "0";
    ...
    public function viewBalanceSheetBulanan()
    {
        ...
        if (sizeof($tmp) > 0) {
            foreach ($tmp as $row) {
                ...
                if (isset($accountChilds[$row['rekening']])) {
                    // $tmpCol['link'] .= "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row['rekening']] . "/" . $row['rekening'] . "'><span class='fa fa-clone'></span></a>";
                ...
                foreach ($tmp as $row) {
                    ...
                    if (strlen($row->kategori) > 1) {
                        if ((round($row->debet, 2) > 0) || (round($row->kredit, 2) > 0)) {
                            ...
                            if (isset($accountChilds[$row->rekening])) {
                                $tmpCol['link'] .= "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "/" . $row->periode . "?date=$oldDate'><span class='fa fa-clone'></span></a>";
                            }
//                        $tmpCol['link'] .= "<span class='pull-right'><a href='" . base_url() . "Ledger/viewMoves_l1/Rekening/" . $row->rekening . "'><span class='glyphicon glyphicon-time'></span></a></span>";
            ...
            if ($_GET['tm'] == 1) {
                ...
                if ($defaultDate >= "2022-11") {
                    $accountRekeningSort = rekening_coa_sort_he_accounting();
                }
                else {
//                arrPrintPink($accountRekeningSort);
                    $accountRekeningSort_OLD = $accountRekeningSort;
                    $accountRekeningSort = array();
                    foreach ($accountRekeningSort_OLD as $cat => $catSpec) {
                        foreach ($catSpec as $key => $val) {
                            if (isset($rekeningCoa[$val])) {
                                $val_new = $rekeningCoa[$val];
                                $accountRekeningSort[$cat][$key] = $val_new;
                            }
                            else {
                                $accountRekeningSort[$cat][$key] = $val;
                            }
                        }
                    }
                }
                ...
                if (sizeof($tmp) > 0) {
                    foreach ($tmp as $row) {
                        ...
                        if (!is_numeric($row->rekening)) {
                            $row->rekening = isset($rekeningCoa[$row->rekening]) ? $rekeningCoa[$row->rekening] : $row->rekening;
                        }
...
//        arrPrint($rekenings);
        ...
        $data = array(
            "mode" => $this->uri->segment(2),
            "title" => "balance (final)",
            "subTitle" => "balance (final) " . my_cabang_nama() . " per-" . lgTranslateTime($defaultDate),
            ...
            foreach ($tmp as $row) {
                ...
                if (strlen($row->kategori) > 1) {
                    if ((round($row->debet, 2) > 0) || (round($row->kredit, 2) > 0)) {
                        ...
                        if (isset($accountChilds[$row->rekening])) {
                            $tmpCol['link'] .= "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "/" . $row->periode . "'><span class='fa fa-clone'></span></a>";
                        }
                        $tmpCol['link'] .= "<span class='pull-right'><a href='" . base_url() . "Ledger/viewMoves_l1/Rekening/" . $row->rekening . "'><span class='glyphicon glyphicon-time'></span></a></span>";
                    ...
                    foreach ($rowSpec as $row) {
                        ...
                        if (strlen($row->kategori) > 1) {
                            if ((round($row->debet, 2) > 0) || (round($row->kredit, 2) > 0)) {
                                ...
                                if ($row->cabang_id != 0) {
                                    if (isset($accountChilds[$row->rekening])) {
//                                        $childLink = "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "?o=" . $row->cabang_id . "&w=" . $whID['gudang_id'] . "'>
//                                        <span class='fa fa-clone'></span></a>";
                                        $childLink = "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "?o=" . $row->cabang_id . "&bln=$bulan&thn=$tahun&periode=" . $periode . "'>
                                        <span class='fa fa-clone'></span></a>";
                                    }
...
//                                        </span>";
                                    $childLink2 = "$childLink <span class='pull-right'>
                                        <a href='" . base_url() . "Ledger/viewMoveDetails/Rekening/" . $row->rekening . "?o=" . $row->cabang_id . "&periode=" . $periode . "'>
                                        <span class='glyphicon glyphicon-time'></span></a>
                                        </span>";
                                }
                                else {
...
//        arrPrint($accountRekeningSort);
//        arrPrint($rekeningsNameNew);
//        cekHere("views_mode: $views_mode, views: $views");
        $data = array(
            "mode" => $views_mode,
            "title" => "Laporan Neraca Konsolidasi (internal) $periode ",
            "subTitle" => "Laporan Neraca Konsolidasi (internal) per-" . lgTranslateTime($defaultDate),
            "categories" => $arrCatView,
            "rekenings" => $rekenings,
            "headers" => $headerss,
            "defaultDate" => $defaultDate,
...
//                                        <span class='glyphicon glyphicon-time'></span></a></span>";
                                $childLink2 = "$childLink <span class='pull-right'><a href='" . base_url() . "Ledger/viewMoveDetails/Rekening/" . $row->rekening . "?o=" . $row->cabang_id . "&periode=" . $periode . "'>
                                        <span class='glyphicon glyphicon-time'></span></a></span>";
        ...
        foreach ($arrCatView as $cat) {
            foreach ($accountRekeningSort[$cat] as $rekName) {
                $rekName = isset($rekeningCoa[$rekName]) ? $rekeningCoa[$rekName] : $rekName;
                if (isset($rekeningsName[$cat])) {
                    if (in_array($rekName, $rekeningsName[$cat])) {
                        $rekeningsNameNew[$cat][$rekName] = $rekName;
                    }
...
//        arrPrint($rekeningsNameNew);
        ...
        $data = array(
            "mode" => $views_mode,
            "title" => "Laporan Neraca Konsolidasi Tahunan (internal) ",
            //            "subTitle" => "balance per-" . lgTranslateTime($defaultDate),
            "subTitle" => "Laporan Neraca Konsolidasi (internal) per- $defaultDate",
            "categories" => $arrCatView,
            "rekenings" => $rekenings,
            "headers" => $headerss,
            "defaultDate" => $defaultDate,
                ...
                foreach ($nerSpec as $rowSpec) {
                    foreach ($rowSpec as $row) {
                        ...
                        if (strlen($row->kategori) > 1) {
                            ...
                            //                            $childLink = "";
                            //                            if (isset($accountChilds[$row->rekening])) {
                            //                                $childLink = "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "'><span class='fa fa-clone'></span></a>";
                            //                            }
                            ...
                            $childLink = "";
                            if (isset($accountChilds[$row->rekening])) {
                                $childLink = "<a href='" . base_url() . "LedgerV2/viewBalances_l1/" . $accountChilds[$row->rekening] . "/" . $row->rekening . "?o=" . $row->cabang_id . "&d=" . $row->div_id . "'><span class='fa fa-clone'></span></a>";
                            }
...
//arrPrint($rekeningsName);
        ...
        $data = array(
            "mode" => "viewNeraca",
            "title" => "balance YTD",
//            "subTitle" => "balance YTD " . my_cabang_nama() . " " . lgTranslateTime(date("Y")),
            "subTitle" => "balance YTD " . my_cabang_nama() . " per-" . formatTanggal(date("Y-m-d"), 'd F Y'),
            "categories" => $arrCatView,
            "rekenings" => $rekeningsNew,
            "headers" => array(
                //                "rek_id" => "code",
...
//                showlast_query("biru");
            ...
            //--------------
            $rekening_selected = $this->uri->segment(3);
            //--------------
            $accountChilds = $this->config->item("accountChilds") != null ? $this->config->item("accountChilds") : array();
            $accountException = $this->config->item("accountRekOppositeExceptions") != null ? $this->config->item("accountRekOppositeExceptions") : array();
            $accountAlias = $this->config->item("accountAlias") != null ? $this->config->item("accountAlias") : array();
...
//                                }
                                $childLink2 = "$childLink <span class='pull-right'>
                                    <a href='" . base_url() . "Ledger/viewMoveDetails_1/Rekening/" . $row->rekening . "?o=" . $row->cabang_id . "&w=" . $whID['gudang_id'] . "'
                                        target='_blank'>
                                        <span class='glyphicon glyphicon-time'></span></a></span>";
                                $childLink3 = base_url() . "Ledger/viewMoveDetails_1/Rekening/" . $row->rekening . "?o=" . $row->cabang_id . "&w=" . $whID['gudang_id'];

                                $rekenings[$row->cabang_id][$row->kategori][$row->rekening]['link'] = $childLink2;
                                $rekenings[$row->cabang_id][$row->kategori][$row->rekening]['link_3'] = $childLink3;
...
//arrPrintPink($resultNeracaByCabang);
//mati_disini();
        $arrCat = array("aktiva", "hutang", "modal", "lain-lain-kr");
        $arrCatView = array("aktiva", "hutang", "modal");
...
//arrPrint($rekeningsNameNew);
        ...
        $data = array(
            ...
            "title" => "Neraca Konsolidasi YTD (internal)",
//            "subTitle" => "Neraca Konsolidasi (internal) per-" . lgTranslateTime($defaultDate),
            "subTitle" => "Neraca Konsolidasi YTD (internal) per-" . formatTanggal(date("Y-m-d"), 'd F Y'),
            "categories" => $arrCatView,
            "rekenings" => $rekenings,
            "headers" => array(
                //                "rekening" => "rekening",
...
//            "accountConsolidation" => $accountConsolidation,
            ...
            "underMaintenance" => underMaintenance(),
            //-----------
            "rekening_selected" => $rekening_selected,
            "rekening_selected_style" => array(
                "style" => "font-weight:bold;font-size:15px;background-color:yellow;",
            ),
            //-----------
        );
        $this->load->view("finance", $data);
...
//        $rekeningAlias = $this->config->item("accountAlias") != NULL ? $this->config->item("accountAlias") : array();
        $rekeningAlias = fetchAccountStructureAlias();
...
//                            $rekeningNama = $addSpec->rekening == "persediaan supplies proses" ? "persediaan supplies" : $addSpec->rekening;
                            $rekeningNama = $addSpec->rekening == "1010030050" ? "1010030010" : $addSpec->rekening;
...
//                    }
                    if (isset($items['1010030010'])) {
                        $anggaran = isset($items['1010030010']['kredit']) ? $items['1010030010']['kredit'] : 0;
                        $items['1010030010']['debet'] = $anggaran;
                    }
...
//                $itemsNew_show[$key] = isset($items[$key]) ? $items[$key] : array();
                if (isset($items[$key])) {
                    $itemsNew_show[$key] = $items[$key];
                }
            }
        }
//arrPrint($itemsNew_show);
        $btnGroups = array();
        $data = array(
            "mode" => "efisiensi",
            "title" => "$itemLabel &nbsp; $dateRange",
...
//        );
    ...
    public function viewCashflowTahunan_()
    {
        /* pembatalan yang melibatkan kas tetap dimunculkan
//        karena tidak bisa dinettokan, karena sifat pembatalan umum
        // salah prosedural sop pemakaian
...
//cekHere("default date: $defaultDate");
        ...
        $data = array(
            "mode" => "$views_mode",
            "title" => "Laporan Cashflow Konsolidasi",
            "subTitle" => "Laporan Cashflow Konsolidasi " . ($year),//$defaultDate
            "categories" => $topHeader,
            "rekenings" => $midHeader,
            "headers" => array(
                "values" => "-",
...
//        );
    ...
    public function detailCashflow_()
    {
