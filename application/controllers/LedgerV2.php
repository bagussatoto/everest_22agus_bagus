...
class LedgerV2 extends CI_Controller
{
    ...
    public function viewBalances_l1_header($relName, $rekName, $cabangID = NULL)
    {
        ...
        if ($this->isGudangDynamicColumnsEnabled($relName)) {
            $gudangColumns = $this->getGudangDynamicColumns($cabangID);
            $headerFields = $this->reorderProdukGudangHeader($headerFields, $gudangColumns);
        }
    ...
    private function isGudangDynamicColumnsEnabled($relName)
    {
        return $relName === "RekeningPembantuProduk";
    }

    private function getGudangDynamicColumns($cabangID = NULL)
    {
        $results = array();
        $supplierTipeIDs = array();

        $mdlPath = APPPATH . "models/Mdls/MdlGudang.php";
        if (!file_exists($mdlPath)) {
            return $results;
        }

        $this->load->model("Mdls/MdlGudang");
        if (!class_exists("MdlGudang", false)) {
            return $results;
        }

        $mdlPathTipe = APPPATH . "models/Mdls/MdlGudangTipe.php";
        if (file_exists($mdlPathTipe)) {
            $this->load->model("Mdls/MdlGudangTipe");
            if (class_exists("MdlGudangTipe", false)) {
                $gt = new MdlGudangTipe();
                $gt->setFilters(array());
                $gt->addFilter("status='1'");
                $gt->addFilter("trash='0'");
                $tipeRows = $gt->lookupAll()->result();
                if (sizeof($tipeRows) > 0) {
                    foreach ($tipeRows as $tipeRow) {
                        $tipeID = isset($tipeRow->id) ? (int)$tipeRow->id : 0;
                        $tipeNama = isset($tipeRow->nama) ? strtolower(trim((string)$tipeRow->nama)) : "";
                        if ($tipeID > 0 && strpos($tipeNama, "supplier") !== false) {
                            $supplierTipeIDs[$tipeID] = $tipeID;
                        }
                    }
                }
            }
        ...
        $mg = new MdlGudang();
        $mg->setFilters(array());
        $mg->addFilter("status='1'");
        $mg->addFilter("trash='0'");
        if ($cabangID !== NULL && $cabangID !== "") {
            $cabangID = (int)$cabangID;
            $mg->addFilter("cabang_id='$cabangID'");
        }
        $mg->setSortBy(array(
            "kolom" => "id",
            "mode" => "asc",
        ));
        $tmp = $mg->lookupAll()->result();
        ...
        if (sizeof($tmp) > 0) {
            foreach ($tmp as $row) {
                ...
                $tipeGudangID = isset($row->tipe_gudang_id) ? (int)$row->tipe_gudang_id : 0;
                $supplierID = isset($row->supplier_id) ? (int)$row->supplier_id : 0;
                $isSupplierGudang = false;
                if ($tipeGudangID > 0 && isset($supplierTipeIDs[$tipeGudangID])) {
                    $isSupplierGudang = true;
                }
                elseif ($supplierID > 0) {
                    $isSupplierGudang = true;
                }
                ...
                $label = isset($row->nama) ? trim((string)$row->nama) : "";
                if ($label == "") {
                    $label = "Gudang " . $gid;
                }
    ...
    private function reorderProdukGudangHeader($headerFields, $gudangColumns)
    {
        $ordered = array();
        $added = array();
        ...
        foreach ($prefixOrder as $k) {
            if (isset($headerFields[$k])) {
                $ordered[$k] = $headerFields[$k];
                $added[$k] = 1;
            }
        }

        if (sizeof($gudangColumns) > 0) {
            foreach ($gudangColumns as $gudangSpec) {
                $qtyKey = $gudangSpec['qty_key'];
                $rpKey = $gudangSpec['rp_key'];
                $ordered[$qtyKey] = $gudangSpec['label'] . "<br>(QTY)";
                $ordered[$rpKey] = $gudangSpec['label'] . "<br>(IDR)";
                $added[$qtyKey] = 1;
                $added[$rpKey] = 1;
            }
        }

        $suffixTotal = array("total_qty_debet", "total_debet");
        foreach ($suffixTotal as $k) {
            if (isset($headerFields[$k])) {
                $ordered[$k] = $headerFields[$k];
                $added[$k] = 1;
            }
    ...
    public function viewBalances_l1()
    {
        ...
        $headerFields = $this->viewBalances_l1_header($relName, $rekName, $cabangID);
//arrPrint($headerFields);
    ...
    public function viewBalances_l1_fetch()
    {
        ...
        $orderColIdx = (isset($dtPost['order'][0]['column'])) ? (int)$dtPost['order'][0]['column'] : -1;
        $orderDir = (isset($dtPost['order'][0]['dir']) && strtolower($dtPost['order'][0]['dir']) === 'desc') ? 'desc' : 'asc';
        $orderField = null;
        if ($orderColIdx > 0 && isset($columns[$orderColIdx - 1])) {
            $orderField = $columns[$orderColIdx - 1];
        }

        $isGudangDynamicOrder = false;
        if ($orderField !== null) {
            $isGudangDynamicOrder = (strpos($orderField, 'gd_qty_') === 0 || strpos($orderField, 'gd_debet_') === 0);
        }
        $isComputedOrderField = in_array($orderField, array('ng_qty_debet', 'ng_debet', 'total_qty_debet', 'total_debet'), true);
        $needsPostSort = ($isGudangDynamicOrder || $isComputedOrderField);

        $dtPostFetch = $dtPost;
        if ($needsPostSort) {
            $dtPostFetch['start'] = 0;
            $dtPostFetch['length'] = -1;
            if (isset($dtPostFetch['order'])) {
                unset($dtPostFetch['order']);
            }
        }

        $res = $com->datatable($cfg, $dtPostFetch, $opts, $columns);
        $rows = $res['data'];
...
//        cekMerah(json_encode($rows));
        ...
        $dynamicGudangColumns = array();
        if ($this->isGudangDynamicColumnsEnabled($relName)) {
            $dynamicGudangColumns = $this->getGudangDynamicColumns($cabangID);
        }

        if (!empty($rows) && sizeof($dynamicGudangColumns) > 0) {
            $rowIndexesByPid = array();

            foreach ($rows as $ixRow => &$rowData) {
                $pid = isset($rowData['pId']) ? (int)$rowData['pId'] : 0;
                if ($pid > 0) {
                    if (!isset($rowIndexesByPid[$pid])) {
                        $rowIndexesByPid[$pid] = array();
                    }
                    $rowIndexesByPid[$pid][] = $ixRow;
                }

                foreach ($dynamicGudangColumns as $gudangSpec) {
                    $rowData[$gudangSpec['qty_key']] = 0;
                    $rowData[$gudangSpec['rp_key']] = 0;
                }
            }
            unset($rowData);

            if (sizeof($rowIndexesByPid) > 0) {
                $pids = array_keys($rowIndexesByPid);
                ...
                $this->db->select(array(
                    "extern_id",
                    "gudang_id",
                    "SUM(qty_debet) AS qty_debet",
                    "SUM(debet) AS debet",
                ), false);
                $this->db->group_by("extern_id, gudang_id");

                if (method_exists($com, "fetchBalancesCustom")) {
                    $tmpGudangRows = $com->fetchBalancesCustom($rekName);
                }
                else {
                    $tmpGudangRows = $com->fetchBalances($rekName);
                }

                if (sizeof($tmpGudangRows) > 0) {
                    foreach ($tmpGudangRows as $gudangRow) {
                        $pid = isset($gudangRow->extern_id) ? (int)$gudangRow->extern_id : 0;
                        $gid = isset($gudangRow->gudang_id) ? (int)$gudangRow->gudang_id : 0;
                        if ($pid <= 0 || $gid <= 0) {
                            continue;
                        }
                        if (!isset($rowIndexesByPid[$pid]) || !isset($dynamicGudangColumns[$gid])) {
                            continue;
                        }

                        $qtyKey = $dynamicGudangColumns[$gid]['qty_key'];
                        $rpKey = $dynamicGudangColumns[$gid]['rp_key'];
                        $qtyVal = isset($gudangRow->qty_debet) ? (float)$gudangRow->qty_debet : 0;
                        $rpVal = isset($gudangRow->debet) ? (float)$gudangRow->debet : 0;

                        foreach ($rowIndexesByPid[$pid] as $rowIx) {
                            $rows[$rowIx][$qtyKey] = $qtyVal;
                            $rows[$rowIx][$rpKey] = $rpVal;
                        }
                    }
                }
            }
        ...
        if (!empty($rows) && sizeof($dynamicGudangColumns) > 0) {
            foreach ($rows as &$rowData) {
                $supplierQtyAll = 0;
                $supplierRpAll = 0;
                $supplierQtyProjectBucket = 0;
                $supplierRpProjectBucket = 0;

                foreach ($dynamicGudangColumns as $gudangSpec) {
                    $gid = isset($gudangSpec['id']) ? (int)$gudangSpec['id'] : 0;
                    $qtyVal = isset($rowData[$gudangSpec['qty_key']]) ? (float)$rowData[$gudangSpec['qty_key']] : 0;
                    $rpVal = isset($rowData[$gudangSpec['rp_key']]) ? (float)$rowData[$gudangSpec['rp_key']] : 0;

                    $supplierQtyAll += $qtyVal;
                    $supplierRpAll += $rpVal;

                    // histori lama menaruh gudang_id > 0 && < 1000 ke bucket project
                    if ($gid > 0 && $gid < 1000) {
                        $supplierQtyProjectBucket += $qtyVal;
                        $supplierRpProjectBucket += $rpVal;
                    }
                }

                $regQty = isset($rowData['qty_debet']) ? (float)$rowData['qty_debet'] : 0;
                $regRp = isset($rowData['debet']) ? (float)$rowData['debet'] : 0;

                $projectQtyRaw = isset($rowData['ng_qty_debet']) ? (float)$rowData['ng_qty_debet'] : 0;
                $projectRpRaw = isset($rowData['ng_debet']) ? (float)$rowData['ng_debet'] : 0;

                $projectQtyPure = $projectQtyRaw - $supplierQtyProjectBucket;
                $projectRpPure = $projectRpRaw - $supplierRpProjectBucket;

                // lindungi floating point noise jadi -0.0000x
                if ($projectQtyPure < 0 && abs($projectQtyPure) < 0.00001) {
                    $projectQtyPure = 0;
                }
                if ($projectRpPure < 0 && abs($projectRpPure) < 0.00001) {
                    $projectRpPure = 0;
                }

                if (isset($rowData['ng_qty_debet'])) {
                    $rowData['ng_qty_debet'] = $projectQtyPure;
                }
                if (isset($rowData['ng_debet'])) {
                    $rowData['ng_debet'] = $projectRpPure;
                }

                if (isset($rowData['total_qty_debet'])) {
                    $rowData['total_qty_debet'] = $regQty + $projectQtyPure + $supplierQtyAll;
                }

                if (isset($rowData['total_debet'])) {
                    $rowData['total_debet'] = $regRp + $projectRpPure + $supplierRpAll;
                }
            }
            unset($rowData);
        }

        if ($needsPostSort && !empty($rows) && $orderField !== null) {
            usort($rows, function ($a, $b) use ($orderField, $orderDir) {
                $va = isset($a[$orderField]) ? $a[$orderField] : 0;
                $vb = isset($b[$orderField]) ? $b[$orderField] : 0;

                $na = is_numeric($va) ? (float)$va : (float)str_replace(array(",", " "), array("", ""), (string)$va);
                $nb = is_numeric($vb) ? (float)$vb : (float)str_replace(array(",", " "), array("", ""), (string)$vb);

                if ($na == $nb) {
                    $pa = isset($a['pId']) ? (int)$a['pId'] : 0;
                    $pb = isset($b['pId']) ? (int)$b['pId'] : 0;
                    if ($pa == $pb) {
                        return 0;
                    }
                    return ($pa < $pb) ? -1 : 1;
        ...
        if (!empty($rows)) {
            ...
            if (isset($cfg['pairedSerial']) && $cfg['pairedSerial'] == true) {
                $opts['supplier_gudang_ids'] = array_keys($dynamicGudangColumns);
