...
class SetupDepresiasiTmp extends CI_Controller
{
...
//        $ctrlName = "Mdl" . $this->uri->segment(1) . $this->uri->segment(3);
        $ctrlName = "MdlsetupDepresiasiAssets";
...
//            $arrdtmp=array();
//            foreach($tmpDep as $dTmp){
//                $arrdtmp[$dTmp->extern_id] = $dTmp->extern_id;
//            }
//
//            $this->load->model("MdlTransaksi");
//            $tr = new MdlTransaksi();
//
//            $tr->addFilter("transaksi.jenis=$jnSlctd");
//            $tr->addFilter("transaksi_data.produk_id in ('". implode("','", $arrdtmp) . "')");
//            $tmpHistT = $tr->lookupRecentUndoneEntries_joined($sesionReplacer)->result();

//arrPrint($arrdtmp);
//            $tmpHist=array();
//            foreach( $tmpHistT as $k=>$datas ){
//                $tmpHist[$datas->produk_id][] = $datas;
//            }

//            arrPrint($tmpHist);
//            die();

            $totalListDepresiasi = 0;
            foreach ($tmpDep as $Data) {
...
//                arrPrint($tmpHist);
//                if(sizeof($tmpHist[$Data->extern_id])>0){
//                    foreach($tmpHist[$Data->extern_id] as $k=>$datas){
//                        if(!isset($arrListDepre[$Data->extern_id][$k])){
//                            $arrListDepre[$Data->extern_id][$k] = array();
//                        }
//                        $arrListDepre[$Data->extern_id][$k] = $datas;
//                    }
//                }
...
//                arrPrint($Data->extern_id . " | " . $Data->kode);
//                arrPrintWebs($arrListDepre);
//                arrPrint($arrListDepre[$Data->extern_id]);
//                arrPrintWebs($arrListDepre[$Data->extern_id]);
//                arrPrintWebs(count($arrListDepre[$Data->extern_id]) . "  ||  " . $Data->used);
//                arrPrintWebs($tmpHist[$Data->extern_id]);

                $countDepre = count($arrListDepre);
                $totalListDepresiasi += $countDepre;

//                arrPrintWebs($Data->kode. "  ||  " . $Data->extern_id . " || " . $Data->used . " || " . $countDepre);
//                arrPrint( (int)$Data->used . " || " . $Data->extern_id . " || " . $countDepre);
...
//                        $existData[$Data->extern_id][$alias] = array();
                    ...
                    elseif ($alias == 'repeat') {
                        ...
                        if ($countDepre > 0) {
                            $addStr = "<div class='text-bold text-center'>tgl " . $Data->$alias . " tiap bulan</div> <a href='$isExist_href' onclick=\"$addClick\" style='border: 1px #39dd3f solid; color: #39dd3f;' class='btn text-bold btn-xs btn-block btn-default hidden-print' > " . $countDepre . "x depresiasi </a>";
                        }
                        else {
                    ...
                    elseif ($alias == 'economic_life_time') {
                        ...
                        $existData[$Data->extern_id][$alias] = isset($Data->$alias) ? "<div class='text-bold text-center'>$countDepre/" . $Data->$alias . " bulan</div><div style='border: 1px solid lightgray;border-radius: 10px;margin-top: 0px;' class='progress $active'> <div class='progress-bar $colorClass text-bold' role='progressbar' aria-valuenow='$prc' aria-valuemin='0' aria-valuemax='100' style='width: " . ($prc > 20 ? $prc : $prcx) . "%'> $prc% $state </div> </div>" : 0;
                    }
                    else {
                        $existData[$Data->extern_id][$alias] = isset($Data->$alias) ? $Data->$alias : "";
                    }
...
//             matiHere("hoop ----DONE---- belom commit");
    ...
    public function viewDepresiasi()
    {
        ...
        if (sizeof($prdSetup[$id]) > 0 && $prdSetup[$id]->used > 0) {
            ...
            $arrTmp1[] = array(
                "dtime" => "---",
                "nomer" => "adjustment " . $prdSetup[$id]->used . "bln",
                "oleh_nama" => "system",
