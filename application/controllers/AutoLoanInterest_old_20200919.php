...
class AutoLoanInterest_old_20200919 extends  CI_Controller
{
...
//            cekOrange('$sourceItems');
//            arrPrint($sourceItems);
//            cekMerah('$paymentSource');
//            arrPrint($paymentSource);
//            arrPrintWebs($sourceItems);
//            arrPrintWebs($paymentSource);
//            cekHere('$sourceItems');
//            arrPrint($sourceItems);
            //region build dulu berdasarkan jenis depresiasi
            $finalData = array();
            foreach($sourceItems as $nomer=>$detilExtern){
//                foreach($detilExternTmp as $tr_id=>$detilExtern){
                if( isset($paymentSource[$nomer]) ){
                    if( !isset($finalData[$nomer]) ){
                        $finalData[$nomer] = array();
                    }
//                        arrPrint($paymentSource[$nomer]);
                    $finalData[$nomer] = $paymentSource[$nomer] + $detilExtern;
                }
//                }
            }
...
//            arrPrint($finalData);
            ...
            foreach ($finalData as $nomer => $detilSourceItems){

                $desc = isset($_GET['desc']) ? htmlspecialchars(trim(nl2br(base64_decode($_GET['desc'])))) : "";
                $descs = str_ireplace($breaks, "\r\n", $desc);

                $extern_id = $detilSourceItems['extern_id'];
                $extern_nama = $detilSourceItems['extern_nama'];
...
//                $noP = "";
//                $totalSisaText = "";
//                $total_nilai_bunga_text = "";
//                $totalPersenBungaText = "";
//                $total_nilai_pph23_text = "";
//                $grandtotalText = "";
                $arrItems = array();
                $total_nilai_bunga=0;
                $total_nilai_pph23=0;
...
//                foreach($detilSourceItemsTmp as $nomer => $detilSourceItems){
//                    arrPrint($detilSourceItems['id']);

                cekHitam($nomer . " ||| nomer");

//                    if(!isset($arrItems[$nomer])){
//                        $arrItems[$nomer] = array();
//                    }

//                    cekHitam($numb . "    =====    " . count($detilSourceItemsTmp));

                $pid = $detilSourceItems['id'];
                $tr_id = $detilSourceItems['transaksi_id'];
                $dateNow = date('d');
                ...
                $valid_bunga = ($paymentSource[$nomer]['sisa']/12);
...
//                    $total_nilai_pph23_text .= $numb==count($detilSourceItemsTmp) ? "<div class='text-right'><u>". number_format($nilai_pph23) ."</u></div>" : "<div class='text-right'>". number_format($nilai_pph23) ."</div>";
//                    $total_nilai_bunga_text .= $numb==count($detilSourceItemsTmp) ? "<div class='text-right'><u>". number_format($nilai_bunga) ."</u></div>" : "<div class='text-right'>". number_format($nilai_bunga) ."</div>";

                if (isset($paymentSource[$nomer])&&$paymentSource[$nomer]['sisa']>1&&($dateNow-$tglDepreItems)==0 && $dateNowLeng != date('Y-m', strtotime($detilSourceItems['last_updated'])) OR
                    isset($paymentSource[$nomer])&&$paymentSource[$nomer]['sisa']>1&&isset($_GET['force'])) {

                    //hanya untuk debug
                    if( isset($_GET['force']) ) {
                        cekMerah($namaP . ' DIPAKSA UNTUK BUNGA ');
                        cekMerah($noP . ' $noP ');
                        cekMerah($paymentSource[$nomer]['sisa'] . ' TOTAL PINJAMAN ');
                    ...
                    $itemsPersenBunga = $detilSourceItems['extern_value_2']*1;

                    $itemsGrandTotal = ($nilai_bunga-$nilai_pph23);
                    $totalSisa += $itemsNilaiSisa;
                    $grandtotal += $itemsGrandTotal;

//                        $grandtotalText .= $numb==count($detilSourceItemsTmp) ? "<div class='text-right'><u>". number_format($itemsGrandTotal) ."</u></div>" : "<div class='text-right'>". number_format($itemsGrandTotal) ."</div>";

                    $subtotal += $nilai_bunga;

//                        $totalSisaText .= $numb==count($detilSourceItemsTmp) ? "<div class='text-right'><u>". number_format($itemsNilaiSisa) ."</u></div>" : "<div class='text-right'>". number_format($itemsNilaiSisa) ."</div>";
//                        $totalPersenBungaText .= "<div class='text-right'>". number_format($itemsPersenBunga) ." (%)</div>";
                    ...
                    $arrItems[$extern_id]=  array(
                        ...
                        "nomer_top2" => trim($detilSourceItems['nomer_pinjaman']),
                        "hpp" =>$nilai_bunga,
                    ...
                    cekMerah(isset($paymentSource[$nomer]) . ' isset($paymentSource[$nomer]) ');
                    cekMerah($paymentSource[$nomer]['sisa'] . ' $paymentSource[$nomer][sisa]>1 ');
...
//                    "persen_bunga" => $totalPersenBungaText,
////                        "persen_bunga_text" => $detilSourceItems['extern_value_2']*1,
                        ...
                        if (isset($this->config->item('heTransaksi_core')[$this->jenisTr]['tableIn']['mainValues'])) {
                            foreach ($this->config->item('heTransaksi_core')[$this->jenisTr]['tableIn']['mainValues'] as $key => $src) {
                                if (isset($tableIn_master_values[$key])) {
                                    $tr->writeMainValues($insertID, array(
                                        "key" => $key,
...
//            }
        ...
        cekOrange("======================================= batas bawah LOAN (" .$this->jenisTr. ")=======================================");
        //endregion Loan Interest

        //===== ========== ========== ========== ========== ========== ========== =====//
        //========== ========== ========== ========== ========== ========== ==========//

        cekUngu("======================================= batas atas HUTANG PIHAK KE-TIGA (" .$this->jenisTr. ")=======================================");
