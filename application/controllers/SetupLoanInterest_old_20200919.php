...
class SetupLoanInterest_old_20200919 extends CI_Controller
{
    public function __construct()
    {
        ...
        $className = "MdlSetupLoanInterest";
        $ctrlName = "SetupLoanInterest";
...
//---init listed-fields
//        $className = "Mdl" . "SetupLoanInterest".$this->uri->segment(3);
...
//arrPrint($paymentSource);
        ...
        $ctrlName = "MdlSetupLoanInterest";
        $className = "SetupLoanInterest";
...
//                            $editTgl = "<br><a href='javascript:void(0)' class='text-red' onclick=\"\">edit</a>";
                    ...
                    elseif($alias=='loan_ages'){
                        $dValue = sizeof($Data->loan_ages) > 0 ? $Data->loan_ages : " <a href='javascript:void(0)' onclick=\"swal('".timeSinceV2(strtotime($transaction_date))."')\" class='link'>".timeSinceV2(strtotime($transaction_date))."</a>";
                    }
                        ...
                        $addClick = "BootstrapDialog.show(
                               {
                                    ...
                                    closable:true,
                                    buttons: [{
                                            icon: 'glyphicon glyphicon-reload',
                                            label: 'Request Loan Interest',
                                            //id: 'btnRefresh',
                                            ...
                                            action: function(dialogRef){
                                                dialogRef.close();
                                                Sweetalert2({
                                                  title: 'Anda Akan melakukan<br><r>Request Loan Interest</r>',
                                                  html: `<input id='nilai_persen' type='hidden' value='$nilai_persen'><input id='nilai_bunga' type='hidden' value='$nilai_bunga'><input id='nilai_dpp' type='hidden' value='$nilai_dpp'><div id='mainHtml'>Kreditur <span class='text-bold text-capitalize'>".$extern_nama."</span> senilai ".number_format($nilai_bunga)." (include pph23) <br> Apakah ingin melanjutkan??</div><div id='customHtml'><span class='btn btn-xs btn-info' onclick='customSweetalertInput()'>custom</span></div>`,
                                                  type: 'warning',
                                                  ...
                                                  preConfirm: (nomer) => {
                                                    var jml_hari = undefined != $('#jml_hari').val() && $('#jml_hari').val() > 0 ? $('#jml_hari').val() : 0;
                                                    var desc = undefined != $('#desc').val() && $('#desc').val() !=''? btoa( $('#desc').val() ) : 0;
                                                    var extVal = jml_hari*1 > 0 ? '&hari=' + jml_hari : '';
                                                    var extDesc = desc != '' ? '&desc=' + desc : '';
                                                    if( undefined != $('#jml_hari').val() && $('#jml_hari').val()==0){
                                                        Sweetalert2({
                                                            title: 'Anda tidak mengisi jumlah hari',
                                                            html: 'apa tetap melanjutkan?? Jika dilanjutkan hal ini akan membuat Bunga full 1 bulan',
                                                            type: 'warning',
                                                            showCancelButton: true,
                                                            confirmButtonText: 'Lanjutkan',
                                                            cancelButtonColor: '#d33',
                                                        }).then( (result)=>{
                                                            return fetch(`../../AutoLoanInterest?debuger=1&force&byexternid=$nomer&exe`)
                                                            .then(response => {
                                                                if (!response.ok) {
...
//                                                                setTimeout( function(){ $('#addClick_". $Data->transaksi_id ."').click() }, 200) ;
                                                        ...
                                                        return fetch(`../../AutoLoanInterest?debuger=1&force&byexternid=$nomer&exe`+extVal+extDesc)
                                                        .then(response => {
                                                            if (!response.ok) {
                                                                throw new Error(response.statusText)
                                                            }
//                                                            setTimeout( function(){ $('#addClick_". $Data->transaksi_id ."').click() }, 200) ;
                                                ...
                                                allowOutsideClick: false
                                                    ...
                                                    if (result) {
                                                        ...
                                                        Sweetalert2(
                                                            'Sukses!',
                                                            'Sukses Melakukan Request Loan Interest.',
                                                            'success'
                                                        )
                                                        dialogRef.close();
                                                        setTimeout( function(){ $('#addClick_". $Data->transaksi_id ."').click() }, 200) ;
...
//                    $existData[$Data->nomer][$alias] = isset($Data->$alias) ? $Data->$alias : ( isset($paymentSource[$Data->nomer][$alias]) ? $paymentSource[$Data->nomer][$alias] : "$dValue");
...
//        arrPrint($arrSettingActive);
    ...
    public function edit()
    {
        ...
        $className = "MdlSetupLoanInterest". $this->uri->segment(3);
        $ctrlName = "SetupLoanInterest";
...
//            matiHere("hoop ----DONE---- belom commit");
    ...
    public function editFrom()
    {
        ...
        //==menampilkan form pengubahan data berdasarkan datamodel (kelas data) dan id-nya yang bersesuaian
        $className = "Mdl" . "SetupLoanInterest";
        $ctrlName = "SetupLoanInterest";
...
//        cekMerah($this->db->last_query());
//
//        foreach($tmpHist as $k=>$dta){
//            arrPrint($dta->id);
//        }

        $tr = new MdlTransaksi();
        $tr->addFilter("transaksi.jenis=$jenisTr");
        $tr->addFilter("transaksi.step_current=1");
        $tr->addFilter("transaksi_data.produk_id='".$id."'");

        cekHere($jenisTr);
        cekHere($id);
        $pendingHist = $tr->lookupRecentUndoneEntries_joined($sesionReplacer)->result();

//        cekUngu($this->db->last_query());
//
//        cekHere($jnSlctd);
//
//        foreach($pendingHist as $k=>$dta){
//            arrPrintWebs($dta->id);
//        }
//
//        matiHere($id);
        ...
        $arrListPending = array();
        if(sizeof($pendingHist)>0){
            asort($pendingHist);
            foreach($pendingHist as $k=>$datas){
                $arrListPending[] = $datas;
            }
        }

        $arrHeader = array(
            "dtime" => "Tgl Loan Interest",
            "nomer" => "Nomer",
            "transaksi_nilai" => "Nilai Bunga (Include PPH23)",
...
//        arrPrintWebs($arrListDepre);
        ...
        if(sizeof($arrListDepre)>0){
            foreach($arrListDepre as $i=>$val){
                ...
                $table .= "<tr>";
                foreach($arrHeader as $ky=>$title){
                    $data_order = $ky=='nomer' ? "data-order='" . $i . "'" : "";
                    $table .= "<td $data_order >";
        ...
        $mulai2 = sizeof($arrListDepre);

        if(sizeof($arrListPending)>0){
            foreach($arrListPending as $i=>$val){
                if(isset($val->oleh_nama)){
                    $linkGoTo = base_url() . "Transaksi/index/4449";
                    $val->oleh_nama = "<span class='btn btn-xs btn-warning' title='klik untuk buka diTab baru' onclick=\"window.open('$linkGoTo', 'barangam')\">belum approve</span>";
                }
                $table .= "<tr>";
                foreach($arrHeader as $ky=>$title){

                    $data_order = $ky=='nomer' ? "data-order='" . ($i+$mulai2) . "'" : "";

                    $table .= "<td $data_order class='bg-warning'>";
                    $table .= formatField($ky, $val->$ky);
                    $table .= "</td>";
                }
                $table .= "</tr>";
