...
//        mati_disini("CILUKBAAA.... TESTING LAGI... HI HI HI  BELUM DICOMMIT");
    ...
    function foreverNew()
    {
        ...
        $dateTimeNow = "2026-01-01 00:05:11";// ini dimatikan

        $prevBl = previousYear();
        $prevBl = "2025";// ini dimatikan
        $dateLast_ex = explode("-", $prevBl);
        $periode = "forever";
        $tahun = $dateLast_ex[0];
//        $prevBl = $tahun = date("Y");
...
//            mati_disini("SETOP");
            ...
            // region pembantu biaya-biaya
            $arrPembantu = array(
                "4010" => "ComRekeningPembantuPenjualan",
                "4020" => "ComRekeningPembantuReturnPenjualan",
                "5010" => "ComRekeningPembantuHpp",
                "6010" => "ComRekeningPembantuBiayaUsaha",
                "6020" => "ComRekeningPembantuBiayaProduksi",
                "6030" => "ComRekeningPembantuBiayaUmum",
                "6050" => "ComRekeningPembantuBiayaGaji",
                "6080" => "ComRekeningPembantuBiayaBpjs",
                "6090" => "ComRekeningPembantuBiayaPph21",
                "6100010" => "ComRekeningPembantuBiaya",
                "7010150" => "ComRekeningPembantuLRLainlainDetail",
                "7010170" => "ComRekeningPembantuPendapatanItem",
            );
            $arrMainRekening = array(
                "ComRekeningPembantuPenjualan",
                "ComRekeningPembantuReturnPenjualan",
                "ComRekeningPembantuHpp",
                "ComRekeningPembantuBiayaGaji",
                "ComRekeningPembantuBiayaBpjs",
                "ComRekeningPembantuBiayaPph21",
            );
            foreach ($arrPembantu as $rek_pembantu => $comPembantu) {
                $this->load->model("Coms/$comPembantu");
                $rpp = new $comPembantu();
                $rpp->addFilter("cabang_id='$cabangID'");
                $rppTmp = $rpp->fetchBalances($rek_pembantu);
                showLast_query("biru");
                cekBiru(count($rppTmp));
                if (sizeof($rppTmp) > 0) {
                    $dataDetail = array();
                    foreach ($rppTmp as $ii => $rppSpec) {
                        $debet = $rppSpec->debet;
                        $kredit = $rppSpec->kredit;
                        $extern_id = $rppSpec->extern_id;
                        $extern_nama = $rppSpec->extern_nama;
                        if ($debet > 0) {
                            $nilai = detectRekByPosition($rek_pembantu, $debet, "debet");;
                        }
                        else {
                            $nilai = detectRekByPosition($rek_pembantu, $kredit, "kredit");
                        }
                        if ($nilai != 0) {
                            $nilai_balik = ($nilai * -1);
                            cekHere("[$rek_pembantu] [$extern_id] [$extern_nama] [$debet] [$kredit] [nilai: $nilai]");
                            if (in_array($comPembantu, $arrMainRekening)) {
                                $dataMain = array(
                                    "loop" => array(
                                        "$rek_pembantu" => $nilai_balik,
                                    ),
                                    "static" => array(
                                        "cabang_id" => $cabangID,
                                        "extern_id" => $extern_id,
                    ...
                    if (sizeof($dataDetail) > 0) {
                        arrPrint($dataDetail);
                        $rpp->pairForever($dataDetail);
                        $rpp->execForever();
                    }
                }
                else {
                    cekMerah("[$rek_pembantu] [$comPembantu] [KOSONG]");
                }
            }
            // endregion pembantu biaya-biaya
