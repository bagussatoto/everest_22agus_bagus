...
class ComRekeningPembantuPenjualanItem extends MdlMother
{
    ...
    private $periodeForever = array("forever");

    /**
     * @return array
     */
    public function getPeriodeForever()
    {
        return $this->periodeForever;
    }

    /**
     * @param array $periodeForever
     */
    public function setPeriodeForever($periodeForever)
    {
    ...
    {
        $this->tableName = "_rek_pembantu_penjualan_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_penjualan",
...
//                    $this->tableName = $table[$key]["cache"];
                        ...
                        $externID = (isset($array_params['static']['extern_id']) && ($array_params['static']['extern_id'] > 0)) ? $array_params['static']['extern_id'] : mati_disini(("unknown customerID*. Please contact system administrator."));
//                    $externID = (isset($array_params['static']['extern_id']) && ($array_params['static']['extern_id'] > 0)) ? $array_params['static']['extern_id'] : die(lgShowAlert("unknown customerID*. Please contact system administrator."));
                        $extern2ID = (isset($array_params['static']['extern2_id']) && ($array_params['static']['extern2_id'] > 0)) ? $array_params['static']['extern2_id'] : 0;
                        ...
                        if ($pakai_cache == 1) {
                            switch ($afterPosition) {
                                ...
                                case "debet":
                                    ...
                                    break;
                                default:
                                    matiHere(__LINE__ . " gagal menentukan posisi rekening DEBET / KREDIT " . __FUNCTION__ . " on file " . __FILE__);
                                    break;
                            }
                            switch ($position) {
                                case "kredit":
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
                                    break;
                                default:
                                    matiHere(__LINE__ . " gagal menentukan posisi rekening DEBET / KREDIT " . __FUNCTION__ . " " . __FILE__);
                                    break;
                            }
                                ...
                                case "forever":
                                    ...
                                    switch ($position) {
                                        ...
                                        case "kredit":
                                            ...
                                            break;
                                        default:
                                            matiHere("Transaksi gagal, karena rekening $key gagal menentukan posisi DEBET/KREDIT.");
                                            break;
                                    }
...
//                                    cekAlert("Gagal Memperbaharui data penjualan periode : $periode <span class='meta'> code : ".__LINE__." ".__METHOD__." #".date("Y-m-d H:i")."</span>");
//                                    return false;
//                                }
                            }
                            else {
...
//            matiHEre(__LINE__." empty return value");
            return false;
        }
    }

    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $rekening2_id, $fulldate)
    {
//        matiHEre(__LINE__);
        if (isset($fulldate)) {
        ...
        $this->addFilter("extern2_id='$rekening2_id'");
...
//        $rek = "kas";
    ...
    public function pairForever($inParams)
    {
        ...
        if (sizeof($this->inParams) > 0) {
            $lCounter = 0;
            foreach ($this->periodeForever as $periode) {
                foreach ($this->inParams as $array_params) {
                    $arrRekening = array();
                    foreach ($array_params['loop'] as $key => $value) {
                        $lCounter++;
