...
class ComRekeningPembantuCustomerProject extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_customer_project_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_customer_project",
        );
    }
        ...
        if (sizeof($this->inParams['loop']) > 0) {
            ...
            foreach ($this->periode as $periode) {
                ...
                foreach ($this->inParams['loop'] as $key => $value) {
                    ...
                    $externID = (isset($inParams['static']['extern_id']) && ($inParams['static']['extern_id'] != NULL)) ? $inParams['static']['extern_id'] : mati_disini(("$msg"));
...
//            arrPrintCyan($this->outParams);
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $pihak_id, $pihak2_id, $fulldate)
    {
        if (isset($fulldate)) {
