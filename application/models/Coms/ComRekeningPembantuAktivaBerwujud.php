...
class ComRekeningPembantuAktivaBerwujud extends MdlMother
{
    ...
    private $tableNames = array(
        "main" => "setup_depresiasi",
        "detail" => "_rek_pembantu_aktiva_berwujud_cache",
    );

    public function __construct()
    {
//        $this->tableName_fifoAvg = "fifo_avg";
        $this->tableName = "_rek_pembantu_aktiva_berwujud_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_aktiva_berwujud",
...
//                        $this->tableName = $table[$key]["cache"];
                        ...
                        $_preValues = $this->cekPreValue($key, $array_params['static']['cabang_id'], $periode, $array_params['static']['extern_id'], $array_params['static']['gudang_id'], $array_params['static']['fulldate']);
...
//        cekkuning($this->db->last_query());
    ...
    public function fetchBalancesAll($rek = "", $key = "", $sortBy = "", $sortMode = "ASC")
    {//==memanggil saldo2 dari rekening tertentu
//        $tableNames = heReturnTableName($this->tableName_master, array($rek));
        $this->db->select("*");
...
//        cekkuning($this->db->last_query());
    ...
    {
        $this->db->select("*,
            _rek_pembantu_aktiva_berwujud_cache.extern_id          as extern_id,
            _rek_pembantu_aktiva_berwujud_cache.extern_nama        as extern_nama,
            _rek_pembantu_aktiva_berwujud_cache.debet    as debet,
            _rek_pembantu_aktiva_berwujud_cache.cabang_id          as cabang_id");
        $criteria = array();
        $criteria2 = "";
        if (sizeof($this->filters) > 0) {
            $this->fetchCriteria();
        ...
        $this->db->join($this->tableNames['detail'], $this->tableNames['detail'] . ".extern_id = " . $this->tableNames['main'] . ".extern_id");
    ...
    public function lookupDepre()
    {
        ...
        $this->db->join(
            ...
            $this->tableNames['detail'],
            $this->tableNames['detail'] . ".extern_id = " . $this->tableNames['main'] . ".extern_id"
