...
class ComRekeningPembantuPpnProject extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_ppnprojek_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_ppnprojek",
        );
    ...
    public function pair($inParams)
    {
        ...
        $itemsMode = 0;// defaultnya adalah 1 karena masuk ke gerbang items
                    ...
                    foreach ($this->periode as $periode) {
                        ...
                        foreach ($this->inParams['loop'] as $key => $value) {
                            ...
                            $_preValues = $this->cekPreValue(
                                ...
                                $this->inParams['static']['cabang_id'],
                                $periode,
                                $this->inParams['static']['extern_id'],// project id
                                $this->inParams['static']['extern2_id']// konsumen id
                            );
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $extern2_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
...
//        $rek = "kas";
    ...
    public function fetchAllMove($rek)
    {
        $this->load->helper("he_mass_table");
        $tableNames = heReturnTableName($this->tableName_master, array($rek));
        $criteria = array();
