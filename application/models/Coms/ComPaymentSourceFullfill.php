...
class ComPaymentSourceFullfill extends MdlMother
{
...
//             mati_disini("ggg");
            if ($_preValue != null) {
                ...
                elseif (isset($this->inParams['static']['transaksi_fullfillment'])) {
                    //area fullfilment
                    $this->outParams[$lCounter]["sisa"] = ($_preValue["sisa"] - $this->inParams['static']['transaksi_fullfillment']);
                    $this->outParams[$lCounter]["returned"] = $_preValue['returned'] + $this->inParams['static']['transaksi_fullfillment'];
                    ...
                    $this->outParams[$lCounter]["cancel_dtime"] = $this->inParams['static']['fulldate'];
                    $this->outParams[$lCounter]["cancel_id"] = $this->inParams['static']['oleh_id'];
                    $this->outParams[$lCounter]["cancel_name"] = $this->inParams['static']['oleh_nama'];
                    $this->outParams[$lCounter]["payment_source_keterangan"] = $this->inParams['static']['keterangan'];
...
//                    }
                ...
                //jika tidka ada paymentsource skip aja
                return true;
...
//                $this->writeMode = "new";
    ...
    private function cekPreValue($cabang_id, $extern_id, $transaksiID = 0)
    {

//        matiHere($jenis ."::".$extern_id."::" .$label ."::".$transaksiID);
        ...
        if (sizeof($this->outParams) > 0) {
            foreach ($this->outParams as $ctr => $params) {
                ...
                switch ($this->writeMode) {
                    case "new":
//                        matiHere("Error detected on fulfillment, plea");
