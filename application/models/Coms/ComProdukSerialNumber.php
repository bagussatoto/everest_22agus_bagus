        ...
        if (sizeof($this->inParams) > 0) {
            foreach ($this->inParams as $ix => $array_params) {
                if (isset($array_params['static']["rejection"]) && $array_params['static']["rejection"] == 1) {
                    if (isset($array_params['static']["returns"]) && $array_params['static']["returns"] == 1) {
                        // serial sudah dikeluarkan saat pembatalan GRN (custom serial)
                        cekMerah("TIDAK ADA SERIAL YANG DIKELUARKAN");
                    }
                    else {
...
//                        $date_po = isset($array_params['static']['transaksi_reference_fulldate']) ? $array_params['static']['transaksi_reference_fulldate'] : $array_params['static']['fulldate'];
                        ...
                        $date_po = str_replace("-", "", $date_po);
                        $part_kode = isset($array_params['static']['part_keterangan']) && strlen($array_params['static']['part_keterangan']) > 1 ? $array_params['static']['part_keterangan'] : "";
...
//matiHere($array_params['static']['transaksi_reference_fulldate'].">".$serial_generate);
                        ...
                        $this->outParams[$lCounter]["count"] = $count_new;
                        $this->outParams[$lCounter]["produk_serial_number_2"] = $serial_generate;
                        $this->outParams[$lCounter]["produk_sku_label"] = isset($array_params['static']['part_keterangan']) ? $array_params['static']['part_keterangan'] : "";
...
//                                    break;
                            ...
                            //ada perubahan karena serial cukup generate aja, pengakuan saat grn ke rekening
                            $pakai_ini = 0;
                            if(isset($array_params['static']["exec_cache"]) && $array_params['static']["exec_cache"]==1){
                                /**
                                 * untuk yang hanya memiliki satu langkah ERP langsung tulis ke rekening serial
                                 * contoh realisasi kompensasi supplier
                                 */
                                $pakai_ini=1;
                            }
                            if ($pakai_ini == 1) {
...
//        $nilai = $tmp['qty_debet'];
    ...
    private function cekPreValueCount($cabang_id, $produk_id, $gudang_id)
    {
