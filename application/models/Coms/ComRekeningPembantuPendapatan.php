...
class ComRekeningPembantuPendapatan extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_subpendapatan_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_subpendapatan",
        );
...
//                    $this->tableName = $table[$key]["cache"];
                            ...
                            $externID = (isset($arrayParams['static']['extern_id']) && ($arrayParams['static']['extern_id'] > 0)) ? $arrayParams['static']['extern_id'] : die(lgShowAlert("unknown cashID*. Please contact system administrator."));

                            //  region cek saldo awal
                            $_preValues = $this->cekPreValue($key, $arrayParams['static']['cabang_id'], $periode, $externID);
...
//                    $afterNumber = round($preNumber + $value);
                            ...
                            if (in_array($key, $configBalanceProtections)) {
                                if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                                    die(lgShowAlert("insufficient balance for $key " . $arrayParams['static']['extern_nama'] . ""));
                                }
                            }
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["rek_id"] = createRekCode($key, $arrayParams['static']['extern_id']);
                            ...
                            if ($pakai_mutasi == 1) {
                                switch ($periode) {
                                    case "forever":
                                        ...
                                        $this->outParams[$lCounter]["mutasi"]["rek_id"] = createRekCode($key, $arrayParams['static']['extern_id']);
