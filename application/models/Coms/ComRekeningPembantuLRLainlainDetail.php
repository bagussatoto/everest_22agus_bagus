...
class ComRekeningPembantuLRLainlainDetail extends MdlMother
{
    ...
    protected $inParams = array( //===inputan dari transaksi

    );
    protected $outParams = array( //===output ke tabel

    );
    protected $outFields = array( // dari tabel cache
        "rekening",
        "periode",
        "cabang_id",
        "cabang_nama",
    ...
    protected $outFieldsMutasi = array( // dari tabel rek mutasi rekening
        "transaksi_id",
        "transaksi_no",
        "transaksi_jenis",
        "cabang_id",
    ...
    protected $periode = array("harian", "bulanan", "tahunan", "forever");
    protected $catException = array();
    ...
    {
        $this->tableName = "_rek_pembantu_lr_lainlain_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_lr_lainlain",
        );
            ...
            if (sizeof($this->inParams['loop']) > 0) {
                ...
                foreach ($this->periode as $periode) {
                    ...
                    foreach ($this->inParams['loop'] as $key => $value) {
                        ...
                        $_preValues = $this->cekPreValue($key, $this->inParams['static']['cabang_id'], $periode, $this->inParams['static']['extern_id'], $this->inParams['static']['extern2_id'], $this->inParams['static']['fulldate']);
                        ...
                        if (array_key_exists("id", $_preValues["cache"]) && ($_preValues["cache"]["id"] > 0)) {
                            ...
                            $mode = "insert";
                            $_preValues_id = 0;
                            if (isset($this->inParams['static']['fulldate'])) {
                                $fulldate_ex = explode("-", $this->inParams['static']['fulldate']);
                                $tgl = $fulldate_ex[2];
                                $bln = $fulldate_ex[1];
                                $thn = $fulldate_ex[0];
                            }
                                ...
                                case "forever":
                                    ...
                                    switch ($position) {
                                        case "debet":
                                            ...
                                            //                                        $urut_debet = $cc->setCounterRekening($key, 0, $this->inParams['static']['cabang_id'], $position);
                                            //                                        $urut_kredit = 0;
                                        ...
                                        case "kredit":
                                            //                                        $cc = New CustomCounter();
                                            //                                        $urut_debet = 0;
                                            //                                        $urut_kredit = $cc->setCounterRekening($key, 0, $this->inParams['static']['cabang_id'], $position);
                                    ...
                                    //                                $cc = New CustomCounter();
                                    //                                $this->outParams[$lCounter]["mutasi"]["urut"] = $cc->setCounterRekening($key, 0, $this->inParams['static']['cabang_id'], "0");
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $pihak_id, $extern2_id, $fulldate)
    {
        if (isset($fulldate)) {
