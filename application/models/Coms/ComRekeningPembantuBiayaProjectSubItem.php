...
class ComRekeningPembantuBiayaProjectSubItem extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_subbiayaproject_detail_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_subbiayaproject_detail",
        );
                ...
                if (sizeof($this->inParams['loop']) > 0) {
                    foreach ($this->periode as $periode) {
                        ...
                        foreach ($this->inParams['loop'] as $key => $value) {
                            ...
                            $extern2_id = isset($this->inParams['static']['extern2_id']) ? $this->inParams['static']['extern2_id'] : 0;
                            $extern3_id = isset($this->inParams['static']['extern3_id']) ? $this->inParams['static']['extern3_id'] : 0;
                            if ($extern2_id == 0) {
                                $msg = "Detail Biaya tidak dikenali, silahkan cek kembali transaski anda atau hubungi admin. code: " . __LINE__;
                                mati_disini($msg);
                            }
                            ...
                            $_preValues = $this->cekPreValue(
                                ...
                                $this->inParams['static']['extern3_id'],
                                $this->inParams['static']['fulldate']
                            );
                            ...
                            if (array_key_exists("id", $_preValues["cache"]) && ($_preValues["cache"]["id"] > 0)) {
                                ...
                                $mode = "insert";
                                $_preValues_id = 0;
                                $date_ex = explode("-", $this->inParams['static']['fulldate']);
                                $tgl = $date_ex[2];
                                $bln = $date_ex[1];
                                $thn = $date_ex[0];
                                $this->outParams[$lCounter]["cache"][$mode]["tgl"] = (sizeof($date_ex) > 0) ? $tgl : date("d");
                                $this->outParams[$lCounter]["cache"][$mode]["bln"] = (sizeof($date_ex) > 0) ? $bln : date("m");
                                $this->outParams[$lCounter]["cache"][$mode]["thn"] = (sizeof($date_ex) > 0) ? $thn : date("Y");
                            }
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
                            ...
                            if ($pakai_exec == 1) {
                                ...
                                if (sizeof($this->outParams) > 0) {
                                    cekMerah("outParams biaya usaha");
...
//                            arrPrint($pSpec_mode);
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $rekening2_id, $rekening3_id, $date = NULL)
    {
        if ($date != NULL) {
        ...
        $this->addFilter("extern3_id='$rekening3_id'");
