...
class ComRekeningPembantuProduk extends MdlMother
{
        ...
        if (sizeof($this->inParams) > 0) {
            ...
            foreach ($this->periode as $periode) {
                foreach ($this->inParams as $array_params) {
                    if (isset($array_params["static"]["kategori_id"]) && $array_params["static"]["kategori_id"] == "4") {
//                        matiHere(__LINE__);
                        $insertIDs[] = 1;
                    }
                    else {
                            ...
                            if ($rejection == 1) {
                                ...
                                if ($pakai_average == 1) {
                                    ...
                                    $f_update = array(
                                        "jml" => $afterQtyNumber,
                                        "hpp" => $afterNumberAvg,
                                        "jml_nilai" => $afterNumber,
                                    );
                                    $f_where = array(
                                        "produk_id" => $array_params['static']['extern_id'],
                                        "jenis" => "produk",
                                        "cabang_id" => $cabangID,
                                        "gudang_id" => $gudangID,
                                    );
                                    $f->updateData($f_where, $f_update);
                                }
                            }
...
//                $on     = isset($j[1]) ? $j[1] : '';
    ...
    public function getSerialCountMap($prod, $rek, $opts)
    {
        ...
        $supplierGudangIDs = array();
        if (isset($opts['supplier_gudang_ids']) && is_array($opts['supplier_gudang_ids'])) {
            foreach ($opts['supplier_gudang_ids'] as $sid) {
                $sid = (int)$sid;
                if ($sid > 0) {
                    $supplierGudangIDs[$sid] = 1;
                }
            }
        }
        ...
        foreach ($rows as $r) {
            ...
            if (isset($supplierGudangIDs[$gid])) {
                $serialKey = "gd_qty_" . $gid;
            } elseif ($gid > 0 && $gid < 1000) {
                $serialKey = "ng_qty_debet";
            } elseif ($gid < 0) {
                $serialKey = "qty_debet";
            } else {
                $serialKey = "wo_qty_debet";
            }

            if (!isset($map[$serialKey][$pid])) {
                $map[$serialKey][$pid] = array();
            }
            if (!isset($map[$serialKey][$pid]['jml_serial'])) {
                $map[$serialKey][$pid]['jml_serial'] = 0;
            }

            $map[$serialKey][$pid]['jml_serial'] += $cnt;
            $map[$serialKey][$pid]['gudang_id'] = $gid;
            $map[$serialKey][$pid]['cabang_id'] = $opts['cabang_id'];
        }
    ...
    public function getSerialIntransitCountMap($prod, $rek, $opts)
    {
        ...
        foreach ($rows as $r) {
            ...
            if (!isset($map[$serialKey][$pid]['jml_serial_transit'])) {
                $map[$serialKey][$pid]['jml_serial_transit'] = 0;
            }

            $map[$serialKey][$pid]['jml_serial_transit'] += $cnt;
            $map[$serialKey][$pid]['gudang_id'] = $gid;
            $map[$serialKey][$pid]['cabang_id'] = $opts['cabang_id'];
