...
class ComPaymentSourceFakturItems extends MdlMother
{
...
//        matiHere(__LINE__);
        ...
        foreach ($inParams as $this->inParams) {
            ...
            if (sizeof($this->inParams['static']) > 0) {
                ...
                $ppn_sudah_faktur = isset($this->inParams['static']['ppn_sudah_faktur']) ? $this->inParams['static']['ppn_sudah_faktur'] : 0;
                ...
                $is_predefined_faktur =isset($this->inParams['static']['is_predefined_faktur']) ? $this->inParams['static']['is_predefined_faktur'] : 0;
                if($is_predefined_faktur){
                    //tidak perlu nulis lagi sudah dientry dahulu fakturny AP hanya bayar hutang pajak saja
                    //cekjika ada selisih
                    $pajak_asli = $this->inParams['static']['ppn_final_asli'];
                    $koreksi_nilai = ($pajak_asli - $ppn_sudah_faktur)*-1;
                    $update = array(
                        "returned"=> $koreksi_nilai > 0 ? $koreksi_nilai: $koreksi_nilai*-1,
                        "sisa"=>$pajak_asli+$koreksi_nilai,
                        "ppn"=>$this->inParams['static']['ppn'],
                        "ppn_sisa"=>$this->inParams['static']['ppn_sisa'],
                        "extern_nilai2"=>$this->inParams['static']['extern_nilai2'],
                    );
                    $where = array(
...
//        }
    ...
    private function cekPreValue($jenis, $cabang_id, $extern_id, $label, $transaksiID = 0, $nofaktur)
    {
