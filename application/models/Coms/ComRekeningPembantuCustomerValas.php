...
class ComRekeningPembantuCustomerValas extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_customer_valas_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_customer_valas",
//            "cache" => "_rek_pembantu_customer_cache",
        );
        $this->rekeningAlias = fetchAccountStructureAlias();
    }
...
//                    $this->tableName = $table[$key]["cache"];
                    ...
                    $qty = isset($inParams['static']['qty']) ? $inParams['static']['qty'] : 0;
                    arrPrint($_preValues);
//cekHitam("$qty *&*&^%$");
//arrPrint($_preValues);
                    ...
                    if ($_preValues['cache']['debet'] > 0) {
                        ...
                        $preQty = detectRekByPosition($key, $_preValues['cache']['qty_debet'], "debet");
                    }
                    else {
                        $preNumber = detectRekByPosition($key, $_preValues['cache']['kredit'], "kredit");
                        $preQty = detectRekByPosition($key, $_preValues['cache']['qty_kredit'], "kredit");
                    }

//                    if ($_preValues['cache']['qty_kredit'] > 0) {
//                      $preQty = detectRekByPosition($key, $_preValues['cache']['qty_kredit'], "kredit");
//
//                    }
//                    else {
...
//                    $afterPositionQty = detectRekPosition($key, $afterqty);

                    $afterPosition = detectRekPosition($key, floor($afterNumber));
                    $afterPositionQty = detectRekPosition($key, floor($afterqty));
...
//                        if ($afterPositionQty != detectRekDefaultPosition($key) && ($afterPositionQty != 0)) {
                        if ($afterPositionQty != detectRekDefaultPosition($key) && (round($afterPositionQty) != 0)) {
                            die(lgShowAlert("insufficient balance for $key. <br>You requested $qty.= <br>Available: $preQty. <br>Post-value should be: $afterqty"));
                        }
                    }
                    ...
                    if ($pakai_cache == 1) {
                        ...
                        switch ($afterPosition) {
                            case "kredit":
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["qty_kredit"] = abs($afterqty);
                            ...
                            case "debet":
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["qty_debet"] = abs($afterqty);
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_debet_periode"] = 0;
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_kredit"] = $_preValues["cache"]["saldo_qty_kredit"] + abs($qty);
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_debet"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_kredit_periode"] = $_preValues["cache"]["saldo_qty_kredit_periode"] + abs($qty);
...
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = 0;
                                ...
                                $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_debet"] = $_preValues["cache"]["saldo_qty_debet"] + abs($qty);
//                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_kredit"] = 0;
                                $this->outParams[$lCounter]["cache"][$mode]["saldo_qty_debet_periode"] = $_preValues["cache"]["saldo_qty_debet_periode"] + abs($qty);
                            ...
                            case "forever":
                                switch ($afterPosition) {
                                    ...
                                    case "debet":
                                        ...
                                        $this->outParams[$lCounter]["mutasi"]["qty_debet_awal"] = $_preValues["cache"]["qty_debet"];
                                        $this->outParams[$lCounter]["mutasi"]["qty_debet_akhir"] = abs($afterqty);

                                        $this->outParams[$lCounter]["mutasi"]["kredit_awal"] = $_preValues["cache"]["qty_kredit"];
                                        $this->outParams[$lCounter]["mutasi"]["kredit_akhir"] = 0;
                                ...
                                switch ($position) {
                                    case "debet":
                                        ...
                                        $this->outParams[$lCounter]["mutasi"]["qty_debet"] = abs($qty);
                                    ...
                                    case "kredit":
                                        ...
                                        $this->outParams[$lCounter]["mutasi"]["qty_kredit"] = abs($qty);
...
//                            unset($pSpec_mode["tabel"]);
                            ...
                            // Debug print dimatikan agar output halaman/API tetap bersih.
                            // arrPrint($pSpec_mode);
...
//            matiHere("untuk cek qty ".__LINE__." ".__FILE__);
