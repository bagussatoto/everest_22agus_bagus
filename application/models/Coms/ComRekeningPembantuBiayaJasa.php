...
class ComRekeningPembantuBiayaJasa extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_subbiaya_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_subbiaya",
        );
                ...
                if (sizeof($this->inParams['loop']) > 0) {
                    foreach ($this->periode as $periode) {
                        ...
                        foreach ($this->inParams['loop'] as $key => $value) {
                            ...
                            $date = array(
                                "tgl" => $this->inParams['static']['tgl'],
                                "bln" => $this->inParams['static']['bln'],
                                "thn" => $this->inParams['static']['thn'],
                            );
                            $_preValues = $this->cekPreValue($key, $this->inParams['static']['cabang_id'], $periode, $this->inParams['static']['extern_id'], $date);
...
// cekHitam(":: run ke $lCounter ::");
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $rekening_id, $date = "")
    {
        if ($date != NULL) {
