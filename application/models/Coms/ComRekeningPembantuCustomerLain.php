...
class ComRekeningPembantuCustomerLain extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_customer_lain_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_customer_lain",
        );
    }
...
//                    $this->tableName = $table[$key]["cache"];
                    ...
                    $msg = "Silahkan memilih dahulu nama konsumen sebelum melanjutkan transaksi.";
//                    $externID = (isset($inParams['static']['extern_id']) && ($inParams['static']['extern_id'] > 0)) ? $inParams['static']['extern_id'] : die(lgShowAlert("$msg"));
                    $externID = (isset($inParams['static']['extern_id']) && ($inParams['static']['extern_id'] != NULL)) ? $inParams['static']['extern_id'] : die(lgShowAlert("$msg"));
                    ...
                    if (in_array($key, $configBalanceProtections)) {
                        if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                            $selisih = ($afterNumber < 0) ? ($afterNumber * -1) : $afterNumber;
                            if ($selisih > 3) {

                                $pName = $inParams['static']['extern_nama'];
                                $pId = $inParams['static']['extern_id'];
//                            die(lgShowAlert("Saldo rekening $key $pName tidak cukup. Anda membutuhkan $value, tersedia $preNumber. Silahkan menghubungi admin."));
                                mati_disini(("Saldo rekening $key $pName tidak cukup. Anda membutuhkan $value, tersedia $preNumber. Silahkan menghubungi admin."));
                            }
                        }
                    }
...
//                                    mati_disini(__LINE__ . " terjadi kesalahan pada saldo piutang customers, saldo piutang bernilai minus. " . __CLASS__ . " :: " . __FUNCTION__);
//                                }
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $pihak_id, $fulldate)
    {
        if (isset($fulldate)) {
...
//        cekkuning($this->db->last_query());
    ...
    public function fetchBalancesCustomer($customerId = "")
    {
        $rekenings = [
            '1010020010' => 'debet_reguler',     // reguler (legacy & hirarki)
            '1010020080' => 'debet_project',     // project (hirarki baru)
            '1010020090' => 'debet_marketplace', // marketplace (legacy)
        ];

        $this->db->select("
            extern_id,
            cabang_id,
            SUM(CASE WHEN rekening = '1010020010' THEN debet ELSE 0 END) AS debet_reguler,
            SUM(CASE WHEN rekening = '1010020080' THEN debet ELSE 0 END) AS debet_project,
            SUM(CASE WHEN rekening = '1010020090' THEN debet ELSE 0 END) AS debet_marketplace,
            SUM(CASE WHEN rekening IN ('1010020010','1010020090') THEN debet ELSE 0 END) AS debet_total,
            SUM(CASE WHEN rekening IN ('1010020010','1010020080') THEN debet ELSE 0 END) AS debet_total_hirarki,
            SUM(CASE WHEN rekening IN ('1010020010','1010020080','1010020090') THEN debet ELSE 0 END) AS debet_total_all
        ");
        $this->db->from($this->tableName);
        $this->db->where_in('rekening', array_keys($rekenings));
        ...
        $this->db->group_by('extern_id,cabang_id');
        ...
        foreach ($results as $row) {
            ...
            if (!isset($grouped[$extId])) {
                $grouped[$extId] = [
                    'extern_id' => $extId,
                    'cabangs' => [],
                    'debet_reguler_total' => 0,
                    'debet_project_total' => 0,
                    'debet_marketplace_total' => 0,
                    'debet_total' => 0,
                    'debet_total_hirarki' => 0,
                    'debet_total_all' => 0,
                ];
            }

            // simpan detail per cabang
            $grouped[$extId]['cabangs'][] = [
                'cabang_id' => $row->cabang_id,
                'debet_reguler' => (float)$row->debet_reguler,
                'debet_project' => (float)$row->debet_project,
                'debet_marketplace' => (float)$row->debet_marketplace,
                'debet_total' => (float)$row->debet_total,
                'debet_total_hirarki' => (float)$row->debet_total_hirarki,
                'debet_total_all' => (float)$row->debet_total_all,
            ];

            // akumulasi total per extern_id
            $grouped[$extId]['debet_reguler_total'] += (float)$row->debet_reguler;
            $grouped[$extId]['debet_project_total'] += (float)$row->debet_project;
            $grouped[$extId]['debet_marketplace_total'] += (float)$row->debet_marketplace;
            $grouped[$extId]['debet_total'] += (float)$row->debet_total;
            $grouped[$extId]['debet_total_hirarki'] += (float)$row->debet_total_hirarki;
            $grouped[$extId]['debet_total_all'] += (float)$row->debet_total_all;
        }

        // kalau mau reset key array jadi urut 0,1,2
