...
class ComRekeningPembantuKasItem extends MdlMother
{
            ...
            foreach ($this->periode as $periode) {
                foreach ($this->inParams as $array_params) {
                    ...
                    foreach ($array_params['loop'] as $key => $value) {
                        ...
                        $_preValues = $this->cekPreValue($key, $array_params['static']['cabang_id'], $periode, $externID);
...
//                                die(lgShowAlert("insufficient balance for $key " . $array_params['static']['extern_nama'] . ""));
                                ...
                                if ($afterNumber > 1) {
                                    ...
                                    matiHEre("saldo tidak cukup untuk akun $key_alias " . $array_params['static']['extern_nama'] . ". Silahkan cek mutasi $key_alias " . $array_params['static']['extern_nama']);
                                }
                            }
                        }
                        ...
                        if ($pakai_cache == 1) {
                            switch ($afterPosition) {
                                case "kredit":
                                    ...
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit"] = $_preValues["cache"]["saldo_kredit"] + abs($afterNumber);
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_debet"] = 0;
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit_periode"] = $_preValues["cache"]["saldo_kredit_periode"] + abs($afterNumber);
                                ...
                                case "debet":
                                    ...
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_debet"] = $_preValues["cache"]["saldo_debet"] + abs($afterNumber);
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_kredit"] = 0;
                                    //                                    $this->outParams[$lCounter]["cache"][$mode]["saldo_debet_periode"] = $_preValues["cache"]["saldo_debet_periode"] + abs($afterNumber);
    ...
    public function fetchAllMoves($rek)
    {//==memanggil saldo2 dari rekening tertentu
        $tableNames = heReturnTableName($this->tableName_master, array($rek));
        $this->db->select("*");
        //        $this->db->where(array("extern_id" => $externID));
