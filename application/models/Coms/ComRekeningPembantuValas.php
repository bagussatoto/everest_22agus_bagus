...
class ComRekeningPembantuValas extends MdlMother
{
        ...
        if (sizeof($tmp) > 0) {
            ...
            foreach ($tmp as $row) {
                $result["cache"] = array(
                    ...
                    "saldo_qty_debet" => $row->saldo_debet,
                    "saldo_qty_kredit" => $row->saldo_kredit,
                    "saldo_qty_debet_periode" => $row->saldo_debet_periode,
                    "saldo_qty_kredit_periode" => $row->saldo_kredit_periode,
                );
            }
        }
        else {
