...
class ComRekeningPembantuKas extends MdlMother
{
...
//                    $cek_value = reformatExponent($value);
//                    $cek_value = ($cek_value < 0) ? ($cek_value * -1) : $cek_value;
//                    cekHere("[cek_value: $cek_value]");
//                    if($cek_value < 1){
//                        return true;
//                    }
                    ...
                    $msg = "Silahkan memilih sumber pembayaran sebelum melanjutkan transaksi. line: " . __LINE__;
                    $externID = (isset($this->inParams['static']['extern_id']) && ($this->inParams['static']['extern_id'] > 0)) ? $this->inParams['static']['extern_id'] : die(lgShowAlert("$msg"));
...
//                            die(lgShowAlert("Saldo rekening $key " . $this->inParams['static']['extern_nama'] . " tidak cukup. Silahkan menghubungi admin."));
...
//                $on     = isset($j[1]) ? $j[1] : '';
    ...
    protected function applyCommonFilters($db, $opts)
    {
        ...
        if (isset($opts['cabang_id']) && $opts['cabang_id'] !== null) {
            $db->where('s.cabang_id', (int)$opts['cabang_id']);
        }
