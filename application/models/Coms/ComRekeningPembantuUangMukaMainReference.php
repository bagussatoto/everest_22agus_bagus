...
class ComRekeningPembantuUangMukaMainReference extends MdlMother
{
    ...
    {
        $this->tableName = "_rek_pembantu_uang_muka_reference_cache";
        $this->tableName_master = array(
            "mutasi" => "_rek_pembantu_uang_muka_reference",
        );
                    ...
                    if (sizeof($this->inParams['loop']) > 0) {
                        foreach ($this->periode as $periode) {
                            ...
                            foreach ($this->inParams['loop'] as $key => $value) {
                                ...
                                $cabangID = isset($this->inParams['static']['cabang_id']) ? $this->inParams['static']['cabang_id'] : 0;
                                ...
                                if (in_array($key, $configBalanceProtections)) {
                                    if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                                        $afterNumber = reformatExponent($afterNumber);
                                        $afterNumber = ($afterNumber < 0) ? ($afterNumber * -1) : $afterNumber;
                                        if ($afterNumber > 1) {
                                            $this->rekeningAlias = fetchAccountStructureAlias();
                                            $key_alias = isset($this->rekeningAlias[$key]) ? $this->rekeningAlias[$key] : $key;
                                            matiHEre("saldo tidak cukup untuk akun $key_alias " . $this->inParams['static']['extern_nama'] . ". Silahkan cek mutasi $key_alias " . $this->inParams['static']['extern_nama']);
                                        }
                                    }
                                }
                ...
                if (sizeof($this->inParams['loop']) > 0) {
                    foreach ($this->periode as $periode) {
                        ...
                        foreach ($this->inParams['loop'] as $key => $value) {
                            ...
                            $table = heReturnTableName($this->tableName_master, $arrRekening);
                            $this->tableName_mutasi = $table[$key]["mutasi"];
                            $extern_id = $this->inParams['static']['extern_id'];
                            $extern_nama = $this->inParams['static']['extern_nama'];
                            $extern2_id = $this->inParams['static']['extern2_id'];
                            $extern2_nama = $this->inParams['static']['extern2_nama'];
                            $cabang_id = $this->inParams['static']['cabang_id'];
...
//                            $afterPosition = "kredit";
                            if (in_array($key, $configBalanceProtections)) {
                                if ($afterPosition != detectRekDefaultPosition($key) && ($afterNumber != 0)) {
                                    ...
                                    if ($afterNumber > 1) {
                                        $this->rekeningAlias = fetchAccountStructureAlias();
                                        $key_alias = isset($this->rekeningAlias[$key]) ? $this->rekeningAlias[$key] : $key;
                                        $text = "$key_alias $extern_nama ($extern2_nama)";
                                        $text_blob = blobEncode($text);
                                        $link = base_url() . "Ledger/viewMoveDetails/RekeningPembantuUangMukaReference/$key/$extern_id/?o=$cabang_id&main_ext2_id=$extern_id&blob_ext=$text_blob&ext2_id=$extern2_id";
                                        mati_disini_linkmutasi("saldo tidak cukup untuk akun $key_alias " . $this->inParams['static']['extern_nama'] . ". Silahkan cek mutasi $key_alias " . $this->inParams['static']['extern_nama'], $link);
                                    }
                                }
                            }
    ...
    private function cekPreValue($rek, $cabang_id, $periode, $pihak_id, $pihak2_id)
    {
        $tgl = date("d");
        $bln = date("m");
        $thn = date("Y");
