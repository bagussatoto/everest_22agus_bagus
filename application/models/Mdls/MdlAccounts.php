...
//                "label" => "rugi laba",
    ...
    public function addExtern_coa($head_code, $datas)
    {
        ...
        !isset($datas['extern_id']) ? matiHere("extern_id empty") : "";
        !isset($datas['extern_jenis']) ? matiHere("extern_jenis empty") : "";
...
//        showLast_query("biru");
    ...
    //----------------------------------
    public function lookupAccountMetaByHeadCode($headCode)
    {
        return $this->db
            ->select("head_code, head_name, is_active, is_transaction")
            ->from($this->tableName)
            ->where("head_code", (string) $headCode)
            ->limit(1)
            ->get()
            ->row();
    }
