...
class MdlNotaItem extends MdlMother
{
    ...
    public function getForSelector($rule = array(), $cabang_id, $gudang_id = null)
    {
        $keyword = isset($rule['keyword']) ? trim($rule['keyword']) : '';
        $limit   = isset($rule['limit']) ? (int)$rule['limit'] : 20;
        $this->db->select('p.id, p.nomer, p.dtime, p.status, p.trash');
        $this->db->from('transaksi p');
        $this->db->where('p.returned', 0);
        $this->db->where('p.trash', 0);
        $this->db->where('p.status', 1);
        $this->db->where('p.trash_4 <>', 1);

        /* ===== FILTER CABANG ===== */
        // Jika tabel memiliki kolom cabang_id, tambahkan filter ini
        if($cabang_id){
            $this->db->where('p.cabang_id', $cabang_id);
        }

        /* ===== FILTER CUSTOMER DAN JENIS ===== */
        if(!empty($rule['customers_id'])){
            $this->db->where('p.customers_id', $rule['customers_id']);
        }
        // Hanya tambahkan filter jenis jika disediakan
        if (!empty($rule['tr_jenis'])) {
            $this->db->where('p.jenis', $rule['tr_jenis']);
        }
        /* ===== PENCARIAN KEYWORD ===== */
        if($keyword !== ''){
            $this->db->group_start()
            ->like('p.nomer', $keyword)
            ->or_like('p.keterangan', $keyword) // tambahkan field lain jika perlu
            ->group_end();
        }

        /* ===== ORDERING ===== */
        $this->db->order_by('p.dtime', 'DESC'); // atau 'p.nomer', 'DESC'
        $this->db->limit($limit);
